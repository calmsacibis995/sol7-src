...
#pragma ident	"@(#)savecore.c	1.58	97/10/13 SMI"	/* UCB 5.8 5/26/86 */
 ...
 */
struct core_data {
	long	nglbs;		/* number of global symbols */
	long	nlcls;		/* number of local symbols */
	size_t	sym_size;	/* size of symbol table */
	size_t	str_size;	/* size of string table */
};
...
int	save_core(void);
long	save_pages(int, int, char *, long);
#ifdef	SAVESWAP
void	save_swap(int);
#endif	/* SAVESWAP */
...
void	Kvm_read(kvm_t *, uintptr_t, char *, size_t);
int	Open(char *, int);
...
long	dumpsize;			/* amount of memory dumped in pages */
long	saved; 				/* pages saved in corefile so far */
...
long	pagesize;			/* dump pagesize */
int	fflag;
struct utsname utsname;
int	damnit;

int	dump_printp = 512;		/* Print saved pages after every N */
...
#ifdef _LP64
			(void) sprintf(kernname,
			    "/platform/%s/kernel/sparcv9/unix", sysbuf);
#else
			(void) sprintf(kernname,
			    "/platform/%s/kernel/unix", sysbuf);
#endif
			if (access(kernname, R_OK) != 0) {
				if (sysinfo(SI_MACHINE, sysbuf,
				    sizeof (sysbuf)) == -1) {
					(void) fprintf(stderr, "%s: could not "
					    "get machine name.\n", progname);
					return (1);
				}
#ifdef _LP64
...
read_dumphdr(void)
{
	...
	(void) Lseek(dfd, (off_t)(-pagesize), SEEK_END);
	(void) Read(dfd, dhp1, sizeof (*dhp1)); /* read the header */
	...
	(void) Lseek(dfd, (off_t)(-(dhp1->dump_dumpsize)), SEEK_END);
	(void) Read(dfd, dhp, dhp1->dump_headersize);

	if (memcmp(dhp, dhp1, sizeof (*dhp)) && !damnit)
		(void) fprintf(stderr, "memcmp failed.\n");
...
good_dumphdr(struct dumphdr *dhp)
{
...
#if defined(sparc)
		switch (dhp->dump_magic) {
		case DUMP_MAGIC_32:
			fprintf(stderr, "dump appears to be from a "
			    "32-bit kernel - cannot save on 64-bit kernel\n");
			break;
		case DUMP_MAGIC_64:
			fprintf(stderr, "dump appears to be from a "
			    "64-bit kernel - cannot save on 32-bit kernel\n");
			break;
		default:
	...
	if (dhp->dump_pagesize != pagesize) {
		(void) fprintf(stderr,
		    "Warning: dump pagesize (%ld) != system pagesize (%ld)\n",
	...
	if (((dhp->dump_flags & DF_COMPLETE) == 0) && !damnit) {
		(void) fprintf(stderr, "%s: dump is not complete, not enough "
		    "space on dump device.\nWARNING: vmcore may be "
			"incomplete.\n", progname);
		syslog(LOG_WARNING, "%s: dump is not complete, not enough "
		    "space on dump device.\nWARNING: vmcore may be "
			"incomplete.\n", progname);
	}
...
get_crashtime(void)
{
	...
	t = (time_t)dhp->dump_crashtime.tv_sec;
	(void) fprintf(stdout, "System went down at %s", ctime(&t));
...
path(char *file)
{
	...
	cp = Malloc(strlen(file) + strlen(dirname) + 2);
...
save_core(void)
{
	...
	(void) fprintf(stdout, "Saving %ld pages of image in vmcore.%d\n",
			dumpsize, bounds);
	syslog(LOG_NOTICE, "Saving %d pages of image in vmcore.%d\n",
		dumpsize, bounds);

	/* Save the first header */
	if (!save_pages(ifd, ofd, cp, 1))
		goto error;
	...
	/* Save the chunks */
	while (dumpsize > saved) {
		npages = (dumpsize - saved) > DUMPPAGES ?
		    DUMPPAGES : (dumpsize - saved);
		npages = save_pages(ifd, ofd, cp, npages);
		if (npages <= 0)
			goto error;
		saved += npages;
		if ((saved - last_saved) >= dump_printp || saved == dumpsize) {
			last_saved = saved;
			(void) fprintf(stdout, "%6ld pages saved\r", saved);
			(void) fflush(stdout);
		}
	}

	/* Save the last header */
	(void) save_pages(ifd, ofd, cp, 1);

error:
	(void) fprintf(stdout, "%6ld pages saved.\n", saved);
...
long
save_pages(int ifd, int ofd, char *cp, long npages)
{
	long	n;

	n = Read(ifd, cp, npages * pagesize);
	if (n == 0) {
		syslog(LOG_WARNING, "WARNING: vmcore may be incomplete\n");
		(void) fprintf(stderr, "WARNING: vmcore may be incomplete\n");
		return (0);
	}
	if (n < 0) {
		syslog(LOG_WARNING, "WARNING: Dump area was too small - "
		    "%d pages not saved", dumpsize - saved);
		(void) fprintf(stderr, "WARNING: Dump area was too small - "
		    "%ld pages not saved\n", dumpsize - saved);
		return (0);
	}
	Write(ofd, cp, n);
...
build_namelist(int bounds, char *corefile)
{
	...
	Sym		*symbols;	/* temporary symbol table */
	Sym		*locals;	/* start of locals in temporary */
	long		nlocals;	/* number of local symbols */
	Sym		*globals;	/* start of globals in temporary */
	long		nglobals;	/* number of nonlocal symbols */
	size_t		loc_size;	/* total size of combined locals */
	size_t		glb_size;	/* total size of combined globals */
	size_t		str_size;	/* total size of string table */
	int		i_nld, o_nld;	/* i/o namelist descriptors */
	int		mod_fd;		/* module file descriptor */
	Shdr		*shp;		/* used for relocation */
	GElf_Shdr	tshdr;
	GElf_Shdr 	*shdr = &tshdr;	/* used for libelf routines */
	long		cnt;		/* temp counter */
	Elf 		*elf;		/* output file's elf desc */
	Elf		*mod_ed;	/* current module's elf desc */
	GElf_Ehdr	ehdr;
	GElf_Ehdr 	*hdr = &ehdr;	/* temp elf hdr */
	 ...
	 * get default_path.
	 */
	Kvm_read(kd, nd[0].n_value, (char *)&ptr, sizeof (ptr));
	if (kvm_read(kd, ptr, &default_path[0], DEFPATHSIZE) < 0) {
	...
	for (wlk = head; wlk != (struct loaded *)0; wlk = wlk->next) {
		...
		loc_size += (wlk->cd.sym_size -
		    (wlk->cd.nglbs * sizeof (Sym)));
		glb_size += (wlk->cd.sym_size -
		    (wlk->cd.nlcls * sizeof (Sym)));
		str_size += wlk->cd.str_size;
	}
	if (((loc_size + glb_size) % sizeof (Sym)) != 0) {
		(void) fprintf(stderr,
	...
	*str_p++ = '\0';
	...
	for (wlk = head; wlk != (struct loaded *)0; wlk = wlk->next) {
		...
		gelf_getehdr(mod_ed, hdr);
		mod_symsec = mod_strsec = (Elf_Scn *)0;
		dyn_symsec = dyn_strsec = (Elf_Scn *)0;
		sec = 0;
		while ((sec = elf_nextscn(mod_ed, sec)) != 0) {
		    if (gelf_getshdr(sec, shdr) != 0) {
		...
		for (cnt = 0; cnt < wlk->mp->nsyms; ++cnt) {
			symname = (char *)((uintptr_t)wlk_str +
			    wlk_sym->st_name);
			if (GELF_ST_BIND(wlk_sym->st_info) == STB_LOCAL) {
				/*
				 * locals first...
				 *
	...
	loc_size = (size_t)((uintptr_t)loc_p - (uintptr_t)locals);
	glb_size = (size_t)((uintptr_t)glb_p - (uintptr_t)globals);
	str_size = (size_t)((uintptr_t)str_p - (uintptr_t)strings);

	symbols = Malloc(loc_size + glb_size);
	wlk_symbols = symbols;

	(void) memcpy(wlk_symbols, locals, loc_size);
	wlk_symbols = (Sym *)((uintptr_t)wlk_symbols + loc_size);
	...
	sec = 0;
	while ((sec = elf_nextscn(elf, sec)) != 0) {
	    if (gelf_getshdr(sec, shdr) != 0) {
		if (shdr->sh_type != SHT_SYMTAB &&
		    shdr->sh_type != SHT_STRTAB)
			continue;
...
void
insert_sparcv9(char *name)
{
	char sparcv9[] = "sparcv9/";
	char str1[256];
	char *np = name;
	char *sp;

	if (strstr(name, sparcv9) == NULL) {
		if (strchr(name, '/') == NULL) {
			(void) strcpy(str1, sparcv9);
			(void) strcat(str1, name);
		} else {
			sp = strtok(np, "/");
			(void) strcpy(str1, sp);
			(void) strcat(str1, "/");
			(void) strcat(str1, sparcv9);
			sp = strtok(NULL, "\0");
			(void) strcat(str1, sp);
		}
		(void) strcpy(name, str1);
	}
}
...
void
Kvm_read(kvm_t *kd, uintptr_t addr, char *buf, size_t nbytes)
{
...
ssize_t
Read(int fd, void *buff, size_t size)
{
	ssize_t ret;

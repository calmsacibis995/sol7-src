/*
	Copyright 07/28/97 Sun Microsystems, Inc. All Rights Reserved
*/
%{
/***************************************************************** 

  @(#)cafe_dem.y	1.25 07/28/97 17:21:54
...
|	'A' '8' big_number
	...
	uname
		...
		if (build_args)
		{
		    ...
		    strncpy($$->arg_data.decl.array_size, yytext, $3);
...
static void put_characters(char *s, int len)
{
    int i;
    for (i = 0;  i < len; ++i)
	out_buffer[obx++] = s[i];
...
static int in_len = 0;		/* symbol length */

static void startup(char *in, char *mem, int mem_size)
{
    if (in) {
	in_buffer = in;
    ...
    mem_reservoir = mem;
    memset(mem_reservoir, 0, mem_size);
}

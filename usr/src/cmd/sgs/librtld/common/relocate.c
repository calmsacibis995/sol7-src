 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)relocate.c	1.14	98/01/06 SMI"
...
count_reloc(Cache * cache, Cache * _cache, Rt_map * lmp, int flags, Addr addr,
    Xword * null, Xword * data, Xword * func)
{
	Rel *		rel;
	Reloc *		reloc;
	Shdr *		shdr;
	...
	for (_cnt = 0; _cnt < cnt; _cnt++, rel++, reloc++) {
		...
		Sym *		sym;
		/* LINTED */
		unsigned char	type = (unsigned char)ELF_R_TYPE(rel->r_info);
		unsigned char	bind;
		unsigned long	offset = rel->r_offset + addr;
		Rt_map *	_lmp, * head;
		int		bound;
		unsigned long	rsymndx = ELF_R_SYM(rel->r_info);
		Slookup		sl;
		...
		sym = 0;
		if ((sym = lookup_sym(&sl, &_lmp, LKUP_DEFT)) != 0) {
			/*

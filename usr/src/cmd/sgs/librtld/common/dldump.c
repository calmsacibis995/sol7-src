 ...
 *	All rights reserved.
 *
 * dldump(3x) creates a new file image from the specified input file.
 */
#pragma ident	"@(#)dldump.c	1.25	98/01/06 SMI"
 ...
 *
 * rt_dldump(Rt_map * lmp, const char * opath, int pfd, int flags, Addr addr)
 *
 * lmp provides the link-map of the ipath (the input file).
 *
 * opath specifies the output file.
 *
 * pfd is a /proc file descriptor which is only provided if the orginal ipath
 * was 0 (indicating the image that started the process (ie. the dynamic
 * executable).
 *
...
rt_dldump(Rt_map * lmp, const char * opath, int pfd, int flags, Addr addr)
{
	...
	if (pfd) {
		 ...
		 * don't know its full path, and thus use /proc to obtain a file
		 * descriptor of the input file. ld.so.1 passes us the proc file
		 * descriptor as it has its own use for the same descriptor.
		 */
	...
	if ((ielf = elf_begin(fd, ELF_C_READ, NULL)) == NULL) {
		...
		cleanup(ielf, oelf, melf, icache, mcache, fd);
		return (1);
	}
	(void) close(fd);
	...
	if ((elf_kind(ielf) != ELF_K_ELF) ||
		...
		cleanup(ielf, oelf, melf, icache, mcache, 0);
		return (1);
	}
	...
	for (num = 0, ophdr = iphdr; num != iehdr->e_phnum; num++, ophdr++) {
		 ...
		 */
		if ((ophdr->p_type == PT_LOAD) && (ophdr->p_flags & PF_W)) {
	...
	if (status)
		num++;
	if ((icache = (Cache *)malloc((num + 1) * sizeof (Cache))) == 0) {
		cleanup(ielf, oelf, melf, icache, mcache, fd);
		return (1);
	}
	 ...
	 */
	if ((shstr = (char *)malloc(shstr_size)) == 0) {
		cleanup(ielf, oelf, melf, icache, mcache, fd);
		return (1);
	}
	_shstr = shstr;
	*_shstr++ = '\0';
	...
	for (_icache++; _icache->c_flags != FLG_C_END; _icache++) {
		...
		_shstr += strlen(_icache->c_name) + 1;
	 ...
	 */
	if ((mcache = (Cache *)malloc(mehdr->e_shnum * sizeof (Cache))) == 0) {
		cleanup(ielf, oelf, melf, icache, mcache, fd);
		return (1);
	}
	_mcache = mcache;
		...
		if (shdr->sh_type == M_REL_SHT_TYPE) {
			if (rel_base == (Rel *)0) {
				...
				rel_data = (Rel *)((Xword)rel_null +
				    (rel_null_no * sizeof (Rel)));
				rel_func = (Rel *)((Xword)rel_data +
				    (rel_data_no * sizeof (Rel)));
			}
	...
	if (dyn_cache) {
		...
		/*
		 * If we're dumping the dynamic executable compensate for its
		 * real base address.
		 */
		if (!addr)
			off += ADDR((Rt_map *)lml_main.lm_head);

		if (update_dynamic(dyn_cache, addr, off, opath, rel_null_no,
		    rel_data_no, rel_func_no)) {
			cleanup(ielf, oelf, melf, icache, mcache, fd);
			return (1);
		}
	}

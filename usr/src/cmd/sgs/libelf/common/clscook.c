...
#ident	"@(#)clscook.c	1.3	98/01/05 SMI"
...
int
_elf32_ehdr(Elf * elf, int inplace)
{
	return (0);
}

int
_elf32_phdr(Elf * elf, int inplace)
{
	return (0);
}

int
_elf32_shdr(Elf * elf, int inplace)
{
	return (0);
}
#endif	/* defined(lint) */
...
int
_elf64_ehdr(Elf * elf, int inplace)
{
	return (0);
}

int
_elf64_phdr(Elf * elf, int inplace)
{
	return (0);
}

int
_elf64_shdr(Elf * elf, int inplace)
{
	return (0);
}
#endif	/* defined(lint) */
...
_elf_cookscn(Elf_Scn * s)
{
	...
	d->db_data.d_type = _elf_mtype(sh->sh_type, work);
	d->db_data.d_buf = 0;
	d->db_data.d_off = 0;
...
_elf_ehdr(Elf * elf, int inplace)
{
	...
	if (inplace && (fsz >= sizeof (Ehdr))) {
		...
		elf->ed_ehdr = (Ehdr *)malloc(sizeof (Ehdr));
		if (elf->ed_ehdr == 0) {
...
_elf_shdr(Elf * elf, int inplace)
{
	...
	if (inplace && (fsz >= msz) &&
		...
		if ((elf->ed_shdr = (Shdr *)malloc(msz)) == 0) {

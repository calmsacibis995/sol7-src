...
#pragma ident	"@(#)update.c	1.16	98/02/06 SMI" 	/* SVr4.0 1.17	*/
...
#if	defined(lint)
off_t
_elf64_update(Elf * elf, Elf_Cmd cmd)
{
	return (0);
}
#endif /* defined(lint) */
...
_elf_upd_lib(Elf * elf)
{
	...
	for (; s != 0; s = s->s_next) {
		...
		sh->sh_addralign = 1;
		if ((sz = _elf_entsz(sh->sh_type, ver)) != 0)
			/* LINTED */
			sh->sh_entsize = (Half)sz;
		sz = 0;
		for (d = s->s_hdnode; d != 0; d = d->db_next) {
			if ((fsz = elf_fsize(d->db_data.d_type, 1, ver)) ==
			    0)
				return (0);
...
_elf_upd_usr(Elf * elf)
{
	...
	for (; s != 0; s = s->s_next) {
		...
		sz = 0;
		for (d = s->s_hdnode; d != 0; d = d->db_next) {
			if ((fsz = elf_fsize(d->db_data.d_type, 1, ver)) == 0)
				return (0);
			j = _elf_msize(d->db_data.d_type, ver);
			fsz *= d->db_data.d_size / j;
			d->db_osz = fsz;

 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)file.c	1.22	98/02/22 SMI"
...
build_file(Elf * src_elf, GElf_Ehdr * src_ehdr, Cmd_Info * cmd_info)
{
	...
	if (src_ehdr->e_phnum != 0) {
		...
		for (x = 0; x < src_ehdr->e_phnum; ++x) {
			...
			(void) gelf_getphdr(src_elf, x, &src);
			(void) gelf_getphdr(dst_elf, x, &dst);
			(void) memcpy(&dst, &src, sizeof (GElf_Phdr));
			(void) gelf_update_phdr(dst_elf, x, &dst);
		}
	...
	if (src_ehdr->e_phnum != 0) {
		int align = gelf_fsize(dst_elf, ELF_T_ADDR, 1, EV_CURRENT);
...
copy_file(int ofd, char *fname, char *temp_file_name)
{
	...
	if ((write(ofd, buf, (unsigned) stbuf.st_size)) !=
	    (unsigned) stbuf.st_size) {
		error_message(WRITE_MANI_ERROR2,
		SYSTEM_ERROR, strerror(errno),
		prog, fname);

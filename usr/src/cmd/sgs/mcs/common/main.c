 ...
 */
#pragma ident	"@(#)main.c	6.19	97/12/30 SMI"
...
#define	TEXT_DOMAIN "SYS_TEST"
	...
	if ((Action = (struct action *)
	    malloc((unsigned) optbufsz * sizeof (struct action))) == NULL) {
...
queue(int activity, char *string)
{
	if (optcnt > optbufsz) {
		...
		if ((Action = (struct action *)
		    ...
		    (unsigned) optbufsz * sizeof (struct action))) == NULL) {

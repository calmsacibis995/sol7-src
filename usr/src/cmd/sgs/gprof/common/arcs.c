...
#pragma ident	"@(#)arcs.c	1.9	97/07/28 SMI"
...
cyclelink()
{
	...
	for (nlp = nl; nlp < npe; nlp++) {
		...
		cyclenlp->childtime = 0.0;	/* cumulative ticks in */
						/* children */
		...
		cyclenlp->propself = 0.0;	/* how much self time */
						/* propagates */
		cyclenlp->propchild = 0.0;	/* how much child time */
						/* propagates */
		cyclenlp->printflag = TRUE;	/* should this be printed? */
		cyclenlp->index = 0;		/* index in the graph list */
		cyclenlp->toporder = DFN_NAN;	/* graph call chain top-sort */
						/* order */
		cyclenlp->cycleno = cycle;	/* internal number of cycle */
						/* on */

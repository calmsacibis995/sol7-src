 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)map.c	1.15	97/06/05 SMI"
...
Dbg_map_size_old(Sym_desc * sdp)
{
	...
	Elf_sym_table_entry(MSG_INTL(MSG_STR_UP_2), sdp->sd_sym,
...
Dbg_map_symbol(Sym_desc * sdp)
{
	...
	if (DBG_NOTCLASS(DBG_SYMBOLS))
		Elf_sym_table_entry(MSG_INTL(MSG_STR_ENTERED), sdp->sd_sym,
		    sdp->sd_aux ? sdp->sd_aux->sa_verndx : 0, NULL,
		    conv_deftag_str(sdp->sd_ref));
}
...
Dbg_map_dash(const char * name, Sdf_desc * sdf)
{
	...
	if (sdf->sdf_flags & FLG_SDF_USED)
		type = MSG_ORIG(MSG_MAP_USED);
	else
		type = MSG_ORIG(MSG_MAP_NEEDED);
...
Dbg_map_pipe(Sg_desc * sgp, const char * sec_name, const Word ndx)
{
	...
	dbg_print(MSG_INTL(MSG_MAP_SEC_ORDER), sgp->sg_name, sec_name, ndx);
}

void
Dbg_map_seg(int ndx, Sg_desc * sgp)
{
	if (DBG_NOTCLASS(DBG_MAP))
		return;

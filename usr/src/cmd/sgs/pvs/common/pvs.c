 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)pvs.c	1.19	97/12/30 SMI"
 ...
 */
static void
sym_local(Cache * cache, Cache * csym, const char * file, int oflag)
{
	...
	const char * name)
{
	...
	for (_num = 1; _num <= num; _num++,
	    vnd = (GElf_Verneed *)((GElf_Word)vnd + vnd->vn_next)) {
		GElf_Vernaux *	vnap = (GElf_Vernaux *)
					((GElf_Word)vnd + vnd->vn_aux);
		 ...
		 */
		/* CSTYLED */
		for (vnap = (GElf_Vernaux *)((GElf_Word)vnap + vnap->vna_next);
		    cnt;
		    cnt--,
			vnap = (GElf_Vernaux *)((GElf_Word)vnap +
						vnap->vna_next)) {
			dep = (char *)(strs + vnap->vna_name);
...
static Ver_desc *
vers_desc(const char * name, unsigned long hash, List * lst, const char * file)
{
	Ver_desc *	vdp;
    ...
    const char * file)
{
	...
	if ((_vdp = vers_desc(name, hash, lst, file)) == 0)
		return (0);
...
static void
vers_syms(GElf_Versym * vsp, Elf_Data * sym_data, int symn, char * strs,
	Ver_desc * vdp, const char * file, int oflag, int vflag)
{
	GElf_Sym	sym;
	int		_symn;
	...
	Ver_desc * vdp, const char * file, int oflag, int vflag)
{
	...
	for (LIST_TRAVERSE(&vdp->vd_deps, _lnp, _vdp)) {
		...
		vers_syms(vsp, sym_data, symn, strs, _vdp, file, oflag, vflag);
 ...
 */
static int
vers_def(Cache * cache, Cache * def, Cache * csym, const char * file, int nflag,
	int oflag, int vflag, const char * name)
{
	 ...
	 */
	for (_num = 1; _num <= num; _num++,
	    vdf = (GElf_Verdef *)((GElf_Word)vdf + vdf->vd_next)) {
		GElf_Half	cnt = vdf->vd_cnt;
		GElf_Half	ndx = vdf->vd_ndx;
		GElf_Verdaux *	vdap = (GElf_Verdaux *)((GElf_Word)vdf +
							vdf->vd_aux);
		const char *	_name;
		...
		if ((vdp = vers_desc(_name, elf_hash(_name), &verdefs,
		    file)) == 0)
			return (0);
		vdp->vd_ndx = ndx;
		vdp->vd_flags = vdf->vd_flags | FLG_VER_AVAIL;

		vdap = (GElf_Verdaux *)((GElf_Word)vdap + vdap->vda_next);
		for (cnt--; cnt; cnt--,
		    vdap = (GElf_Verdaux *)((GElf_Word)vdap + vdap->vda_next)) {
			_name = (char *)(strs + vdap->vda_name);
	...
	for (LIST_TRAVERSE(&verdefs, lnp, vdp)) {
		 ...
		 * version.
		 */
		vers_syms(vsp, sym_data, symn, strs, vdp, file, oflag, vflag);
		if (name) {
			...
			if (vflag && bvdp && strcmp(name, bvdp->vd_name)) {
				...
				vers_syms(vsp, sym_data, symn, strs, bvdp, file,
				    oflag, vflag);
			}
		}
	}
...
main(int argc, char ** argv)
{
	...
	const char *	name;
	char *		names;
	int		dflag, lflag, nflag, oflag, rflag, sflag, vflag;
	Cache *		cache, * _cache;
	...
	cname = argv[0];
	name = NULL;
	dflag = lflag = nflag = oflag = rflag = sflag = vflag = 0;

	opterr = 0;
	while ((var = getopt(argc, argv, "dlnorsvN:")) != EOF) {
		switch (var) {
		case 'd':
			dflag = USR_DEFINED;
			break;
	...
	for (; optind < argc; optind++) {
		 ...
		 */
		if (_cache_loc)
			sym_local(cache, _cache_loc, file, oflag);

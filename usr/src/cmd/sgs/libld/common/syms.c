 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)syms.c	1.65	98/01/22 SMI"
	...
	Ofl_desc * ofl, Word ndx)
{
	...
	if (scp == 0) {
		...
		scp->sc_end = (Sym_desc *)((Xword)scp->sc_free +
	...
	*nsym = *osym;
	...
	sdp->sd_GOTndx = 0;
	DBG_CALL(Dbg_syms_entered(nsym, sdp));
	return (sdp);
}
    ...
    int flags, Ofl_desc * ofl)
{
	...
	if (usdp = sym_find(uname, hash, ofl)) {
		...
		if ((usdp->sd_sym->st_shndx == SHN_UNDEF) ||
			...
			usdp->sd_flags |= flags;
			DBG_CALL(Dbg_syms_updated(usdp, uname));
		} else
	...
	if (name && (sdp = sym_find(name, SYM_NOHASH, ofl)) &&
		...
		sdp->sd_flags |= flags;
		DBG_CALL(Dbg_syms_updated(sdp, name));
	}
	return (1);
}
...
static const int format[] = {
	MSG_SYM_UND_UNDEF,	MSG_SYM_UND_NOVER,
	MSG_SYM_UND_IMPL,	MSG_SYM_UND_NOTA
};
...
sym_spec(Ofl_desc * ofl)
{
	...
	if (!(flags & FLG_OF_RELOBJ)) {
		...
		if (sym_find(MSG_ORIG(MSG_SYM_GOFTBL_U), SYM_NOHASH, ofl))
		...
		for (scp = ofl->ofl_symbkt[bkt]; scp; scp = scp->sc_next) {
			for (sdp = (Sym_desc *)(scp + 1);
				...
				if (undef) {
					 ...
					 *
					 * The one exception is a symbol
					 * which is defined to by found
					 * in the parent (FLG_SY_PARENT).
					 *
				...
				if ((shndx == SHN_COMMON) &&
				    ...
				    ((sdp->sd_flags & FLG_SY_LOCAL) &&
				    (flags & FLG_OF_PROCRED)))) {
					size = S_ROUND(size, sym->st_value) +
						sym->st_size;
				...
				if ((sdp->sd_flags & FLG_SY_LOCAL) &&
					...
					if (!((sdp->sd_flags & FLG_SY_ELIM) &&
					    (flags & FLG_OF_PROCRED))) {
						ofl->ofl_scopecnt++;
						ofl->ofl_locsstrsz += len;
					}
				} else {
					ofl->ofl_globcnt++;
					ofl->ofl_globstrsz += len;
				}
			}
		}
	}
...
sym_process(Is_desc * isc, Ifl_desc * ifl, Ofl_desc * ofl)
{
	...
	for (ndx = (int)local; ndx < total; sym++, ndx++) {
		...
		if ((sdp = sym_find(name, hash, ofl)) == NULL) {
			...
			if (sym_resolve(sdp, sym, ifl, ofl, ndx) == S_ERROR)
				return (S_ERROR);
		}

 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)files.c	1.67	98/01/26 SMI"
	...
	Ofl_desc * ofl, int * ifl_errno)
{
	...
	*ifl_errno = 0;
	...
	if ((ifl = (Ifl_desc *)libld_calloc(1, sizeof (Ifl_desc))) ==
	    (Ifl_desc *)0)
	...
	ifl->ifl_elf = elf;
	ifl->ifl_flags = flags;
	if ((ifl->ifl_isdesc = (Is_desc **)libld_calloc(ehdr->e_shnum,
	    sizeof (Is_desc *))) == 0)
		return ((Ifl_desc *)S_ERROR);
	...
	Elf_Scn *scn, Word ndx, int ident, Ofl_desc * ofl)
{
	 ...
	 */
	if ((isp = (Is_desc *)libld_calloc(sizeof (Is_desc), 1)) ==
	    (Is_desc *)0)
		return (S_ERROR);
	isp->is_shdr = shdr;
	isp->is_file = ifl;
	...
	Elf_Scn *scn, Word ndx, int ident, Ofl_desc * ofl)
{
	eprintf(ERR_WARNING, MSG_INTL(MSG_FIL_INVALSEC), ifl->ifl_name, name,
		conv_sectyp_str((unsigned)shdr->sh_type));
	return (1);
}
	...
	Elf_Scn *scn, Word ndx, int ident, Ofl_desc * ofl)
{
	...
	for (dyn = (Dyn *)dp->d_buf; dyn->d_tag != DT_NULL; dyn++) {
		...
		case DT_NEEDED:
		case DT_USED:
			if ((_ifl = (Ifl_desc *)libld_calloc(1,
 ...
 * $ORIGIN token, either from their needed name, or via a runpath.  Probably
 * the most common reference to explict dependencies (via -L) will be sufficient
 * to find any associated implict dependencies, but just in case we expand any
 * occurance of $ORIGIN here.
 *
 * Expansions is really just a concatenation of the parents directory name.
 * For example, an explict dependency foo/bar/lib1.so with a dependency on
 * $ORIGIN/lib2.so would be expanded to for/bar/lib2.so.
 *
 * Note, if any errors occur we simply return the original name.
 */
static char *
expand(const char * parent, const char * name)
{
	char		_name[PATH_MAX], * nptr;
	const char *	optr;
	int		nrem = PATH_MAX - 1, expanded = 0;
...
process_dynamic(Is_desc * isc, Ifl_desc * ifl, Ofl_desc * ofl)
{
	...
	for (dyn = data; dyn->d_tag != DT_NULL; dyn++) {
		if (dyn->d_tag == DT_SONAME) {
			 ...
			 */
			needed = expand(ifl->ifl_name, needed);
			...
			if (rpath && (sdf->sdf_rpath == 0))
				sdf->sdf_rpath = expand(sdf->sdf_rfile, rpath);
...
rel_process(Is_desc * isc, Ifl_desc * ifl, Ofl_desc * ofl)
{
	...
	if (shdr->sh_type != M_REL_SHT_TYPE) {
		eprintf(ERR_FATAL, MSG_INTL(MSG_FIL_INVALSEC), ifl->ifl_name,
		    isc->is_name, conv_sectyp_str((unsigned)shdr->sh_type));
		ofl->ofl_flags |= FLG_OF_FATAL;
		return (0);
	}
 ...
 */
Ifl_desc *
process_open(const char * path, Xword dlen, int fd, Ofl_desc * ofl, Half flags,
	int * ifl_errno)
{
	Elf *		elf;
	Ifl_desc *	ifl;
...
finish_libs(Ofl_desc * ofl)
{
	...
	for (LIST_TRAVERSE(&ofl->ofl_soneed, lnp1, sdf)) {
		...
		if (sdf->sdf_rpath != NULL) {
			...
			if ((rpath =
			    (char *)libld_malloc(strlen(sdf->sdf_rpath)
				+ 1)) == 0)
				return (S_ERROR);
			(void) strcpy(rpath, sdf->sdf_rpath);
			DBG_CALL(Dbg_libs_rpath(sdf->sdf_rfile, rpath));
			if ((path = strtok(rpath,
			    MSG_ORIG(MSG_STR_COLON))) != NULL) {
				do {
					ifl_errno = 0;

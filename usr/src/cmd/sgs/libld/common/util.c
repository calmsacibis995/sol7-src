 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)util.c	1.32	97/09/22 SMI"
...
list_appendc(List * lst, const void * item)
{
	...
	if ((_lnp = (Listnode *)libld_malloc(sizeof (Listnode))) ==
	    (Listnode *)0)
		return (0);
...
sdf_add(const char * name, List * lst)
{
	...
	if (!(sdf = (Sdf_desc *)libld_calloc(sizeof (Sdf_desc), 1)))
...
const char *
add_string(const char * old, char * str)
{
	char *	new;

	if (old) {
		if ((new = (char *)
		    libld_calloc(1, strlen(old) + strlen(str) + 2)) == 0)

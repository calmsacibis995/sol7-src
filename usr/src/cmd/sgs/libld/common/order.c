 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)order.c	1.19	98/01/05 SMI"
...
{
	Word t1_link = ndx, t2_link, info, ret_link;
	Is_desc *isp1, *isp2;
	int error = 0;
 ...
 * This routine does the input processing of the
 * ordered sections.
 */
Xword
process_ordered(Ifl_desc *ifl, Ofl_desc *ofl, Word ndx, Word limit)
{
	Is_desc *isp = ifl->ifl_isdesc[ndx];
	Word head_ndx;
	Is_desc *isp2;
	Os_desc *osp;
	Os_desc *osp2;
	...
	/*
	 * I might have been checked and marked error
	 * already.
	 */
	...
	if ((head_ndx = get_head_ndx(ifl, ndx, limit)) == 0) {
		...
		if (isp->is_osdesc == NULL)
			return ((Xword)place_section(ofl, isp, isp->is_key, 0));
		return ((Xword) isp->is_osdesc);
	}

 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)relocate.c	1.61	98/02/02 SMI"
...
reloc_GOT_relative(Boolean local, Rel_desc * rsp, Rel * reloc, Ofl_desc * ofl)
{
	...
	if (sdp->sd_GOTndx == 0) {
		...
		sdp->sd_GOTndx = assign_got_ndx(rtype, 0, sdp, ofl);
...
reloc_init(Ofl_desc * ofl)
{
	 ...
	 */
	if ((ofl->ofl_flags1 & FLG_OF1_RELCNT) && ofl->ofl_reloccnt)
...
reloc_process(Ofl_desc * ofl)
{
	 ...
	 */
#if defined(__sparc) || defined(__i386) || defined(__sparcv9)
...
reloc_remain_entry(Rel_desc * orsp, Os_desc * osp, Ofl_desc * ofl)
{
	...
	/*
	 * Only give relocation errors against loadable read-only
	 * segments.
	 */
	if ((osp->os_sgdesc->sg_phdr.p_type != PT_LOAD) ||
	    (osp->os_sgdesc->sg_phdr.p_flags & PF_W))
		return;

	/*
	 * If we are in -ztextwarn mode, it's a silent error if
	 * a relocation is due to a 'WEAK REFERENCE'.  This is because
	 * if the symbol is not provided at run-time we will not perform
	 * a text-relocation.
	 */
	...
	if (reloc_title) {
		/*
		 * If building with '-ztext' then emit a fatal
		 * error.  If building a executable then only
		 * emit a 'warning'
		 */

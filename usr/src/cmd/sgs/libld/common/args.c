 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)args.c	1.77	98/02/24 SMI"
 ...
 *
 *	-z initfirst 	mark object so the .init section of this object is
 *			executed before the .init sections of other objects.
 *
 *	-z defs | nodefs
 ...
 *
 *	-z textoff
 *			permit relocations against text segment
 *
 *	-z textwarn
 *			warn if relocations exist against text segment
 *
...
usage_mesg(Boolean detail)
{
	...
	(void) fprintf(stderr, MSG_INTL(MSG_ARG_DETAIL_ZNO));
...
check_flags(Ofl_desc * ofl, int argc)
{
	...
	if (dflag != SET_FALSE) {
		...
		/*
		 * By default Text warnings are given when building
		 * an executable unless the '-b' flag was specified.
		 * This implies that you will build unclean text
		 * so no warning are given unless specifically asked
		 * for.
		 */
...
#if	defined(_ELF64)
	/*
	 * Due to last minute problems in OSNet versioning is disabled
	 * by default for 64bit objects.  This is only to be for
	 * *BETA* and will be re-enabled when the versioning information
	 * has been corrected.
	 */

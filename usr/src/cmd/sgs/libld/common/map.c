 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)map.c	1.49	98/01/05 SMI"
	...
	for (;;) {
		...
		if (!isascii(*nextchr) ||
			...
			eprintf(ERR_FATAL, MSG_INTL(MSG_MAP_ILLCHAR),
			    mapfile, Line_num, *((unsigned char *) nextchr));
			/* LINTED */
...
map_equal(const char * mapfile, Sg_desc * sgp, Ofl_desc * ofl)
{
	...
	while ((tok = gettoken(mapfile)) != TK_SEMICOLON) {
		...
		/* Segment address, length, alignment or rounding number. */
			...
			if ((number = STRTOADDR(&Start_tok[1], &end_tok, 0))
				== XWORD_MAX) {
	...
	if (sgp->sg_flags & FLG_SG_EMPTY) {
		...
		if (sgp->sg_phdr.p_flags != 0) {
			eprintf(ERR_FATAL, MSG_INTL(MSG_MAP_SEGEMNOPERM),
				mapfile, Line_num, sgp->sg_phdr.p_flags);
			return (S_ERROR);
		}
...
map_atsign(const char * mapfile, Sg_desc * sgp, Ofl_desc * ofl)
{
	...
	if (sgp->sg_sizesym != NULL) {
		eprintf(ERR_FATAL, MSG_INTL(MSG_MAP_SEGSIZE), mapfile, Line_num,
			sgp->sg_name);
		return (S_ERROR);
	}
	...
	if ((sdp = sym_find(Start_tok, SYM_NOHASH, ofl)) == NULL) {
		...
		if (sym->st_shndx == SHN_UNDEF) {
			...
			eprintf(ERR_FATAL, MSG_INTL(MSG_MAP_SYMDEF), mapfile,
			    Line_num, sdp->sd_name, sdp->sd_file->ifl_name);
			return (S_ERROR);
		}
	}
...
map_dash(const char * mapfile, char * name, Ofl_desc * ofl)
{
	...
	while ((tok = gettoken(mapfile)) != TK_SEMICOLON) {
		...
		if (*Start_tok == '$') {
			...
			Start_tok++;
			lowercase(Start_tok);
			if (strcmp(Start_tok, MSG_ORIG(MSG_MAP_USED)) == 0) {
 ...
 * 	version name {
 *		local:
 *		    symbol { = (FUNC|OBJECT) (PASSTHRU|PARENT)
 *				($ABS|$COMMON) A V };
 *			*;
 *		global:
 *		symbolic:
		...
		case TK_EQUAL:
			...
			while ((tok = gettoken(mapfile)) != TK_SEMICOLON) {
				...
				if (Start_tok[0] == 'v' ||
					...
					if ((number = STRTOADDR(&Start_tok[1],
					    &end_tok, 0)) == XWORD_MAX) {
						eprintf(ERR_FATAL,
		...
		if (new_segment) {
			...
			default:
				eprintf(ERR_FATAL, MSG_INTL(MSG_MAP_UNKSEGTYP),
				    mapfile, Line_num, sgp1->sg_phdr.p_type);
				return (S_ERROR);
			}
			lnp2 = NULL;
			for (LIST_TRAVERSE(&ofl->ofl_segs, lnp1, sgp2)) {

 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)libs.c	1.40	98/01/26 SMI"
...
process_archive(const char * name, int fd, Ar_desc * adp, Ofl_desc * ofl)
{
	...
	do {
		...
		for (arsym = adp->ad_start, aup = adp->ad_aux; arsym->as_name;
			 ...
			 */
			DBG_CALL(Dbg_syms_ar_resolve(ndx, arsym,
							arname, allexrt));
			if ((err = (Xword)process_ifl(arpath, NULL, fd, arelf,
			    FLG_IF_EXTRACT | FLG_IF_NEEDED, ofl, &ifl_errno))
			    == S_ERROR)
				return (S_ERROR);

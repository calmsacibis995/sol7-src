 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)update.c	1.74	98/02/09 SMI"
...
update_osym(Ofl_desc * ofl)
{
	...
	*shdrstr = '\0';
	 ...
	 */
	if ((_gottable = gottable = calloc(ofl->ofl_gotcnt,
	    sizeof (Gottable))) == 0)
		return (S_ERROR);
...
#error Unknown architecture!
				...
				/* LINTED */
				_gottable->gt_gotndx = (Word)sdp->sd_GOTndx;
				_gottable++;
			}
	...
	if (ofl->ofl_isbss) {
		...
		/* LINTED */
		bssndx = (Half)elf_ndxscn(osp->os_scn);
	}
...
update_odynamic(Ofl_desc * ofl)
{
	...
	for (LIST_TRAVERSE(&ofl->ofl_sos, lnp, ifl)) {
		if ((ifl->ifl_flags &
			...
			continue;
		/*
		 * Create and set up the DT_POSFLAG_1 entry here if
		 * required.
		 */
		if ((ifl->ifl_flags & (FLG_IF_LAZYLD|FLG_IF_GRPPRM)) &&
		    (ifl->ifl_flags & (FLG_IF_NEEDED|FLG_IF_USED))) {
		...
		/* LINTED */
		ifl->ifl_neededndx = (Half)(((Xword)dyn - (Xword)_dyn) /
		    sizeof (Dyn));
		dyn++;
	}
...
update_overdef(Ofl_desc * ofl)
{
	...
	for (LIST_TRAVERSE(&ofl->ofl_verdesc, lnp1, vdp)) {
		...
		/*
		 * XX64: had wanted versioning structs to remain
		 *	 same size of Elf64, if this is going on
		 *	 it wont work. goes for vda_name and
		 *	 vda_next in this case:
		 */
		/* LINTED */
		vdap->vda_name = (Word)vdp->vd_name;
		vdap++;
		/* LINTED */
		_vdap->vda_next = (Word)((Xword)vdap - (Xword)_vdap);
		 ...
		 */
		/* LINTED */
		vdf->vd_aux = (Word)((Xword)(vdf + 1) - (Xword)vdf);
		vdf->vd_cnt = cnt;
		...
		vdf = (Verdef *)vdap, num++;
		/* LINTED */
		_vdf->vd_next = (Word)((Xword)vdf - (Xword)_vdf);
	}
	_vdf->vd_next = 0;
	...
	for (LIST_TRAVERSE(&ofl->ofl_sos, lnp, ifl)) {
		...
		if (sdf && (sdf->sdf_flags & FLG_SDF_SPECVER)) {
			...
			for (LIST_TRAVERSE(&sdf->sdf_verneed, lnp2, sdv)) {
				...
				cnt++;
				/* LINTED */
				_vnap->vna_next = (Word)((Xword)vnap -
				    (Xword)_vnap);
			}
		} else {
		 ...
		 */
		/* LINTED */
		vnd->vn_aux = (Word)((Xword)(vnd + 1) - (Xword)vnd);
		/* LINTED */
		vnd->vn_cnt = (Half)cnt;
		...
		/* LINTED */
		_vnd->vn_next = (Word)((Xword)vnd - (Xword)_vnd);
	}
	_vnd->vn_next = 0;

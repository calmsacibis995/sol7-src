 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)version.c	1.31	98/01/05 SMI"
...
vers_sym_process(Is_desc * isp, Ifl_desc * ifl)
{
	...
	if ((symshdr->sh_size / symshdr->sh_entsize) != (vershdr->sh_size /
		...
		eprintf(ERR_WARNING, MSG_INTL(MSG_ELF_VERSYM), ifl->ifl_name,
			isp->is_name,
			vershdr->sh_size / vershdr->sh_entsize,
...
vers_def_process(Is_desc * isp, Ifl_desc * ifl, Ofl_desc * ofl)
{
	...
	for (_num = 1; _num <= num; _num++,
		...
		Half 		cnt = vdf->vd_cnt;
		Half		ndx = vdf->vd_ndx;
		Verdaux *	vdap = (Verdaux *)((Xword)vdf + vdf->vd_aux);
		 ...
		 */
		vdap = (Verdaux *)((Xword)vdap + vdap->vda_next);
		for (cnt--; cnt; cnt--,
		    vdap = (Verdaux *)((Xword)vdap + vdap->vda_next)) {
			Ver_desc *	_ivdp;
...
vers_need_process(Is_desc * isp, Ifl_desc * ifl, Ofl_desc * ofl)
{
	...
	for (_num = 1; _num <= num; _num++,
		...
		const char *	name;
		Half		cnt = vnd->vn_cnt;
		Vernaux *	vnap = (Vernaux *)((Xword)vnd + vnd->vn_aux);
		Half		_cnt;
		...
		for (_cnt = 0; cnt; _cnt++, cnt--,
		    vnap = (Vernaux *)((Xword)vnap + vnap->vna_next)) {
			if (!(sdv =

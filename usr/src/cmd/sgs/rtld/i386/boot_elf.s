 ...
 */
#pragma ident	"@(#)boot_elf.s	1.12	97/04/28 SMI"
...
.start_pltexit:
	...
	/*
	 * If audit_arg_count > 0 then we limit the number of
	 * arguements that will be duplicated to audit_arg_count.
	 *
	 * If (prev_stack_size > (audit_arg_count * 4))
	 *	prev_stack_size = audit_arg_count * 4;
	 */
	movl	audit_arg_count@GOT(%ebx),%eax
	movl	(%eax), %eax			/    %eax = audit_arg_count
	cmpl	$0, %eax
	jle	.grow_stack

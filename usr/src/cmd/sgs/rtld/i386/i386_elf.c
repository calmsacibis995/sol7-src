 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)i386_elf.c	1.53	98/02/02 SMI"
	...
	uint_t symndx, uint_t pltndx, caddr_t to, uint_t sb_flags)
{
	 ...
	 */
	dyn_plt = (unsigned char *)((uintptr_t)AUDIT(rlmp)->af_dynplts +
...
elf_bndr(Rt_map * lmp, unsigned long reloff, caddr_t from)
{
	...
	if (!lmp || ((reloff % sizeof (Rel)) != 0)) {
		eprintf(ERR_FATAL, MSG_INTL(MSG_REL_PLTENTRY),
		    ...
		    (!lmp ? (reloff / sizeof (Rel)) : -1), EC_ADDR(from),
	...
	if (bind) {
		/*
		 * If we're returning control to the user clean up any
		 * open file descriptors that may have resulted from the
		 * implementation of this binding (bringing in a filtee
		 * for example).
		 */

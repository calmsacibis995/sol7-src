...
#pragma ident	"@(#)boot_elf.s	1.7	98/01/07 SMI"
 ...
 */
void
elf_rtbndr(caddr_t pc, u_long reloc, Rt_map * lmp, caddr_t from)
{
	...
	mov	%i3, %o3		! Callers address is arg 4	
	sub	%i1, 0x38, %o0		! compute addr of .PLT0 from addr of .PLT1 jmpl

	ldx	[%i1 + 8], %o2		! 8 = addr of lmp - addr of .PLT1 jmpl to .PLT2
!	ldx	[%i1 + 40], %o2		! in this case we're coming from .PLT1,
					! use 8 (sizeof 2 instructions).  if coming
					! from .PLT0, use 40 bytes (10 instructions)
	
	srl	%i0, 10, %o1		! shift offset set by sethi
	call	elf_bndr		! returns function address in %o0
	add	%o0, %o1, %o0		! pc = .PLT0 plus (. - .PLT0) from sethi
...
/* ARGSUSED */
void
elf_plt_init(unsigned int *plt, caddr_t bindfunc)
{
	plt[0] = M_SAVE_SP176SP;
	plt[1] = M_SETHI_L0 | HH22(bindfunc);
	plt[2] = M_SETHI_L1 | LM22(bindfunc);
	plt[3] = M_OR_L0L0 | HM10(bindfunc);
	plt[4] = M_SLLX_L032L0;
	plt[5] = M_OR_L0L1L0;
	plt[6] = M_JMPL_L0O1 | LO10(bindfunc);
	plt[7] = M_MOV_G1O0;
}
...
void
elf_plt_write(unsigned int *pc, unsigned long *symval)
{
	...
	if ((nsym >> 44) == 0) {
		/* LINTED */
		pc[4] = (uint_t)(M_JMPL_G1G0 | L44(sym));
		pc[3] = M_SLLX_G112G1;
		/* LINTED */
		pc[2] = (uint_t)(M_XNOR_G5G1 | M44(nsym));
		/* LINTED */
		*(ulong_t *)pc =
			((ulong_t)M_NOP << 32) | (M_SETHI_G5 | H44(nsym));
	} else {
		pc[6] = M_JMPL_G5G0 | LO10(sym);
		pc[5] = M_OR_G1G5G5;
		pc[4] = M_SLLX_G132G1;
		pc[3] = M_OR_G1G1 | HM10(sym);
		pc[2] = M_SETHI_G5 | LM22(sym);
		/*LINTED*/
		*(ulong_t *)pc =
			((ulong_t)M_NOP << 32) | (M_SETHI_G1 | HH22(sym));
	}
}
...
2:	or	%l7, %lo(_GLOBAL_OFFSET_TABLE_ - (1b - .)), %l7
	...
	be,pt	%icc, .start_pltenter
	 ldx	[%l1 + SYMDEF_OFF], %l0		! l0 = sym.st_value(calling address)
...
.start_pltexit:
	 ...
	 * cover both.
	 */
	ldx	[%fp + STACK_BIAS + -(2 * CLONGSIZE)], %l1	! %l1 = callers stack size
	sub	%l1, MINFRAME, %l1		! %l1 = argument space on caller's
						!	stack
	/*
	 * Next we compare the prev. stack size against the
	 * audit_arg_count.  We copy at most 'audit_arg_count'
	 * arguments.  The default arg count is 64.
	 *
	 ...
	 */
	ldx	[%l7 + audit_arg_count], %l2
	lduw	[%l2], %l2			! %l2 = audit_arg_count
	cmp	%l2, 6

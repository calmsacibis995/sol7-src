 ...
 *	  All Rights Reserved
 */
#pragma ident	"@(#)a.out.c	1.41	98/01/21 SMI"
...
static Pnode		aout_secure_dirs[] = {
	{ MSG_ORIG(MSG_PTH_USR4LIB),	MSG_PTH_USR4LIB_SIZE,
		0,			&aout_secure_dirs[1] },
	{ MSG_ORIG(MSG_PTH_USRLIB),	MSG_PTH_USRLIB_SIZE,
		0,			&aout_secure_dirs[2] },
	{ MSG_ORIG(MSG_PTH_USRUCBLIB),	MSG_PTH_USRUCBLIB_SIZE,
		0,			&aout_secure_dirs[3] },
	{ MSG_ORIG(MSG_PTH_USRLCLIB),	MSG_PTH_USRLCLIB_SIZE,
		0,			0 }
};
 ...
 */
static int
aout_needed(Lm_list * lml, Rt_map * lmp)
{
	Rt_map	*	nlmp;
	void *		need;
	const char *	name;
		...
		if (((Lnk_obj *)(need))->lo_library) {
			...
			if (tracing)
				...
				for (dir = get_next_dir(&dirlist, lmp); dir;
					...
					if (path = (LM_GET_SO(lmp)(dir->p_name,
					    file)))
						break;
				}
				if (!path) {
...
aout_dlsym(Dl_obj * dlp, Rt_map * clmp, const char * name, Rt_map ** _lmp)
{
	...
	do {
		...
		if (dlp->dl_magic == DL_DLOPEN_0) {
			for (tlmp = LIST(clmp)->lm_head; tlmp;
				...
				if (!(MODE(tlmp) & RTLD_GLOBAL))
					continue;
				sl.sl_imap = tlmp;
				if (sym = LM_LOOKUP_SYM(clmp)(&sl,
				    _lmp, (LKUP_DEFT | LKUP_FIRST)))
					return (sym);
			}
		} else {
		 ...
		 * will be in the "C" name space, where the implementation
		 * prepends a "_" to the symbol as it emits it.  Therefore,
		 * attempt to find the symbol with the "_" prepend.
		 */
		buffer[0] = '_';
		(void) strcpy(&buffer[1], name);

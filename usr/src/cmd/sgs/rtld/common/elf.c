 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)elf.c	1.94	98/02/02 SMI"
...
#ifdef _ELF64
	{ MSG_ORIG(MSG_PTH_USRLIB64),	MSG_PTH_USRLIB64_SIZE,	0,	0 }
#else
...
elf_fix_name(const char * name, Rt_map * lmp)
{
	 ...
	 */
#ifdef _ELF64
	if ((strcmp(name, MSG_ORIG(MSG_PTH_RTLD_64)) == 0) ||
#else
...
/*
 * elf_lazyload() has two return modes.
 *
 * Return: 0
 *	*symp is set to symbol.  This occurs when symbol is bound
 *	to a COPY reloc and/or the referencing object is bound
 *	with 'DIRECT BINDINGS' enabled.
 * Return 1:
 *	lazy loaded object has been sucessfully loaded.  symbol rezolution
 *	may continue as normal.
 *
 * Failure:
 *	A failure is noted by:
 *
 *		(Return == 0) && (*symp == 0)
 *
 */
int
elf_lazyload(uint_t boundto, Slookup * slp, Rt_map ** dlmp, Sym ** symp,
	int flag)
{
	...
	nlmp = (Rt_map *)LIST(clmp)->lm_head;
	hash = elf_hash(name);


	/*
	 * Alwasy check the head of the link-map for the
	 * symbol in question.  This is for the possible 'COPY_RELOC'
	 * symbols.
	 */
	if (!(flag & LKUP_COPY) &&
	    (sym = SYMINTP(nlmp)(name, nlmp, dlmp, (flag & ~LKUP_SPEC),
	    hash))) {
		*symp = sym;
		return (0);
	}
	...
	if ((flag & LKUP_NOINT) == 0) {
		for (LIST_TRAVERSE(&(LIST(clmp)->lm_interpose), lnp, nlmp)) {
			if (sym = SYMINTP(nlmp)(name, nlmp, dlmp,
			    (flag & ~LKUP_SPEC), hash)) {
				*symp = sym;
				return (0);
			}
	...
	if (boundto >= SYMINFO_BT_LOWRESERVE) {
		if (boundto == SYMINFO_BT_SELF)
			nlmp = clmp;
		else if ((boundto == SYMINFO_BT_PARENT) && PARENT(clmp))
			nlmp = PARENT(clmp);
		else
			return (0);
	} else {
		Dyninfo *	dip = &DYNINFO(clmp)[boundto];
		/*
		 * If this object is to be LAZYLOADED, then
		...
		if ((nlmp = dip->di_lmp) == 0) {
			...
			soname = (const char *)STRTAB(clmp) +
			    dip->di_dyn->d_un.d_val;

			if ((nlmp = is_so_loaded(LIST(clmp), soname)) == 0) {
				Rt_map **	tobj;
				int		mode;

				mode = MODE(clmp);
				DBG_CALL(Dbg_file_lazyload(soname, NAME(clmp),
					name));
				...
				if ((rtld_flags & RT_FL_LAZYLDING) == 0) {
					lazyloading++;
					rtld_flags |= RT_FL_LAZYLDING;
				}

				if (dl_new_so(LIST(clmp), soname, clmp,
				    &nlmp, mode) == 0)
					return (0);
				...
				/*
				 * Only call .init's if ld.so.1 has
				 * already done it's initial pass.
				 * Otherwise ld.so.1 will catch these
				 * init's along with all the others.
				 */
				if ((rtld_flags & RT_FL_APPLIC) &&
				    ((rtld_flags & RT_FL_LAZYLDING) == 0)) {
					if ((tobj = tsort((Rt_map *)nlmp,
					    RT_SORT_REV)) ==
					    (Rt_map **)S_ERROR)
						return (0);
			...
			if (nlmp && (bound_add(REF_DIRECT, clmp, nlmp) == 0))
				return (0);
			dip->di_lmp = nlmp;
		}
	}
	/*
	 * Was the caller built with direct_bindings.  If so
	 * find our symbol and return it now.
	 */
	if (FLAGS(clmp) & FLG_RT_DIRECT) {
		*symp = SYMINTP(nlmp)(name, nlmp, dlmp, flag, hash);
		return (0);
	}
	return (1);
}
		...
		for (vnap = (Vernaux *)((Xword)vnd + vnd->vn_aux); cnt;
			...
			if (tracing) {
				if (found) {
				    if (rtld_flags & RT_FL_VERBOSE)
					(void)
					    printf(MSG_ORIG(MSG_LDD_VER_FOUND),
...
elf_needed(Lm_list * lml, Rt_map * lmp)
{
	...
	for (lazyload = 0, dynndx = 0, need = (Dyn *)DYN(lmp);
	    need->d_tag != DT_NULL; need++, dynndx++) {
		Dyninfo *	dip;

		dip = DYNINFO(lmp) ? &DYNINFO(lmp)[dynndx] : 0;
		switch (need->d_tag) {
		case DT_POSFLAG_1:
			if ((need->d_un.d_val & DF_P1_LAZYLOAD) &&
			    !(rtld_flags & RT_FL_NOLAZYLD))
				lazyload = 1;
			if (need->d_un.d_val & DF_P1_GROUPPERM)
				groupperm = 1;
			continue;
		case DT_NEEDED:
			if (dip) {
				...
				/*
				 * If object is to be lazyloaded, it is not
				 * loaded now.
				 */
				if (lazyload) {
					lazyload = groupperm = 0;
					continue;
				}
			}
			break;
		case DT_USED:
			...
			/*
			 * Only load here if BIND_NOW has been set.
			 */
			if ((MODE(lmp) & RTLD_NOW) == 0) {
				lazyload = groupperm = 0;
				continue;
			}
			break;
		...
		/*
		 * setup syminfo->si_boundto lookup cache.
		 */
		if (dip)
			dip->di_lmp = nlmp;
		 ...
		 */
			...
			if ((sip = SYMINFO(lmp)) && DYNINFO(lmp)) {
				...
				sip = (Syminfo *)((unsigned long)sip +
				    ndx * SYMINENT(lmp));
				dip = &DYNINFO(lmp)[sip->si_boundto];
				if ((sip->si_flags & SYMINFO_FLG_DIRECT) &&
				    (sip->si_flags & SYMINFO_FLG_PASSTHRU)) {
					Rt_map *	blmp;
					if ((blmp = dip->di_lmp) == 0) {
						const char *	soname;
						soname = (const char *)
						    STRTAB(lmp) +
						    dip->di_dyn->d_un.d_val;

						blmp = is_so_loaded(LIST(lmp),
							soname);
						if (!blmp) {
							DBG_CALL(
							    Dbg_file_lazyload(
					...
					return (elf_find_sym(ename, blmp,
					    dlmp, flag, hash));
				}
			}
			*dlmp = lmp;
	...
	int		fixed)
{
	...
	size_t		plen;		/* leading padding for image */
	int		mflag;
	...
	for (any = 0, pnp = FILTEES(lmp); pnp; pnp = pnp->p_next) {
		...
		if (pnp->p_info == 0) {
			...
			if (rlmp) {
				...
				/*
				 * Indicate that filtee processing is underway.
				 * This allows load_so() to discriminate between
				 * issuing an error message should an auxiliary
				 * filters' filtee not be found.
				 */
				FLAGS(lmp) |= FLG_RT_FLTRING;

				dlp = dl_new_so(LIST(lmp), filter, lmp,
				...
				if (dlp && (rtld_flags & RT_FL_APPLIC)) {
					...
					if ((tobj = tsort((Rt_map *)pnp->p_info,
					    RT_SORT_REV)) ==
					    (Rt_map **)S_ERROR)
						return ((Sym *)0);
					if (tobj != (Rt_map **) NULL)
    ...
    unsigned long paddr, unsigned long padimsize)
{
	...
	if (ld) {
		 ...
		 */
		if ((DYNINFO(lmp) = (Dyninfo *)calloc(dyncnt,
		    sizeof (Dyninfo))) == 0)
			return (0);
	 ...
	 * Validity check.
	 */
	if ((rtld_flags & RT_FL_APPLIC) && (FLAGS(lmp) & FLG_RT_NOOPEN)) {
		/*
		 * The DF_1_NOOPEN object is being dlopen() by the
		 * application.
		 */
		eprintf(ERR_FATAL, MSG_INTL(MSG_GEN_NOOPEN), NAME(lmp));
		return (0);
	}
...
elf_map_so(Lm_list * lml, const char * pname, const char * oname)
{
	...
	if (interp && !(strcmp(name, interp->i_name))) {
		...
		/* LINTED */
		phdr = (Phdr *)((char *)faddr + ehdr->e_ehsize);
	...
	/*
	 * If 'auditing' is enabled then call 'la_objopen()' entrypoint
	 * now.
	 */
	if ((rtld_flags & RT_FL_AUDITING) &&
	    !(LIST(lmp)->lm_flags & LML_FLG_NOAUDIT)) {
		if (audit_objopen(lmp) == 0) {
			(void) munmap((caddr_t)faddr, memsize);
			return (0);
		}
	}
	...
	if (dlp->dl_magic == DL_DLOPEN_0) {
		for (tlmp = LIST(clmp)->lm_head; tlmp;
			...
			if (!(MODE(tlmp) & RTLD_GLOBAL))
				continue;
			sl.sl_imap = tlmp;
			if (sym = LM_LOOKUP_SYM(clmp)(&sl, _lmp,
		...
		int		cnt;
		Slookup		_sl;
		Rt_map *	lmp = (Rt_map *)dlp->dl_depends.head->data;
		_sl.sl_imap = lmp;
		_sl.sl_name = name;
		_sl.sl_permit = 0;
		_sl.sl_cmap = lmp;
		...
		for (cnt = 0, dip = DYNINFO(lmp); cnt < DYNINFOCNT(lmp);
		    cnt++, dip++) {
			if ((dip->di_dyn) &&
			    (dip->di_dyn->d_tag == DT_NEEDED)) {
				if (dip->di_lmp == 0) {
					if ((elf_lazyload(cnt, &_sl, _lmp,
					    &sym, LKUP_COPY |
					    LKUP_NOINT) == 0) && (sym == 0))
						return (0);
				}
...
elf_are_u_compatible(Word * what)
{
	...
	if (ehdr->e_machine != M_MACH) {
		 ...
		 * (UltraSPARC 1) instructions, anything else
		 * in the e_flags mask is erroneous.
		 */
		if (ehdr->e_flags != EF_SPARC_SUN_US1) {
			*what = ehdr->e_flags;
			return (DBG_IFL_ERR_FLAGS);
		}
#endif /* __sparcv9 */

 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)object.c	1.31	98/01/21 SMI"
...
elf_obj_init(Lm_list * lml, const char * name)
{
	...
	handle = dlused_core(MSG_ORIG(MSG_FIL_LIBLD),
	    (bind_mode | RTLD_GLOBAL | RTLD_WORLD), (Rt_map *)lml_rtld.lm_head);
	if (handle == 0)
		return (0);
...
elf_obj_file(Lm_list * lml, const char * name)
{
	...
	if (!olmp) {
		/*
		 * Load the link-editor library.  Note we switch off
		 * tracing during this loading so that an ldd(1) of an
		 * object preload only shows the user the objects that
		 * they care about.
		 */
...
elf_obj_fini(Lm_list * lml, Rt_map * lmp)
{
	...
	if ((rtld_flags & RT_FL_AUDITING) &&
	    !(LIST(nlmp)->lm_flags & LML_FLG_NOAUDIT)) {

...
#pragma ident	"@(#)_rtld.h	1.79	98/02/02 SMI"
...
#define	RT_FL_AUDITING	0x00080000	/* auditing is active */
...
#define	RT_FL_NODIRECT	0x04000000	/* disable direct bindings & */
					/*	lazyloading of objects */
#define	RT_FL_NOLAZYLD	0x08000000	/* disable lazyloading of objects */
#define	RT_FL_LAZYLDING	0x10000000	/* lazyloading in process */
...
#define	RT_SORT_FWD	0		/* topological sort (.fini) */
#define	RT_SORT_REV	1		/* reverse topological sort (.init) */
#define	RT_SORT_UNUSED	0xffffffff	/* Skip this one */

/*
 * Maximum range of Thread_Interface functions that ld.so.1 is interested in.
 */
#define	LC_MAX	11


typedef struct prfbuf {
...
#define	_GET_TIME(x)	x = gethrvtime()
...
extern Sxword		lc_version;	/* current version of libthread int. */

extern List		dynlm_list;	/* dynamic list of link-maps */
extern char **		environ;	/* environ pointer */
extern const char *	environ_name;
...
extern size_t		platform_sz; 	/* platform string size */
extern int		rtld_flags;	/* status flags for RTLD */
extern size_t		cachelinesz;	/* cache line size for ppc */
...
extern Dl_obj *		dlp_create(Rt_map *, Rt_map *, int, int);
...
extern Dl_obj *		dl_new_so(Lm_list *, const char *, Rt_map *,
				Rt_map **, int);
extern void *		dlmopen_core(Lm_list *, const char *, int, Rt_map **,
				Rt_map *, ulong_t);
...
extern int		expand(const char **, size_t *, Rt_map *, int);
extern Fmap *		fm_init();
...
extern Listnode *	list_appafter(List *, void *, void *);
...
extern Rt_map ** 	tsort(Rt_map *, int);
extern void		zero(caddr_t, size_t, int);

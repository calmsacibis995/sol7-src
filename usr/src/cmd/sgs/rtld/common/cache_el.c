 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)cache_elf.c	1.10	97/03/04 SMI"
...
elf_cache(const char ** file)
{
	...
	if (cd_dir && !(rtld_flags & RT_FL_SECURE)) {
		(void) snprintf(path, PATH_MAX, MSG_ORIG(MSG_FMT_PATH), cd_dir,
	 ...
	 * integrity
	 */
	cachehead = (Rtc_head *)((Xword)ehdr + ehdr->e_entry);
	 ...
	 */
	if (cachehead->rtc_base != (Xword)cachehead) {
		(void) close(fd);
		(void) munmap((caddr_t)ehdr, status.st_size);
	...
	if (cachehead->rtc_objects != 0) {
		...
		if (mmap((caddr_t)cachehead->rtc_begin,
		    (size_t)((Xword)cachehead->rtc_end -
		    (Xword)cachehead->rtc_begin), PROT_READ,

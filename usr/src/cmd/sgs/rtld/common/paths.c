 ...
 *	All Rights Reserved.
 */
#pragma ident	"@(#)paths.c	1.47	98/01/05 SMI"
 ...
 */
int
expand(const char ** name, size_t * len, Rt_map * lmp, int new)
{
	char		_name[PATH_MAX];
	const char *	optr, * _optr;
	char *		nptr;
	size_t		olen = 0, nlen = 0, nrem = PATH_MAX, _len;
	int		expanded = 0, _expanded;
	...
	while ((olen < *len) && nrem) {
		...
		if (strncmp(optr, MSG_ORIG(MSG_TKN_ORIGIN),
			 ...
			 */
			if ((platform == (char *)0) && (platform_sz !=
			    (size_t)-1)) {
				char	_info[SYS_NMLN];

				platform_sz = (int)sysinfo((int)SI_PLATFORM,
				    _info, SYS_NMLN);
				if ((platform_sz != (size_t)-1) &&

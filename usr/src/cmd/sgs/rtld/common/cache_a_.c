 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)cache_a.out.c	1.18	97/04/01 SMI"
	...
	const char *file;
{
	...
	if ((liblen = extract_name(&n)) == -1)
		return (NULL);
	if ((libname = (char *)malloc(liblen + 1)) == 0)
		return (NULL);
...
find_so(const char * ds)
{
	 ...
	 * and initialize it.
	 */
	dbp = (struct db *)calloc(sizeof (struct db), 1);
	...
	struct db *dbp;			/* db associated with this dbd */
{
	...
	dbdp = (struct dbd *)malloc(sizeof (struct dbd));
	dbdp->dbd_db = dbp;
	dbdp->dbd_next = NULL;
	*dbdpp = dbdp;
	return (dbdp);

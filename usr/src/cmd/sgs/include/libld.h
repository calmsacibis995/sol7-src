 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)libld.h	1.75	98/02/09 SMI"
...
struct ofl_desc {
	...
	const char *	ofl_filtees;	/* shared objects we are a filter for */
	...
	const char *	ofl_cache;	/* cache path to store in .dynamic */
...
#define	FLG_OF_WARN	0x40000000	/* warning durring input processing. */
...
#define	FLG_OF1_GRPPRM	0x00000400	/* dependencies are to have */
					/*	GROUPPERM enabled */
#define	FLG_OF1_OVRFLW	0x00000800	/* size excedes 32-bit limit */
					/*	limitation of 32-bit libld */
...
#define	FLG_IF_USED	0x004		/* record shared object as DT_USED */
...
struct os_desc {			/* Output section descriptor */
	...
	List		os_comdats;	/* list of COMDAT sections allready */
...
struct sym_desc {
	...
	Xword		sd_GOTndx;	/* index into GOT for symbol */
...
#define	FLG_SDV_MATCHED	0x01		/* VERDEF vound and matched */
 ...
 */
extern const char *	add_string(const char *, char *);
extern Xword		create_outfile(Ofl_desc *);
extern Elf64_Off	_elf_getxoff(Elf_Data *);
extern Xword		ent_setup(Ofl_desc *);
extern void		eprintf(Error, const char *, ...);
extern Xword		finish_libs(Ofl_desc *);
extern void *		libld_malloc(size_t);
extern Xword		ld_support_loadso(const char *);
...
extern Xword		make_sections(Ofl_desc *);
extern void		ofl_cleanup(Ofl_desc *);
extern Xword		open_outfile(Ofl_desc *);
extern Ifl_desc *	process_open(const char *, Xword, int, Ofl_desc *,
			    Half, int *);
extern Xword		reloc_init(Ofl_desc *);
extern Xword		reloc_process(Ofl_desc *);
...
extern Xword		sym_validate(Ofl_desc *);
extern Addr		update_outfile(Ofl_desc *);
extern Ver_desc *	vers_base(Ofl_desc *);
extern Xword		vers_check_defs(Ofl_desc *);

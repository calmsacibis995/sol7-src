 ...
 */
#pragma ident	"@(#)sgs.h	1.24	98/01/05 SMI"
 ...
 */
#define	S_ALIGN(x, a)	((Sxword)(x) & ~(((Sxword)(a) ? (Sxword)(a) : 1) - 1))
#define	S_ROUND(x, a)   ((Sxword)(x) + (((Sxword)(a) ? (Sxword)(a) : 1) - 1) & \
				~(((Sxword)(a) ? (Sxword)(a) : 1) - 1))
...
#define	S_ABS(x)	(((Sxword)(x) < 0) ? -((Sxword)(x)) : (x))
...
#define	S_ERROR		(~(unsigned long)0)
...
struct	listnode32 {			/* a node on a linked list */
	uint32_t	data;		/* the data item */
	uint32_t	next;		/* the next element */
};

struct	list32 {			/* a linked list */
	uint32_t	head;		/* the first element */
	uint32_t	tail;		/* the last element */
};
#endif	/* _SYSCALL32 */

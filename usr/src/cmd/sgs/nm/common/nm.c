...
#pragma ident	"@(#)nm.c	6.32	98/01/05 SMI"
	...
	GElf_Shdr *shdr, char *filename)
{
	 ...
	 */
	if ((sd = elf_getdata(p_sd, NULL)) == NULL) {
		(void) printf(gettext(
...
{
	char *sym_key	= NULL;
	int class	= (int)ehdr->e_ident[EI_CLASS];
	...
	if (x_flag) {
		if (class == ELFCLASS32)
			(void) printf("0x%.8llx ", sym_data->value);
		else
			(void) printf("0x%.16llx ", sym_data->value);
	} else if (o_flag) {
		if (class == ELFCLASS32)
			(void) printf("0%.11llo ", sym_data->value);
		else
			(void) printf("0%.22llo ", sym_data->value);
	} else {
		if (class == ELFCLASS32)
			(void) printf("%.10llu ", sym_data->value);
		else
			(void) printf("%.18llu ", sym_data->value);
	}
...
#define	SYM_LEN 10
	...
	if (d_flag) {
		if (class == ELFCLASS32)
			...
			(void) printf("%.18llu %.18llu ",
				sym_data->value, sym_data->size);
	} else if (o_flag) {
		if (class == ELFCLASS32)
			(void) printf("%.11llo %.11llo ",
				sym_data->value, sym_data->size);
		else
			(void) printf("%.22llo %.22llo ",
				sym_data->value, sym_data->size);
	} else {	/* Hex and it is the default */
		if (class == ELFCLASS32)
			(void) printf("%.8llx %.8llx ",
				sym_data->value, sym_data->size);
		else
			(void) printf("%.16llx %.16llx ",
				sym_data->value, sym_data->size);
	}
	(void) putchar('\n');
}
...
)
{
	...
	if (o_flag) {
		if (class == ELFCLASS32)
			(void) printf("0%.11llo|0%.11llo|",
				sym_data->value,
				sym_data->size);
		else
			(void) printf("0%.22llo|0%.22llo|",
				sym_data->value,
				sym_data->size);
	} else if (x_flag) {
		if (class == ELFCLASS32)
			(void) printf("0x%.8llx|0x%.8llx|",
				sym_data->value,
				sym_data->size);
		else
			(void) printf("0x%.16llx|0x%.16llx|",
				sym_data->value,
				sym_data->size);
	} else {
		if (class == ELFCLASS32)
			(void) printf("%10llu|%8lld|",
				sym_data->value,
				sym_data->size);
		else
			(void) printf("%18llu|%16lld|",
				sym_data->value,
				sym_data->size);
	}

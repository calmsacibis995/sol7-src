 ...
 * All rights reserved.
 */
#pragma ident	"@(#)ld.c	1.15	98/01/21 SMI"
...
static int	type_input_file(int fd);
static int	ldf_process_files(int argc, char ** argv);
static Xword	ldf_prepend_argv(char *, int *, char ***);
...
main(int argc, char ** argv)
{
	int		orig_argc = argc;
	char **		orig_argv = argv;
	void *		handle;		/* dlopen handle to libld */
	...
	if ((ld_options = getenv(MSG_ORIG(MSG_LD_OPTIONS))) != NULL) {
		...
		if (ldf_prepend_argv(ld_options, &argc, &argv) == S_ERROR)
			exit(EXIT_FAILURE);
	}
	 ...
	 */
	if ((type = ldf_process_files(argc, argv)) == ELFCLASSNONE)
		type = ELFCLASS32;	/* compatibility */
	 ...
	 */
	if (type == ELFCLASS64)
		handle = dlopen(MSG_ORIG(MSG_LD_LIB64),
				RTLD_LAZY | RTLD_GLOBAL);
	else
		handle = dlopen(MSG_ORIG(MSG_LD_LIB32),
				RTLD_LAZY | RTLD_GLOBAL);

	if (handle == NULL) {
		eprintf(ERR_FATAL, dlerror(), MSG_ORIG(MSG_STR_EMPTY));
		exit(EXIT_FAILURE);
	}
	 ...
	 */
	main_ptr = (void (*)(int, char **)) dlsym(handle,
 ...
 */
static Xword
ldf_prepend_argv(char * ld_options, int * argcp, char *** argvp)
{
	int	nargc;			/* New argc */
	char **	nargv;			/* New argv */
	char *	arg, * string;
 ...
 */
static int
ldf_process_files(int argc, char ** argv)
{
...
/*
 * XX64:	hack alert!  Interposition of malloc is broken
 *		because libld is now dlopen'd.  This stuff must
 *		be moved to 'ld' proper to avoid bad free's.
 */
...
libld_malloc(size_t size)
{
	...
	if ((ldhp == 0) ||
		...
		Ld_heap *	new_ldhp;
		size_t	ldhpsz = S_ROUND(sizeof (Ld_heap), HEAPALIGN);
		...
		new_ldhp->lh_free = vptr = (void *)((unsigned long)new_ldhp +
			ldhpsz);
		new_ldhp->lh_end = (void *)((Xword)new_ldhp + alloc_size);

...
#pragma ident	"@(#)debug.c	6.10	97/12/30 SMI"
...
static void not_interp(short);
static void user_def_type();
static void mod_fund_type();
static void location();
static void mod_u_d_type();
...
static void print_line(long size, char * filename);

/*
 * Get the debugging data and call print_record to process it.
 */
void
dump_debug(char * filename)
{
	extern SCNTAB *	p_debug;
	size_t		dbg_size;
	...
	if ((p_debug_data = (unsigned char *)
	    get_scndata(p_debug->p_sd, &dbg_size)) == NULL) {
...
lookuptag(short tag)
{
	...
	case TAG_global_subroutine:
		return ("global subrtn");
	case TAG_global_variable:
		return ("global var");
	case TAG_imported_declaration:
		return ("imported decl");
	case TAG_inline_subroutine:
		return ("inline subrtn");
	case TAG_label:
		return ("label");
...
lang(long l)
{
	...
	switch (l) {
	default:
		(void) sprintf(buf, "<LANG_0x%lx>", l);
		return (buf);
	case LANG_UNK:
		return ("LANG_UNK");
	case LANG_ANSI_C_V1:
		return ("LANG_ANSI_C_V1");
	}
}
...
order(short a)
{
	...
	switch (a) {
	case ORD_row_major:
		return ("ORD_row_major");
	case ORD_col_major:
		return ("ORD_col_major");
	default:
		(void) sprintf(buf, "<OP_0x%x>", a);
		return (buf);
	}
...
dump_line(char *filename)
{
	...
	if ((p_line_data = (unsigned char *)
	    get_scndata(p_line->p_sd, &l_size)) == NULL) {
		(void) fprintf(stderr, "%s: %s: no data in %s section\n",
	...
	ptr = p_line_data;
	print_line((long)l_size, filename);
}
...
static void
print_line(long size, char * filename)
{
	long  line;
	long  pcval;
	long  base_address;
	...
	while (size > 0) {
		...
		if (size < length-4) {
			(void) fprintf(stderr,
"%s: %s: bad line info section -  size=%ld length-4=%ld\n",

...
#pragma ident	"@(#)dump.c	6.37	98/01/09 SMI"
...
"Offset      Symndx                Type            Addend\n\n");
		...
		(void) printf("%-#12llx", rela.r_offset);
		if (!v_flag) {
...
"%s: %s: invalid symbol table offset - %d - in %s\n",
		...
		(void) printf("%-#12llx", rel.r_offset);
		if (!v_flag) {
...
demangled_name(char * s)
{
	...
	buf = (char *)malloc(strlen(dn) + 1 + 1 + strlen(s) + 1 + 1);
	if (buf == NULL)
		return (s);
	...
	int range, int index)
{
	...
	while (range > 0) {
		...
		(void) printf("[%d]\t ", index++);
		(void) printf("0x%-10llx", sym.st_value);
		(void) printf("%-9lld", sym.st_size);

		if (!v_flag) {
			(void) printf("%d\t\t%d\t%d\t%#x\t",
			    type, bind, (int)sym.st_other, (int)sym.st_shndx);
		} else {
			switch (type) {
			case STT_NOTYPE:
				(void) printf("%s\t", "NOTY");
			...
			(void) printf("\t  %d\t", (int)sym.st_other);
			...
			case SHN_COMMON:
				...
				break;
			default:
				(void) printf("%d", (int)sym.st_shndx);
			}
			(void) printf("\t");
		}
...
print_shdr(SCNTAB *s, int num_scns, int index)
{
	...
	for (num = 0; num < num_scns; num++, p++) {
		...
		if (!v_flag) {
			(void) printf("%u\t%llu\t",
			p->p_shdr.sh_type, p->p_shdr.sh_flags);
		} else {
			...
			case SHT_SUNW_COMDAT:
				...
				break;
			default:
				(void) printf("%u", p->p_shdr.sh_type);
				break;
			}
			(void) printf("    ");
		...
		(void) printf("%-#13llx%-#13llx%-#13llx\t%s\n",
			p->p_shdr.sh_addr,
			p->p_shdr.sh_offset,
			p->p_shdr.sh_size,
			p->scn_name);

		(void) printf("\t%u\t%u\t%-#13llx%-#13llx\n\n",
			p->p_shdr.sh_link,
			p->p_shdr.sh_info,
	...
	SCNTAB *p_scns, int num_scns, char *filename)
{
	...
	if ((p_scns->p_shdr.sh_link < 1) || (p_scns->p_shdr.sh_link >
	    p_ehdr->e_shnum)) {
		(void) fprintf(stderr, "%s: %s: invalid sh_link field: "
			"section #: %d sh_link: %d\n",
			prog_name, filename, elf_ndxscn(p_scns->p_sd),
			p_scns->p_shdr.sh_link);
		return;
	}
		...
		while (p_dyn.d_tag != DT_NULL) {
			...
			case (DT_USED):
				...
				if (v_flag && strlen(dt_name))
					...
					(void) printf(Fmtptr, p_dyn.d_un.d_ptr);
				break;
			case (DT_PLTRELSZ):
				(void) printf(Fmttag, (const char *)"PLTSZ");
				(void) printf(Fmtptr, p_dyn.d_un.d_val);
				break;
			case (DT_PLTGOT):
...
dump_elf_header(Elf *elf_file, char *filename, GElf_Ehdr * elf_head_p)
{
	...
	if (!v_flag) {
		(void) printf("%-13d%-11d%-13d%-12d%d\n",
			elf_head_p->e_ident[4],
			elf_head_p->e_ident[5],
			(int)elf_head_p->e_type,
	...
	(void) printf("%-#13llx%-#11llx%-#13llx%-#12x%#x\n",
		elf_head_p->e_entry,
		elf_head_p->e_phoff,
		elf_head_p->e_shoff,
		elf_head_p->e_flags,
...
dump_section_table(Elf *elf_file, GElf_Ehdr *elf_head_p, char *filename)
{
	...
	while ((scn = elf_nextscn(elf_file, scn)) != 0) {
		...
		s_name = (char *) elf_strptr(elf_file,
			elf_head_p->e_shstrndx, buffer->p_shdr.sh_name);
	...
	if (p_debug && D_flag) {
		/* no Elf64 stabs yet... */
		if (elf_head_p->e_ident[EI_CLASS] == ELFCLASS32)
			dump_debug(filename);
	}
	if (p_line && l_flag) {
		dump_line(filename);

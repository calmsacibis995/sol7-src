 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)profile.c	1.36	98/01/27 SMI"
...
la_version(uint_t version)
{
	if (version != LAV_CURRENT)
		fprintf(stderr, MSG_INTL(MSG_GEN_AUDITVERSION),
			LAV_CURRENT, version);

	if ((pname = getenv(MSG_ORIG(MSG_LD_PROFILE))) == NULL)
		fprintf(stderr, MSG_INTL(MSG_GEN_PROFNOTSET));
...
profile_open(const char * fname, Link_map * lmp)
{
	...
	if (*Profile == '\0') {
		...
		if ((profile_dir = getenv(MSG_ORIG(MSG_LD_PROFOUT))) == 0)
			profile_dir = MSG_ORIG(MSG_PTH_VARTMP);

		(void) sprintf(Profile, MSG_ORIG(MSG_FMT_PROFILE), profile_dir,
#if	defined(_LP64)
		    fname, MSG_ORIG(MSG_SUF_PROFILE_64));
#else
		    fname, MSG_ORIG(MSG_SUF_PROFILE));
#endif
	}

	if ((fd = open(Profile, (O_RDWR | O_CREAT), 0666)) == -1) {
		err = errno;
		fprintf(stderr, MSG_INTL(MSG_SYS_OPEN), Profile,
		    strerror(err));
		return (0);
	}

	/*
	 * Now we determine the valid pc range for this object.
	 * The lpc is easy (lmp->l_addr), to determine the hpc
	 * we must exmaine the Phdrs.
	 */
	lpc = hpc = (caddr_t)lmp->l_addr;
	/* LINTED */
	ehdr = (Ehdr *)lpc;
	/* LINTED */
	for (i = 0, phdr = (Phdr *)(ehdr->e_phoff + lpc);
		...
		if (phdr->p_type == PT_DYNAMIC)
			/* LINTED */
			dynp = (Dyn *)(lpc + phdr->p_vaddr);
	}


	/*
	 * In order to determine the number of symbols in the object
	 * we scan the dynamic section until we find the DT_HASH
	 * entry (hash[1] == symcnt).
	 */
	for (; dynp->d_tag != DT_NULL; dynp++)
		if (dynp->d_tag == DT_HASH) {
			/* LINTED */
			unsigned int *	hashp = (unsigned int *)
				(dynp->d_un.d_ptr + lpc);
			nsym = hashp[1];
			break;
		}
	 ...
	 */
	Lpc = lpc = (caddr_t)PRF_ROUNDWN(lpc, sizeof (long));
	Hpc = hpc = (caddr_t)PRF_ROUNDUP(hpc, sizeof (long));
	...
	if (status.st_size == 0) {
		if (ftruncate(fd, Fsize) == -1) {
			err = errno;
			fprintf(stderr, MSG_INTL(MSG_SYS_FTRUNC), Profile,
			    strerror(err));
			(void) close(fd);
			return (0);
		}
	...
	if ((addr = (caddr_t)mmap(0, Fsize, (PROT_READ | PROT_WRITE),
		...
		err = errno;
		fprintf(stderr, MSG_INTL(MSG_SYS_MMAP), Profile,
		    strerror(err));
		(void) close(fd);
		return (0);
	}
	...
	if (Hptr->hd_hpc) {
		if (Hptr->hd_hpc != (caddr_t)(hpc - lpc)) {
			fprintf(stderr, MSG_INTL(MSG_GEN_PROFSZCHG),
			    Profile);

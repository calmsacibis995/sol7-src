 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)stab.c	1.17	97/07/11 SMI"
 ...
 */
static	Sbld_rec	sb_data = { NULL, 0 };

/*
 * holds information out the output file being created.
 */
static const char *	out_fname = NULL;
static const char *	in_fname = NULL;	/* current input file */
static Half 		out_e_type = ET_NONE;
    ...
    const char * symbol)
{
	...
	if (data->fd == NULL) {
		...
		switch (fork()) {
		case -1:
			eprintf(ERR_WARNING, MSG_INTL(MSG_SYS_FORK),
		...
		case 0:
			...
			eprintf(ERR_WARNING, MSG_INTL(MSG_SYS_EXEC),
			    in_fname, MSG_ORIG(MSG_STR_SBFOCUS),
			    strerror(errno), MSG_INTL(MSG_STAB_NOSBROW));
			exit(-1);
...
pass1_stabindex(const Elf_Data *s_data, const Elf_Data *str_data,
		const int cwd_len, const int name_len)
{
	struct nlist	* elem,
			* last = NULL;
	int		i,
...
pass2_stabindex(Elf_Data *s_data, Elf_Data *str_data, const char *name,
		int name_len, int cwd_pos, int free_pos)
{
	struct nlist	* elem,
			* last = NULL;
	int		i,
	...
	for (i = 0; i < num_elem; i++) {
		...
		case N_OBJ:
			...
			if ((*str == '\0') && first_object) {
				 ...
				 */
				elem->n_un.n_strx = cwd_pos - str_offset;
				first_object = 0;
			} /* if */
			else if (*str == '\0') {
				/*
				...
				(void) strcpy((char *)str_data->d_buf +
					free_pos, name);
				elem->n_un.n_strx = free_pos - str_offset;
				free_pos += name_len + 1;
				first_object = 1;
			} /* if */
			break;
	...
	last->n_value = str_data->d_size - str_offset;
...
find_scn(Elf *elf, const char *elf_strtab, const char *name,
	const Word sh_type, const Word sh_flags, Elf_Scn **ret_scn)
{
	Elf_Scn *	scn = NULL;
	Shdr *		scn_shdr;
...
get_str_data(Elf * elf, const char * strtab, const char * name, Shdr * shdr)
{
	...
	if (shdr->sh_link == 0) {
		...
		if (strscn_ndx == 0) {
			eprintf(ERR_FATAL, MSG_INTL(MSG_STAB_MISTBL), in_fname);
			return ((Elf_Data *)S_ERROR);
		} else if (strscn_ndx == (size_t) -1) {
			eprintf(ERR_FATAL, MSG_INTL(MSG_STAB_BADTBL), in_fname);
			return ((Elf_Data *)S_ERROR);
		}
	} else {
		if ((str_scn = elf_getscn(elf, shdr->sh_link)) == NULL) {
			eprintf(ERR_ELF, MSG_ORIG(MSG_ELF_GETSCN), in_fname);
			return ((Elf_Data *)S_ERROR);
		}
	}

	if ((str_data = elf_getdata(str_scn, NULL)) == NULL) {
		eprintf(ERR_ELF, MSG_ORIG(MSG_ELF_GETDATA), in_fname);
		return ((Elf_Data *)S_ERROR);
	}
    ...
    Shdr * shdr, Elf_Data * s_data)
{
	...
	if (cwd == NULL) {
		if ((cwd = getcwd(NULL, MAXPATHLEN)) == NULL) {
			eprintf(ERR_FATAL, MSG_INTL(MSG_SYS_GETCWD), in_fname);
			return (S_ERROR);
		}
		cwd_len = strlen(cwd);
	}
	 ...
	 */
	if ((data = (Elf_Void *)libld_malloc(s_data->d_size)) == 0)
		return (S_ERROR);
	 ...
	 */
	if ((data = (Elf_Void *)libld_malloc(str_data->d_size + new_size)) == 0)
		return (S_ERROR);
    ...
    const char * out_name, Half etype)
{
	...
	for (i = 0; i < num_elem; i++) {
		...
		case 0:
			...
			break;
		case N_BROWS:
			str = (char *) str_data->d_buf + elem->n_un.n_strx +
			    str_offset;
			if (etype == ET_REL)

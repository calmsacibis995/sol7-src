...
#ident	"@(#)dumpoptr.c 1.53 98/02/18"
...
static u_int	lastprompt;		/* last prompt issued */
	...
	int	arg;
{
	...
	ITITERATE(i, itwalk) {
		...
		if ((arg == 'w') && dumpme) {
			 ...
			 * haven't been backed up within a day.
			 */
		...
		if (arg == 'W') {
			/*
			 * Handle the W option: print out ALL filesystems
			 * including recent dump dates and dump levels.
			 * Mark the backup missing filesystems with a > .
			 */
			(void) printf(gettext(

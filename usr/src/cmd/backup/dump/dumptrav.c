...
#pragma ident	"@(#)dumptraverse.c	1.52	97/06/20 SMI"
...
static offset_t offset;	/* current offset in file */
    ...
    struct dinode *ip;
{
	...
	size = (offset_t)NDADDR * (offset_t)sblock->fs_bsize;
	if (size > ip->di_size)
		size = ip->di_size;

	blksout((daddr_t *)&ip->di_db[0], size);
    ...
    struct dinode *ip;
{
	...
	lf_blksout((daddr_t *)&ip->di_db[0], size);
	...
	offset_t *size;
{
	...
	if (lvl <= 0) {
		cnt = (offset_t)NINDIR(sblock) * (offset_t)sblock->fs_bsize;
		if (cnt > *size)
			cnt = *size;
		*size -= cnt;
		blksout(&idblk[0], cnt);
	...
	offset_t bytes;
{
	...
	if (pos) {
		...
		if (offset < bytepos) {
			 ...
			 */
			diff = bytepos - offset;
			tpblkskip = (int)(howmany(diff, TP_BSIZE));
			blkp += diff / sblock->fs_bsize;

			bytecnt = diff % (offset_t)sblock->fs_bsize;
			byteoff = (off_t)(bytecnt % (offset_t)sblock->fs_fsize);
			tpblkoff = (int)(howmany(bytecnt, TP_BSIZE));
			fragoff = (off_t)(bytecnt / (offset_t)sblock->fs_fsize);
			bytecnt = (offset_t)sblock->fs_bsize - bytecnt;
		}
	}
	...
	while (bytes > 0) {
		if (bytes < TP_NINDIR*TP_BSIZE)
			count = (int)(howmany(bytes, TP_BSIZE));
		else
			count = TP_NINDIR;
		if (tpblkskip) {
			if (tpblkskip < TP_NINDIR) {
				bytes -= ((offset_t)tpblkskip *
				    (offset_t)TP_BSIZE);
				skip = tpblkskip;
				tpblkskip = 0;
			} else {
	...
	offset_t filesize;
{
	...
	bread(fsbtodb(sblock, d), dblk, (long)(roundup(filesize, DEV_BSIZE)));
	loc = 0;
	while (loc < filesize) {

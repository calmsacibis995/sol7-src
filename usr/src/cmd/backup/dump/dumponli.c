...
#ident	"@(#)dumponline.c 1.65 97/03/27"
...
/*
 * Make an address record map of "bombs" for the specified number
 * of bytes.  Returns the number of initialized map entries.
 *
 * This routine's semantics are now identical to a system that
 * does not support the _FIOAI ioctl() which has been removed
 * in Solaris 2.6.
 */
static int
makeaddrs(bytes)

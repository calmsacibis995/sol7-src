...
#pragma ident	"@(#)dumptape.c	1.121	97/12/03 SMI"
...
#endif
{
	...
	if (strchr(tape, ':')) {
		...
		*tape++ = 0;
		...
		if (cp != (char *)0) {
			if (remuser)	/* release previous copy */
				free(remuser);
			remuser = strdup(host);
			remuser[(int) (cp - host)] = 0;
			cp++;
		}
		else
			cp = host;
...
#endif
{
	...
	if (host) {
		...
		if (cp == (char *)0) {
			cp = host;
			if (remuser) {
				rhost = xmalloc(strlen(host) +
						strlen(remuser) + 2);
				sprintf(rhost, "%s@%s", remuser, host);
			}
		}
		else
			cp++;

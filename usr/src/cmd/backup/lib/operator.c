...
#ident	"@(#)operator_lib.c 1.46 97/08/12"
...
#ifdef __STDC__
static rpcprog_t gettransient(rpcprog_t);
 ...
 *	(should be MAXMSGLEN).
 *    seqp - the address of a int (for sequence number)
 *
 * Returns:
 *	-1:	error or no input
...
oper_receive(altfds, buf, len, seqp)
	...
	u_int	*seqp;		/* RETURN: message sequence number */
{
	struct sigvec ignore, savepipe;
	fd_set readfds;

%{
#ifndef lint
static char rcsid[] = "@(#)getdate.y 1.8 97/04/15";
#endif
    ...
    struct timeb	*now;
{
	...
	if (getdate_err == 1 /* NODATEMASK */) {
		...
		fputs(gettext("Enter date as mmddhhmm[yy]: "), stdout);
		fflush(stdout);
	...
	(void)sprintf(localedate, "%d:%2.2d %d/%d %d%d",
	    tm->tm_hour, tm->tm_min, tm->tm_mon + 1,
	    tm->tm_mday, (tm->tm_year < 70) ? 20 : 19, tm->tm_year);
	p = localedate;
    }

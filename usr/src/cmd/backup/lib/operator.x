...
struct msg_dest {
	...
	u_int	md_callback;			/* RPC program, 0 == default */
	time32_t md_gen;				/* generation number */
};
#ifdef RPC_HDR
%
%/*
...
#endif
struct msg_id {
	u_int	mid_pid;		/* pid of sender */
	u_int	mid_seq;		/* sender-generated sequence number */
...
#endif
struct msg_t {
	time32_t msg_time;			/* timestamp */
	time32_t msg_ttl;			/* message's time-to-live */
	...
	u_int	msg_callback;			/* program for RPC callbacks */
	int	msg_auth;			/* auth flavor for callbacks */
	int	msg_uid;			/* user-ID of sender */
	int	msg_gid;			/* group-ID of sender */
	int	msg_level;			/* message's level 0-7 */
	int	msg_type;			/* message's type */
	int	msg_len;			/* length of actual message */
...
%extern CLIENT	*oper_connect(const char *, const rpcprog_t);
%extern u_int	oper_send(const time_t, const int, const int, const char *);
%extern u_int	oper_reply(const char *, const msg_id *, const char *);
%extern u_int	oper_cancel(const u_int, const int);
%extern void	oper_end(void);
%extern int	oper_control(int, caddr_t);
%extern int	oper_receive(const fd_set *, char *, const int, u_int *);
...
%extern u_int	oper_send();
%extern u_int	oper_reply();
%extern u_int	oper_cancel();
...
%#endif
%
%#define	xdr_time32_t	xdr_int /* time_t's are long (ILP32) */
%
%/*
% * Return codes
% */

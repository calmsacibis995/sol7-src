...
#ifndef lint
static char sccsid[] =
	"@(#)symtab.c 1.8 90/11/12 SMI"; /* from UCB 5.1 5/28/85 */
#endif not lint

#ident	"@(#)symtab.c 1.8 97/06/12"
	...
	register struct entry *ep;
{
	...
	panic(gettext("%s: pathname too long\n"), cp);
	return (cp);
}
	...
	int type;
{
	...
	if (freelist != NIL) {
		np = freelist;
		freelist = np->e_next;
		(void) bzero((char *)np, (long)sizeof (struct entry));
	} else {
		np = (struct entry *)calloc(1, sizeof (struct entry));
		if (np == NIL)
			panic(gettext("no memory to extend symbol table\n"));
	}
	np->e_type = type & ~LINK;
	ep = lookupparent(name);
	if (ep == NIL) {
		if (inum != ROOTINO || lookupino(ROOTINO) != NIL)
			panic(gettext("bad name to %s %s\n"), "addentry", name);
	...
	if (type & LINK) {
		ep = lookupino(inum);
		if (ep == NIL)
			panic(gettext("link to non-existant name\n"));
		np->e_ino = inum;
...
static struct strhdr strtblhdr[allocsize(MAXPATHLEN) / STRTBLINCR];
	...
	char *name;
{
	...
	if (np != NULL) {
		...
		if (cp == NULL)
			panic(gettext("no space for string table\n"));
	}
	(void) strcpy(cp, name);
	return (cp);
}
	...
	char *name;
{
	...
	/*LINTED [name points to at least sizeof (struct strhdr *) bytes]*/
	...
	long checkpt;
{
	...
	if ((fd = fopen(filename, "w")) == NULL) {
		perror("fopen");
		panic(gettext("cannot create save file %s for symbol table\n"),
			filename);
	}
	clearerr(fd);
	/*
	...
	char *filename;
{
	...
	if (filename == NULL) {
		...
		if (entry == (struct entry **)NIL)
			panic(gettext("no memory for entry table\n"));
	...
	if ((fd = open64(filename, O_RDONLY)) < 0) {
		perror("open");
		panic(gettext("cannot open symbol table file %s\n"), filename);
	}
	if (fstat64(fd, &stbuf) < 0) {
		perror("stat");
		panic(gettext("cannot stat symbol table file %s\n"), filename);
	}
	/*
	 * The symbol table file is too small so say we can't read it.
	 */
	if (stbuf.st_size < sizeof (struct symtableheader)) {
		panic(gettext("cannot read symbol table file %s\n"), filename);
	}
	tblsize = stbuf.st_size - sizeof (struct symtableheader);
	base = calloc(sizeof (char), (size_t)tblsize);
	if (base == NULL)
		panic(gettext("cannot allocate space for symbol table\n"));
	...
	case 'R':
		...
		break;
	default:
		panic(gettext("initsymtable called from command %c\n"),
			(u_char)command);
		break;
	}
	maxino = hdr.maxino;
	...
	baseep =
	    /*LINTED [rvalue % 4 == 0]*/
	    (struct entry *)(base + hdr.stringsize - sizeof (struct entry));

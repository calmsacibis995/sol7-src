...
#pragma ident	"@(#)tape.c	1.57	98/02/03 SMI"
...
#endif
{
	...
	if (archivefile) {
		...
		if (pipein)
			mt = 0;
		else if ((mt = (metamucil_mode == NOT_METAMUCIL) ? 
					open64(magtape, 0) :
					open(magtape, 0)) < 0) {
			(void) fprintf(stderr, "%s: %s\n", magtape,
					strerror(errno));
	...
	unsigned size;
{
	...
	old_euid = geteuid();	/* remember who I was */
	(void) seteuid(stbuf.st_uid);

	/* acl() returns 0 upon success, -1 on error (Bug #1214399) */
	if (acl(name, SETACL, n, aclent) < 0) {
		static int once = 0;
		 ...
		 * EPERM  - not the owner of the object
		 *

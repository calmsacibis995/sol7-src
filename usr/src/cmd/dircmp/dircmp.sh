...
#	All rights reserved.
#ident	"@(#)dircmp.sh 1.11	97/04/23 SMI"	/* SVr4.0 1.13  */
PATH=/usr/bin
...
> /usr/tmp/dc$$g
#
# Process the results of the 'ls -lgnd' to obtain file size info and identify a large
# file's existence.
#
while read -u3 tmp tmp tmp fsize1 tmp tmp tmp a &&
      read -u4 tmp tmp tmp fsize2 tmp tmp tmp b; do
	#
	# A window of opportunity exists where the ls -lgnd above could produce different

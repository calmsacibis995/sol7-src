...
#ident	"@(#)print.c	1.64	97/11/22 SMI"
...
print_dis(int regnum, addr_t pc)
{
	...
	if (!(Prstatus.pr_flags & PR_PCINVAL)) {
		 ...
		 */
		if ((unsigned long) Prstatus.pr_lwp.pr_instr == (unsigned long) bpt && (bp = bkptlookup(dot)))
#ifdef	_LP64
		{
			upper = ((bp->ins) >> 32);
			Prstatus.pr_lwp.pr_instr = (unsigned long) upper;
		db_printf(2, "bp->ins %J pr_instr is %J dot is %J instr is %X\n",
				(unsigned long) bp->ins, Prstatus.pr_lwp.pr_instr, dot, t_srcinstr(chkget(dot,ISP)));
		}
#else
		...
		printins('i', ISP,
		    ...
		    (unsigned int) Prstatus.pr_lwp.pr_instr : t_srcinstr(chkget(dot, ISP))));
		printc('\n');
		return;
	}
#endif

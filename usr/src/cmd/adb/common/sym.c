...
static void dosym(Elf32_Sym *es, int flag);
static void mergedefs(struct asym *s, Elf32_Sym *es);
...
#else
static struct modctl *modules_kaddr;
static int *mod_mix_changed_kaddr = &dummy_module_mix;
#endif
...
stinit(int fsym, Elf32_Shdr *sh, int nsect, int flag)
{
	...
	*(int *) strtab = sh[str_sect].sh_size + (int) sizeof(sh[str_sect].sh_size);
	...
	while (ntogo) {
		...
		if (es->st_name)
			es->st_name = (uintptr_t) strtab + (uintptr_t) es->st_name +
				(uintptr_t) sizeof(sh[str_sect].sh_size);
		dosym(es++, flag);
		ninbuf--;
		ntogo--;
	}
...
static void
mergedefs(struct asym *s, Elf32_Sym *es)
{
 ...
 */
#ifdef _LP64
ssymoff(long v, int type, char *buf)
#else
ssymoff(int v, int type, char *buf)
#endif
{
	unsigned w = 0;
	int len_sofar;
...
#ifdef _LP64
	if ((unsigned long)v < 10)
		sprintf( &buf[ len_sofar ], "%x", v );
	else
		sprintf( &buf[ len_sofar ], "0x%lx", v );
#else
	if ((unsigned)v < 10)
		sprintf( &buf[ len_sofar ], "%x", v );
	else
		sprintf( &buf[ len_sofar ], "0x%x", v );
#endif
...
#ifndef _LP64
static struct modctl krtld_modctl;
#else
...
kobj_init(void)
{
	...
	stubs_base_kaddr = lookup_base("stubs_base")->s_value;
	stubs_end_kaddr = lookup_base("stubs_end")->s_value;
#ifdef _LP64
	if (elf64mode)
#endif
		modules_kaddr = (struct modctl *)lookup_base("modules")->s_value;
#ifdef _LP64
	else modules_kaddr32 = (struct modctl32 *) lookup_base("modules")->s_value;
#endif
	mod_mix_changed_kaddr = (int *)lookup_base("mod_mix_changed")->s_value;

...
#ident	"@(#)output.c	1.39	98/02/09 SMI"
...
printf( const char *fmat, ... )
{
	...
	while (c = *fmt++) {
		...
		case 'R':
			if (radix == 0x10)
				printnum((unsigned) VAL, (sygned? '+': c), radix); 
			else
	...
	time_t tvec;
{
	...
	if (timeptr == NULL){  strcpy(digitptr, "VALUE TOO LARGE");
...
printhex(x)
	u_int x;
{
	printhex_cmn((u_longlong_t)(u_int)x, 8);
}

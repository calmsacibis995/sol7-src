...
#pragma ident "@(#)command.c	1.38	98/02/09 SMI"
...
#endif
	    "\n"
	    "    Display Formats     one byte    two bytes   four bytes\n"
	    "    octal               b           o           O\n"
	    "    decimal                         d           D\n"
	    "    hexadecimal         B           x           X\n"
	    "    instruction         i           i           i\n"
...
docasew:
				...
				do {
					...
					lw = (unsigned int) (expv & 0xffffffff);
					uw = (unsigned int) (expv >> 32);
					put(dot, itype, uw);
					put(dot+4, itype, lw);
				    }
#endif
		...
		case '>':
			...
			if (savc == '/') {
				...
				switch (savc) {
				case 'c':
					savdot = *(unsigned char *)&dot;
					break;
				case 's':
					savdot = *(unsigned short *)&dot;
					break;
				case 'i':
					savdot = *(unsigned int *)&dot;
					break;
				case 'l':
					savdot = *(unsigned long *)&dot;
					break;
				default:

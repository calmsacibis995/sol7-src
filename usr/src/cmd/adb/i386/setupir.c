/*
 * Copyright (c) 1986, 1987, 1988, 1989, 1990, 1991, 1992, 1996 by
 * Sun Microsystems, Inc.  All Rights Reserved.
 */

#ident "@(#)setupir.c	1.24	97/11/21 SMI"
...
setsym(void)
{
	...
	case EM_486:
		if (filhdr.e_ident[EI_CLASS] != ELFCLASS32 ||
			...
			(void) printf("%s: erroneous Intel386/486 ELF ident field.\n", symfil);
		}
		break;
	case EM_SPARC:
	case EM_M32:
			...
			if (proghdr[textseg].p_type == PT_INTERP) {
				...
				if (rtld_path == NULL)
					...
					(read(fsym, (char *)rtld_path, intp_hdr->p_filesz)
			    		!= intp_hdr->p_filesz)) {
			    			(void) printf("Unable to read PT_INTERP path.\n");
			    			return;
												    			}
		    		use_shlib = 1;
		...
		for (sn = 0; sn < (int) filhdr.e_shnum; sn++) {
			if (secthdr[sn].sh_type == SHT_STRTAB) {
				if ((sect_names = malloc(secthdr[sn].sh_size))
								== NULL) {
					(void) printf("Unable to allocate section name table.\n");
					outofmem();
				}
				if ((loc = lseek(fsym, secthdr[sn].sh_offset,
					...
					(void) printf("Unable to read section names.\n");
					goto bad_shdr;
				}
				...
				if (symfile_type == REL) {
					/* for x86 only */
					if ((strcmp((char *)loc, ".strtab") == 0)
					|| (strcmp((char *)loc, ".shstrtab") == 0))
						break;
				} else {
...
setcor(void)
{
	...
	if (fcor == -1)
		  return;
	if (kernel != NOT_KERNEL) {
		if (!strcmp(corfil, LIVE_KERNEL_COREFILE)) {
	...
	if (((stb.st_mode&S_IFMT) == S_IFREG) &&
		...
		if (lseek(fcor, (long) core_ehdr.e_phoff, L_SET) !=
						    core_ehdr.e_phoff) {
			(void) printf("Unable to find core file program header.\n");
			goto bad1;
		}
		...
		if (lseek(fcor, (long) core_phdr[nindx].p_offset, L_SET) !=
			...
			(void) printf("Unable to find process state info in core file.\n");
			goto bad1;
		}
		...
		if ((noteseg = malloc(core_phdr[nindx].p_filesz)) == NULL) {
			(void) printf("Unable to allocate core file state info tables");
			fcor_rng1->mpr_e = MAXFILE;
			outofmem();
		}

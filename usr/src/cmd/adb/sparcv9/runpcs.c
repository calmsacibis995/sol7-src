...
#ident	"@(#)runpcs.c	1.13	98/01/22 SMI"
...
#ifdef KADB
		(void) adb_ptrace(runmode, pid, userpc,
			execsig, changed_pc);
		if (runmode != PTRACE_SINGLESTEP) {
			bpwait(runmode);
			chkerr();
...
delbp()
{
	...
	if (bpstate == BPOUT)
		return;
	for (bp = bkpthead; bp; bp = bp->nxtbkpt) {
		if (bp->flag && (bp->flag & BKPT_ERR) == 0 && !iswp(bp->type) &&		    bp->type != BPDEFR)
...
int mode;
{
	...
	tbia();
rewait:
	oldisig = (void (*)(int)) signal(SIGINT, (void (*)(int))sigint_handler);
	oldqsig = (void (*)(int)) signal(SIGQUIT, (void (*)(int))sigquit_handler);
#if obsolete_code
	...
	if ((stat & 0177) != 0177) {
		...
		if (signo != SIGTRAP) {
			if (mode == PTRACE_CONT && sigpass[signo]) {
				(void) chk_ptrace(PTRACE_CONT, pid, (int *)1, signo);
				goto rewait;
			} /* else */
			sigprint(signo);
			unmarkwp();

...
#ident	"@(#)printsr.c	1.9	98/02/09 SMI"
	...
	int modif;
{
	...
	stacktop(&pos);
	db_printf(2,"Printstack aftern stacktop\n"); 
	if (hadaddress) {
		pos.k_fp = address;
		pos.k_nargs = 0;
		pos.k_pc = MAXINT;
	...
	db_printf(2,"Printstack aftern findentry\n"); 
...
printargs ( def_nargs, val, ppos, modif ) struct stackpos *ppos; {
	...
	addr_t adj_sp;
        addr_t adj_ccall_sp;
	unsigned low_word_adj;
        unsigned offset_adj;
	unsigned long longword;
	extern unsigned long getLong(addr_t, int, int);

	adj_sp = ((ppos->k_fp % 2)?(ppos->k_fp + V9BIAS64):ppos->k_fp);
        offset_adj = ((ppos->k_fp % 2)?2:1);
	low_word_adj = (ppos->k_fp % 2);
	...
	regp = adj_sp + FR_I0*offset_adj ;


	callers_fp = getLong( adj_sp + (addr_t) FR_SAVFP*offset_adj, DSP, low_word_adj );
	db_printf(1, "callers_fp %J\n", callers_fp);
	adj_sp = ((callers_fp%2)?(callers_fp + V9BIAS64):callers_fp);
	offset_adj = ((callers_fp % 2)?2:1); 
	low_word_adj = (callers_fp%2);
	if( callers_fp )
		ccall_fp = getLong( adj_sp + (addr_t) FR_SAVFP*offset_adj, DSP, low_word_adj );
	else
		ccall_fp = 0;
	adj_ccall_sp = ((ccall_fp % 2)?(ccall_fp + V9BIAS64):ccall_fp);

	db_printf(1, "printargs:  caller's fp %J; ccall_fp %J",
		callers_fp, ccall_fp );
	...
	if ( nargs ) {
		...
		for( anr=0; anr < nargs; ++anr ) {
			if( anr == NARG_REGS ) {
				...
				regp = adj_sp + FR_XTRARGS ;
				db_printf(1, "printargs:  xtrargs %X", regp );
			}
			if (offset_adj == 1){
				printf("%J", (unsigned)get(regp, DSP));
				regp += 4;
			}else{
				longword = getLong(regp, DSP, 1);
				printf("%J", longword);
				regp += 8;
			}

			/* Don't print past the caller's FP */
			if( adj_ccall_sp  &&  regp >= adj_ccall_sp )
				break;

...
#pragma ident	"@(#)ptrace.c	1.9	97/11/24 SMI"
...
extern	char	*map(int);
extern	pid_t	getpid();	/* We can't include unistd.h */
extern	void	_exit();
...
ptrace(int request, pid_t pid, long addr, long data, caddr_t addr2)
{
	...
	db_printf(5, "ptrace: request=%s, pid=%D, addr=%J, data=%J, addr2=%J",
...
again:
	...
	{	
		...
		v9stk = ((uintptr_t) ps->pr_lwp.pr_reg[R_SP] % 2);
		adj = (v9stk?2:1);
		bias = (v9stk?V9BIAS64:0);
		if ((caddr_t)((uintptr_t) ps->pr_lwp.pr_reg[R_SP] + bias)<
		    (caddr_t)addr + sizeof (data) &&
		    (caddr_t)((uintptr_t) ps->pr_lwp.pr_reg[R_SP] + bias + adj*16*sizeof (int)) >
		    (caddr_t)addr)
			setregs = TRUE;
	}
#endif
	...
	case PTRACE_CONT:
		...
		if (setregs) {
			...
			if (v9stk){
				...
				16*sizeof (long),
				(off_t)ps->pr_lwp.pr_reg[R_SP] + bias);
			} else {

				(void) pread(asfd, (char *)ilreg,
				16*sizeof (int),
				(off_t)ps->pr_lwp.pr_reg[R_SP] + bias);

				for (i = 0; i < 16; i++)
					ps->pr_lwp.pr_reg[R_L0 + i] =
						(unsigned long) ilreg[i];
			}
...
MakeLwpName(char *procname, pid_t pid, id_t lwpid)
{
	...
	(void) strcat(procname, "/lwp/0000000000");
	s = procname + strlen(procname);
	while (lwpid) {
		*--s = lwpid%10 + '0';
		lwpid /= 10;

...
#pragma ident	"@(#)sparc.h	1.9	98/01/28 SMI"
...
#ifndef _ASM
extern int elf64mode;		/* ELF64 mode (ie. target is 64-bit) */
#endif
/*
...
#ifndef _ASM
struct stackpos {
	u_long	k_pc;		/* where we are in this proc */
	u_long	k_fp;		/* this proc's frame pointer */
	u_int	k_nargs;	/* This we can't figure out on sparc */
	u_long	k_entry;	/* this proc's entry point */
	u_long	k_caller;	/* PC of the call that called us */
	u_int	k_flags;	/* sigtramp & leaf info */
	u_long	k_regloc[ LAST_STK_REG - FIRST_STK_REG +1 ];
};
#endif
	/* Flags for k_flags:  */
...
#define	PCFUDGE 0
#ifdef BPT_INIT
#define	KADB_BP (0x91D02000U + ST_KADB_BREAKPOINT)
	unsigned bpt =
#ifdef KADB
	    KADB_BP; /* asm("bpt:	ta   126   "); */
#else
	    0x91D02001U; /* asm("bpt:	ta   1    "); */
#endif
#else	/* !BPT_INIT */
extern unsigned bpt;
#endif

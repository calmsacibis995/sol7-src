...
#ident	"@(#)accesssr.c	1.12	98/02/09 SMI"
...
unsigned long
getLong(addr,space, low_word_adj)
addr_t	addr;
int	space;
int 	low_word_adj;
/* 
 * 	If low_word_adj is 0, that means we are actually
 *	looking at a v8 stack, so just return the word
 *	instead of long pointed to by addr
 */
{
	extern int elf64mode;
	unsigned long val;
	int tval[2];
	tval[1] = get(addr+4, space);
	tval[0] = get(addr, space);
	if (low_word_adj == 0){ tval[1] = tval[0];
			     tval[0] = 0;
	 }
	val = *(unsigned long *) tval;
	return(val);
}
	...
	case Reg_G0:
	 ...
	 default:
		if (reg >= Reg_F0  &&  reg <= Reg_F31) {
			...
			ra->ra_raddr = (v9flag ? 
			    (long *) &(arp_v9->r_window[w].rw_local[r]) :
			    (long *) &(arp->r_window[w].rw_local[r]));
			ra->ra_type = r_window;
			if (v9flag) ra->ra_mode = RA_64BIT;
		}
#endif KADB
		 else {
			regerr( reg, 0 );
...
long val;
{
	...
	case r_window: /* Go figure it out */
		(void) reg_windows( reg, (unsigned long) val, 1 );
		break;
#endif KADB
	}
}
...
#ifdef KADB
	case r_window: /* Go figure it out */
		val = reg_windows(reg, (unsigned long) val, 0);
		break;
#endif KADB
	default:
...
regs_to_core()
{
	...
	Prstatus.pr_lwp.pr_reg[R_CCR] = a->r_tstate;
...
unsigned long get_from_sp(sp, off, regset)
addr_t sp;
unsigned off;
int regset; /* 0=locals, 1= ins */
{
...
void put_to_sp(sp, off, regset, val)
addr_t sp;
unsigned off;
int regset; /* 0=locals, 1= ins */
unsigned long val;
...
{        
	...
	if (v9flag) {
	 ...
	 for (reg = 0; reg <= 7; reg++)
                        put_to_sp(Prstatus.pr_lwp.pr_reg[R_O6], reg, 0, a->r_locals[reg]);
                for (reg = 0; reg <=7; reg++)
                        put_to_sp(Prstatus.pr_lwp.pr_reg[R_O6], reg, 1, a->r_ins[reg]);
	} else {	
	        register struct allregs *a = &adb_regs;
				...
				/* invalid otherwise */
{
	...
	usp = spos->k_fp;
	adj_sp = ((spos->k_fp % 2)?(spos->k_fp + V9BIAS64):spos->k_fp);
...
{ extern int adb_debug;
	...
	if( cto7 == (addr_t) CALL_INDIRECT ) {
		 ...
		 */
		sv_i7 = getLong( adj_sp + FR_SAVPC*offset_adj, SSP, low_word_adj );
...
{ 
	...
	v9stk = (int) (spos->k_fp % 2);
        offset_adj = (v9stk?2:1);
        low_word_adj = (v9stk?4:0);
        adj_sp = (v9stk?(spos->k_fp + V9BIAS64):spos->k_fp);

	errflg = 0;
	db_printf(8,"find_entry adj_sp %J\n", adj_sp);
	spos->k_caller = 
                getLong( adj_sp + FR_SAVPC*offset_adj, DSP, low_word_adj);
	db_printf(8,"find_entry caller %J\n", spos->k_caller);
	if( errflg  &&  fromTop  /* &&  regs_not_on_stack( )*/ ) {
		errflg = 0;
		spos->k_fp = readreg(Reg_FP);
		 v9stk = (int) (spos->k_fp % 2); offset_adj = (v9stk?2:1);
                low_word_adj = (v9stk?4:0);
                adj_sp = (v9stk?(spos->k_fp + V9BIAS64):spos->k_fp);
                spos->k_caller = getLong( adj_sp + FR_SAVPC*offset_adj, DSP, low_word_adj );
	...
	register struct stackpos *spos;
{
...
#endif !KADB
	{
		...
		if( (spos->k_flags & (K_LEAF|K_SIGTRAMP)) == 0 ) {
			spos->k_fp = getLong(adj_sp+FR_SAVFP*offset_adj, DSP ,low_word_adj);
		}
	...
	int was_leaf;
{
	...
	adj_sp = ((kfp % 2)?(kfp + V9BIAS64):kfp);
        offset_adj = ((kfp % 2)?2:1);
        low_word_adj = ((kfp % 2)?4:0);

	wf = was_leaf ? kfp : (addr_t) getLong( kfp + FR_IREG(Reg_FP)*offset_adj , DSP, low_word_adj );
	ucp = get( wf + 40, DSP );
#if DEBUG

...
#ifndef lint
#pragma ident	"@(#)analyze.c	1.11	97/05/08 SMI"
#endif	lint
 ...
 */
unsigned int purge_patterns[] = {	/* patterns to be written */
	0xaaaaaaaa,		/* 10101010... */
...
unsigned int alpha_pattern =  0x40404040;	/* 10000000...  == @@@@... */
...
"\nThe last %d passes were successful, running alpha pattern pass", NPPATTERNS);
		 ...
		 */
#ifdef	lint
		size = 0;	/* lint thinks: used before set */
#endif	lint
	...
	int	mode;
{
	...
	fmt_print("Repairing %s error on %d (",
	...
	daddr_t	blkno;
{
	...
	register unsigned *ptr = (u_int *)pattern_buf;
	...
	if ((flags & SCAN_COMPARE) || (flags & SCAN_PURGE_READ_PASS)) {
		for (i = nints, ptr = (u_int *)pattern_buf; i; i--)
			if (*ptr++ != data) {
 ...
 */
int
verify_blocks(flags, blkno, blkcnt, data, driver_flags)
	int	flags, driver_flags, blkcnt;
	register unsigned data;
	daddr_t	blkno;
{
	register int	status, i, nints;

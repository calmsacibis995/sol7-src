...
#pragma ident	"@(#)startup.c	1.38	98/02/06 SMI"
...
do_options(int argc, char *argv[])
{
	 ...
	 */
#ifdef	lint
	next = 0;	/* lint thinks: used before set */
#endif	lint
...
add_device_to_disklist(char *devname, char *devpath)
{
	...
	if (mlp == NULL) {
		 ...
		 */
		tctlr = calloc(1, sizeof (struct ctlr_type));
		tctlr->ctype_ctype = dkinfo.dki_ctype;
		tctlr->ctype_name = calloc(1, DK_DEVLEN);
		(void) strcpy(tctlr->ctype_name, dkinfo.dki_cname);
		tctlr->ctype_ops = calloc(1, sizeof (struct ctlr_ops));
		...
		*(tctlr->ctype_ops) = genericops;
		...
		mlp->next = calloc(1, sizeof (struct mctlr_list));
	...
	if ((dkinfo.dki_ctype == DKC_SCSI_CCS) ||
	    (dkinfo.dki_ctype == DKC_MD21)) {
		i = scsi_rdwr(DIR_READ, search_file, (daddr_t)0,
			1, (char *)&search_label, F_SILENT);
		switch (i) {
		case DSK_RESERVED:
	...
	if (status && ctlr->ctype_ctype == DKC_SCSI_CCS) {
...
make_controller_list()
{
	...
	for (x = nctypes; x != 0; x--) {
		ctlrp = calloc(1, sizeof (struct mctlr_list));
		ctlrp->next = controlp;
...
char *arglist[];
{
	...
	for (; *disklist != NULL; disklist++) {
		...
		strcpy(arglist[diskno], *disklist);
		diskno++;
	}
	arglist[diskno] = NULL;
}
...
char	*name;
{
	...
	memset(symname, 0, MAXPATHLEN);
	memset(localname, 0, MAXPATHLEN);

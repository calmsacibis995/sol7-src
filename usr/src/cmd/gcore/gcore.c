...
#ident	"@(#)gcore.c	1.18	97/12/23 SMI"	/* SVr4.0 1.1	*/
...
dumpcore(struct ps_prochandle *Pr, pid_t pid)
{
	...
	if (write(dfd, &ehdr, sizeof (Ehdr)) != sizeof (Ehdr)) {
...
elfnote(int dfd, int type, char *ptr, int size)
{
	...
	(void) write(dfd, &note, sizeof (Note));
	(void) write(dfd, ptr, roundup(size, sizeof (Word)));
}

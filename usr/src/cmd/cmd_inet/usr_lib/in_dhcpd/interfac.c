#ident  "@(#)interfaces.c	1.79	97/10/29 SMI"
...
static u_short udp_chksum(char *, struct pseudo_udp *);
...
"%s: UDP checksum:  0x%x != 0x%x, source: %s\n",
		...
		if (ifp->type == DHCP_DLPI) {
			plp->offset = (u_char) (sizeof (struct ip) +
			    sizeof (struct udphdr));
			plp->len = data.len - plp->offset;
			plp->pkt = (PKT *)((u_int)data.buf + plp->offset);
		} else {
			plp->offset = 0;
			plp->len = data.len;
...
"Interface: %s - DLPI version 2 expected, not version %d\n",
			...
			default:
				dhcpmsg(LOG_ERR,
"Interface: %s - Unsupported Mac type: 0x%x\n",
				    ifp->nm, dl.info_ack.dl_mac_type);
				(void) close(ifp->recvdesc);
				ifp = zap_ifp(&ifp_prevp, ifp);
				num_interfaces--;
				continue;
...
"Interface: %s - Error setting BOOTP packet filter.\n", ifp->nm);
			...
			if (dlbindreq(ifp->recvdesc, (u_long)ETHERTYPE_IP,
			    0, DL_CLDLS, 0) != 0) {
				dhcpmsg(LOG_ERR,
...
write_interface(IF *ifp, PKT *pktp, int len, struct sockaddr_in *to)
{
	...
	if ((err = sendto(ifp->senddesc, (caddr_t)pktp, len, 0,
		...
		mutex_lock(&ifp->ifp_mtx);
		ifp->transmit++;
		mutex_unlock(&ifp->ifp_mtx);
	}
...
disp_if_stats(IF *ifp)
{
	...
	dhcpmsg(LOG_CRIT, "Total Packets Transmitted: %d\n", ifp->transmit);
	dhcpmsg(LOG_CRIT, "Total Packets Received: %d\n", ifp->received);
	dhcpmsg(LOG_CRIT, "Total Packet Duplicates: %d\n", ifp->duplicate);
	dhcpmsg(LOG_CRIT, "Total Packets Dropped: %d\n", ifp->dropped);
	dhcpmsg(LOG_CRIT, "Total Packets Processed: %d\n", ifp->processed);
	dhcpmsg(LOG_CRIT, "Total Protocol Errors: %d\n", ifp->errors);
}
 ...
 * Must be MT SAFE - Called by monitor threads and ICMP validation threads.
 */
u_short
ip_chksum(char *p, u_short len)
{
	/* LINTED [IP headers will start on a short boundary at least] */
	u_short		*sp = (u_short *)p;
	u_long		sum = 0;
 ...
 *
 * Must be MT SAFE - called by monitor threads, which is why the udp_ckp
 * is a buffer, and not statically allocated.
 */
static u_short
udp_chksum(char *data, struct pseudo_udp *udp_ckp)
{
	u_long		end_hdr;
	u_long		sum = 0L;
	u_short		cnt;
	 ...
	 */
	sp = (u_short *)udp_ckp;
	cnt = ntohs(udp_ckp->len) + sizeof (struct pseudo_udp) -
	    sizeof (struct udphdr);
	 ...
	 */
	if (cnt & 1) {
		data[ntohs(udp_ckp->len) - sizeof (struct udphdr)] = '\0';
		cnt++;	/* make even */
	}
	cnt >>= 1;
	while (cnt--) {

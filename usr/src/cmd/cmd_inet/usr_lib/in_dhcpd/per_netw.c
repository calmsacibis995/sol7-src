#ident	"@(#)per_network.c	1.38	97/10/29 SMI"
...
ascii_to_pnrec(PER_NET_DB *pndbp, enum pn_type type, void *vp, PN_REC *pnp)
{
	...
	if (type == PN_ASCII)
		...
		cp = rowp->ca[2];
	if ((pnp->clientip.s_addr = inet_addr(cp)) == -1) {
		dhcpmsg(LOG_ERR,
	...
	if (type == PN_ASCII)
		...
		cp = rowp->ca[3];
	if ((pnp->serverip.s_addr = inet_addr(cp)) == -1) {
		dhcpmsg(LOG_ERR,
...
pnrec_to_ascii(PER_NET_DB *pndbp, PN_REC *pnp, PN_ASCII_REC *pn_asp)
{
	...
	/* FLAGS */
	len = sizeof (pn_asp->as_flags);
	err = octet_to_ascii(&pnp->flags, 1, pn_asp->as_flags, &len);
	if (err != 0)  {
		dhcpmsg(LOG_ERR,
		    "Error converting flags field to ASCII. Table: %s\n",
		    pndbp->name);
		return (err);
	}
	...
	/* LEASE */
	(void) sprintf(pn_asp->as_lease, "%ld", (u_long)ntohl(pnp->lease));
    ...
    struct in_addr *sip, PN_REC *pnp)
{
	...
	case PN_CID:
		...
		blen = DHCP_SCRATCH;
		(void) octet_to_ascii((u_char *)val, val_len, buffer, &blen);

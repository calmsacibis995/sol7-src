#ident	"@(#)dhcptab.c	1.60	97/04/24 SMI"
...
typedef struct {
	...
	u_short	code;		/* Parameter code */
	u_char	vend;		/* Vendor code, if code == CD_VENDOR_SPEC */
	char	**class;	/* client class table */
	CDTYPE	type;		/* Type of parameter */
	u_char	gran;		/* Granularity of type */
	u_char	max;		/* maximum items of type at granularity */
} SYM;
...
static char *get_string(char **, char *, u_char *);
...
static void print_error_msg(int, u_char);
...
readtab(int preserve)
{
	...
	for (i = 0; i < dhcptab.rows; i++) {
		...
		hc = hash_HashFunction((u_char *)mc->nm,
		    (u_int)strlen(mc->nm));
...
get_macro(char *mnamep)
{
	...
	hc = hash_HashFunction((u_char *)mnamep, (u_int)strlen(mnamep));
...
eval_symbol(char **symbol, MACRO *mc)
{
	...
	case OP_ADDITION:
		...
		case INCLUDE:
			...
			include[ilen] = '\0';
			hc = hash_HashFunction((u_char *)include, (u_int)ilen);
...
process_data(ENCODE *ec, SYM *sym, char **pos)
{
	...
	case OCTET:
		if (sym->max)
			...
			ec->len = MAXUCHARLEN;
		if ((ec->data = get_octets((u_char **)pos,
		    (u_char *)&ec->len)) == (u_char *)NULL) {
			error = E_BAD_OCTET;
			free(ec->data);
			ec->len = 0;
		}
 ...
 */
static char *
get_string(char **src, char *dest, u_char *length)
{
...
get_addresses(char **src, SYM *sym, ENCODE *ec, int items)
{
	...
	if (error == 0) {
		...
		ec->data = (u_char *)smalloc((u_int) ec->len);
...
static void
print_error_msg(int error, u_char index)
{
	switch (error) {
	case E_BAD_IPADDR:
...
vendor symbol definition: '%s'\n",
		...
		if ((u_int)strlen(*ptr) > (u_int)DHCP_MAX_CLASS_SIZE) {
...
vendor symbol: '%s'. Must be less than: %d\n",
		...
		for (i = 0, cp = (char *)strtok(*ptr, " \t\n");
		    ...
		    cp = (char *)strtok(NULL, " \t\n"), i++) {
			newsym.class[i] = (char *)smalloc(strlen(cp) + 1);
			(void) strcpy(newsym.class[i], cp);
		}
...
"Out of range (%d-%d) site option code: %d in symbol definition: '%s'\n",
			...
			if (code <= DHCP_LAST_OPT || code >= DHCP_SITE_OPT) {
				dhcpmsg(LOG_ERR,
"Out of range (%d-%d) DHCP extend option code: %d in symbol definition: '%s'\n",
				    DHCP_LAST_OPT + 1, DHCP_SITE_OPT - 1,
				    code, symname);
				return (E_SYNTAX_ERROR);
			}
		}

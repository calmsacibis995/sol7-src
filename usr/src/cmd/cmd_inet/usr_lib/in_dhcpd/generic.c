#ident	"@(#)generic.c	1.45	97/10/27 SMI"
...
disp_cid(PKT_LIST *plp, char *bufp, int len)
{
	...
	tlen = len;
	(void) octet_to_ascii((u_char *)cp, cplen, bufp, &tlen);
	return (bufp);
}
...
{
	u_char len;
	u_char *vp, *vdata, *data, *endp, *main_optp,  *opt_endp;
	u_short code;
	u_char overload = 0, using_overload = 0;
	ENCODE	*ep, *prevep, *tvep;
	u_int vend_len;

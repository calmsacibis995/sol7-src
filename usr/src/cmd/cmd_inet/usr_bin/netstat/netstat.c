...
#pragma ident	"@(#)netstat.c	1.37	97/12/04 SMI"
...
static void	dhcp_report(const char *);
...
static	int DHCPflag = 0; /* DHCP statistics */

static char usage[] =
		"[ -adgimnprsDMv ] [-I interface] [interval] [system] [core]";
...
main(int argc, char **argv)
{
	...
	if ((af == AF_INET) || (af == AF_UNSPEC)) {
		...
		if (!(Iflag || Kflag || Rflag || Sflag || Mflag ||
	...
	"Group = %ld, mib_id = %ld, length = %ld, valp = 0x%lx\n",
		...
		case MIB2_TCP: {
			...
			prval("tcpTimKeepaliveProbe",
						tcp->tcpTimKeepaliveProbe);
			prval("tcpTimKeepaliveDrop",
						tcp->tcpTimKeepaliveDrop);
			prval("tcpListenDrop",
						tcp->tcpListenDrop);
			prval("tcpListenDropQ0",
						tcp->tcpListenDropQ0);
			prval("tcpHalfOpenDrop",
						tcp->tcpHalfOpenDrop);
			prval("tcpOutSackRetrans",
						tcp->tcpOutSackRetransSegs);
			prval_end();
			break;
			}
		case MIB2_UDP: {
...
static void
dhcp_report(const char *ifname)
{
	char		*argv[2];

	argv[0] = 0;
	argv[1] = "status";
	setifdhcp("netstat", ifname, 2, argv);
}
...
arp_report(mib_item_t *item)
{
	...
	char		buf1[IFNAMSIZ + 1];
	char		buf2[MAXHOSTNAMELEN + 1];
	char		buf3[STR_EXPAND * OCTET_LENGTH + 1];
	char		buf4[32];	/* IFFLAGS */
...
ire_report(mib_item_t *item)
{
	...
	char			buf1[MAXHOSTNAMELEN + 1];
	char			buf2[MAXHOSTNAMELEN + 1];
	char			buf3[MAXHOSTNAMELEN + 1];
	char			buf4[IFNAMSIZ + 1];
	char			flags[5];	/* IFFLAGS */
...
tcp_report(mib_item_t *item)
{
	...
	/*
	 * buf1 and buf2 below are for the hostname as well as the portname
	 * There is no limit on portname length so we assume MAXHOSTNAMELEN
	 * as the limit
	 */
	char			buf1[MAXHOSTNAMELEN + MAXHOSTNAMELEN + 1];
	char			buf2[MAXHOSTNAMELEN + MAXHOSTNAMELEN + 1];
...
udp_report(mib_item_t *item)
{
	...
	"Group = %ld, mib_id = %ld, length = %ld, valp = 0x%lx\n",
		...
		while ((char *)ude < item->valp + item->length) {
			...
			if (!(Aflag || ude->udpEntryInfo.ue_state >=
			    MIB2_UDP_connected)) {
				ude++;
				continue;
			}
	...
	int n;
{
	...
	/* XXX What if packet is > 9999m? buf is not enough */
...
{
	struct servent *sp = 0;
	static char	line[MAXHOSTNAMELEN + 1];	/* used for portname */
	char		*cp;
	...
	if (sp || port == 0)
		(void) sprintf(cp, "%.*s", MAXHOSTNAMELEN,
				sp ? sp->s_name : "*");
	else

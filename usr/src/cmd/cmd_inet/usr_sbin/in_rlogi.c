...
#pragma	ident	"@(#)in.rlogind.c	1.26	97/05/19 SMI"	/* SVr4.0 1.9 */
	...
	struct sockaddr_in *fromp;
{
	...
	if (fromp->sin_family != AF_INET ||
	    fromp->sin_port >= (u_short) IPPORT_RESERVED ||
...
gotpty:
	...
	if (pid == 0) {
		...
		close(tt);
		execl("/bin/login", "login", "-d", line, "-r", hp->h_name,
			(char *)0);
#else
		close(f), close(p);
...
rmut()
{
	pam_handle_t *pamh;
	struct utmpx *up;
	char user[33], ttyn[33], rhost[258];	/* XXXpaf sizes from wtmpx! */

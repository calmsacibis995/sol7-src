...
%{
#ident	"@(#)ftpcmd.y	1.22	98/03/04 SMI"	/* SVr4.0 1.6	*/
		...
		= {
			...
			if ($2) {
				if ((cliport >= IPPORT_RESERVED) &&
				    data_dest.sin_addr.s_addr != INADDR_ANY)
			...
			*cs++ = '\0';
			if (debug) {
				...
				if (strncasecmp(s, "PASS", 4) != 0 || guest)
					syslog(LOG_DEBUG, "FTPD: command: %s",
					    s);
				else
					syslog(LOG_DEBUG, "FTPD: command: "
					    "PASS <password>");
			}
			tmpline[0] = '\0';
			return(s);
	...
	*cs++ = '\0';
	if (debug) {
		...
		if (strncasecmp(s, "PASS", 4) != 0 || guest)
			...
			syslog(LOG_DEBUG, "FTPD: command: PASS <passwd>");
	}
	return (s);
}

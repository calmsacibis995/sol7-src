...
# All rights reserved.
#
#ident	"@(#)dhcpconfig.ksh	1.33	98/01/13 SMI"
...
unconfigure()
{
	...
	then
		...
		let CONFIG_FILE=1
		# Issue warning
		print - "Unconfigure will stop the DHCP service and remove /etc/default/dhcp.">&2
	fi
...
get_time_offset()
{
	...
	local_time=`/usr/bin/date '+%d %H %M'`
	gmt_time=`/usr/bin/date -u '+%d %H %M'`
...
#
# Display nonloopback, UP network interfaces.
#
get_interfaces()
{
	ifconfig -a | grep 'flags=[0-9]*.*[^A-Z]UP[^A-Z]' |  cut -d: -f1 | sed -e '/lo0/d' | sort -u
}

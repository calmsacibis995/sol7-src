#
#pragma ident   "@(#)Makefile.com 1.2     97/12/05 SMI"
#
...
#
VER= 		LOCAL-`date +%y%m%d.03/14/98M%S`
...
ns_parser.c ns_parser.h: ../common/ns_parser.y
	$(YACC) ../common/ns_parser.y
	mv y.tab.c ns_parser.c
	mv y.tab.h ../common/ns_parser.h

tmp_version.c: ../common/version.c
	sed -e "s|%WHEN%|$(DT)|" -e "s|%VERSION%|"$(VER)"|" \
	-e "s|%WHOANDWHERE%|${VERSION}-$(RELEASE)-$(RELEASE_DATE)|" \
	< ../common/version.c > tmp_version.c 
...
clean:	$(SUBDIRS)
	$(RM) $(NMDOBJS)
	$(RM) ns_parser.c ns_parser.h
	$(RM) tmp_version.c tmp_version.o

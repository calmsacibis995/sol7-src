...
#ident	"@(#)in.rexecd.c	1.20	98/02/12 SMI"	/* SVr4.0 1.6	*/
...
static struct	sockaddr_in asin = { AF_INET };
static char pass[16];
...
doit(int f, struct sockaddr_in *fromp)
{
	...
	if (chostp)
		(void) strncpy(hostname, chostp->h_name, sizeof (hostname));
	else
	...
	if (port != 0) {
		...
		if (bind(s, (struct sockaddr *)&asin, sizeof (asin)) < 0)
			exit(1);
		(void) alarm(60);
		fromp->sin_port = htons((ushort_t)port);

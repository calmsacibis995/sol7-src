...
#ident	"@(#)inetd.c	1.42	97/06/21 SMI"	/* SVr4.0 1.21	*/
...
void freeconfig(), usage(), audit_inetd_service();
...
/* ARGSUSED */
void
echo_dg(			/* Echo service -- echo data back */
	int s,
	struct servtab *sep)
{
	char buffer[BUFSIZ];
	...
	if ((i = recvfrom(s, buffer, sizeof (buffer), 0, &sa, &size)) < 0)
		return;
	if (inetd_biltin_srcport(((struct sockaddr_in *)&sa)->sin_port))
...
/* ARGSUSED */
void
daytime_stream(		/* Return human-readable time of day */
	int s,
	struct servtab *sep)
{
	char buffer[256];
...
/* ARGSUSED */
void
daytime_dg(		/* Return human-readable time of day */
	int s,
	struct servtab *sep)
{
	char buffer[256];

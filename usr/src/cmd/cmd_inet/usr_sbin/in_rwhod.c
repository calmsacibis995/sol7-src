...
#ident	"@(#)in.rwhod.c	1.16	97/06/21 SMI"	/* SVr4.0 1.7	*/
...
#define	MAXIFS	256	/* If SIOCGIFNUM not defined */
...
struct	neighbor {
	...
	u_long	n_subnet;		/* AF_INET subnet */
...
usage:		fprintf(stderr, "usage: in.rwhod [ -m [ ttl ] ]\n");
	...
	if ((misc_ksp = kstat_lookup(kc, "unix", 0, "system_misc")) == NULL) {
		syslog(LOG_ERR,
		    "kstat_lookup('unix', 0, 'system_misc') failed");
		exit(1);
	}
	if (kstat_read(kc, misc_ksp, NULL) == -1) {
		syslog(LOG_ERR,
		    "can't kstat_read load average statistics from kernel");
		exit(1);
	}
	...
	for (;;) {
		...
		if (!verify(wd.wd_hostname) ||
		    strlen(wd.wd_hostname) >= sizeof (wd.wd_hostname) - 1) {
...
onalrm()
{
	...
	if (kstat_read(kc, misc_ksp, NULL) == -1) {
		syslog(LOG_ERR,
		    "onalrm:can't kstat_read load average statistics "
		    "from kernel");
		exit(1);
	}
	...
	if (read(kmemf, (char *)avenrun, sz) == -1) {
		syslog(LOG_ERR,
		    "in.rwhod:can't read \"avenrun\" load average statistics "
		    "from kernel: %m");
		exit(1);
	}
	...
	if (multicast_mode == SCOPED_MULTICAST) {
		...
		if (multicast_mode == PER_INTERFACE_MULTICAST &&
			...
			if (setsockopt(s, IPPROTO_IP, IP_MULTICAST_IF,
			    (char *)&(((struct sockaddr_in *)np->n_addr)->
			    sin_addr),
	...
	int s;
{
	...
	if (ioctl(s, SIOCGIFNUM, (char *)&numifs) < 0) {
		syslog(LOG_ERR, "ioctl SIOCGIFNUM: %m");
		return (0);
	}
#else
	numifs = MAXIFS;
	...
	for (n = ifc.ifc_len / sizeof (struct ifreq); n > 0; n--, ifr++) {
		...
		if ((((struct sockaddr_in *)np->n_addr)->sin_family ==
		    AF_INET) &&

...
#ident	"@(#)ifconfig.c	1.41	97/11/12 SMI"
...
int	setifmetric(), setifbroadaddr(), setifipdst(), setifether();
int	setifmtu(), inetplumb(), inetunplumb();
int	setdebugflag();
void	printb();
void	status();

extern	int setifrevarp();
char 	*plumbstr = "plumb";
...
struct	cmd {
	...
	int	(*c_func)();
	int	c_abortonfail;		/* do not continue parsing args */
...
/* BEGIN CSTYLED */
	...
	{ "-trailers",	IFF_NOTRAILERS,	setifflags,	0,	AF_ANY },
	{ "arp",	-IFF_NOARP,	setifflags,	0,	AF_ANY },
	{ "-arp",	IFF_NOARP,	setifflags,	0,	AF_ANY },
	{ "private",	IFF_PRIVATE,	setifflags,	0,	AF_ANY },
	{ "-private",	-IFF_PRIVATE,	setifflags,	0,	AF_ANY },
	{ "netmask",	NEXTARG,	setifnetmask,	0,	AF_INET },
	{ "metric",	NEXTARG,	setifmetric,	0,	AF_ANY },
	{ "mtu",	NEXTARG,	setifmtu,	0,	AF_ANY },
	{ "broadcast",	NEXTARG,	setifbroadaddr,	0,	AF_ANY },
	{ "ipdst",	NEXTARG,	setifipdst,	0,	AF_NS },
	{ "auto-revarp", 0,		setifrevarp,	1,	AF_ANY },
	{ "plumb",	0,		inetplumb,	1,	AF_ANY },
	{ "unplumb",	0,		inetunplumb,	0,	AF_ANY },
	{ "debug",	0,		setdebugflag,	0,	AF_ANY },
	{ 0,		0,		setifether,	0,	AF_UNSPEC },
	...
	char *argv[];
{
	...
	if (argc < 2) {
		fprintf(stderr,
"usage: ifconfig <interface> | -a | -au | auD | -ad | -adD\n");
...
"\t[ <af> ] [ <address> [ <dest_addr> ] ] [ up ] [ down ]\n",
		...
		fprintf(stderr, "or\n");
		fprintf(stderr,
		    "\tifconfig <interface> | -a | -au | -auD | -ad| -adD\n");
		fprintf(stderr, "%s%s%s%s\n", "\tauto-dhcp|dhcp\n",
		    "\t[wait <time> | forever\n", "\t[primary]\n",
		    "\tstart | drop | ping | release | status\n");
		exit(1);
	}
	argc--, argv++;
	 ...
	 *  -auD All "up" interfaces, not controlled by DHCP
	 */
	if (strcmp(name, "-a") == 0)
		...
		foreachinterface(ifconfig, argc, argv, af, IFF_UP,
		    IFF_DHCPRUNNING);
	else if (strcmp(name, "-adD") == 0)
		foreachinterface(ifconfig, argc, argv, af, 0,
		    IFF_UP | IFF_DHCPRUNNING);
	else if (strcmp(name, "-aZ") == 0) {
		debug += 3;
		foreachinterface(ifconfig, argc, argv, af, 0, 0);
	} else if (strcmp(name, "-auZ") == 0) {
		debug += 3;
		foreachinterface(ifconfig, argc, argv, af, IFF_UP, 0);
	} else if (strcmp(name, "-adZ") == 0) {
		debug += 3;
		foreachinterface(ifconfig, argc, argv, af, 0, IFF_UP);
	} else if (strcmp(name, "-auDZ") == 0) {
		debug += 3;
		foreachinterface(ifconfig, argc, argv, af, IFF_UP,
		    IFF_DHCPRUNNING);
	} else if (strcmp(name, "-adDZ") == 0) {
		debug += 3;
		foreachinterface(ifconfig, argc, argv, af, 0,
		    IFF_UP | IFF_DHCPRUNNING);
	} else
		ifconfig(argc, argv, af, (struct ifreq *)NULL);
	...
	int offflags;
{
	...
	if (argc > 0 && (strcmp(*argv, plumbstr) == 0)) {
		...
		di_walk_minor(root, class, DI_CHECK_ALIAS, NULL, devfs_entry);
		di_fini(root);
		...
		ifc.ifc_buf = buf;
		ifrp = ifc.ifc_req;
		for (nilp = first_if(nil_head), n = 0; n < num_ni;
		    nilp = next_if(nilp), n++, ifrp++) {
			nip = get_nifp(nilp);
	...
	register struct ifreq *ifrp;
{
	...
	if (strcmp(*argv, "auto-dhcp") == 0 || strcmp(*argv, "dhcp") == 0) {
		if (af == AF_INET)
			setifdhcp("ifconfig", name, argc, argv);
		return;
	}
	while (argc > 0) {
...
/* BEGIN CSTYLED */
#define	IFFBITS \
"\020\1UP\2BROADCAST\3DEBUG\4LOOPBACK\5POINTOPOINT\6NOTRAILERS\7RUNNING\10NOARP\11PROMISC\12ALLMULTI\13INTELLIGENT\14MULTICAST\15MULTI_BCAST\16UNNUMBERED\17DHCP\020PRIVATE"
/* END CSTYLED */
...
inetplumb()
{
	...
	if (mux_fd == -1) {
		...
		sprintf(buf, "open %s", IP_DEV_NAME);
		perror(buf);
		return (1);
	}
...
struct sockaddr *saddr;
{
	...
	memcpy(saddr->sa_data, eaddr, sizeof (*eaddr));
}
...
devfs_entry(di_node_t node, di_minor_t minor, void *arg)
{
	...
	if (di_minor_type(minor) == DDM_MINOR) {
		strcpy(cname, di_minor_name(minor));
	} else {

...
#pragma	ident	"@(#)in.telnetd.c	1.28	97/12/04 SMI"	/* SVr4.0 1.9 */
 ...
 * publication.
 *
 * 	(c) 1986 to 1996  Sun Microsystems, Inc
...
char	remopts[256];
char	myopts[256];
u_char doopt[] = { (u_char)IAC, (u_char)DO, '%', 'c', 0 };
u_char	dont[] = { (u_char)IAC, (u_char)DONT, '%', 'c', 0 };
u_char	will[] = { (u_char)IAC, (u_char)WILL, '%', 'c', 0 };
u_char	wont[] = { (u_char)IAC, (u_char)WONT, '%', 'c', 0 };
...
getterminaltype()
{
	...
	if (remopts[TELOPT_TTYPE] == OPT_YES) {
		static u_char sbbuf[] = { (u_char)IAC, (u_char)SB,
		    (u_char)TELOPT_TTYPE, (u_char)TELQUAL_SEND,
		    (u_char)IAC, (u_char)SE };
	...
	if (remopts[TELOPT_XDISPLOC] == OPT_YES) {
		static u_char sbbuf[] = { (u_char)IAC, (u_char)SB,
		    (u_char)TELOPT_XDISPLOC, (u_char)TELQUAL_SEND,
		    (u_char)IAC, (u_char)SE };
	...
	if (remopts[TELOPT_NEW_ENVIRON] == OPT_YES) {
		static u_char sbbuf[] = { (u_char)IAC, (u_char)SB,
		    (u_char)TELOPT_NEW_ENVIRON, (u_char)TELQUAL_SEND,
		    (u_char)IAC, (u_char)SE };
	...
	if (remopts[TELOPT_OLD_ENVIRON] == OPT_YES) {
		static u_char sbbuf[] = { (u_char)IAC, (u_char)SB,
		    (u_char)TELOPT_OLD_ENVIRON, (u_char)TELQUAL_SEND,
		    (u_char)IAC, (u_char)SE };
...
gotpty:
	...
	hp = gethostbyaddr((const char *)&who->sin_addr,
			sizeof (struct in_addr), who->sin_family);
	if (hp)
		host = hp->h_name;
	else
	...
	int	size;
{
	...
	if ((nbytes = read(net, (char *)netip + ncc + tsize, size)) != size)

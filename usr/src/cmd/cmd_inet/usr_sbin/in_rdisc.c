...
#ident	"@(#)in.rdisc.c	1.18	97/07/16 SMI"
...
static int	is_directly_connected(struct in_addr in, struct phyint *pi);
...
static void	added_logint(struct logint *li, int s,
		    struct sockaddr_in *joinaddr);
static void	deleted_logint(struct logint *li, int s,
		    struct sockaddr_in *joinaddr);

static int	initifs(int s, struct sockaddr_in *joinaddr, int preference);
static boolean_t getconfig(int sock, int if_flags, struct sockaddr *addr,
...
u_long g_preference = 0;	/* Setable with -p option */
...
advertise(struct sockaddr_in *sin)
{
	...
	for (pi = phyint; pi != NULL; pi = pi->pi_next) {
		...
		if (isbroadcast(sin))
			 ...
			 * address.
			 */
			if (is_directly_connected(sin->sin_addr, pi)) {
				if (debug) {
...
pr_pack(char *buf, int cc, struct sockaddr_in *from)
{
...
#endif notdef
		if (in_cksum((u_short *)ALIGN(buf+hlen), cc)) {
			if (verbose)
		...
		for (i = 0; (unsigned)i < rap->icmp_num_addrs; i++) {
			...
			if (verbose)
				logtrace("\taddress %s, preference 0x%x\n",
					...
					ap->preference);
			if (!responder) {
				if (is_directly_connected(ina, NULL))
					record_router(ina,
							(long)ap->preference,
							rap->icmp_lifetime);
			}
	...
	case ICMP_ROUTER_SOLICITATION: {
		...
		if (ip->ip_src.s_addr == 0) {
			...
			if (!is_directly_connected(ip->ip_src, NULL)) {
				if (verbose)
...
initifs(int s, struct sockaddr_in *joinaddr, int preference)
{
	...
	for (n = ifc.ifc_len/sizeof (struct ifreq); n > 0; n--, ifr++) {
		...
		if (li != NULL) {
			...
			if (!getconfig(sock, ifreq.ifr_flags, &ifr->ifr_addr,
			    &ifreq, &newli)) {
				free_logint(li);
				continue;
			}
...
static boolean_t
getconfig(int sock, int if_flags, struct sockaddr *addr,
    struct ifreq *ifr, struct logint *li)
{
	struct ifreq ifreq;
	struct sockaddr_in *sin;
 ...
 */
static int
is_directly_connected(struct in_addr in, struct phyint *opi)
{
	struct phyint *pi;
	struct logint *li;
 ...
 */
static void
deleted_logint(struct logint *li, int s,
    struct sockaddr_in *joinaddr)
{
	struct phyint *pi;
	struct logint *oli;
...
flush_unreachable_routers(void)
{
	...
	while (*tpp != NULL) {
		tp = *tpp;
		if (!is_directly_connected(tp->router, NULL)) {
			*tpp = tp->next;
			if (debug) {

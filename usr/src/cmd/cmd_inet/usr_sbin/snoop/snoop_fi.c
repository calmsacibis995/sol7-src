...
#ident	"@(#)snoop_filter.c	1.21	97/11/19 SMI"	/* SunOS	*/
...
next()
{
	...
	/*
	 * RFC1123 states that host names may now start with letters. Need
	...
	if (isdigit(*tkp)) {
		...
		if (colons == 5)
			...
			if (dots != 3 || size != size1) {
				...
				tokentype = ADDR_IP;
		} else if (alphas && *tkp != '\0' && !isspace(*tkp)) {
			tokentype = ALPHA;
			tkp += strspn(tkp, namechars);
			if (*tkp == '[') {
				tokentype = FIELD;

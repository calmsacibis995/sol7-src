...
#ident	"@(#)snoop_dns.c	1.1	98/01/26 SMI"	/* SunOS	*/
...
static int skip_question(char *header, char *data);
static int print_question(char *line, char *header, char *data, int detail);
static int print_answer(char *line, char *header, char *data, int detail);
...
static int print_question(char *line, char *header, char *data, int detail)
{
	...
	data += print_domain_name(line, header, data);
	line += strlen(line);
...
static int print_answer(char *line, char *header, char *data, int detail)
{
	...
	switch (type) {
		...
		case T_MR:
		case T_PTR:
			print_domain_name(line, header, data);
			break;
		case T_MX:
			data_next = data;
		...
		case T_MINFO:
			...
			line += strlen(line);
			data_bak = data_bak +
				print_domain_name(line, header, data_bak);
			break;
	}
	data += rdlen;
	return (data - data_bak);
...
static char * get_domain_name(char *header, char *data, char *namebuf)
{
	...
	while (1) {
		...
		if ((len & 0xc0) == 0xc0) {
			...
			get_domain_name(header, new_data, name);
			return (data);
		} else {
			while (len > 0) {
				*name = *data;
...
static int print_domain_name(char *line, char *header, char *data)
{
	int total_len = 0;
	char name[1024];
	char *new_data;

	new_data = get_domain_name(header, data, name);

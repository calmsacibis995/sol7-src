...
#ident	"@(#)snoop_capture.c	1.23	97/05/16 SMI"	/* SunOS	*/
	...
	for (bp = buf; bp < bufstop; bp += nhdrp->sbh_totlen) {
		...
		if (nhdrp->sbh_totlen > interface->mtu_size) {
			(void) fprintf(stderr, "offset %d: length=%d\n",
				bp - buf, nhdrp->sbh_totlen);
			if (cap) {
fprintf(stderr, "(warning) packet length greater than MTU in capture file\n");
				continue;
			} else {
fprintf(stderr, "(warning) packet length greater than MTU in buffer\n");
				continue;
			}
		}
		/*

...
#ident	"@(#)rpcbind.c	1.25	98/02/09 SMI"
...
"\tnetdir_getbyname for host HOST_SELF, service \"rpcbind\" succeeded\n");
		...
		if (netdir_getbyname(nconf, &nh, &na) != 0 || na->n_cnt == 0) {
			syslog(LOG_ALERT,
"netid %s: cannot find an address for host HOST_SELF_CONNECT, service \"rpcbind\"",
				np->nc_netid);
			if (debugging)
				fprintf(stderr,
	...
	char *argv[];
{
	...
	while ((c = getopt(argc, argv, "dwal:")) != EOF) {
		...
		case 'l':
			...
			break;
		default:	/* error */
			fprintf(stderr,	"usage: rpcbind [-d] [-w] [-l listen_backlog]\n");
			exit (1);
		}
	}

...
#pragma	ident	"@(#)drvconfig.c	1.37	97/06/19 SMI"
...
static int installing_flag = 0;

static struct mperm *mphead;

#define	R_PERMFILE "etc/minor_perm"
#define	R_ALIASFILE "etc/driver_aliases"
#define	R_DEVICESDIR "devices"
#define	R_NAME_TO_MAJOR "etc/name_to_major"

#define	PERMFILE "/" R_PERMFILE
#define	ALIASFILE "/" R_ALIASFILE
#define	DEVICESDIR "/" R_DEVICESDIR
...
static int translate_major(char *, dev_t, dev_t *);
static void usage();
...
main(int argc, char *argv[])
{
	...
	(void) strcpy(mc.rootdir, DEVICESDIR);
	while ((opt = getopt(argc, argv, "a:bc:dm:np:r:R:i:")) != -1) {
		switch (opt) {
		case 'a':
		...
		case 'R':
			if (chdir(optarg)) {
				fprintf(stderr, "drvconfig: can't change "
					"directory to '%s'\n", optarg);
				exit(1);
			}
			(void) strcpy(mc.rootdir, R_DEVICESDIR);
			permfile = R_PERMFILE;
			alias_file = R_ALIASFILE;
			installing_flag = DRV_INSTALLING;

			/* cache and make sure alternate name_to_major exists */
			if (get_major_no("", R_NAME_TO_MAJOR) == ERROR)
				exit(1);
			break;
		case 'r':
	...
	if (!noload_flag) {
		...
		if (installing_flag != DRV_INSTALLING) {
			if (instance_sync("drvconfig", instance_file, 0) ==
			    -1) {
				exit(1);
			}
		}
	...
	struct FTW *ftw_info)
{
	...
	if ((flags == FTW_F) && ((node_stat->st_mode & S_IFCHR) ||
		...
		if (strncmp(NODE_PREPEND, node + ftw_info->base,
			...
			if ((installing_flag == DRV_INSTALLING) && (ret == 0)) {
				/*
				 * remove and recreate minor node ensuring that
				 * major number is correct
				 */
				if ((unlink(node) != -1) &&
				    (translate_major(drv, node_stat->st_rdev,
					&new_dev_t) != ERROR) &&
				    (mknod(devname, node_stat->st_mode,
					    new_dev_t) == 0))
...
/*
 *  given a driver name and the current dev_t from the running kernel,
 *  return the new_dev_t by looking up in the installed disk's name_to_major.
 */
static int
translate_major(char *driver, dev_t old_dev_t, dev_t *new_dev_t)
{

	major_t major;
	minor_t newminor, oldminor;
	...
	if (strcmp(driver, "clone") != 0) {
		...
		if (UNIQUE == (major = get_major_no(driver,
						    R_NAME_TO_MAJOR))) {
			if (mc.debugflag)
				fprintf(stderr, "drvconfig:  could not find "
					"major number for '%s'\n", driver);
			return (ERROR);
		}

		*new_dev_t = makedev(major, minor(old_dev_t));
		if (mc.debugflag && (old_dev_t != *new_dev_t))
			fprintf(stderr, "drvconfig: driver: '%s' old: %lu,%lu "
				"new: %lu,%lu\n", driver, major(old_dev_t),
				minor(old_dev_t), major(*new_dev_t),
				minor(*new_dev_t));
		return (NOERR);
	} else {
		/*
		 *  for the clone is special.  We need to look at its minor
		 *  since it really represents a major
		 */
		...
		oldminor = minor(old_dev_t);
		if (modctl(MODGETNAME, cdriver, sizeof (cdriver), &oldminor)) {
			if (mc.debugflag)
				fprintf(stderr, "drvconfig: MODGETNAME failed "
					"for major number %lu\n", oldminor);
			return (ERROR);
		}

		if (UNIQUE == (newminor = get_major_no(cdriver,
							R_NAME_TO_MAJOR))) {
			if (mc.debugflag)
				fprintf(stderr, "drvconfig: could not find "
					"major number for '%s'\n", cdriver);
		...
		*new_dev_t = makedev(major, newminor);
		if (mc.debugflag && (old_dev_t != *new_dev_t))
			fprintf(stderr, "drvconfig:  driver: '%s' old: "
				" %lu,%lu  new: %lu,%lu \n", driver,
				major(old_dev_t), minor(old_dev_t),
				major(*new_dev_t), minor(*new_dev_t));
		return (NOERR);
	}
}

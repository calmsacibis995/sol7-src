...
#pragma	ident	"@(#)ps.c	1.42	98/02/09 SMI"
...
#define	TSZ	30
	...
	if (!(aflg|Aflg|dflg|Gflg|Uflg|uflg|tflg|pflg|gflg|sflg)) {
		...
		(void) sprintf(pname, "%s/%ld/psinfo", procdir, getpid());
...
prcom(psinfo_t *psinfo, char *tp)
{
	...
	if (fflg) {						/* UID */
		...
		(void) printf(" %5ld", psinfo->pr_euid);
	}
	(void) printf("%6ld", psinfo->pr_pid);			/* PID */
	if (lflg || fflg)
		(void) printf("%6ld", psinfo->pr_ppid);		/* PPID */
	if (jflg) {
		(void) printf("%6ld", psinfo->pr_pgid);		/* PGID */
		(void) printf("%6ld", psinfo->pr_sid);		/* SID  */
	}
	if (Lflg)
		(void) printf("%6ld", psinfo->pr_lwp.pr_lwpid);	/* LWP */
	if (Pflg) {
	...
	if (lflg) {
		if (yflg) {
			(void) printf("%6u", psinfo->pr_rssize); /* RSS */
			(void) printf("%7u", psinfo->pr_size);	/* SZ */
		} else {
			(void) printf("%9lx",			/* ADDR */
			    (long)psinfo->pr_addr);
			(void) printf("%7u",			/* SZ */
			    psinfo->pr_size/kbytes_per_page);
		}
		if (psinfo->pr_lwp.pr_wchan)
			(void) printf("%9lx",
			    (long)psinfo->pr_lwp.pr_wchan);	/* WCHAN */
		else
			(void) printf("         ");
	}
	if (fflg) {						/* STIME */
...
prtpct(u_short pct, int width)
{
	...
	if ((width -= 2) < 2)
		width = 2;
	(void) printf("%*lu.%lu", width, value / 10, value % 10);
}
...
print_field(psinfo_t *psinfo, struct field *f, char *tp)
{
	...
	case F_RUSER:
		if ((pwd = getpwuid(psinfo->pr_uid)) != NULL)
			...
			(void) printf("%*ld", width, psinfo->pr_uid);
		break;
	case F_USER:
		if ((pwd = getpwuid(psinfo->pr_euid)) != NULL)
			(void) printf("%*s", width, pwd->pw_name);
		else
			(void) printf("%*ld", width, psinfo->pr_euid);
		break;
	case F_RGROUP:
		if ((grp = getgrgid(psinfo->pr_gid)) != NULL)
			(void) printf("%*s", width, grp->gr_name);
		else
			(void) printf("%*ld", width, psinfo->pr_gid);
		break;
	case F_GROUP:
		if ((grp = getgrgid(psinfo->pr_egid)) != NULL)
			(void) printf("%*s", width, grp->gr_name);
		else
			(void) printf("%*ld", width, psinfo->pr_egid);
		break;
	case F_RUID:
		(void) printf("%*ld", width, psinfo->pr_uid);
		break;
	...
	case F_GID:
		...
		break;
	case F_PID:
		(void) printf("%*ld", width, psinfo->pr_pid);
		break;
	case F_PPID:
		(void) printf("%*ld", width, psinfo->pr_ppid);
		break;
	case F_PGID:
		(void) printf("%*ld", width, psinfo->pr_pgid);
		break;
	case F_SID:
		(void) printf("%*ld", width, psinfo->pr_sid);
		break;
	case F_PSR:
		...
		break;
	case F_LWP:
		(void) printf("%*ld", width, psinfo->pr_lwp.pr_lwpid);
		break;
	case F_NLWP:
	...
	case F_PMEM:
		...
		break;
	case F_OSZ:
		(void) printf("%*u", width, psinfo->pr_size/kbytes_per_page);
		break;
	case F_VSZ:
		(void) printf("%*u", width, psinfo->pr_size);
		break;
	case F_RSS:
		(void) printf("%*u", width, psinfo->pr_rssize);
		break;
	case F_NICE:

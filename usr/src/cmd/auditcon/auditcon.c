...
#pragma ident	"@(#)auditconfig.c	1.21	97/11/25	SMI"	/* BSM */
...
do_setstat()
{
	...
	as.as_audit	= (u_int) -1;
	as.as_auditctl	= (u_int) -1;
	as.as_dropped	= (u_int) -1;
	as.as_enqueue	= (u_int) -1;
	as.as_generated	= (u_int) -1;
	as.as_kernel	= (u_int) -1;
	as.as_nonattrib	= (u_int) -1;
	as.as_rblocked	= (u_int) -1;
	as.as_totalsize	= (u_int) -1;
	as.as_wblocked	= (u_int) -1;
	as.as_written	= (u_int) -1;
...
char *cond_str;
{
	...
	(void) eauditon(A_SETCOND, (caddr_t)&cond, (int)sizeof (cond));
}
	...
	u_int *cond;
{
	...
	*cond = 0;
	...
	if (strcmp(buf, "auditing") == 0) {
		*cond = AUC_AUDITING;
		free(buf);
		return (0);
	}

	if (strcmp(buf, "noaudit") == 0) {
		*cond = AUC_NOAUDIT;
		free(buf);
		return (0);
	}
	...
	u_int *policy_mask;
{
	...
	for (tok = strtok(buf, ","); tok != (char *)NULL;
	...
	char *event_str;
{
	...
	if (etype == AC_KERN_EVENT && (evp->ae_number > as.as_numevent)) {
		...
		exit_error("%s %s: \"%s\"",
			...
			event_str,
			event_str,
			gettext(" appears to be a kernel event."),
			gettext("Check configuration"));
	}
	endauevent();
}
...
print_asid(asid)
	au_asid_t asid;
{
	(void) printf(gettext("audit session id = %d\n"), asid);
}
	...
	au_mask_t *pmp;
{
	...
	if (getauditflagschar(auflags, pmp, NULL) < 0)
		(void) strcpy(auflags, gettext("unknown"));
	...
	au_tid_t *tidp;
{
	...
	if ((phe = gethostbyaddr((char *)&tidp->machine,
				sizeof (tidp->machine),

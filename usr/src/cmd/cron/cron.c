...
/*	Portions Copyright(c) 1988, 1997 Sun Microsystems, Inc.	*/
/*	All Rights Reserved.					*/
...
#ident  "@(#)cron.c 1.40     97/11/05 SMI"
...
#define	CANTSTAT	"can't stat home dir.\n Commands will not be executed."
...
int ecid = 1;		/* event class id for el_remove(); MUST be set to 1 */
...
extern char *xmalloc();
extern void* xcalloc(size_t nElements, size_t size);
...
char *Def_path		= NULL;
char path[LINE_MAX]	= "PATH=";
char supath[LINE_MAX]	= "PATH=";
char homedir[LINE_MAX]	= "HOME=";
char logname[LINE_MAX]	= "LOGNAME=";
char tzone[LINE_MAX]	= "TZ=";
char *envinit[] = {
	homedir,
	logname,
	ROOTPATH,
...
void dscan(DIR *dir, int (*fp)());
void add_atevent(struct usr *u, char *job, time_t tim, int jobtype);
void rm_ctevents(struct usr *u);
void cleanup(struct runinfo *rn, int r);
void crabort(char *msg, int type);
void msg();
void parsqdef(char *c);
void defaults();
int initialize(int i);
int quedefs(int i);
int idle(long t);
int ex(struct event *e);
int read_dirs();
int mail(char *a, char *b, int i);
int readcron(struct usr *u);
int next_ge(int i, char *list);
int resched(int i);
int msg_wait();

static time_t	t_old;		/*  1106565 - make global static	*/
...
/*
 * Values returned by verify_user_cred:
 */
...
char **argv;
{
	...
	long seconds;
	pid_t rfork;
	int reset = 0;	/* fix for 1121154 */

	/*
	 * Only the priviledged user can run this command.
	 */
	if (getuid() != 0)
...
begin:
	...
	while (TRUE) {		/* MAIN LOOP */
		...
		/* fix for 1121154 - trying to run at wrong time */
		...
		case CRONEVENT:
			...
			/*
			 * start fix for bugid 1104537.  check if
			 * time(0)< last_time. if so, then the system
			 * clock has gone backwards. to prevent this job
			 * from being started twice, we reschedule this
				...
				if (last_time > time(0)) {
					...
					next_event->time =
					    next_time(next_event,
							(time_t)newtime);
					t_old = time(0); /* 1106565 fix	*/
				} else {
					next_event->time =
					    next_time(next_event, (time_t)0);
				} /* if, else			*/
			} /* end scope of bugfix 1104537	*/
...
initialize(int firstpass)
{
	...
	/* fix for 1088657 - log should be root-only */
...
char	*name;
{
	...
	char	namebuf[MAX_INPUT];
	char	*pname;

	if ((pw = getpwnam(name)) == NULL) {
		if (strcmp(name, ".") != 0 && strcmp(name, "..") != 0) {
			msg("No such user as %s - cron entries not created",
			    name);
		}
		return (0);
	}
...
struct usr *u;
{
	...
	while (fgets(line, CTLINESIZE, cf) != NULL) {
		...
		e = (struct event *) xmalloc(sizeof (struct event));
		e->etype = CRONEVENT;
		if ((e->of.ct.minute = next_field(0, 59, u)) == NULL)
		    goto badline;
		if ((e->of.ct.hour = next_field(0, 23, u)) == NULL)
		    goto badline;
		if ((e->of.ct.daymon = next_field(1, 31, u)) == NULL)
		    goto badline;
		if ((e->of.ct.month = next_field(1, 12, u)) == NULL)
		    goto badline;
		if ((e->of.ct.dayweek = next_field(0, 6, u)) == NULL)
		    goto badline;
		if (line[++cursor] == '\0') {
...
again:
		...
		e->time = next_time(e, (time_t)0); /* fix for 1104537 */
...
badline:
	...
	/* mail message for all blank lines in crontab file */
	if (blank_line) {
		for (i = 0; i < CTLINESIZE; i++)
			line[i] = ' ';
		mail(u->name, EOLN, ERR_CRONTABENT);
	}
...
int format;
{
	...
	if (fork_val == 0) {
		...
		pipe = popen(strcat(strcat(strcpy(temp, MAIL), " "),
				    usrname), "w");
		if (pipe != NULL) {
			fprintf(pipe, "To: %s\n", usrname);
			switch (format) {
			case ERR_CRONTABENT:
				fprintf(pipe, CRONTABERR);
				i = strrchr(line, '\n');
				if (i != NULL)
				    *i = ' ';
				fprintf(pipe, "\t%s\n\t%s\n", line, msg);
...
struct usr *u;
{
	...
	start = cursor;
	/* fix for 1039689 if a newline, return something other than NULL */
	if (line[cursor] == '\n')
	    return ("\n");
	if (line[cursor] == '\0') {
		mail(u->name, EOLN, ERR_CRONTABENT);
...
time_t
next_time(e, tflag)		/* fix for bugid 1104537	*/
struct event *e;
time_t tflag;			/* fix for bugid 1104537	*/
{
	/*
	...
	if (tflag == 0) {		/* start fix for bugid 1104537	*/
		t = time((time_t *)0);	/* original way of doing things	*/
	} else {
		t =  tflag;
	}				/* end fix for bugid 1104537	*/
	tm = localtime(&t);
...
struct event *e;
{
...
#endif
	if ((e->u)->uid == 0) {	/* Fix for Bug 1081824 - set default path */
		/* fix for 1087611 - path settable in defaults file */
		envinit[2] = supath;
	} else {
		envinit[2] = path;
	}
	...
	if (e->etype == CRONEVENT) {
		...
		if ((rp->jobname =
			(char *)malloc(strlen(at_cmdfile)+1)) != NULL)
	...
	/*
	 * fix for bugid 1097578. we make sure that the system time
...

	{
		...
		if ((dhltime - e->time) < 0) {
			...
			delayed++;
			t_old = time((time_t *)0); /* fix for 1106565	*/
			return (0);
		}
	}
	/*
	 * fix for 1121154 - see if we're trying to run at the wrong time
	 * (usually off by an hour for daylight time switch or off by a
	 * day for handling leap years), and if so, reset to correct it.
	 */
	if (e->etype == CRONEVENT) {
		time_t now;
		struct tm *tmnow;
		...
		time(&now);
		tmnow = localtime(&now);
		nextday = next_ge(tmnow->tm_mday, e->of.ct.daymon);
		nexthr = next_ge(tmnow->tm_hour, e->of.ct.hour);

		if (nexthr != tmnow->tm_hour)
		    return (1);
		/*
		 * if both day of the month and day of the week are specified
		 * as a list of elements, both are adhered to.
		 * bugid : 1178638 1255421.
		 */

		if ((strcmp(e->of.ct.dayweek, "*") == 0) &&
			(nextday != tmnow->tm_mday))
		    return (1);
	}
	...
	/* bugid 1230784 : do stat of home dir. in child to avoid cron hang. */
	if (stat(e->u->home, &buf) == -1) {
		msg("can't stat home dir(%s) for user %s.",
			e->u->home, e->u->name);
		msg("cmd \"%s\" not executed", e->cmd);
		mail(e->u->name, CANTSTAT,
	...
	audit_cron_session(e->u->name, e->u->uid);
	/*
	...
	*/
	...
	if (verify_user_cred(e->u) != VUC_OK ||
	    ...
	    setuid(e->u->uid) == -1)  {
		msg("Bad user (%s) or setgid failed or setuid failed (%s)",
			e->u->name, e->u->name);
		exit(1);
	}
	 ...
	 * will be closed when we exec
	 */
	if (getrlimit(RLIMIT_NOFILE, &rl) == 0) {
		for (i = (int)rl.rlim_cur-1; i > 2; i--)
...
long tyme;
{
	...
	while (t > 0L) {
...
#ifdef DEBUG
			fprintf(stderr,
				"in idle - setting alarm for %ld sec\n", alm);
#endif
			alarm((unsigned) alm);
			pid = wait(&prc);
			alarm(0);
		...
		now = time((time_t *)0);
		/*
		 * fix for 1041328 - if there's nothing to do, set
		 * the next time to INFINITY, like we do the first
		 * time in main(), instead of trying to deref a
		 * null struct next_event ptr. If an at job or cron
		 * submission comes along, the alarm time will get
		 * reset then.
		 */
		if (next_event == NULL) {
			if (el_empty())
	...
	if (!lstat(pr->outfile, &buf)) {
		if ((buf.st_mode != S_IFLNK) &&
			...
			if ((pr->pid = fork()) == 0) {
				...
				/*
				 * Get uid for real user and become that
				 * person. We do this so that mail won't
				 * come from root since this could be a
				 * security hole.
				 * If failure, quit - don't send mail as
				 * root.
				 */
...
msg_wait()
{
	...
	if (next_event == NULL)
		...
		/* clock drifted backward, compute new execution time */
		if (next_event->etype == CRONEVENT && last_time > now)
			next_event->time = next_time(next_event, now);
...
defaults()
{
	...
	if (defopen(DEFFILE) == 0) {
		...
		if ((Def_path = defread("PATH=")) != NULL) {
		    ...
		    (void) strcpy(path, NONROOTPATH);
		if ((Def_supath = defread("SUPATH=")) != NULL) {
			(void) strcat(supath, Def_supath);
		} else
		    (void) strcpy(supath, ROOTPATH);
		defopen((char *) NULL);
	}
}
...
verify_user_cred(const struct usr *u)
{
...
#ifdef DEBUG
		fprintf(stderr, "nGroupsMax = %ld\n", nGroupsMax);
#endif
	}

#ifdef DEBUG
	fprintf(stderr, "verify_user_cred (%s-%d)\n", pw->pw_name,
		pw->pw_uid);
	fprintf(stderr, "verify_user_cred: pw->pw_gid = %d, u->gid = %d\n",
		pw->pw_gid, u->gid);
#endif
	...
	if (nGroupsMax > 0) {
		...
		if (OrigGrps) {
			setgroups(numOrigGrps, OrigGrps);
		}
	}

#ifdef DEBUG
	fprintf(stderr, "verify_user_cred: VUC = %d\n", retval);
#endif

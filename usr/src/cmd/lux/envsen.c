...
#pragma ident   "@(#)envsen.c 1.8     97/09/17 SMI"

/*LINTLIBRARY*/


/*
 *	luxadm Administration program
 */
...
read_envsen_data(char *path)
{
	...
	err  = scsi_rec_diag_cmd(fd, (u_char *)buf,
		SCSI_RSM_ES_IN_PAGE_SIZE, PAGE_STRING);
	(void) close(fd);
	if (err != 0) {
		(void) fprintf(stdout,
...
/*
 * Turn on or off or display the current state of the led
 * for the device at path_phys.
 */
void
led(char *path_phys, int options)
{
	char    *s, *s1;
	int	port1, target, ndevices;
	struct	rsm_es_out rsm_es_out;

	P_DPRINTF("led: hunting for %s\n", path_phys);
	if (strstr(path_phys, ":ctlr")) {
		/*
		 * path is of a controller, not a device
		 */
		(void) fprintf(stderr,
			"%s: pathname %s has no user controllable LEDs\n",
			whoami, path_phys);
		exit(-1);
	}
...
fill_rsm_es_out(struct rsm_es_out *rsm_es_out)
{
	...
	(void) bcopy((void *)&rsm_es_out->devstat[0],
			(void *)&envsen_data[0].rsm_es_in->devstat[0],
		...
		int target)
{
	...
	err  = scsi_send_diag_cmd(fd, (u_char *)rsm_es_out,
				SCSI_RSM_ES_OUT_PAGE_SIZE);
	(void) close(fd);
	if (err != 0) {

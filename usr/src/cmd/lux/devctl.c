...
#pragma ident   "@(#)devctl.c 1.2    97/05/02 SMI"


#ifdef	SSA_HOTPLUG
/*
 * this file only used for ssaadm-compatability hotplug stuff
 */
...
dev_handle_insert(char *path)
{
	...
	if ((pp = get_dev_or_bus_phys_name(path)) == NULL) {
	...
	/* acquire device */
	if (devctl_acquire(pp, DC_EXCL, &dcp) != 0) {

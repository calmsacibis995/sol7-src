...
#pragma ident   "@(#)genf.c 1.7     98/01/14 SMI"
...
/*	Global variables */
extern	char	l_error_msg[];	    /* global error message */
extern	char	*l_error_msg_ptr;   /* global pointer to the error message */
...
struct dirent	*entp;
	...
	if ((char_ptr = strstr(drv_path, CTLR_POSTFIX)) == NULL) {
	    ...
	    MSGSTR(-1, "Invalid controller physical path name: %s\n"),
		ctlr_path);
	    /* set error ptr */
	...
	*char_ptr = '\0';	/* Terminate sting  */
	if ((dirp = opendir(drv_path)) == NULL) {
	    (void) sprintf(l_error_msg,
	    MSGSTR(-1, "Error opening directory: %s\n"),
		ctlr_path);
	    /* set error ptr */
	    l_error_msg_ptr = (char *)&l_error_msg;
	    p_error_msg_ptr = (char *)&l_error_msg;
	...
	while ((entp = readdir(dirp)) != NULL) {
		...
		(void) sprintf(l_error_msg,
		MSGSTR(-1, "Invalid device name: %s\n"),
		    entp->d_name);
		/* set error ptr */
		l_error_msg_ptr = (char *)&l_error_msg;
		p_error_msg_ptr = (char *)&l_error_msg;
 ...
 */
char *
get_dev_or_bus_phys_name(char *path)
{
	char	*res;
	char	*phys;
	char	*cp;
	...
	if ((phys = get_physical_name(path)) == NULL) {
		return (NULL);
	}
	...
	if ((strcmp(cp - strlen(CTLR_POSTFIX), CTLR_POSTFIX) != 0) &&
		...
		/* just return the name we got from get_physical_name() */
		return (phys);
	}
	...
	cp = strrchr(phys, ':');
	*cp = '\0';
	res = zalloc(strlen(phys) + FAKE_FILE_NAME_LEN + 1);
	(void) strcpy(res, phys);
	(void) strcat(res, FAKE_FILE_NAME);
	destroy_data(phys);

#ifdef	DEBUG
	P_DPRINTF("get_dev_or_bus_phys_name: "
	    "returning: %s\n", res);
#endif
	return (res);
}
...
struct scsi_command_name {
	...
	SCMD_READ_DEFECT_LIST,	"Read Defect List",
	SCMD_GDIAG,		"Receive Diagnostic",
	SCMD_SDIAG,		"Send Diagnostic",
		...
		char msg_string[])
{
	...
	switch (rq->es_key) {
	case KEY_NO_SENSE:
		(void) sprintf(msg_string, MSGSTR(-1, "No sense error"));
		break;
	case KEY_RECOVERABLE_ERROR:
		(void) sprintf(msg_string, MSGSTR(-1, "Recoverable error"));
		break;
	case KEY_NOT_READY:
		(void) sprintf(msg_string, MSGSTR(-1, "Not ready error"));
		break;
	case KEY_MEDIUM_ERROR:
		(void) sprintf(msg_string, MSGSTR(-1, "Medium error"));
		break;
	case KEY_HARDWARE_ERROR:
		(void) sprintf(msg_string, MSGSTR(-1, "Hardware error"));
		break;
	case KEY_ILLEGAL_REQUEST:
		(void) sprintf(msg_string, MSGSTR(-1, "Illegal request"));
		break;
	case KEY_UNIT_ATTENTION:
		(void) sprintf(msg_string, MSGSTR(-1, "Unit attention error"));
		break;
	case KEY_WRITE_PROTECT:
		(void) sprintf(msg_string, MSGSTR(-1, "Write protect error"));
		break;
	case KEY_BLANK_CHECK:
		(void) sprintf(msg_string, MSGSTR(-1, "Blank check error"));
		break;
	case KEY_VENDOR_UNIQUE:
		(void) sprintf(msg_string, MSGSTR(-1, "Vendor unique error"));
		break;
	case KEY_COPY_ABORTED:
		(void) sprintf(msg_string, MSGSTR(-1, "Copy aborted error"));
		break;
	case KEY_ABORTED_COMMAND:
		(void) sprintf(msg_string, MSGSTR(-1, "Aborted command"));
		break;
	case KEY_EQUAL:
		(void) sprintf(msg_string, MSGSTR(-1, "Equal error"));
		break;
	case KEY_VOLUME_OVERFLOW:
		(void) sprintf(msg_string, MSGSTR(-1, "Volume overflow"));
		break;
	case KEY_MISCOMPARE:
		(void) sprintf(msg_string, MSGSTR(-1, "Miscompare error"));
		break;
	case KEY_RESERVED:
		(void) sprintf(msg_string, MSGSTR(-1, "Reserved value found"));
		break;
	default:
		(void) sprintf(msg_string, MSGSTR(-1, "Unknown error"));
		break;
	}
	...
	if (rq->es_valid) {
		...
		(void) sprintf(&msg_string[strlen(msg_string)],
			MSGSTR(-1, ": block %d (0x%x)"), blkno, blkno);
	}
	...
	if (rq->es_add_len >= 6) {
		(void) sprintf(&msg_string[strlen(msg_string)],
		MSGSTR(-1, "Additional sense: 0x%x   ASC Qualifier: 0x%x\n"),
	...
	if (rq->es_key == KEY_ILLEGAL_REQUEST) {
		string_dump(MSGSTR(-1, "cmd:    "), (u_char *) ucmd,
			sizeof (struct uscsi_cmd), HEX_ONLY, msg_string);
		string_dump(MSGSTR(-1, "cdb:    "), (u_char *) ucmd->uscsi_cdb,
			ucmd->uscsi_cdblen, HEX_ONLY, msg_string);
	}
	string_dump(MSGSTR(-1, "sense:  "),
...
zalloc(int count)
{
	...
	if ((ptr = (void *) calloc(1, (unsigned)count)) == NULL) {
		(void) sprintf(l_error_msg,
			MSGSTR(-1, "Error: unable to calloc more space.\n"));
		l_error_msg_ptr = (char *)&l_error_msg; /* set error ptr */
		p_error_msg_ptr = (char *)&l_error_msg;
	}
	return (ptr);

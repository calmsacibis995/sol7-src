...
#pragma	ident	"@(#)fcupdate.c 1.5     97/08/06 SMI"
...
fc_update(unsigned int verbose, unsigned int force, char *file)
{
	...
	if (!file)
		...
		if (system(strings_buf) == 0) {
			(void) fprintf(stderr,
		MSGSTR(-1, "Error: %s is a FC100/S fcode file\n"), file);
			return;
		}
		(void) sprintf(strings_buf,
		...
		if (system(strings_buf) != 0) {
			(void) fprintf(stderr,
		MSGSTR(-1, "Error: %s is not a FC/S fcode file\n"), file);
			return;
		}
	}
	...
	(void) fprintf(stdout,
	MSGSTR(-1, "Found Path to %d FC/S Cards\n"), numslots);
	...
	for (i = 0; i < numslots; i++) {
		...
		if (force || findstring((u_char *)NULL, (u_char *) soc_name,
				(u_char *) buffer,
					SAMPLE_SIZ) == FOUND) {
			(void) fprintf(stdout,
			MSGSTR(-1, "Found an FC/S card in slot: %s\n"),
			slotname);
			...
			if (fflag) {
				...
				if (findstring((u_char *)&version[0],
				    ...
				    SAMPLE_SIZ) == FOUND) {
					(void) fprintf(stdout,
			MSGSTR(-1, "Detected FC/S Version: %s for %s\n"),
						version, slotname);
				}
			}
		}

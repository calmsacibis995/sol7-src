...
#pragma	ident	"@(#)passwd.c	1.33	97/04/05 SMI"	/* SVr4.0 1.4.3.9 */
	...
	char *setattr[];
{
	...
	int	k;
	int	flag;
	int	entry_n, entry_x, entry_f;
	char		*tmp;

	flag = 0;
	k = 0;
	entry_n = entry_x = entry_f = -1;
	...
	/*
	 * If option -f appears before -x, -n, exchange them in the table
	 * field the forced password change should be the last mark
	 */
	if (entry_f >= 0) {
		int swapentry;
		swapentry = (entry_x > entry_n) ? entry_x : entry_n;
		if (entry_f < swapentry) {
			tmp = setattr[swapentry];
			setattr[swapentry] = setattr[entry_f];
			setattr[entry_f] = tmp;

			/* and so the next swap is under control... */
			if (swapentry == entry_x)
				entry_x = entry_f;
			else	entry_n = entry_f;
		}
	}

	/*
	 * If option -f appears before -x, exchange them in the table because

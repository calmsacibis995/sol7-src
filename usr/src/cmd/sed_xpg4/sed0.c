	...
	while ((ch = getopt(argc, argv, "e:f:n")) != -1) {
		...
		case 'f':
			if ((fin = fopen(optarg, "r")) == NULL) {
				(void) fprintf(stderr,
			gettext("sed: Cannot open pattern-file: %s\n"),
					optarg);
				exit(2);
			}
...
fcomp(char *source)
{
	...
	if (last_bad) {
		(void) fprintf(stderr,
	gettext("sed: Found escape character at end of editing script.\n"));
		exit(2);
	}
	if (*linebuf == '#') {
...
comploop:
		...
		if (our_errno == REEMPTY) {
			...
			(void) fprintf(stderr,
			gettext("sed: The first RE may not be null.\n"));
				exit(2);
			}
		...
		gettext("sed: %s is an unrecognized command.\n"), linebuf);
			...
			case ':':
				...
				if (lpt = search(lab)) {
					if (lpt->address) {
				(void) fprintf(stderr,
			gettext("sed: Duplicate labels: %s\n"), linebuf);
						exit(2);
					}
				} else {
					lab->chain = 0;
			...
			case 's':
				...
				while (strspn(cp, "gpPw0123456789")) {
					...
					gettext(CGMES), linebuf);
						...
						if (nfiles >= MAX_FILES) {
							(void) fprintf(stderr,
	gettext("sed: The w function allows a maximum of ten files.\n"));
							exit(2);
						}
	...
	gettext("sed: Cannot create file %s.\n"), fname[nfiles]);
		...
		if (rep->r1.command == SCOM && !rep->r1.re1)
			if (lastre)
				...
				(void) fprintf(stderr,
			gettext("sed: The first RE cannot be null.\n"));
				exit(2);
			}
		if (++rep >= cmdend) {
			(void) fprintf(stderr,
...
static struct addr *address(void)
{
	...
	if (rcp > cp) {
		...
		*rsp++ = nlno;
		...
		if (nlno >= NLINES) {
			(void) fprintf(stderr,
	gettext("sed: There are too many line numbers specified.\n"));
			exit(2);
		}
		*rsp++ = CCEOF;
		if (rsp >= reend) {
...
static struct addr	*comple(wchar_t reeof)
{
	...
	/* string with last regular expression encountered */

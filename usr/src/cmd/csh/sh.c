...
#ident	"@(#)sh.c	1.31	95/12/05 SMI"	/* SVr4.0 1.6	*/
...
#define TEXT_DOMAIN "SYS_TEST"	/* Use this only if it weren't */
	...
	if ((cp = getenvs_("USER")) != NOSTR)
		...
		if (pw != NULL) {
			...
			setenv(S_USER, strtots((tchar *)0, pw->pw_name ));
		}
...
static fd_set fdinuse;		/* The list of files opened by csh */
	...
	int is_reinit;
{
	...
	if (argc >= 3)
		...
		fcntl(script_fd, F_SETFD);	/* Make sure to close
						 *  this file on exec.
						 */
	...
	for (i = 3; i < NoFile; i++)
		if (i != script_fd) {
			if (is_reinit) {
				if (FD_ISSET(i, &fdinuse)) {
					close(i);
					unsetfd(i);
				}
			} else {
				close(i);	/* No need for unsetfd() */
			}
		}

/*
 * Copyright (c) 1995-1998 by Sun Microsystems, In.c
 * All Rights Reserved
 */

#pragma ident	"@(#)adaptor.c	1.14	98/02/04 SMI"
...
adaptor_available(const char *printer)
{
	...
	for (dir = strtok(tmp_path, ":,"); dir != NULL;
		...
		if (paradigm_version < 0)
			snprintf(object, sizeof (object), "%s/bsd_%s.so",
				dir, paradigm_name);
		else
			snprintf(object, sizeof (object), "%s/bsd_%s.so.%d",
				dir, paradigm_name, paradigm_version);
...
adaptor_function(const char *paradigm, const char *function)
{
	...
	snprintf(name, sizeof (name), "%s_%s", paradigm, function);

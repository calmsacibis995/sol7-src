...
#pragma ident	"@(#)main.c	1.15	98/02/08 SMI"
...
#define	ACK(fp)		{ fputc(NULL, fp); fflush(fp); }
#define	NACK(fp)	{ fputc('\001', fp); fflush(fp); }
...
remote_host_name(FILE *fp)
{
	...
	if (getpeername(fd, (struct sockaddr *)&peer, (socklen_t *)&peer_len)
	    == 0) {

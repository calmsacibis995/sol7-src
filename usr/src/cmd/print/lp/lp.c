...
#pragma ident "@(#)lp.c	1.28	98/02/03 SMI"
...
{
	(void) fprintf(stderr, "No space in /var/spool/print to store job");
	exit(-1);
}
...
submit_local_lp(char *program, int ac, char *av[])
{
	...
	if ((pw = getpwuid(ruid)) != NULL)
		(void) initgroups(pw->pw_name, pw->pw_uid);
	(void) setuid(ruid);
...
error_notify(char *user, int id, char *msg, ...)
{
	if (_notified++ == 0) {
		...
		va_start(ap, msg);
		(void) sprintf(cmd, "/bin/write %s >/dev/null 2>&1", user);
		fp = popen(cmd, "w+");
		(void) fprintf(fp,
...
sysv_options(int ac, char *av[])
{
	...
	while ((c = getopt(ac, av, "H:P:S:T:d:f:i:o:q:t:y:cmwn:prs")) != EOF)
		...
		case 'o':
			...
			s5options = (char **)list_append((void**)s5options,
							(void *)strdup(optarg));
...
stdin_to_file()
{
	...
	name = tempnam("/tmp", "stdin");
	if ((fd = open(name, O_CREAT|O_TRUNC|O_WRONLY|O_EXCL, 0640)) < 0)
		return (NULL);
...
main(int ac, char *av[])
{
	...
	if (check_client_spool(NULL) < 0) {
		(void) fprintf(stderr,
			"couldn't validate local spool area (%s)\n",
			SPOOL_DIR);
		exit(-1);
	}
	...
	if (rejecting(binding->printer) != 0) {
		(void) fprintf(stderr, gettext("%s: queue is disabled\n"),
				printer);
		exit(1);
	}
	 ...
	 */
	sprintf(buf, "/etc/lp/printers/%s/configuration", binding->printer);
	...
	if (mail != 0) {
		(void) sprintf(buf, "%s@%s", user, hostname);

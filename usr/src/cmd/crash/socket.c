...
#pragma ident	"@(#)socket.c	1.8	97/09/07 SMI"
...
prsocket(void *addr, int phys)
{
	...
	readbuf(addr, 0, phys, -1, &sos, sizeof (sos), "socket table slot");
	so = &sos;
	if (full)
		fprintf(fp, "%s", heading);
	...
	if (so->so_laddr_sa != NULL && so->so_laddr_len != 0) {
		readmem(so->so_laddr_sa, 1, -1, &addrbuf,
	...
	if (so->so_faddr_sa != NULL && so->so_faddr_len != 0) {
		readmem(so->so_faddr_sa, 1, -1, &addrbuf,
...
prsoconfig(int full, int lock)
{
	...
	readmem((void *)Sphead->st_value, 1, -1, &addr,
	    sizeof (addr), "sphead");

	while (addr != NULL) {
		readmem(addr, 1, -1, &sps, sizeof (sps), "sockparams");

		if (sps.sp_devpath != NULL) {
			/* NULL terminate */
			devpath[sizeof (devpath) - 1] = NULL;
			readmem(sps.sp_devpath, 1, -1, devpath,

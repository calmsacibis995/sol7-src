...
#pragma ident	"@(#)proc.c	1.21	97/09/07 SMI"
    ...
    char *heading, int lock)
{
	...
	readbuf(addr, (off_t)procaddr, phys, -1,
	...
	if ((t = procbuf.p_tlist) != NULL) {
		readbuf(t, 0, phys, -1, &tb, sizeof (tb), "thread entry");
		switch (tb.t_state) {
	...
	if (lock) {
		if (procbuf.p_lockp != NULL) {
			struct plock pl;
			readbuf(procbuf.p_lockp, 0, phys, -1,
	...
	readmem(procbuf.p_sessp, 1, -1, &sess, sizeof (sess), "session");
	fprintf(fp, "\n\tSession: ");
	...
	readmem(procbuf.p_cred, 1, -1, &uc, sizeof (uc), "process credentials");
...
readsid(struct sess *sessp)
{
	...
	readmem(sessp, 1, -1, &s, sizeof (struct sess), "session structure");
...
readpid(struct pid *pidp)
{
	...
	readmem(pidp, 1, -1, &p, sizeof (struct pid), "pid structure");
...
readuid(struct cred *credp)
{
	...
	readmem(credp, 1, -1, &uc, sizeof (uc), "process credentials");
	return (uc.cr_ruid);
}

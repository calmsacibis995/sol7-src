...
#pragma ident	"@(#)vfs.c	1.8	97/09/07 SMI"
...
prvfs(int full, int phys, void *addr)
{
	...
	readmem(vfsswbuf.vsw_name, 1, -1, fsname, sizeof (fsname),
	    "fs_name");
...
getvfsarg()
{
	...
	if (args[optind]) {
		...
		readmem((void *)Vfs->st_value, 1, -1, &vfsp, sizeof (vfsp),
			"head of vfs list");
		while (vfsp)

...
#pragma ident	"@(#)prnode.c	1.7	97/09/07 SMI"
...
prprnode(int full, long slot, int phys, void *addr, char *heading, int lock)
{
	...
	if (addr != (void *)-1) {
		...
		if (procaddr)
			readbuf(procaddr, 0, phys, -1, &pbuf,
				sizeof (pbuf), "proc table");
		else
			return;
		if (pbuf.p_trace == 0)
			return;
		readmem(pbuf.p_trace, 1, -1, &vnbuf, sizeof (vnbuf), "vnode");
		readmem(vnbuf.v_data, 1, -1, &prnbuf, sizeof (prnbuf),
		    "prnode");
	}
	...
	fprintf(fp, " 0x%.8lu 0x%.8x 0x%.8x 0x%.8x 0x%.8lu 0x%.8x\n",
		...
		prnbuf.pr_ino,
		prnbuf.pr_hatid);
	fprintf(fp, "     0x%.8lu 0x%.8lu 0x%.8lu 0x%.8lu 0x%.8x 0x%.8lu\n",

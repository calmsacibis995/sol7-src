...
#pragma ident	"@(#)lck.c	1.12	97/09/07 SMI"
...
getprocid()
{
	...
	for (i = 0; i < vbuf.v_proc; i++) {
		...
		if (prp == NULL)
			...
			readmem(prp, 1, -1, &prbuf, sizeof (proc_t),
			    "proc table");
			readmem(prbuf.p_pidp, 1, -1, &pid, sizeof (struct pid),
			    "pid table");
	...
	if (args[optind]) {
		do {
			if ((addr = strcon(args[optind], 'h')) == -1)
				error("\n");
			fprintf(fp,
"TYP WHENCE  START    LEN    PID   STATE PREV     NEXT\n");
...
kmprilcks(void *kaddr, void *buf)
{
	...
	lptr = head;
	readmem(lptr, 1, -1, &fibuf, sizeof (fibuf), "filock info");
	...
	do {
		...
		fprintf(fp, "  %s   %-8llx %-8s %-4d   %-5d %04x",
			str,
			fibuf.l_start,
			hexbuf,
...
prilcks()
{
	...
	fprintf(fp,
"INO      TYP  START    END      PROC   PID  STATE   PREV     NEXT     LOCK\n");
...
prlcks(int phys, void *addr)
{
	...
	if (fibuf.l_flock.l_type == F_RDLCK)
		...
		fprintf(fp, " - ");
	fprintf(fp, " %1d       %-8llx %-6d %-6d %04x %-8x %-8x\n",

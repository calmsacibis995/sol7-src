...
#pragma ident	"@(#)stream.c	1.24	97/10/22 SMI"
...
prstream(int all, void *addr, int phys)
{
	...
	readbuf(addr, 0, phys, -1, &st, sizeof (st), "stream table slot");
...
prqueue(int all, void *qaddr, void *uqaddr, int phys)
{
	...
	if (uqaddr) {
		...
		readmem(qaddr, !phys, -1, &q, sizeof (q), "queue");
		qp = &q;
	}
...
prmblk(void *addr, int phys)
{
	...
	readbuf(addr, 0, phys, -1, &mb, sizeof (mb), "mblk");
	dp = mb.b_datap;
	readbuf(dp, 0, phys, -1, &db, sizeof (db), "dblk");
	mp = &mb;
	dp = &db;
	if (strfull)
		fprintf(fp, "%s", mblkheading);
	...
	if (dp->db_frtnp != NULL) {
		frtn_t fr;
		char *fname;
		readbuf(dp->db_frtnp, 0, phys, -1, &fr,
			sizeof (frtn_t), "frtn");
...
prmblkuser(void *kaddr, void *buf, size_t size, kmem_bufctl_audit_t *bcp)
{
	...
	mp = buf;
	readbuf(mp->b_datap, 0, 0, -1, &db, sizeof (db), "dblk");
	dp = &db;
...
prstrstat()
{
	...
	while (qp) {
		qruncnt++;
		readmem(qp, 1, -1, &q, sizeof (q), "queue run list");
		qp = q.q_link;
	}
...
prlinkblk(int all, void *addr)
{
	...
	readmem(addr, 1, -1, &linkbuf, sizeof (linkbuf), "linkblk table");
	lp = &linkbuf;

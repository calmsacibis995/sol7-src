...
#pragma ident	"@(#)disp.c	1.6	97/09/07 SMI"
...
getdispq()
{
	...
	while ((cpu_addr != first_cpu_addr) || (link == &cpu_list)) {
		...
		*link = cpu;
		readmem(cpu_addr, 1, -1, cpu, sizeof (cpu_t), "cpu");
		...
		*link = NULL;
		...
		readmem(disp_tbl_addr, 1, -1, disp_tbl, tbl_size, "disp table");
	}

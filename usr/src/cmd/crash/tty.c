...
#pragma ident	"@(#)tty.c	1.8	97/09/07 SMI"
...
getcount(char *type)
{
	...
	strcpy(temp, type);
	strcat(temp, "_cnt");
	readsym(temp, &count, sizeof (count));
	return (count);
}
...
char *heading;
{
	...
	if ((phys || !Virtmode) && (slot == -1))
		readmem((void *)addr, 0, -1, &tbuf, sizeof (tbuf), "tty");
	else if (slot == -1)
		readmem((void *)addr, 1, -1, &tbuf, sizeof (tbuf), "tty");
	else
		readmem((void *)(addr + slot * sizeof (tbuf)), 1, -1, &tbuf,
			sizeof (tbuf), "tty");
	...
	if (!full) {
		if (ld_type && tbuf.t_state) {
			readmem((void *)(addr +
			    (intptr_t)(slot * sizeof (s_tty))), 1, -1,
			    &s_tty, sizeof (struct strtty), "");
			readmem(s_tty.t_rdqp, 1, -1, &q,
			    sizeof (struct queue), "");
			if (!prsldterm(full, (intptr_t)q.q_next))
					print_header = 1;
		}
		return;
	}
	...
	fprintf(fp, "\tt_line %x\n", tbuf.t_line);
	fprintf(fp, "\tPointer to ioctl block %p\n", tbuf.t_ioctlp);
	fprintf(fp, "\tPointer to large input block %p\n", tbuf.t_lbuf);
	fprintf(fp, "\n");

	if (ld_type) {
		readmem((void *)(addr + (long)(slot * sizeof (s_tty))), 1, -1,
			(char *)&s_tty, sizeof (struct strtty), "");
		readmem(s_tty.t_rdqp, 1, -1, (char *)&q,
			sizeof (struct queue), "");
		if (!prsldterm(full, (intptr_t)q.q_next))
			print_header = 1;
	}
...
intptr_t addr;
{
	 ...
	 * name and id
	 */
	readmem((void *)addr, 1, -1, (char *)&q, sizeof (struct queue), "");
	/*
	...
	readmem(q.q_ptr, 1, -1, &ldterm, sizeof (struct ldterm_mod), "");

	fprintf(fp, "%s", heading);
	fprintf(fp, "\t%-9s%6d%9p%9p%9lu ", mname, minfo.mi_idnum,
	...
	fprintf(fp, "\tpointer to echo buffer %9p\n", ldterm.t_echomp);
...
"\tCode set indicator %o\tbytes left for current character %u\n",
	...
	fprintf(fp,
"\tPointer to array cols. %p\tPointer to m_blk for col. array %p\n",

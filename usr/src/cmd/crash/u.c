...
#pragma ident	"@(#)u.c	1.32	97/09/07 SMI"
...
getublock(long slot, intptr_t addr)
{
	...
	/* Virtmode must be 1 */
	readbuf(procp, 0, 0, -1, &procbuf, sizeof (procbuf), "process table");
...
getuser(void)
{
	...
	if (args[optind]) {
		...
		readmem((void *)V->st_value, 1, -1, &vbuf,
...
pruser(int full, long slot, intptr_t addr, int lock)
{
	...
	readmem(ubp->u_flist, 1, -1, uf, sz, "user file array");
...
fpu_avail(void)
{
	...
	readmem((void *)fp_kind_sym->st_value, 1, -1, &fp_kind,
...
prpcb(void *addr, int full)
{
	...
	readmem(threadb.t_lwp, 1, -1, &lwpb, sizeof (lwpb), "lwp");
	pcbp = &lwpb.lwp_pcb;
...
prstack(char *type, intptr_t stkhi, intptr_t stkfp)
{
	...
	readmem((void *)stkfp, 1, -1, stk, stksize, type);
...
prustk(void *addr, int phys)
{
	...
	readmem(threadb.t_procp, 1, -1, &procb, sizeof (procb), "proc");
	ubp = kvm_getu(kd, &procb);
	readmem(threadb.t_lwp, 1, -1, &lwpb, sizeof (lwpb), "lwp entry");

	if (lwpb.lwp_regs == 0)
		fprintf(fp, "No access to user registers\n");
	else {
		readmem(lwpb.lwp_regs, 1, -1, &rg, sizeof (rg), "ar0 regs");
#if defined(i386)
	prregs(&rg);
...
prktrace(intptr_t addr)
{
	...
	readmem((void *)addr, 1, -1, &threadb, sizeof (threadb), "thread");
#if defined(i386)
	stkfp = rg[EBP];
	cur_pc = rg[EIP];
	prregs(&rg);
...
prkfp(intptr_t addr)
{
	...
	readmem((void *)addr, 1, -1, &tb, sizeof (tb), "thread structure");

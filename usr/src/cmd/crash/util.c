...
#pragma ident	"@(#)util.c	1.22	97/10/22 SMI"
...
/* lseek and read */
void
readmem(void *addr, int mode, int proc, void *buf, size_t size, char *name)
{
	if ((proc != -1) && (addr < (void *)USRSTACK)) {
		struct proc procbuf;
		struct as asbuf;

		readbuf((void *)-1, procv + proc * sizeof (procbuf), 0, -1,
			(char *)&procbuf, sizeof (procbuf), "proc table");
		if (!procbuf.p_stat)
			error("process slot %d doesn't exist\n", proc);
		if (procbuf.p_as == NULL)
			error("no context on slot %d\n", proc);
		readmem(procbuf.p_as, 1, -1, &asbuf, sizeof (asbuf),
		    "address space");
		if (kvm_as_read(kd, &asbuf, (intptr_t)addr, buf, size) != size)
			error("read error on %s at %p\n", name, addr);
	} else if (mode) {
		if (kvm_read(kd, (intptr_t)addr, buf, size) != size)
			error("read error on %s at %p\n", name, addr);
	} else {
		if (lseek(mem, (intptr_t)addr, 0) == -1)
			error("seek error on address %p\n", addr);
		if (read(mem, buf, size) != size)
...
readsym(char *sym, void *buffer, unsigned size)
{
	...
	readmem((void *)np->st_value, 1, -1, buffer, size, sym);
}

/* lseek and read into given buffer */
void
readbuf(void *addr, off_t offset, int phys, int proc,
	void *buffer, size_t size, char *name)
{
	if ((phys || !Virtmode) && (addr != (void *)-1))
		...
		readmem((void *)offset, 1, proc, buffer, size, name);
}
...
makeslottab(void)
{
	...
	if (maketab) {
		...
		readmem(practive, 1, -1, &prp, sizeof (proc_t *), "practive");
		for (; prp != NULL; prp = pr.p_next) {
			readmem(prp, 1, -1, &pr,
				sizeof (struct proc), "proc table");
			readmem(pr.p_pidp, 1, -1, &pid,
...
getcurproc(void)
{
	...
	readmem(Curthread, 1, -1, &thread, sizeof (thread), "current thread");
...
getcurthread(void)
{
	 ...
	 */
	readmem((void *)panic_thread_sym->st_value, 1, -1, &panic_thread,
	...
	cpup = cpu_addr;
	do {
		readmem(cpup, 1, -1, &cpu_addr, sizeof (intptr_t), "cpu addr");
		cpup++;
	} while (cpu_addr == NULL);

	readmem(cpu_addr, 1, -1, &cpu, sizeof (cpu), "cpu structure");
...
kmem_cache_apply(kmem_cache_t *kcp, void (*func)(void *kaddr, void *buf))
{
	...
	for (cpu_seqid = 0; cpu_seqid < ncpus; cpu_seqid++) {
		...
		if (ccp->cc_rounds > 0 &&
			...
			if (kvm_read(kd, (u_long)kmp, (void *)mp,
			    magbsize) == -1)
				goto out2;
...
addr2sym(void *addr)
{
	...
	if (sym)
		return (sym);
	error("cannot find symbol for address 0x%08x in symbol table\n", addr);
}

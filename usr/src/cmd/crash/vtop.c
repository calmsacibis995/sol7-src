...
#pragma ident	"@(#)vtop.c	1.8	97/09/07 SMI"
...
vtop(intptr_t vaddr, long slot)
{
	...
	readbuf((void *)-1, (off_t)slottab[slot].p, 0, -1,
	    &procbuf, sizeof (procbuf), "proc table");
	if (!procbuf.p_stat)
		return (-1LL);
	if (procbuf.p_as == NULL)
		return (-1LL);
	if ((void *)vaddr > (void *)KERNELBASE)
		procbuf.p_as = (struct as *)(symsrch("kas")->st_value);
	readmem(procbuf.p_as, 1, -1, &asbuf, sizeof (asbuf), "address space");

...
#pragma ident	"@(#)ufs_inode.c	1.28	97/09/07 SMI"
	...
	if (args[optind]) {
		...
		do {
			if ((addr = strcon(args[optind], 'h')) == -1)
				continue;
			readbuf((void *)addr, 0, phys, -1, (char *)&vnbuf,
...
prfile(int all, int phys, void *addr)
{
	...
	readbuf(addr, 0, phys, -1, &fbuf, sizeof (fbuf), "file table");
	...
	if (fbuf.f_count && (fbuf.f_vnode != 0)) {
		/* read in vnode */
		readmem(fbuf.f_vnode, 1, -1, &vno, sizeof (vno), "vnode");
	...
	readmem(fbuf.f_cred, 1, -1, credbufp,

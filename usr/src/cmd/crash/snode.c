...
#pragma ident	"@(#)snode.c	1.9	97/09/07 SMI"
...
int lock;
{
	...
	if (addr == -1) {
		readbuf((void *)addr,
		    (long)Stable + slot * sizeof (snp), phys, -1,
		    &snp, sizeof (snp), "snode address");
		if (snp == 0)
			return;
	}
	readbuf((void *)addr, (long)snp, phys, -1, &snbuf, sizeof (snbuf),
	    "snode table");
	...
	for (;;) {
		...
		if ((addr != -1) || (snbuf.s_next == NULL))
			return;
		snp = snbuf.s_next;
		readbuf((void *)addr, (long)snp, phys, -1, (char *)&snbuf,

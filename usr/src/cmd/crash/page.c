...
#pragma ident	"@(#)page.c	1.17	97/09/07 SMI"
...
pras(int full, long slot, int phys, void *addr, char *heading, int lock)
{
	...
	if (procaddr) {
		readbuf(procaddr, 0, phys, -1, &prbuf, sizeof (prbuf),
		    "proc table");
	} else {
		return;
	}
	...
	readbuf((void *)-1, (long)(prbuf.p_as), phys, -1, (char *)&asbuf,
	...
	if (asbuf.a_lrep == AS_LREP_LINKEDLIST) {
		...
		readbuf(addr, (intptr_t)asbuf.a_segs.skiplist, phys, -1,
		    &ssl, sizeof (ssl), "skiplist structure");
		seg = ssl.segs[0];
		readbuf(addr, (intptr_t)asbuf.a_cache.spath, phys, -1,
		    &spath, sizeof (spath), "spath structure");
		readbuf(addr, (intptr_t)spath.ssls[0], phys, -1,
		    &ssl, sizeof (ssl), "skiplist structure");
		seglast = ssl.segs[0];
	}
...
"    BASE       SIZE     AS       NEXT        PREV         OPS        DATA\n");
	...
	do {
		readbuf(addr, (intptr_t)seg, phys, -1, (char *)&segbuf,

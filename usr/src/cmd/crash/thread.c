...
#pragma ident	"@(#)thread.c	1.7	97/09/07 SMI"
...
getthread()
{
	...
	if (args[optind]) {
		...
		readmem((void *)Thread->st_value, 1, -1, &addr, sizeof (addr),
...
prthread(void *addr, int phys, int all, int full, char *heading)
{
	...
	do {
		...
		readbuf(addr, 0, phys, -1, &tb, sizeof (tb), "thread");
...
int change, reset;
{
	...
	if (reset) {
		readmem(Curthread, 1, -1, &t, sizeof (t), "thread struct");

...
#pragma ident	"@(#)rt.c	1.4	97/09/07 SMI"
...
getrtproc()
{
	...
	readmem(Rtproc, 1, -1, &rtbuf, sizeof (rtbuf), "rtproc table");
	...
	for (; rtp != Rtproc; rtp = rtbuf.rt_next) {
		readmem(rtp, 1, -1, (char *)&rtbuf, sizeof (rtbuf), "rtproc");
		prrtproc(&rtbuf);
	}
	return (0);
}

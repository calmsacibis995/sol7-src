...
#pragma	ident	"@(#)vacation.c 1.38     98/01/27 SMI"	/* SVr4.0 1.1   */
 ...
 * publication.
 *
 *	(c) 1986 - 1994, 1997  Sun Microsystems, Inc
...
#ifndef lint
static char	SccsId[] = "@(#)vacation.c	1.38	98/01/27 SMI";
					/* from UCB 5.3 7/1/85 */
#endif not lint
...
char	*Subject = NULL;	/* subject in message header */
...
#ifdef SYSV
	memcpy((char *) &then, (char *) &((struct dbrec *) d.dptr)->sentdate,
		sizeof (then));
#else
	bcopy((char *) &((struct dbrec *) d.dptr)->sentdate, (char *) &then,
		sizeof (then));
#endif
	if (then + Timeout < now)
		return (FALSE);
	...
	char *myname;
{
	...
	if (i == 0) {
		...
		execl("/usr/lib/sendmail", "sendmail", "-eq", "-f", myname,
			"--", user, NULL);
		syserr("can't exec /usr/lib/sendmail");
		exit(1);
	}
	close(pipefd[0]);
	...
	do {
		...
		if (f) {
			...
			if (ask("Would you like to see it")) {
				sprintf(cmd, "/usr/bin/more %s", file);
				system(cmd);
			}
			...
			if (myname)
				fprintf(f,
					"From: %s (via the vacation program)\n",
					myname);

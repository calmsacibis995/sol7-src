/*
 * Copyright (c) 1991, 1992, 1994, 1995, 1997, 1998 by Sun Microsystems, Inc.
 * All rights reserved.
 */

#pragma ident	"@(#)nisedit.c	1.8	98/02/05 SMI"
	...
	nis_name domain;
{
	...
	if (fp == NULL) {
		...
		if (editor == NULL)
			editor = "/usr/bin/vi";
		cmd = (char *)malloc(strlen(editor) + strlen(tmpfname) + 2);
		sprintf(cmd, "%s %s", editor, tmpfname);
		system(cmd);
		fp = fopen(tmpfname, "r");
		...
		*p = '\0';               /* get rid of \n in the string */
		...
		while ((next_char = getc(fp)) != EOF) {
			if (next_char == '\t' || next_char == ' ') {
				...
				p = lp;
				lp = (char *)malloc(strlen(p) + strlen(lbuf));
				if (p != NULL) {
					strcpy(lp, p);
					free(p);
				}
	...
	char *lbuf;
{
	...
	int  cdepth = 0;	/*
				 * if > 0 we're in a user name comment
				 * (in parens)
				 */

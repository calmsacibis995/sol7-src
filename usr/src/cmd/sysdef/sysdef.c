...
#ident	"@(#)sysdef.c	1.25	97/08/09 SMI"	/* SVr4.0 1.37 */
...
char	*LS_MODULES = "/bin/ls -R -p -i -1 /kernel";
 ...
 */
	...
	*/
	 ...
	 */
	if ((lspipe = popen(LS_MODULES, "r")) == NULL) {
	...
	sprintf(srtbuf, "/bin/sort -y0 - > %s",   sorted_fname);
	...
	}
	{
		...
		while (fgets(line, 99, lspipe) != NULL) {
			/* 'line' has <cr>  */
			/* skip blank lines and dir entries */
			...
			if ((line[0] == '/')) {
				...
				if ((tmp = strchr(&line[1],
				    (int)'/')) != NULL) {
					(void) strcpy(path, tmp);
					if ((pathptr = strtok(&path[1],
					    ":")) == NULL)
						pathptr = &path[1];
					(void) strcat(pathptr, "/");
				}
		...
		if ((fp = fopen(sorted_fname, "r")) == NULL) {
 ...
 */
void
setln(char *cf, int nidx, int block, int software)
{
	if (lnptr >= &ln[lnsize]) {
		lnsize = lnptr - ln;
		if ((ln = (struct link *)realloc(ln,
		    (lnsize + EXPAND) * sizeof (struct link))) == NULL) {
			fprintf(stderr,
			    "Internal Link space allocation failed\n");
			exit(1);
		}
		lnptr = &ln[lnsize];
		lnsize += EXPAND;
	...
	lnptr->l_cfnm = malloc(strlen(cf) + 2);	/* add space and null */
	if (!lnptr->l_cfnm)

...
#pragma	ident	"@(#)isainfo.c	1.1	98/01/30 SMI"
...
kisalist(void)
{
	...
	if (uname(&unm) != -1) {
		 ...
		 */
		if (strcmp(unm.machine, "sun4c") == 0 ||
		    strcmp(unm.machine, "sun4m") == 0 ||
...
#define	BITS_FLAG	0x1
#define	NATIVE_FLAG	0x2
#define	KERN_FLAG	0x4
...
main(int argc, char *argv[])
{
	...
	if ((flags & (KERN_FLAG|NATIVE_FLAG)) == (KERN_FLAG|NATIVE_FLAG))
		errflg++;
	...
	do {
		if (verbose)
			printf(vfmt, bits, isa);
		else if (flags & BITS_FLAG)
		  	printf("%d", bits);
		else
		   	printf("%s", isa);

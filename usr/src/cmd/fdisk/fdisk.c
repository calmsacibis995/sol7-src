...
#pragma ident	"@(#)fdisk.c	1.44	97/11/22 SMI"
...
/*
 * Support fdisk(1M) on SPARC platform
 *	byte/short/long swapping macros for accessing
 *	struct mboot and struct ipart to resolve
 *	little endian on SPARC
 */
#ifdef sparc
...
int acyl;			/* Number of Alt sectors       */
	...
	if ( Table[0].systid == UNUSED || io_wholedisk) {
		...
		if ( s[0] == 'y' || s[0] == 'Y' || io_wholedisk) {
			...
			Table[0].begsect = 1;
	        	...
	        	unixend = Numcyl - 1;
			unixstart = 2;
			Table[0].relsect = lel(unixstart * heads * sectors);
			Table[0].numsect = lel((long)((Numcyl-2) * heads * sectors));
	        	Table[0].systid = UNIXOS;   /* Solaris */
#else
...
mboot_read()
{
	...
	if (io_mboot == (char *)0) {
		/*
		 * BugID 1249035
		 *	mboot file must be delivered on all platforms
		 *	and installed in a non platform-dependent
		 *	directory of /usr/lib/fs/ufs.
		 */
...
pcreate()
{
	...
	while (tsystid == 'z') {
		...
		printf("  (6=DOS16, 7=DOSEXT, 8=DOSBIG)\n");
...
clear_vtoc(int new_pt)
{
	...
	if (lseek (Dev, (lel(Table[new_pt].relsect) * sectsiz) + VTOC_OFFSET, 0)
		== -1) {
	...
	for (count = 1; count < 6; count++) {
		...
		if (lseek (Dev, (solaris_offset + backup_block) * 512, 0)
			== -1) {
			fprintf(stderr, "fdisk: Error seeking to backup label"
				" on (%s)\n", Dfltdev);
			return (0);
		}
...
/*
 * Check to see if this disk supports fixed disk partition table.
 */
int
check_support_fdisk()
{
	...
	(void) strcpy(np, Dfltdev);
	/*
	 * Skip if the path is included with pN
	 */
	if (strchr(np, 'p') == NULL) {
		npt = strrchr(np, 's');

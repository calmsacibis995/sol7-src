...
#pragma ident	"@(#)mv.c	1.51	97/04/03 SMI"
...
#define	TEXT_DOMAIN "SYS_TEST"	/* Use this only if it weren't */
	...
	if (strlen(argv[argc-1]) > sizeof(target)) {
		(void) fprintf(stderr,
		gettext("%s: Target %s file name length exceeds MAXPATHLEN %d\n"),
...
chkfiles(char *source, char **to)
{
	...
	if ((*statf)(target, &s2) >= 0) {
		...
		if ((*statf)(target, &s2) >= 0) {
			...
			overwrite = iflg && !silent && isatty(fileno(stdin));
			override = !cpy && (access(target, 2) < 0) &&
			    !silent && isatty(fileno(stdin)) && !ISLNK(s2);
 ...
 * If pflg is set (cp -p) and this is the XPG4 version and utime() fails,
 * print a diagnostic message and arrange for a non-zero
 * exit status.
 */
static int
 ...
 * chg_mode()
 *
 * This function is called upon "cp -p".
 *
 * Try to preserve the owner and group id.  If chown() fails,
 * do not print a diagnostic message or exit with a non-zero value; instead
...
copydir(char *source, char *target)
{
	...
	if (pflg) {
		 ...
		 * ownership, modes, and times can be reserved during "cp -p";
	...
	if (fixmode != (mode_t)0) {
		/*
		 * This is necessary if setacl fails, e.g.
		 * for non-acl file systems
		 */
		if (pflg) {
		...
		if (s1aclp != NULL) {
			if ((acl(target, SETACL, s1aclcnt, s1aclp)) < 0) {
				if (pflg) {
					(void) fprintf(stderr, gettext(

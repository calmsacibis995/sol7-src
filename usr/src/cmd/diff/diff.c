...
#ident	"@(#)diff.c	1.32	97/12/08 SMI"	/* SVr4.0 1.17	*/
...
static void	pfiletype(mode_t);
static void	sort(struct line *, int);
static void	unsort(struct line *, int, int *);
static void	filename(char **, char **, struct stat *);
	...
	if (do_output) {
		while (cvp <= context_vec_ptr) {
			...
			if (ch == 'a')
				...
				fetch(ixold, lowa, b, 0, "  ", 1);
			else {
				/* The last argument should not affect */
				/* the behavior of fetch() */
				fetch(ixold, lowa, a-1, 0, "  ", 1);
				fetch(ixold, a, b, 0,
				    ch == 'c' ? "! " : "- ", 1);
			}
			lowa = b + 1;
...
diffdir(char **argv)
{
	...
	if (opt == D_IFDEF)
		error("cannot specify -D with directories");
	...
	if (fmt1 != S_IFREG || fmt2 != S_IFREG) {
		if (fmt1 == fmt2) {
			...
			if (lflag)
				...
				(void) printf(gettext("File %s is a "), file1);
				pfiletype(fmt1);
				(void) printf(gettext(" while file %s is a "),
					file2);
				pfiletype(fmt2);
				(void) putchar('\n');
			}
...
pfiletype(mode_t fmt)
{
	...
	case S_IFCHR:
		(void) printf(gettext("character special file"));
		break;

	case S_IFBLK:
		(void) printf(gettext("block special file"));
		break;

	case S_IFREG:
		(void) printf(gettext("plain file"));
		break;

	case S_IFIFO:
		(void) printf(gettext("named pipe"));
		break;

	default:
		(void) printf(gettext("unknown type"));
		break;
	}
}
...
static void
filename(char **pa1, char **pa2, struct stat *st)
{
	char *a1, *b1, *a2;
...
copytemp(char *fn)
{
	...
	(void) strcpy(tempfile[whichtemp++],
...
getbufwchar(int filen, int *len)
{
	...
	if (mbcurmax == 1) {
		...
		int	ch = getc(bufwchar[filen].iop);

...
#ident	"@(#)ex_cmds.c	1.16	97/04/18 SMI"	/* SVr4.0 1.22	*/
#include "ex.h"
#include "ex_argv.h"
#include "ex_temp.h"
#include "ex_tty.h"
...
error(value(vi_TERSE) ? gettext("Offset out-of-bounds") :
	gettext("Offset after command too large"));
				setdot1();
				goto print;
			}
		}
...
/* crypt */
			...
			ent_crypt:
				...
				permflag = 1;
				if ((kflag = run_setkey(perm, (key = vgetpass(gettext("Enter key:"))))) == -1) {
					xflag = 0;
					kflag = 0;
					crflag = 0;
...
changdir:
					...
					if (skipend()) {
						...
						if (p == NULL)
							error(gettext("Home directory unknown"));
					} else
						getone(), p = file;
					eol();
					if (chdir(p) < 0)
...
casek:
			...
			if (endcmd(c))
				serror((vi_TERSE) ? gettext("Mark what?") :
					gettext("%s requires following letter"), Command);
			donewline();
			if (!islower(c))
...
/* preserve */
					...
					if (preserve() == 0)
						 ...
						 */
						noerror(gettext("File preserved."));

#else /* XPG4 */
						error(gettext("File preserved."));
#endif /* XPG4 */
					}
				}
				tail2of("print");
				...
				case 'w':
					...
					if (!exclam()) {
						...
						if (chng && dol > zero)
							error((vi_TERSE) ?
								gettext("No write") :
								gettext("No write since last change (:rewind! overrides)"));
					}
					eol();
					erewind();
					next();
...
/* EOF */
		...
		ungetchar(c);
		{
			...
			error((vi_TERSE) ? gettext("What?") :
				gettext("Unknown command character '%s'"), multic);
		}
	}
}

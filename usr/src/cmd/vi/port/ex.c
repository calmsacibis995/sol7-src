...
#ident	"@(#)ex.c	1.17	97/11/26 SMI"	/* SVr4.0 1.28	*/
#include "ex.h"
#include "ex_argv.h"
#include "ex_temp.h"
#include "ex_tty.h"
...
#define TEXT_DOMAIN "SYS_TEST"
	...
	while(1) {
		...
		while ((c = getopt(ac,(char **)av,"VU:Lc:Tvt:rlw:xRCsS")) !=
		    EOF)
#else
		while ((c = getopt(ac,(char **)av,"VLc:vt:rlw:xRCsS")) != EOF)
#endif
			switch(c) {
			case 's':
				hush = 1;
...
#ifdef TRACE
				sprintf((char *)usage,
gettext("Usage: %s [- | -s] [-l] [-L] [-wn] [-R] [-S] [-r [file]] [-t tag] [-T] [-U tracefile]\n\
       [-v] [-V] [-x] [-C] [+cmd | -c cmd] file...\n"), cmdnam);
#else
				sprintf((char *)usage,
gettext("Usage: %s [- | -s] [-l] [-L] [-wn] [-R] [-S] [-r [file]] [-t tag]\n\
       [-v] [-V] [-x] [-C] [+cmd | -c cmd] file...\n"), cmdnam);
#endif

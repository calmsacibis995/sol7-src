...
#ident	"@(#)ls.c	1.40	97/12/11 SMI"	/* SVr4.0 1.38.5.17	*/
...
/*
 *	Copyright (c) 1986,1987,1988,1996,1997 by Sun Microsystems, Inc.
 *	All Rights Reserved.
 */
	...
	if (lflg) {
		...
		if ((p->lmtime < year) || (p->lmtime > now)) {
			(void) cftime(time_buf,
			    dcgettext(NULL, FORMAT1, LC_TIME), &p->lmtime);
			curcol += printf("%s", time_buf);
		} else {
			(void) cftime(time_buf,
			    dcgettext(NULL, FORMAT2, LC_TIME), &p->lmtime);
			curcol += printf("%s", time_buf);
		}
	}
...
findincache(struct cachenode **head, long val)
{
	...
	*parent = c = calloc(1, sizeof (struct cachenode));
	c->val = val;
	return (c);
}

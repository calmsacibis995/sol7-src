...
#ident	"@(#)prt_xxx.c	1.10	97/11/13 SMI"	/* SVr4 */
...
{
	indent_to_level(ilev);
	printf("    Interrupt Priority=0x%x (ipl %d)",
	    ip->intrspec_pri, INT_IPL(ip->intrspec_pri));
	if (ip->intrspec_vec)
		printf(", vector=0x%x (%d)",
		    ip->intrspec_vec, ip->intrspec_vec);
	printf("\n");
}

void
obio_print(caddr_t data, int ilev)
{
...
{
	indent_to_level(ilev);
	printf("    Phys hi=0x%x, Phys lo=0x%x, Phys len=%x\n",
...
void
pcmcia_print(caddr_t data, int ilev)
{
	int i, nreg, nintr;
...
void
sbus_print(caddr_t data, int ilev)
{
	int i, nreg, nrng, nintr;
	struct ddi_parent_private_data *dp;
	struct regspec *reg;
	 ...
	 */
	if ((nintr != 0) && (strcmp(uts_buf.machine, "sun4u") != 0)) {
		indent_to_level(ilev);
		printf("Interrupt Specifications:\n");
...
dump_priv_data(int ilev, di_node_t node)
{
	...
	if ((priv = di_parent_private_data(node)) == NULL)
		return;

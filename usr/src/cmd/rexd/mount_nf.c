#ident	"@(#)mount_nfs.c	1.24	97/04/29 SMI"
...
void		addtomnttab(struct mnttab *);
void		delfrommnttab(char *);
...
/* */
	...
	if (mount("", dir, flags, fstype, &args, sizeof(args)) < 0) {
		netbuf_free(args.addr);
	...
	if (Debug)
		printf("before addtomnttab (%d)\n", errno);

	addtomnttab(&mnt);

	if (Debug)
		printf("after addtomnttab (%d)\n", errno);
	return (0);
}
...
char *mntdir;
{
	...
	if (mnttab == NULL) {
		errprintf(NULL, "Could not open %s\n", MNTTAB);
		return;
	}

	if (lockf(fileno(mnttab), F_LOCK, 0L) < 0) {
		errprintf(NULL, "Cannot lock %s\n", MNTTAB);
		(void) fclose(mnttab);
	}

	/* Read the entire mnttab into memory except for the entry */
	/* we are trying to delete */

	while (getmntent(mnttab, &mnt) == 0) {
		if (strcmp(mnt.mnt_mountp, mntdir) == 0) {
			/* matched to entry we are deleting */
			continue;
		} else {
			/* add entry to list in memory */
	...
	/* now truncate mnttab and write almost all of it back */

	rewind(mnttab);
	if (ftruncate(fileno(mnttab), 0)) {
		errprintf(NULL, "Truncate of %s faield\n", MNTTAB);
		(void) fclose(mnttab);
		return;
	}
	...
	for (mntl = mntl_head; mntl; mntl = mntl->mntl_next) {
		if (putmntent(mnttab, mntl->mntl_mnt) <= 0) {
			errprintf(NULL, "putmntent error in %s\n", MNTTAB);
			(void) fclose(mnttab);
			return;
		}
	}
...
struct mnttab *mntp;
{
	...
	if (putmntent(fp, mntp) <= 0) {
		(void) fclose(fp);
		errprintf("Cannot add to mnttabb:%s\n", MNTTAB);
		return;
	}

/*	Cpyright (c) 1986, 1987, 1988, 1989  Sun Microsystems, Inc	*/
...
#ident	"@(#)man.c	1.36	97/11/18 SMI"	/* SVr4.0 1.3	*/
...
int	whatonly;
int	use_default;
int	compargs;	/* -c option for catman */
...
#define	TEXT_DOMAIN "SYS_TEST"
	...
	while ((c = getopt(argc, argv, opts[catmando])) != -1)
		...
		case 'c':	/* n|troff compatibility */ 
			if (no_sroff)
				printf(gettext("catman: SGML conversion not "
	...
	if (compargs && (nowhatis || whatonly || troffit)) {
		...
		(void) fprintf(stderr, "-c option cannot be used with [-w][-n][-t]\n");
		exit(2);
	} 
...
makecat(char *path, char **dv, int ndirs)
{
	...
	for (i = fmt = 0; i < ndirs; i++) {
		(void) sprintf(mandir, "%s/%s%s", path, MANDIRNAME, dv[i]);
		(void) sprintf(smandir, "%s/%s%s", path, SGMLDIR, dv[i]);
		...
		if (compargs && !manflag) {
			if (mkdir(mandir, 0775) < 0) {
				perror(mandir);
				continue;
			}
			(void) chmod(mandir, 0775);
		}
		...
		if (smanflag) {
			while ((d = readdir(sdp))) {
				...
				if (format(path, sdirp, (char *)0, d->d_name) > 0)
					fmt++;
			}
		}
...
format(char *path, char *dir, char *name, char *pg)
{
	...
	int plen = PLEN;
	register FILE *md;
	int count, temp, sgml_flag = 0, check_flag = 0;
	char prntbuf[2*BUFSIZ];
	char *ptr, *ptrend, *expbuf, *lbuf;
	int i;
	...
	if (*dir == 's') {
		subdirs[0] = SGMLDIR;
		++plen;
		(void) sprintf(manpname_sgml, "%s/man%s/%s", path, dir+plen, pg);
	} else
		subdirs[0] = MANDIRNAME;
...
so_again:	if (++socount > SOLIMIT) {
		...
		if ((check_flag == 1) && ((new_s = strrchr(s, '/')) != NULL)) {
				new_s++;
				(void) sprintf (s, "%s%s/%s", subdirs[0], dir+plen, new_s);
		}
	...
	if (compargs || (!catonly && stat(manpname, &mansb) >= 0 &&
		...
		if (!no_sroff && catmando && match(subdirs[0], MANDIRNAME, PLEN) && 
		...
		expbuf = compile(sgml_symbol, (char *)0, (char *)0);

		if ((temp = open(manpname, 0)) == NULL) {
				perror(manpname);
				return (-1);
		}
		...
		ptr = prntbuf;
		while ((ptrend = memchr (ptr, '\n', prntbuf + count - ptr)) != NULL) {
			*ptrend = '\0';
			lbuf = ptr;
			if (step(lbuf, expbuf)) {
				sgml_flag = 1;
				(void) sprintf(cbp, "%s %s ", SROFF_CMD, manpname);
				cbp += strlen(cbp);
				break;
			}
			ptr = ptrend + 1;
		...
		if (sgml_flag == 1) {
			if (check_flag == 0) {
				(void) sprintf(tmpdir, "/tmp/sman_%d", (pid_t) getpid());
				(void) sprintf(tmpbuf, "%s > %s", cmdbuf, tmpdir);
				if (sys(tmpbuf)) {
					(void) fprintf(stderr, "sys(%s) fail!\n", tmpbuf);
                       		 	(void) fprintf(stderr, gettext(" aborted (sorry)\n"));
                        		(void) fflush(stderr);
                        		return (-1);
				} else {
					if ((md = fopen(tmpdir, "r")) == NULL) {
						(void) fprintf(stderr, gettext("%s: null file\n"),
                            				tmpdir); 
                        			(void) fflush(stderr);
                        			return (-1);
					...
					/* if the file is empty, it's a fragment, do nothing */
                			if (fgets(manbuf, BUFSIZ-1, md) == NULL) {
                        			(void) fclose(md);
						return(1);
                			}
				...
				if (catmando && compargs)
					(void) sprintf(cmdbuf, "cat %s > %s", tmpdir, manpname_sgml);
				else
					(void) sprintf(cmdbuf, " cat %s | tbl | eqn | %s %s %s > %s",
						...
						tmpname);
                        	...
                        	if (catmando && compargs)
                                	(void) sprintf(cbp, " > %s", manpname_sgml);
                        	else
...
{
	FILE *fp;
	static char submandir[MAXDIRS*2];
	char *sect;

#pragma ident	"@(#)MessageReporter.cxx	1.2	97/04/24 SMI"
...
// jsa, 4/14/97 this method added to kill SPRO warning messages.  copied directly from above
						    ...
						    Offset &off)
{
  ...
  if (!(options_ & openEntities)) {
	...
	if (!inputSourceOrigin)
	  break;
	Location defLoc;
	if (!inputSourceOrigin->defLocation(inputSourceOrigin
					    ->startOffset(index),
					    defLoc))
	  break;
	index = defLoc.index();
	origin = defLoc.origin().pointer();
      }
      else {

#pragma ident	"@(#)SOEntityCatalog.cxx	1.2	97/04/24 SMI"
...
private:
  ...
  CharsetInfo sysidCharset_;
  CharsetInfo catalogCharset_;
};
					 ...
					 Messenger &mgr) const
{
  ...
  while (maps.size() > 0) {
    ...
    systemId.unparse(sysidCharset_, catalogSystemId);
				 ...
				 Messenger &mgr)
{
  ...
  in_ = (mustExist
	 ? em->open(sysid, sysidCharset, origin, 0, mgr)
	 : em->openIfExists(sysid, sysidCharset, origin, 0, mgr));
  if (!in_)
    return;
  catalog_ = catalog;
  mgr_ = &mgr;

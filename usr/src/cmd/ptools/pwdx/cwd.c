...
#pragma ident	"@(#)cwd.c	1.10	98/01/30 SMI"
...
#define	NAME_MAX	512
#define	BUF_SIZE	1536	/* 3/2 of PATH_MAX for /a/a/a... case */
...
typedef struct data {
	...
	char	dotdots[BUF_SIZE+NAME_MAX];
	int	dirsize;
	int	diroffset;
	char	dirbuf[1024];
} data_t;
...
{
	char *str = p->path;
	int size = sizeof (p->path);
	int pdfd;	/* parent directory stream */
	struct dirent *dir;
...
static char *
strncpy(char *t, const char *s, size_t n)
{
	char *p = t;

/*
 * Copyright (c) 1996,1997 Sun Microsystems
 * All rights reserved.
 */

#ident	"@(#)msgfmt.c	1.11	97/05/02 SMI"
	...
	char			*msgstr;
{
	...
	while (p1) {
		...
		if (b == 0) {
			...
			if ((node = (struct msg_struct *) malloc
			(sizeof (struct msg_struct))) == NULL) {
				(void) fprintf(stderr,
					"failed to allocate memory");
				exit(2);
			}
	 ...
	 * Therefore, append it.
	 */
	if ((node = (struct msg_struct *) malloc(sizeof (struct msg_struct)))
	== NULL) {
		(void) fprintf(stderr, "failed to allocate memory");
		exit(2);
	}

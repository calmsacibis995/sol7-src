#ifndef lint
static char	*sccsid = "@(#)option.c 1.13 97/12/17 SMI;";
static char	*cmw_sccsid = "@(#)option.c 2.3 92/08/12 SMI; SunOS CMW";
static char	*bsm_sccsid = "@(#)option.c 4.8.1.1 91/11/12 SMI; BSM Module";
static char	*mls_sccsid = "@(#)option.c 3.2 90/11/13 SMI; SunOS MLS";
#endif
...
static audit_pcb_t *get_next_pcb();
static int	a_isnum();
static int	check_file();
int	derive_date();
static int	gather_dir();
int	parse_time();
static int	proc_class();
static int	proc_date();
static int	proc_group();
static int	proc_id();
static int	proc_object();
static int	proc_subject();
static int	proc_type();
static int	proc_user();
static int	process_fileopt();
static obj_ent_t *obj_lkup();
static void	proc_pcb();
static int	proc_file();
...
char	**argv;
{
	...
	int	toptind = 1;	/* index into argv[] - parallels optind */
	int	offset;
	extern int	optind, opterr;	/* in getopt() */
	extern char	*optarg;	/* in getopt() - holds arg to flag */

	static char	*options = "ACD:M:QR:S:VO:a:b:c:d:e:g:j:m:o:r:u:";

	opterr = FALSE;			/* silence getopt() */
...
start_over:
	while ((opt = getopt(argc, argv, options)) != EOF) {
		...
		default:
			/*
			 * Get correct spot in argv[] for error message.
			 */
			if (toptind == optind)
				offset = optind;
				else
				offset = optind - 1;
			(void) sprintf(errbuf, gettext("unknown option (%s)"),
			    (optind > 0) ? argv[offset] : NULL);
			error_str = errbuf;
			error = TRUE;
		}
		if (error) {
		...
		/*
		 * Keep track of getopt()'s position in argv[] for
		 * our error reporting.
		 */
		toptind = optind;
	}
...
char	*optarg;
{
	...
	if ((obj_str = strtok(optarg, "=")) == (char *)0 ||
		...
		(void) sprintf(errbuf,
		    gettext("invalid object arg (%s)"), obj_arg);
		error_str = errbuf;
		return (-1);
	}
	...
	case OBJ_SOCK:
		...
		if (*obj_val == '0') {
			(void) sscanf(obj_val, "%x", &obj_id);
			socket_flag = SOCKFLG_PORT;
			return (0);
		}
		if ((he = gethostbyname(obj_val)) == (struct hostent *)0) {
			(void) sprintf(errbuf,
			    gettext("invalid machine name (%s)"), obj_val);
			error_str = errbuf;
			return (-1);
		}
		(void) memcpy(&obj_id, he->h_addr_list[0], sizeof (obj_id));
 ...
 * .desc	Process a record type. It is either as a number or a
 *	mnemonic.
...
int	opt;
{
	...
	if (opt == 'd') {
		if (m_day == TRUE) {
			error_str =
		gettext("'d' option may not be used with 'a' or 'b'");
			return (-1);
		}
		m_day = TRUE;
	}
	...
	if ((opt == 'a') && (m_after != 0)) {
		error_str = gettext(
			"'a' option specified multiple times");
		return (-1);
	}
	if ((opt == 'b') && (m_before != 0)) {
		error_str = gettext(
			"'b' option specified multiple times");
		return (-1);
	}
 ...
 */
static int
process_fileopt(argc, argv, optindex)
int	argc;
char	**argv;
int	optindex;
{
...
char	*dir;
{
	...
	sprintf(dname, "%s/files", dir);

	if ((dirp = opendir(dname)) == NULL) {
		if (errno != ENOTDIR) {
			(void) sprintf(errbuf,
				gettext("%s can't open directory - %s"),
				ar, dname);
			perror(errbuf);
		}
		return (0);
 ...
 */
static int
proc_file(fname, mode)
char	*fname;
int	mode;
{
	int len, reject = FALSE;
...
int	do_check;
{
	 ...
	 */
	namep = a_calloc(1, strlen(slp));
	(void) strcpy(namep, slp);
	if (namep[14] != '.' || namep[29] != '.') {
		(void) sprintf(errbuf, gettext(
			"invalid filename format (%c or %c)"),
			namep[14], namep[29]);
		error_str = errbuf;
		return (ret);
	}
...
char	*suffix;
{
	...
	/*
	 * Uh-oh, the entire ar ray is used and we haven't gotten one yet.
	 * Allocate a bigger array.
	 */
	pcbsize += PCB_INC;
	size = pcbsize * sizeof (audit_pcb_t);
	zerosize = size - ((pcbsize - PCB_INC) * sizeof (audit_pcb_t));
	if ((audit_pcbs = (audit_pcb_t *) realloc((char *) audit_pcbs,
			size)) == NULL) {
		(void) sprintf(errbuf,
			gettext("%s memory reallocation failed (%d bytes)"),
			ar, size);
		perror(errbuf);
	...
	memset((void *) & audit_pcbs[pcbsize-PCB_INC], 0, (size_t)zerosize);

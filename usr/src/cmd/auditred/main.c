#ifndef lint
static char	*sccsid = "@(#)main.c 1.3 93/02/17 SMI;";
static char	*cmw_sccsid = "@(#)main.c 2.2 92/01/30 SMI; SunOS CMW";
static char	*bsm_sccsid = "@(#)main.c 4.8 91/11/09 SMI; BSM Module";
static char	*mls_sccsid = "@(#)main.c 3.2 90/11/13 SMI; SunOS MLS";
#endif
...
static void	delete_infiles();
static void	rm_outfile();
static void	c_close();
static void	p_close();
static void	trim_mem();
static void	gather_pcb();
static int	mfork();
static void	rm_mem();
static void	calc_procs();
static int	a_pow();
static void	mcount();
static int	close_outfile();
static int	write_file_token();
static int	write_trailer();
static int	rename_outfile();
 ...
 * File globals.
 */
int	max_sproc;	/* maximum number of subprocesses per process */
int	total_procs;	/* number of processes in the process tree */
int	total_layers;	/* number of layers in the process tree */
...
int	lo, hi;
{
	...
	perror(gettext("auditreduce: couldn't get write stream for pipe"));
			...
			/* Parent route. */
				...
				pcbn->pcb_rec = a_calloc(1, AUDITBUFSIZE);
 ...
 * .desc If the user asked us to (via 'D' flag) then unlink the input file
s.
...
{
	if (f_outfile != NULL) {
		f_outtemp = a_calloc(1, strlen(f_outfile) + 8);
		(void) strcpy(f_outtemp, f_outfile);
		(void) strcat(f_outtemp, "XXXXXX");
		if (mktemp(f_outtemp) == NULL) {
			(void) sprintf(errbuf,
			    gettext("%s couldn't create temporary file"), ar);
			perror(errbuf);
			return (-1);
		}
		if (freopen(f_outtemp, "w", stdout) == NULL) {
			(void) sprintf(errbuf,
			gettext("%s can't assign %s to the standard output"),
				ar, f_outfile);
			perror(errbuf);
			return (-1);
		}
...
int	nelem, size;
{
	...
	if ((ptr = (char *)calloc((unsigned) nelem, (unsigned) size)) ==
	    NULL) {
 ...
 * .desc Setup the signal catcher to catch the SIGCHLD signal.
 *	If a child exits with either a non-zero exit code or
 *	was killed by a signal to it then we will also exit with
 *	a non-zero exit code. In this way abnormal conditions can
 *	be passed up to the root process and the entire run be
 *	halted. Also catch the int and quit signals. Remove the
 *	output file since it is in an inconsistent state.
 * .call ret = init_sig().
 * .arg none.
 * .ret 0 - no errors detected.
...
init_sig()
{
	...
	if (signal(SIGILL, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGILL signal failed"));
		return (-1);
	}
	if (signal(SIGTRAP, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGTRAP signal failed"));
		return (-1);
	}
	if (signal(SIGIOT, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGIOT signal failed"));
		return (-1);
	}
	if (signal(SIGEMT, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGEMT signal failed"));
		return (-1);
	}
	if (signal(SIGFPE, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGFPE signal failed"));
		return (-1);
	}
	if (signal(SIGBUS, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGBUS signal failed"));
		return (-1);
	}
	...
	if (signal(SIGSYS, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGSYS signal failed"));
		return (-1);
	}
	if (signal(SIGPIPE, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGPIPE signal failed"));
		return (-1);
	}
	if (signal(SIGALRM, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGALRM signal failed"));
		return (-1);
	}
	...
	if (signal(SIGURG, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGURG signal failed"));
		return (-1);
	}
	if (signal(SIGTTIN, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGTTIN signal failed"));
		return (-1);
	}
	if (signal(SIGTTOU, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGTTOU signal failed"));
		return (-1);
	}
	if (signal(SIGIO, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGIO signal failed"));
		return (-1);
	}
	...
	if (signal(SIGVTALRM, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGVTALRM signal failed"));
		return (-1);
	}
	if (signal(SIGPROF, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGPROF signal failed"));
		return (-1);
	}
	if (signal(SIGWINCH, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGWINCH signal failed"));
		return (-1);
	}
	/*
		if (signal(SIGLOST, int_handler) == SIG_ERR) {
			perror(gettext("auditreduce: SIGLOST signal failed"));
			return (-1);
		}
	*/
	if (signal(SIGUSR1, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGUSR1 signal failed"));
		return (-1);
	}
	if (signal(SIGUSR2, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGUSR2 signal failed"));
		return (-1);
	}
 ...
 * .desc	Catch the SIGINT and SIGQUIT signals. Remove the root process
 *	output file because it is in an inconsistent state.
 * .ret	void.
 */
void

#ifndef lint
static char	*sccsid = "@(#)time.c 1.8 97/12/04 SMI;";
static char	*cmw_sccsid = "@(#)time.c 2.2 92/01/30 SMI; SunOS CMW";
static char	*bsm_sccsid = "@(#)time.c 4.8 91/11/09 SMI; BSM Module";
static char	*mls_sccsid = "@(#)time.c 3.2 90/11/13 SMI; SunOS MLS";
#endif
...
struct tm *tme;
{
	...
	time_t r1secs, r2secs, tm_to_secs();
	...
	(void) memcpy((void *) & ltime,
		(void *)localtime(&secs), sizeof (ltime));
	...
	gsecs -= secs;
	(void) memcpy((void *) & ltime,
		(void *)localtime(&gsecs), sizeof (ltime));
...
/*
 * .func	gctime - print Greenwhich mean time
 * .desc	Print Greenwhich Mean Time in a human readable form.
 * .call ptr = gctime(clock).
 * .arg	clock	- ptr to seconds since epoch.
 * .ret	ptr		- ptr to string.
 */
char	*
gctime(clock)
time_t *clock;
...
struct tm *tme;
{
	...
	if (tme->tm_sec < 0 || tme->tm_sec > 59) {
		...
		if (!(days_in_year(tme->tm_year) == 366 && tme->tm_mon == 1 &&
...
struct tm *tme;
{
	...
	if (len != strspn(str, digits)) {
		(void) sprintf(errbuf, "%s not all digits (%s)",
				do_invalid(), str);
		error_str = errbuf;
		return (-1);
	}

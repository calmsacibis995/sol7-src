#ifndef lint
static char	*sccsid = "@(#)proc.c 1.12 97/11/24 SMI;";
		...
		if (ret > 0) {
			...
			if (header_type == AUT_HEADER32) {
				...
				s > (time_t)INT32_MAX)
					...
					secs = (time_t)s;
			    if (m < (suseconds_t)INT32_MIN ||
				m > (suseconds_t)INT32_MAX)
					msecs = 0;
			    else
					msecs = (suseconds_t)m;
#else
...
register audit_pcb_t *pcb;
{
	 ...
	 */
	while ((uint)(adr.adr_now - adr.adr_stream) < bytes) {
...
char	*fn;
{
	...
	if (id == AUT_OTHER_FILE32) {
	    int32_t secs, msecs;
	    (void) adrf_int32(&adr, (int32_t *) & secs, 1);
	    (void) adrf_int32(&adr, (int32_t *) & msecs, 1);
	} else {
	    int64_t secs, msecs;
	    (void) adrf_int64(&adr, (int64_t *) & secs, 1);
	    (void) adrf_int64(&adr, (int64_t *) & msecs, 1);
#if ((!defined(_LP64)) || defined(_SYSCALL32))
	    if (secs < (time_t)INT32_MIN ||
		secs > (time_t)INT32_MAX) {
...
char	*fn;
{
	...
	case AUT_OTHER_FILE32: {
		...
		adrf_int32(&adr, (int32_t *) & secs, 1);
		adrf_int32(&adr, (int32_t *) & msecs, 1);
	...
	case AUT_OTHER_FILE64: {
		...
		leadin =  2 * sizeof (int64_t) +
				sizeof (short) + sizeof (char);
		adrf_int64(&adr, (int64_t *) & secs, 1);
		adrf_int64(&adr, (int64_t *) & msecs, 1);
		adrf_short(&adr, &ssize, 1);
		*buf = a_calloc(1, ssize + leadin);
		adr_start(&adr, *buf);
		adr_char(&adr, &id, 1);

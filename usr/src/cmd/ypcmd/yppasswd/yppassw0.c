#ident	"@(#)yppasswdd.c	1.13	96/10/23 SMI"
...
main(int argc, char **argv)
{
	...
	if ((nconf = getnetconfigent("udp")) == NULL) {
		syslog(LOG_ERR, "transport not supported\n");
		exit(Egetnetconfigent);
	}
	if ((tli = t_open(nconf->nc_device, O_RDWR, NULL)) < 0) {
		syslog(LOG_ERR, "can\'t open TLI endpoint\n");
		exit(Et_open);
	}
	if (netdir_options(nconf, ND_SET_RESERVEDPORT, tli, NULL)) {
...
#ifdef	DEBUG
	{
		...
		if ((state = t_sync(tli)) < 0) {
		    fprintf(stderr, "t_sync failed: %s\n", t_errlist[t_errno]);
		    exit(Et_sync);
		}
		if (t_getinfo(tli, &tinfo) < 0) {
		...
		case T_IDLE:
		    ...
		    break;
		case T_INREL:
		    fprintf(stderr, "other side wants to release\n");
		    break;
		case T_INCON:
...
#endif
	rpcb_unset((u_long)YPPASSWDPROG, (u_long)YPPASSWDVERS, nconf);
	transp = svc_tli_create(tli, nconf, NULL, 0, 0);
	if (transp == NULL) {
	 ...
	 */
	if ((nconf = getnetconfigent("ticlts")) == NULL) {

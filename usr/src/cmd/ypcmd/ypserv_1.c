...
#ident	"@(#)ypserv_map.c	1.11	97/08/27 SMI"
...
int check_secure_net(struct sockaddr_in *caller, char *ypname);
...
yp_map_access(SVCXPRT *transp, uint_t *error, DBM *fdb)
{
	...
	/*
	 * Joys of TI-RPC.  This'll probably break at some point in the
	 * future.
	 */
	nbuf = svc_getrpccaller(transp);
	caller = ((struct sockaddr_in *) nbuf->buf);

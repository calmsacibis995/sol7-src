 ...
 * All rights reserved.
 */
#pragma ident	"@(#)powerd.c	1.26	98/02/13 SMI"
...
static int		power_fd;	/* power driver */
...
static thread_t		tid;
static char		*autoshutdown_cmd[MAXARGS + 1];
static char		*power_button_cmd[MAXARGS + 1];
static char		*low_battery_cmd[MAXARGS + 1];
...
static int conv2argv(char *, char **);
...
main(int argc, char *argv[])
{
	...
	if (mutex_init(&poweroff_mutex, USYNC_THREAD, NULL) != 0) {
		(void) fprintf(stderr, "Unable to initialize mutex lock\n");
		exit(EXIT_FAILURE);
	}
	...
	/*
	 * If "power" device node can be opened, create a new thread to
	 * monitor the power-button.
	 */
	if ((power_fd = open(POWER, O_RDONLY)) != -1) {
...
kill_handler(int sig)
{
	...
	if (prom_fd != -1)
		(void) close(prom_fd);
	if (power_fd != -1) {
		if (thr_kill(tid, SIGKILL) != 0) {
			LOGERROR("Unable to kill the power-button-monitor "
			    "thread.");
			ret_code = EXIT_FAILURE;
		}
		(void) close(power_fd);
...
work_handler(int sig)
{
	...
	char		name[80], behavior[80] = "noshutdown";
	 ...
	 * Default commands
	 */
	(void) strcpy(inbuf, AUTOS_CMD);
	(void) conv2argv(inbuf, autoshutdown_cmd);
	(void) strcpy(inbuf, POWERB_CMD);
	(void) conv2argv(inbuf, power_button_cmd);
	(void) strcpy(inbuf, LOWB_CMD);
	(void) conv2argv(inbuf, low_battery_cmd);
	...
	while (parse_line(infile, inbuf) != EOF) {
		...
		} else if (strcmp(name, "autoshutdown-cmd") == 0) {
			if (conv2argv(inbuf, autoshutdown_cmd) == -1)
				LOGERROR("error in reading "
				    "\"autoshutdown-cmd\"");
		} else if (strcmp(name, "power-button-cmd") == 0) {
			if (conv2argv(inbuf, power_button_cmd) == -1)
				LOGERROR("error in reading "
				    "\"power-button-cmd\"");
		} else if (strcmp(name, "low-battery-cmd") == 0) {
			if (conv2argv(inbuf, low_battery_cmd) == -1)
				LOGERROR("error in reading "
				    "\"low-battery-cmd\")");
	...
	if (batt_info.status == EOL) {
		...
		if (batt_info.charge == DISCHARGE) {
			...
			if (batt_info.status == EMPTY ||
				...
				close(log_fd);
#endif /* LOG */
				(void) poweroff("Low Battery Shutdown",
				    low_battery_cmd);
				(void) time(now);
			}
		} else {
...
power_button_monitor(void *arg)
{
	...
	while (1) {
		...
		/*
		 * Ioctl returns when the power-button is pressed.
		 */
		if (ioctl(power_fd, POWER_BUTTON_MON, NULL) == -1) {
			LOGERROR("Call to monitor power-button failed.");
			thr_exit((void *) 0);
		}

		if (poweroff(NULL, power_button_cmd) != 0) {
...
static int
conv2argv(char *input_string, char **argv)
{
	int i;
	char *start, *cp;

	/*
	 * Free up previously allocated character arrays (if any).
	 */
	...
	/*
	 * Bypass the command keyword at beginning of the line.
	 */
	...
	while (isspace(*cp))
		cp++;
	if (*cp == '\0') {
		LOGERROR("No command has been found.");
		return (-1);
	}
	...
	while (i < MAXARGS) {
		...
		while (!isspace(*cp) && *cp != '\0')
			cp++;
		if (*cp == '\0') {
			argv[i] = (char *)malloc(strlen(start) + 1);
			if (argv[i] == NULL) {
				LOGERROR("Not enough memory to allocate.");
				return (-1);
			}
			(void) strcpy(argv[i], start);
			return (0);
		}
		*cp = '\0';
		argv[i] = (char *)malloc(strlen(start) + 1);

...
#pragma ident	"@(#)gssdtest.c	1.38	98/01/21 SMI"
...
extern OM_uint32 kgss_acquire_cred();
extern OM_uint32 kgss_release_cred();
extern OM_uint32 kgss_init_sec_context();
extern OM_uint32 kgss_accept_sec_context();
extern OM_uint32 kgss_delete_sec_context();
extern OM_uint32 kgss_process_context_token();
extern OM_uint32 kgss_sign();
extern OM_uint32 kgss_verify();
extern OM_uint32 kgss_seal();
extern OM_uint32 kgss_unseal();
extern OM_uint32 kgss_display_status();
extern OM_uint32 kgss_indicate_mechs();
extern OM_uint32 kgsscred_expname_to_unix_cred();
extern OM_uint32 kgsscred_name_to_unix_cred();
extern OM_uint32 kgss_get_group_info();
extern OM_uint32 gss_release_oid_set_and_oids();
extern OM_uint32 gssd_release_name_and_type();
...
char **argv;
{
	...
	if (init_sec_context_phase == 0) {
		initiator_context_handle = GSS_C_NO_CREDENTIAL;
	...
	if ((status = gss_import_name(&minor_status, &name,
		...
		if (input_token != GSS_C_NO_CREDENTIAL)
	...
	if (status != GSS_S_COMPLETE) {
		printf(gettext("server ret err (octal) %o (%s)\n"),
			status, status > 0 ?
			gettext("gss_export_sec_context_error") :
	...
	if (status != GSS_S_COMPLETE) {
		printf(gettext("server ret err (octal) %o (%s)\n"),
			status, status > 0 ?
			gettext("gss_import_sec_context_error") :
			gettext("system error"));
		return;
	}

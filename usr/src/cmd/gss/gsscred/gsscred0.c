...
#pragma ident  "@(#)gsscred.c 1.13     97/11/07 SMI"
	    ...
	    const char *mechOidStr)
{
	...
	if ((mechOid = gsscred_MechNameToOid(mechOidStr)) == NULL) {
		fprintf(stderr,
	...
	if (name != NULL) {
		...
		if (tableSource == GSSCRED_FLAT_FILE)
			...
			retCode	= xfn_addGssCredEntry(tableSource,
					(const char *)hexMechOid.value,
					(const char *)hexBufDesc.value, uidPtr,
					commentPtr, &errDetail);
...
{
	void *xfnHdl = NULL;
	GssCredEntry entry, *entryPtr, *entryTmpPtr;
	char *searchMech = NULL, hexMech[256],
	...
	if (mechOidStr != NULL) {
		...
		if ((userMechOid = gsscred_MechNameToOid(mechOidStr)) == NULL) {
			fprintf(stderr,
				gettext("\nInvalid mechanism specified [%s]."),
				mechOidStr);
			return (0);
	...
	if ((name == NULL && uidStr == NULL && mechOidStr == NULL) ||
		...
		if (!xfn_getFirstGssCredEntry(tableSource, searchMech,
				&xfnHdl, &entry, &errDetails)) {
			if (errDetails) {
				fprintf(stderr,
					gettext("\nError searching gsscred "
						"table [%s]."),
		...
		fprintf(stdout, "\n%s\t%d\t%s", entry.principal_name,
			entry.unix_uid, entry.comment);
		free(entry.principal_name);
		free(entry.comment);

		while (xfn_getNextGssCredEntry(xfnHdl, &entry)) {
			fprintf(stdout, "\n%s\t%d\t%s", entry.principal_name,
				entry.unix_uid, entry.comment);
			free(entry.principal_name);
			free(entry.comment);
		}
		xfn_deleteGssCredSearchHandle(xfnHdl);
		return (1);
	}
	...
	if (name == NULL && uidStr != NULL) {
		...
		if (tableSource == GSSCRED_FLAT_FILE)
			...
			entryPtr = xfn_getGssCredEntry(tableSource,
					searchMech, NULL, uidStr, &errDetails);
			while (entryPtr != NULL) {
				fprintf(stdout, "\n%s\t%d\t%s",
	...
	for (i = 0; i < numOfMechs; i++) {
		...
		if (mechOidStr == NULL) {
			...
			if (!gsscred_AsHex(&inBufDesc, &outBufDesc))
				continue;
			searchMech = (char *)outBufDesc.value;
		} else
			anOid = userMechOid;
		...
		if (tableSource == GSSCRED_FLAT_FILE)
			...
			entryPtr = xfn_getGssCredEntry(tableSource, searchMech,
					(const char *)searchName.value,
					uidStr, &errDetails);
			while (entryPtr != NULL) {
				fprintf(stdout, "\n%s\t%d\t%s",
					entryPtr->principal_name,
		...
		const char *uidStr, const char *mechOidStr)
{
	...
	if (name == NULL && uidStr == NULL) {
		...
		if (tableSource == GSSCRED_FLAT_FILE)
			...
			status = xfn_deleteGssCredEntry(tableSource, searchMech,
					NULL, NULL, &errDetails);
	...
	if (name == NULL && uidStr != NULL) {
		...
		if (tableSource == GSSCRED_FLAT_FILE)
			...
			status = xfn_deleteGssCredEntry(tableSource,
					searchMech, uidStr, NULL, &errDetails);

...
#pragma ident  "@(#)gsscred_file.c 1.6     97/10/27 SMI"
...
static const char credFile[] = "/etc/gss/gsscred.db";
static const char credFileTmp[] = "/etc/gss/gsscred.db.tmp";
		...
		const char *comment, char **errDetails)
{
	...
	fprintf(fp, "\n%s\t%s\t%s", (char *)hexName->value, uid, comment);
	fclose(fp);
	return (1);
}  /* *******  file_addGssCredEntry ****** */
...
file_getGssCredUid(const gss_buffer_t expName, uid_t *uidOut)
{
	...
	/* go through the file in sequential order - first match exits */
	while (fgets(entry, MAX_ENTRY_LEN, fp) != NULL) {

...
#pragma ident	"@(#)syslogd.c	1.64	98/02/12 SMI"
 ...
 * cause it to re-exec itself, thus reconfiguring.
 *
 * Defined Constants:
 *
...
#define	DOORFILE	"/etc/.syslog_door"

static char		*Version = "1.64";
...
static int nofork = 0;			/* don't fork if re-execing */
static int logfd = -1;			/* log driver file descriptor */
...
main(int argc, char **argv)
{
	...
	/*
	 *	Because we reinitialize via a re-exec
	 *	this early code needs to be protected
	 *	from reiterative signals. So we block
	 *	SIGHUP for now.
	 */
	(void) sighold(SIGHUP);
...
#define	TEXT_DOMAIN "SYS_TEST"
	...
	/*
	 * when allocating space for the Cmdline vector, we allow
	 * for 4 additional arguments - 3 that may be added by
	 * us, and the terminating null pointer.  We will add
	 * some undocumented flags to the command we use for
	 * re-execing ourself.
	 */
	Cmdline = malloc((argc + 4) * sizeof (char *));
	if (Cmdline == NULL) {
		MALLOC_FAIL_EXIT;
	}
	for (i = 0; i < argc; i++) {
		if ((Cmdline[i] = strdup(argv[i])) == NULL) {
			MALLOC_FAIL_EXIT;
		}
	}
	Cmdline[i++] = (char *)NULL;
	...
	Initialized = 0;
	while ((i = getopt(argc, argv, "df:p:m:nz:")) != EOF) {
		switch (i) {
		...
		case 'm':		/* mark interval */
			...
			break;
			/*
			 * the following 2 options are undocumented
			 * and are intended as a means for syslogd
			 * to communicate status back to itself
			 * when it reinitializes (re-execs)
			 */
		case 'n':		/* no fork - undocumented */
			nofork++;
			break;
		case 'z':		/* /dev/log fd (already open) */
			logfd = atoi(optarg);
			break;
		default:
			usage();
	...
	/*
	 * if not invoked with -n (implying we were started by a
	 * user or system boot), add -n to arg vector as a signal
	 * to ourself not to fork if we re-exec (when asked to
	 * reinit by SIGHUP).
	 */
	if (!nofork) {
		for (clp = Cmdline; *clp != NULL; (void)*clp++)
			;
		*clp++ = strdup("-n");	/* add nofork option */
		*clp++ = (char *)NULL;
	}
	...
	/*
	 * close all fd's except 0-2 and our log device fd
	 */
	...
	for (;;) {
		...
		case SIGALRM:
			...
			break;
		case SIGHUP:
			DPRINT0(1, "got SIGHUP - will restart\n");
			flushmsg(0);
...
/*
 * attempts to open the local log device and return a file descriptor.
 * the fd acquired here is treated specially, in that we don't
 * set the close-on-exec flag. The intention is to keep the log open
 * across re-inits, so we don't re-read system messages we've
 * already seen.  The other part of this hack is that we pass in
 * an undocumented flag and argument telling the re-exec'ed syslogd
 * which fd is the log device.
 */
static int
openklog(char *name, int mode)
{
	...
	char **ep;
	/*
	 * if we were re-execed, we already have the log device
	 * open, and should just use the already open fd
	 * that we passed ourself (with -z) across exec.
	 */
	if (nofork && logfd != 0)	/* nofork means we reexeced */
		return (logfd);
	...
	/*
	 * arrange to pass ourself -z fd if we get re-execed. This
	 * is necessary because if we re-open the log, we get
	 * duplicate messages.
	 */
	(void) sprintf(buf, "%d", fd);
	for (ep = Cmdline; *ep != NULL; (void)*ep++)
		;
	*ep++ = strdup("-z");
	*ep++ = strdup(buf);
	*ep++ = (char *)NULL;
...
formatsys(struct log_ctl *lp, char *msg, int sync)
{
	 ...
	 *
	 * NOTE:  This is only a convention and does not guarantee
...
logmsg(void *ap)
{
	...
	for (;;) {
		...
		for (f = Files; f < &Files[nlogs]; f++) {
			...
			if ((mp->flags & MARK) && (mp->ts - f->f_time) <
				(MarkInterval * 60 / 2)) {
				continue;
			}
	...
	DPRINT0(5, "file selector thread exited\n");
	return (NULL);
}
...
logit(void *ap)
{
	...
	for (;;) {
		...
		if ((f->f_msgflag & CURRENT_VALID) &&
			...
			(strcmp(f->f_prevmsg.msg + 16,
				f->f_current.msg + 16) == 0) &&
			...
			if (f->f_current.flags & (MARK) ||
				f->f_prevcount > 0) {
				DPRINT0(2, "logit: new mark or saved\n");
				set_flush_msg(f);
...
set_flush_msg(struct filed *f)
{
	...
	(void) sprintf(f->f_prevmsg.msg+16, "last message "
		"repeated %d %s", f->f_prevcount, tbuf);
	f->f_prevcount = 0;
	f->f_msgflag |= OLD_VALID;
}
...
writemsg(int selection, struct filed *f)
{
	...
	case F_CONSOLE:
		if (flags & IGN_CONS) {
			DPRINT0(1, "Logging to CONSOLE (ignored)\n");
			break;
		}
		/*FALLTHROUGH*/
	case F_TTY:
	...
	case F_WALL:
		...
		(void) strcat(eomp, "\r\n"); /*lint !e669*/
		/*
		** Since wallmsg messes with utmp we need
...
wallmsg(struct filed *f, char *from, char *msg)
{
	...
	if (access(UTMP_FILE, R_OK) != 0 || stat(UTMP_FILE, &statbuf) != 0) {
		...
		(void) sprintf(line, "%s %s", UTMP_FILE,
	...
	while ((utp = getutent()) != NULL) {
		...
		if ((w = malloc(sizeof (walldev_t))) != NULL) {
			...
			(void) strncpy(w->ut_name, utp->ut_name, 10);
	...
	if (ttyf >= 0) {
		if (fstat(ttyf, &statb) != 0) {
			...
			(void) sprintf(errorbuf, "%s %s owns '%s' %s %.*s",
				...
				"but utmp says", UNAMESZ, w->ut_name);
			errno = 0;
			logerror(errorbuf);
...
logerror(char *type)
{
	...
	if (Initialized)
		 ...
		 */
		if ((cfd = open(ctty, O_WRONLY|O_APPEND|O_NOCTTY)) >= 0) {
 ...
 * synchronization. Instead, we create a new file called
 * /etc/.syslog_door and create a door for the library
...
/*ARGSUSED*/
static void
server(void *cookie, char *argp, int arg_size,
	    door_desc_t *dp, int n)
{
...
nofile:
	...
	/*
	 * allocate thread stacks - need enough for 3 main threads
	 * plus 1 for each logger thread.
	 */
	if ((stack_ptr = alloc_stacks(3 + nlogs)) == NULL) {
		perror("alloc_stacks");
		exit(1);
	}
	...
	stack_ptr += stacksize + redzonesize;
	if (pthread_create(&net_thread, &net_attr, net_poll, NULL) != 0) {
		logerror("pthread_create failed");
		exit(1);
	}
...
filed_init(struct filed *f)
{
	...
	(void) pthread_attr_setstackaddr(&stack_attr, stack_ptr);
	stack_ptr += stacksize + redzonesize;
...
cfline(char *line, int lineno, struct filed *f)
{
	...
	case '/':
		...
		f->f_file = open(p, O_WRONLY|O_APPEND|O_NOCTTY|omode);
		if (f->f_file < 0) {
			if (omode && errno == ENXIO) {
				char ebuf[80];
				sprintf(ebuf, "%s - no reader", p);
				errno = 0;
				logerror(ebuf);
			} else
				logerror(p);

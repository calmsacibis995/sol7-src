#ident  "@(#)history.c 1.15     97/03/09 SMI"        /* From AT&T Toolchest */
...
retry:
	...
	if(cp=nam_strval(HISTSIZE))
		maxlines = (unsigned int)sh_arith(cp);
	else
		maxlines = HIS_DFLT;
...
static void hist_trim(n)
{
	...
	hist_ptr->fixfd = fdo;
	hist_ptr = 0;
	if((unlink(name)!=0 && errno != ENOENT) || !hist_open())
	{
...
off_t size;
{
	...
	while((c=io_getb(fd))!=EOF) switch(state)
	{
		case 1:
			if(c==H_CMDNO)
			{
...
void hist_flush()
{
	...
	/* remove whitespace from end of commands */
	{
		...
		*savep = '\0';
		...
		for (fd->ptr = fd->base; fd->ptr <= last; ) {
			c = mb_nextc((const char **)&fd->ptr);
			if (!sh_iswspace(c)) {
				if (c == '\\' &&
					mb_peekc(fd->ptr) != '\n') {
					(void)mb_nextc((const char **)&fd->ptr);
				}
				newlast = fd->ptr - 1;
			}
		}
...
set_count:
	...
	if(flush && !htrim)
	{
		...
		if(fd->flag&IOERR || fsync(fp->fixfd) < 0)
		{
			c = (fp->fixind--)&fixmask;
...
char *nl;
{
	...
	if(offset<0 || !fp)
	{
		p_str((const char *)gettext(e_unknown), '\n');
		return;
	}
	io_seek(fp->fixfd,offset,SEEK_SET);
	while((c = io_getb(fp->fixfd)) != EOF)
	{
		if(c && oldc=='\n')
			p_str(nl,0);
		else if(oldc==last &&  c=='\n')
...
char *string;
{
	...
	int line = 0;
	register wchar_t *wcs, *wcs_save;

	if (!string || (wcs_save = wcs = mbstowcs_alloc(string)) == NULL) {
		return -1;
	}
...
register char *s1;
{
	...
	io_seek(fp->fixfd,offset,SEEK_SET);
	while ((c = io_getc(fp->fixfd)) && c!=WEOF)
	{
		if (c=='\n')
		{
			if(count++ ==line)
...
char *replace;
{
	...
	do {
		int	len;
		wchar_t	x;
		len = sh_mbtowc(&x, new, MB_CUR_MAX);
		new += len;
	} while (*new != '='); /* skip to '=' */
	io_init(fd,&fb,inbuff);
	stakseek(0);
	while ((c=io_getb(fd)) != EOF)
		stakputascii(c);
	string = stakfreeze(1);
	io_fclose(fd);
	*new++ =  0;

#ident	"@(#)jobs.c	1.23	97/03/09 SMI"	/* From AT&T Toolchest */
...
#   endif /* alliant */
	...
	while((job.mytgid=tcgetpgrp(JOBTTY)) != job.mypgid)
	{
		...
		if(ntry++ > MAXTRY)
		{
			p_str((const char *)gettext(e_no_start), NL);
			return;
		}
	}
#  endif /* WAIT1ARG */
...
int job_close()
{
	...
	if(beenhere++ == 0 && job.pwlist)
	{
		...
		if(count)
		{
			p_str((const char *)gettext(e_terminate), NL);
			return(-1);
		}
		else if(running && sh.login_sh)
		{
			p_str((const char *)gettext(e_running), NL);
			return(-1);
		}
	}
#   ifndef  WAIT1ARG
...
char *jobs[];
{
	...
	else while(job = *jobs++)
	{
		...
		if(*job == '%')
		{
			if(pw = job_bystring(job))
				pid = pw->p_pid;
					...
					/*
					 * XPG4: The command identified
					 * by the last pid is unknown
					 */
				cmd_shcfail(e_no_job, ENOTFOUND);
		}
		else
#endif /* JOBS */
			pid = atoi(job);
		/*
		 * XPG4: wait/tp2
		 * P.800: If one or more pid operands are specified
		 * that represent unknown process IDs, wait will
		 * treat them as if they were known process IDS
		 * that exited with exit status 127.
		 */
		if (job_bypid(pid) == (struct process *) NULL) {
			sh.exitval = ENOTFOUND;
			continue;
		}
		/* XPG4: VSC test case wait/tp9 */
		if (jobs && *jobs)
			in_jbwait = 1;
		job_wait(-pid);	/* XPG4: Make sure the last status returned */
		in_jbwait = 0;
	}
}
...
char *jobs[];
{
	...
	else if(*jobs==0)	/* current job */
	{
		 ...
		 * XPG4: (sleep 10 & fg ) inside a couple level
		 * of functions, then pw->p_pgrp = 0.
		 * It makes senses only when job.jobcontrol
		 * is active.
		 */
		while(pw && pw->p_pgrp==0 && job.jobcontrol)
			pw = pw->p_nxtjob;
		if((*fun)(pw,arg))
			r = 2;
	}
...
register int flag;
{
	...
	if((flag&N_FLAG) &&
		(!(px->p_flag&P_NOTIFY)||px->p_pgrp==0))
		return(0);
	...
	do
	{
		...
		{
			msg = (const char *)gettext(e_Done);
			n = px->p_exit;
		}
		else
			msg = (const char *)gettext(e_Running);
		p_str(msg,0);
		msize = mbscolumns(msg);
		if(n)
		{
			p_str(e_nullstr,'(');
			p_num((int)n,')');
		...
		{
			p_str((const char *)gettext(e_coredump), 0);
			msize += mbscolumns((const char *)gettext(e_coredump));
		}
...
#	define stopsig	1
	...
	if(pw->p_flag&P_BYNUM)
	{
		...
		else
		{
			...
			{
				if((r = ksh_killpg(-pid,sig))!=0 && !stopsig)
				{
					job_unstop(job_bypid(pw->p_pid));
					ksh_killpg(-pid,SIGCONT);
				}
			}
		}
#else
	...
	else
	{
		if(pid = pw->p_pgrp)
		{
			if((r = ksh_killpg(pid,sig))>=0 && (sig==SIGHUP||sig==SIGTERM))
				job_unstop(pw);
			if(r>=0)
#   ifdef _poll_
				poll("",0,100);
...
#   endif /* _poll_ */
	...
	error:
		...
		if(errno == EPERM)
			msg = e_access;
		p_str((const char *)gettext(msg), NL);
		r = 2;
	}
	return(r);
}
...
pid_t pid;
{
	...
	job.numpost++;
	if(job.pipeflag && job.pwlist && !job_invalid())
	{
...
#   endif /* apollo */
	...
	while(1)
	{
		...
		if (WIFSTOPPED(wstat))
		{
			...
			if(myjob && (!is_option(INTFLG) || (st.states&FORKED)))
				continue;
		...
		/*
		 * XPG4: Fail wait/tp9 test case as the latter job
		 * to be waited is unposted here before it has
		 * the chance to be waited.
		 */
		else if(!in_jbwait && !in_joblist && pw->p_pgrp==0 && p!=sh.subpid && (!job.pipeflag||p!=job.curpgid))
			job_unpost(pw);
...
register struct process *pw;
{
	...
	/*
	 * XPG4: (sleep 2 & fg ) will hang
	 * fg.sh/tp15
	 */
	if(!bgflag)
		job_wait(job.jobcontrol?pw->p_pid:-pw->p_pid);
...
register struct process *px;
{
	...
	if(num!=0)
	{
		if(px->p_fgrp != px->p_pgrp)
			ksh_killpg(px->p_fgrp,SIGCONT);
		ksh_killpg(px->p_pgrp,SIGCONT);
	}
}
#endif	/* SIGTSTP */
...
register struct process *pwtop;
{
	...
	p_str(" unpost pid",'=');
	p_num(pwtop->p_pid,' ');
	p_str(" unpost  pwtop->p_exit",'=');
	p_num(pwtop->p_exit,NL);
	p_flush();
#endif /* DEBUG */
...
/*
 * CAUTION
 * This function can return results of gettext().
 * So returned string must be used before another call
 * of this function, gettext(), or setlocale().
 * See gettext(3c) for details.
 */
...
int sig;
{
	...
	if ( sig == SIGAPOLLO )
		return( apollo_error() );
#endif /* apollo */
	/* In old code, pointer to returned from strsignal() was stored
	   in sh.sigmsg[sig]. It didn't work well when LC_MESSAGES
	   is changend because strsignal() returns translated string.
	   So, now call strsignal() when message is needed. */
	if (sig == SIGSTOP || sig == SIGTSTP ||
		sig == SIGTTIN || sig == SIGTTOU) {
		/* for these 4 signals, ksh itself has messages */
		return ((char *)gettext((const char *)sh.sigmsg[sig]));
	} else if (sig < (MAXTRAP-1) && ((sd = strsignal(sig)) != NULL)) {
		/* use strsignal() messages */
		return (sd);
	} else {
...
/*
 * job is invalid if the first process on the process list for that job
 * has completed and another process in the list has not
 *
 */
int
job_invalid()
{
	...
	if (px->p_flag & P_DONE)
	{
		for(px=px->p_nxtproc; px; px=px->p_nxtproc)
		{
			/*
			 * we have found another process in the
			 * job that has not been marked as DONE
			 *
			 */
			if (! (px->p_flag & P_DONE))
			{

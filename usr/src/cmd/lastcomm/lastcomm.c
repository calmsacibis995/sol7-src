...
#pragma	ident	"@(#)lastcomm.c	1.5	97/08/12 SMI"	/* SVr4.0 1.2	*/
	...
	for (bn = ((unsigned)sb.st_size / BUF_SIZ) + 1; bn >= 0; bn--) {
		...
		for (; acp >= buf; acp--) {
			...
			printf("%-*.*s %s %-*s %-*s %6.2f secs %.16s\n",
			    ...
			    fldsiz(utmp, ut_user), getname(acp->ac_uid),
...
int
ok(int argc, char *argv[], struct acct *acp)
{
	int j;

...
#pragma ident	"@(#)rmm.c	1.48	96/11/25 SMI"
...
exec_mounts(struct action_arg **aa)
{
	...
	char			*name = getenv("VOLUME_NAME");
	struct mount_args	*ma = NULL;	/* match mount args found */
	int			i;
	int			ret_val = 0;
	char			*mntpt;
	 ...
	 */
	if (mount_args != NULL) {
		for (i = 0; mount_args[i] != NULL; i++) {

			/* try to match the name against the RE */
#ifdef	DEBUG_RE
			dprintf("exec_mounts: regexec(\"%s\", \"%s\") ...\n",
			    mount_args[i]->ma_namere, name);
#endif
			if (regexec(&(mount_args[i]->ma_re), name, 0L, NULL,
			    0) == 0) {
				ma = mount_args[i];
#ifdef	DEBUG_RE
				dprintf("exec_mounts: found a NAME match!\n");
#endif
				break;
			}

			/* try to match the symname against the RE */
#ifdef	DEBUG_RE
			dprintf("exec_mounts: regexec(\"%s\", \"%s\") ...\n",
			    mount_args[i]->ma_namere, symdev);
#endif
			if (regexec(&(mount_args[i]->ma_re), symdev, 0L, NULL,
			    0) == 0) {
				ma = mount_args[i];
#ifdef	DEBUG_RE
				dprintf(
				    "exec_mounts: found a SYMNAME match!\n");
#endif
				break;
			}
		}
...
hard_mount(struct action_arg *aa, struct mount_args *ma)
{
	...
	bool_t		rdonly = FALSE;
	bool_t		nosuid = FALSE;
	bool_t		notraildot = FALSE;
	bool_t		norr = FALSE;
	bool_t		nomaplcase = FALSE;
	bool_t		nointr = FALSE;
	bool_t		hidden = FALSE;
	bool_t		foldcase = FALSE;
	char		lopts[RMM_OPTSTRLEN];	/* mount option string */
	...
	/*
	 * keep track of whether or not the mount args (if any) say
	 * current fs type is supported
	 */
	if (ma == NULL) {
		fs_is_supported = TRUE;		/* support all ?? */
	} else {
		fs_is_supported = fs_supported(aa->aa_type, ma);
	}
	...
	if (sb.st_mode & S_IWUSR) {
		 ...
		 */
		if (ma != NULL) {
			if (fs_is_supported && (ma->ma_flags & MA_READONLY)) {
	...
	if (strcmp(aa->aa_type, "pcfs") != 0) {
		 ...
		 * default is nosuid (except on pcfs)
		 */
		nosuid = TRUE;			/* default value */
		if (ma != NULL) {
			if (fs_is_supported && (ma->ma_flags & MA_SUID)) {
				nosuid = FALSE;
			}
		}
	}
	...
	/*
	 * XXX: at this point we should just give the user the options they've
	 * requested for the filesystem types they've specified ... but ...
	 * if they specify (for example) "mount fred -o notraildot", i.e.
	 * they forget to say "hsfs", then nothing else will mount
	 *
	 * so, despite certain recomdations, try to make sense of the
	 * options requested based on filesystem type
	 */

	if ((ma != NULL) && fs_is_supported) {

		/* check for "pcfs" options */
		if (strcmp(aa->aa_type, "pcfs") == 0) {
			if (ma->ma_flags & MA_HIDDEN) {
				hidden = TRUE;
			}
			if (ma->ma_flags & MA_NOHIDDEN) {
				hidden = FALSE;
			}
			if (ma->ma_flags & MA_FOLDCASE) {
				foldcase = TRUE;
			}
			if (ma->ma_flags & MA_NOFOLDCASE) {
				foldcase = FALSE;
			}
		}

		/* check for "hsfs" options */
		if (strcmp(aa->aa_type, "hsfs") == 0) {
			if (ma->ma_flags & MA_NODOT) {
				notraildot = TRUE;
			}
			if (ma->ma_flags & MA_NORR) {
				norr = TRUE;
			}
			if (ma->ma_flags & MA_NOMAPLCASE) {
				nomaplcase = TRUE;
			}
		}

		/* check for "ufs" options */
		if (strcmp(aa->aa_type, "ufs") == 0) {
			if (ma->ma_flags & MA_NOINTR) {
				nointr = TRUE;
			}
		}
	}
	...
	/* initialize opt string for mount */
	lopts[0] = NULLC;

	/* add in options requested */
	if (rdonly) {
	...
	if (notraildot) {
		(void) strcat(lopts, "notraildot,");
	}
	if (norr) {
		(void) strcat(lopts, "norr,");
	}
	if (nomaplcase) {
		(void) strcat(lopts, "nomaplcase,");
	}
	if (nointr) {
		(void) strcat(lopts, "nointr,");
	}
	if (hidden) {
		(void) strcat(lopts, "hidden,");
	}
	if (foldcase) {
		(void) strcat(lopts, "foldcase,");
	}
	/* removing trailing "," */
	if (lopts[0] != NULLC) {
		lopts[strlen(lopts)-1] = NULLC;
	}
	...
	if ((pid = fork()) == 0) {
		...
		if (lopts[0] == NULLC) {
			...
			dprintf("%s(%ld): %s -F %s -o %s %s %s\n", prog_name,
			    prog_pid, MOUNT_CMD, aa->aa_type, lopts,
	...
	if ((WEXITSTATUS(rval) == 0) &&
		...
		if (lopts[0] == NULLC) {
			...
			dprintf("%s(%ld): \"%s\" mounted (%s) at %s\n",
			    prog_name, prog_pid, mountpoint, lopts,
			    ctime(&tloc));
		}
		aa->aa_mnt = TRUE;
...
clean_fs(struct action_arg *aa)
{
	...
	(void) fprintf(stderr,
	    gettext("%s(%ld) warning: %s is dirty, cleaning (please wait)\n"),
	    prog_name, prog_pid, aa->aa_path);
	...
	if (WEXITSTATUS(rval) != 0) {
		(void) fprintf(stderr, gettext(
	"%s(%ld) warning: \"fsck -p\" of \"%s\" failed, returning %d\n"),
...
exec_umounts(struct action_arg **aa)
{
	...
	if (mount_args != NULL) {
		for (i = 0; mount_args[i] != NULL; i++) {
#ifdef	DEBUG_RE
			dprintf("exec_umounts: regexec(\"%s\", \"%s\")\n",
			    mount_args[i]->ma_namere, name);
#endif
			if (regexec(&(mount_args[i]->ma_re), name, 0L, NULL,
			    0) == 0) {
#ifdef	DEBUG_RE
				dprintf("exec_umounts: found a match!\n");
#endif
				ma = mount_args[i];
				break;
			}
...
#ifdef	DEBUG_MA
	if (ma == NULL) {
		dprintf("exec_umounts: no mount args!\n");
	}
#endif
	...
	for (ai = 0; aa[ai]->aa_path; ai++) {
		...
		if ((ma != NULL) && (ma->ma_flags & MA_SHARE)) {

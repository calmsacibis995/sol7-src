...
#pragma ident	"@(#)eject.c	1.35	97/05/05 SMI"
...
ejectit(char *name, bool_t volmgt_is_running)
{
	 ...
	 */
	if (!volmgt_is_running && _dev_mounted(name)) {
		if (_dev_unmount(name) == 0) {
			if (!force_eject) {
				(void) fprintf(stderr,
...
display_busy(char *path, bool_t vm_running)
{
	...
	uint		bblen;			/* busy_base string length */
	...
	*cp = '\0';
	bblen = (uint)(cp - busy_base);		/* faster than strlen() */

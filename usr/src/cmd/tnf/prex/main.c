...
#pragma ident  "@(#)main.c 1.110 96/09/02 SMI"
...
#if defined(sparc)
#define	PREX32EXEC	"/usr/bin/sparcv7/prex"
#elif defined(i386)
#define	PREX32EXEC	"/usr/bin/i86/prex"
#endif
...
get_elf_class(char *filename)
{
	...
	if ((elffd = open(filename, O_RDONLY)) < 0)
		return (elfclass);
...
{
	(void) execve(PREX32EXEC, argv, envp);
	(void) fprintf(stderr,

...
#pragma ident	"@(#)newkey.c	1.22	97/05/20 SMI"
...
char	*username;
{
	...
	if (uid == 0) {
		/*
		 * This verification is enforced, since newkey(1M) should
		 * only be run by root on the machine (for files), or
		 * on the YP master server (for NIS).
		 *
		 * This section does just that: to authenticate the
		 * user.
		 */
	...
	/*
	 * There is no way to verify the root passwd
	 * of the target host, so we just verified the local root
	 * passwd to authenticate the user and trust the passwd
	 * for the target host entered by the user.
	 */
	if ((! username) && (uid == 0)) {
		...
		pass = getpass(prompt);
		if (pass && strlen(pass) == 0) {
			(void) fprintf(stderr, "%s: Invalid root password.\n",
				program_name);
			return (0);
		}
		strcpy(password, pass);

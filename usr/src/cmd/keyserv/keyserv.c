...
#pragma ident	"@(#)keyserv.c	1.22	97/11/19 SMI"
...
main(argc, argv)
	...
	char *argv[];
{
	int sflag = 0, s1flag = 0, s2flag = 0, nflag = 0;
	char *options, *value;
	extern char *optarg;
	extern int optind;
	int c, d;
	...
	while ((c = getopt(argc, argv, "ndDt:cs:")) != -1)
		switch (c) {
		case 'n':
			nflag++;
			break;
	...
	if (disk_caching) {
		...
		for (i = 0; mechs[i] != NULL; i++) {
			if ((mechs[i]->keylen < 0) || (mechs[i]->algtype < 0)) {
				continue;
			}
...
getotherrootkeys(char *name)
{
	...
	while (fgets_ignorenul(line, MAXROOTKEY_LINE_LEN, rootkey)) {
		...
		if (!addmasterkey(key, name, algtype))
			return (0);
...
failed to generate host's netname when establishing root's key.\n");
	...
	fprintf(fp, "%s\n", secret);
	return (storeotherrootkeys(fp, name, passwd));
}

...
#pragma ident	"@(#)init.c	1.39	97/12/08 SMI"	/* SVr4.0 1.32.3.27 */
...
initialize()
{
	...
	static int states[] = {LVL0, LVL1, LVL2, LVL3, LVL4, LVL5, LVL6,
							SINGLE_USER};
	struct CMD_LINE	cmd;
	char 	line[MAXCMDL];
	FILE	*fp;
...
char	*program;	/* Name of program if INIT_PROCESS, otherwise NULL */
{
	...
	if ((oldu = getutxid(u)) != NULL) {
		 ...
		 * for the time being. The flag __NOOP is used
		 * pam_close_session to short circuit the utmp/wtmp processing
		 * there.
		 */
		if (oldu->ut_user &&
			...
			strncpy(user, oldu->ut_user, sizeof (oldu->ut_user));
			user[sizeof (oldu->ut_user)] = '\0';
			strncpy(ttyn, oldu->ut_line, sizeof (oldu->ut_line));
			ttyn[sizeof (oldu->ut_line)] = '\0';
			strncpy(rhost, oldu->ut_host, sizeof (oldu->ut_host));
			rhost[sizeof (oldu->ut_host)] = '\0';

			if ((pam_start("init", user, NULL, &pamh))
							== PAM_SUCCESS)  {
...
register char *string;
{
	 ...
	 */
	for (ptr2 = &word[0]; ptr2 < &word[sizeof (dummy->ut_user)] &&
	    ptr < string; /* CSTYLED */)
		*ptr2++ = *ptr++;

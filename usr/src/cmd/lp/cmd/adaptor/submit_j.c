...
#pragma ident	"@(#)submit_job.c	1.14	98/02/03 SMI"
...
mail(REQUEST *request, char *req_file, char *fmt, ...)
{
	...
	snprintf(buf, sizeof (buf), "/bin/mail %s", request->user);
	clean_string(buf);
...
parse_cf(char *cf, const char *host)
{
	...
	snprintf(buf, sizeof (buf), "%s%s%s", (host ? host : ""),
		(host ? "!" : ""), (user ? user : "nobody"));
	...
	if (file_list != NULL) {
		...
		snprintf(buf, sizeof (buf), "flist='%-.64s'", tmp);
		appendlist(&options, buf);
		free(tmp);
		freelist(file_list);
	}
			...
			char **df_list)
{
	...
	while ((df_list != NULL) && (*df_list != NULL)) {
		...
		sprintf(buf, "%s/%d-%d", tmp_dir, request_id, ++file_no);
		rename(*df_list++, buf);
		chown(buf, LP_UID, 0);
		if (stat(buf, &st) == 0)
			job_size += st.st_size;
	...
	sprintf(buf, "%s-%d", printer, request_id);
	secure.req_id = strdup(buf);
	secure.uid = LP_UID;
	secure.gid = 0;

	/* save the request file */
	sprintf(buf, "%s/%d-0", host, request_id);
	...
	if (putsecure(buf, &secure) < 0) {
		...
		sprintf(buf, "%s/%s/%d-0", Lp_Tmp, host, request_id);
		unlink(buf);
		unlink_files(request->file_list);
		return (-1);
	}
	...
	if (status != MOK) {
		...
		/* clean it up */
		sprintf(buf, "%s/%s/%d-0", Lp_Requests, host, request_id);
		unlink(buf);
		sprintf(buf, "%s/%s/%d-0", Lp_Tmp, host, request_id);
		unlink(buf);
		unlink_files(request->file_list);

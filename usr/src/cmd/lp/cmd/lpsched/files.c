...
#ident	"@(#)files.c	1.14	97/05/14 SMI"	/* SVr4.0 1.4.1.6	*/
...
_alloc_req_id ( void )
{
	...
	} while ( wrap != curr && (ssp = (RSTATUS *)request_by_id_num(ret)) ) ; 
...
)
{
	...
	if (!prefix) {
		...
		sprintf (base, "%d-%d", id, MOST_FILES);
		plus = 0;
	} else {

...
static char	*bsm_sccsid = "@(#)mkdevalloc.c 1.6 97/11/17 SMI; SunOS BSM";
#endif

/*
 * scan /dev directory for mountable objects and construct device_allocat
e
 * file for allocate....
 *
 * devices are:
...
dotape()
{
	...
	for (i = 0; i < 8; i++) {
		for (j = 0; j < n; j++) {
			if (tape[j].number == i) {
				(void) printf(
					"st%d;st;reserved;reserved;alloc;",
					i);
				(void) printf("%s%s\n", SECLIB, "/st_clean");
				break;
			}
	...
	for (i = 0; i < 8; i++) {
		for (j = 0; j < n; j++) {
			if (audio[j].number == i) {
				...
				(void) printf("reserved;reserved;alloc;");
				(void) printf("%s%s\n", SECLIB, "/audio_clean");
				break;
			}
		}
...
dofloppy()
{
	...
	int sz;			/* size of symlink value */
	char *cp;		/* pointer into string */
	int first;		/* kludge to put space between entries */
	...
	while (dep = readdir(dirp)) {
		...
		strncmp(dep->d_name, "rfd0", SIZE_OF_RFD0)) continue;
	    ...
	    if ((stat.st_mode&S_IFMT) != S_IFLNK) continue;

		/* get name from symbolic link */
	    if ((sz = readlink(fp[i].name, linkvalue, sizeof (linkvalue))) < 0)
		continue;
	    nm = (char *)malloc(sz+1);
	...
	for (i = 0; i < 8; i++) {
	    for (j = 0; j < n; j++) {
		if (fp[j].number == i) {
		    printf("fd%d;fd;reserved;reserved;alloc;", i);
		    printf("/etc/security/lib/fd_clean\n");
		    break;
		}
	    }
	}
...
found1:
	...
	for (i = 0; i < 8; i++) {
		for (j = 0; j < n; j++) {
			if (cd[j].id == i) {
				(void) printf(
					"sr%d;sr;reserved;reserved;alloc;",
					i);
				(void) printf("%s%s\n", SECLIB, "/sr_clean");
				break;
			}

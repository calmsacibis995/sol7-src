...
/*
 * this routine enables or disables BRK or abort sequence feature
 */
static int
...
kbd_defaults(int kbd)
{
	...
	if (p != NULL) {
		/*
		 * ABORT must equal "enable" or "disable"
		 */
		if ((strcmp(p, "enable") == 0) || (strcmp(p, "disable") == 0))
...
static char *usage1 =
	"kbd [-r] [-t] [-a enable|disable] [-c on|off] [-d keyboard device]";

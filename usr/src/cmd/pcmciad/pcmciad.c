...
#ident "@(#)pcmciad.c	1.42     97/06/09 SMI"
...
/*
 * load_nexi()
 *	run through the /etc/driver_classes file and modload()
 *	each driver in the "pcmcia" class.
 */
...
{
	FILE *file;
	char buff[256], driver[64], class[64];

	file = fopen("/etc/driver_classes", "r");
	if (file == NULL)
		return;
	if (debug & DBG_MODLOAD)
		(void) printf("loading nexi\n");
	while (fgets(buff, sizeof (buff), file) != NULL) {
		(void) sscanf(buff, "%s %s", driver, class);
		if (debug & DBG_MODLOAD)
			(void) printf("\t%s\n", driver);
		if (strcmp(class, "pcmcia") == 0)
			(void) modload(driver);
	}
}
	...
	case EM_EVENT_IND:
		...
		case PCE_INIT_DEV:
			...
			if (md->op == SS_CSINITDEV_CREATE_DEVICE) {
			    fix_device_path(pcmcia_root_dir, md->path);
			}
			arg = (void *)md;
			break;
...
modload(char *name)
{
	...
	(void) memset((char *)&mc, '\0', sizeof (mc));
	...
	if ((ret = modctl(MODCONFIG, NULL, (caddr_t)&mc)) < 0) {
		char *alias;
		/* try decoding the possible alias */
		if ((alias = find_alias(name)) != NULL) {
			if (debug & DBG_MODLOAD) {
				(void) printf("Found an alias: %s\n",
								alias);
			}
			ret = modload(alias);
			if (ret >= 0)

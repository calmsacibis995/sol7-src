...
#ident	"@(#)dc.c	1.11	96/12/19 SMI"	/* SVr4.0 1.5.1.5	*/
...
commnds(){
	...
	while(1){
		...
		case 'Y':
			...
			printf(gettext("longest %ld active %ld maxsize %ld\n"), longest,
				active, maxsize);
			printf(gettext("new all %d rel %d copy %d more %d lbytes %d\n"),
...
casediv:
		...
		case '^':
			...
			arg1 = pop();
			EMPTY;
			if(sunputc(arg1) != 0)error(gettext("exp not an integer\n"));
			arg2 = pop();
			EMPTYR(arg1);
...
ddone:
	...
	else if(under) chsign(divr);		/* restore undershot adjust */
	rem = divd;
	return(p);
}
dscale(){
...
load(){
	...
	if(sptr != 0){
		...
		q = salloc(1);
		if(c <= LASTFUN){
			printf(gettext("function %c undefined\n"),c+'a'-1);
			sputc(q,'c');
			sputc(q,'0');
			sputc(q,' ');
			sputc(q,'1');
			...
			if(i < ARRAYST){
				...
				do {
					...
					while((q = getwd(p)) != NULL){
						...
						if(q != 0){
							if(((int)q->beg & 01) != 0){
								printf(gettext("array %o elt %d odd\n"),i-ARRAYST,ct);
printf(gettext("tmps %o p %o\n"),tmps,p);
								sdump("elt",q);
							}
							redef(q);
						}

...
#pragma ident	"@(#)rmtab.c 1.7     97/01/02 SMI"
...
/*
 * XXX Fix for bug 4023817
 * This is an iterative version of xdr_mountlist
 * that avoids the recursive, stack-blowing behavior
 * of the routine generated by rpcgen.  The correct
 * fix is to make rcpgen produce non-recursive code
 * for encoding/decoding linked lists - but until that's
 * done - this local encode-only version will have to do.
 *
 * See also: bug 1197979: Recursive use of xdr_pointer ...
 */
static bool_t
xdr_mountlist(xdrs, objpp)
	register XDR *xdrs;
	mountlist **objpp;
{
	return (xdr_mountbody(xdrs, (mountbody *) *objpp));
}

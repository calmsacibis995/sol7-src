...
#ident	"@(#)mountd.c	1.54	98/02/17 SMI"	/* SVr4.0 1.11  */
...
mutex_t mnttab_lock;		/* prevent concurrent mnttab readers */
...
void check_sharetab(void);
struct share *findentry(char *);
...
static int getclientsflavors_new(struct share *, struct netbuf *,
	struct nd_hostservlist *, int *);
int check_client_new(struct share *, struct netbuf *,
	struct nd_hostservlist *, int);
...
anon_client()
{
	...
	anon_hsl = (struct nd_hostservlist *) malloc(sizeof (*anon_hsl));
	if (anon_hsl == NULL)
		return (NULL);

	anon_hs = (struct nd_hostserv *) malloc(sizeof (*anon_hs));
	if (anon_hs == NULL) {
		free(anon_hsl);
		return (NULL);
	}
 ...
 */
void
getclientsnames(transp, nbuf, serv)
	SVCXPRT *transp;
	struct netbuf **nbuf;
 ...
 */
static int
mount_enoent_error(path, rpath, clnames, nb, flavor_list)
	char *path;
	char *rpath;
	struct nd_hostservlist *clnames;
	struct netbuf *nb;
	...
	struct svc_req *rqstp;
{
	...
	if (version == MOUNTVERS3) {
		...
		fhp = fh3;
		memset(fhp + NFS3_CURFHSIZE, 0, NFS3_FHSIZE-NFS3_CURFHSIZE);
	} else {
 ...
 * Refer Bugid 1210409
 * Stat the rpath
 * Gets the mnttab and skips the entries that are not lofs.
 * Looks for the mountpoint which is a substring of the rpath.
...
find_lofsentry(char *rpath, int *done_flag)
{
	...
	char stat_path[MAXPATHLEN+2];

	if (*done_flag++)
		return (retcode);

	strcpy(stat_path, rpath);
	strcat(stat_path, "/.");
	if (stat(stat_path, &r_stbuf) != 0)
		return (retcode);

	/*
	 * While fsgetmntlist() uses lockf() to
	 * lock the mnttab before reading it in,
	 * the lock is ignorant of threads in
	 * the same process.  Mayhem can result.
	 * So read in the mnttab with the protection
	 * of a mutex.
	 */
 ...
 */
in_access_list(nb, clnames, access_list)
	struct netbuf *nb;
	struct nd_hostservlist *clnames;
	char	*access_list;	/* N.B. we clobber this "input" parameter */
{
	...
	for (gr = (char *) strtok_r(access_list, ":", &lasts);
		gr != NULL; gr = (char *) strtok_r(NULL, ":", &lasts)) {
...
netmatch(struct netbuf *nb, char *name)
{
	...
	memcpy(&claddr, &((struct sockaddr_in *) nb->buf)->sin_addr.s_addr,
 ...
 */
static int
getclientsflavors_old(sh, nb, clnames, flavors)
	struct share *sh;
	struct netbuf *nb;
	struct nd_hostservlist *clnames;
	int *flavors;
 ...
 */
static int
getclientsflavors_new(sh, nb, clnames, flavors)
	struct share *sh;
	struct netbuf *nb;
	struct nd_hostservlist *clnames;
	int *flavors;
{
	...
	while (*p) {
		...
		case OPT_SEC:
			...
			while ((f = (char *) strtok_r(val, ":", &lasts))
					!= NULL) {
 ...
 * list.  There are special cases for "secure" and
 * "kerberos" flavors.  Other flavors are values
 * of the new "sec=" option.
 */
int
check_client(sh, nb, clnames, flavor)
	struct share *sh;
	struct netbuf *nb;
	struct nd_hostservlist *clnames;
	int flavor;
...
int
check_client_old(sh, nb, clnames, flavor)
	struct share *sh;
	struct netbuf *nb;
	struct nd_hostservlist *clnames;
	int flavor;
 ...
 * single entry, e.g.
 *
 *   sec=krb4:krb5,rw=clnt1:clnt2,ro,sec=sys,ro
 *
 */

int
check_client_new(sh, nb, clnames, flavor)
	struct share *sh;
	struct netbuf *nb;
	struct nd_hostservlist *clnames;
	int flavor;
...
check_sharetab()
{
	...
	if (prev_sharetab_time != last_sharetab_time) {
		(void) rw_unlock(&sharetab_lock);
		return;
	}
	...
	while ((res = getshare(f, &sh)) > 0) {
		...
		shp = (struct sh_list *)malloc(sizeof (*shp));
		if (shp == NULL)
			goto alloc_failed;
		if (share_list == NULL)
			share_list = shp;

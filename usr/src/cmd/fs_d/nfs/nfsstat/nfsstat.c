...
/*
 * Copyright (c) 1984,1996-1997 by Sun Microsystems, Inc.
 * All rights reserved.
 */

#pragma ident	"@(#)nfsstat.c	1.29	97/04/21 SMI"	/* SVr4.0 1.9	*/
...
req_width(kstat_t *req)
{
	...
	for (i = 0; i < nreq; i++) {
		...
		if (tot)
			...
			per = 0;
		(void) sprintf(fixlen, "%lu %d%%", knp[i].value.ul, per);
		len = strlen(fixlen) + 1;
		if (field_width < len)
			field_width = len;
	}
...
mi_print(void)
{
	...
	*/
	...
	for (ksp = kc->kc_chain; ksp; ksp = ksp->ks_next) {
		...
		printf(",rsize=%d,wsize=%d", mik.mik_curread, mik.mik_curwrite);
		printf(",retrans=%d", mik.mik_retrans);
		printf("\n");

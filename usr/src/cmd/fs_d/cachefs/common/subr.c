...
#pragma ident "@(#)subr.c   1.22     98/02/03 SMI"
...
cachefs_dir_lock(const char *cachedirp, int shared)
{
	...
	int xx;
	char buf[MAXPATHLEN];
	char *lockdirp = "/tmp/.cachefs.";
	struct flock fl;
	char *strp;

	/* see if path name is too long */
	xx = strlen(cachedirp) + strlen(lockdirp) + 3;
	if (xx >= MAXPATHLEN) {
	...
	strcpy(buf, lockdirp);
	strcat(buf, cachedirp);
	strp = &buf[strlen(lockdirp)];
    ...
    struct cache_label *clabel)
{
	 ...
	 * This file will be <2G.
	 */
	sprintf(path, "%s/%s", dirp, CACHEFS_LOCK_FILE);
	...
	/* note: we do not count this directory in the resources */
	sprintf(path, "%s/%s", dirp, CACHEFS_LOSTFOUND_NAME);
	if (mkdir(path, 0700) == -1) {
		pr_err(gettext("mkdir %s failed: %s"), path,
		    strerror(errno));
		return (-2);
	...
	xx = 0;
	sprintf(path, "%s/%s", dirp, NOBACKUP_NAME);
...
cachefs_delete_all_cache(char *dirp)
{
	...
	sprintf(path, "%s/%s", dirp, LOG_STATUS_NAME);
	(void) unlink(path);

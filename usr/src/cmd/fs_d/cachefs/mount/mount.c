...
#pragma ident "@(#)mount.c   1.56     98/02/03 SMI"
...
int set_cfs_args(char *optionp, struct cachefs_mountargs *margsp, int *mflagp,
    char **backfstypepp, char **reducepp, int *notifyp);
...
#define	TEXT_DOMAIN	"SYS_TEST"
	...
	record_mount(mntp, specp, margs.cfs_backfs, backfstypep,
		margs.cfs_cachedir, margs.cfs_cacheid, optionp, reducep);
    ...
    char **backfstypepp, char **reducepp, int *notifyp)
{
	...
	*reducep = '\0';
	...
	if (margsp->cfs_options.opt_flags & (CFS_DISCONNECTABLE | CFS_SOFT))
		if (margsp->cfs_options.opt_flags & CFSOPT_NONSHARED) {
	...
	/* if snr, make sure they have the unbundled portion installed */
	if (margsp->cfs_options.opt_flags & CFS_DISCONNECTABLE) {
		if (stat64("/etc/fs/cachefs", &sinfo) != 0) {
			pr_err(gettext("disconnectable option"
			    " is not installed"));
			ret = -1;
		}
	}

	/* else if not snr, no need to notify the cachefsd */
	else {
		*notifyp = 0;
	}

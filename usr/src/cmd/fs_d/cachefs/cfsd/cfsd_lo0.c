...
#ident   "@(#)cfsd_logelem.c 1.7	 97/11/03 SMI"
/*
...
void
logelem_print_cred(cred_t *credp)
{
	char buf[100];
	char format[10];
	int xx;
...
logelem_print_attr(cfs_vattr_t *vp)
{
	...
	if (vp->va_mask & AT_VCODE)
		dbug_print(("dump", "  vcode %d", vp->va_vcode));
...
logelem_format_fid(cfsd_logelem_object_t *logelem_object_p, cfs_fid_t *fidp)
{
	...
	for (index = 0; index < (int)fidp->fid_len; index += sizeof (u_int)) {
		memcpy(&val, &fidp->fid_data[index], sizeof (u_int));
		sprintf(format, "%08x ", val);
		strcat(logelem_object_p->i_fidbuf, format);
	}
}
...
#define	MAXTRIES 10
	...
	if (gotdirfid) {
		...
		for (index = 0; index < MAXTRIES; index++) {
			/* construct the name */
			sprintf(namebuf2, "%s.conflict.%s.%x", machnamep,
			    namep, index);
	...
	if (wrotefile == 0) {
		...
		if (xx == EINVAL) {
			...
			sprintf(mesgbuf, gettext("Moved %s to %s/%s/%s."),
			    namep, CACHEFS_LOSTFOUND_NAME, newname);
			logelem_resolution(logelem_object_p, mesgbuf);
		}
	}
	...
	char *opp, char *info, const char *namep, int xx)
{
	 ...
	 */
	sprintf(mesgbuf, gettext("%s failed"), opp);
	if (namep) {
		strcat(mesgbuf, gettext(" on "));
		strcat(mesgbuf, namep);
	}
	strcat(mesgbuf, ".");
	if (info) {
		strcat(mesgbuf, " ");
	...
	if (xx) {
		sprintf(errorbuf, gettext(" Error: %s."), strerror(xx));
		strcat(mesgbuf, errorbuf);
	}
	...
	const char *namep)
{
	...
	sprintf(mesgbuf, gettext("Operation on %s skipped."), namep);
	logelem_resolution(logelem_object_p, mesgbuf);
}
/*
 * -----------------------------------------------------------------
	...
	char *opp, const char *namep, char *mesgp, int time_log)
{
	...
	strcat(mesgbuf, ".");
	if (mesgp) {
		strcat(mesgbuf, mesgp);
		strcat(mesgbuf, ".");
	}
	strcat(mesgbuf, " ");
	switch (time_log) {
	case 0:
		strcat(mesgbuf, gettext("while rolling log."));
		break;
	case 1:
		strcat(mesgbuf, gettext("while disconnected."));
		break;

	default:
		strcat(mesgbuf, gettext("while unknown operation."));
		break;
	}
...
int
logelem_roll(cfsd_logelem_object_t *logelem_object_p, u_long *seqp)
{
	int retval = 0;
...
int
logelem_roll_setattr(cfsd_logelem_object_t *logelem_object_p, u_long *seqp)
{
...
int
logelem_roll_setsecattr(cfsd_logelem_object_t *logelem_object_p, u_long *seqp)
{
	int xx;
	cfs_fid_t filefid, *fp;
	struct cfs_dlog_mapping_space map;
...
int
logelem_roll_create(cfsd_logelem_object_t *logelem_object_p, u_long *seqp)
{
	int xx;
	cfs_fid_t *fp;
	cfs_fid_t dirfid;
...
int
logelem_roll_remove(cfsd_logelem_object_t *logelem_object_p, u_long *seqp)
{
...
int
logelem_roll_rmdir(cfsd_logelem_object_t *logelem_object_p, u_long *seqp)
{
	int xx;
	cfs_fid_t *fp;
	cfs_fid_t dirfid;
...
int
logelem_roll_mkdir(cfsd_logelem_object_t *logelem_object_p, u_long *seqp)
{
	int xx;
	cfs_fid_t *fp;
	cfs_fid_t dirfid;
...
int
logelem_roll_link(cfsd_logelem_object_t *logelem_object_p, u_long *seqp)
{
	int xx;
	cfs_fid_t *fp;
...
int
logelem_roll_symlink(cfsd_logelem_object_t *logelem_object_p, u_long *seqp)
{
	int xx;
	cfs_fid_t *fp;
	cfs_fid_t dirfid;
...
int
logelem_roll_rename(cfsd_logelem_object_t *logelem_object_p, u_long *seqp)
{
	int xx;
	cfs_fid_t *fp;
...
int
logelem_roll_modified(cfsd_logelem_object_t *logelem_object_p, u_long *seqp)
{
	int xx;
	cfs_fid_t filefid, *fp;
	struct cfs_dlog_mapping_space map;
...
logelem_dump_rename(cfsd_logelem_object_t *logelem_object_p)
{
	...
	dbug_print(("dump", "deleted cid %08x",

...
#pragma ident	"@(#)roll_log.c	1.11	97/07/25 SMI"
...
rl_roll_log(char *bdev)
{
	...
	if (is_mounted(&li, bdev) == RL_TRUE) {
		...
		/*
		 * Device is not mounted.  We need to mount the device read
...
make_mp(log_info_t *lip)
{
	...
	/*
	 * Create directory base name.  Start is with a ".", so that it
	 * will be less noticable.
	 */
	(void) sprintf(template, ".rlg.%ld.XXXXXX", getpid());
	(void) strcpy(tmp_name, template);
	(void) mktemp(tmp_name);

	/* Attempt to make the directory. */
	for (i = 0; i < list_len; i++) {
		(void) sprintf(tmp_dir, "%s/%s", tmp_dir_list[i], tmp_name);
	...
	if (rv != RL_SUCCESS) {
		(void) fprintf(stderr, "Unable to create temporary "
			"directory in any of the directories listed "
			"below.\n");
		for (i = 0; i < list_len; i++) {
			(void) fprintf(stderr, "\t%s\n", tmp_dir_list[i]);
		}
		(void) fprintf(stderr, "Please correct this problem and "
			"rerun the program.\n");
	}

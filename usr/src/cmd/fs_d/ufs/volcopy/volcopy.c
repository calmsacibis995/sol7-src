...
#ident	"@(#)volcopy.c	1.18	97/08/12 SMI"	/* SVr4.0 1.8	*/
...
va_dcl
{
	...
	if ((fmt_p = va_arg(v_Args, char *)) != (char *)NULL)
		(void) vfprintf(stdout, gettext(fmt_p), v_Args);
	if (verify) {
...
fslog()
{
	...
	sprintf(cmd,
"echo \"%s;%.6s;%.6s -> %s;%.6s;%.6s on %.24s\" >>/var/adm/log/filesave.log",
...
va_dcl
{
	...
	(void) vfprintf(stderr, gettext(fmt_p), v_Args);
	(void) fflush(stderr);
	va_end(v_Args);
	if (severity > 0) {
		(void) cleanup();

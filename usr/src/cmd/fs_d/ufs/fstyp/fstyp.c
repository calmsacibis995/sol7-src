...
#ident	"@(#)fstyp.c	1.27	97/11/26 SMI"	/* SVr4.0 1.5 */
...
dumplog(const char *name)
{
	...
	(void) llseek(0, (offset_t)afs.fs_logbno * DEV_BSIZE, 0);
	...
	printf("\n");
	ep = &ebp->extents[0];
	(void) llseek(0, (offset_t)ep->pbno * DEV_BSIZE, 0);
	...
	printf("statebno\t%" PRId32 "\t\tlogbno\t\t%" PRId32 "\n",
		ud->od_statebno, ud->od_logbno);
	printf("maxtransfer\t%" PRIu32 "\t\tdevbsize\t%" PRIu32 "\n",
		ud->od_maxtransfer, ud->od_devbsize);
	printf("\t\t\t\tlogalloc\t%" PRIu32 "\n", ud->od_logalloc);

...
#ident	"@(#)mkfs.c	1.54	97/08/04 SMI"	/* SVr4.0 1.9 */
...
#define	TEXT_DOMAIN "SYS_TEST"
	...
	fsi = open64(fsys, 0);
	if (fsi < 0) {
		fprintf(stderr, gettext("%s: cannot open\n"), fsys);
		lockexit(32);
	}
	if (mflag) {
		dump_fscmd(fsys, fsi);
	...
	if (!Nflag) {
		...
		if (directory) {
			if (ismounted == 0) {
				fprintf(stderr, gettext("%s is not mounted\n"),
					special);
				lockexit(32);
			}
			wlockfs();
		...
		if (fso < 0) {
			fprintf(stderr, gettext("%s: cannot create\n"), fsys);
			lockexit(32);
		}
	 ...
	 * verify device size
	 *
	 * For the -N case, verify by reading the last sector.
	 * Otherwise, verify by writing the last sector.
	 *
	...
	if (sblock.fs_ntrak <= 0 || sblock.fs_ntrak > fssize) {
		fprintf(stderr, gettext("preposterous ntrack %d\n"),
			sblock.fs_ntrak);
		lockexit(32);
	}

	if (sblock.fs_nsect <= 0 || sblock.fs_nsect > fssize) {
		fprintf(stderr, gettext("preposterous nsect %d\n"),
			sblock.fs_nsect);
		lockexit(32);
	}
...
grow00:
	...
	if (sblock.fs_nrpos <= 0) {
		fprintf(stderr, gettext("preposterous nrpos %d\n"),
			sblock.fs_nrpos);
		lockexit(32);
	}
	...
	if (sblock.fs_bsize < MINBSIZE) {
		fprintf(stderr,
			gettext("block size %d is too small, minimum is %d\n"),
			sblock.fs_bsize, MINBSIZE);
		lockexit(32);
	}
	if (sblock.fs_bsize > MAXBSIZE) {
		fprintf(stderr,
			gettext("block size %d is too big, maximum is %d\n"),
	...
	if (rps <= 0 || rps > 1000) {
		fprintf(stderr,
			gettext("preposterous rotations per second %d\n"), rps);
		lockexit(32);
	}
grow10:
	if (argc > 9)
	...
	while (CGSIZE(&sblock) > sblock.fs_bsize) {
		...
		if (sblock.fs_fsize == sblock.fs_bsize) {
			fprintf(stderr, gettext("There is no block size that \
...
sblock.fs_spc, mincpg);
	...
	if (inodecramped || mapcramped) {
		if (sblock.fs_bsize != bsize) {
			fprintf(stderr,
			    gettext("%s to be changed from %d to %d\n"),
...
alloc(int size, int mode)
{
	...
	for (d = 0; d < acg.cg_ndblk; d += sblock.fs_frag)
		if (isblock(&sblock, cg_blksfree(&acg), d / sblock.fs_frag))
			goto goth;
	fprintf(stderr, gettext("internal error: can't find block in cyl 0\n"));
	lockexit(32);
goth:
...
rdfs(daddr_t bno, int size, char *bf)
{
	...
	if (llseek(fsi, (offset_t)bno * sectorsize, 0) < 0) {
		fprintf(stderr, gettext("seek error: %ld\n"), bno);
		perror("rdfs");
		lockexit(32);
	}
	n = read(fsi, bf, size);
	if (n != size) {
		fprintf(stderr, gettext("read error: %ld\n"), bno);
		perror("rdfs");
		lockexit(32);
	}
}
...
wtfs(daddr_t bno, int size, char *bf)
{
	...
	n = write(fso, bf, size);
	if (n != size) {
		fprintf(stderr, gettext("write error: %ld\n"), bno);
		perror("wtfs");
		lockexit(32);
	}
}
...
checksblock()
{
	...
	if (sblock.fs_ncg < 1) {
		fprintf(stderr, gettext("Bad superblock; ncg out of range\n"));
		lockexit(32);
	}
	if (sblock.fs_cpg < 1) {
		fprintf(stderr, gettext("Bad superblock; cpg out of range\n"));
		lockexit(32);
	}
	if (sblock.fs_ncg * sblock.fs_cpg < sblock.fs_ncyl ||
	    (sblock.fs_ncg - 1) * sblock.fs_cpg >= sblock.fs_ncyl) {
		fprintf(stderr, gettext("Bad superblock; ncyl out of range\n"));
		lockexit(32);
	}
...
logsetup(char *devstr)
{
	...
	if (sblock.fs_logbno) {
		...
		rdfs((daddr_t)sblock.fs_logbno, DEV_BSIZE, buf);
		...
		rdfs((daddr_t)ebp->extents[0].pbno, DEV_BSIZE, ud_buf);
	...
	char	*bdev;
{
	...
	if ((statarea.st_mode & S_IFMT) != S_IFBLK) {
		fprintf(stderr, gettext("can't check mount point; "));
		fprintf(stderr, gettext("%s is not a block device\n"), bdev);
		lockexit(32);
	}
...
again:
	...
	if (notenoughspace()) {
		...
		fprintf(stderr, gettext("Not enough free space\n"));
		lockexit(32);
	}

...
#pragma	ident	"@(#)autod_parse.c 1.18     97/04/29 SMI"
...
static struct mapent *do_mapent_hosts(char *, char *, u_int);
 ...
 *			char *subdir, u_int isdirect, bool_t mount_access)
 ...
 * cases of xfn and /net map parsing.
			...
			char *subdir, u_int isdirect, bool_t mount_access)
{
	...
	if (strncmp(mapname, FNPREFIX, FNPREFIXLEN) == 0) {
		/*
		 * called from the xfn map parser - there's only
		 * one mountpoint, so there's no hierarchies involved.
		 */
		if (mapline_to_mapent(&mapents, ml, key, mapname,
				mapopts, defaultopts, isdirect) != PARSE_OK)
			goto parse_error;

		if (set_mapent_opts(mapents, NULL,
			(mapents->map_mntopts ? mapents->map_mntopts :
	...
	/*
	 * Modify the mapentry list for non-xfn mapentries.
	 * we *must* do this only after the mapentry list is completely
	 * built (since we need to have parse_fsinfo called first).
	 */
 ...
 * 			char *key, u_int isdirect, bool_t mount_access)
 ...
 *			u_int isdirect, char *mapopts, bool_t mount_access)
...
get_dir_from_path(char *dir, char **path, int dirsz)
{
	...
	if (dirsz <= 0) {
		if (verbose)
			syslog(LOG_ERR,
			"get_dir_from_path: invalid directory size %s");
		return (PARSE_ERROR);
	}

	/* get rid of leading /'s and spaces in path */
	while (**path == '/')
		(*path)++;
	...
	*dir = '\0';
	...
	/* get rid of trailing /'s and spaces in path */
	while (**path == '/')
		(*path)++;

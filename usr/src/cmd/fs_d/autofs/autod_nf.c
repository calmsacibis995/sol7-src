...
#pragma ident	"@(#)autod_nfs.c	1.71	98/01/29 SMI"
	...
	int overlay;
{
	...
	/*
	 * Try loopback if we have something on localhost; if it doesn't
	 * work, we will fall back to NFS and ignore the first entry.
	 */
...
retry:
		...
		if (trace > 1) {
		    trace_prt(1,
		    "  nfsmount: Get mount version: request vers=%d min=%d\n",
		    vers, versmin);
		}
		while ((cl = clnt_create_vers(host, MOUNTPROG, &outvers,
			 ...
			 * is handled in clnt_create_vers(). Its not done to
	...
	rpcvers_t versmin;
{
	...
	while ((cl = clnt_create_vers_timed(hostname, NFS_PROGRAM, &outvers,
	    versmin, vers_to_try, "datagram_v", &rpc_to_new)) == NULL) {
		if (trace > 4) {
		    trace_prt(1, "  pingnfs: Can't ping %s: RPC error=%d\n",
		...
		if (versp != NULL) {	/* recheck the cache */
			*versp = vers_to_try;
			if (trace > 4) {
			    trace_prt(1, "  pingnfs: check cache: vers=%d\n",
				*versp);
			}
			switch (cache_check(hostname, versp)) {
			case GOODHOST:
 ...
 * to NFS Version 2. Return 0 on success and -1 on error.
 */
int

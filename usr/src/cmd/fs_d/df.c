...
#pragma	ident	"@(#)df.c 1.41	97/03/21 SMI"	/* SVr4.0 1.34  */
...
extern int fsrdlock_mnttab();
extern void fsunlock_mnttab(int);
...
#ifdef  XPG4
	errmsg(ERR_NONAME,
	"Usage: %s [-F FSType] [-abegklntPV] [-o FSType-specific_options]"
		" [directory | block_device | resource]", program_name);
#else
	errmsg(ERR_NONAME,
	"Usage: %s [-F FSType] [-abegklntVv] [-o FSType-specific_options]"
		" [directory | block_device | resource]", program_name);
#endif
	exit(1);
	/* NOTREACHED */
 ...
 * Returns 0 on success, 1 if something goes wrong.
 */
static void
init_remote_fs()
{
...
/*
 * Create a new "struct mnttab" and make sure that its fields point
 * to malloc'ed memory
 */
static struct mnttab *
mntdup(struct mnttab *old)
{
	struct mnttab *new = NEW(struct mnttab);
...
mtab_error(char *mtab_file, int status)
{
	if (status == MNT_TOOLONG)
		...
		errmsg(ERR_NOFLAGS,
			"a line in %s has to many fields", mtab_file);
...
mtab_read_file()
{
	...
	while ((status = getmntent(fp, &mtab)) == 0) {
		...
		mtep->mte_ignore = (hasmntopt(&mtab, MNTOPT_IGNORE) != NULL);
	}
...
#ifdef XPG4
	while ((arg = getopt(argc, argv, "F:o:abekVtgnlP")) != EOF) {
#else
	while ((arg = getopt(argc, argv, "F:o:abekVtgnlv")) != EOF) {
#endif
		if (arg == 'F') {
			if (F_option)
...
path_mount_entry(struct df_request *dfrp, dev_t devno)
{
	...
	for (;;) {
		...
		for (i = mount_table_entries - 1; i >= 0; i--) {
			struct mtab_entry	*mtep = &mount_table[i];
			struct mnttab		*mtp = mtep->mte_mount;

			if (EQ(mtp->mnt_fstype, MNTTYPE_SWAP))
				continue;

			len = strlen(mtp->mnt_mountp);

			/*
			 * Skip mount points that are not prefixes of the path
			 * We have a matching prefix if the mount point is
			 * a prefix of the path, and the path character after
			 * the matching part is a '/' or a NUL, or the last
			 * character of the mount point is a '/' (which
			 * happens only for the root file system).
			 */
			if (! (strncmp(dirpath, mtp->mnt_mountp, len) == 0 &&
				(dirpath[len] == NUL || dirpath[len] == '/' ||
					mtp->mnt_mountp[len-1] == '/')))
				continue;

			/*
			 * We got a prefix; check if device numbers match.
			 * If there is a cached device number in the
			 * mtab_entry, use it, otherwise get the device number
			 * either from the mnttab entry or by stat'ing the
			 * mount point.
			 */
			if (! mtep->mte_dev_is_valid) {
				...
				struct stat64 st;
				dev_t dev = NODEV;
				char *str = hasmntopt(mtp, MNTOPT_DEV);

				if (str != NULL && (p = strchr(str, '='))) {
					errno = 0;
					dev = strtoul(p+1, &q, 16);
					if (p == q || dev == 0 || errno)
						dev = NODEV;
				}
				if (dev == NODEV)
		...
		/*
		 * If this is not a loopback mount, we are done.
		 */
		 ...
		 */
		if (EQ(match->mte_mount->mnt_mountp,
				match->mte_mount->mnt_special))
			errmsg(ERR_FATAL,
	"special device and mount point are the same for loopback file system");

		/*
		 * Replace the mount point with the directory it is mounted
		 * on and repeat
		 */
		(void) strcpy(path_buf, match->mte_mount->mnt_special);
		(void) strcat(path_buf, &dirpath[len]);
		(void) strcpy(dirpath, path_buf);

		if (realpath(dirpath, dirpath) == NULL) {
			errmsg(ERR_PERROR, "cannot canonicalize %s:", dir);
			return;
		}
	}
...
vfs_error(char *file, int status)
{
	if (status == VFS_TOOLONG)
		...
		errmsg(ERR_NOFLAGS, "a line in %s has to many fields", file);
			...
			size_t *request_count)
{
	...
	if (argc == 0) {
		for (i = 0; i < mount_table_entries; i++) {
			struct mnttab *mtp = mount_table[i].mte_mount;

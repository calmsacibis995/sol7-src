...
#ifdef SVR4
#pragma ident  "@(#)setup.c	2.15 96/07/23 Sun Microsystems"
#else
static char sccsid[] = "@(#)setup.c	2.15 96/07/23 Sun Microsystems";
#endif
#endif
...
static struct kernel_symbol bootsym = { "_boottime", 0, 0};
...
#else	/* (SVR4) */
if (find_loc(&bootsym) == 0)
   then {
	PERROR("Error reading boot time:");
	return -1;
	}
   else {
	read_bytes((off_t)bootsym.offset, (char *)&boot_at,
		   sizeof(boot_at));
	boot_at.tv_usec = 0;	/* this field is ticks from the kernel */
				/* so ignore it */
	}
#endif	/* (SVR4) */
...
if ((host = gethostbyname(snmp_sysName)) == (struct hostent *)0)
 ...
 {
 u_long tlong;
 tlong = ntohl(*(u_long *)(host->h_addr));

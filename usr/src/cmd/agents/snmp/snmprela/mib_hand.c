 ...
 */
#pragma ident  "@(#)mib_handler.c	1.17 96/10/30 Sun Microsystems"
...
#define SAP_TBL_ENTRY_INVALID 2
	...
	switch(offset){
	  case OFFSET_AGENT_PORT_NUMBER: 
		...
		entry->agentPortNumber = entry->address.sin_port = num; 
  		entry->address.sin_addr.s_addr = my_ip_address.s_addr;
...
int chk_Agent(char* t)
{
  Agent *entry=(Agent *)t;
/*
  if(entry == NULL || entry->agentStatus == SAP_TBL_ENTRY_INVALID)
*/
...
/*
  if(entry->name.subids == NULL || entry->name.len == 0) 
	return SAP_TBL_FAIL;
*/
		  	...
		  	Integer *num, String *str, Oid *name, int offset)
{
	...
	if( first_pass_entry==NULL && (entry = tbl_search(this,AgentID))==NULL){
	  ...
	  /* made a duplication */
	  if(pass==FIRST_PASS){
		/* first_pass_entry != NULL => no clone
 		 * first_pass_entry == NULL and entry exists(must)
		 * => clone */
...
int set_agentProcessID(int pass, IndexType index, Integer *AgentProcessID)
{
	...
	return(set_table_entry_field(tbl,AgentID,pass,AgentProcessID,NULL,NULL,offset));
}

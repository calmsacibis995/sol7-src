 ...
 */
#pragma ident  "@(#)session.c	1.13 97/04/24 Sun Microsystems"
...
static int stalling = FALSE; /* (5-29-96) */
...
static void trace_session(Session *session)
{
	...
	if(trace_level > 2)
	{
		...
		trace("\ti_flags:        0x%x\n", session->i_flags);
		if(stalling == TRUE)
		  trace("Request Processing Pipeline is stalling\n");
	}
...
void session_dispatch()
{
	...
	if(session)
	{
 	...
 	if(anyOutstandingSetRequestRunning()){
		if(stalling==FALSE && pdu) stalling = TRUE;
		return; /* no process */
        }else if(first_session==NULL){
		if(stalling == TRUE) stalling = FALSE;
		/* process request */
	}else{ /* get or getnext */
		if(pdu->type != SET_REQ_MSG){
		  if(stalling == TRUE) stalling = FALSE;
		  /* process request */
		}else{
		    if(stalling == FALSE && pdu) stalling = TRUE;
...
static int session_timeout_loop(struct timeval *now)
{
	...
	/* For each session outstanding, check to see if the tiemout has expired. */
	...
	for(sp = first_session; sp; sp = sp->next_session)
	{
		for(rp = sp->first_request; rp; rp = rp->next_request)
		{
			...
			if(timercmp(&rp->expire, now, <))
			{
				...
				error(ERR_MSG_AGENT_NOT_RESPONDING,
					subtree->agent->name);

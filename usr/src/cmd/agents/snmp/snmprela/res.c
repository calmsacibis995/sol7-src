 ...
 */
#pragma ident  "@(#)res.c	1.11 97/07/16 Sun Microsystems"
...
static void resource_detach(SapResource *tgt)
{
...
void resource_handling(SapResource *rp)
{
   ...
   PidRec *pp;
	...
	if(first_agent != NULL && first_agent != prev_agent){
	  ...
	  /* init the res_name as the agentName */
		...
		buf[0]='\0';
		memcpy(buf,first_agent->agentName.chars,first_agent->agentName.len);
	  ...
	  if(rp->policy && !strcmp(rp->policy,POLICY_SPAWN)){
		/* check the global flag recovery_on,to decide
		   spawn or not, recovery flag should set then 
		   reset */
		if(recovery_on==FALSE ||
		  (rp->agent!=NULL && res_agent_is_alive(rp->res_name)==FALSE))
		{
			...
			if(rp->agent->address.sin_port == 0 ||
			   rp->agent->agentPortNumber ==0){
				...
				if(port_num==0)
				  error("can't find a valid port for the agent %s",(first_agent->name? first_agent->name:""));
			}
...
static int ssa_subagent_is_alive(Agent *agent)
{
  struct timeval timeout; 
   ...
   */ 
   timeout.tv_sec = 5;
   timeout.tv_usec = 0; 
   return(SSAAgentIsAlive(NULL,agent->agentPortNumber,NULL,&timeout));
}
...
void write_pid_file(char *filename)
{
  ...
  for(sp=first_res;sp;sp=sp->next_res){
	if(sp->policy==NULL || strcmp(sp->policy,POLICY_SPAWN)) continue;
 	if(sp->agent==NULL) continue;
   	if(sp->agent->agentName.chars!=NULL){
		memcpy(name,sp->agent->agentName.chars,sp->agent->agentName.len);
		name[sp->agent->agentName.len]='\0';
	}else{
		strcpy(name,"UNKNOWN");
	}
	fprintf(fp,"%s %s %d %d\n",
		(sp->res_name!=NULL?sp->res_name:"UNKNOWN"),
		name,
		sp->agent->agentProcessID,
		sp->agent->agentPortNumber);
  } 
  fclose(fp);
}

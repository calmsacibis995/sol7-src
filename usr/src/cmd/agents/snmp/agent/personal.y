 ...
 * 7-3-96	Jerry Yeung		add watchdog
 * 7-15-96	Jerry Yeung		change the API for ssa_subagent_open
 * 7-16-96	Jerry Yeung		change API function name
 * 10-18-96	Jerry Yeung		chk reg. file
 */
 ...
 */
#pragma ident  "@(#)personal.y	1.10 97/07/16 Sun Microsystems"
...
static SubGroup *static_sub_group=NULL;
static TrapSlot *static_trap_slot=NULL;
...
static int tables_to_subtrees(char *error_label);
static int is_first_entry(Table *table);
static void table_free(Table *tp);
static void table_list_delete();
static void trace_tables();
...
agent_item :		t_openbracket
		{
			...
			static_agent->agent_id = (int)getpid();
			static_agent->agent_status = SSA_OPER_STATUS_NOT_IN_SERVICE;
/*
			static_agent->address.sin_addr.s_addr = my_ip_address.s_addr;
*/
		}
		name subtrees_tables timeout optional_watch_dog_time optional_port t_closebracket
		{
			...
			if(static_agent->address.sin_port==0)
 ...
 */ 
			...
			if( (static_agent->agent_id =
			     SSASubagentOpen(max_agent_reg_retry,static_agent->name)) == 0 )
			{
                              if (count == 5) {
			...
			static_agent->process_id = (int)getpid();
		}
...
subtree_item : subids_list
		{
			...
			if(subtree_add(static_agent->agent_id, static_agent, static_subids, static_len) == -1)
			{
				error("error at line %d", yylineno);
				YYERROR;
			}
...
static void trace_tables()
{
	...
	for(tp = first_table; tp; tp = tp->next_table)
	{
		trace("\t%-30s %-30s",
			tp->agent->name,
			SSAOidString(&(tp->regTblOID)));
		trace(" %[%3d-%3d] [%3d-%3d]\n",
			tp->regTblStartColumn,
			tp->regTblEndColumn,
/*
...
static int tables_to_subtrees(char *error_label)
{
	...
	for(tp = first_table; tp; tp = tp->next_table)
	{
		if(is_first_entry(tp))
		{
			for(column = tp->regTblStartColumn; column <= tp->regTblEndColumn; column++)
			{
				if(subids_cat(tp->regTblOID.subids, tp->regTblOID.len) == -1)
				{
					return -1;
				}
				...
				if(subtree_add(tp->agent->agent_id,tp->agent, static_subids, static_len) == -1)
				{
...
/*
		for(column = tp->regTblStartColumn; column <= tp->regTblEndColumn; column++)
		{
			if(subids_cat(tp->regTblOID.subids, tp->regTblOID.len))
			{
				return -1;
			}
			if(subids_cat(&one, 1) == -1)
			...
			if(subids_cat(tp->indexs.subids, tp->indexs.len) == -1)
			{
				return -1;
			}
		
			if(subtree_add(tp->agent->agent_id, static_subids, static_len) == -1)
			{
				sprintf(error_label, "subtree_add() failed for table %s for the agent %s",
					SSAOidString(&(tp->regTblOID)),
					tp->agent->name);
...
*/
		...
		for(index = tp->regTblStartRow; index <= tp->regTblEndRow; index++)
		{
			for(column = tp->regTblStartColumn; column <= tp->regTblEndColumn; column++)
			{
				if(subids_cat(tp->regTblOID.subids, tp->regTblOID.len))
...
static int is_first_entry(Table *table)
{
	...
	for(tp = first_table; tp; tp = tp->next_table)
	{
		...
		if(SSAOidCmp(&(tp->regTblOID), &(table->regTblOID)) == 0)
		{
/*
...
*/
			if(tp->regTblStartRow < table->regTblStartRow)
			{
				return False;
			}
		}

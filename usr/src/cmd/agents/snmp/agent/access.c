 ...
 */
#pragma ident  "@(#)access.c	1.4 96/07/22 Sun Microsystems"

/* HISTORY
 * 6-24-95	Jerry Yeung	Handle wild card(wild-card)
 */
...
Manager* manager_add(char *name, char *error_label)
{
	...
	if(name == NULL)
	{
		sprintf(error_label, "BUG: manager_add(): name is NULL");
		return NULL;
	}
	...
	if(strcmp(name,WILD_CARD_HOST_NAME)){
		for(m = first_manager; m; m = m->next_manager)
		{
			...
			{
				sprintf(error_label, ERR_MSG_MANAGER_DUP, name);
				return m;
			}
		}
	}else{
...
int is_valid_manager(Address *address,Manager **mngr)
{
	Manager *m;
...
int community_add(char *name, int type, char *error_label)
{
	...
	if(name == NULL)
	{
		sprintf(error_label, "BUG: community_add(): name is NULL");
		return -1;
	}

	if(name[0] == '\0')
	{
		sprintf(error_label, "BUG: community_add(): name is empty");
		return -1;
	}

	if( (type != READ_ONLY) && (type != READ_WRITE) )
	{
		sprintf(error_label, "BUG: community_add(): bad type (%d)", type);
		return -1;
	}
	...
	for(c = first_community; c; c = c->next_community)
	{
		...
		if(ret == 0)
		{
			sprintf(error_label, ERR_MSG_COMMUNITY_DUP, name);
			return 1;
		}
...
EFilter* efilter_add(char *name, char *error_label)
{
	...
	if(name == NULL)
	{
		sprintf(error_label, "BUG: efilter_add(): name is NULL");
		return NULL;
	}
...
TrapSlot* trap_slot_add(int num,EFilter *efilter,char *error_label)
{
	...
	if(num < 0)
	{
		sprintf(error_label, "BUG: trap_slot_add(): name is NULL");
		return NULL;
	}
...
static NameOidPair* set_name_and_oid_pair(char *inbuf)
{
	...
	np->oid = oid;
	np->name = malloc(strlen(name_str)+1);
	strcpy(np->name,name_str);
...
void load_enterprise_oid(char* filename)
{
   ...
   */
   ...
   insert_name_oid_pair("sun","1.3.6.1.4.1.42");

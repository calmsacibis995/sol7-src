/* Copyright 02/14/97 Sun Microsystems, Inc. All Rights Reserved.
 */
#pragma ident  "@(#)backend.c	1.22 97/02/14 Sun Microsystems"
...
static Subid snmp_subids[] = { 1,3,6,1,2,1,11,0}; /* 0 is null(hack) */
...
/*
			if(current->n_indexs > 3)
			{
				error("WARNING: The agent will not support so many INDEXs (%d) for the entry %s",
						current->n_indexs,
						current->label);
			}
*/
...
static void output_extern_trap_function(FILE *fp)
{
  ...
  for(ip=trap_list;ip;ip=ip->next){
	/* int trap_handler_TRAP_NAME()
		if(condition) then
		trap_send_with_more_para(tgrAdxPtr,myAdx,1,enterprise,
...
static void output_index_table(FILE *fp, struct tree *current, int *index_index)
{
	...
	for(indexs = current->indexs; indexs; indexs = indexs->next)
	{
		if(indexs->tp)
		{
			if(indexs->next == NULL)
			{
				fprintf(fp, "/* %6d */ { %17s, \"%s\", &node_table[%d] },\n",
					*index_index,
					"NULL",
					indexs->label,
			...
			{
				fprintf(fp, "/* %6d */ { &index_table[%4d], \"%s\", &node_table[%d] },\n",
					*index_index,
					(*index_index) + 1,
					indexs->label,
					indexs->tp->node_index);
		...
		else
		{
			...
			{
				fprintf(fp, "/* %6d */ { &index_table[%4d], \"%s\", NULL },\n",
					*index_index,
					(*index_index) + 1,
					indexs->label);
			}
...
static void output_tree_c(struct tree *current)
{
	...
	char backup_pathname[MAX_PATHNAME_LEN];
	struct stat buf;
	FILE *fp;
	int subid_index;
	int enum_index;
...
static void output_trap_structure(FILE *fp)
{
  ...
  /* output basic struct */
  /*
  fprintf(fp,"struct CallbackItem {\n");
  fprintf(fp,"\tObject *ptr;\n");
  fprintf(fp,"\tint type,next;\n");
  fprintf(fp,"};\n");

  fprintf(fp,"struct TrapHndlCxt {\n");
  fprintf(fp,"\tchar name[256];\n");
  fprintf(fp,"\tint is_sun_enterprise;\n");
  fprintf(fp,"\tint genric,specific;\n");
  fprintf(fp,"};\n");
 */
  ...
  for(ip=trap_list;ip;ip=ip->next){
    ...
    for(var=ip->var_list;var;var=var->next){
	  ...
	  if((var->tp=find_node(root,var->label)) == NULL)
	    error_exit("output_trap_structure():Unknown variable:%s",var->label);
	  tp = var->tp;	
	  if(tp->node_type != OBJECT) 
		error_exit("variable: %s is not individual object",var->label);
	  fprintf(fp,"{&object_table[%d],",tp->object_index);
	  switch(tp->type){ /* only accept object node type */
		...
		case TYPE_OBJID:
			...
			break;
		default:
		  error_exit("unknown object type of variable %s",var->label);
	 }
	 numCallItem++;
	 if(var->next)
		fprintf(fp,"%d},\n",numCallItem);
  ...
  for(ip=trap_list;ip;ip=ip->next){
	...
	if(!strcmp(ip->enterprise_label,"snmp")){
	  ...
	  get_subids_by_name(ip->enterprise_subids,ip->enterprise_label);
	}
  ...
  fprintf(fp,"\treturn(_SSASendTrap(name));\n");
  fprintf(fp,"}\n"); 
  }
}

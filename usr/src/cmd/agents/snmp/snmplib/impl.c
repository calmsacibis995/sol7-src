 ...
 */
#pragma ident  "@(#)impl.c	1.5 96/07/22 Sun Microsystems"
...
void SSAStringZero(String *string)
{
	if(string == NULL)
	{
		fprintf(stderr, "BUG: SSAStringZero(): string is NULL");
		return;
	}
...
int SSAOidCpy(Oid *oid1, Oid *oid2, char *error_label)
{
	...
	if(oid1 == NULL)
	{
		sprintf(error_label, "BUG: SSAOidCpy(): oid1 is NULL");
		return -1;
	}

	if(oid2 == NULL)
	{
		sprintf(error_label, "BUG: SSAOidCpy(): oid2 is NULL");
		return -1;
	}

	if(oid1->subids)
	{
		sprintf(error_label, "BUG: SSAOidCpy(): oid1->subids is not NULL");
		return -1;
	}

	if(oid1->len)
	{
		sprintf(error_label, "BUG: SSAOidCpy(): oid1->len is not 0");
		return -1;
	}
	...
	oid1->subids = (Subid *) malloc(oid2->len * sizeof(Subid));
	if(oid1->subids == NULL)
	{
		sprintf(error_label, ERR_MSG_ALLOC);
		return -1;
	...
	memcpy(oid1->subids, oid2->subids, oid2->len * sizeof(Subid));
	oid1->len = oid2->len;
...
void SSAOidZero(Oid *oid)
{
	if(oid == NULL)
	{
		fprintf(stderr, "BUG: SSAOidZero(): oid is NULL");
		return;
	}
...
int SSAOidInit(Oid *oid, Subid *subids, int len, char *error_label)
{
	...
	if(len != 0)
	{
		oid->subids = (Subid *) malloc(len * sizeof(Subid));
		if(oid->subids == NULL)
		{
			sprintf(error_label, ERR_MSG_ALLOC);
			return -1;
		}
		memcpy(oid->subids, subids, len * sizeof(Subid));
		oid->len = len;
	}
...
Oid *SSAOidStrToOid (char *name, char *error_label)
{
      ...
      if (name[i] == '.')
          len++;
                  ...
                  fprintf(stderr, "%s is not a valid oid name\n",  name);
                  return(NULL);
          }
        }
        ...
        len++;
        subids = (Subid *) malloc(len * sizeof(Subid));
        if (subids == NULL) {
        ...
        name_oid = SSAOidNew();
        SSAOidInit(name_oid, subids, len, error_label);
        free(subids);
        return(name_oid);
}

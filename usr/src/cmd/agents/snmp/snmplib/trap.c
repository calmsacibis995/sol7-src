 ...
 */
#pragma ident  "@(#)trap.c	1.5 96/07/22 Sun Microsystems"
...
Subid sun_subids[] = { 1, 3, 6, 1, 4, 1, 42 };
Oid sun_oid = { sun_subids, 7 };
...
int trap_send(IPAddress *ip_address, Oid *enterprise, int generic, int specific, SNMP_variable *variables, char *error_label)
{
	...
	/* time_stamp */
	pdu->time_stamp = request_sysUpTime(error_label);
	...
	SNMP_variable *variables,char *error_label)
{
	...
	/* time_stamp */
	if (time_stamp == -1)
		pdu->time_stamp = request_sysUpTime(error_label); /* default */
	else
...
int trap_send_with_more_para(IPAddress *ip_address,
							 ...
							 char *error_label)
{
 return(trap_send_raw(ip_address,my_ip_addr, NULL,i_flag,enterprise,generic,
...
int trap_send_to_all_destinators7( int i_flag, Oid *enterprise, int generic,
                                   int specific, u_long time_stamp,
                                   SNMP_variable *variables, char *error_label)
{
        Trap_Destinator *d;
        IPAddress my_ip_addr;
        ...
        for(d = first_trap_destinator; d; d = d->next_trap_destinator)
        {
                if(trap_send_with_more_para(&(d->ip_address), my_ip_addr,0,enterprise, generic, specific, SNMP_TRAP_PORT,time_stamp,variables, error_label))
                {
                        return -1;
                }
        }

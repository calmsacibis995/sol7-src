 ...
 */
#pragma ident  "@(#)madman_api.c	1.4 96/07/22 Sun Microsystems"
...
static int translate_variable(SNMP_variable *variable, int translator, long *pointer, char *error_label);
static int extract_one_index_from_column(Oid *instance, Oid *object, long *index);
static int extract_two_indexes_from_column(Oid *instance, Oid *object, long *index1, long *index2);
static int extract_three_indexes_from_column(Oid *instance, Oid *object, long *index1, long *index2, long *index3);
...
int applEntry_send_request(SNMP_session *session, u_char type, long applIndex, char *error_label)
{
	SNMP_pdu *request;
	int i;
	SNMP_column *column;
...
ApplEntry *applEntry_process_response(SNMP_session *session, SNMP_pdu *response, char *error_label)
{
	...
	pointer = (long *) &(applEntry->applName);
	variable = response->first_variable;
	for(i = 0; i < applTable.column_num; i++)
	{
		column = applTable.columns[i];
		...
		/* ckeck oid and extract applIndex */
...
int assocEntry_send_request(SNMP_session *session, u_char type, long applIndex, long assocIndex, char *error_label)
{
	SNMP_pdu *request;
	int i;
	SNMP_column *column;
...
AssocEntry *assocEntry_process_response(SNMP_session *session, SNMP_pdu *response, char *error_label)
{
	...
	pointer = (long *) &(assocEntry->assocRemoteApplication);
	variable = response->first_variable;
	for(i = 0; i < assocTable.column_num; i++)
	{
		column = assocTable.columns[i];
...
int mtaEntry_send_request(SNMP_session *session, u_char type, long applIndex, char *error_label)
{
	SNMP_pdu *request;
	int i;
	SNMP_column *column;
...
MtaEntry *mtaEntry_process_response(SNMP_session *session, SNMP_pdu *response, char *error_label)
{
	...
	pointer = (long *) &(mtaEntry->mtaReceivedMessages);
	variable = response->first_variable;
	for(i = 0; i < mtaTable.column_num; i++)
	{
		column = mtaTable.columns[i];
...
int mtaGroupEntry_send_request(SNMP_session *session, u_char type, long applIndex, long mtaGroupIndex, char *error_label)
{
	SNMP_pdu *request;
	int i;
	SNMP_column *column;
...
MtaGroupEntry *mtaGroupEntry_process_response(SNMP_session *session, SNMP_pdu *response, char *error_label)
{
	...
	pointer = (long *) &(mtaGroupEntry->mtaGroupReceivedMessages);
	variable = response->first_variable;
	for(i = 0; i < mtaGroupTable.column_num; i++)
	{
		column = mtaGroupTable.columns[i];
		...
		/* ckeck oid and extract applIndex and mtaGroupIndex */
...
int mtaGroupAssociationEntry_send_request(SNMP_session *session, u_char type, long applIndex, long mtaGroupIndex, long mtaGroupAssociationIndex, char *error_label)
{
	SNMP_pdu *request;
	int i;
	SNMP_column *column;
...
MtaGroupAssociationEntry *mtaGroupAssociationEntry_process_response(SNMP_session *session, SNMP_pdu *response, char *error_label)
{
	...
	pointer = (long *) &(mtaGroupAssociationEntry->mtaGroupAssociationIndex);
	variable = response->first_variable;
	for(i = 0; i < mtaGroupAssociationTable.column_num; i++)
	{
		column = mtaGroupAssociationTable.columns[i];
...
int dsaOpsEntry_send_request(SNMP_session *session, u_char type, long applIndex, char *error_label)
{
	SNMP_pdu *request;
	int i;
	SNMP_column *column;
...
DsaOpsEntry *dsaOpsEntry_process_response(SNMP_session *session, SNMP_pdu *response, char *error_label)
{
	...
	pointer = (long *) &(dsaOpsEntry->dsaAnonymousBinds);
	variable = response->first_variable;
	for(i = 0; i < dsaOpsTable.column_num; i++)
	{
		column = dsaOpsTable.columns[i];
...
int dsaEntriesEntry_send_request(SNMP_session *session, u_char type, long applIndex, char *error_label)
{
	SNMP_pdu *request;
	int i;
	SNMP_column *column;
...
DsaEntriesEntry *dsaEntriesEntry_process_response(SNMP_session *session, SNMP_pdu *response, char *error_label)
{
	...
	pointer = (long *) &(dsaEntriesEntry->dsaMasterEntries);
	variable = response->first_variable;
	for(i = 0; i < dsaEntriesTable.column_num; i++)
	{
		column = dsaEntriesTable.columns[i];
...
int dsaIntEntry_send_request(SNMP_session *session, u_char type, long applIndex, long dsaIntIndex, char *error_label)
{
	SNMP_pdu *request;
	int i;
	SNMP_column *column;
...
DsaIntEntry *dsaIntEntry_process_response(SNMP_session *session, SNMP_pdu *response, char *error_label)
{
	...
	pointer = (long *) &(dsaIntEntry->dsaName);
	variable = response->first_variable;
	for(i = 0; i < dsaIntTable.column_num; i++)
	{
		column = dsaIntTable.columns[i];
...
int x4msMtaEntry_send_request(SNMP_session *session, u_char type, long x4msMtaIndex, char *error_label)
{
	SNMP_pdu *request;
	int i;
	SNMP_column *column;
...
X4msMtaEntry *x4msMtaEntry_process_response(SNMP_session *session, SNMP_pdu *response, char *error_label)
{
	...
	pointer = (long *) &(x4msMtaEntry->x4msMtaName);
	variable = response->first_variable;
	for(i = 0; i < x4msMtaTable.column_num; i++)
	{
		column = x4msMtaTable.columns[i];
		...
		/* ckeck oid and extract x4msMtaIndex */
...
int x4msUserEntryPart1_send_request(SNMP_session *session, u_char type, long x4msUserIndex, char *error_label)
{
	SNMP_pdu *request;
	int i;
	SNMP_column *column;
...
X4msUserEntryPart1 *x4msUserEntryPart1_process_response(SNMP_session *session, SNMP_pdu *response, char *error_label)
{
	...
	pointer = (long *) &(x4msUserEntryPart1->x4msUserTotalMessages);
	variable = response->first_variable;
	for(i = 0; i < x4msUserTablePart1.column_num; i++)
	{
		column = x4msUserTablePart1.columns[i];
...
int x4msUserEntryPart2_send_request(SNMP_session *session, u_char type, long x4msUserIndex, char *error_label)
{
	SNMP_pdu *request;
	int i;
	SNMP_column *column;
...
X4msUserEntryPart2 *x4msUserEntryPart2_process_response(SNMP_session *session, SNMP_pdu *response, char *error_label)
{
	...
	pointer = (long *) &(x4msUserEntryPart2->x4msUserNewMessages);
	variable = response->first_variable;
	for(i = 0; i < x4msUserTablePart2.column_num; i++)
	{
		column = x4msUserTablePart2.columns[i];
		...
		/* ckeck oid and extract x4msUserIndex */
		if(extract_one_index_from_column(&(variable->name), column->name, &x4msUserIndex))
		{
			snmp_errno = SNMP_ERR_NOSUCHNAME;
			x4msUserEntryPart2_free(x4msUserEntryPart2);
...
int x4msUserAssociationEntry_send_request(SNMP_session *session, u_char type, long x4msUserIndex, long x4msUserAssociationIndex, char *error_label)
{
	SNMP_pdu *request;
	int i;
	SNMP_column *column;
...
X4msUserAssociationEntry *x4msUserAssociationEntry_process_response(SNMP_session *session, SNMP_pdu *response, char *error_label)
{
	...
	pointer = (long *) &(x4msUserAssociationEntry->x4msUserAssociationIndex);
	variable = response->first_variable;
	for(i = 0; i < x4msUserAssociationTable.column_num; i++)
	{
		column = x4msUserAssociationTable.columns[i];
		...
		/* ckeck oid and extract x4msUserIndex and x4msUserAssociationIndex */
...
int x4grpEntry_send_request(SNMP_session *session, u_char type, long x4grpIndex, char *error_label)
{
	SNMP_pdu *request;
	int i;
	SNMP_column *column;
...
X4grpEntry *x4grpEntry_process_response(SNMP_session *session, SNMP_pdu *response, char *error_label)
{
	...
	pointer = (long *) &(x4grpEntry->x4grpName);
	variable = response->first_variable;
	for(i = 0; i < x4grpTable.column_num; i++)
	{
		column = x4grpTable.columns[i];
		...
		/* ckeck oid and extract x4grpIndex */
...
int x4grpMappingEntry_send_request(SNMP_session *session, u_char type, long x4grpIndex, long x4grpMappingMSIndex, long x4grpMappingMTAIndex, char *error_label)
{
	SNMP_pdu *request;
	int i;
	SNMP_column *column;
...
X4grpMappingEntry *x4grpMappingEntry_process_response(SNMP_session *session, SNMP_pdu *response, char *error_label)
{
	...
	pointer = (long *) &(x4grpMappingEntry->x4grpMappingMSIndex);
	variable = response->first_variable;
	for(i = 0; i < x4grpMappingTable.column_num; i++)
	{
		column = x4grpMappingTable.columns[i];
		...
		/* ckeck oid and extract x4grpIndex, x4grpMappingMSIndex and x4grpMappingMTAIndex */
...
int x5dsaReferenceEntry_send_request(SNMP_session *session, u_char type, long x5dsaReferenceIndex, char *error_label)
{
	SNMP_pdu *request;
	int i;
	SNMP_column *column;
...
X5dsaReferenceEntry *x5dsaReferenceEntry_process_response(SNMP_session *session, SNMP_pdu *response, char *error_label)
{
	...
	pointer = (long *) &(x5dsaReferenceEntry->x5dsaReferenceType);
	variable = response->first_variable;
	for(i = 0; i < x5dsaReferenceTable.column_num; i++)
	{
		column = x5dsaReferenceTable.columns[i];
		...
		/* ckeck oid and extract x5dsaReferenceIndex */
...
char *applOperStatus_string(long applOperStatus)
{
...
char *assocApplicationType_string(long assocApplicationType)
{
	static char buffer[20];
...
static int translate_variable(SNMP_variable *variable, int translator, long *pointer, char *error_label)
{
	...
	{
		case TO_INTEGER:
			*pointer = (long) *(variable->val.integer);
			break;

		case TO_ASCII:
			*pointer = (long) malloc(variable->val_len + 1);
			if(*pointer == NULL)
			{
				sprintf(error_label, ERR_MSG_ALLOC);
				return -1;
			}
			memcpy(*pointer, variable->val.string, variable->val_len);
			((char *) (*pointer))[variable->val_len] = '\0';
		...
		case TO_STRING:
			*pointer = (long) malloc(sizeof(String));
			if(*pointer == NULL)
			{
				sprintf(error_label, ERR_MSG_ALLOC);
				return -1;
			}
			((String *) *pointer)->len = variable->val_len;
			((String *) *pointer)->chars = variable->val.string;
			variable->val_len = 0;
			variable->val.string = NULL;
		...
		case TO_OID:
			*pointer = (long) malloc(sizeof(Oid));
			if(*pointer == NULL)
			{
				sprintf(error_label, ERR_MSG_ALLOC);
				return -1;
			}
			((Oid *) *pointer)->len = (variable->val_len)/sizeof(Subid);
			((Oid *) *pointer)->subids = variable->val.objid;
			variable->val_len = 0;
			variable->val.string = NULL;

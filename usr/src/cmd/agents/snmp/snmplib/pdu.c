 ...
 */
#pragma ident  "@(#)pdu.c	1.8 97/07/16 Sun Microsystems"
...
SNMP_variable *snmp_typed_variable_new(Oid *name, u_char type, SNMP_value *value, char *error_label)
{
	...
	switch(type)
	{
		...
		case GAUGE:
		case TIMETICKS:
			new->val.integer = (long *) malloc(sizeof(long));
...
/* Should * sizeof(Subid), yiru's fix*/
			...
			new->val_len = value->v_oid.len * sizeof(Subid);
...
/*
	while( (length = recvfrom(sd, (char *) packet, PACKET_LENGTH, 0,
		(struct sockaddr *) &network_address, &address_length) == -1)
		&& (errno == EINTR) )
*/
...
/*
	trace("%s.%d -> %s.%d\n\n",
		...
		inet_ntoa(network_address.sin_addr), network_address.sin_port);
	trace("%d.%d.%d.%d.%d -> %d.%d.%d.%d.%d\n\n",
		((u_char *) &(address->sin_addr.s_addr))[0],
		((u_char *) &(address->sin_addr.s_addr))[1],
		((u_char *) &(address->sin_addr.s_addr))[2],
		((u_char *) &(address->sin_addr.s_addr))[3],
		address->sin_port,
		((u_char *) &(network_address.sin_addr.s_addr))[0],
		((u_char *) &(network_address.sin_addr.s_addr))[1],
		((u_char *) &(network_address.sin_addr.s_addr))[2],
		((u_char *) &(network_address.sin_addr.s_addr))[3],
		network_address.sin_port);
*/
/*
	while( (bytes = sendto(sd, (char *) packet, length, 0,
		(struct sockaddr *) &network_address, sizeof(Address)) == -1)
		&& (errno == EINTR) )
*/
	bytes = sendto(sd, (char *) packet, length, 0,
...
*/
	...
	switch(variable->type)
	{
		...
		case OBJID:
			...
			oid.len = variable->val_len / sizeof(Subid);
			trace("%s\n", SSAOidString(&oid));
			break;
...
/* header of message */
	data = asn_parse_header(data, &length, &type, static_error_label);
	if(data == NULL)
	{
...
/* version */
	data = asn_parse_int(data, &length, &type, (long *) &pdu->version,
...
/* parse community */
	...
	data = asn_parse_string(data, &length, &type, (u_char *) community,
		&len, static_error_label);
	if(data == NULL)
	{
...
/* header od pdu */
	...
	switch(pdu->type)
	{
		...
		/* request id */
			data = asn_parse_int(data, &length, &type, (long *) &pdu->request_id,
		...
		/* error status */
			data = asn_parse_int(data, &length, &type, (long *) &pdu->error_status,
		...
		/* error index */
			data = asn_parse_int(data, &length, &type, (long *)&pdu->error_index,
		...
		/* enterprise */
			...
			data = asn_parse_objid(data, &length, &type, subids,
		...
		/* agent address */
			...
			data = asn_parse_string(data, &length, &type,
				(u_char *)&pdu->ip_agent_addr.s_addr, &len, static_error_label);
			if(data == NULL)
			{
		...
		/* generic trap */
			data = asn_parse_int(data, &length, &type, (long *)&pdu->generic,
		...
		/* specific trap */
			data = asn_parse_int(data, &length, &type, (long *)&pdu->specific,
		...
		/* time stamp */
			data = asn_parse_int(data, &length, &type, (long *)&pdu->time_stamp,
...
/* header of variable */
	d = asn_parse_header(d, &len, &type, static_error_label);
	if(d == NULL)
	{
...
/* name */
	...
	d = asn_parse_objid(d, &len, &type, subids, &(variable->name.len), static_error_label);
	if(d == NULL)
	{
...
/* find out what type of object this is */
	...
	value_start = d;
	d = asn_parse_header(d, &variable->val_len, &variable->type, static_error_label);
	if(d == NULL)
	{
	...
	switch(variable->type)
	{
		...
		case GAUGE:
		case TIMETICKS:
			variable->val.integer = (long *) malloc(sizeof(long));
			...
			d = asn_parse_unsigned_int(value_start, &len, &variable->type,
				(unsigned long *)variable->val.integer, sizeof(long), static_error_label);
			if(d == NULL)
			{
		...
		case OPAQUE:
			...
			d = asn_parse_string(value_start, &len, &variable->type,
		...
		case OBJID:
			...
			d = asn_parse_objid(value_start, &len, &variable->type,
			...
			variable->val_len = variable->val_len * sizeof(Subid);
...
/* encode the variables in packet */
	...
	total_length = cp - packet;

/* encode the header for the variables in buf */
	buf_len = PACKET_LENGTH;
	cp = asn_build_header(buf, &buf_len, (u_char)(ASN_SEQUENCE | ASN_CONSTRUCTOR),
...
/* copy the encoded variables from packet to buf */
	...
	total_length = total_length + (int) (cp - buf);
...
/* encode the pdu in packet */
	...
	switch(pdu->type)
	{
		...
		/* request id */
			cp = asn_build_int(packet, &len, (u_char) INTEGER,
				(long *) &pdu->request_id, sizeof(pdu->request_id),
				static_error_label);
			if(cp == NULL)
			{
		...
		/* error status */
			cp = asn_build_int(cp, &len, (u_char) INTEGER,
				(long *) &pdu->error_status, sizeof(pdu->error_status),
				static_error_label);
			if(cp == NULL)
			{
		...
		/* error index */
			cp = asn_build_int(cp, &len, (u_char) INTEGER,
				(long *) &pdu->error_index, sizeof(pdu->error_index),
				static_error_label);
			if(cp == NULL)
			{
		...
		/* enterprise */
			cp = asn_build_objid(packet, &len, (u_char) OBJID,
		...
		/* agent-addr */
			cp = asn_build_string(cp, &len, (u_char) IPADDRESS,
		...
		/* generic trap */
			cp = asn_build_int(cp, &len, (u_char) INTEGER,
				(long *) &pdu->generic, sizeof(pdu->generic),
				static_error_label);
			if(cp == NULL)
			{
		...
		/* specific trap */
			cp = asn_build_int(cp, &len, (u_char) INTEGER,
				(long *) &pdu->specific, sizeof(pdu->specific),
				static_error_label);
			if(cp == NULL)
			{
		...
		/* time stamp  */
			cp = asn_build_unsigned_int(cp, &len, (u_char) TIMETICKS,
				(unsigned long *) &pdu->time_stamp, sizeof(pdu->time_stamp),
				static_error_label);
			if(cp == NULL)
			{
...
/* copy the encoded variables and their header from buf to packet */
	...
	total_length = total_length + (int) (cp - packet);


/* encode the header of the pdu in buf */
	len = PACKET_LENGTH;
	cp = asn_build_header(buf, &len, (u_char) pdu->type, total_length, static_error_label);
	if(cp == NULL)
	{
...
/* encode the message in packet */
	...
	cp = asn_build_header(packet, &len, (u_char)(ASN_SEQUENCE | ASN_CONSTRUCTOR),
		total_length + strlen(pdu->community) + 5,
		static_error_label);
	if(cp == NULL)
	{
...
/* community */
	cp = asn_build_string(cp, &len, (u_char) STRING, 
		(u_char *) pdu->community, strlen(pdu->community),
		static_error_label);
	if(cp == NULL)
	{
...
static u_char *snmp_pdu_encode_variable(SNMP_variable *variable, u_char *data, int *length, char *error_label)
{
	...
	dummy_len = *length;
	data_ptr = data;
	data = asn_build_header(data, &dummy_len, (u_char)(ASN_SEQUENCE | ASN_CONSTRUCTOR), 0,
		static_error_label);
	if(data == NULL)
	{
	...
	header_len = data - data_ptr;
	*length = *length - header_len;
	data = asn_build_objid(data, length, (u_char) OBJID,
	...
	switch(variable->type)
	{
		...
		case COUNTER:
		case TIMETICKS:
	    		data = asn_build_int(data, length, variable->type,
				(long *) variable->val.integer, variable->val_len, static_error_label);
			if(data == NULL)
			{
		...
		case IPADDRESS:
		case OPAQUE:
			data = asn_build_string(data, length, variable->type,
				variable->val.string, variable->val_len, static_error_label);
			if(data == NULL)
			{
		...
		case OBJID:
			data = asn_build_objid(data, length, variable->type, variable->val.objid,
				(int) (variable->val_len / sizeof(Subid)), static_error_label);
			if(data == NULL)
			{
		...
		case NULLOBJ:
			data = asn_build_null(data, length, variable->type, static_error_label);
			if(data == NULL)
			{
	...
	dummy_len = (data - data_ptr) - header_len;
	...
	if(asn_build_header(data_ptr, &dummy_len, (u_char)(ASN_SEQUENCE | ASN_CONSTRUCTOR),
...
SNMP_variable *snmp_variable_dup(SNMP_variable *variable, char *error_label)
{
	SNMP_variable *new;
	...
	if(variable->val_len)
	{
		new->val.string = (u_char *) malloc(variable->val_len);
		if(new->val.string == NULL)
		{
			sprintf(error_label, ERR_MSG_ALLOC);
			snmp_variable_free(new);
		...
		memcpy(new->val.string, variable->val.string, variable->val_len);		}
	else
	{
		new->val.string = NULL;
	}
...
SNMP_variable *ssa_append_integer_variable(SNMP_variable *list, Oid *oid, int num,char *error_label)
{
  SNMP_value value;
...
SNMP_variable *ssa_append_string_variable(SNMP_variable *list, Oid *oid, String str, char *error_label)
{
  SNMP_value value;

  if(str.chars == NULL || str.len == 0) return NULL;
  value.v_string.chars = (u_char *)str.chars;
  value.v_string.len = str.len;
  list = snmp_typed_variable_append(list,oid,STRING,&value,error_label);
  if(list == NULL){
        error("ssa_append_string_variable failed: oid: %s, value: %s\n"

 ...
 */
#pragma ident  "@(#)snmp_api.c	1.3 96/07/01 Sun Microsystems"
...
typedef struct _SNMP_request_list {
	...
	u_long	timeout;	/* length to wait for timeout in usec */
...
static u_long static_request_id = 0;
...
SNMP_session *snmp_session_open(char *peername, char *community, int retries, long timeout, void callback(), void *callback_magic, char *error_label)
{
...
*/
	...
	if(isp->sd < 0)
	{
		...
		{
			fprintf(stderr, ERR_MSG_CAN_NOT_ABORT_SESSION,
...
static int snmp_session_read_loop(fd_set *fdset)
{
	...
	for(slp = first_session; slp; slp = slp->next)
	{
		if(FD_ISSET(slp->internal->sd, fdset))
		{
			...
			if(pdu == NULL)
			{
				fprintf(stderr, ERR_MSG_RECEIVED_MANGLED_PACKET,
					static_error_label);
				return;
			}
		...
		for(rp = isp->requests; rp; rp = rp->next_request)
		{
			if(timercmp(&rp->expire, &now, <))
			{
				...
				else
				{
					...
					{
						fprintf(stderr, "snmp_pdu_send() failed: %s\n",
							static_error_label);
					}

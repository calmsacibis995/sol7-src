...
*/
#pragma ident  "@(#)miexample_svc.c	1.3 96/10/18 Sun Microsystems"
...
printf("This is a typical example of a DMI Management Application.\n");
printf("It illustrates only the bare minimum functionality required to\n");
printf("be recognized, by the DMI Service Provider, as a Management Appln.,\n");
printf("and to receive DMI indications. This example does NOT require any\n");
printf("user input. The indication reception will be displayed, as actions\n");
printf("leading to indications, are initiated on the Service Provider, \n");
printf("by utility programs like dmi_cmd.\n");
printf("For more on DMI Indications, please refer to the SEA Development Guide\n\n");
printf("This program functionally takes the following actions\n");
printf("- Registers with the RPC as a server of DMI MI callbacks\n");
printf("- Creates a client RPC handle using the ConnectToServer API call\n");
printf("- Registers with DMI SP as a Mgmt. Appln. using DmiRegister call\n");
printf("- Subscribes with the DMI SP for receiving DMI indications\n");
printf("- Sets Event Filtering to receive all types of indications from any\n");
printf("  component, group class or severity\n"); 
printf("- Each callback function only prints the type of callback received\n"); 
   exit(1);
}
...
main(int argc, char *argv[])
{
        ...
        while((opt = getopt(argc, argv, "s:h?")) != EOF) {
          switch(opt) {
            case 's':
                      ...
                      printf("Run with -h option for USAGE/SYNOPSIS of this program\n\n");
                      break;
            case 'h':
            case '?':
            default:
	...
	if (t_getstate(0) != -1 || t_errno != TBADF) {
		...
		_rpcpmstart = 1;
		openlog("miexample", LOG_PID, LOG_DAEMON);
...
printf("Registering the DMI MI callbacks, as a server with the RPC....\n\n");
...
printf("Creating a RPC client handle with the DMI Service Provider....\n\n");
  if (!ConnectToServer(&connectin, &dmi_rpchandle)) {
     _msgout("ConnectToServer call failed");
     exit(1);
  }
printf("Registering with DMI Service Provider....\n\n");
  regin.handle = 0;
...
printf("Subscribing with the DMI Service Provider for DMI indications....\n\n");
...
printf("Setting Event Filter with the DMI Service Provider for DMI indications....\n\n");
...
printf("Wait Permanently for DMI Indications....\n\n");
		svc_run();
		exit(1);
		/* NOTREACHED */
	}	else {

...
#pragma ident	"@(#)fcall.c	1.3	98/02/17 SMI"
...
/*
 * Initial establishment of breakpoints on traced library functions.
 * if 'exec' is true, the victim process is freshly exec()d.
 */
void
establish_breakpoints(struct ps_prochandle *Pr, int exec)
{
	if (Dynpat == NULL)
		return;
	 ...
	 */
	if (Rdb_agent == NULL &&
	    (Rdb_agent = proc_rd_agent(Pr)) != NULL) {
		rd_notify_t notify;
		struct bkpt *Bp;
	...
	/*
	 * A freshly exec()d, dynamically linked process doesn't get
	 * breakpoints now.  It will happen later, after ld.so.1
	 * has mapped in all of its libraries (RD_CONSISTENT state
	 * on the special RD_DLACTIVITY breakpoint).
	 */
	if (exec && Rdb_agent != NULL)
		return;
	 ...
	 */
	(void) proc_object_iter(Pr, object_iter, Pr);
...
establish_stacks(struct ps_prochandle *Pr)
{
	 ...
	 */
	(void) proc_lwp_iter(Pr, lwp_stack_traps, Pr);
	...
	if ((td_init() != TD_OK || td_ta_new(Pr, &Thr_agent) != TD_OK)) {
		Thr_agent = NULL;
		return;
	}
	(void) td_ta_thr_iter(Thr_agent, thr_stack_traps, Pr, TD_THR_ANY_STATE,
	...
	const char *object_name, struct dynpat *Dyp)
{
	 ...
	 */
	(void) proc_symbol_iter(Pr, object_name,
...
create_bkpt(struct ps_prochandle *Pr, uintptr_t pc, int ret, int set)
{
	 ...
	 */
	if (ret &&
	    ((pmp = proc_addr_to_map(Pr, pc)) == NULL ||
...
report_htable_stats()
{
	...
	for (Stk = callstack; Stk != NULL; Stk = Stk->next) {
		(void) fprintf(stderr,
			...
			(u_long)Stk->stkbase,
			(u_long)Stk->stkend,
			(u_long)(Stk->stkend - Stk->stkbase));
	}
	(void) fprintf(stderr, "primary unix stack --------\n");
	(void) fprintf(stderr,
		"    base = 0x%.8lx  end = 0x%.8lx  size = %ld\n",
		(u_long)Psp->pr_stkbase,
		(u_long)(Psp->pr_stkbase + Psp->pr_stksize),
		(u_long)Psp->pr_stksize);
}
...
find_stack(struct ps_prochandle *Pr, uintptr_t sp)
{
	...
	/* thread stacks? */
	if (has_libthread && Thr_agent == NULL) {
		if ((error = td_init()) != TD_OK) {
			(void) fprintf(stderr,
				"td_init() failed with %d\n", error);
			has_libthread = FALSE;
			Thr_agent = NULL;
		} else if ((error = td_ta_new(Pr, &Thr_agent)) != TD_OK) {
			(void) fprintf(stderr,
	...
	if (hflag) {
		(void) fprintf(stderr,
			...
			(u_long)sp,
			(u_long)thrinfo.ti_stkbase,
			(u_long)thrinfo.ti_stkbase + thrinfo.ti_stksize);
	}
...
get_tid(struct ps_prochandle *Pr, struct callstack *Stk)
{
	...
	if (has_libthread && Thr_agent == NULL &&
	    (td_init() != TD_OK || td_ta_new(Pr, &Thr_agent) != TD_OK)) {
		has_libthread = FALSE;
		Thr_agent = NULL;
	}
	if (Thr_agent == NULL) {
...
reset_breakpoints(struct ps_prochandle *Pr)
{
	...
	(void) establish_breakpoints(Pr, 1);
}
...
trap_one_stack(struct ps_prochandle *Pr, prgregset_t reg)
{
	...
	while (nframe--) {
		...
		if (proc_lookup_by_addr(Pr, frame[nframe].pc, sym_name,
...
get_arguments(struct ps_prochandle *Pr, long *argp)
{
	...
	if (data_model != PR_MODEL_LP64)
		for (i = 0; i < 4; i++)
			argp[i] = (u_int)Psp->pr_lwp.pr_reg[R_O0+i];
	else
		for (i = 0; i < 4; i++)
...
return_count(struct ps_prochandle *Pr, uintptr_t *ppc)
{
	...
	struct bkpt *Bp;
	int count;
	u_char instr[6];	/* instruction at pc */

...
#pragma ident	"@(#)expound.c	1.38	97/12/23 SMI"	/* SVr4.0 1.10	*/
...
static	void	print_siginfo32(siginfo32_t *);
...
show_statvfs(struct ps_prochandle *Pr)
{
	...
	if (sys_nargs > 1 && (offset = sys_args[1]) != NULL &&
		...
		"%s\tflag=%s\n",
			pname,
			svfsflags((u_long)statvfs.f_flag));
...
show_sigaltstack(struct ps_prochandle *Pr, long offset, const char *name)
{
	...
	if (offset != NULL &&
		...
		(void) printf("%s\t%s: sp=0x%.8lX size=%lu flags=0x%.4X\n",
			pname,
			name,
			(u_long)altstack.ss_sp,
			(u_long)altstack.ss_size,
			altstack.ss_flags);
	}
}
...
#ifdef _LP64
static void
print_siginfo32(siginfo32_t *sip)
{
	const char *code = NULL;
	...
	const char *str, long addroff, long lenoff, long len)
{
	...
	if (lenoff != 0) {
		if (Pread(Pr, &len, sizeof (len), lenoff) != sizeof (len))
			return;
	}
	...
	case AF_UNIX:
		/* Null terminate */
		if (len >= 2)
			soun->sun_path[len - 2] = NULL;
		(void) printf("%s\t%s = %s\n", pname, str, soun->sun_path);
		break;
	}
}

...
#pragma ident	"@(#)main.c	1.46	98/02/19 SMI"	/* SVr4.0 1.5	*/
 ...
 */
static	void	setup_basetime(pstatus_t *, hrtime_t, struct timeval *);
...
main(int argc, char *argv[])
{
	...
	/* options */
#define	OPTIONS	"ZFpfcaeilht:T:v:x:s:S:m:M:u:U:r:w:o:"
	...
	if (errflg || (argc <= 0 && ngrab <= 0)) {
		(void) fprintf(stderr,
	"usage:\t%s [-fcaeil] [-[tTvx] [!]syscalls] [-[sS] [!]signals] \\\n",
			command);
		(void) fprintf(stderr,
	...
	if (Dynpat != NULL) {
		...
		if ((Psp->pr_lwp.pr_why != PR_SYSENTRY &&
			...
			establish_breakpoints(Pr, created);
			establish_stacks(Pr);
		}
	}
 ...
 */
static void
setup_basetime(pstatus_t *Psp, hrtime_t basehrtime, struct timeval *basedate)
{
	...
	if ((dflag|Dflag) && !cflag) {
		...
		ptim = ctime(&basedate->tv_sec);
		if ((pdst = tzname[daylight? 1 : 0]) == NULL)
			pdst = "???";
		if (dflag) {
			(void) printf(
				"Base time stamp:  %lu.0000  [ %.24s %s ]\n",
				basedate->tv_sec, ptim, pdst);
			Flush();
		}
	}
}

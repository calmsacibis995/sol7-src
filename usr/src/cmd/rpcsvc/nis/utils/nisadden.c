...
#pragma ident "@(#)nisaddent.c	1.48	97/11/19 SMI"
	...
	nis_result *res;
{
	...
	for (i = 0; i < res->objects.objects_len; i++) {
		if ((c0 = OBJ_COL_VAL(i, 0)) && (c1 = OBJ_COL_VAL(i, 1)) &&
		    (c2 = OBJ_COL_VAL(i, 2)) && (c3 = OBJ_COL_VAL(i, 3))) {
			sprintf(buf, "unix.%s@%s", c2, nis_domain_of(c0));
			strcat(buf, tabtocol(buf, 2));
			strcat(buf, c2);
			strcat(buf, ":");
			strcat(buf, c3);
	...
	nis_object *entry;
{
	...
	c3 = KEYVAL(3);
	c4 = KEYVAL(4);
	if (c0 && *c0 && (c1 || (c2 && c3 && c4)) && ! did_netgroup(c0)) {
...
{
	int i, j;
	char buf[BIGBUF+1], *c0, *c1, *c2, *c3, *c4, *c5;
...
struct ttypelist_t ttypelist[] = {
	...
	{ "netid", "netid.byname", "cred.org_dir", "cred_tbl",
	    "auth_type=LOCAL", 0, 0, " \t",
	    dbmniskey_netid, nisdbmkey_netid, filedbmkey, genent_netid,
	    dump_netid, 0,
	...
	void *udata;
{
	...
	return (removeentry(table, entry));
}

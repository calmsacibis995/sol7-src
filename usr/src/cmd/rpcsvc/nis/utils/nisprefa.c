...
#ident	"@(#)nisprefadm.c	1.6	96/12/10 SMI"
...
extern u_long __inet_get_addr(void *, int);
...
main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "vZTFLGalmurxo:d:C:")) != EOF) {
		switch (c) {
			...
			if (geteuid() != (uid_t)0) {
				fprintf(stderr,
					"%s: -L option must be run as root.\n",
					argv[0]);
				exit(1);
			}
			op_count++;
...
get_servers(server_list *servers, char *domain, char *client, int create)
{
	...
	nis_result *res;
	nis_object *obj;
	u_long flags = FOLLOW_PATH|FOLLOW_LINKS|EXPAND_NAME;

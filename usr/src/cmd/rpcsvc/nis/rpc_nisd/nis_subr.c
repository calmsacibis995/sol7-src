#pragma ident	"@(#)nis_subr_proc.c	1.49	96/12/17 SMI"
/*
 *	nis_subr_proc.c
 *
	...
	enum name_pos p)
{
	...
	if (dd->dl_serve == 1) /* master */
		...
		dd->dl_stamps[dd->dl_serve-1] =
			nis_cptime_msg(GETSERVER(d_obj, 0), name, FALSE);
		/*
		 * If timestamp == 0, (i.e., failed to retrieve a timestamp),
		 * add ping item to list with largest possible time. If we
		 * didn't do this, the replica would forget about this update.
		 */
		if (dd->dl_stamps[dd->dl_serve-1] == 0)
			dd->dl_stamps[dd->dl_serve-1] = ~0UL;
		if (!readonly &&

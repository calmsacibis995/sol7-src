#pragma ident	"@(#)yp_ns_proc.c	1.26	97/11/19 SMI"
/*
 *	yp_ns_proc.c
 *
...
static char record[YPMAXRECORD];
static char keyval[YPMAXRECORD];
struct ypresp_all {
	long		status;
	string_t	table_name;
	nis_name	princp;
...
domain_list_check(char *dname)
{
	...
	dl = (struct domain_list *)malloc(sizeof (*dl));
	if (dl == 0)
		return (0);
	dl->domain = strdup(dname);
	if (dl->domain == 0) {
		free(dl);
		return (0);
	}
...
ypproc_master_svc(req, rqstp)
	...
	struct svc_req *rqstp;
{
	static	struct ypresp_master resp;
	static char master[YPMAXPEER];
...
ypproc_first_svc(req, rqstp)
	...
	struct svc_req *rqstp;
{
	static struct ypresp_key_val resp;
...
((resp.key_column_ndx = get_keyndx(&(dbres->obj->TA_data), column)) < 0 ||
	...
	/*
	 *  We create a child to do the reply.  The parent
	 *  returns 0 so that the svc code won't try to send
	 *  a reply.
	 */
...
ypproc_maplist_svc(dname, rqstp)
	...
	struct svc_req *rqstp;
{
	static	struct ypresp_maplist maplist;
	...
	struct ypresp_all *resp;
{
	...
	bool_t more = TRUE;
	nis_error status;
	static char short_tblnm[YPMAXMAP];
...
/*
 *  N.B.  This global static is used by the qsort routine below because
 *        qsort doesn't have a way to pass in data besides the entries
 *        to sort.  This won't work for MT, but could be fixed by
...
map2table(xx_mapname, xx_table, xx_column)
	...
	char **xx_column;
{
	static char tbl[YPMAXMAP], col[NIS_MAXATTRNAME];
	char *p, *t = tbl, *c = col;

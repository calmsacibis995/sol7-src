/*
 * Copyright (c) 1990-1995, 1997, by Sun Microsystems, Inc.
 * All rights reserved.
 */

#pragma ident	"@(#)nis_db.c	1.36	97/12/12 SMI"
	...
	nis_name	name)	/* name of directory to checkpoint */
{
	...
	if (status != DB_SUCCESS) {
		syslog(LOG_ERR,
			"db_checkpoint: Unable to checkpoint table '%s'",
			table ? table : "(all)");
	...
	nis_name	name)	/* name of directory to checkpoint */
{
	...
	if (fnr->status == NIS_NOTFOUND || fnr->status == NIS_SUCCESS) {
		...
		if (status != DB_SUCCESS) {
			syslog(LOG_ERR,
			"db_checkpoint: Unable to checkpoint '%s'", name);
			err++;
		}
	}
	...
	while (fnr->status == NIS_SUCCESS) {
		if (__type_of(fnr->obj) == NIS_TABLE_OBJ ||
			...
			if (status != DB_SUCCESS) {
				syslog(LOG_ERR,
			"db_checkpoint: Unable to checkpoint '%s'", tblname);
				err++;
			}
		}
...
checkpoint_db()
{
	...
	for (cp; cp; cp = nxt) {
		...
		if (res->cp_status != NIS_SUCCESS) {
			syslog(LOG_WARNING,
			"checkpoint_db: unable to completely checkpoint %s",
				cp->item.name);
		}
...
checkpoint_table(char *name)
{
	...
	if (status != DB_SUCCESS) {
		syslog(LOG_ERR,
			"checkpoint_table: Unable to checkpoint '%s' (%d)",
			name, status);
		return (0);
	}
	return (1);

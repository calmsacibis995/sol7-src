...
setup_server()
{
	...
	then
		...
		then
			...
			$ECHO "sed -e \"s/^$ENTRY/$ADDR/\" $HOSTS_FILE \
					> /tmp/hosts.$$"
...
#
# old_setup_security(): this routine is the same as the setup_security
# above.  The difference is that this is targetted for Solaris 2.2 or
# older.  It runs the keylogin and chkey according to security level.
#               $1  specifies root or user setup
#
old_setup_security()
{
	...
	$ECHO "keylogin $OPTION > /dev/null"
	if [ $? -ne 0 ]
	then
		echo "**ERROR: keylogin failed."
		echo ""
		echo "The network password that you have typed is invalid."
		echo "If this machine was initialized before as an NIS+ client"
		echo "and the network password has been changed, please type the new"
		echo "network password."
		echo "Or re-type the network password that your administrator gave you."
		echo ""
 
		$ECHO "keylogin $OPTION > /dev/null"
		if [ $? -ne 0 ]
		then
			echo "**ERROR: keylogin failed again."
			echo "Please contact your system administrator to verify your network password."
			exit 1
		fi
	fi
	...
	do
		...
		if [ $MESS = "user" ]
		then
			echo "Please type your login password."
		else
			echo "Please type the $MESS login password for `uname -n`."
		fi
		$ECHO "chkey -f -s nisplus > /dev/null"
		if [ $? -ne 0 ]
		then
			echo ""
			echo "**ERROR: chkey failed."
			...
			if [ $COUNT -ge 3 ]
			then
				echo "**ERROR: chkey failed.  Please do the following manually:"
				echo "	1. run \"chkey\" and type your own $MESS password at the password prompt."
				exit 1
			else
				echo "  you must type your own $MESS password."
			fi
		else
			RESULT=0
		fi
...
restore_service()
{
	if [ $OS -eq 5 ]
	then
		SWITCH=`grep "^passwd:" /etc/nsswitch.conf`

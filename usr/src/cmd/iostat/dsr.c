...
#pragma ident	"@(#)dsr.c	1.6	97/12/16 SMI"
...
build_disk_list(void *v)
{
	...
	rv = (disk_list_t **)malloc(sizeof (disk_list_t *));
	if (rv) {
		*rv = (disk_list_t *)0;
...
rummage_path_to_inst(nms_t *list)
{
	...
	if (inpt) {
		...
		while (fgets(ibuf, sizeof (ibuf), inpt) != (char *)0) {
			if (ibuf[0] != '#') {
...
pline(char *ib, nms_t *list)
{
	...
	*rpt = '\0';
	...
	if ((entry = find_str(ib, list)) == (nms_t *)0)
		goto done;
...
main()
{
	disk_list_t *l;
	int	i;
	l = build_disk_list(NULL);
	while (l) {
		printf("%s%d -> %s\n", l->dtype, l->dnum, l->dsk);
		for (i = 0; i < NDKMAP; i++) {
			if (l->dpart[i] != NULL)
				printf(" %s \n", l->dpart[i]);
		}

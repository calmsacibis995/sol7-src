...
#pragma	ident	"@(#)su.c	1.43	97/12/16 SMI"	/* Svr4.0 1.9.5.16 */
...
#define	TEXT_DOMAIN "SYS_TEST"	/* Use this only if it wasn't */
	...
	else if ((flags = (getuid() != (uid_t)0)) != 0) {
...
ok:
	 ...
	 */
	if (eflag) {
		(void) strncat(homedir, dir,
			sizeof (homedir) - strlen("HOME="));
		homedir[sizeof (homedir) - 1] = '\0';
		(void) strncat(logname, name,
			sizeof (logname) - strlen("LOGNAME="));  /* M003 */
		logname[sizeof (logname) - 1] = '\0';
		if (hz = getenv("HZ")) {
			(void) strncat(hzname, hz,
				sizeof (hzname) - strlen("HZ="));
			hzname[sizeof (hzname) - 1] = '\0';
		}
		if (tz = getenv("TZ")) {
			(void) strncat(tznam, tz,
				sizeof (tznam) - strlen("TZ="));
			tznam[sizeof (tznam) - 1] = '\0';
		}
		(void) strncat(shelltyp, pshell,
			sizeof (shelltyp) - strlen("SHELL="));
		shelltyp[sizeof (shelltyp) - 1] = '\0';
		(void) chdir(dir);
		...
		if ((term = getenv("TERM")) != NULL) {
			(void) strncat(termtyp, term,
				sizeof (termtyp) - strlen("TERM="));
			termtyp[sizeof (termtyp) - 1] = '\0';
...
validate(char *usernam)
{
	...
	if ((error = pam_acct_mgmt(pamh, PAM_SILENT)) != 0) {
		if (error == PAM_ACCT_EXPIRED) {

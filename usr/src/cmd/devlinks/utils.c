#ident	"@(#)utils.c	1.8	97/08/12 SMI"
/*
...
create_dirs(const char *dirp)
{
	 ...
	 * in the input sting ends in a '/'.  This junk can be removed when
	 * mkdirp is fixed.
	 */
	char *cp;
	...
	/*
	 * If this proves slow, can try statting directory before
	 * doing mkdirp().
	 */
...
getdevaddr(const char *devnm, const unsigned int subno)
{
	...
	*np = '\0';
	...
	if (subno != 0) {
		...
		/* So another substing call doesn't stop on this */
		strcpy(nbuf, subfp);
	}
 ...
 */
const char *
substring(const char *string, const char sep_char, const int ss_no)
{
	static char buf[256];
	char *cp = buf;
	int s_index = 1;
	...
	for (dp = cache; dp != NULL; dp = dp->next) {
		if (strcmp(dirname, dp->dir->dirname) == 0) {
			...
			if (dirp->state != CACHE_S_CLOSED) {
			    fmessage(1,
				"Can't re-open already open directory '%s'\n",
				dirname);
			/*NOTREACHED*/
			}
...
int
cache_symlink(const char *linkdata, const char *dirname, const char *filename)
{
	int rc;
	CACHE_HDR *dp;
	CACHE_IENT *ientp;
	...
	for (ientp = dp->dir->head; ientp != NULL; ientp = ientp->next) {
		if (strcmp(ientp->ent.name, filename) == 0) {
			 ...
			 */
			if (ientp->ent.linkto &&
			    strcmp(ientp->ent.linkto, linkdata) == 0)
				return (0);

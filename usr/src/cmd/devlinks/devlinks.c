...
#pragma	ident	"@(#)devlinks.c	1.20	97/11/13 SMI"
...
/*ARGSUSED*/
int
devfs_cache_entry(di_node_t node, di_minor_t minor, void *arg)
{
    struct devfscache *chp, *curchp;
    int i;

	/* follow official indentation style */
	int len;
	char *devtype, *cp;
	char devfsnm[PATH_MAX + 1];
	...
	/*
	 * Some minor nodetype are not defined, especially the pseudo
	 * nodes. This is a weak point in the implementation of the
	 * framework, which should be fixed.
	 *   We copy the kludge in devfs_find(): assuming NULL node
	 *   type is the same as DDI_PSEUDO.
	 */
	if ((devtype = di_minor_nodetype(minor)) == NULL)
		devtype = DDI_PSEUDO;
...
get_devfs_entries(void)
{
	...
	if ((root = di_init("/", DINFOSUBTREE | DINFOMINOR)) == DI_NODE_NIL) {
	    	wmessage("devlinks: di_init failed,"
	 ...
	 *       in link generation.
	 */
	di_walk_minor(root, NULL, 0, NULL, devfs_cache_entry);
	di_fini(root);

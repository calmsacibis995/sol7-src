...
#pragma ident  "@(#)tapes.c 1.13     97/12/23 SMI"
...
get_devfs_var(const char *lnname)
{
	/*
	 * At present this code works for names of the form:
	 *	../../devices/junk0.../junkN/xx@[targetno,]unito:dev_attr
	 */
	...
	/*
	 * Verify correct format of device attributes
	 * We ignore any minor names beginning with a digit
	 */
	sp++;
	if (*sp == 0 || *sp < '0' || *sp > '9') {
...
void
addtnode(
	int devno,		/* Tape device no. in /dev - land */
...
devfs_entry(di_node_t node, di_minor_t minor, void *arg)
{
	...
	(void) sprintf(devicenm, "../../devices/%s", fsbase);
...
get_devfs_entries(void)
{
        ...
        if ((root = di_init("/", DINFOSUBTREE | DINFOMINOR)) == DI_NODE_NIL) {
                wmessage("tapes: di_init failed, check the devinfo driver.\n");
                exit (1);
        }
        /*
         ...
         *       in link generation.
         */
        di_walk_minor(root, DDI_NT_TAPE, 0, NULL, devfs_entry);
        di_fini(root);
}

void

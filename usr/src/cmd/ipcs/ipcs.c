...
#pragma	ident	"@(#)ipcs.c	1.21	98/01/21 SMI"
...
#define	USAGE "usage: ipcs [-abcmopqstX] [-C corefile] [-N namelist]\n"
...
static char chdr[] = "T         ID      KEY        MODE        OWNER    GROUP",
	...
	*mem = NULL,		/* memory file */
	opts[] = "abcimopqstC:N:X"; /* allowable options for getopt */
...
main(int argc, char *argv[])
{
	...
	(void) printf(gettext("IPC status from %s as of %s"),
	    mem ? mem : "<running system>", ctime(&time));
	...
	if (mflg) {
		...
		for (i = 0, addr = (uintptr_t)pshm;
			...
			if (oflg)
				(void) printf("%7lu", mds.shm_nattch);
			if (bflg)
			...
			if (iflg)
				(void) printf("%9u", mds.shm_cnattch);
			(void) printf("\n");
		}
	}

...
#pragma ident	"@(#)server.c	1.19	97/02/26 SMI"
...
getcacheptr(char * s)
{
	static const char *caches[3] = {"passwd", "group", "hosts"};
...
#define	NSCD_TABLES		3	/* passwd, group, hosts */
...
server_tsd_bind(void *arg)
{
	static void *value = 0;
        ...
        /*
         *      disable cancellation to prevent hangs when server threads disappear
         */
...
server_create()
{
	...
	mutex_unlock(&create_lock);
	thr_create(NULL, 0, server_tsd_bind, NULL, THR_BOUND|THR_DETACHED, NULL);
}
...
main(int argc, char ** argv)
{
	...
	if (geteuid()) {
		if (argc != 2 || strcmp(argv[1], "-g")) {
			fprintf(stderr,
			    "Must be root to use any option other than -g.\n\n");
	...
	while ((opt = getopt(argc, argv, "S:Kf:c:ge:p:n:i:l:d:s:h:o:")) != EOF) {
		nsc_stat_t * cache;
		char * cacheopt;
	...
	if(!loaded_config_file) {
		fprintf(stderr, 
			"No configuration file specifed and /etc/nscd.conf not present\n");
		exit(1);
	}
...
/*ARGSUSED*/
void switcher(void *cookie, void *argp, int arg_size, door_desc_t *dp,
	      int n_desc)
{
	union {
		nsc_data_t data;
	...
	case KILLSERVER: {
		...
		if (dc.dc_euid != 0) {
			logit(
			      "SETADMIN call failed (cred): caller pid %d, uid %d, euid %d\n",
		...
		if(ptr->nsc_callnumber == KILLSERVER) {
			logit(
			      "Nscd received KILLSERVER cmd from pid %d, uid %d, euid %d\n",
			      dc.dc_pid, dc.dc_ruid, dc.dc_euid);
			exit(0);
		}
		else
...
usage(char * s)
{
	...
	fprintf(stderr,
		"	[-h cachename,keep_hot_count] [-o cachename,\"yes\"|\"no\"]\n");
	...
	fprintf(stderr,
		"\n	Supported caches: passwd, group, hosts\n");
	exit(1);
	...
	int newlogfd;
		...
		logfd = -1;
                ...
                if ((newlogfd = open(s, O_EXCL|O_WRONLY|O_CREAT, 0644)) < 0) {
                         ...
                         * safely is hard = it could be a hard link or a symbolic
                         * link to a system file.
                         *
                         */
                        struct stat before;
                            ...
                            (before.st_uid == 0)) {    /* owned by root */
                                if ((newlogfd =
                                     open(s, O_APPEND|O_WRONLY, 0644)) < 0) {
                                        logit("Cannot open new logfile \"%s\": %s\n",
                                              s,
                                              strerror(errno));
                                        return (-1);
                                }
                        } else {
                                logit("Cannot use specified logfile \"%s\": file is/has links or isn't owned by root\n",
                                      s);
                                return(-1);
                        }
                }
...
dump_stat(nsc_stat_t * ptr)
{
	...
	if(hitrate)
	    hitrate =  (100*(ptr->nsc_pos_cache_hits + ptr->nsc_neg_cache_hits))/hitrate;
	...
	printf("%10d  most active entries to be kept valid\n",
	       ptr->nsc_keephot);
	printf("%10s  check /etc/{passwd,group,hosts} file for changes\n",
	       (ptr->nsc_check_files?"Yes":"No"));

	printf("%10s  use possibly stale data rather than waiting for refresh\n",

...
#pragma ident	"@(#)getgr.c	1.9	97/04/25 SMI"
...
getgr_lookup(nsc_return_t *out, int maxsize, nsc_call_t * in, time_t now)
{
 ...
 getout:
	...
	if((current_admin.group.nsc_secure_mode != 0)
	   &&
	   ...
	   !(UPDATEBIT & in->nsc_callnumber)) {
		...
		if (_door_cred(&dc) < 0) {
			...
			if (dc.dc_egid != out->nsc_u.grp.gr_gid) {
				/*
				...
				char * foo = out->nsc_u.buff + sizeof(struct group)
				    +  (int)out->nsc_u.grp.gr_passwd;

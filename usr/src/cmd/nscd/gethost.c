...
#pragma ident   "@(#)gethost.c 1.9     97/04/25 SMI"
...
static gethost_addrkeepalive( int keep, int interval );
static gethost_invalidate_unlocked( void );
static gethost_namekeepalive( int keep, int interval );
static int addr_to_int( char * addr );
static int int_to_addr( int h );
static int update_host_bucket( nsc_bucket_t ** old, nsc_bucket_t * new, int callnumber );
...
static void do_invalidate(nsc_bucket_t ** ptr, int callnumber)
{
	if(*ptr != NULL && *ptr != (nsc_bucket_t *) -1) { /* leave pending calls alone */
...
static void do_findhnams(nsc_bucket_t *ptr, int * table, char * name)
{
	...
	if(ptr != NULL && ptr != (nsc_bucket_t *) -1) { 
		...
		char * tmp = (char *) insertn(table, ptr->nsc_hits, (int)strdup(name));
		if (tmp != (char *) -1) 
			free(tmp);
	}
}
...
gethost_revalidate()
{
	...
	while(1) {
		...
		if((count=current_admin.host.nsc_keephot)!=0) {
...
static gethost_invalidate_unlocked()
{
...
gethost_lookup(nsc_return_t *out, int maxsize, nsc_call_t * in, time_t now)
{
	...
	while (1) {
		if (MASKUPDATEBIT(in->nsc_callnumber) == GETHOSTBYADDR) {
			bucket = get_hash(addr_hash, (char*)addr_to_int(in->nsc_u.addr.a_data));
		} else {
	...
	if(*bucket == NULL && current_admin.avoid_nameservice) {
		...
		/*
		 *  block any threads accessing this bucket if data is non-existent or
		 *  out of date
		 */
		...
		/* 
		 *  if still not -1 bucket we are doing update... mark to prevent
		...
		if (MASKUPDATEBIT(in->nsc_callnumber) == GETHOSTBYADDR) {
			...
			p = _uncached_gethostbyname_r(in->nsc_u.name, &out->nsc_u.hst,
						      out->nsc_u.buff+sizeof(struct hostent), 
						      bufferspace,
						      &saved_errno);
		}
		...
		if (p == NULL) { /* data not found */
			...
			memcpy(out, &(retb->nsc_data),
			       retb->nsc_data.nsc_bufferbytesused);
			update_host_bucket((nsc_bucket_t**)bucket, retb, in->nsc_callnumber);
			goto getout;
		}
		...
		if((retb->nsc_timestamp < now) &&
		   ...
		   !(retb->nsc_status & ST_UPDATE_PENDING)) {
			...
			retb->nsc_status |= ST_UPDATE_PENDING; /* cleared by deletion of old data */
			launch_update(in);
		}
	}
	...
	if((new != NULL) 
	   ...
	   (new != (nsc_bucket_t *) -1)) { 
		...
		if(new->nsc_data.nsc_return_code == SUCCESS) {
			new->nsc_timestamp = time(NULL) + current_admin.host.nsc_pos_ttl;
		}
		else {
			new->nsc_timestamp = time(NULL) + current_admin.host.nsc_neg_ttl;
		}
	}
...
fixbuffer(nsc_return_t * in, int maxlen)
{
	...
	for(i=0;i<numaddrs;i++) {
		memcpy(dest, in->nsc_u.hst.h_addr_list[i], in->nsc_u.hst.h_length);
		strs = in->nsc_u.hst.h_length;

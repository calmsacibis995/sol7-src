...
#pragma ident	"@(#)getpw.c	1.9	97/04/25 SMI"
...
static getpw_invalidate_unlocked( void );
static getpw_namekeepalive( int keep, int interval );
static int update_pw_bucket( nsc_bucket_t ** old, nsc_bucket_t * new, int callnumber );
...
static getpw_invalidate_unlocked()
{
	...
	if(*bucket == NULL && current_admin.avoid_nameservice) {
		...
		if (MASKUPDATEBIT(in->nsc_callnumber) == GETPWUID) {
			p = _uncached_getpwuid_r(in->nsc_u.uid, &out->nsc_u.pwd,
					   out->nsc_u.buff+sizeof(struct passwd), bufferspace);
			saved_errno = errno;
		} else {
			p = _uncached_getpwnam_r(in->nsc_u.name, &out->nsc_u.pwd,
					   out->nsc_u.buff+sizeof(struct passwd), bufferspace);
			saved_errno = errno;
		}
		...
		if (p == NULL) { /* data not found */
			...
			memcpy(out, &retb->nsc_data,
			       retb->nsc_data.nsc_bufferbytesused);
			update_pw_bucket((nsc_bucket_t **)bucket, retb, in->nsc_callnumber);
			goto getout;
		}
	...
	if((current_admin.passwd.nsc_secure_mode != 0)
	   ...
	   !(UPDATEBIT & in->nsc_callnumber)) {
		...
		if (_door_cred(&dc) < 0) {
			...
			if (dc.dc_euid != out->nsc_u.pwd.pw_uid) {
				/*
				...
				char * foo = out->nsc_u.buff + sizeof(struct passwd)
	...
	if((new != NULL) 
	   ...
	   (new != (nsc_bucket_t *) -1)) { 
		...
		if(new->nsc_data.nsc_return_code == SUCCESS) {
			new->nsc_timestamp = time(NULL) + current_admin.passwd.nsc_pos_ttl;
		}
		else {
			new->nsc_timestamp = time(NULL) + current_admin.passwd.nsc_neg_ttl;
		}
	}

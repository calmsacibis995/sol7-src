 ...
 * All rights reserved.
 */
#pragma	ident	"@(#)gen.c 1.47	97/11/19  SMI"
...
extern int	lc_has_collating_elements; /* from gram.y */
...
{
	extern int	copying;
		/* might be using internal, non-escaped, data */
...
*/
static void
fp_puthdr(FILE *fp, int kind, size_t size)
{
	fprintf(fp,
"\t{%d, _LC_MAGIC, _LC_VERSION_MAJOR, _LC_VERSION_MINOR, %d},\n",
		kind, /* LINTED */
		(int)size);
}
...
static void
gen_charmap(FILE *fp, _LC_charmap_t *lc_cmap)
{
...
"/*------------------------- CHARMAP OBJECT  -------------------------*/\n");
	...
	*/
	...
	fp_putmeth(fp, CHARMAP_BTOWC_AT_NATIVE);
	fp_putmeth(fp, CHARMAP_WCTOB_AT_NATIVE);
	fp_putmeth(fp, CHARMAP_MBSINIT);
	fp_putmeth(fp, CHARMAP_MBRLEN);
	fp_putmeth(fp, CHARMAP_MBRTOWC_AT_NATIVE);
	fp_putmeth(fp, CHARMAP_WCRTOMB_AT_NATIVE);
	fp_putmeth(fp, CHARMAP_MBSRTOWCS_AT_NATIVE);
	fp_putmeth(fp, CHARMAP_WCSRTOMBS_AT_NATIVE);
	...
	if (Native == FALSE) {		/* producing a non-native locale */
		fprintf(fp,
"static _LC_methods_charmap_t user_methods_charmap={\n");
		...
		*/
		...
		fp_putmeth(fp, CHARMAP_BTOWC);
		fp_putmeth(fp, CHARMAP_WCTOB);
		fp_putmeth(fp, CHARMAP_MBSINIT);
		fp_putmeth(fp, CHARMAP_MBRLEN);
		fp_putmeth(fp, CHARMAP_MBRTOWC);
		fp_putmeth(fp, CHARMAP_WCRTOMB);
		fp_putmeth(fp, CHARMAP_MBSRTOWCS);
		fp_putmeth(fp, CHARMAP_WCSRTOMBS);
	...
	/* class core */
	fp_puthdr(fp, (int) _LC_CHARMAP, sizeof (_LC_charmap_t));
...
compress_masks(_LC_ctype_t *ctype)
{
	...
	for (i = 256; i <= max_wchar_enc; i++) { /* for each codepoint > 255 */
		/*
		Search for a mask in the 'qmask' array which matches the mask
		for	the character corresponding to 'i'.
		*/
		for (j = 0; j < umasks; j++)
...
#ifdef craigm
		if (lc_ctype->lower == NULL) {
			wchar_t *lp = (wchar_t *) calloc(sizeof (wchar_t),
				lc_ctype->max_upper + 1);

			/* tolower unspecified, make it */
			/* the inverse of toupper */
			lc_ctype->lower = lp;
...
#define	N_PER_LINE_TRANSFORMATIONS	8
		...
		for (; i <= lc_ctype->transtabs[k].tmax;
			...
			for (j = 0; (j < N_PER_LINE_TRANSFORMATIONS) &&
				...
				fprintf(fp, "0x%04x, ",
lc_ctype->transtabs[k].table[i + j - lc_ctype->transtabs[k].tmin]);
...
fprintf(fp, "0x%04x, ",
	transtabs->table[i + j - transtabs->tmin]);
				}
				line_count += N_PER_LINE_TRANSFORMATIONS;
if ((i == 0) ||
	(i == transtabs->tmin) ||
	...
	if ((max_wchar_enc > 255) && !copying_ctype) {
		...
		for (i = 256; i <= lc_ctype->qidx_hbound; i += N_PER_LINE) {
			...
			if ((line_count % 512) == 0) {
				fprintf(fp, "};\n");
				fprintf(fp,
"static const unsigned char qidx_%d[] = {\n",
					line_count);
			}
#endif
...
"/*---------------------   TRANSFORMATION TABLES   --------------------*/\n");
	fprintf(fp, "static const _LC_transnm_t transname[]={\n");
...
"static const _LC_transtabs_t	transtabs_%s_%d = {\n",
				transname, no_of_tbls);
			fprintf(fp, "\ttransformation_%s_%d,\n",
				transname, no_of_tbls);
			fprintf(fp, "\t0x%08x, \t0x%08x,\n",
				transtabs->tmin, transtabs->tmax);
			if (transtabs->next) {
				fprintf(fp,
"\t(_LC_transtabs_t *)&transtabs_%s_%d\n",
	...
	fprintf(fp, "static const _LC_transtabs_t transtabs[]={\n");
	...
	for (i = 0; i < lc_ctype->ntrans; i++) {
		...
		fprintf(fp, "\t\t0x%08x, \t0x%08x,\n",
			transtabs->tmin, transtabs->tmax);
		if (transtabs->next) {
			fprintf(fp, "\t\t(_LC_transtabs_t *)&transtabs_%s_1\n",
				transname);
		} else {
			fprintf(fp, "\t\tNULL\n");
		}
...
"/*-------------------------   CTYPE OBJECT   -------------------------*/\n");
	...
	*/
	...
	if (Native == FALSE) {
		fprintf(fp,
"static _LC_methods_ctype_t user_methods_ctype={\n");
	...
	/* class core */
	fp_puthdr(fp, (int) _LC_CTYPE, sizeof (_LC_ctype_t));
...
/* The current localedef does not use coll_eq() */
#if 0
static int
...
/* The current localedef does not use compress_collate() */
#if 0
/*
...
*/
static void
compress_collate(_LC_collate_t *coll)
{
	int	i, j;
	 ...
	 */
	if (!coll->co_coltbl || !coll->co_cetbl)
		return;
	...
	for (i = max_wchar_enc; i > 0; i--) {
		...
		if (coll->co_cetbl[i] != coll->co_cetbl[i-1])
			/*
		...
		for (j = 0; j <= (int)((unsigned int)coll->co_nord); j++) {
			if (!coll_eq(coll->co_coltbl[i][j],
				coll->co_coltbl[i - 1][j]))
				goto nomatch;
		}
	}
...
fp_putsubsflag(FILE *fp, int flags)
{
	...
	case (_SUBS_ACTIVE|_SUBS_REGEXP):
		fputs("_SUBS_REGEXP|", fp);	/* FALLTHRU */
...
"/*------------------------- COLLELLS --------------------------------*/\n");
	...
	/* Generate weight lists of collation elements. */
	if (coll->co_coltbl != NULL && coll->co_cetbl != NULL) {
		for (i = 0; i <= coll->co_hbound; i++) {
			/*
			 * If there are any collation elements beginning with
			 * 'i' then create it.
			 */
			if (coll->co_cetbl[i] != NULL) {
				_LC_collel_t *ce;

				fprintf(fp,
				    "static const wchar_t cew%d[][%d]={\n",
					i, coll->co_nord + 1);
				for (j = 0, ce = (_LC_collel_t *)
				    &(coll->co_cetbl[i][j]);
					ce->ce_sym != NULL;
					    ce = (_LC_collel_t *)&
					    (coll->co_cetbl[i][++j])) {
					/*
					 * Create collation weights for this
					 * collation element
					 */
					for (k = 0;
					    k <= (int)
					    ((unsigned int)coll->co_nord);
						k++)
						fprintf(fp, "%d, ",
						    ce->ce_wgt[k]);
					fprintf(fp, "\n");
		...
		for (i = 0; i <= coll->co_hbound; i++) {
			/*
			 * If there are any collation elements beginning with
			 * 'i' then have a pointer to a list of weights.
			 */
			if (coll->co_cetbl[i] != NULL) {
				_LC_collel_t *ce;

				fprintf(fp,
				    "static const _LC_collel_t ce%d[]={\n", i);
				/*
				 * One entry for each collation elementing
				 * beginning with 'i'.
				 */
				for (j = 0, ce = (_LC_collel_t *)
				    &(coll->co_cetbl[i][j]);
					ce->ce_sym != NULL;
					    ...
					    &(coll->co_cetbl[i][++j])) {
					fprintf(fp,
	    "{ \"%s\", {(_LC_weight_t)&cew%d[%d][0]} },\n", ce->ce_sym, i, j);
				}
				fprintf(fp, "{ 0, 0 },\n};\n");
			}
		}
	} /* end of if (coll->co_coltbl != NULL && coll->co_cetbl != NULL) */
...
"/*------------------------- COLLTBL WEIGHTS -------------------------*/\n");
		...
		for (j = 0; j <= (int)((unsigned int)coll->co_nord); j++) {
			...
			for (line_count = i = 0; i <= coll->co_hbound; i++) {
#ifndef	_NO_GCC_HACK
				line_count++;
				if ((line_count % 512) == 0) {
					fprintf(fp, "};\n");
				fprintf(fp,
				    "static const wchar_t ct_wgts%d_%d[]={\n",
					j, line_count);
				}
#endif
				fprintf(fp, "%d,\n", coll->co_coltbl[i][j]);
...
"/*------------------------- COLLTBL ---------------------------------*/\n");
		...
		fprintf(fp, "static const _LC_weight_t colltbl[] ={\n");

		for (i = 0; i <= (int)((unsigned int)coll->co_nord); i++)
			fprintf(fp, "(_LC_weight_t)ct_wgts%d,\n", i);

		fprintf(fp, "};\n\n");
	}

	if (coll->co_cetbl != NULL) {
		/* This locale has some collating elements. */
#ifndef _NO_GCC_HACK
		int	line_count = 0;
#endif
		fprintf(fp, "static const _LC_collel_t	*cetbl[] = {\n");
		...
		for (i = 0; i <= coll->co_hbound; i++) {
			...
			if ((line_count % 512) == 0) {
				fprintf(fp, "};\n");
				fprintf(fp,
"static const _LC_collel_t	*cetbl_%d[] = {\n",
					line_count);
			}
#endif
			fprintf(fp, "{ ");
...
"/*------------------------- SUBSTITUTION STR-------------------------*/\n");
		...
		if (coll->co_nsubs > 0) {
			fprintf(fp, "static const wchar_t subs_wgts[][%d]={\n",
			    coll->co_nord+1);
			for (i = 0; i < (int)((unsigned int)coll->co_nsubs);
			    i++) {
				for (j = 0;
				    j <= (int)((unsigned int)coll->co_nord);
					j++)
					fp_putsubsflag(fp,
						coll->co_subs[i].ss_act[j]);
		...
		fprintf(fp, "static const _LC_subs_t substrs[] = {\n");
		for (i = 0; i < (int)((unsigned int)coll->co_nsubs); i++) {

			fprintf(fp,
"\t{ (_LC_weight_t)subs_wgts[%d], \"%s\", \"%s\" },\n",
				i,
...
"/*------------------------- COLLATE OBJECT  -------------------------*/\n");
	...
	*/
	...
	if (Native == FALSE) {
		fprintf(fp,
"static _LC_methods_collate_t user_methods_collate={\n");
	...
	fprintf(fp, "static _LC_collate_t lc_coll = {{\n");

	/* class core */
	fp_puthdr(fp, (int) _LC_COLLATE, sizeof (_LC_collate_t));
	...
	if (coll->co_coltbl != NULL) {
		...
		if (coll->co_cetbl != NULL) {
			fp_putsym(fp, "cetbl"); /* _LC_collel_t *cetbl; */
		} else {
			fp_putsym(fp, "(const _LC_collel_t **)NULL");
		}
	} else {
...
"/*------------------------- MONETARY OBJECT  -------------------------*/\n");
	...
	fprintf(fp,
"static _LC_methods_monetary_t native_methods_monetary={\n");
	...
	*/
	...
	/* class core */
	fp_puthdr(fp, (int) _LC_MONETARY, sizeof (_LC_monetary_t));
...
"/*-------------------------   TIME OBJECT   -------------------------*/\n");
	...
	*/
	...
	/* class core */
	fp_puthdr(fp, (int) _LC_TIME, sizeof (_LC_time_t));
...
"/*------------------------- NUMERIC OBJECT  -------------------------*/\n");
	...
	*/
	...
	/* class core */
	fp_puthdr(fp, (int) _LC_NUMERIC, sizeof (_LC_numeric_t));
...
"/*------------------------- MESSAGE OBJECT  -------------------------*/\n");
	...
	fprintf(fp,
"static _LC_methods_messages_t native_methods_messages={\n");
	...
	*/
	...
	/* class core */
	fp_puthdr(fp, (int) _LC_MESSAGES, sizeof (_LC_messages_t));
...
"/*-------------------------- LOCALE OBJECT --------------------------*/\n");
	...
	*/
	...
	/* class core */
	fp_puthdr(fp, (int) _LC_LOCALE, sizeof (_LC_locale_t));

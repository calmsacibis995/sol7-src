 ...
 * All rights reserved.
 */
#pragma	ident	"@(#)semstack.c 1.7	97/10/15  SMI"
...
sem_push(item_t *i)
{
	if (stack_top < max_stack) {
		...
		if (max_stack == 0) {	/* not initialized */
...
#ifdef SDEBUG
			fprintf(stderr, "stack initialized: %d\n", max_stack);
#endif
			...
			new_stack = (item_t **)realloc((void *)stack,
				sizeof (item_t *) * new_size);
			if (new_stack == NULL) {
				error(ERR_MEM_ALLOC_FAIL, __FILE__, __LINE__);
				/* NOTREACHED */
			}
#ifdef SDEBUG
			fprintf(stderr, "stack realloced: %d\n", new_size);
#endif
			stack = new_stack;
			max_stack = new_size;
		}

 ...
 * All rights reserved.
 */
#pragma	ident	"@(#)sem_chr.c 1.20	97/12/06  SMI"
		...
		*s = MALLOC(char, it->value.chr->len + 1);
		...
		if (it->type == SK_INT) {
			*s = MALLOC(char, MB_LEN_MAX + 1);
			(void) mbs_from_fc(*s, it->value.int_no);
		} else
			INTERNAL_ERROR;
	}
...
sem_existing_symbol(char *s)
{
	...
	if (sym == NULL) {
		...
		sym->data.chr->fc_enc = *s;
		sym->data.chr->len =
			mbs_from_fc((char *)sym->data.chr->str_enc, *s);
...
extract_digit_list(char *s, int *value)
{
	...
	for (i = 0; s[i] != '\0' && !isdigit(s[i]); i++)	/* LINTED */
		;
...
sem_symbol_range_def(void)
{
	...
	for (i = start; i <= end; i++) {
		...
		if (eucpc != eucpc_from_dense)
			...
			width = INT_METHOD(
				(int (*)(_LC_charmap_t *, wchar_t))
				METH_OFFS(CHARMAP_WCWIDTH_AT_NATIVE))
				(&charmap, pc);
...
wc_from_fc(int fc)
{
	...
	if (j >= 2) {
		/*
		 * if first byte doesn't have high bit or second byte in
		 * restricted range.
		 */
...
fill_euc_info(_LC_euc_info_t *euc_info)
{
	...
	case 3:
		...
		break;
	default:
		fprintf(stderr, "unsupport length for EUC CS1 length=%d\n",
	...
	case 3:
		...
		break;
	default:
		fprintf(stderr, "unsupport length for EUC CS2 length=%d\n",
	...
	case 3:
		...
		break;
	default:
		fprintf(stderr, "unsupport length for EUC CS3 length=%d\n",
	...
	if (euc_cs2_max == 0)		/* EUC CS2 not used */
		...
		euc_info->cs3_base =
			euc_cs2_max - euc_cs2_min + euc_info->cs2_base + 1;
	...
	if ((euc_cs2_max == 0) &&
		...
		euc_info->cs1_base =
			euc_cs3_max - euc_cs3_min + euc_info->cs3_base + 1;

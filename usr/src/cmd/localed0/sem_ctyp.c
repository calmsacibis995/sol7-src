 ...
 * All rights reserved.
 */
#pragma	ident	"@(#)sem_ctype.c 1.18	97/07/25  SMI"
...
/* ARGSUSED */
void
add_char_ct_name(_LC_ctype_t *ctype,
	...
	int defined)
{
	...
	if (ctype->nbinds > MAX_CHAR_CLASS_TRANS) {
		/* do more craigm */
		fprintf(stderr,
			"Can't add any more classifications/transformations\n");
		exit(1);
	}
	...
	char *ctype_symbol_name)
{
	...
	if (sym1 != NULL) {
		...
		(void) add_symbol(&cm_symtab, sym0);
	}
...
sem_set_lcbind_symbolic_value(void)
{
	...
	for (i = 0; i < length_lcbind_symbol_table; i++) {
		if (strcmp(symbol_name->value.str,
			...
			fprintf(stderr,
				"Error: duplicate LCBIND symbolic name\n");
			exit(1);
		}
		if (hex_num == lcbind_symbol_table[i].value) {
			fprintf(stderr,
				"ERROR: LCBIND duplicate value %8x\n", hex_num);
			exit(1);
		}
	}

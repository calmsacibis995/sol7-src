 ...
 * All rights reserved.
 */
#pragma	ident	"@(#)sem_coll.c 1.22	97/07/28  SMI"
 ...
 * Internal symbol
 */
#define	INTERNAL_SYM_LOW	"INT_LOW"
#define	INTERNAL_SYM_HIGH	"INT_HIGH"

#ifdef _LP64
#define	_WCHAR_MAX	UINT_MAX
#else
#define	_WCHAR_MAX	ULONG_MAX
#endif
...
nxt_coll_wgt(void)
{
	static wchar_t nxt_coll_val = 0x01010101;
...
set_coll_wgt(_LC_weight_t *weights, wchar_t weight, int ord)
{
	...
	if (*weights == NULL) {
		*weights = MALLOC(wchar_t, collate.co_nord + 1);
...
sem_coll_sym_ref(void)
{
	...
	case ST_CHR_SYM:
		...
		if (*(s->data.chr->wgt) == NULL) {
			*(s->data.chr->wgt) = MALLOC(wchar_t, collate.co_nord);
	...
	case ST_COLL_ELL:
		...
		if (ce->ce_wgt == NULL) {
			ce->ce_wgt = MALLOC(wchar_t, collate.co_nord);
	...
	case ST_COLL_SYM:
		if (*(s->data.collsym) == NULL) {
			*(s->data.collsym) = MALLOC(wchar_t, collate.co_nord);
...
sem_coll_literal_ref(void)
{
	...
	rc = INT_METHOD(
		...
		METH_OFFS(CHARMAP_WCWIDTH_AT_NATIVE))(&charmap, pc);
	if (rc > max_disp_width)
		max_disp_width = rc;

	/* LINTED */
	dummy->data.chr->width = (short) rc;
...
sem_collel_list(_LC_weight_t *w, symbol_t *tgt, int order)
{
	...
	if (i->value.int_no == 1) {
		...
		case ST_CHR_SYM:		/* character */
			weight = get_coll_wgt(si->value.sym->data.chr->wgt,
				order);
	...
	/* assign collation weight for self reference */
					...
					set_coll_wgt(
						si->value.sym->data.chr->wgt,
						next_weight, -1);
					set_coll_wgt(w, next_weight, order);
				} else {
					add_forw_reference(tgt, si->value.sym,
						order);
					/* needs to allocate wights array */
					set_coll_wgt(w, UNDEFINED, order);
					return;
				}
			} else {
	 ...
	 */
		...
		case ST_COLL_SYM:	/* collation symbol */
			weight = get_coll_wgt(si->value.sym->data.collsym,
				order);
			if ((weight == UNDEFINED) || (weight == SUB_STRING)) {
	/*
	 * a symbol with UNDEFINED collation can only appear on the
...
strncat(subs, (char *)buf[n]->value.sym->data.chr->str_enc,	MB_LEN_MAX);
			else
				INTERNAL_ERROR;
	 ...
	 */
		if (tgt->sym_type == ST_COLL_ELL) {
			srcs_temp = MALLOC(char,
				strlen(tgt->data.collel->str) + 1);
			strcpy(srcs_temp, tgt->data.collel->str);
		} else {
			srcs_temp = MALLOC(char, MB_LEN_MAX+1);
			strncpy(srcs_temp,
				(char *)tgt->data.chr->str_enc, MB_LEN_MAX);
		}
	 ...
	 */
				if (collate.co_subs[n].ss_act == NULL) {
					collate.co_subs[n].ss_act =
						MALLOC(wchar_t,
						collate.co_nord + 1);
					set_coll_wgt(
						&(collate.co_subs[n].ss_act),
...
sem_set_dflt_collwgt(void)
{
	...
	case ST_CHR_SYM:
		...
		if (get_coll_wgt(&(collate.co_coltbl[pc]), 0) != UNDEFINED) {
		...
		set_coll_wgt(&(collate.co_coltbl[pc]),
			weight, -1);

		/* put weight in symbol table entry for character. */
		sym->data.chr->wgt = &(collate.co_coltbl[pc]);
		break;
...
sem_set_dflt_collwgt_range(void)
{
	...
	for (i = start; i <= end; i++) {
		...
		if ((wc = wc_from_fc(i)) >= 0) {
			...
			if (get_coll_wgt(&(collate.co_coltbl[wc]),
				0) != UNDEFINED) {
				diag_error(ERR_DUP_COLL_RNG_SPEC, "<???>",
					s0->sym_id, s1->sym_id);
				return;
			}
			/* get next available collation weight */
	...
	/* put weight in coll table at spot for wchar encoding */
			set_coll_wgt(&(collate.co_coltbl[wc]), weight, -1);
...
sem_sort_spec(void)
{
	 ...
	 */
	collate.co_sort = MALLOC(wchar_t, collate.co_nord+1);
	...
	for (i = 0; i <= max_wchar_enc; i++) {
		...
		set_coll_wgt(&(collate.co_coltbl[i]), UNDEFINED, -1);
	}
	 ...
	 */
	/*
	 * seed the symbol table with IGNORE,
	 * INTERNAL_SYM_LOW, and INTERNAL_SYM_HIGH
	 */
/*
 ...
 */
	...
	(void) add_symbol(&cm_symtab, s);

/*
 * INTERNAL_SYM_LOW gets the first available collation value.
 */
	s = create_symbol(INTERNAL_SYM_LOW, 0);	/* "INTERNAL_LOW" */
	s->sym_type = ST_CHR_SYM;
	s->data.chr = MALLOC(chr_sym_t, 1);
	s->data.chr->wc_enc = 0;
	s->data.chr->width = 0;
	...
	(void) add_symbol(&cm_symtab, s);
	/*
	 * INTERNAL_SYM_HIGH gets the maximum collation value which
	 * can be contained in a wchar_t.
	 * This ensures that all values will collate < than INTERNAL_SYM_HIGH
	 *
	 * This is just a temporary until the last collation weight has actually
	 * been assigned.  We will reset every collation weight which is
	 * INTERNAL_SYM_HIGH to the current max plus one at gen() time.
	 */
	s = create_symbol(INTERNAL_SYM_HIGH, 0);
	s->sym_type = ST_CHR_SYM;
	s->data.chr = MALLOC(chr_sym_t, 1);
	s->data.chr->wc_enc = 0;
	s->data.chr->width = 0;
	...
	/*
	 * UNDEFINED may collate INTERNAL_SYM_HIGH or INTERNAL_SYM_LOW.
	 * By default, characters not specified in the collation order
	 * collate INTERNAL_SYM_HIGH.
	 */
...
sem_def_collel(void)
{
	...
	(void) add_symbol(&cm_symtab, sym_name);
	...
	rc = INT_METHOD(
		(int (*)(_LC_charmap_t *, wchar_t *, const char *, size_t))
		METH_OFFS(CHARMAP_MBTOWC_AT_NATIVE))(&charmap, &pc,
		(const char *)temp_ptr, MB_LEN_MAX);
	...
	if (collate.co_cetbl[pc] != NULL) {
		 ...
		 */
		for (i = 0;
			collate.co_cetbl[pc][i].ce_sym != NULL;
			i++)	/* LINTED */
			;
	...
	for (i = j = 0; i < n_syms; i++, j++) {
		...
		c = strcmp(sym, collate.co_cetbl[pc][i].ce_sym);
		if (c < 0 && i == j) {
			coll_sym[j].ce_sym = sym;
			/*
			 * We shouldn't set any weight here yet!
			 * set_coll_wgt(&(coll_sym[j].ce_wgt), UNDEFINED, -1);
			 */
			j++;
		}
		coll_sym[j].ce_sym = collate.co_cetbl[pc][i].ce_sym;
		/*
		 * We shouldn't set any weight here yet!
		 * set_coll_wgt(&(coll_sym[j].ce_wgt), UNDEFINED, -1);
		 */
...
*  and assignment of specific value for INTERNAL_SYM_HIGH and UNDEFINED
*  collation weights.
*/
void
sem_collate(void)
{
	...
	s = loc_symbol(&cm_symtab, INTERNAL_SYM_HIGH, 0);
	if (s == NULL)
		INTERNAL_ERROR;
	high = s->data.chr->wgt;
	...
	for (p = forw_ref_head; p; p = p->next) {
		...
		case ST_CHR_SYM:
			...
			if (collate.co_coltbl[sw][0] == UNDEFINED) {
				error(ERR_FORWARD_REF, p->symbol->sym_id);
				/* NOTREACHED */
			}
			...
			if (p->target->sym_type == ST_CHR_SYM) {
				...
				collate.co_coltbl[tw][p->order] =
				    collate.co_coltbl[sw][0];

			} else if (p->target->sym_type == ST_COLL_ELL) {

			    tw = p->target->data.collel->pc;
			    for (i = 0, ce = &(collate.co_cetbl[tw][i]);
					ce->ce_sym != NULL;
					ce = &(collate.co_cetbl[tw][++i]))
					if (strcmp(ce->ce_sym,
						p->target->data.collel->sym)
						== 0) {
						ce->ce_wgt[p->order] =
						collate.co_coltbl[sw][0];
						break;
		...
		case ST_COLL_ELL:
			...
			for (i = 0, ce = &(collate.co_cetbl[sw][i]);
			    ce->ce_sym != NULL;
			    ce = &(collate.co_cetbl[sw][++i]))
			    if (strcmp(ce->ce_sym,
					p->symbol->data.collel->sym) == 0) {
					break;
				}
	 ...
	 */
			...
			break;
		} /* end of "switch (p->symbol->sym_type)" */
	} /* end of "for (p = forw_ref_head; p ; p = p->next)" */

	/*
	 * Substitute symbols with UNDEFINED, and INTERNAL_SYM_HIGH weights
	 * for the weights ultimately determined for UNDEFINED
	 * and INTERNAL_SYM_HIGH.
	 */
	for (i = 0; i <= max_wchar_enc; i++) {

	    if (wchar_defined(i)) {
			for (j = 0; j <= (int)((unsigned int)collate.co_nord);
				j++) {
				if (collate.co_coltbl[i][j] == UNDEFINED) {
					collate.co_coltbl[i][j] =
						(*undefined)[j];
					if (warn)
						diag_error(ERR_NO_UNDEFINED);
					warn = FALSE;
				}
				if (collate.co_coltbl[i][j] == (wchar_t) IGNORE)
	...
	for (j = 0, ce = &(collate.co_cetbl[i][j]);
		...
		for (k = 0;	k <= (int)((unsigned int)collate.co_nord);
			k++) {
			if (ce->ce_wgt[k] == UNDEFINED)
				ce->ce_wgt[k] = (*undefined)[j];
			if (ce->ce_wgt[k] == _WCHAR_MAX)
				ce->ce_wgt[k] = (*high)[j];
			if (ce->ce_wgt[k] == (wchar_t) IGNORE)
				ce->ce_wgt[k] = 0;
		}
	}
			}
...
setup_substr(void)
{
	...
	for (n = 0; n < (int)((unsigned int)collate.co_nsubs); n++) {
	    ...
	    collate.co_subs[n].ss_act = MALLOC(wchar_t, collate.co_nord + 1);

 ...
 * All rights reserved.
 */
#pragma	ident	"@(#)scan.c 1.22	97/11/10  SMI"
		...
		if (c == escape_char) {
			...
			do {
				...
				case 'd':  /* decimal constant */
					for (c = input(), s = number;
						isdigit(c);
						c = input())	/* LINTED */
*s++ = yytext[++yypos] = (char) c;
...
*s++ = yytext[++yypos] = (char) c;
	...
	if (isdigit(c)) { /* octal constant */
		for (s = number; c >= '0' && c <= '7' &&
			c != EOF; c = input())	/* LINTED */
			*s++ = yytext[++yypos] = (char) c;
		*s++ = '\0';
		if (c == '8' || c == '9') {
...
if (kw != NULL) {
	...
	strcmp(yytext, "comment_char") == 0 && yycharno == 14) {
	comment_char = getspecialchar(ERR_COM_CHAR_MISSING);
	continue;

} else if (!seenfirst &&
	strcmp(yytext, "escape_char") == 0 && yycharno == 13) {
	escape_char = getspecialchar(ERR_ESC_CHAR_MISSING);
	continue;
}
/*

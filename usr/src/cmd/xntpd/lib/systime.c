...
#pragma ident	"@(#)systime.c	1.1	96/11/01 SMI"
	...
	l_fp *ts;
{
...
#ifdef DEBUG
	if (debug > 4)
		syslog(LOG_DEBUG, "adj_systime(%s sec): offset = %s%s sec\n",
	...
	if (offset_i > 0 || offset_f >= tsf_maxslew) {
		...
		"systime: dwTimeAdjustment = %d, sys_clock_offset = %s sec\n",
				dwTimeAdjustment, mfptoa(offset_i, offset_f, 9));
#endif /* SYS_WINNT */
#endif
	}
	...
	if (!SetSystemTimeAdjustment(dwTimeAdjustment, FALSE)) {
...
#ifdef DEBUGRS6000
		syslog(LOG_ERR, "adj_systime(%s sec): offset = %s%s sec\n",
			mfptoa((adj<0?-1:0), adj, 9), isneg?"-":"",
			umfptoa(offset_i, offset_f, 9));
		...
		if (oadjtv.tv_sec != 0 || oadjtv.tv_usec != 0) {
			...
			if (max_no_complete > 0) {
				...
				if (debug > 4)
					syslog(LOG_DEBUG,
					    "Previous adjtime() incomplete, residual = %s sec\n",
					    tvtoa(&oadjtv));
#endif
				if (--max_no_complete == 0)
					syslog(LOG_WARNING,
...
#ifdef DEBUG
	if (debug > 3)
		syslog(LOG_DEBUG, "step: %s sec, sys_clock_offset = %s sec, adjtv = %s sec, timetv = %s sec\n",
...
#ifdef DEBUG
	if (debug > 3)
		syslog(LOG_DEBUG, "step: new timetv = %s sec\n", utvtoa(&timetv));
#endif

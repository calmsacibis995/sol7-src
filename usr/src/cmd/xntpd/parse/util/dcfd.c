...
#pragma ident	"@(#)dcfd.c	1.2	96/11/12 SMI"

/*
 * /src/NTP/REPOSITORY/v3/parse/util/dcfd.c,v 3.22 1995/02/11 15:51:35 kardel Exp
 *  
 * dcfd.c,v 3.22 1995/02/11 15:51:35 kardel Exp
 *
  ...
  char **argv;
{
  ...
  int  ac = argc;
  char *file = NULL;
  char *drift_file = "/etc/inet/dcfd.drift";
  int fd;
  int offset = 15;
  int offsets = 0;
  ...
  if (fd == -1)
      ...
      if (TTY_GETATTR(fd,  &term) == -1)
	{
      ...
      term.c_cc[VMIN] = 1;
#if defined(SYS_IRIX4) || defined(SYS_IRIX5)/* somehow doesn't grok PARENB & IGNPAR (mj) */

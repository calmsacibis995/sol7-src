...
#pragma ident	"@(#)ntp_intres.c	1.1	96/11/01 SMI"
...
ntp_intres()
{
	...
	for (;;) {
		if (confentries == NULL)
			exit(0);
#ifndef SYS_WINNT
		/* No need to worry in the NT environment whether the
	...
	struct conf_peer *conf;
{
...
#else
		ReadFile((HANDLE)sockfd, (char *)&reqpkt, (DWORD)REQ_LEN_MAC,
	...
	if ((dwWait == WAIT_FAILED) || (dwWait == WAIT_TIMEOUT)) {
		if (dwWait == WAIT_FAILED)
			syslog(LOG_ERR, "WaitForSingleObject failed: %m");
		return 0;
	}
#endif /* SYS_WINNT */
	...
	for (;;) {
		...
		if ((ret == FALSE) && (GetLastError() != ERROR_IO_PENDING)) {
			syslog(LOG_ERR, "ReadFile() fails: %m");
			return 0;
		}
		dwWait = WaitForSingleObject(hReadWriteEvent, (DWORD) TIMEOUT_SEC * 1000);
		if ((dwWait == WAIT_FAILED) || (dwWait == WAIT_TIMEOUT)) {
			if (dwWait == WAIT_FAILED) {
				syslog(LOG_ERR, "WaitForSingleObject fails: %m");
				return 0;
			}
			continue;
		}

...
#pragma ident	"@(#)ntp_timer.c	1.1	96/11/01 SMI"
...
init_timer()
{
	...
	if (hMutex == NULL) {
		syslog(LOG_ERR, "cannot create a mutex: %m\n");
		exit(1);
	}
...
timer()
{
    ...
    if (!ReleaseMutex(hMutex)) {
		syslog(LOG_ERR, "alarming cannot release mutex: %m\n");
		exit(1);
    }
#endif /* SYS_WINNT */
    ...
    DWORD dwUser, DWORD dw1, DWORD dw2)
{
	...
	if (initializing2) {
			...
			if (!SetThreadPriority(TimerThreadHandle, (DWORD) THREAD_PRIORITY_HIGHEST))
			 	syslog(LOG_ERR, "SetThreadPriority failed: %m");
	...
	switch (dwWaitResult) {
    	...
    	default:
			/* Cannot get mutex ownership due to time-out. */
				syslog(LOG_ERR, "alarming error with mutex: %m\n");
				exit(1);
	}

...
#pragma ident	"@(#)refclock_gpstm.c	1.1	96/11/01 SMI"
...
{
	register int i, j;
	register u_int32 tmp_ui, tmp_uf;
	...
	*offset = gpstm->offset[median];
	 L_ADD(offset,&fudgefactor1[gpstm->unit]);
	...
	register struct refclockbug *bug;
{
	...
	if (gpstm->lasttime != 0)
		...
		bug->values[0] = 0;
	bug->values[1] = (u_int32)gpstm->reason;
	bug->values[2] = (u_int32)gpstm->year;
	bug->values[3] = (u_int32)gpstm->day;
	bug->values[4] = (u_int32)gpstm->hour;
	bug->values[5] = (u_int32)gpstm->minute;
	bug->values[6] = (u_int32)gpstm->second;
	bug->values[7] = (u_int32)gpstm->msec;

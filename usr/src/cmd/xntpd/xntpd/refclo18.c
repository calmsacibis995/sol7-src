...
#pragma ident	"@(#)refclock_parse.c	1.1	96/11/01 SMI"

#if defined(REFCLOCK) && (defined(PARSE) || defined(PARSEPPS))
/*
 * /src/NTP/REPOSITORY/v3/xntpd/refclock_parse.c,v 3.80 1995/07/29 08:04:17 kardel Exp
 *
 * refclock_parse.c,v 3.80 1995/07/29 08:04:17 kardel Exp
 *
...
static char rcsid[]="refclock_parse.c,v 3.80 1995/07/29 08:04:17 kardel Exp";
#endif
     ...
     u_long            state;
{
  ...
  if (!err->err_cnt && do_it)
    syslog(LOG_INFO, "PARSE receiver #%d: interval for following error message class is at least %s",
  ...
  if (do_it && err->err_suppressed)
    {
      syslog(LOG_INFO, "PARSE receiver #%d: %d message%s suppressed, error condition class persists for %s",
     ...
     parsectl_t  *tcl;
{
  ...
  if (ioctl(parse->generic->io.fd, I_STR, (caddr_t)&strioc) == -1)
    {
      ERR(ERR_INTERNAL)
	syslog(LOG_ERR, "PARSE receiver #%d: stream_timecode: ioctl(fd, I_STR, PARSEIOC_TIMECODE): %m", CLK_UNIT(parse->peer));
      return 0;
    }
     ...
     struct parseunit *parse;
{
  ...
  while ((rtc = select(fd + 1, &fdmask, 0, 0, &selecttime)) != 1)
	...
	{
	  ERR(ERR_NODATA)
	    syslog(LOG_WARNING, "PARSE receiver #%d: no data[old] from device (check receiver / cableling)", CLK_UNIT(parse->peer));
	}
  ...
  while ((rtc = select(fd + 1, &fdmask, 0, 0, &selecttime)) != 1)
	...
	{
	  ERR(ERR_NODATA)
	    syslog(LOG_WARNING, "PARSE receiver #%d: no data[new] from device (check receiver / cableling) ", CLK_UNIT(parse->peer));
	}
  ...
  do
      ...
      while ((rtc = select(fd + 1, &fdmask, 0, 0, &selecttime)) != 1)
	{
	  ...
	  if (rtc == -1)
	    ...
	    {
	      ERR(ERR_NODATA)
		syslog(LOG_WARNING, "PARSE receiver #%d: no data from device (check receiver / cableling) ", CLK_UNIT(parse->peer));
	    }
...
#define O_NOCTTY 0
  ...
  if (TTY_GETATTR(fd232, &tm) == -1)
      ...
      if ((parse_clockinfo[type].cl_lflag & ICANON) == 0)
	{
      ...
      tm.c_cflag     = parse_clockinfo[type].cl_cflag;
      tm.c_iflag     = parse_clockinfo[type].cl_iflag;
      tm.c_oflag     = parse_clockinfo[type].cl_oflag;
      tm.c_lflag     = parse_clockinfo[type].cl_lflag;
	

#ifdef HAVE_TERMIOS
      if ((cfsetospeed(&tm, parse_clockinfo[type].cl_speed) == -1) ||
	  (cfsetispeed(&tm, parse_clockinfo[type].cl_speed) == -1))
	{
	  syslog(LOG_ERR, "PARSE receiver #%d: parse_start: tcset{i,o}speed(&tm, speed): %m", unit);
	  parse_shutdown(CLK_UNIT(parse->peer), peer); /* let our cleaning staff do the work */
	  return 0;
	}
#else
      tm.c_cflag     |= parse_clockinfo[type].cl_speed;
#endif
...
#define TCIOFLUSH 2
  ...
  if (!notice)
    {
      NLOG(NLOG_CLOCKINFO) /* conditional if clause for conditional syslog */
	syslog(LOG_INFO, "NTP PARSE support: Copyright (c) 1989-1995, Frank Kardel");
      notice = 1;
    }
	...
	struct peer *peer;
{
  ...
  if (!parse->pollonly)
      ...
      if (parse->pollneeddata)
	{
	  ...
	  ERR(ERR_NODATA)
	    syslog(LOG_WARNING, "PARSE receiver #%d: no data from device within poll interval (check receiver / cableling)", CLK_UNIT(parse->peer));
	}
  ...
  struct refclockstat *out;
{
  ...
  if (out)
      ...
      for (i = 0; i <= CEVNT_MAX; i++)
	{
      ...
      sprintf(tt, "refclock_driver_version=\"refclock_parse.c,v 3.80 1995/07/29 08:04:17 kardel Exp\"");
  ...
  if (parse->generic->currentstatus != (u_char) event)
      ...
      else
	{
	  NLOG(NLOG_CLOCKSTATUS)
	    syslog(LOG_INFO, "PARSE receiver #%d: SYNCHRONIZED",
		   CLK_UNIT(parse->peer));
	}
  ...
  parsetime_t      *parsetime;
{
  ...
  if (parse->laststatus != parsetime->parse_status)
      ...
      if ((parsetime->parse_status & CVT_MASK) == CVT_FAIL)
	{
	  ...
	  if (!PARSE_GETTIMECODE(parse, &tmpctl))
	    ...
	    {
	      ERR(ERR_BADDATA)
		syslog(LOG_WARNING, "PARSE receiver #%d: FAILED TIMECODE: \"%s\" (check receiver configuration / cableling)",
  ...
  if (PARSE_POWERUP(parsetime->parse_state))
      ...
      else
	{
	  ...
	  NLOG(NLOG_CLOCKSTATUS)
	    ERR(ERR_BADSTATUS)
	      syslog(LOG_ERR,"PARSE receiver #%d: TIMECODE NOT CONFIRMED",
			     CLK_UNIT(parse->peer));
	}
    }
  ...
  struct parseunit *parse;
{
   ...
   */
  if (TTY_GETATTR(parse->generic->io.fd, &tm) == -1)
    {
  ...
  else
    {
      ...
      if (TTY_SETATTR(parse->generic->io.fd, &tm) == -1)
	{
 ...
 *
 * Revision 3.79  1995/07/15  07:19:51  kardel
 * removed CTTY - not portable - and we are not using TERMIO signals
 *
 * Revision 3.78  1995/07/02  19:52:53  kardel
 * safeguard for stray parse_receive (async-io) & noctty
 *
 * Revision 3.77  1995/06/18  12:28:35  kardel
 * moved to more standard refclock handling
 * regressive error message generation
 * syslog cleanup
 *
 * Revision 3.76  1995/03/26  16:12:05  kardel
 * GPS166 now keeps phase when losing SVs
 *
 * Revision 3.75  1995/02/14  08:24:04  kardel
 * Babbling clocks do always async IO
 *
 * Revision 3.74  1995/02/11  15:47:02  kardel
 * Trimble TAIP tidbits
 *
 * Revision 3.73  1994/12/18  18:59:21  kardel
 * fix statistics output
 *
 * Revision 3.72  1994/12/04  19:04:53  kardel
 * set VMIN/VTIME only on non canonical input processing
 *
 * Revision 3.71  1994/12/04  17:54:21  kardel
 * setup VMIN & VTIME in termio(s) structures
 *
 * Revision 3.70  1994/12/04  17:08:59  kardel
 * improve TRIMBLE TAIP poll command
 *
 * Revision 3.69  1994/11/19  17:26:56  kardel
 * irix specialties
 *
 * Revision 3.68  1994/11/13  18:36:10  kardel
 * identify 'device=...' correctly in mode 6 messages
 *
 * Revision 3.67  1994/11/01  23:21:43  kardel
 * adjust PPS logic
 *
 * Revision 3.66  1994/10/18  08:36:57  kardel
 * avoid name clash with FreeBSD/BSD4.4 clockinfo
 *
 * Revision 3.65  1994/10/16  21:37:25  kardel
 * fdpps is now part of refclock_atom.c
 *
 * Revision 3.64  1994/10/16  18:56:03  kardel
 * integrate RCC8000 modifications from R. E. Broughton
 *
 * Revision 3.63  1994/10/03  22:02:45  kardel
 * fix JAN_1970, add pre-version of RCC8000
 *
 * Revision 3.62  1994/10/03  10:06:11  kardel
 * 3.4e reconcilation
 *

...
#pragma ident	"@(#)refclock_atom.c	1.3	97/03/21 SMI"
 ...
 */
#if defined(PPS) || defined(PPSCLK) || defined(PPSCD)
...
#ifdef PPS720
/*
 * pps_sample720 - sample the PCL-720
 */
u_long
pps_sample720()
{
	 ...
	 * within 65535us.
	 */
	if (inb(pcl720_data_24_31(PCL720_IOB)) & 0x01) {
		NLOG(NLOG_CLOCKINFO) /* conditional if clause for conditional syslog */
...
#ifdef PPS720_DEBUG
	syslog(LOG_DEBUG, "PCL-720: %u us", f);
#endif
	return (f);
}
#endif
	...
	struct peer *peer;
{
	...
	pp->polls++;
#ifdef PPS720_DEBUG
	syslog(LOG_DEBUG, "pps_poll: pps_update=%d, pollcnt=%u",
	       pps_update, up->pollcnt);
#endif
	if (up->pollcnt == 0) {

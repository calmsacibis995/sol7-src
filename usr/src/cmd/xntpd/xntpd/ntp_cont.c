...
#pragma ident	"@(#)ntp_control.c	1.1	96/11/01 SMI"
	...
	struct peer *peer;
{
	...
	if (!(err & PEER_EVENT)) {
		...
		if (ctl_sys_last_event != (u_char)err) {
			NLOG(NLOG_SYSEVENT)
			  syslog(LOG_INFO, "system event '%s' (0x%02x) status '%s' (0x%02x)",
				 eventstr(err), err,
		...
		if (peer->num_events < CTL_PEER_MAXEVENTS)
			peer->num_events++;
		NLOG(NLOG_PEEREVENT)
		  syslog(LOG_INFO, "peer %s event '%s' (0x%02x) status '%s' (0x%02x)",
		...
		syslog(LOG_ERR, "report_event: err '%s' (0x%02x), no peer", eventstr(err), err);
#ifdef DEBUG

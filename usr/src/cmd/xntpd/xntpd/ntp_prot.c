...
#pragma ident	"@(#)ntp_proto.c	1.1	96/11/01 SMI"
	...
	int trustable;			/* used as "valid header" */
{
	...
	switch (dwWaitResult) {
    	...
    	default:
			...
			syslog(LOG_ERR, "receive error with mutex: %m\n");
			exit(1);
	}
#endif /* SYS_WINNT */
    ...
    if (!ReleaseMutex(hMutex)) {
		syslog(LOG_ERR, "receive cannot release mutex: %m\n");
		exit(1);
    }
#endif /* SYS_WINNT */
...
clear_all()
{
	...
	if (sys_peer)
	  NLOG(NLOG_SYNCSTATUS)
	    syslog(LOG_INFO, "synchronisation lost");
...
clock_select()
{
	...
	if (osys_peer != sys_peer) {
		...
		report_event(EVNT_PEERSTCHG, (struct peer *)0);
		NLOG(NLOG_SYNCSTATUS)
		  syslog(LOG_INFO, "synchronized to %s, stratum=%d", src,
			 sys_peer->stratum);
	}
}

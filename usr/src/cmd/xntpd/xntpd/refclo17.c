...
#pragma ident	"@(#)refclock_omega.c	1.2	96/11/12 SMI"
...
struct omegaunit {
	...
	u_int32 coderecv;		/* timecodes received */
	u_char lencode;			/* length of last timecode */
	u_int32 lasttime;		/* last time clock heard from */
	 ...
	 * Status tallies
 	 */
	u_int32 polls;			/* polls sent */
	u_int32 noreply;			/* no replies to polls */
	u_int32 badformat;		/* bad format */
	u_int32 baddata;			/* bad data */
	u_int32 timestarted;		/* time we started this */
};
	...
	int code;
{
	...
	if (omega->status != (u_char)code) {
		...
		NLOG(NLOG_CLOCKEVENT) /* conditional if clause for conditional syslog */
		  syslog(LOG_INFO,
		    "omega clock %s event '%s' (0x%02x)\n", ntoa(&peer->srcadr), ceventstr(code), code);
	}
}
...
{
	register int i, n;
	register u_int32 med_ui, med_uf, tmp_ui, tmp_uf;
	l_fp off[NSTAMPS];
	u_fp disp;
	...
	register struct refclockbug *bug;
{
	...
	if (omega->lasttime != 0)
		...
		bug->values[0] = 0;
	bug->values[1] = (u_int32)omega->reason;
	bug->values[2] = (u_int32)omega->year;
	bug->values[3] = (u_int32)omega->day;
	bug->values[4] = (u_int32)omega->hour;
	bug->values[5] = (u_int32)omega->minute;
	bug->values[6] = (u_int32)omega->second;
	bug->values[7] = (u_int32)omega->msec;

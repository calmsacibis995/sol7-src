...
#pragma ident	"@(#)ntp_refclock.c	1.3	97/03/21 SMI"
...
#ifdef PPS
extern	int	pps_enable;	/* pps enabled indicator */
#endif PPS
	...
	int flags;		/* line discipline flags */
{
	...
	if (flags & LDISC_PPS) {
...
#ifdef SYS_SOLARIS
		if (ioctl(fd, TIOCSPPS, (char *)&pps_enable) < 0) {
#else
	    ...
	    "refclock_ioctl: optional ppsclock streams module unavailable");
			...
			fdpps = fd;
			syslog(LOG_NOTICE,
	    "refclock_ioctl: kernel pps mods enabled %x", pps_enable);
		}
	}
#else
	if (flags & LDISC_PPS)

...
#pragma ident	"@(#)ntpdate.c	1.6	97/08/12 SMI"
	...
	char *argv[];
{
	...
	extern char *Version;
#else
	char *Version = "version=<UNDEFINED - MUST FIX NT NMAKEFILE>";
	 ...
	 */
	while ((c = ntp_getopt(argc, argv, "a:bde:k:o:p:qst:vumw")) != EOF)
		switch (c) {
		case 'a':
			c = atoi(ntp_optarg);
			sys_authenticate = 1;
		...
		case 'e':
			if (!atolfp(ntp_optarg, &tmp)
			    || tmp.l_ui != 0) {
				(void) fprintf(stderr,
			    gettext("%s: encryption delay %s is unlikely\n"),
				    progname, ntp_optarg);
				errflg++;
			} else {
		...
		case 'p':
			...
			if (c <= 0 || c > NTP_SHIFT) {
				(void) fprintf(stderr,
			gettext("%s: number of samples (%d) is invalid\n"),
				    progname, c);
				errflg++;
			} else {
				sys_samples = c;
		...
		case 't':
			if (!atolfp(ntp_optarg, &tmp)) {
				(void) fprintf(stderr,
				gettext("%s: timeout %s is undecodeable\n"),
				    progname, ntp_optarg);
				errflg++;
			} else {
...
"usage: %s [-bdmqsvw] [-a key#] [-e delay] [-k file] [-p samples] [-o version#] [-t timeo] server ...\n"),
		    progname);
		exit(2);
	}
...
#define	LOG_NTP	LOG_DAEMON
	...
	if (sys_numservers == 0) {
		syslog(LOG_ERR, gettext("no servers can be used, exiting"));
		exit(1);
	}

	if (multicast && sys_numservers > 1) {
		syslog(LOG_ERR, gettext("only 1 multicast addr can be used, exiting"));
		exit(1);
	}
	...
	if (sys_authenticate) {
		...
		if (!authreadkeys(key_file)) {
			syslog(LOG_ERR, gettext("no key file, exitting"));
			exit(1);
		}
		if (!authhavekey(sys_authkey)) {
			...
			syslog(LOG_ERR, 
			    gettext("authentication key %s unknown"), buf);
			exit(1);
		}
	}
	init_io();
	...
	if (!SetPriorityClass(process_handle, (DWORD) REALTIME_PRIORITY_CLASS)) {
		syslog(LOG_ERR, gettext("SetPriorityClass failed: %m"));
	}
#endif /* SYS_WINNT */
	...
	for (;;) {
		collect();
		/*
		 * We'ave collect()ed all the data we want, so try to adjust the clock.
		 */
		errflg = clock_adjust();
		if (errflg && multicast) {
			/*
			 * Either no server answered our multicast broadcast
			 * request or none was suitable, so last we try to
			 * join the multicast group with incremental ttl's
			 * until either we receive a broadcast from a server
			 * we can use or we have reached a max ttl and give up.
			 */
			int ttl;
			struct ip_mreq mreq;

syslog(LOG_NOTICE,
    gettext("no server suitable for synchronization found yet")
);
...
syslog(LOG_NOTICE,
    gettext("trying ttl %d for multicast server synchronization"),
    ttl
);
...
syslog(LOG_NOTICE,
    gettext("no multicast server suitable for synchronization found")
);
		...
		if (! wait_secs)
			break;
		/*
		 * Wait option specified, so we keep trying until we synch up
		 * but first we go to sleep awhile before trying again. Use a
		 * an exponential backed off value.
		 */
syslog(LOG_NOTICE,
    gettext("waiting %d seconds before trying again"),
    wait_secs
);
		sleep(wait_secs);
	...
	if (errflg)
		syslog(LOG_ERR,
		    gettext("no server suitable for synchronization found"));
	exit(errflg);
}
	...
	while (complete_servers < sys_numservers || wait_time > current_time) {
...
#endif /* SYS_WINNT */
                                syslog(LOG_ERR, gettext("select() error: %m"));
                        }
                        if (alarm_flag) {               /* alarmed? */
                                was_alarmed = 1;
                                alarm_flag = 0;
		...
		if (was_alarmed) {
			if (multicast == MC_BROADCAST) {
				/*
				 * First alarm after a sendto() to the
				 * multicast addr, so from now on only
				 * deal with those servers we'ave herd
				 * from (if any).
				 */
				multicast = MC_SENDTO;
			}
	...
	register struct server *server;
{
	...
	if (sys_authenticate) {
		...
		if (debug > 1)
			printf(gettext("transmit auth to %s\n"),
			    ntoa(&(server->srcadr)));
	} else {
		get_systime(&(server->xmt));
		HTONL_FP(&server->xmt, &xpkt.xmt);
	...
	struct recvbuf *rbufp;
{
	...
	if (rbufp->recv_length == LEN_PKT_NOMAC)
		...
		if (debug)
			printf(gettext("receive: packet length %d\n"),
			    rbufp->recv_length);
	...
	if ((PKT_MODE(rpkt->li_vn_mode) != MODE_SERVER
		...
		if (debug)
			printf(gettext("receive: mode %d stratum %d\n"),
	...
	if (server == NULL) {
		if (debug)
			printf(gettext("receive: server not found\n"));
		return;
	}
	...
	if (!sys_authenticate)
		...
		if (has_mac && ntohl(rpkt->keyid) == sys_authkey &&
		    authdecrypt(sys_authkey, (u_int32 *)rpkt, LEN_PKT_NOMAC))
				is_authentic = 1;
		if (debug)
		    printf(gettext("receive: authentication %s\n"),
			is_authentic ? gettext("passed") : gettext("failed"));
	}
	server->trust <<= 1;
	if (!is_authentic)
		server->trust |= 1;
	...
	if (debug > 3)
		printf(gettext("offset: %s, delay %s\n"), lfptoa(&ci, 6),
		    fptoa(di, 5));
...
clock_adjust()
{
	...
	if (dostep) {
		if (simple_query || l_step_systime(&server->offset)) {
			syslog(LOG_NOTICE,
			    gettext("step time server %s offset %s sec"),
			    ntoa(&server->srcadr),
		...
		if (simple_query || l_adj_systime(&server->offset)) {
			syslog(LOG_NOTICE,
			    gettext("adjust time server %s offset %s sec"),
			    ntoa(&server->srcadr),
			    lfptoa(&server->offset, 6));
		}
#else
	 ...
	 */
		printf("\nThe -b option is required by ntpdate for Win/NT platfrom\n");
		exit(1);
#endif /* SYS_WINNT */
	}
	return(0);
	...
	char *serv;
{
	...
	if (sys_numservers >= sys_maxservers) {
		if (!toomany) {
			...
			toomany = 1;
			syslog(LOG_ERR,
	gettext("too many servers (> %d) specified, remainder not used"),
			    sys_maxservers);
		}
		return;
	}
	...
	if (!getnetnum(serv, &netnum)) {
		syslog(LOG_ERR, gettext("can't find host %s\n"), serv);
		return;
	}
	...
	struct sockaddr_in *addr;
{
	...
	if (multicast == MC_ENABLED) {
		 ...
		 * multicast state and mark the multicast addr as complet
...
init_alarm()
{
	 ...
	 */
   	...
   	if (!OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &hToken)) {
        	syslog(LOG_ERR, "OpenProcessToken failed: %m");
		exit(1);
	}
    	...
    	if (GetLastError() != ERROR_SUCCESS)
        	syslog(LOG_ERR, "AdjustTokenPrivileges failed: %m");
	...
	if(timeGetDevCaps(&tc, sizeof(TIMECAPS)) != TIMERR_NOERROR) {
    		syslog(LOG_ERR, "timeGetDevCaps failed: %m");
		exit(1);
	}
	...
	wTimerID = timeSetEvent(
    	...
    	if (wTimerID == 0) {
        	syslog(LOG_ERR, "timeSetEvent failed: %m");
		exit(1);
	}
#endif /* SYS_WINNT */
}
    ...
    DWORD dwUser, DWORD dw1, DWORD dw2)
{
	...
	if (debug > 3) {
 		GetSystemTime(&st);
		printf(
	gettext("thread %u (timer callback): time %02u:%02u:%02u:%03u\n"),
		    threadID, st.wHour, st.wMinute, st.wSecond,
		    st.wMilliseconds);
		fflush(stdout);
	}
#endif
...
init_io()
{
	...
	if ((fd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
		syslog(LOG_ERR, gettext("socket() failed: %m"));
		exit(1);
		/*NOTREACHED*/
	}
...
#endif /* SYS_WINNT */
				syslog(LOG_ERR,
				gettext("the NTP socket is in use, exiting"));
			else
				syslog(LOG_ERR, gettext("bind() fails: %m"));
			exit(1);
		}
	}
	...
	FD_SET(fd, &fdmask);
        ...
        if (fcntl(fd, F_SETFL, O_NONBLOCK) < 0) {
                syslog(LOG_ERR, gettext("fcntl(FNDELAY|FASYNC) fails: %m"));
                exit(1);
                /*NOTREACHED*/
        }
#else /* O_NONBLOCK */
...
Need non blocking I/O
	...
	if (ioctlsocket(fd,FIONBIO, (u_long *) &on) == SOCKET_ERROR) {
		syslog(LOG_ERR, gettext("ioctlsocket(FIONBIO) fails: %m"));
		exit(1);
	}
#endif /* SYS_WINNT */
}
...
#endif /* SYS_WINNT */
			syslog(LOG_ERR, gettext("sendto(%s): %m"), ntoa(dest));
	}
}
...
if (debug)
	printf(gettext("input_handler: ignore packet (%d).\n"), free_recvbufs);
	...
	l_fp *ts;
{
	...
	if (adjtv.tv_usec != 0 && !debug) {
		if (adjtime(&adjtv, &oadjtv) < 0) {
			syslog(LOG_ERR,
			    gettext("Can't adjust the time of day: %m"));
			return 0;
		}
	}
	return 1;
	...
	FILE *fp;
{
	...
	if (!debug) {
		(void) fprintf(fp,
		    gettext("server %s, stratum %d, offset %s, delay %s\n"),
		    ntoa(&pp->srcadr), pp->stratum,
		    lfptoa(&pp->offset, 6), fptoa(pp->delay, 5));
	    return;
	}

	(void) fprintf(fp, gettext("server %s, port %d\n"),
	    ntoa(&pp->srcadr), ntohs(pp->srcadr.sin_port));

	(void) fprintf(fp,
	    gettext("stratum %d, precision %d, leap %c%c, trust %03o\n"),
	...
	(void) fprintf(fp,
	    gettext("refid [%s], delay %s, dispersion %s\n"),
	    str, fptoa(pp->delay, 5),
	    ufptoa(pp->dispersion, 5));
	
	(void) fprintf(fp, gettext("transmitted %d, in filter %d\n"),
	    pp->xmtcnt, pp->filter_nextpt);

	t = (time_t)pp->reftime.l_i;
	(void) fprintf(fp, gettext("reference time:      %s"),
	    ctime(&t));
	t = (time_t)pp->org.l_i;
	(void) fprintf(fp, gettext("originate timestamp: %s"),
	    ctime(&t));
	t = (time_t)pp->xmt.l_i;
	(void) fprintf(fp, gettext("transmit timestamp:  %s"),
	    ctime(&t));
	
	(void) fprintf(fp, gettext("filter delay: "));
	...
	(void) fprintf(fp, gettext("filter offset:"));
	...
	(void) fprintf(fp, gettext("delay %s, dispersion %s\n"),
	    fptoa(pp->delay, 5), ufptoa(pp->dispersion, 5));

	(void) fprintf(fp, gettext("offset %s\n\n"),
	    lfptoa(&pp->offset, 6));
}

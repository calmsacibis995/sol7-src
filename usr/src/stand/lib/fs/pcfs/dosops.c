...
#pragma ident	"@(#)dosops.c	1.37	97/11/25 SMI"
...
arch_fix_spt(int v, _fdisk_p fp)
{
	...
	if (ihp->unit < 2) {
		 ...
		 * Therefore, we must call read_sectors directory and find
		 * out what the device supports. Once that is done we can
		 * update ihp again.
		 */

		if (read_sectors(ihp, 0, 1) != -1) {

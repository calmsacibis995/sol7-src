#pragma ident	"@(#)inet.c	1.22	97/04/29 SMI"
...
inet_init(bootdev_t *bd, struct sainet *sain, char *tmpbuf)
{
	...
	bzero(&sain->sain_myaddr, sizeof (struct in_addr));
	bzero(&sain->sain_hisaddr, sizeof (struct in_addr));
	bcopy(etherbroadcastaddr, sain->sain_hisether, sizeof (ether_addr_t));
#ifdef	DEBUG
	...
	struct sainet *sain, caddr_t tmpbuf)
{
	...
	if (bcmp(&ip->ip_dst, &sain->sain_hisaddr,
	    sizeof (struct in_addr)) != 0) {
		bcopy(&ip->ip_dst, &sain->sain_hisaddr,
		    sizeof (struct in_addr));
		arp(bd, sain, tmpbuf);
	}
	eh->ether_type = htons(ETHERTYPE_IP);
	bcopy(sain->sain_myether, &eh->ether_shost, sizeof (ether_addr_t));
	bcopy(sain->sain_hisether, &eh->ether_dhost, sizeof (ether_addr_t));
	/* checksum the packet */
	ip->ip_sum = 0;
...
ip_input(bootdev_t *bd, caddr_t buf, struct sainet *sain)
{
	...
	if (eh->ether_type == ntohs(ETHERTYPE_IP) &&
		...
		if ((sain->sain_hisaddr.s_addr & 0xFF000000) == 0 &&
		    bcmp(&etherbroadcastaddr, &eh->ether_dhost,
...
#endif
		if (bcmp(&ip->ip_dst, &sain->sain_myaddr,
		    sizeof (struct in_addr)) != 0)
			return (0);
		return (len);
	}
	if (eh->ether_type == ntohs(ETHERTYPE_ARP) &&
		...
		if (bcmp(ea->arp_spa, &sain->sain_hisaddr,
		    sizeof (struct in_addr)) == 0) {
			bcopy(ea->arp_sha, sain->sain_hisether,
			    sizeof (ether_addr_t));
		}
		if (ea->arp_op == ntohs(ARPOP_REQUEST) &&
		    (bcmp(ea->arp_tpa, &sain->sain_myaddr,
		    sizeof (struct in_addr)) == 0)) {
			ea->arp_op = htons(ARPOP_REPLY);
			bcopy(ea->arp_sha, &eh->ether_dhost,
			    sizeof (ether_addr_t));
			bcopy(sain->sain_myether, &eh->ether_shost,
			    sizeof (ether_addr_t));
			bcopy(ea->arp_sha, ea->arp_tha, sizeof (ether_addr_t));
			bcopy(ea->arp_spa, ea->arp_tpa, sizeof (ea->arp_tpa));
			bcopy(sain->sain_myether, ea->arp_sha,
			    sizeof (ether_addr_t));
			bcopy(&sain->sain_myaddr, ea->arp_spa,
			    sizeof (ea->arp_spa));
#ifdef sparc
			if (prom_getversion() < 0) {
				(*bd->sa.si_sif->sif_xmit)(bd->sa.si_devdata,
				    buf, sizeof (struct arp_packet));
			} else {
#endif
...
#endif /* !NOREVARP */
		bcopy(etherbroadcastaddr, sain->sain_hisether,
		    sizeof (ether_addr_t));
		return;
	}
	out.arp_eh.ether_type = htons(ETHERTYPE_ARP);
	out.arp_ea.arp_op = htons(ARPOP_REQUEST);
	bcopy(etherbroadcastaddr, out.arp_ea.arp_tha, sizeof (ether_addr_t));
	bcopy(&sain->sain_hisaddr, out.arp_ea.arp_tpa,
...
#ifdef NOREVARP
	bzero(&sain->sain_myaddr, sizeof (struct in_addr));
	bcopy(&sain->sain_myether.ether_addr_octet[3],
	    (&sain->sain_myaddr)+1, 3);
#else
	out.arp_eh.ether_type = htons(ETHERTYPE_REVARP);
	out.arp_ea.arp_op = htons(REVARP_REQUEST);
	bcopy(sain->sain_myether, out.arp_ea.arp_tha, sizeof (ether_addr_t));
...
{
	struct arp_packet *in = (struct arp_packet *)tmpbuf;
	int e, count, time, feedback, len, delay = 2;
	char    *ind = "-\\|/";
	struct in_addr tmp_ia;

	bcopy(etherbroadcastaddr, &out->arp_eh.ether_dhost,
	    sizeof (ether_addr_t));
	bcopy(sain->sain_myether, &out->arp_eh.ether_shost,
	    sizeof (ether_addr_t));
	...
	bcopy(sain->sain_myether, out->arp_ea.arp_sha,
	    sizeof (ether_addr_t));
	bcopy(&sain->sain_myaddr, out->arp_ea.arp_spa,
	...
	for (count = 0; /* no conditional */; count++) {
		if (count == WAITCNT) {
			if (out->arp_ea.arp_op == ARPOP_REQUEST) {
				...
				bcopy(out->arp_ea.arp_tpa, &tmp_ia,
				    sizeof (tmp_ia));
				inet_print(tmp_ia);
			} else {
		...
		while (prom_gettime() <= time) {
			...
			if (out->arp_ea.arp_op == ntohs(ARPOP_REQUEST)) {
				...
				bcopy(in->arp_ea.arp_sha, sain->sain_hisether,
				    sizeof (ether_addr_t));
				return;
			} else {		/* Reverse ARP */
				if (in->arp_eh.ether_type !=
				...
				if (count >= WAITCNT) {
					...
					bcopy(in->arp_ea.arp_tpa, &tmp_ia,
					    sizeof (tmp_ia));
					inet_print(tmp_ia);
				}
				bcopy(in->arp_ea.arp_tpa, &sain->sain_myaddr,
				    sizeof (sain->sain_myaddr));
				/*
				 * short circuit first ARP
				 */
				bcopy(in->arp_ea.arp_spa, &sain->sain_hisaddr,
...
ipcksum(caddr_t cp, unsigned short count)
{
	...
	unsigned int	sum = 0;
	unsigned int	oneword = 0x00010000;

...
/* both of these padded to allow for 32-bit alignment */
static char rpc_xmit_buf[MAX_PKT_SIZE + sizeof (uint32_t)];
static char rpc_rcv_buf[NFSBUF_SIZE + sizeof (uint32_t)];
	...
	u_int		auth)		/* type of authentication wanted. */
{
	...
	extern u_short pmap_getport();	/* get entry from port list */
	...
	trm_msg = (caddr_t)IALIGN(&rpc_xmit_buf[0]);	/* alignment hack */
	rcv_msg = (caddr_t)IALIGN(&rpc_rcv_buf[0]);	/* alignment hack */
	xmit_auth = (AUTH *)0;
...
call_again:
	...
	} while (ntohl(*((uint32_t *)(rcv_msg))) != xid);
	...
	rpcproc_t proc)
{
	...
	call_msg.rm_call.cb_prog = (rpcprog_t)prog;
	call_msg.rm_call.cb_vers = (rpcvers_t)vers;

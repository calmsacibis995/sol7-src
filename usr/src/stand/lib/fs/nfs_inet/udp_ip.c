...
#pragma ident	"@(#)udp_ip.c	1.36	97/04/29 SMI"
...
xmit(
	...
	int ckie_size,		/* size of outgoing "cookie" */
	caddr_t rcv_addr,	/* address to put incoming packet in. */
	int *rcv_size,		/* var to hold rcv data size */
	...
	struct sainet *net)	/* network addresses */
{
	...
	int			xdelay;		/* delay between retrys */
	int			xtime;		/* xmit time */
	int			pktlen;		/* len of incoming packet */
	struct udpip_pkt	*o_p; 		/* outgoing pkt */
	struct udpip_pkt	*i_p; 		/* incoming pkt */
	struct sainet		*netaddr;	/* our addresses */
	int			status;		/* return status */
	u_short	ip_id;		/* current ip pkt id */
	u_short	datalen;	/* size of UDP pkt */
	int			ip_hlen;	/* ip header length */
	short			cur_off;	/* ip fragment offset */
	caddr_t	curr_rcv;	/* current rcv buf addr */
	caddr_t	data_start;	/* start of data in pkt */
...
fragments.\n");
	...
	if ((status == RPC_SUCCESS) && (net != (struct sainet *)0)) {
		...
		(void) bcopy(&i_p->e_h.ether_shost.ether_addr_octet,
		    &net->sain_hisether, sizeof (ether_addr_t));
	}
done:
...
udp_checksum(caddr_t	addr)	/* start of data */
{
	...
	cnt = ntohs(ck.len) + sizeof (struct pseudo_udp) -
		sizeof (struct udphdr);
	end_hdr = (u_int)sp + (u_int)sizeof (struct pseudo_udp);
...
{
	int i;
	u_short	total = 0;	/* culmulative lengths */
	extern struct ip_frag	fragment[];

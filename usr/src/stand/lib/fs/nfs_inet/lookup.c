...
#pragma ident	"@(#)lookup.c	1.24	97/04/29 SMI"
...
extern enum clnt_stat rpc_call(rpcprog_t prog, rpcvers_t vers, rpcproc_t proc,
...
next:
	...
	if (((cino = get_dcache(bootd.handle, component, dino)) != NULL) &&
		...
		status = rpc_call((rpcprog_t)NFS_PROGRAM,
...
getsymlink(struct nfs_fh *fh, struct pathname *pnp)
{
	...
	status = rpc_call((rpcprog_t)NFS_PROGRAM, (rpcvers_t)NFS_VERSION,

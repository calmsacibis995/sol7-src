#pragma ident	"@(#)netaddr.c	1.22	97/04/29 SMI"
...
int	ether_arp[(MAX_PKT_SIZE + sizeof (int) - 1) / sizeof (int)];
	...
	enum netaddr_type type)	/* which one */
{
	...
	if (type ==  DESTIN)
		bcopy(ea, &sain_s.sain_hisether, sizeof (ether_addr_t));
	else	/* SOURCE */
		bcopy(ea, &sain_s.sain_myether, sizeof (ether_addr_t));
}
...
init_netaddr(struct sainet *s)
{
	...
	(void) bcopy(&s->sain_myether, &sain_s.sain_myether,
	    sizeof (ether_addr_t));
	(void) bcopy(&s->sain_hisether, &sain_s.sain_hisether,
	    sizeof (ether_addr_t));
}
...
get_arp(struct sainet *s)
{
	...
	extern int 	ether_arp[];	/* defined here */
	extern ether_addr_t etherbroadcastaddr; /* defined in inet.c */
	struct arp_packet out;		/* buffer for outgoing */
	struct arp_packet *in;		/* incoming buffer */
	int		count;		/* number of tries so far */
	int		bytes;		/* number of bytes xmit */
	u_int		time;		/* next transmit time */
	int		feedback;	/* progress counter */
	char    	*ind;		/* progress indicator */
	int		len;		/* response size */
	int		delay;		/* tranmit delay */

	/* if it's an ip broadcast, then it's simple */
	if (in_broadaddr(s->sain_hisaddr)) {
		bcopy(&etherbroadcastaddr, &s->sain_hisether,
		    sizeof (ether_addr_t));
		return (1);
	}
	...
	bcopy(&s->sain_hisaddr, &out.arp_ea.arp_tpa,
	    sizeof (out.arp_ea.arp_tpa));
	bcopy(&etherbroadcastaddr, &out.arp_eh.ether_dhost.ether_addr_octet,
		sizeof (ether_addr_t));
	bcopy(&s->sain_myether, &out.arp_eh.ether_shost.ether_addr_octet,
	    sizeof (ether_addr_t));
	bcopy(&s->sain_myether, &out.arp_ea.arp_sha, sizeof (ether_addr_t));
	bcopy(&s->sain_myaddr, &out.arp_ea.arp_spa,
	...
	for (count = 0, feedback = 0; count <= ARP_TRIES; count++) {
		...
		while (prom_gettime() <= time) {
			...
			if (in->arp_ea.arp_op != ntohs(ARPOP_REPLY))
				continue;
			if (bcmp(in->arp_ea.arp_spa, &out.arp_ea.arp_tpa,
			...
			bcopy(in->arp_ea.arp_sha, s->sain_hisether,
			    sizeof (ether_addr_t));
			return (1); /* got it */
		}

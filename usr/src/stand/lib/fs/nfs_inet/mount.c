#pragma ident	"@(#)mount.c	1.23	97/04/29 SMI"
...
device.\n");
	...
	/* initialization */
	bzero(&root_hostname, SYS_NMLN);
	bzero(&server_path, SYS_NMLN);
	bzero(&root_tmp, sizeof (root_tmp));
	...
	resp_wait = 16;
	do {
		status = rpc_call((rpcprog_t)MOUNTPROG, (rpcvers_t)MOUNTVERS,

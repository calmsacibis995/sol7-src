...
#ident "@(#)map_prog.c	1.11	98/01/29 SMI"
...
static paddr_t map_mem_big_un(paddr_t, u_int);
...
static paddr_t
map_mem_big_un(paddr_t phys, u_int virt)
{
	uint virtaddr_end = virt + FOURMB_PAGESIZE;
	uint physaddr_sav;
	uint pdir = MMU_L1_INDEX(virt);
#if (defined(BOOT_DEBUG))
	uint size_sav;
#endif
	struct memlist m, *mp;
	extern uint	bpd_loc;
	uint * pdp, pdph = bpd_loc;

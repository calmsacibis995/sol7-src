...
#pragma ident	"@(#)ls.c	1.15	97/02/21 SMI"
...
/*
 * Copyright (c) 1984,1985,1986,1987,1988,1989,1996,1997, by Sun Microsystems, Inc.
 * All Rights Reserved.
 */
...
gstat(struct afile *fp, char *file, int statarg, off_t *pnb)
{
	...
	*fp = azerofile;
	...
	if (statarg || sflg || lflg || tflg) {
		...
		if (acl(file, GETACLCNT, 0, NULL) > MIN_ACL_ENTRIES)
			fp->acl = '+';
		else
			fp->acl = ' ';

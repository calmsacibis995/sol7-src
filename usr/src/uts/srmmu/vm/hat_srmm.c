...
#pragma ident	"@(#)hat_srmmu.c	1.226	98/02/04 SMI"
...
static struct kmem_cache *srmmu_kmcache;
static struct kmem_cache *srpt_kmcache;
static struct kmem_cache *srptbl_kmcache;
...
const char *srmmu_version = "02/04/98 1.226";
...
srmmu_init(void)
{
	 ...
	 *
	 * We have 3 pools:
	 *	srmmu_kmcache - for srmmu structures.
	 *	srptbl_kmcache - for ptbl_gr_t's
	 *	srpt_kmcache - for the pt's
	 */
	srmmu_kmcache = kmem_cache_create("srmmu_kmcache",
	    sizeof (srmmu_t), sizeof (u_int), NULL, NULL, NULL, NULL, NULL, 0);
	...
	u_int len;
{
	...
	while (len) {
		...
		if (len >= MMU_L1_SIZE && MMU_L1_OFF(addr) == 0 &&
			...
			pp = page_nextn(pp, mmu_btop(MMU_L2_SIZE));
			addr += MMU_L2_SIZE;
			len -= MMU_L2_SIZE;
...
/*
 * This routine is for sunmmu machines that can only look at
 * the ptes of the current ctx.  srmmu tables are memory based,
 * so we don't need to do anything here.
 */
/*ARGSUSED*/
static struct as *
srmmu_setup(as, allocflag)
	register struct as *as;
	int	allocflag;	/* XXX not used */
{
	return (NULL);
}
	...
	int flags;
{
	...
	do {
		...
		if (as != &kas) {
			...
			if (((u_int) addr < SYSBASE) || (addr >= Syslimit)) {
				flag = 0;
			} else {
				flag = NO_ALLOC;
			}
...
srmmu_srmmu_alloc()
{
	...
	if (srmmu == NULL) {
		 ...
		 */
		srmmu = kmem_cache_alloc(srmmu_kmcache, KM_SLEEP);
		if (static_ctx) {
...
srmmu_create_pts(caddr_t start, int level, int sleep, int keep)
{
	...
	ptbl_gr = kmem_cache_alloc(srptbl_kmcache, sleep);
	if (ptbl_gr == NULL) {
		return (NULL);
	}
...
ialloc_ptbl()
{
	...
	srptbl_kmcache = kmem_cache_create("ptbl_kmcache",
	    sizeof (ptbl_gr_t), sizeof (u_int), NULL, NULL, NULL,
	    NULL, NULL, 0);

	srpt_kmcache = kmem_cache_create("pt_kmcache",
	    MMU_PAGESIZE, MMU_PAGESIZE, NULL, NULL, NULL, NULL, NULL, 0);
...
srmmu_ptbl_create(u_int sleep)
{
	...
	ptg = (caddr_t)kmem_cache_alloc(srpt_kmcache, sleep);
	if (ptg == NULL) {
		return (NULL);
	}
	ptbl = srmmu_create_pts(ptg, PTBL_LEVEL_3, sleep, 1);
	if (ptbl == NULL) {
		kmem_cache_free(srpt_kmcache, ptg);
	}
	return (ptbl);
}
...
srmmu_steal_this_ptbl(ptbl_t *ptbl, kmutex_t *p_mtx)
{
	...
	if (PTBL_CAN_STEAL(ptbl->ptbl_flags, ptbl->ptbl_lockcnt) &&
	    ...
	    == LK_PTBL_OK)) {
		/*
		 * Both the L3 and L2 are locked,
		 * unlink the L3 from the L2
		 * and then unload the L3.
		 */
		...
		vcnt = srmmu_unload_loop(ptbl, 0, NL3PTEPERPT, SR_NOFLUSH, rpp);
		ptbl->ptbl_flags = 0;
		mutex_exit(p_mtx);
...
srmmu_l1_ptbl_alloc()
{
	...
	if (ptbl == NULL) {
		ptg = (caddr_t)kmem_cache_alloc(srpt_kmcache, KM_SLEEP);
...
get_hment()
{
	...
	if (hme == NULL) {
		...
		hme = kmem_perm_alloc(sizeof (srhment_t), 0, KM_SLEEP);
	}

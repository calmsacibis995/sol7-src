...
#pragma ident	"@(#)overflow.s	1.34	98/02/01 SMI"
 ...
 *	%g4 = scratch
 */
	ENTRY_NP(srmmu_window_overflow)
	mov	%psr, %l0

	ALTENTRY(_window_overflow)
...
9:
	...
	ld	[%g3 + CPU_MPCB], %g3	! get thread pointer
	tst	%g3			! lwp == 0 for kernel threads
...
.wo_super:
	...
	st	%l4, [%g3 + MPCB_UWM]	! update mpcb->mpcb_uwm
	!
...
.wo_shared_window:
	...
	!
	ld	[%g3 + MPCB_REGS + O6*4], %g4	! user's saved stack pointer
	set     KERNELBASE, %g1
	!
	! In order to save the window onto the stack, the stack
	! must be aligned on a word boundary, and the part of the
...
.wo_save_to_buf:
	...
	ld	[%g3 + MPCB_WBCNT], %g1	! mpcb_wbcnt*4 is offset into mpcb_spbuf
	add	%g1, 1, %g2		! increment mpcb_wbcnt to reflect save
	st	%g2, [%g3 + MPCB_WBCNT]

...
#pragma ident	"@(#)locore.s	1.142	98/02/05 SMI"
...
{}
	...
	ENTRY_NP(_start)
	/ We jump to __start to make sure the  function enable_big_page_support
	/ does not span multiple 4K pages. This is  essential, since we need
	/ to turn paging off in enable_big_page_support
	jmp	__start
...
enable_big_page_support_done:
	...
	/ It loads the argument that it received in %ebx in to cr3
	/ The new cr3 should point to page directory pointer table.
	/	
	.globl	enable_pae
enable_pae:
	movl	%cr0, %eax
	...
	movl	%eax, %cr0	/ disable paging
	movl	%cr4, %eax
	orl	$CR4_PAE, %eax	/ enable physical address extension
	movl	%eax, %cr4	
	movl	%ebx, %cr3	/ load pointer table
	movl	%cr0, %eax
	orl	$CR0_PG, %eax	/ enable paging
	movl	%eax, %cr0
	jmp	enable_pae_done	
...
.set PAGESIZE, 0x1000
	...
	popl	%ecx
	movl	%ecx, %eax
	xorl	$EFL_ID, %eax		/ try complemented bit
	pushl	%eax
	popfl
	pushfl
	popl    %eax
...
p5_mmx_not_supported:
	testl	$P6_MCE_SUPPORTED, %edx
	je	p6_mce_not_supported
	testl	$P6_MCA_SUPPORTED, %edx
	je	p6_mce_not_supported
	orl	$X86_MCA, x86_feature
p6_mce_not_supported:
	testl	$P6_PAE_SUPPORTED, %edx
	je	p6_pae_not_supported
	orl	$X86_PAE, x86_feature
p6_pae_not_supported:
	testl	$P6_CXS_SUPPORTED, %edx
	je	p6_cxs_not_supported
	orl	$X86_CXS, x86_feature
p6_cxs_not_supported:
	...
	orl	$CR4_DE, cr4_value	/ enable hardware I/O breakpoints
	cmpl	$5, cpu_family
	jne	maybe_p6
	leal	pentium_tm, %eax
...
maybe_p6:
	...
	movl	$1, pentiumpro_bug4046376
	movl	$1, pentiumpro_bug4064495
cpu_unknown:
	testl	$X86_LARGEPAGE, x86_feature
	je	cpu_done	
	movl	$CR4_PSE, %eax
...
enable_large_page_support:
	orl	%eax, cr4_value
	pushl	%eax
	pushl	$enable_big_page_support
	call	setup_121_andcall
	addl	$8, %esp
	jmp	cpu_done
...
#define NOP 0x90
	...
	/ Now that our CPU structure is set up, we can patch _mcount
	/ to go live. 
	movb	$NOP, _mcount			/ Hot patch _mcount
#endif
...
/begin inline syscall_entry
	...
	addl	$1, LWP_RU_SYSC(%esi)
	adcl	$0, LWP_RU_SYSC+4(%esi)
#if	NORMALRETURN != 0
...
/*ARGSUSED*/
longlong_t
loadable_syscall(long a0, long a1, long a2, long a3, long a4, long a5)
{
	return (0);
}
...
set_sup_regs:
	...
	movl	REGS_PC(%esp), %edx
	subl	$mutex_exit, %edx
	cmpl	$mutex_exit_critical_size, %edx
	jae	.mutex_exit_check_done
	movl	$mutex_exit, REGS_PC(%esp)	/ restart mutex_exit()
...
sr_sup_preempt:
	movl	%gs:CPU_THREAD, %ebx
	cmpb	$0, T_PREEMPT_LK(%ebx)	/ Is this thread already in kpreempt?
...
/*ARGSUSED*/
void
setup_idt_base(caddr_t new_idt_addr) {}

#else	/* lint */
	ENTRY(setup_idt_base)
	movl 4(%esp), %eax
	movl %eax, KIDTptr+2
	lidt	KIDTptr
	ret
	SET_SIZE(setup_idt_base)
#endif	/* lint */
...
/*ARGSUSED*/
int
find_cpufrequency(ulong_t *pit_counter, ulong_t *processor_clks)
{
	return (0);
}
...
not_a_p5_or_p6:
	...
	outb	(%dx)
	outb	(%dx)
me_as486: decl	%ecx		/ 1 clock
	nop			/ 1 clock
	jnz	me_as486	/ 3 clock
	nop
	xorl	%eax, %eax
	movl	$PITCTR0_PORT, %edx
	inb	(%dx)
	...
	/ This function can not be invoked with PAE enabled, this could
	/ be used to enable PAE
	/
	.globl	setup_121_andcall
setup_121_andcall:
	pushl	%ebp
	movl	%esp, %ebp
	pushl	%ebx
...
_i486_tm:	  .string	"i486"
	...
	.globl	x86_feature
	.globl	cr4_value
	.globl	pentiumpro_bug4046376
	.globl	pentiumpro_bug4064495
cr4_value:	  .long 0
cpu_namestrp:	  .long	0
x86_feature:		  .long 0
pentiumpro_bug4046376: .long 0
pentiumpro_bug4064495: .long 0

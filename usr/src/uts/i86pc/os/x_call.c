...
#pragma ident	"@(#)x_call.c	1.28	98/01/09 SMI"
...
xc_capture_cpus(cpuset_t set)
{
	 ...
	 */
	iptr = (u_int *)&xc_mboxes[X_CALL_MEDPRI].arg2;
	if (*iptr & CPU->cpu_mask) {
		...
		if ((int)addr != -1)
			...
			mmu_tlbflush_all();
		atomic_andl((u_long *)iptr, ~CPU->cpu_mask);
	}
	while (*iptr & cpu_online_set) {
	}
...
u_int	tlb_flush_gen;
u_int
xc_broadcast_tlbflush(cpuset_t set, caddr_t addr)
{
	register int cix;
	register struct cpu *cpup;
	int	i;
	...
	set = xc_mboxes[X_CALL_MEDPRI].arg2 =  (set & cpu_online_set)
			& ~CPU->cpu_mask;
	tlb_flush_gen += 2;
	gen = tlb_flush_gen;
	i = 0;
...
xc_waitfor_tlbflush(u_int gen)
{
	...
	if (tlb_flush_gen != gen)
		return;
	iptr = (u_int *)&xc_mboxes[X_CALL_MEDPRI].arg2;
	while ((tlb_flush_gen == gen) && (*iptr & cpu_online_set));
	return;

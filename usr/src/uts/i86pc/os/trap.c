...
#pragma ident	"@(#)trap.c	1.82	98/02/12 SMI"
...
static int
die(unsigned type, struct regs *rp, caddr_t addr, processorid_t cpuid)
{
	char *trap_name;
	char trap_name_buf[64];

	panic_reg = (int)rp;
	type &= ~USER;
	trap_name = sprintf(trap_name_buf, "%s on cpu %d",
	    type < TRAP_TYPES ? trap_type[type] : "trap", cpuid);
 ...
 */
static int
instr_is_syscall(proc_t *p, caddr_t pc)
{
	...
	if (default_copyin(pc, (caddr_t)instr, LCALLSIZE) == 0 &&
	    instr[0] == 0x9a &&
	    instr[1] == 0 &&
	    instr[2] == 0 &&
	    instr[3] == 0 &&
...
trap(struct regs *rp, caddr_t addr, processorid_t cpuid)
{
	...
	if (type == T_PGFLT) {
		u_int	attr;
		extern int pentiumpro_bug4046376;
		int	priv_violation, acess_violation;
		...
		if ((errcode & PF_ERR_PROT) && (pentiumpro_bug4046376)) {
			 ...
			 */
			if (hat_getattr(curproc->p_as->a_hat, addr, &attr)
			    == -1)
				errcode &= ~PF_ERR_PROT;
			else {
				priv_violation = (errcode & PF_ERR_USER) &&
					!(attr & PROT_USER);
				acess_violation = (errcode & PF_ERR_WRITE) &&
					!(attr & PROT_WRITE);
				if (!priv_violation && !acess_violation)
					goto cleanup;
			}
		}
	}
	...
	case T_PGFLT + USER:	/* user page fault */
		...
		watchpage = (p->p_warea != NULL && pr_is_watchpage(addr, rw));
		/*
		...
		if (watchpage && addr == (caddr_t)rp->r_usp &&
		    rw == S_READ && instr_is_syscall(p, (caddr_t)rp->r_eip)) {
	...
	case T_EXTOVRFLT:	/* extension overrun fault */
		/* check if we took a kernel trap on behalf of user */
		{
			...
			if (rp->r_eip != (int)ndptrap_frstor)
				(void) die(type, rp, addr, cpuid);
			type |= USER;
		}
		/*FALLTHROUGH*/
	...
	case T_EXTERRFLT:	/* extension error fault */
		/* check if we took a kernel trap on behalf of user */
		{
			extern  void ndptrap_frstor(void);
			if (rp->r_eip != (int)ndptrap_frstor)
...
kern_gpfault(register int *rp)
{
	...
	if ((lwp != NULL) && lwp->lwp_gpfault) {
		...
		lwp->lwp_regs = rp;	/* get correct copy of regs in core */
#ifdef C2_AUDIT
...
user_genprotfault(struct regs *rp)
{
	...
	if (*sinst_ptr == INST_RDMSR) {
		(void) rdmsr((u_int)rp->r_ecx, (uint64_t *)eaxedx);
		rp->r_eax = eaxedx[0];
		rp->r_edx = eaxedx[1];
	} else if (*sinst_ptr == INST_WRMSR) {
		eaxedx[1] = rp->r_edx;
		eaxedx[0] = rp->r_eax;
		(void) wrmsr(ecx, (uint64_t *)eaxedx);
	} else
		return (1);

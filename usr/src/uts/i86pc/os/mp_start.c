...
#pragma ident	"@(#)mp_startup.c	1.44	98/02/11 SMI"
...
start_other_cpus(int cprboot)
{
	...
	/*
	 * MP machine do not suffer from a cache inconsistency problem.
	 */
	cacheflsh = 0;
...
mp_startup(void)
{
	...
	cpu_add_active(cp);
	atomic_orl(&cpu_online_set, cp->cpu_mask);
...
mp_cpu_stop(struct cpu *cp)
{
	...
	atomic_andl(&cpu_online_set, ~cp->cpu_mask);
	return (0);
}

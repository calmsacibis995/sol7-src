...
#pragma ident	"@(#)memscrub.c	1.2	98/01/09 SMI"
 ...
 * Global Routines:
 */
int memscrub_add_span(pfn_t pfn, u_int pages);
int memscrub_delete_span(pfn_t pfn, u_int bytes);
caddr_t memscrub_window;

/*
 * Global Data:
 */
extern int kadb_map(caddr_t, uint64_t, uint64_t *);
#define	map_memscrub_window	kadb_map
/*
...
u_int memscrub_min_pages = MEMSCRUB_MIN_PAGES;
...
static int memscrub_verify_span(uint64_t *addrp, u_int *pagesp);
...
memscrub_init()
{
	...
	if ((addr = rmalloc(kernelmap, (long)CLSIZE)) == NULL) {
		return;
	}
	memscrub_window = Sysbase + mmu_ptob(addr);
	/*
	 * copy phys_install to memscrub_memlist
	 */
	for (src = phys_install; src; src = src->next) {
		if (memscrub_add_span((pfn_t)btop(src->address),
				(u_int)btop(src->size))) {
...
memscrubber()
{
	...
	for (;;) {
		 ...
		 */
		    ...
		    while (pages--) {
			kpreempt_disable();
			(void) map_memscrub_window(memscrub_window, address,
				NULL);
 ...
 */
static int
memscrub_verify_span(uint64_t *addrp, u_int *pagesp)
{
	struct memlist *mlp;
	uint64_t address = *addrp;
	uint64_t bytes = ptob((uint64_t)*pagesp);
 ...
 */
int
memscrub_add_span(pfn_t pfn, u_int pages)
{
	uint64_t address = ptob((uint64_t)pfn);
	uint64_t bytes = ptob((uint64_t)pages);
	struct memlist *dst;
 ...
 */
int
memscrub_delete_span(pfn_t pfn, u_int pages)
{
	uint64_t address = ptob((uint64_t)pfn);
	uint64_t bytes = ptob((uint64_t)pages);

...
#pragma ident	"@(#)bp_map.c	1.16	98/01/07 SMI"
...
bp_mapin(register struct buf *bp)
{
	...
	o = (uintptr_t)bp->b_un.b_addr & PAGEOFFSET;
	npf = btopr(bp->b_bcount + o);

	/*
	 * Allocate kernel virtual space for remapping.
	 */
	mutex_enter(&maplock(kernelmap));
	while ((a = rmalloc_locked(kernelmap, npf)) == 0) {
		mapwant(kernelmap) = 1;
...
bp_mapout(register struct buf *bp)
{
	...
	if (bp->b_flags & B_REMAPPED) {
		addr = (caddr_t)((uintptr_t)bp->b_un.b_addr & PAGEMASK);
		bp->b_un.b_addr = (caddr_t)((uintptr_t)bp->b_un.b_addr &
		    PAGEOFFSET);
		npf = mmu_btopr(bp->b_bcount + (uintptr_t)bp->b_un.b_addr);
		hat_unload(kas.a_hat, addr,
		    mmu_ptob(npf), HAT_UNLOAD_UNLOCK);
		a = mmu_btop(addr - Sysbase);
		rmfree(kernelmap, npf, a);
		bp->b_flags &= ~B_REMAPPED;
	}
}

...
#pragma ident	"@(#)mc.c	1.17	98/01/30 SMI"
...
/*
 * 2.7 onwards - microchannel bus is no longer supported. The bus
 * nexus is disabled by this variable. It can be re-enabled by
 * setting to 0, for important customers who must use older hardware.
 */
static int mc_forceload = -1;
...
mc_probe(register dev_info_t *devi)
{
	...
	cmn_err(CE_WARN, "mc: systems and peripherals based on the\n"
		"\tMicro Channel Architecture are no longer supported\n");
	...
	ddi_ctl_enum_t ctlop, void *arg, void *result)
{
	...
	case DDI_CTLOPS_SIDDEV:
		/*
		 * If it is not an old driver, then mc devices are
		 * self-identifying
		 */
		if (old_driver(dip))
			return (DDI_FAILURE);

...
#pragma ident	"@(#)i86_subr.s	1.87	98/02/01 SMI"
...
/*
 * cr4()
 *      Return the value of register cr4
 */
#if defined(lint) || defined(__lint)
...
/*ARGSUSED*/
char *
copyoutstr_noerr(char *s1, const char *s2, size_t *len)
{ return ((char *)0); }
...
/* ARGSUSED */
int
scanc(size_t size, u_char *cp, u_char *table, u_char mask)
{ return 0; }
...
/* ARGSUSED */
void
user_pde64_clear(uint64_t *pde_nzmask, uint64_t *pagedir, u_int *pde_mask)
{}

#else	/* lint */


	/ user_pde64_clear(&pde_nzmask[cpuid], pagedir, &pde_mask[cpuid])
	.globl	user_pde64_clear
user_pde64_clear:
	pushl	%ebp
	movl	%esp, %ebp
	pushl	%ebx
...
next_chunk64:
	bsfl	(%ebx), %eax
	jz	do_the_next32bits64
	btrl	%eax, (%ebx)
	movl	%eax, %edx
	rol	$0x08, %edx		/ %edx = 256 * %eax 32,  8 byte entries
	movl	0xc(%ebp), %esi
	addl	%edx, %esi
	movl	(%edi,%eax,4), %edx
	movl	$0x0, (%edi, %eax, 4)	/ clear pde_mask[cpuid][%eax]
next_entry64:
	bsfl	%edx, %ecx
	jz	next_chunk64
	btrl	%ecx, %edx
	movl	$0x0, (%esi, %ecx, 8)	/ We only clear lower 32 bits of the pte
	jmp	next_entry64

	/ work on the second uint32_t in the uint64_t	
do_the_next32bits64:
	addl	$4, %ebx		/ upper 32 bits of pde_nzmask[cpuid]
	addl	$128, %edi		/ &pde_mask[cpuid][32]
	addl	$0x2000, 0xc(%ebp)	/ &pagedir[1024]
next_chunk164:
	bsfl	(%ebx), %eax
	jz	i86mmu_pde_clear_done64
	btrl	%eax, (%ebx)
	movl	%eax, %edx
	rol	$0x08, %edx
	movl	0xc(%ebp), %esi
...
/*
 * XXX pde_mask defined and used in the C code as an int *
 */
/* ARGSUSED */
void
...
void
atomic_add_16(uint16_t *target, int16_t value)
{ *target += value; }

void
atomic_add_long(u_long *target, long value)
{ *target += value; }

void
atomic_add_32(uint32_t *target, int32_t value)
{ *target += value; }

uint32_t
atomic_add_32_nv(uint32_t *target, int32_t value)
{ return (*target += value); }

u_long
atomic_add_long_nv(u_long *target, long value)
{ return (*target += value); }
...
/* ARGSUSED */
uint64_t rdmsr(u_int r, uint64_t *mtr)
{ return (0); }

/* ARGSUSED */
void wrmsr(u_int r, uint64_t *mtr) {}

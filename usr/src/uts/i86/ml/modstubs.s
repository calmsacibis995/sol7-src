...
#pragma ident	"@(#)modstubs.s	1.65	98/02/06 SMI"
...
stubs_common_code:
	...
	ret
.L1:
	pushl	%eax			/ save the ret value from mod_hold_stub
	movl	$MAXNARG+2, %ecx
	...
	popl	%ecx			/ ret value from mod_hold_stub
	pushl	%eax			/ save any return values from the stub
	pushl	%edx
	pushl	%ecx			/ hold_flag from mod_hold_stub()
	pushl	%esi
 ...
 */
	...
	NO_UNLOAD_STUB(doorfs, door_create_kernel,	nomod_zero);
	NO_UNLOAD_STUB(doorfs, door_upcall,		nomod_einval);
	WSTUB(doorfs, door_create_unref,		nomod_zero);
	END_MODULE(doorfs);
#endif
 ...
 * An "argument" is passed in %eax by STUB_COMMON
 */
...
{ return (0); }
	...
	push	%eax		/* move "argument" to stack for C call */
	call	install_stub

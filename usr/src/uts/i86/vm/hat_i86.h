...
#pragma ident	"@(#)hat_i86.h	1.37	98/01/09 SMI"
...
extern	pte_t *kernel_only_pagedir;
...
struct hat {
	...
	uint_t			hat_cpusrunning;
	uint_t			hat_rss;
	short			hat_numhwpp;
	short			hat_stat;
	ushort_t		hat_flags;
	kcondvar_t 		hat_cv;
	pteptr_t 		hat_pagedir;
#ifdef	PTE36
	uint64_t 		*hat_pgdirpttblent; /* L1 pagedir entries */
#else
	uint_t			hat_pdepfn;
#endif
	struct	hat		*hat_next;
 ...
 */
#define	HMESPACE_SIZE	((NPTEPERPT/NUMHME_ALLOCATE) * sizeof (intptr_t))
...
#define	ODD_PFNUM(addr)	((((uint_t)(addr)) >> MMU_STD_PAGESHIFT) & 0x01)
...
#define	HMETABLE_INDEX(a)	(((uint_t)a >> HME_SHIFT) & HME_MASK)
#define	PTEI86HME_MASK	((NUMHME_ALLOCATE * sizeof (pte_t)) - 1)
#define	PTE_TO_I86HMEPTE(a)	((struct pte *)(((uint_t)(a)) & \
							~PTEI86HME_MASK))
 ...
 * 'struct i86hme'
 */
#define	I86MMU_PTEHMEPROT(pte, a) ((a)->hme_impl |= ((*(uint_t *)pte) & 0x06))
#define	I86MMU_PROTFROMHME(a)	(((uint_t)((a)->hme_impl & 0x06)) >> 1)
...
typedef struct hwptepage {
	...
	uint_t	*hwpp_hme;	/* Pointer to the area holding hme pointers */
	pte_t	*hwpp_pte;	/* Pointer to page of ptes */
	uint_t	hwpp_pfn;	/* Page frame number */
	struct 	hat *hwpp_hat;
	pteval_t hwpp_pde;	/* page directory entry */
	ushort_t hwpp_base;	/* Base address of 4 meg section of as */
	ushort_t hwpp_lockcnt;	/* Number of locks on  ptes in this hwpp */
	ushort_t hwpp_numptes;	/* number of valid ptes in this hwpp */
	ushort_t hwpp_mapping; 	/* This hwpp maps a 4Mb page */
} hwptepage_t;
 ...
 */
#ifdef	PTE36
#define	I86MMU_USER_TEXT		0x40
#define	I86MMU_USER_SHLIB		0x6ff
#else
...
#define	IOMAP_SYNC(addr, ptr)	{ if (addr >= (caddr_t)SYSBASE) \
		((pte_t *)(V_IOMAP_ADDR))[ \
		mmu_btop((uint_t)addr - SYSBASE)] = *ptr; }
#else
#define	IOMAP_SYNC(addr, ptr)
#endif

extern	uint_t	xc_broadcast_tlbflush(cpuset_t, caddr_t);

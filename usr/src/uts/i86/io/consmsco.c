...
#pragma ident	"@(#)consmsconf.c	1.29	97/03/14 SMI"
...
#define	PATHBUFLEN	80

char *
prom_stdinpath(void)
{
	static char	*stdinpath, path_buf[PATHBUFLEN];

	if (stdinpath != (char *)0)
		return (stdinpath);

	if (prom_getprop(prom_optionsnode(), "stdin", path_buf) > 0) {
		return (stdinpath = path_buf);
	}
	return ((char *)0);
}
...
asymajor()
{
	...
	return (asym ? asym : (asym = ddi_name_to_major(asy_name)));
}
...
consconfig(void)
{
	...
	(void) ddi_install_driver(asy_name);

	rconsdev = 0;
	if ((stdiopath = prom_stdinpath()) == 0)
		goto not_on_same_asy;
	...
	if (getmajor(rconsdev) != asymajor())
		rconsdev = 0;

not_on_same_asy:

	if ((rconsdev == 0) && (console == CONSOLE_IS_ASY)) {
		/*
		 * This is to support backward compatibility. It's an ugly
		 * hack because you can't determine which instance number
		 * will go with which port.
		 */

		rconsdev = makedevice(ddi_name_to_major(asy_name), 0);
	}

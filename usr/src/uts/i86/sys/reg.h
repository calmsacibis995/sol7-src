...
#pragma ident	"@(#)reg.h	1.15	97/11/05 SMI"
...
#define	i8080reg(r) union {  /* 8080-like "general purpose" registers */\
	unsigned long e ## r ## x;					\
	struct { unsigned short r ## x; } word;				\
	struct { unsigned char  r ## l, r ## h; } byte;			\
}

#define	i8086reg(r) union { /* 16/32-bit "special purpose" registers  */\
	unsigned long e ## r;						\
	struct { unsigned short r; } word;				\
}
...
struct bop_regs {
	...
	unsigned short ds;	/* Segment registers */
	unsigned short es;
	unsigned short fs;
	unsigned short gs;

...
#pragma ident	"@(#)ddi_i86.c	1.16	97/02/26 SMI"
...
{
	return (ddi_swap16(inw((u_int)addr)));
}
...
{
	return (ddi_swap32(inl((u_int)addr)));
}
...
{
	outw((u_int)addr, ddi_swap16(value));
}
...
{
	outl((u_int)addr, ddi_swap32(value));
}

...
#pragma ident	"@(#)sfmmu_asm.s	1.113	98/02/09 SMI"
 ...
 *
 * LP64 NOTE: As long our kernel tsb's are no larger than
 *	      8MB the code below will work. 
 */
#define	GET_TSB_POINTER(tsbreg, tagacc, tsbp, tmp1, tmp2, tmp3)		\
	/* get ctx */;							\
	sll	tagacc, TAGACC_CTX_SHIFT, tmp1;				\
	srl	tagacc, MMU_PAGESHIFT, tmp2;				\
	srl	tmp1, TAGACC_CTX_SHIFT, tmp1;				\
	sethi	%hi(TSB_MIN_OFFSET_MASK), tsbp;				\
	and	tsbreg, TSB_SZ_MASK, tmp3;	/* tsb size */		\
	...
	andn	tsbreg, TSB_SZ_MASK, tsbp;	/* get tsb base */	\
...
#define	TSB_UPDATE_TL(tsb8k, tte, tagtarget, tmp1, tmp2, ttepa, label)	\
	TSB_LOCK_ENTRY(tsb8k, tmp1, tmp2, label)			;\
	/*								;\
	...
	brgez,a,pn tmp1, label/**/f					;\
	 st	tmp2, [tsb8k + TSBE_TAG]	/* unlock */		;\
	TSB_INSERT_UNLOCK_ENTRY(tsb8k, tte, tagtarget)			;\
label:

#define	TSB_UPDATE(tsb8k, tteva, tagtarget, tmp1, tmp2, label)		\
	...
	brgez,a,pn tteva, label/**/f					;\
	 st	tmp2, [tsb8k + TSBE_TAG]	/* unlock */		;\
	TSB_INSERT_UNLOCK_ENTRY(tsb8k, tteva, tagtarget)		;\
label:

#define	TSB_INVALIDATE(tsb8k, tag, tmp1, tmp2, tmp3, label)		\
	ld	[tsb8k], tmp1						;\
	sethi	%hi(TSBTAG_LOCKED), tmp2				;\
label/**/1:								;\
	cmp	tmp1, tmp2						;\
	be,a,pn	%icc, label/**/1					;\
	  ld	[tsb8k], tmp1						;\
	ldx	[tsb8k + TSBE_TAG], tmp3	/* load tag */		;\
	cmp	tag, tmp3						;\
	bne,pt	%xcc, label/**/2					;\
 ...
 * This macro uses virtual addresses to access the tsb.
 * We assume that the TSBREG.split == 0
 *
...
#define	FLUSH_TSBCTX(tsbreg, ctxnum, tmp1, tmp2, tmp3)			\
	...
	andn	tsbreg, TSB_SZ_MASK, tmp1	/* get base va */	;\
...
1:									;\
	brnz,a,pt tmp2, 0b		/* if not end of TSB go back */ ;\
...
#define	LOAD_TSBINFO(tsbinfo, tmp1, tmp2, tmp3)				\
	GET_UTSB_REGISTER(tmp2);					\
	ldx	[tsbinfo + TSBINFO_REG], tmp1;	/* tsbreg */		\
	cmp	tmp2, tmp1;			/* same tsbreg ! */	\
	be,pn	%xcc, 1f;						\
	set	MMU_TSB, tmp3;						\
...
3:
	...
	GET_TSB_POINTER(%o3, %o4, %g1, %g2, %g3, %g4) /* g1 = tsbp */
 ...
 */ 
	...
	GET_TSB_POINTER(%o2, %g2, %g1, %o3, %o4, %o5)	/* g1 = tsbp */
	BUILD_TSB_TAG(%o0, %o1, %o3, %o4)	/* o3 = target tag */
	TSB_INVALIDATE(%g1, %o3, %o2, %o4, %o5, tsbinv_label)
	retl
	membar	#StoreStore|#StoreLoad
...
#define	USE_ALTERNATE_GLOBALS						\
	...
	ba,pt	%icc, .sfmmu_pagefault_common
	  nop
...
label/**/1:								;\
	...
	blu,a,pt  %xcc, ismhit			/* ism hit */		;\
	  add	maptr, ISMMAP_SFMMU, maptr 	/* maptr = &ism_sfmmu*/ ;\
									;\
...
#define GET_TTE(tag, hatid, tte, hmeblkpa, hmeblkva, tsbarea, hmentoff, \
	 ...
	 * hmeblkva = hmeblkva						;\
	 */								;\
	brz,pn	hmeblkva, exitlabel	/* exit if hblk not found */	;\
	  nop								;\
...
#define	SAVE_TSBPTR(tsbptr, missarea)			\
	stn	tsbptr, [missarea + TSBMISS_TSBPTR]	/* save tsbptr */

#define RESTORE_TSBPTR(tsbptr, missarea)		\
...
4:
	...
	membar	#Sync
utsb_fault_lock:
	ldn	[%g6 + (TSBMISS_SCRATCH + TSBMISS_HMEBP)], %g5
	HMELOCK_EXIT(%g5)
	/* fall through */
utsb_pagefault:
	/*
...
5:
	...
	GET_TSB_POINTER(%g6, %g2, %g1, %g5, %g2, %g7)
	ldxa	[%g0]ASI_DMMU, %g2
	TSB_UPDATE_TL(%g1, %g3, %g2, %g5, %g6, %g4, 7)
	/*
	 * Now, load into TLB
...
kvtop_nohblk:
	 ...
	 * if neccesary.
	 */
	ldn	[%o2 + (TSBMISS_SCRATCH + TSBMISS_HMEBP)], %o3
	HMELOCK_EXIT(%o3)

/*
 * Copyright (c) 1989, 1990, 1991, 1997-1998 by Sun Microsystems, Inc.
 * All rights reserved.
 */

#pragma ident   "@(#)hat_sfmmu.c 1.171     98/02/07 SMI"
...
static u_int 		hblk1_avail;	/* number of free 1 hme hme_blks */
static u_int 		hblk8_avail;	/* number of free 8 hme hme_blks */
...
static u_int 	hblk8_allocated, hblk1_allocated;
static u_int	hblk8_prealloc_count;
static u_int	hblkalloc_inprog;

#ifdef DEBUG
static u_int 	hblk8_inuse, hblk1_inuse;
static u_int	nhblk8_allocated, nhblk1_allocated;
...
static struct 	hme_blk *sfmmu_hblk_alloc(sfmmu_t *, caddr_t,
			struct hmehash_bucket *, u_int, hmeblk_tag);
...
static struct hme_blk *sfmmu_tteload_find_hmeblk(sfmmu_t *,
				struct hmehash_bucket *, caddr_t, u_int);
...
void		sfmmu_memtte(tte_t *, pfn_t, u_int, int);
...
static uint64_t	sfmmu_vtop_attr(u_int, int mode, tte_t *);
...
static void	sfmmu_expand_tsbsize(sfmmu_t *sfmmup);
...
u_int		nctxs;			/* total number of contexts */
 ...
 * 1- our rss is larger than a certain factor.
 * 2- we are not already using a large tsb.
 */
#define	SFMMU_SELECT_TSBSIZE(sfmmup)			\
	(tsb512k_num > 0 && (sfmmup)->sfmmu_rss >= tsb_rss_factor)
...
sfmmu_hblk_init(void)
{
	...
	SFMMU_STAT_SET(sf_hblk8_startup_use,
	    (u_int)(hblk8_allocated - hblk8_avail));
...
hat_alloc(struct as *as)
{
	...
	if (sfmmup == ksfmmup)
		CPUSET_OR(sfmmup->sfmmu_cpusran, all);
	sfmmup->sfmmu_free = 0;
...
sfmmu_memload_batchsmall(struct hat *hat, caddr_t vaddr, machpage_t **pps,
		    u_int attr, u_int flags, pgcnt_t npgs)
{
	tte_t	tte;
	machpage_t *pp;
	struct hmehash_bucket *hmebp;
 ...
 */
void
sfmmu_memtte(tte_t *ttep, pfn_t pfn, u_int attr, int tte_sz)
{
	ASSERT(!(attr & ~SFMMU_LOAD_ALLATTR));
	...
	caddr_t vaddr, machpage_t **pps, u_int flags)
{
	...
	if (!TTE_IS_VALID(&tteold)) {
		...
		if (size == TTE8K) {
			atomic_add_32((u_int *)&sfmmup->sfmmu_rss, 1);
		} else {
			...
			ctx = sfmmutoctx(sfmmup);
			atomic_add_16(&sfmmup->sfmmu_lttecnt, 1);
			CTX_SET_LTTES(ctx);
			/*
	...
	if ((size == TTE8K) && !(flags & SFMMU_NO_TSBLOAD)) {
		...
		if (sfmmup != ksfmmup) {
			 ...
			 *
			 * Note: It's important that sfmmu_expand_tsbsize()
			 * ONLY be called on behalf of the currently running
			 * process. In the case of procfs we need to make
			 * sure its not called because it may re-set the
			 * tsb register.
			 */
			if (sfmmup == astosfmmu(curthread->t_procp->p_as) &&
			    SFMMU_CHECK_TSBSIZE(sfmmup)) {
...
sfmmu_pagearray_setup(caddr_t addr, machpage_t **pps, tte_t *ttep, int remap)
{
	...
	for (i = 0; i < npgs; i++, pps++) {
		...
		if (sfmmu_vacconflict_array(addr, pp, &cflags)) {
			 ...
			 */
			for (j = 0; j <= i; j++) {
				(void) sfmmu_vacconflict_array(addr, pps[j],
				    &cflags);
			}
		}
...
readtte:
	...
	if (TTE_IS_VALID(&tte)) {
		...
		ttesz = get_hblk_ttesz(hmeblkp);
		if (ttesz != TTE8K) {
			panic("sfmmu_pageunload - large page");
		}
...
sfmmu_uvatopfn(caddr_t vaddr, struct hat *sfmmup)
{
	...
	while (ismblkp && ism_hatid == NULL) {
		ism_map = ismblkp->ismblk_maps;
		for (i = 0; ism_map[i].ism_sfmmu && i < ISM_MAP_SLOTS; i++) {
			if (vaddr >= ism_start(ism_map[i]) &&
			    vaddr < ism_end(ism_map[i])) {
				sfmmup = ism_hatid = ism_map[i].ism_sfmmu;
				vaddr = (caddr_t)(vaddr -
					ism_start(ism_map[i]));
				break;
			}
		}
		ismblkp = ismblkp->ismblk_next;
	}
	if (sfmmulock) {
...
hat_get_mapped_size(struct hat *hat)
{
	...
	if (hat != NULL)
		return ((size_t)ptob(hat->sfmmu_rss +
		    hat->sfmmu_lttecnt * TTEPAGES(TTE4M)));
	else
		return (0);
}
	...
	struct hat *ism_hatid, caddr_t sptaddr, size_t len)
{
	...
	while (!added) {
		...
		for (i = 0; i < ISM_MAP_SLOTS; i++)  {
			if (map[i].ism_sfmmu == NULL) {
				map[i].ism_sfmmu = ism_hatid;
				map[i].ism_seg = (uintptr_t)addr | sh_size;
				added = 1;
				break;
			}
		}
		if (!added && !blkp->ismblk_next) {
			blkp->ismblk_next = ism_map_blk_alloc();
		}
		blkp = blkp->ismblk_next;
	}

	atomic_add_32((u_int *)&sfmmup->sfmmu_rss, ism_hatid->sfmmu_rss);

	atomic_add_16((u_short *)&sfmmup->sfmmu_lttecnt,
			ism_hatid->sfmmu_lttecnt);

	/*
	 * Since the tl=1 tsb miss handler will hash for 4M
	 * and 8K entries we need to set this only if we have
	 * large pages.
	 */
	 ...
	 * Note: It's important that sfmmu_expand_tsbsize()
	 * ONLY be called on behalf of the currently running
	 * process. In the case for fork() we need to make sure
	 * its not called because it may re-set the tsb register.
	 */
	if (sfmmup == astosfmmu(curthread->t_procp->p_as) &&
	    SFMMU_CHECK_TSBSIZE(sfmmup)) {
		sfmmu_expand_tsbsize(sfmmup);
...
hat_unshare(struct hat *sfmmup, caddr_t addr, size_t len)
{
	...
	while (!found && blkp) {
		map = blkp->ismblk_maps;
		for (i = 0; i < ISM_MAP_SLOTS; i++) {
			if (addr == ism_start(map[i]) &&
			    sh_size == ism_size(map[i])) {
				found = 1;
				break;
			}
		}
	...
	if (found) {
		...
		ASSERT(hat_get_mapped_size(ism_hatid));
		/*
	...
	if (found) {
		...
		atomic_add_32((u_int *)&sfmmup->sfmmu_rss, -spt_rss);
		atomic_add_16(&sfmmup->sfmmu_lttecnt, -spt_lttecnt);
	}
	sfmmu_allow_ctx_steal(sfmmup);
}
...
ism_map_blk_alloc(void)
{
	...
	map = blkp->ismblk_maps;
	for (i = 0; i < ISM_MAP_SLOTS; i++) {
		ASSERT(map[i].ism_sfmmu == NULL);
		ASSERT(map[i].ism_seg == 0);
	}
	ASSERT(blkp->ismblk_next == NULL);
#endif /* DEBUG */
	...
	while (blklist) {
		blkp1 = blklist;
		blklist = blklist->ismblk_next;
		blkp1->ismblk_next = NULL;
...
{
#ifdef DEBUG
	ism_map_t	*map = blkp->ismblk_maps;
	int 		i;
...
sfmmu_reuse_ctx(struct ctx *ctx, sfmmu_t *sfmmup)
{
	...
	CPUSET_AND(cpuset, cpu_ready_set);
	/*
	 * XXX: We should be able to clear the cpu set at this point
	 *	in order not to generate unnecessary xcalls in the future.
	 */
	xc_attention(cpuset);
...
sfmmu_free_sfmmu(sfmmu_t *sfmmup)
{
	...
	sfmmup->sfmmu_ismblk = NULL;
	while (blkp) {
		nx_blkp = blkp->ismblk_next;
		blkp->ismblk_next = NULL;
...
sfmmu_hblk_alloc(sfmmu_t *sfmmup, caddr_t vaddr,
	struct hmehash_bucket *hmebp, u_int size, hmeblk_tag hblktag)
{
	struct hme_blk *hmeblkp = NULL;
 ...
 */
void
sfmmu_tsb_miss(struct regs *rp, uintptr_t tagaccess, uint traptype)
{
	struct hmehash_bucket *hmebp;
	 ...
	 * Set ism_hatid if vaddr falls in a ISM segment.
	 */
	ismblkp = sfmmup_orig->sfmmu_ismblk;
	if (ismblkp) {
...
sfmmu_tlbcache_demap(caddr_t addr, sfmmu_t *sfmmup,
	struct hme_blk *hmeblkp, pfn_t pfnum, int noflush, int flags)
{
	int ctxnum, vcolor;
	cpuset_t cpuset;
...
sfmmu_cache_flushcolor(int vcolor)
{
	...
	cpuset = cpu_ready_set;
	CPUSET_DEL(cpuset, CPU->cpu_id);
	xt_some(cpuset, vac_flushcolor_tl1, vcolor, 0);
...
static
void
sfmmu_ctx_demap(sfmmu_t *sfmmup)
{
	int ctxnum;
	cpuset_t cpuset;
	 ...
	 * Flush tsb
	 */
	sfmmu_unload_tsbctx(ctxnum);

	/*
	 * Flush tlb in all CPUs that ran the process whose
	 * ctx we are demapping.
	 */
	xt_some(cpuset, (uintptr_t)sfmmu_ctx_steal_tl1, ctxnum,
		INVALID_CONTEXT, 0, 0);
	/*
...
sfmmu_init_tsbs()
{
	...
	/*
	 * Kernel and invalid context use same tsb.
	 */
	CTX_SET_TSBINDEX(ctxnumtoctx(KCONTEXT), KTSBNUM, 0);
	CTX_SET_TSBINDEX(ctxnumtoctx(INVALID_CONTEXT), KTSBNUM, 0);
	...
	/*
	 * Is kernel tsb is also shared by users?
	 */
	if (tsb512k_num == 0) {
		ASSERT(tsb_num == 1);
	...
	if (khmehash_num > SHRT_MAX || uhmehash_num > SHRT_MAX)
	...
	for (i = 0; i < NCPU; tsbmissp++, i++) {
		...
		tsbmissp->khashsz = (u_short) khmehash_num;
		tsbmissp->khashstart = khme_hash;
		tsbmissp->uhashsz = (u_short) uhmehash_num;

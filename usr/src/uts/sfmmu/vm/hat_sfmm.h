...
#pragma ident	"@(#)hat_sfmmu.h	1.87	98/02/07 SMI"
 ...
 */
#define	ISM_VBASE_SHIFT	MMU_PAGESHIFT4M		/* 4MB */
 ...
 * will be constrained to a 4MB alignment. Also since every newly
 * created ISM segment is created out of a new address space at base va
 * of 0 we don't need to store it.
 */
#define	ISM_ALIGN	(1 << ISM_VBASE_SHIFT)	/* base va aligned to 4MB  */
...
/*
 * All segments mapped with ISM are guarenteed to be 4MB aligned.
 * Also size is guaranteed to be in 4MB chunks.
 ...
 */
typedef struct {
	uintptr_t	ism_seg;  /* base va + sz of ISM segment */
	sfmmu_t		*ism_sfmmu; /* hat id of dummy ISM as */
} ism_map_t;

#define	ism_start(map)	((caddr_t)((map).ism_seg & ~ISM_SZ_MASK))
#define	ism_size(map)	(map.ism_seg & ISM_SZ_MASK)
...
typedef struct ism_map_blk {
	ism_map_t	ismblk_maps[ISM_MAP_SLOTS];
	struct ism_map_blk *ismblk_next;
} ism_map_blk_t;
...
struct hat {
	...
	int		sfmmu_rss;	/* approx # of pages used by as */
	ushort_t	sfmmu_lttecnt;	/* # of large mappings in this hat */
...
struct tsbmiss {
	...
	caddr_t		tsbptr;		/* hardware computed ptr */
	struct	ctx	*ctxs;
	...
	uint32_t		uprot_traps;
	uint32_t		kprot_traps;
...
/*
 * The TSB
 * Only 128K, 256K and 1/2MB sizes can be supported. Currently, we
 * implement 128K and 512K TSBs.
 * Only common configuration is supported.
 * Only 8k entries populating tsb is supported.
 */
...
#define	TSB_FREEMEM_LARGE	0x80000		/* 4 gb */
...
#define	PP_GROUPLEADER(pp, sz) \
	(PP2MACHPP(page_numtopp_nolock((pp)->p_pagenum & ~(TTEPAGES(sz)-1))))

/* Return the root page for this page based on p_cons */
#define	PP_PAGEROOT(pp)	(((pp))->p_cons == 0 ? (pp) : \
	PP_GROUPLEADER((pp), (pp)->p_cons))

#define	PP_PAGENEXT(pp)	(PP2MACHPP(page_numtopp_nolock((pp)->p_pagenum + 1)))
#define	PP_PAGEPREV(pp)	(PP2MACHPP(page_numtopp_nolock((pp)->p_pagenum - 1)))
 ...
 */
extern uint64_t	va_to_pa(caddr_t);

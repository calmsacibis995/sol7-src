...
#pragma	ident	"@(#)rpcsec_gss.c	1.46	98/02/01	SMI"
	...
	for (i = 0; i < GSSAUTH_TABLESIZE; i++) {
		...
		for (p = ga_cache_table[i]; p; p = next) {
			...
			if ((p->ref_time + rpc_gss_cache_time >
				hrestime.tv_sec) || p->in_use) {
				if ((p->ref_time + rpc_gss_cache_time <=
				    hrestime.tv_sec) && p->in_use) {
				    RPCGSS_LOG0(2, "gssauth_cache_reclaim: "
					"in_use\n");
				}
				prev = p;
				continue;
...
rpc_gss_secget(CLIENT *clnt,
	...
	cred_t *cr)
{
	struct ga_cache_entry **head, *current, *new;
	AUTH *auth = NULL;
	rpc_gss_data	*ap;
	...
	IS_ALIGNED(cr);
#ifdef DEBUG
if (HASH(cache_key, cr->cr_uid) < 0) {
	prom_printf("cache_key %p, cr %p\n", cache_key, (void *)cr);
}
#endif
	ASSERT(HASH(cache_key, cr->cr_uid) >= 0);
	head = &ga_cache_table[HASH(cache_key, cr->cr_uid)];
	NOT_NULL(head);
	IS_ALIGNED(head);

	/*
	 *  Get the gss auth handle from the cache table if
	 *  it is there.
	 */
	rw_enter(&ga_cache_table_lock, RW_READER);
	for (current = *head; current; current = current->next) {
		...
		if ((cache_key == current->cache_key) &&
			...
			!current->in_use) {
			current->in_use = TRUE;
			current->ref_time = hrestime.tv_sec;
	...
	/*
	 *  If gss auth handle is not in the cache table, create
	 *  a new gss auth handle and add it into the cache table.
	 */
	if (!auth) {
		ga_cache_miss++;
		if (auth = rpc_gss_seccreate(clnt, principal, mechanism,
			service_type, qop, options_req, options_ret, cr)) {
	...
	cred_t			*cr)		/* client's unix cred */
{
	 ...
	 */
	if (!rpc_gss_seccreate_pvt(&gssstat, &minor_stat, auth, ap, mechanism,
				&ap->mech_type, &ret_flags, &time_rec, cr)) {
		if (ap->target_name) {
			(void) gss_release_name(&minor_stat, &ap->target_name);
		}
		kmem_free((char *)ap, sizeof (*ap));
		kmem_free((char *)auth, sizeof (*auth));
		RPCGSS_LOG0(1, "rpc_gss_seccreate: init context failed\n");
		return (NULL);
	}
...
rpc_gss_seccreate_pvt(gssstat, minor_stat, auth, ap, desired_mech_type,
			actual_mech_type, ret_flags, time_rec, cr)
	OM_uint32		*gssstat;
	OM_uint32		*minor_stat;
	AUTH			*auth;
	rpc_gss_data		*ap;
...
next_token:
	*gssstat = kgss_init_sec_context(minor_stat,
	...
	if (*gssstat != GSS_S_COMPLETE && *gssstat != GSS_S_CONTINUE_NEEDED) {
		RPCGSS_LOG(1, "rpc_gss_seccreate_pvt: gssstat = 0x%x\n",
			*gssstat);
	...
	if (call_arg.length != 0) {
		...
		if ((call_res.gss_major != GSS_S_COMPLETE) &&
			...
			RPCGSS_LOG(1, "rpc_gss_seccreate_pvt: "
				"call_res.gss_major %x\n", call_res.gss_major);
			goto cleanup;
		}
	...
	/*
	 * Done!  Context is established, we have ctx_handle.
	 */
	ap->established = TRUE;
	ap->seq_num = 1;
...
* if we get an error log a message and
...
*/
	*gssstat  = kgss_export_sec_context(minor_stat, ap->context,
	...
	if (*gssstat == GSS_S_NAME_NOT_MN) {
		...
		free_results = FALSE;
		RPCGSS_LOG(1, "rpc_gss_seccreate_pvt: export_sec_context "
			"failed   gssstat = 0x%x\n",
			*gssstat);
	...
	if (*gssstat == GSS_S_COMPLETE) {
		...
		RPCGSS_LOG(1, "rpc_gss_seccreate_pvt:"
			"import sec context failed   gssstat = 0x%x\n",
			*gssstat);
		(void) kgss_delete_sec_context(minor_stat,
					&ap->context, NULL);
		(void) gss_release_buffer(minor_stat, &process_token);
	...
	char			*buf;	/* pointer of send buffer */
{
	...
	in_buf.value = buf;
	if (kgss_sign(&minor, ap->context, ap->qop, &in_buf,
	...
	struct opaque_auth	*verf;
{
	...
	/*
	 * If context is not established, ignore verifier.
	 */
	if (!ap->established)
		return (TRUE);

	seq_num_net = (u_int) htonl(ap->seq_num);
	...
	cred_t		*cr;
{
	...
	if (msg->rjcted_rply.rj_why == RPCSEC_GSS_NOCRED ||
		...
		if (rpc_gss_seccreate_pvt(&gssstat, &minor_stat, auth, ap,
			ap->mech_type, (gss_OID *)NULL, (int *)NULL,
			(OM_uint32 *)NULL, cr)) {
			RPCGSS_LOG(2,
			"rpc_gss_refresh: refresh auth %p\n", (void *)auth);
			return (TRUE);
		} else {
			ap->invalid = TRUE;
			RPCGSS_LOG0(1,
			"rpc_gss_refresh: can't create a new context\n");
			return (FALSE);
		}
	}
	...
	caddr_t			xdr_ptr;
{
	...
	XDR			xdrs;
	char			*tmp_buf;
	u_int			xdr_buf_len, cred_buf_len;

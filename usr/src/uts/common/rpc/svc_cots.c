...
#pragma ident	"@(#)svc_cots.c	1.34	97/06/26 SMI" /* SVr4.0 1.10 */
...
allocate_space:
	...
	xprt = (SVCXPRT *) kmem_alloc(sizeof (SVCXPRT), KM_SLEEP);
	cd = (cots_data_t *) kmem_zalloc(sizeof (cots_data_t) + addr_size,
	    KM_SLEEP);
...
svc_cots_krecv(SVCXPRT *xprt, mblk_t *mp, struct rpc_msg *msg)
{
	...
	RPCLOG(4, "svc_cots_krecv_start xprt = %p:\n", (void *)xprt);

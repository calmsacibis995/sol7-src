...
/*
 * Copyright (c) 1992, 1994-1997 by Sun Microsystems, Inc.
 * All rights reserved.
 */

#pragma ident	"@(#)rpcmod.c	1.63	97/11/26 SMI"
...
static struct sysent rpcsysent = {
	2,
	SE_32RVAL1 | SE_ARGC, /* rpcsys() returns one 32-bit result in rval1 */
	rpcsys
};
...
_init(void)
{
	...
	mt_kstat_init();
	(void) callb_add(connmgr_cpr_reset, 0, CB_CL_CPR_RPC, "rpc");
...
#define	RM_CLOSING	0x1		/* somebody is trying to close slot */
	u_int		rm_state;	/* state of the slot. see above */
	u_int		rm_flowed;	/* cnt of threads blocked on flow ctl */
	u_int		rm_ref;		/* cnt of external references to slot */
...
rpcmodopen(queue_t *q, dev_t *devp, int flag, int sflag, cred_t *crp)
{
	...
	cv_init(&rmp->rm_fwait, NULL, CV_DEFAULT, NULL);
...
#ifndef OK_INT32_PTR
#define	OK_INT32_PTR(p)  ((((uintptr_t)p) & (sizeof (uint32_t) - 1)) == 0)
#endif
...
typedef struct mir_s {
	...
	bufcall_id_t mir_bufcall_id;		/* id of outstanding bufcall */
	...
	u_int	*mir_max_msg_sizep;	/* Reference to sanity check size */
		/*
...
static void	mir_bufcall_callback(void *);
...
extern	u_int	*clnt_max_msg_sizep;
extern	u_int	*svc_max_msg_sizep;
u_int	clnt_max_msg_size = RPC_MAXDATASIZE;
u_int	svc_max_msg_size = RPC_MAXDATASIZE;

static void
mir_bufcall_callback(void *q_param)
{
...
mir_open(queue_t *q, dev_t *devp, int flag, int sflag, cred_t *credp)
{
	...
	mir->mir_frag_len = -sizeof (uint32_t);

	mutex_init(&mir->mir_mutex, NULL, MUTEX_DEFAULT, NULL);
	rw_init(&mir->mir_send_lock, NULL, RW_DEFAULT, NULL);
...
same_mblk:;
		...
		if (frag_len < 0) {
			...
			{
				u_int l = frag_header & ~MIR_LASTFRAG;
		...
		if (excess > 0) {
			...
			if (mp1 == NULL) {
				...
				RPCLOG0(1, "mir_rput: dupb failed\n");
				mir->mir_bufcall_id = bufcall(1, BPRI_MED,
				    mir_bufcall_callback, q);
	...
	if (frag_len > 0 && mir->mir_max_msg_sizep &&
		...
		cmn_err(CE_NOTE,
		    ...
		    "maximum (%u). Fragment header was 0x%x. Disconnecting",
		    (mir->mir_type == RPC_CLIENT) ? "server" :
		    (mir->mir_type == RPC_SERVER) ? "client" :
		    "test tool",
		    frag_len, *mir->mir_max_msg_sizep, frag_header);
...
mir_rsrv(queue_t *q)
{
	 ...
	 */
	{
		...
		mir->mir_first_non_processed_mblk = NULL;
...
mir_clnt_stop_idle(queue_t *wq)
{
	...
	RPCLOG(1, "mir_clnt_stop_idle: wq 0x%p\n", (void *)wq);
	ASSERT(MIR_LOCK_NOT_HELD(mir));
	MIR_LOCK(mir);
...
mir_timer(queue_t *wq, mir_t *mir, mblk_t *mp)
{
	...
	/*
	 * If we were waiting for memory, then enable the read service
	 * procedure.
	 */
	if (mir->mir_awaiting_memory)
		qenable(RD(wq));
	...
	case RPC_CLIENT:
		 ...
		 * timeout regardless, the mir_req_cnt will not be a
		 * valid indicator.
		 *
		 ...
		 */
		if (mir->mir_awaiting_memory || (mir->mir_req_cnt > 0 &&
		    mir->mir_use_timestamp +
...
printf("mir_timer[%d]: doing client timeout\n", lbolt / hz);
	...
	case RPC_SERVER:
		 ...
		 * by mir_idle_start whenever the stream goes idle
		 * (mir_req_cnt == mir_reply_cnt).  The timer is cancelled in
		...
		/*
		 * If the stream is idle and we were waiting on memory
		 * restart the idle timer.
		 */
		if (mir->mir_awaiting_memory) {
			mir_idle_start(wq, mir);
			MIR_UNLOCK(mir);
			return;
		}
		MIR_UNLOCK(mir);
		...
		/*
		 * The stream is really idle so notify nfsd to close it.
		 */
...
ioc_eperm:
		...
		case RPC_TEST:
			...
			if (strqset(q, QMAXPSZ, 0, RPC_MAXDATASIZE) != 0) {
				/*EMPTY*/
				RPCLOG0(1, "mir_wput: strqset failed\n");
			}
			unfreezestr(q);
	...
	case M_PROTO:
		...
		case T_ORDREL_REQ:
			...
			/*
			 * If we have already held or sent down a
			 * T_ORDREL_REQ, don't send any more.
			 */
			if (mir->mir_ordrel_mp || mir->mir_ordrel_pending) {
				freemsg(mp);
				MIR_UNLOCK(mir);
				return;
			}
			 ...
			 * new inbound data, and reset the idle timer.
			 */
			mir->mir_ordrel_pending = 1;
			mir_idle_start(q, mir);
			MIR_UNLOCK(mir);
...
mir_wsrv(queue_t *q)
{
	...
	while (mp = getq(q)) {
		 ...
		 * (ignoring flow control) without carving it up.
		 */
		if (mp->b_datap->db_type != M_DATA) {
			MIR_UNLOCK(mir);
			putnext(q, mp);

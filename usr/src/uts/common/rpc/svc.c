...
#pragma ident	"@(#)svc.c	1.62	97/11/16 SMI" /* SVr4.0 1.9 */
...
static clock_t rpc_thread_timo = RPC_THREAD_TIMEOUT;
...
static void svc_thread_cleanup(SVCXPRT *clone_xprt, bool_t holds_thread_lock);
...
svc_run(SVCXPRT	*xprt)
{
	mblk_t	*mp;
	clock_t	timeleft;
	register SVCXPRT *clone_xprt;
	caddr_t p2buf;
	callb_cpr_t cprinfo;
	kmutex_t cpr_mutex;

	/* allocate the new xprt before grabbing any locks. */
	clone_xprt = kmem_zalloc(sizeof (SVCXPRT), KM_SLEEP);
	p2buf = kmem_zalloc(SVC_MAX_P2LEN, KM_SLEEP);
	 ...
	 */
	mutex_init(&cpr_mutex, NULL, MUTEX_DEFAULT, NULL);
		...
		while (xprt->xp_req_head == NULL) {
			...
			if (timeleft <= 0 &&
				...
				if (xprt->xp_threads > xprt->xp_min_threads) {
					svc_thread_cleanup(clone_xprt, TRUE);
					kmem_free(clone_xprt, sizeof (SVCXPRT));
					kmem_free(p2buf, SVC_MAX_P2LEN);
					thread_exit();
					/* NOTREACHED */
				}
			...
			if (!xprt->xp_wq) {
				mutex_exit(&xprt->xp_req_lock);
				svc_thread_cleanup(clone_xprt, FALSE);
				kmem_free(clone_xprt, sizeof (SVCXPRT));
				kmem_free(p2buf, SVC_MAX_P2LEN);
				thread_exit();
				/* NOTREACHED */
...
/*
 * Clean up for a service thread and return.
 */

static void
svc_thread_cleanup(SVCXPRT *clone_xprt,

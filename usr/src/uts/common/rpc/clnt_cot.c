...
#pragma ident	"@(#)clnt_cots.c	1.68	97/11/20 SMI"
/* SVr4.0 1.10  */
 ...
 * publication.
 *
 *	Copyright (c) 1986-1991, 1995-1997 by Sun Microsystems, Inc.
...
static void	clnt_dispatch_send(queue_t *, mblk_t *, calllist_t *, u_int,
	u_int);
...
static u_int clnt_cots_maxrecv	= CLNT_MAXRECV_WITHOUT_RETRY;

u_int *clnt_max_msg_sizep;
...
clnt_cots_kcreate(dev_t dev, struct netbuf *addr, int family, rpcprog_t prog,
	rpcvers_t vers, u_int max_msgsize, cred_t *cred, CLIENT **ncl)
{
	CLIENT *h;
	cku_private_t *p;
...
call_again:
	...
	if (h->cl_auth->ah_cred.oa_flavor != RPCSEC_GSS) {
		...
		XDR_SETPOS(xdrs, (u_int)(mp->b_rptr - mp->b_datap->db_base +
		    WIRE_HDR_SIZE));
...
conn_kstat_update(kstat_t *ksp, int rw)
{
	...
	ksp->ks_ndata = (u_int)(ksp->ks_data_size / sizeof (kstat_named_t));
	return (0);
}
		...
		if (cm_entry->x_server.buf) {
			if (cm_entry->x_family == AF_INET &&
				...
				bcopy(&cm_entry->x_server.buf[4],
				    ...
				    sizeof (ck_entry->x_server.value.ui32));
				bcopy(&cm_entry->x_server.buf[2],
				    &port, sizeof (port));
				ck_entry->x_port.value.ui32 = (uint32_t)port;
	...
	for (cm_entry = cm_hd; cm_entry; cm_entry = cm_entry->x_next) {
		...
		if (cm_entry->x_wq == q) {
			...
			case T_ORDREL_REQ:
				if (cm_entry->x_dead) {
#ifdef	DEBUG
				    cmn_err(CE_NOTE, "idle timeout on dead "
					"connection: %p\n", (void *)cm_entry);
#endif
					break;
				}
...
connmgr_cpr_reset(void *arg, int code)
{
	...
	for (cxp = cm_hd; cxp; cxp = cxp->x_next) {
		if (cxp->x_family == AF_INET && cxp->x_connected == TRUE) {

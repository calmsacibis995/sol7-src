...
#pragma	ident	"@(#)svc_gen.c	1.33	97/06/26 SMI"	/* SVr4.0 1.5	*/
	...
	SVCXPRT		**nxprt)
{
	...
	if (maxthreads < 1) {
		RPCLOG(1, "svc_tli_kcreate: maxthreads (%d) must be > 0\n",
		    maxthreads);
		return (EINVAL);
	}
	...
	mutex_init(&xprt->xp_lock, NULL, MUTEX_DEFAULT, NULL);
	mutex_init(&xprt->xp_thread_lock, NULL, MUTEX_DEFAULT, NULL);
	cv_init(&xprt->xp_req_cv, NULL, CV_DEFAULT, NULL);
	...
	cv_init(&xprt->xp_dead_cv, NULL, CV_DEFAULT, NULL);
	xprt->xp_netid = NULL;
	if (netid != NULL) {

...
#pragma ident	"@(#)auth_kerb.c	1.20	98/02/02 SMI"
	...
	int calltype, AUTH **retauth)
{
	...
	*retauth = NULL;
	...
	(void) strncpy(ak->ak_service, service, ANAME_SZ);

	/* if no instance specified, make it the NULL instance */
	if (srv_inst == NULL)
		ak->ak_srv_inst[0] = '\0';
	else
		(void) strncpy(ak->ak_srv_inst, srv_inst, INST_SZ);
	...
	if (realm == NULL) {
		...
		(void) strncpy(ak->ak_realm, realm, REALM_SZ);
...
authkerb_marshal(AUTH *auth, XDR *xdrs, struct cred *cr)
{
	...
	IXDR_PUT_INT32(ixdr, ak->ak_timestamp.tv_sec);
	IXDR_PUT_INT32(ixdr, ak->ak_timestamp.tv_usec);
#ifndef NOENCRYPTION
	if (cred->akc_namekind == AKN_FULLNAME) {
#ifdef KERB_DEBUG
...
#endif /* KERB_DEBUG */
		IXDR_PUT_U_INT32(ixdr, ak->ak_window);
		IXDR_PUT_U_INT32(ixdr, ak->ak_window - 1);
#ifdef SUNDES_CRYPT
#ifdef DEBUG_CRYPT
	...
	if (cred->akc_namekind == AKN_FULLNAME) {
		...
		kprint("authkerb_marshal: FULLNAME [nocrypt] svc %s inst %s\n",
		    ak->ak_service, ak->ak_srv_inst);
#endif /* KERB_DEBUG */
		IXDR_PUT_U_INT32_T(ixdr, ak->ak_window);
		IXDR_PUT_U_INT32_T(ixdr, ak->ak_window - 1);
	} else {
#ifdef KERB_DEBUG
...
authkerb_validate(AUTH *auth, struct opaque_auth *rverf)
{
	...
	verf.akv_nickname = IXDR_GET_U_INT32(ixdr);
	...
	(void) es_ecb_encrypt((des_cblock *)&buf,
			    (des_cblock *)&buf, key_s, 0);
	/* clean up */
	 ...
	 */
	ixdr = (uint32_t *)verf.akv_xtimestamp.c;
	verf.akv_timestamp.tv_sec = IXDR_GET_INT32(ixdr) + 1;
	verf.akv_timestamp.tv_usec = IXDR_GET_INT32(ixdr);
...
authkerb_refresh(AUTH *auth, struct rpc_msg *msg, cred_t *cr)
{
	...
	bcopy(&ak->ak_ticket, &cred->akc_fullname.ticket,
		sizeof (KTEXT_ST));

...
#ident	"@(#)svcauthdes.c	1.30	97/09/22 SMI"	/* SVr4.0 1.10	*/
...
*/
	...
	*ixdr++ = (int32_t)verf.adv_int_u;
	...
	if (cred->adc_namekind == ADN_FULLNAME) {
		...
		entry->rname = mem_alloc(strlen(cred->adc_fullname.name) + 1);
		if (entry->rname != NULL) {
			(void) strcpy(entry->rname, cred->adc_fullname.name);
		} else {
			/* EMPTY */
			RPCLOG0(1, "_svcauth_des: out of memory\n");
		}
...
cache_init(void)
{
	...
	bzero(authdes_cache, sizeof (struct cache_entry) * authdes_cachesz);
...
cache_ref(short sid)
{
	struct cache_entry *curr = &authdes_cache[sid];

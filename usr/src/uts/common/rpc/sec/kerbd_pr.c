#ident	"@(#)kerbd_prot.c	1.5	97/04/29 SMI"
/*
 * Please do not edit this file.
 * It was generated using rpcgen. (not any more, apparently)
 */
 ...
 *
 * Copyright 1990-1997 Sun Microsystems, Inc.
 * All rights reserved.
 */
...
xdr_kgetkcred_resd(XDR *xdrs, kgetkcred_resd *objp)
{
	...
	if (xdrs->x_op == XDR_ENCODE) {
		...
		if (!xdr_uint32(xdrs, &objp->checksum)) {
			return (FALSE);
		}
		...
		if (buf == NULL) {
			if (!xdr_int32(xdrs, &objp->life)) {
				return (FALSE);
			}
			if (!xdr_uint32(xdrs, &objp->time_sec)) {
				return (FALSE);
			}
			if (!xdr_uint32(xdrs, &objp->address)) {
				return (FALSE);
			}
		} else {
			IXDR_PUT_INT32(buf, objp->life);
			IXDR_PUT_U_INT32(buf, objp->time_sec);
			IXDR_PUT_U_INT32(buf, objp->address);
		}
	...
	if (xdrs->x_op == XDR_DECODE) {
		...
		if (buf == NULL) {
			...
			objp->life = IXDR_GET_INT32(buf);
			objp->time_sec = IXDR_GET_U_INT32(buf);
			objp->address = IXDR_GET_U_INT32(buf);
		}
		if (!xdr_TICKET(xdrs, &objp->reply)) {
			return (FALSE);
		}
	...
	if (!xdr_u_int(xdrs, &objp->time_sec)) {
		return (FALSE);
	}
	if (!xdr_uint32(xdrs, &objp->address)) {
		return (FALSE);

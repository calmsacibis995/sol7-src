#pragma ident	"@(#)kerb_subr.c	1.10	97/04/29 SMI"
...
AUTH_STAT
kerb_get_session_cred(char *service, char *instance, uint32_t faddr,
	KTEXT ticket, authkerb_clnt_cred *cred)
{
	...
	bcopy((caddr_t)au_data.session, (caddr_t)cred->session,
		sizeof (cred->session));
	(void) strncpy(cred->pname,  au_data.pname,  ANAME_SZ);
	(void) strncpy(cred->pinst,  au_data.pinst,  INST_SZ);
	(void) strncpy(cred->prealm, au_data.prealm, REALM_SZ);

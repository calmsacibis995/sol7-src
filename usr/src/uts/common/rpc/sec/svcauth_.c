...
#pragma ident	"@(#)svcauth_kerb.c	1.19	97/04/29 SMI"
 ...
 */
int
svc_kerb_reg(SVCXPRT *xprt, rpcprog_t prog, rpcvers_t vers, char *name,
	char *inst, char *realm)
{
	int retval;
	...
	if (realm == (char *)NULL) {
		...
		(void) strncpy(mcp->realm, realm, REALM_SZ);
 ...
 */
enum auth_stat
_svcauth_kerb(struct svc_req *rqst, struct rpc_msg *msg)
{
	int32_t *ixdr;
	...
	case AKN_FULLNAME:
		...
		/* copy the ticket */
		bcopy(ixdr, ticket.dat, length);
		ixdr += (RNDUP(length) / BYTES_PER_XDR_UNIT);
		ticket.length = length;
		ticket.mbz = 0;

		window = (uint32_t)*ixdr++;	/* get the window */
	...
	verf.akv_xtimestamp.key.high = (uint32_t)*ixdr++;
	verf.akv_xtimestamp.key.low =  (uint32_t)*ixdr++;
	verf.akv_winverf = (uint32_t)*ixdr++;
	...
	if (namekind == AKN_FULLNAME) {
		...
		bcopy(cred->session, &sessionkey, sizeof (des_block));
	} else { /* AKN_NICKNAME */
		sid = cred->nickname;
		...
		bcopy(authkerb_cache[sid].clnt_cred.session, &sessionkey,
		    sizeof (des_block));
	}
	 ...
	 */
	ixdr = (int32_t *)msg->rm_call.cb_verf.oa_base;
	*ixdr++ = (int32_t)verf.akv_xtimestamp.key.high;
	*ixdr++ = (int32_t)verf.akv_xtimestamp.key.low;
	*ixdr++ = (int32_t)verf.akv_nickname;
	...
	if (namekind == AKN_FULLNAME) {
		...
		(void) strncpy(entry->clnt_cred.pname, cred->pname, ANAME_SZ);
		(void) strncpy(entry->clnt_cred.pinst, cred->pinst, INST_SZ);
		(void) strncpy(entry->clnt_cred.prealm, cred->prealm, REALM_SZ);
		entry->clnt_cred.expiry = cred->expiry;
		bcopy(&sessionkey, entry->clnt_cred.session,
		    sizeof (des_block));
		entry->window = window;
#ifdef KERB_UCRED
		 ...
		 */
		(void) strncpy(cred->pname, entry->clnt_cred.pname, ANAME_SZ);
		(void) strncpy(cred->pinst, entry->clnt_cred.pinst, INST_SZ);
		(void) strncpy(cred->prealm, entry->clnt_cred.prealm, REALM_SZ);
...
cache_init(void)
{
	...
	bzero(authkerb_cache, sizeof (struct cache_entry) * authkerb_cachesz);
...
#ifdef KERB_DEBUG
void
authkerb_cache_flush(char *name, char *instance, char *realm)
{
...
cache_ref(short sid)
{
	struct cache_entry *curr = &authkerb_cache[sid];
 ...
 */
static short
cache_spot(authkerb_clnt_cred *au, struct timeval *timestamp)
{
	struct cache_entry *cp;
	int i;
	authkerb_clnt_cred *cu;
	...
	short *grouplen, int *groups)
{
	...
	struct cache_entry *entry;
	struct bsdcred *cred;
	uint32_t sid;				/* nickname into cache */

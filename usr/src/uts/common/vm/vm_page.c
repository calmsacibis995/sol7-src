...
#pragma ident   "@(#)vm_page.c	1.189	97/10/25 SMI"
 ...
 */
int
page_create_wait(size_t npages, u_int flags)
{
	pgcnt_t		total;
	u_int		i;
	struct pcf	*p;
...
static page_t *
page_create_get_something(vnode_t *vp, u_offset_t off, struct as *as,
    caddr_t vaddr, u_int flags)
{
	u_int		count;
	page_t		*pp;
	u_int		locked, i;
 ...
 */
page_t *
page_create(vnode_t *vp, u_offset_t off, size_t bytes, u_int flags)
{
...
page_t *
page_create_va(vnode_t *vp, u_offset_t off, size_t bytes, u_int flags,
    struct as *as, caddr_t vaddr)
{
	page_t		*plist = NULL;
	pgcnt_t		npages;
...
fail:
	...
	{
		u_int overshoot = (u_int)(pages_req - found_on_free);
...
free_vp_pages(vnode_t *vp, u_offset_t off, size_t len)
{
	...
	u_offset_t eoff;
	kmutex_t *vphm;
	page_t *vp_list_head = 0, *vp_list_tail = 0;
	page_t *next_2_tail, *next_pp;
	u_int first = 1;
	...
	/*
	 * Acquire the appropriate vnode hash lock since
	 * we are going through the "vp" list.
	 */
	...
	if ((pp = epp = vp->v_pages) != 0) {
		do {
			...
			if (pp->p_offset < off || pp->p_offset >= eoff)
				continue;
			/* XXX - somebody needs to handle these (pageout) */
			if (hat_ismod(pp) || hat_page_is_mapped(pp))
				continue;
			...
			/*
			 * Relink the page so that all candidates to be
			 * disposed are linked contiguously.
			 */
			if (first) {
				vp_list_head = pp;
				vp_list_tail = pp;
				...
				if (vp_list_tail->p_vpnext != pp) {
					next_2_tail = vp_list_tail->p_vpnext;
					vp_list_tail->p_vpnext = pp;
					pp->p_vpprev->p_vpnext = pp->p_vpnext;
					pp->p_vpnext->p_vpprev = pp->p_vpprev;
					pp->p_vpprev = vp_list_tail;
					pp->p_vpnext = next_2_tail;
					next_2_tail->p_vpprev = pp;
				}
				vp_list_tail = pp;
			}
		} while ((pp = next_pp) != epp);
	}
	mutex_exit(vphm);
	if ((pp = vp_list_head) != 0) {
		/*
		 * Dispose of the pages
		 */
		do {
			next_pp = pp->p_vpnext;
			...
			pp = next_pp;
		} while (pp != vp_list_tail->p_vpnext);
	}
}
		...
		if (availrmem > pages_pp_maximum) {
			...
			if (pp->p_cowcnt < (u_short) PAGE_LOCK_MAXIMUM) {
				if (++pp->p_cowcnt == PAGE_LOCK_MAXIMUM) {
					cmn_err(CE_WARN,
					"Page frame 0x%lx locked permanently\n",
					    page_pptonum(pp));
				}
			}
			r = 1;
	...
	int cow)		/* expect cowcnt instead of lckcnt */
{
	...
	ASSERT((npp != opp) ? PAGE_LOCKED(npp) : 1);
...
page_busy(int cleanit)
{
	...
	do {
		...
		if (!PP_ISFREE(pp) && pp->p_vnode != NULL &&
			...
			VN_HOLD(vp);
			page_unlock(pp);
			VOP_PUTPAGE(vp, off, PAGESIZE, B_ASYNC | B_FREE, kcred);
			VN_RELE(vp);
		}

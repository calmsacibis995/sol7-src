...
#pragma ident	"@(#)page.h	1.106	98/01/06 SMI"
...
typedef struct page {
	...
	uchar_t p_state;		/* p_free, p_created */
} page_t;
...
int	page_exists(struct vnode *, u_offset_t);
...
page_t	*page_get_freelist(struct vnode *, u_offset_t, struct as *,
		caddr_t, size_t, uint_t flags);

page_t	*page_get_cachelist(struct vnode *, u_offset_t, struct as *,
		caddr_t, uint_t flags);

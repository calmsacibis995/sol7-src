...
#pragma ident   "@(#)seg_spt.c 1.36     98/02/04 SMI"
...
segspt_free(struct seg	*seg)
{
	...
	if (spt != NULL) {
		...
		kmem_free(spt->vp, sizeof (struct vnode));
    ...
    struct page ***ppp, enum lock_type type, enum seg_rw rw)
{
	...
	size_t page_index, npages, total_pages;
	...
	if (type == L_PAGEUNLOCK) {
		...
		atomic_add_long(&ssd->softlockcnt, -npages);
...
segspt_shmattach(struct seg *seg, caddr_t *argsp)
{
	...
	ssd = kmem_zalloc((sizeof (struct sptshm_data)), KM_NOSLEEP);
	if (ssd == NULL)
		return (ENOMEM);
...
segspt_shmfree(struct seg *seg)
{
	...
	kmem_free(seg->s_data, sizeof (struct sptshm_data));
}

...
#pragma ident	"@(#)vm_as.c	1.114	98/02/01 SMI"
...
static void as_setwatchprot(struct as *, struct seg *, caddr_t, size_t, u_int);
 ...
 * address space.  AS_CLAIMGAP must be held by the caller and it is
 * re-acquired before returning to the caller.
 */
void
as_rangewait(struct as *as, kcondvar_t *cvp)
{
	ASSERT(AS_ISCLAIMGAP(as));
 ...
 */
void
as_rangebroadcast(struct as *as, kcondvar_t *cvp)
{
	mutex_enter(&as->a_contents);
	cv_broadcast(cvp);
	mutex_exit(&as->a_contents);
 ...
 */
int
as_setprot(struct as *as, caddr_t addr, size_t size, u_int prot)
{
	struct seg *seg;
	size_t ssize;
	caddr_t raddr;			/* rounded down addr */
 ...
 */
int
as_checkprot(struct as *as, caddr_t addr, size_t size, u_int prot)
{
	struct seg *seg;
	size_t ssize;
	caddr_t raddr;			/* rounded down addr */
	...
	void *argsp)
{
	...
	if (as != &kas &&
	    as->a_size + rsize > U_CURLIMIT(&u, RLIMIT_VMEM)) {
		AS_LOCK_EXIT(as, &as->a_lock);
		return (ENOMEM);
	}
...
as_gap(struct as *as, size_t minlen, caddr_t *basep,
    size_t *lenp, u_int flags, caddr_t addr)
{
...
as_ctl(struct as *as, caddr_t addr, size_t size, int func, int attr,
    uintptr_t arg, ulong *lock_map, size_t pos)
{
	...
	while (rsize != 0) {
		 ...
		 */
		case MC_ADVISE:
			(void) SEGOP_ADVISE(seg, raddr, ssize, (u_int)arg);
			break;
...
as_exec(struct as *oas, caddr_t ostka, size_t stksz,
    struct as *nas, caddr_t nstka, u_int hatflag)
{
	struct seg *stkseg;

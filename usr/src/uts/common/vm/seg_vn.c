...
#pragma ident	"@(#)seg_vn.c	1.181	98/02/08 SMI"
 ...
 */
static struct segvn_crargs zfod_segvn_crargs = {
	NULL,
	NULL,
	0,
	MAP_PRIVATE,
...
static struct segvn_crargs kzfod_segvn_crargs = {
	NULL,
	NULL,
	0,
	MAP_PRIVATE,
...
static int	segvn_faultpage(struct hat *, struct seg *, caddr_t,
    u_offset_t, struct vpage *, page_t **, u_int,
	...
	enum seg_rw rw)			/* type of access at fault */
{
	...
	oldap = ap;
	pp = anon_private(&ap, seg, addr, opp, pageflags, svd->cred);
	if (pp == NULL) {
...
static int
segvn_setprot(struct seg *seg, caddr_t addr, size_t len, u_int prot)
{
	struct segvn_data *svd = (struct segvn_data *)seg->s_data;
...
static int
segvn_checkprot(struct seg *seg, caddr_t addr, size_t len, u_int prot)
{
	struct segvn_data *svd = (struct segvn_data *)seg->s_data;
	struct vpage *vp, *evp;
...
static int
segvn_getprot(struct seg *seg, caddr_t addr, size_t len, u_int *protv)
{
	struct segvn_data *svd = (struct segvn_data *)seg->s_data;
 ...
 */
static int
segvn_sync(struct seg *seg, caddr_t addr, size_t len, int attr, u_int flags)
{
	struct segvn_data *svd = (struct segvn_data *)seg->s_data;
	struct vpage *vpp;
	page_t *pp;
 ...
 */
static int
segvn_advise(struct seg *seg, caddr_t addr, size_t len, u_int behav)
{
	struct segvn_data *svd = (struct segvn_data *)seg->s_data;
	size_t page;
	int err = 0;
    ...
    enum lock_type type, enum seg_rw rw)
{
	...
	*ppp = pl = pplist;
	...
	if (a >= addr + len) {
		seg_pinsert(seg, addr, len, pl, rw, segvn_reclaim);
		SEGVN_LOCK_EXIT(seg->s_as, &svd->lock);

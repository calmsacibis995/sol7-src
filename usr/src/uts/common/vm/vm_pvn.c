...
#pragma ident	"@(#)vm_pvn.c	1.151	97/05/19 SMI"
...
pvn_read_kluster(
	...
	int isra)
{
	register ssize_t deltaf, deltab;
	register page_t *pp;
	page_t *plist = NULL;
	spgcnt_t pagesavail;
	u_offset_t vp_end;
	...
	if ((plist = page_create_va(vp, off,
	    PAGESIZE, PG_EXCL | PG_WAIT, seg->s_as, addr)) == NULL)
		return ((page_t *)NULL);
		...
		*lenp = MIN(vp_len, PAGESIZE);
		...
		for (deltab = PAGESIZE; off >= vp_off + deltab;
			...
			if ((pp = page_create_va(vp, off - deltab,
			    PAGESIZE, PG_EXCL, seg->s_as, addr - deltab))
			    == NULL)
		...
		for (deltaf = PAGESIZE; off + deltaf < vp_end;
			...
			if ((pp = page_create_va(vp, off + deltaf,
			    PAGESIZE, PG_EXCL, seg->s_as, addr + deltaf))
			    == NULL)

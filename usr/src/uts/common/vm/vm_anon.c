...
#pragma ident	"@(#)vm_anon.c	1.136	98/02/23 SMI"
			...
			ulong_t new_idx, size_t size)
{
	...
	npages = btopr(size);
	while (npages-- > 0) {
		if ((ap = anon_get_ptr(old, old_idx)) != NULL) {
			(void) anon_set_ptr(new, new_idx, ap, ANON_SLEEP);
			ahm = &anonhash_lock[AH_LOCK(ap->an_vp, ap->an_off)];
			mutex_enter(ahm);
			ap->an_refcnt++;
			mutex_exit(ahm);
...
anon_free(struct anon_hdr *ahp, ulong_t index, size_t size)
{
	...
	while (npages-- > 0) {
		if ((ap = anon_get_ptr(ahp, index)) != NULL) {

...
#pragma ident	"@(#)vm_seg.c	1.53	98/01/13 SMI"
...
static long seg_pwindow;	/* max # of pages that can be cached */
static long seg_plocked;	/* # of pages which are cached by pagelock */
int seg_preapahead;
...
#define	p_hash(seg) \
	(P_HASHMASK & \
	(((uintptr_t)(seg) >> P_BASESHIFT) & P_PMASK))
...
seg_alloc(struct as *as, caddr_t base, size_t size)
{
	...
	if (as != &kas &&
	    !valid_usr_range(segbase, segsize, as->a_userlimit))
		return ((struct seg *)NULL);	/* bad virtual addr range */

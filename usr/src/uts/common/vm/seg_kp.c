...
#pragma	ident	"@(#)seg_kp.c	1.59	98/02/04	SMI"
...
static int	segkp_pagelock(struct seg *seg, caddr_t addr, size_t len,
			...
			enum seg_rw rw);
static void	segkp_freevm(struct seg *seg, caddr_t base, size_t len);
...
static caddr_t	segkp_getvm(struct seg *seg, size_t size, int wait);
static caddr_t	segkp_get_internal(struct seg *seg, size_t len, u_int flags,
...
static int	segkp_unlock(struct hat *hat, struct seg *seg, caddr_t vaddr,
			size_t len, struct segkp_data *kpd, u_int flags);
 ...
 * in red_deep_lbolt and red_deep_thread repsectively.
 */
	...
	struct segkp_data **tkpd;
{
	...
	if ((flags & KPD_NO_ANON) == 0) {
		...
		anon_idx = (vbase - seg->s_base) >> PAGESHIFT;
	...
	for (i = 0, va = vbase; i < np; i++, va += PAGESIZE) {
		...
		if (kpd->kp_anon != NULL) {
			...
			ASSERT(!page_exists(&kvp, (u_offset_t)va));

			if ((pp = page_create_va(&kvp, (u_offset_t)va,
	...
	size_t len;
{
	...
	for (i = 0; i < np; i++, va += PAGESIZE) {
		...
		if (kpd->kp_anon) {
			...
			anon_free(kpd->kp_anon, kpd->kp_anon_idx + i, PAGESIZE);
			anon_unresv(PAGESIZE);
			TRACE_5(TR_FAC_VM,
			    TR_ANON_SEGKP, "anon segkp:%u %u %u %u %u",
			    kpd, curproc->p_pid, va, PAGESIZE, 0);
		} else {
			pp = page_lookup(&kvp, (u_offset_t)va, SE_EXCL);
			if (kpd->kp_flags & KPD_LOCKED) {
				if (pp == NULL) {
					cmn_err(CE_PANIC,
					    "segkp_release: no page to unlock");
...
segkp_map_red(void)
{
	...
	if (curthread->t_red_pp == NULL) {
		...
		caddr_t red_va = (caddr_t)
		    (((uintptr_t)curthread->t_stkbase & PAGEMASK) - PAGESIZE);
		 ...
		 */
		red_pp = page_create_va(&kvp, (u_offset_t)red_va,
		    PAGESIZE, PG_WAIT, &kas, red_va);
		ASSERT(red_pp != NULL);
		...
		/*
		 * We're locking down the mapping resources, so we need to
		 * increment the page's lckcnt.  cow isn't going to happen,
		 * and this is the kernel (hence the 0 and 1, respectively).
		 * Under these conditions, page_pp_lock() can't fail;
		 * assert that it doesn't.
		 */
#ifdef DEBUG
		res = page_pp_lock(red_pp, 0, 1);
		ASSERT(res != 0);
#else
		(void) page_pp_lock(red_pp, 0, 1);
#endif
	...
	stkbase =
	    (caddr_t)(((uintptr_t)curthread->t_stkbase & PAGEMASK) - PAGESIZE);
...
segkp_unmap_red(void)
{
	 ...
	 * hat_unload() to explicitly unlock the mapping resources, and
	 * page_pp_unlock() (cow is 0, kernel is 1) to decrement the lckcnt.
	 */
	hat_unload(kas.a_hat, red_va, PAGESIZE, HAT_UNLOAD_UNLOCK);

	(void) page_lock(curthread->t_red_pp, SE_EXCL, NULL, P_RECLAIM);
	page_pp_unlock(curthread->t_red_pp, 0, 1);

	/*
	 * Destroy the page, with dontfree set to zero (i.e. free it).
	 */
	page_destroy(curthread->t_red_pp, 0);
	...
	u_int flags;
{
	 ...
	 */
	va = (caddr_t)((uintptr_t)vaddr & PAGEMASK);
	vaddr = va;
	vlim = va + len;
	lock = flags & KPD_LOCKED;
	i = (va - kpd->kp_base) >> PAGESHIFT;
	...
	int wait;
{
	...
	if (wait)
		return ((caddr_t)rmalloc_wait(kpsd->kpsd_kpmap, size));
	else
		return ((caddr_t)rmalloc(kpsd->kpsd_kpmap, size));
}

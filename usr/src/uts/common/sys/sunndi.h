...
#pragma ident	"@(#)sunndi.h	1.12	98/01/06 SMI"
...
/*
 * Called to configure children of a hot-plugged adapter driver
 */
void
ndi_nexus_config_children(struct devnames *dnp, dev_info_t *dip);

/*
 * Hot-plug daemon operations
 */
typedef struct hp_daemon_op {
	int			hpd_op_type;
	struct hp_daemon_op	*hpd_op_next;
	union {
		struct {
			struct devnames	*hpd_op_dnp;
			dev_info_t	*hpd_op_dip;
		} cc;
	} u_op;
} hp_daemon_op_t;

#define	HPD_OP_CONFIG_CHILDREN	0x01

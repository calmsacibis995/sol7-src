...
#pragma ident	"@(#)socketvar.h	1.36	98/01/06 SMI"	/* SVr4.0 1.3 */
...
/*
 * Alignment check for the start of TPI messages.
 */
#define	ALIGN_TPI_size		(sizeof (t_scalar_t))
#define	ALIGN_TPI_opthdr_size	(sizeof (t_scalar_t))
#define	ISALIGNED_tpi_prim_t(addr) \
		(((uintptr_t)(addr) & (ALIGN_TPI_size - 1)) == 0)
#define	ISALIGNED_tpi_opthdr(addr) \
		(((uintptr_t)(addr) & (ALIGN_TPI_opthdr_size - 1)) == 0)

#define	ROUNDUP_tpi_opt(len) \
	(((len) + ALIGN_TPI_opthdr_size - 1) & ~(ALIGN_TPI_opthdr_size - 1))


#define	ISALIGNED_cmsghdr(addr) \
		(((uintptr_t)(addr) & (_MAX_ALIGNMENT - 1)) == 0)

#define	ROUNDUP_cmsglen(len) \
	(((len) + _MAX_ALIGNMENT - 1) & ~(_MAX_ALIGNMENT - 1))
...
/* solisten() flags */
#define	_SOLISTEN_XPG4_2	0x01	/* xpg4.2 semantics */
...
int		so_addr_verify(struct sonode *, struct sockaddr *,
			socklen_t);
...
int		solisten(struct sonode *, int, int);

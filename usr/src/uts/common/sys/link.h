...
#pragma ident	"@(#)link.h	1.36	98/02/10 SMI"	/* SVr4.0 1.9	*/
...
#ifdef _SYSCALL32
struct r_debug32 {
	int32_t		r_version;	/* debugging info version no. */
	caddr32_t	r_map;		/* address of link_map */
	uint32_t	r_brk;		/* address of update routine */
	r_state_e	r_state;
	uint32_t	r_ldbase;	/* base addr of ld.so */
	caddr32_t	r_ldsomap;	/* address of ld.so.1's link map */
	rd_event_e	r_rdevent;	/* debug event */
	rd_flags_e	r_flags;	/* misc flags. */
};
#endif

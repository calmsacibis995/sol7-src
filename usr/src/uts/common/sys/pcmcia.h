...
#pragma ident	"@(#)pcmcia.h	1.47	98/01/06 SMI"
...
typedef struct pcmcia_if {
	ulong_t	  pcif_magic;	/* magic number to verify correct scructure */
	ulong_t	  pcif_version;
...
typedef struct pcmcia_cs {
	ulong_t	pccs_magic;	/* magic number of verify correct structure */
	ulong_t	pccs_version;
	int   (*pccs_callback)();
	int   (*pccs_getconfig)();
} pcmcia_cs_t;
...
struct pcmcia_adapter {
	...
	uint_t		pca_avail_intr;
	inthandler_t	pca_int_handlers;
	struct ramap	*pca_io;
	struct ramap	*pca_mem;
};
...
#define	PCA_IO_OPENED		0x0100 /* nexus is opened */
 ...
 */
#define	PC_REG_RELOC(x)		((((ulong_t)x) & 0x1) << 31)
...
#define	PC_REG_PHYS_HI(n, p, t, c, s, f, r) (ulong_t)( \
			PC_REG_RELOC(n) | \
			PC_REG_PREFETCH(p) | \
			PC_REG_TYPE(t) | \
			PC_REG_SPACE(c) | \

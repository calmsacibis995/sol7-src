...
#pragma ident	"@(#)strsubr.h	1.103	98/01/06 SMI"	/* SVr4.0 1.17	*/
...
struct syncq {
	...
	struct _sq_save {
		...
		void	*l5;		/* saved sparc %l5 reg */
					...
					/* optimized putnext */
	ushort_t	sq_count;	/* # threads inside */
	ushort_t	sq_flags;	/* state and some type info */
	mblk_t		*sq_head;	/* queue of deferred messages */
	mblk_t		*sq_tail;	/* queue of deferred messages */
	ushort_t	sq_type;	/* type (concurrency) of syncq */
	...
	/*
	 * Array of threads that are draining this syncq.  This is
	 * used to prevent recursion in PERMOD syncq's.  The
	 * sq_max_threads value is the size of the sq_threadlist[]
	 * array.
	 */
	uint_t		sq_max_threads;
	kthread_id_t	*sq_threadlist;
};
...
extern void outer_enter(syncq_t *outer, uint32_t flags);
...
extern void putnext_tail(syncq_t *, mblk_t *, uint32_t, size_t);

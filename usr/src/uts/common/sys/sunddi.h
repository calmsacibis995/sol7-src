...
#pragma ident	"@(#)sunddi.h	1.86	98/01/24 SMI"
...
/*
 * new, non-exported interface for handling self identifying
 * devices that don't have Fcode or other system PROM support
 * The semantics for these types of nodes are not fully defined
 * at this time other than the nodes were not created by OBP/OF
 * or similar mechanism but the nodes do persist unlike "pseudo"
 * nodes.
 */
#define	DEVI_SID_NODEID		((int)-2)	/* SID hardware without PROM */

/*
 * test if node is a PROM supported node
 */
#define	DEVI_PROM_NODE(nodeid)	((nodeid) != DEVI_PSEUDO_NODEID && \
						(nodeid) != DEVI_SID_NODEID)

#define	DEVI_PSEUDO_NEXNAME	"pseudo"
#define	DEVI_ISA_NEXNAME	"isa"
#define	DEVI_EISA_NEXNAME	"eisa"
#define	DEVI_MCA_NEXNAME	"mc"
#define	CLONE_DEV	1
...
#define	DDI_NT_NET	"ddi_network"		/* network devices */
...
#define	DDI_NT_NEXUS	"ddi_nexus"		/* nexus drivers */
...
devmap_devmem_setup(devmap_cookie_t dhp, dev_info_t *dip,
	struct devmap_callback_ctl *callback_ops, uint_t rnumber,
	offset_t roff, size_t len, uint_t maxprot, uint_t flags,
	ddi_device_acc_attr_t *accattrp);

int
devmap_umem_setup(devmap_cookie_t dhp, dev_info_t *dip,
	struct devmap_callback_ctl *callback_ops, ddi_umem_cookie_t cookie,
	offset_t off, size_t len, uint_t maxprot, uint_t flags,
	ddi_device_acc_attr_t *accattrp);

int
devmap_devmem_remap(devmap_cookie_t dhp, uint_t rnumber, offset_t roff,
	size_t len, uint_t maxprot, ddi_device_acc_attr_t *accattrp);

int
devmap_umem_remap(devmap_cookie_t dhp, ddi_umem_cookie_t cookie,
	offset_t off, size_t len, uint_t maxprot,
	ddi_device_acc_attr_t *accattrp);
...
#define	DDI_REGS_ACC_INVALID	(-11)

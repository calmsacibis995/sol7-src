...
#pragma ident	"@(#)pcic_var.h	1.28	98/01/06 SMI"
...
typedef struct pcic_pm_t {
	int		state;	/* state */
	unsigned long	ptime;	/* previous time check */
...
typedef struct pcicdev_t {
	...
	uint_t			pc_type;
	char			*pc_chipname;
	uint_t			pc_irqs;	/* the possible IRQ levels */
	uint_t			pc_smi;		/* SMI IRQ */
	uint_t			pc_irq;		/* IO IRQ */
	int			pc_io_type;
	...
	ulong_t			pc_timestamp; /* last time touched */
...
#define	PCI_ID(vend, dev)	((ulong_t)(((uint_t)(vend) << 16) | (dev)))
...
#define	PCIC_PCI_CLASS(class, subclass)	(((class) << 16) | ((subclass) << 8))
...
#define	mhztons(c)		(1000000 / (long)((c) * 1000))

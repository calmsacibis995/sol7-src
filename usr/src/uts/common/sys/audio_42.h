...
#pragma ident	"@(#)audio_4231_dma.h	1.9	98/01/06 SMI"

/*
 * This file contains platform-specific definitions for the various
 * DMA controllers used to support the CS4231 on SPARC/x86 platforms.
 */
 ...
 * used as the system DMA engine on the x86 platform.
 * XX64 - not LP64-safe, contains pointers.
 */

struct i8237_dma {
	uint32_t pb_chan;		/* Playback DMA channel */
	uint32_t rc_chan;		/* Record DMA channel */
	uint32_t buflim;		/*  */

	uchar_t	*PBbuffer[2];		/* pointers to PB I/O buffers */
	int32_t	PBlength[2];		/* length of data in PB buffers */
	uint32_t PBbuflen;		/* length of allocated PB buffers */
	uint32_t PBbufnum;		/* buffer being transferred */
	aud_cmd_t *PB_paused_buf;	/* buffer paused while playing */
	caddr_t	PB_paused_ptr;		/* pointer to next data to load */
	uint8_t *RCbuffer[2];		/* pointers to RC I/O buffers */
	int32_t RClength[2];		/* length of data in RC buffers */
	uint32_t RCbuflen;		/* length of allocated RC buffers */
	int32_t bytecount[2];		/* transfer byte count */
	int32_t samplecount[2];		/* transfer sample count */

	ushort_t flags;			/* used for DMA double-buffering */
};

...
#pragma ident	"@(#)kmem_impl.h	1.8	98/01/06 SMI"
...
typedef struct kmem_backend {
	void		*(*be_page_alloc)(size_t, uint_t);
	void		(*be_page_free)(void *, size_t);
	size_t		be_pagesize;
	int		be_pageshift;
	int		be_memclass;
	pgcnt_t		be_pages_inuse;
	int		be_clients;
	kstat_t		*be_kstat;
	uint_t		be_alloc[KMEM_PAGE_BUCKETS];
	pgcnt_t		be_inuse[KMEM_PAGE_BUCKETS];
} kmem_backend_t;
...
typedef struct kmem_cache {
	...
	int		cache_global_alloc;	/* total global allocations */
	int		cache_global_free;	/* total global frees */
	int		cache_alloc_fail;	/* total failed allocations */

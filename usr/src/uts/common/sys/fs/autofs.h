...
#pragma ident	"@(#)autofs.h	1.21	98/02/13 SMI"
 ...
 *		 fn_rwlock.
 *
 * 	fn_rwlock: readers/writers lock to protect the subdirectory.
 *		   Protects: fn_dirents
 *		             fn_size
 *		             fn_linkcnt
 *                 - Grab readers when checking if certain fn_node exists
 *                   under fn_dirents.
 *                 - Grab writers to add a new fn_node under fn_dirents.
 *
 * The flags:
 *	MF_INPROG:
...
typedef struct fnnode {
	...
	cred_t		*fn_cred;		/* pointer to cred, used for */
						/* "thisuser" processing */
	krwlock_t	fn_rwlock;		/* protects fn_dirents list */

...
#pragma ident	"@(#)ufs_log.h	1.33	98/02/06 SMI"
...
typedef struct mt_map {
	...
	u_long		mtm_debug;	/* set at create time */
	u_long		mtm_age;	/* mono-inc; tags mapentries */
	mapentry_t	*mtm_cancel;	/* to be canceled at commit */
	u_long		mtm_nhash;	/* # of hash anchors */
 ...
 */
struct topstats {
	u_long		mtm_top_num[TOP_MAX];
	u_long		mtm_top_size_etot[TOP_MAX];
	u_long		mtm_top_size_rtot[TOP_MAX];
	u_long		mtm_top_size_max[TOP_MAX];
	u_long		mtm_top_size_min[TOP_MAX];
	u_long		mtm_delta_num[DT_MAX];
};
...
#define	LUFS_ENABLE(ufsvfsp, fl, cr, error) \
	error = (*lufsops.lufs_enable)(ufsvfsp, fl, cr)

#define	LUFS_DISABLE(ufsvfsp, fl, error) \
	error = (*lufsops.lufs_disable)(ufsvfsp, fl)
...
extern u_long		ldl_logscan_nbcommit(off_t);
...
extern void	top_begin_sync(struct ufstrans *, top_t, u_long);
extern void	top_end_sync(struct ufstrans *, int *, top_t, u_long);
extern void	top_read_roll(struct buf *, ml_unit_t *, u_short *);
...
extern void		logmap_secmap_roll(mapentry_t *, offset_t, u_short *);
...
extern int	top_begin_debug(ml_unit_t *, top_t, u_long);
extern int	top_end_debug_1(ml_unit_t *, mt_map_t *, top_t, u_long);

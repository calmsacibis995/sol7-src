...
#pragma ident	"@(#)snode.h	1.33	98/01/06 SMI"
...
struct snode {
	...
	/*
	 * Updated atomicly. No lock required.
	 */
	ulong_t s_nextr;		/* next byte read offset (read-ahead) */

	/* These fields are protected by spec_syncbusy */
	struct	snode *s_list;		/* used for syncing */
	/* These fields are protected by s_lock */
	size_t	s_size;			/* block device size in bytes */

...
#pragma	ident	"@(#)rlimit.c	1.17	97/09/22 SMI"	/* from SVr4.0 1.78 */
...
ulimit(int cmd, long arg)
{
	...
	case UL_GMEMLIM: /* Return maximum possible break value. */
	{
		 ...
		 */
		ASSERT32(U_CURLIMIT(up, RLIMIT_DATA) <= UINT32_MAX);

		size = (size_t)U_CURLIMIT(up, RLIMIT_DATA);
		 ...
		 */
		if (!UNLIMITED_CUR(up, RLIMIT_VMEM)) {
			/*
			...
			size = (size_t)(U_CURLIMIT(up, RLIMIT_VMEM) & PAGEMASK);
...
getrlimit32(int resource, struct rlimit32 *rlp)
{
	...
	if (rlim64.rlim_max > (rlim64_t)UINT32_MAX) {
		...
		if (resource == RLIMIT_FSIZE) {
			mutex_enter(&p->p_lock);
			if (savemax)
				up->u_saved_rlimit.rlim_max = rlim64.rlim_max;
			if (savecur)
				up->u_saved_rlimit.rlim_cur = rlim64.rlim_cur;
			mutex_exit(&p->p_lock);
		}

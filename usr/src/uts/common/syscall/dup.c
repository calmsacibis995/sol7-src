...
#ident	"@(#)dup.c	1.5	98/02/18 SMI"	/* SVr4 1.103	*/
...
dupdest(file_t *fp, int dest)
{
	...
	if (dest < up->u_nofiles) {
		...
		while (ufp->uf_pofile & FRESERVED2) {
			cv_wait_stop(&p->p_flock2, &up->u_flock);
			ufp = &up->u_flist[dest];
		}
	...
	ufp = &up->u_flist[dest];
	ufp->uf_ofile = fp;
	ufp->uf_pofile &= ~(FRESERVED|FCLOSING|FCLOSEXEC);

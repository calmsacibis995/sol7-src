...
#pragma ident	"@(#)poll.c	1.98	98/02/18 SMI"	/* SVr4 1.103	*/
 ...
 */
int
poll(pollfd_t *fds, unsigned long nfds, int time_out)
{
	int i, fdcnt = 0;
	clock_t start;
	timeout_id_t id;
	...
	if ((ps = curthread->t_pollstate) == NULL) {
		...
		mutex_init(&ps->ps_lock, NULL, MUTEX_DEFAULT, NULL);
		mutex_init(&ps->ps_no_exit, NULL, MUTEX_DEFAULT, NULL);
		cv_init(&ps->ps_cv, NULL, CV_DEFAULT, NULL);
		cv_init(&ps->ps_busy_cv, NULL, CV_DEFAULT, NULL);
	...
	while (nfds == 0) {
		...
		if (time_out >= 0) {
			 ...
			 * Note: this may overflow if hz > MILLISEC!
			 */
			clock_t hz_timo = MSEC_TO_TICK_ROUNDUP(time_out) -
				(lbolt - start);
			...
			id = realtime_timeout(polltime, curthread, hz_timo);
		}
		...
		/*
		 * If we were woken by the expiration of the timer
	...
	/*
	 * Make a quick pass without holding any locks to see if we found
	 * data on any fd.  If not then we will lock the thread to prevent
	 * a race condition with wakeup and scan once again.  The second
	 * time we will also need to lock the pollhead structure before we
	 * do a VOP_POLL.  We store the pollhead pointers for all fd's on
	 * first quick pass.
	 */
	if (p->p_lwptotal == 1) {
		/*
		 * Single-threaded case.  Don't use getf()/releasef().
		 * We still have to be careful about the u_flist pointer
		 * and reference counts because /proc might sneak in and
		 * stop us in one of the VOP_POLL() calls and create an
		 * agent lwp while we are stopped.  When we return from
		 * VOP_POLL() we know that the agent lwp no longer exists
		 * and that we are single-threaded again.
		 */
		int nofiles = up->u_nofiles;	/* up->u_nofiles only grows */

		for (i = 0; i < nfds; i++) {
			int fd = pollfdp[i].fd;
			pollhead_t *memphp = NULL;
			struct uf_entry *pufp;
			file_t *fp;
			...
			pufp = &up->u_flist[fd];
			if ((fp = pufp->uf_ofile) == NULLFP) {
				pollfdp[i].revents = POLLNVAL;
				fdcnt++;
				continue;
			}
			pufp->uf_refcnt++;
			error = VOP_POLL(fp->f_vnode, pollfdp[i].events,
				fdcnt, &pollfdp[i].revents, &memphp);
			ASSERT(p->p_lwptotal == 1);

			/* be careful here -- up->u_flist may have changed */
			up->u_flist[fd].uf_refcnt--;
		...
		/*
		 * Multi-threaded case.  Use getf()/releasef().
		 */
		for (i = 0; i < nfds; i++) {
			int fd = pollfdp[i].fd;
			pollhead_t *memphp = NULL;
...
retry:
	...
	for (i = 0; i < nfds; i++) {
		...
		if (fdcnt == 0) {
			/*
			 * The case of fdcnt != or == 0 is different because
			 * we don't need to do any locking before VOP_POLL
			 * if fdcnt != 0.  We will never put this thread on
			 * the php returned by the VOP_POLL call in such a
			 * situation.  So the need to lock the php before
			 * VOP_POLL does not exist.
			 */
			...
			/*
			 * The value in memphp is don't care since we do not
			 * plan to put this thread of that php anyway.
			 */
			if (error) {
				polldel(ps);
				mutex_exit(&ps->ps_lock);
...
ppwaitemptylist(pollhead_t *php)
{
	...
	while (php->ph_list != NULL) {
		pl->pp_flag = 1;
		cv_wait_stop(&pl->pp_wait_cv, &pl->pp_lock);
	}

...
#pragma	ident	"@(#)sem.c	1.56	97/09/09 SMI" /* from S5R4 1.17 */
...
_init(void)
{
	...
	rmfree(semmap, seminfo.semmns, (ulong_t)1);
...
semctl(int semid, uint semnum, int cmd, uintptr_t arg)
{
	...
	case IPC_RMID:
		...
		rmfree(semmap, (size_t)sp->sem_nsems,
...
semget(key_t key, int nsems, int semflg)
{
	...
	if (s) {
		...
		if ((i = rmalloc(semmap, (size_t)nsems)) == NULL) {
			sp->sem_perm.mode = 0;

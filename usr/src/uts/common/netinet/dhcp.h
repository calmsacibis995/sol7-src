...
#pragma ident	"@(#)dhcp.h	1.16	98/01/06 SMI"
...
typedef struct dhcp {
	uchar_t		op;		/* message opcode */
	uchar_t		htype;		/* Hardware address type */
	uchar_t		hlen;		/* Hardware address length */
	uchar_t		hops;		/* Used by relay agents */
	ulong_t		xid;		/* transaction id */
	ushort_t	secs;		/* Secs elapsed since client boot */
	ushort_t	flags;		/* DHCP Flags field */
	...
	uchar_t		chaddr[16];	/* Client hardware addr */
	uchar_t		sname[64];	/* Optl. boot server hostname */
	uchar_t		file[128];	/* boot file name (ascii path) */
	uchar_t		cookie[4];	/* Magic cookie */
	uchar_t		options[60];	/* Options */
} PKT;
...
typedef struct  dhcp_list {
	...
	uchar_t			offset;		/* BOOTP packet offset */
				/*
 ...
 */
#define	DISCOVER	((uchar_t)1)
#define	OFFER		((uchar_t)2)
#define	REQUEST		((uchar_t)3)
#define	DECLINE		((uchar_t)4)
#define	ACK		((uchar_t)5)
#define	NAK		((uchar_t)6)
#define	RELEASE		((uchar_t)7)
#define	INFORM		((uchar_t)8)

/*
 * Generic DHCP protocol defines
 */
#define	DHCP_PERM	((ulong_t)0xffffffff)	/* "permanent" lease time */

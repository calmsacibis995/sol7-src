...
#pragma	ident	"@(#)modsubr.c	1.63	98/01/30	SMI"
...
static void impl_free_parlist(struct devnames *dnp);
...
static struct hwc_spec *gather_named_props(struct hwc_spec *hwc);
...
install_stub(struct mod_stub_info *stub)
{
	...
	if (stub->mods_errfcn == NULL) {
		...
		return ((uintptr_t)stub->mods_errfcn);
	}
}
...
make_node(dev_info_t *dev_info, struct modconfig *mc)
{
	...
	(void) sprintf(node, "%s", mc->rootdir);
	p = node + strlen(node);
	...
	*p = '\0';
	p++;
	dot_node = kmem_alloc(NODE_PREPEND_LEN + 256, KM_SLEEP);
	(void) sprintf(dot_node, "%s/%s%s", node, NODE_PREPEND, p);
	p--;
	*p = '/';	/* restore node - we need it later */
	...
	for (dmdp = (struct ddi_minor_data *)
		...
		name = dmdp->ddm_name;
		dprintf("minor data: %s %s %x %x %x %p\n",
		    ddi_node_name(dev_info), name,
		    getmajor(dmdp->ddm_dev),
		    getminor(dmdp->ddm_dev),
		    dmdp->ddm_spec_type, (void *)dmdp->ddm_node_type);

		(void) sprintf(p, ":%s", name);
		 ...
		 */
		if (create_dot_file) {
			(void) sprintf(q, ":%s", name);

			if ((error = vn_create(dot_node, UIO_SYSSPACE, &vattr,
			    EXCL, 0, &vp, CRMKNOD, 0, 0)) == EEXIST) {
	...
	kmem_free(dot_node, 256 + NODE_PREPEND_LEN);
	return (0);
}
...
static struct hwc_spec *
gather_named_props(struct hwc_spec *hwc)
{
	struct hwc_spec *tmp, *tmp2;
	struct hwc_spec *devinfo_hwc = NULL;	/* returned list */
	struct hwc_spec *devinfo_hwc_tail = NULL;
...
impl_make_parlist(major_t major)
{
	...
	(void) sprintf(confname, "drv/%s.conf", ddi_major_to_name(major));
    ...
    struct par_list *pl, int *unitp)
{
	...
	for (hp = pl->par_specs; hp != NULL; hp = hp->hwc_next) {
		if (match_parent(devi, devi_driver_name,
			...
			ASSERT(DEVI(proto)->devi_drv_prop_ptr == NULL);
...
impl_unattach_devs(major_t major)
{
	...
	dnp->dn_flags &= ~(DN_DEVI_MADE | DN_CONF_PARSED);
}
...
{
	struct bind *bp1;
	size_t namelen, bind_namelen;
	int hashndx;
	static char nem[] = "Not enough memory for binding file";

	bp1 = kmem_zalloc(sizeof (struct bind), KM_NOSLEEP);
	if (bp1 == NULL) {
		cmn_err(CE_PANIC, nem);
		/*NOTREACHED*/
	}
	namelen = strlen(name);
	...
	if (bind_name != NULL) {
		...
		(void) strcpy(bp1->b_bind_name, bind_name);
	} else
...
init_devnamesp(int size)
{
	...
	for (i = 0, dp = devnamesp; i < size; ++i, ++dp)  {
		mutex_init(&(dp->dn_lock), NULL, MUTEX_DEFAULT, NULL);
		cv_init(&(dp->dn_wait), NULL, CV_DEFAULT, NULL);
	}
...
init_syscallnames(int size)
{
	...
	syscallnames = kobj_zalloc(size * sizeof (char *), KM_NOSLEEP);
...
make_all_nodes(struct modconfig *mc)
{
	...
	for (i = 0; i < devcnt; i++) {
		if (load_allp[i]) {
			...
			e_ddi_enter_driver_list(&devnamesp[i], &listcnt);
			...
			LOCK_DEV_OPS(&(devnamesp[i].dn_lock));
			e_ddi_exit_driver_list(&devnamesp[i], listcnt);
			UNLOCK_DEV_OPS(&(devnamesp[i].dn_lock));
			ddi_rele_driver(i);
		}
	}
...
void
modvprintf(const char *fmt, va_list adx)
{
...
mod_containing_pc(caddr_t pc, char *modname)
{
	...
	do {
		...
		if ((pc >= m->text) &&
			...
			(void) strcpy(modname, mcp->mod_modname);
			mutex_exit(&mod_lock);
			return (1);
		}
	} while (mcp != &modules);
...
struct par_list *
impl_read_driver_conf(major_t major)
{
	struct hwc_spec	*hp;
	struct par_list	*pl = NULL;
...
void
impl_free_driver_conf(struct par_list *pl)
{

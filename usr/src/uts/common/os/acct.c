...
#pragma	ident	"@(#)acct.c	1.35	98/01/30 SMI" /* from SVR4.0 1.18 */
...
acct(char st)
{
	...
	acctbuf.ac_io = compress((u_long)p->p_ru.ioch);
	acctbuf.ac_rw = compress((u_long)(p->p_ru.inblock + p->p_ru.oublock));
	cr = CRED();

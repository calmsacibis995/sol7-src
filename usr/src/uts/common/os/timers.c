...
#pragma ident	"@(#)timers.c	1.63	97/10/22 SMI"
...
/*
 * Get value of an interval timer.  The process virtual and profiling
 * virtual time timers are kept in the lwp struct, since they can be
 * swapped out.  These are kept internally in the way they are specified
 * externally: in time until they expire.
 *
 * The real time interval timer is kept in the thread struct for the lwp,
 * and its value (it_value) is kept as an absolute time rather than as a
 * delta, so that it is easy to keep periodic real-time signals from
 * drifting.  The same is true for the real time profiling timer, except
 * that it is kept in the proc structure because it affects all lwps in
 * the process.
 *
 * Virtual time timers are processed in the hardclock() routine of
 * kern_clock.c.  The real time timers are processed by timeout routines,
 * called from the softclock() routine.  Since a callout may be delayed
 * in real time due to interrupt processing in the system, it is possible
 * for the real time timeout routines (realitexpire and realprofexpire,
 * given below), to be delayed in real time past when they are supposed
 * to occur.  It does not suffice, therefore, to reload the real timer
 * .it_value from the real time timers .it_interval.  Rather, we compute
 * the next time in absolute time the timer should go off.
 */
int
...
int
xgetitimer(u_int which, struct itimerval *itv, int iskaddr)
{
	...
	if (which == ITIMER_REAL || which == ITIMER_REALPROF) {
		uniqtime(&now);
		/*
...
int
setitimer(u_int which, struct itimerval *itv, struct itimerval *oitv)
{
	int error;
...
int
xsetitimer(u_int which, struct itimerval *itv, int iskaddr)
{
	struct itimerval aitv;
	struct timeval now;
	struct proc *p = curproc;
	...
	case ITIMER_REAL:
		...
		if (timerisset(&aitv.it_value)) {
			...
			curthread->t_itimerid = realtime_timeout(realitexpire,
			    curthread, hzto(&aitv.it_value));
		}
		curthread->t_realitimer = aitv;
		break;
	case ITIMER_REALPROF:
		while (p->p_rprof_timerid) {
		...
		if (timerisset(&aitv.it_value)) {
			...
			do {
				if (t->t_rprof == NULL) {
					t->t_rprof = kmem_zalloc(
					    sizeof (struct rprof), KM_NOSLEEP);
	...
	for (;;) {
		...
		if (TVTSCMP(&t->t_realitimer.it_value, &hrestime, >)) {
			t->t_itimerid = realtime_timeout(realitexpire,
			    t, hzto(&t->t_realitimer.it_value));
			mutex_exit(&p->p_lock);
			return;
		}
	}
...
static void
realitsexpire(void *arg)
{
	timerstr_t *timerp = arg;
	int cnt2nth;
	kthread_t *t;
	struct proc *p;
...
realprofexpire(void *arg)
{
	...
	for (;;) {
		...
		if (TVTSCMP(&p->p_rprof_timer.it_value, &hrestime, >)) {
			p->p_rprof_timerid = realtime_timeout(realprofexpire,
			    p, hzto(&p->p_rprof_timer.it_value));
			mutex_exit(&p->p_lock);
			return;
		}
	}
...
clock_settime(clockid_t clock_id, timespec_t *tp)
{
	...
	if (suser(CRED())) {
		...
		if (get_udatamodel() == DATAMODEL_NATIVE) {
			if (copyin(tp, &tspec, sizeof (tspec)))
				return (set_errno(EFAULT));
		} else {
			timespec32_t tspec32;

			if (copyin(tp, &tspec32, sizeof (tspec32)))
				return (set_errno(EFAULT));

			TIMESPEC32_TO_TIMESPEC(&tspec, &tspec32);
		}
...
clock_gettime(clockid_t clock_id, timespec_t *tp)
{
	...
	case __CLOCK_REALTIME3:
		gethrestime(&tspec);
		if (get_udatamodel() == DATAMODEL_NATIVE) {
			if (copyout(&tspec, tp, sizeof (tspec)))
				return (set_errno(EFAULT));
		} else {
			timespec32_t tspec32;

			if (TIMESPEC_OVERFLOW(&tspec))
				return (set_errno(EOVERFLOW));
			TIMESPEC_TO_TIMESPEC32(&tspec32, &tspec);

			if (copyout(&tspec32, tp, sizeof (tspec32)))
				return (set_errno(EFAULT));
		}
		break;
	case CLOCK_VIRTUAL:
...
timer_create(clockid_t clock_id, struct sigevent *evp, timer_t *timerid)
{
	...
	if (evp != NULL) {
		...
		if (get_udatamodel() == DATAMODEL_NATIVE) {
			...
			ev.sigev_value.sival_ptr =
			    (void *)(uintptr_t)ev32.sigev_value.sival_ptr;
		}
	...
	for (ti = -1, i = 0, timerp = p->p_itimer;
	...
	sigqp->sq_info.si_signo = ev.sigev_signo;
	sigqp->sq_info.si_value = ev.sigev_value;
	...
	mutex_exit(&p->p_lock);
	if (copyout(&ti, timerid, sizeof (timer_t))) {
		mutex_enter(&p->p_lock);
		timerp->trs_flags &= ~TRS_INUSE;
		mutex_exit(&p->p_lock);
		return (set_errno(EFAULT));
...
timer_delete(timer_t timerid)
{
	...
	error = timer_del_cleanup(itimerp);
...
timer_del_cleanup(timerstr_t *itimerp)
{
	...
	if (itimerp->trs_flags & TRS_PENDING) {
		...
		bzero(itimerp->trs_sigqp, sizeof (sigqueue_t));
	}
	bzero(itimerp, sizeof (timerstr_t));
...
timer_gettime(timer_t timerid, struct itimerspec *value)
{
	...
	if (!error) {
		if (get_udatamodel() == DATAMODEL_NATIVE) {
			if (copyout(&aits, value, sizeof (aits)))
				error = EFAULT;
		} else {
			if (ITIMERSPEC_OVERFLOW(&aits))
				error = EOVERFLOW;
			else {
				itimerspec32_t aits32;

				ITIMERSPEC_TO_ITIMERSPEC32(&aits32, &aits)
				if (copyout(&aits32, value, sizeof (aits32)))
					error = EFAULT;
			}
		}
	}
 ...
 */
int
timer_settime(timer_t timerid, int flags, struct itimerspec *value,
    struct itimerspec *ovalue)
{
	...
	if (ovalue != NULL) {
		if ((error = timer_gettime(timerid, ovalue)) != 0)
			return (error);
	}
	if (value == NULL)
		return (0);
	...
	if (get_udatamodel() == DATAMODEL_NATIVE) {
		if (copyin(value, &aits, sizeof (aits)))
			return (set_errno(EFAULT));
	} else {
		struct itimerspec32 aits32;

		if (copyin(value, &aits32, sizeof (aits32)))
			return (set_errno(EFAULT));

		ITIMERSPEC32_TO_ITIMERSPEC(&aits, &aits32);
	}
	...
	case __CLOCK_REALTIME3:
		...
		if (timerspecisset(&aits.it_value)) {
			...
			itimerp->trs_callout_id =
			    realtime_timeout(realitsexpire, itimerp,
				timespectohz(&aits.it_value, now));
		}
...
timer_getoverrun(timer_t timerid)
{
	...
	if (!error)
		overrun = p->p_itimer[timerid].trs_overrun2;
	mutex_exit(&p->p_lock);
	if (error)
...
#ifdef DEBUG
	if (tod.tod_year < 70 || tod.tod_year > 8029 ||

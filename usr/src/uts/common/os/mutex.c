...
#pragma ident	"@(#)mutex.c	1.11	98/02/05 SMI"
...
static sobj_ops_t mutex_sobj_ops = {
	SOBJ_MUTEX, mutex_owner, turnstile_unsleep, turnstile_change_pri
};
...
mutex_panic(char *msg, mutex_impl_t *lp)
{
	...
	if (casptr((void **)&panic_mutex_addr, NULL, lp) == NULL)
		panic_mutex = *lp;
...
mutex_vector_enter(mutex_impl_t *lp)
{
	...
	u_int		spin_count = 0;	/* how many times we spun */
	cpu_t 		*cpup, *last_cpu;
	turnstile_t	*ts;
...
spin:
		...
		if (MUTEX_OWNER(vlp) == owner && MUTEX_HAS_WAITERS(vlp)) {
			...
			turnstile_block(ts, TS_WRITER_Q, lp, &mutex_sobj_ops);
			sleep_time += gethrtime();
		} else {
			turnstile_exit(lp);
		}
...
mutex_destroy(kmutex_t *mp)
{
	...
	if (lp->m_owner == 0) {
		...
		if (MUTEX_HAS_WAITERS(lp) || MUTEX_OWNER(lp) != curthread)
			mutex_panic("mutex_destroy: lock still active", lp);
		LOCKSTAT_EXIT(LS_ADAPTIVE_MUTEX_HOLD, lp, curthread, 1);
		MUTEX_DESTROY(lp);
	} else {
...
lock_set_spin(lock_t *lp)
{
	...
	while (LOCK_HELD((volatile lock_t *)lp) || !lock_try(lp)) {
...
void
lock_set_spl_spin(lock_t *lp, int new_pil, u_short *old_pil_addr, int old_pil)
{
	int spin_count = 1;
	...
	do {
		...
		while (LOCK_HELD((volatile lock_t *)lp)) {
			if ((volatile char *)panicstr) {
				*old_pil_addr = (u_short) splr(new_pil);
				return;
			}
			spin_count++;
		}

...
#pragma ident	"@(#)sig.c	1.110	98/02/11 SMI"	/* from SVr4.0 1.52 */
...
#include <sys/errno.h>
#include <sys/proc.h>
#include <sys/poll.h>	/* only needed for kludge in sigwaiting_send() */
	...
	if (sigismember(&coredefault, sig)) {
		...
		if (p->p_flag & SKILLED)
			...
			if (core("core", ttoproc(t), CRED(),
			    (u_long)U_CURLIMIT(&u, RLIMIT_CORE), sig) == 0)
				code = CLD_DUMPED;
#ifdef C2_AUDIT
			if (audit_active)		/* audit core dump */
...
{
	int sig = sigqp->sq_info.si_signo;
	int fromuser = SI_FROMUSER(&sigqp->sq_info);
...
sigqalloc(sigqhdr_t **sqhp)
{
	...
	if (sqh != NULL) {
		...
		if (sqh->sqb_count > 0) {
			...
			struct_zero(&sq->sq_info, sizeof (k_siginfo_t));
			sq->sq_backptr = sqh;
		} else {
			mutex_exit(&sqh->sqb_lock);
		}

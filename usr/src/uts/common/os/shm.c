...
#pragma ident	"@(#)shm.c	1.79	98/02/04 SMI"
	...
	if (spt_on(flags)) {
		...
		if (addr == 0) {
			...
			if (((uintptr_t)base & (share_size - 1)) ||
			    valid_usr_range(base, len, as->a_userlimit) == 0 ||
...
shmctl(int shmid, int cmd, struct shmid_ds *uds)
{
	...
	case IPC_O_STAT:
		...
		ods32.shm_perm.seq = (ushort)sp->shm_perm.seq;
		...
		ods32.shm_nattch = (ushort)sp->shm_nattch;
		ods32.shm_cnattch = (ushort)sp->shm_cnattch;
	...
	*sapp = sap->sa_next;
	...
	for (sp = shmem, lock = shmem_locks; sp < &shmem[shminfo.shmmni];
		...
		if (sp->shm_amp == amp) {
			...
			mutex_exit(lock);
			if (isspt(sp))
				sp->shm_cnattch--; /* decrement when detach */
			break;
		}
		mutex_exit(lock);
	}

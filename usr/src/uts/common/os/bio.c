...
#pragma ident	"@(#)bio.c	1.85	97/11/06 SMI"	/* SVr4.0 1.29	*/
...
struct ufsbiostats ub = {
	...
	{ "lufs strategy reads",	KSTAT_DATA_UINT32 },
	{ "lufs userdata writes",	KSTAT_DATA_UINT32 },
	{ "lufs strategy writes",	KSTAT_DATA_UINT32 },
	{ "ldl strategy reads",		KSTAT_DATA_UINT32 },
	{ "ldl strategy writes",	KSTAT_DATA_UINT32 },
	{ "master reads",		KSTAT_DATA_UINT32 },
	{ "snarf prewrites",		KSTAT_DATA_UINT32 },
	{ "roll writes",		KSTAT_DATA_UINT32 }
};
...
bread_common(void *arg, dev_t dev, daddr_t blkno, long bsize)
{
	...
	bp = getblk_common(ufsvfsp, dev, blkno, bsize);
 ...
 */
struct buf *
getblk_common(void * arg, dev_t dev, daddr_t blkno, long bsize)
{
...
binit(void)
{
	...
	if (v.v_bufhwm < BIO_MIN_HWM || v.v_bufhwm > bio_max_hwm) {
		...
		if (bufhwm != 0) {	/* User specified value */
			cmn_err(CE_WARN,
			    "binit: bufhwm out of range (%d). Using %lu",
			    bufhwm, bio_max_hwm);
		}
	}
	 ...
	 */
	v.v_hbuf = 1 << highbit((((ulong)v.v_bufhwm * 1024) / MAXBSIZE) /
	    BIO_HASHLEN);
...
pageio_setup(struct page *pp, size_t len, struct vnode *vp, int flags)
{
	...
	if (flags & B_READ) {
		...
		TRACE_3(TR_FAC_VM, TR_PAGE_WS_IN,
		    "page_ws_in:pp %p vp %p offset %llx",

...
#pragma ident	"@(#)streamio.c	1.231	98/02/17 SMI"
...
strclose(struct vnode *vp, int flag, cred_t *crp)
{
	 ...
	 * poll data from stream head and then wait until the pollers have
	 * gone away.
	 */
...
strread(struct vnode *vp, struct uio *uiop, cred_t *crp)
{
	...
	int first;
	char rflg;
	u_int mark;		/* Contains MSG*MARK and _LASTMARK */
...
strrput(queue_t *q, mblk_t *bp)
{
	...
	case M_DATA:
		if ((rput_opt & SR_IGN_ZEROLEN) &&
		    bp->b_rptr == bp->b_wptr && msgdsize(bp) == 0) {
			/*
...
do_sendfp(struct stdata *stp, struct file *fp, struct cred *cr)
{
	...
	mate = qp;
		 ...
		 */
		while (((qp = qp->q_next) != NULL) && SAMESTR(qp))
		;
	...
	int fmode)
{
	...
	/*
	 * Use the minimum of sd_maxblk and q_maxpsz.
...
chkrd:
	if (sd_flags & STRPRI) {
		...
		/*
		 * Note: No need to do the equiv of polllock() here since

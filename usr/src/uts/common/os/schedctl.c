...
#pragma ident	"@(#)schedctl.c	1.18	98/02/13 SMI"
 ...
 * control structures (sc_pool_list), with lwp_pool_head pointing to
...
typedef struct sc_page_ctl {
	...
	ulong		*spc_map;	/* bitmap of allocated space on page */
...
schedctl(unsigned int flags, int upcall_did, sc_shared_t **addrp)
{
	...
	if (flagdiff & SC_BLOCK) {
		 ...
		 */
		if (vp && casptr((void **)(&p->p_sc_door), NULL, vp) != NULL)
			VN_RELE(vp);
...
schedctl_cleanup(kthread_t *t)
{
	...
	if (tdp->sc_flags & SC_STATE) {
		...
		(void) removectx(t, NULL, schedctl_null,
		    schedctl_null, schedctl_fork, schedctl_null);
	}
...
schedctl_shared_alloc(long tag, sc_shared_t **kaddrp, sc_shared_t **uaddrp)
{
	...
	if (pagep == NULL) {
		 ...
		 * the mapping to both user and kernel address spaces
		 * for the new page and lock it in memory.
		 */
		...
		pagep->spc_map = kmem_zalloc(sizeof (ulong) * sc_bitmap_words,
		    KM_SLEEP);
...
schedctl_shared_free(long tag, sc_shared_t *ssp)
{
	...
	if (--procp->spm_refcnt == 0) {
		 ...
		 * so free the mapping structure for this process.
...
schedctl_getpage(struct anon_map **newamp, caddr_t *newaddr)
{
	 ...
	 * Allocate the page.
	 */
	error = anon_map_getpages(amp, 0, PAGESIZE, &pp, &kvseg,
	    kaddr, S_CREATE, NULL);
	if (error != NULL) {
		amp->refcnt--;
		anonmap_free(amp);
		rmfree(kernelmap, 1, a);
		anon_unresv(PAGESIZE);
		return (error);
	}
	hat_memload(kas.a_hat, kaddr, pp, PROT_READ | PROT_WRITE | HAT_NOSYNC,
	    HAT_LOAD_LOCK);
	if (!page_pp_lock(pp, 0, 0)) {
		hat_unload(kas.a_hat, kaddr, PAGESIZE,
...
schedctl_freepage(struct anon_map *amp, caddr_t kaddr)
{
	...
	swap_xlate(anon_get_ptr(amp->ahp, 0),  &vp, &off);
	pp = page_lookup(vp, (u_offset_t)off, SE_SHARED);
	if (pp == NULL)
		cmn_err(CE_PANIC, "schedctl_freepage: page not in the system");
	page_pp_unlock(pp, 0, 0);
	hat_unload(kas.a_hat, kaddr, PAGESIZE,
	    HAT_UNLOAD_NOSYNC | HAT_UNLOAD_UNLOCK);
	rmfree(kernelmap, 1, (ulong_t)btokmx(kaddr));
	page_unlock(pp);

...
#pragma ident	"@(#)share.c	1.9	98/02/12 SMI"
...
add_share(struct vnode *vp, struct shrlock *shr)
{
	 ...
	 */
	if ((shr->s_access == 0) && (shr->s_sysid == 0)) {
		return (EINVAL);
	}
...
del_share(struct vnode *vp, struct shrlock *shr)
{
	...
	while (*shrlp) {
		if ((shr->s_own_len == (*shrlp)->shr->s_own_len &&
			...
			(shr->s_own_len == 0 &&
			    ((shr->s_sysid == 0 &&
				shr->s_pid == (*shrlp)->shr->s_pid) ||
			    (shr->s_sysid != 0 &&
				shr->s_sysid == (*shrlp)->shr->s_sysid)))) {
			shrl = *shrlp;
			*shrlp = shrl->next;
...
shr_has_remote_shares(vnode_t *vp, int32_t sysid)
{
	...
	shrl = vp->v_shrlocks;
	while (shrl) {
		if ((sysid != 0 && shrl->shr->s_sysid == sysid) ||
		    (sysid == 0 && shrl->shr->s_sysid != 0)) {
			result = 1;
			break;
		}
		shrl = shrl->next;

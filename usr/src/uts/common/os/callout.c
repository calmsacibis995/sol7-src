...
#pragma	ident	"@(#)callout.c	1.21	97/10/22 SMI"
 ...
 * If the kmem_perm_alloc() fails, we try every possible kmem
 * cache up to 1000 bytes in the hopes of finding something on
 * someone's freelist.  This logic should be deleted once the
 * allocator supports KM_URGENT.
 */
static callout_t *
...
/*
 * Threads for normal callout processing.
 */
static void
callout_thread(callout_table_t *ct)
{
	CALLB_CPR_INIT_SAFE(curthread, "callout");
	for (;;) {
		mutex_enter(&ct->ct_lock);
		cv_wait(&ct->ct_threadpool, &ct->ct_lock);
		mutex_exit(&ct->ct_lock);
		callout_execute(ct);
	}
}
		...
		for (f = 0; f < CALLOUT_FANOUT; f++) {
			...
			if (t == CALLOUT_NORMAL) {
				for (i = 0; i < CALLOUT_THREADS; i++) {
					if (thread_create(NULL, 0,
					    callout_thread, (caddr_t)ct, 0,
					    &p0, TS_RUN, maxclsyspri) == NULL) {
						panic("callout_thread create");
					}
				}
			}
		}

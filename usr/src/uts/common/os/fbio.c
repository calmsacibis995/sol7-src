 ...
 * All rights reserved.
 */
#pragma ident	"@(#)fbio.c	1.24	97/09/28 SMI"
...
fbread(vnode_t *vp, offset_t off, u_int len, enum seg_rw rw, struct fbuf **fbpp)
{
	...
	o = (u_long)(off & (offset_t)MAXBOFFSET);
	if (o + len > MAXBSIZE)
 ...
 *
 * fbrelse()	release fbp
 * fbrelsei()	release fbp and invalidate pages	XXX unused?
 * fbwrite()	direct write
 * fbwritei()	direct write and invalidate pages	XXX unused?

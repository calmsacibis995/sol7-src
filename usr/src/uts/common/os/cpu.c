...
#pragma ident	"@(#)cpu.c	1.79	97/10/22 SMI"
...
cpu_poweroff(cpu_t *cp)
{
	...
	if (error == 0) {
		cp->cpu_type_info.pi_state = P_POWEROFF;
		cp->cpu_state_begin = hrestime.tv_sec;
	}
#endif /* MP */
...
cpu_info_kstat_update(kstat_t *ksp, int rw)
{
	...
	case P_ONLINE:
		(void) strcpy(cpu_info_template.ci_state.value.c, "on-line");
		break;
	case P_POWEROFF:
		(void) strcpy(cpu_info_template.ci_state.value.c,
		    "powered-off");
		break;
	case P_OFFLINE:
	default:
		(void) strcpy(cpu_info_template.ci_state.value.c, "off-line");
		break;
	}
...
{
	size_t len = sizeof (cpuset_t) / sizeof (uint32_t);

...
#ident	"@(#)rmap.c	2.38	98/02/07 SMI" /* from SunOS 4.0 2.11 */
...
mapinit(struct map *mp, size_t size, ulong_t addr, char *name, size_t mapsize)
{
	 ...
	 */
	mutex_init(&maplock(mp), NULL, MUTEX_DEFAULT, NULL);

	/*
	 * Initialize the condition variable in the map.
	 */
	cv_init(&map_cv(mp), NULL, CV_DEFAULT, NULL);
}
...
/*
 * Simple place holder code that implements interfaces for:
 *
 *	1) Allocating 32-bit id that is bound to a memory address
 *
 *	2) Lookup of memory address given 32-bit id.
 *
 *	3) Free 32 bit id.
 *
 * Note: id32_alloc() may block in kmem_alloc()
 */

uint32_t
id32_alloc(void *ptr)
{
	void **id = kmem_alloc(sizeof (*id), KM_SLEEP);
	uintptr_t id32;

	*id = ptr;
	id32 = (uintptr_t)((caddr_t)id - kernelheap) / sizeof (void *);
	ASSERT64(id32 <= UINT32_MAX);
...
{
	kmem_free((void *)(kernelheap + ((uintptr_t)id * sizeof (void *))),
	    sizeof (void *));
}
...
{
	return (*(void **)(kernelheap + ((uintptr_t)id * sizeof (void *))));
}

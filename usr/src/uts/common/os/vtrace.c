...
#pragma ident	"@(#)vtrace.c	1.27	98/02/01 SMI"
...
u_char	null_event_map[VT_MAPSIZE];	/* when tracing is disabled */

u_long	bytes2data[512];		/* see store_data() */

static	int trace_npages;	/* number of trace buffer pages */
static	int trace_nbytes;	/* above, times PAGESIZE */
...
#define	TBUF_WRITE(cp, tdp, size) \
	...
	if (cp >= tdp->tbuf_wrap) { \
		while (cp < tdp->tbuf_end) { \
			*((u_long *) cp) = FTT2HEAD(TR_FAC_TRACE, TR_PAD, 0); \
			cp += sizeof (u_long); \
		} \
		cp = tdp->tbuf_start; \
	} \
...
trace_write_data(char *data, int size, int cpunum)
{
	...
	if (cpunum == -1) {
		trace_write_buffer((u_long *) data, size);
	} else {
...
trace_kthread_label(kthread_id_t t, int cpunum)
{
	...
	ktl.pid		= (u_long) (ttoproc(t)->p_pid);
	ktl.lwpid	= (u_long) (t->t_tid);
	ktl.tid		= (u_long) t;
	ktl.startpc	= (u_long) (t->t_startpc);
...
void
trace_process_name(u_long pid, char *name)
{
...
void
trace_process_fork(u_long cpid, u_long ppid)
{
	if (tracing_state & ADMIN_TRACING)
...
trace_init(int npages, int lowater)
{
	...
	for (i = 0; i < NCPU; i++) {
		...
		tdp->real_event_map = (u_char *)emp;
		bp = kmem_getpages(trace_npages, KM_SLEEP);
		if (trace_forced_reset) {
			cmn_err(CE_NOTE, "vtrace: forced reset detected");
			kmem_freepages(bp, trace_npages);
			return (EBUSY);
		}
		tdp->tbuf_start = bp;
		...
		tdp->tbuf_redzone = (char *)
			((u_int) (tdp->tbuf_end - 0x1000) & ~0xfff);
...
trace_vn_write(struct vnode *vp, struct cred *cr, char *addr, int len)
{
	...
	limit = U_CURLIMIT(PTOU(ttoproc(curthread)), RLIMIT_FSIZE);
...
trace_flush(int force)
{
	...
	for (i = 0; i < NCPU; i++) {
		...
		tdp->tbuf_redzone = (tail >= start + 0x1000) ?
			(char *)((u_int)(tail - 0x1000) & ~0xfff) :
			(char *)((u_int)(end - 0x1000) & ~0xfff);
...
trace_init_buffer(int cpunum)
{
	...
	tdp->last_hrtime_lo32 = (u_long) (start_hrtime & 0xffffffff);
	...
	trace_label(TR_FAC_TRACE, TR_END, sizeof (u_long),
	...
	trace_label(TR_FAC_TRACE, TR_TITLE, sizeof (vt_title_t),
		"Title:%s", cpunum);
	trace_label(TR_FAC_TRACE, TR_PAD, sizeof (u_long),
		"pad_record", cpunum);
	...
	if (VT_TEST_FT(tdp->real_event_map,
	    TR_FAC_MCOUNT, TR_MCOUNT_ENTER, VT_ENABLED)) {
		trace_label(TR_FAC_MCOUNT, TR_MCOUNT_ENTER, 12,
			"call path: %K	called %K", cpunum);
	}
	...
	vtp.abs_time_p->time.hi32 = (u_long) (abs_hrtime >> 32);
	vtp.abs_time_p->time.lo32 = (u_long) (abs_hrtime & 0xffffffff);
	TBUF_WRITE(vtp.char_p, tdp, sizeof (vt_abs_time_t));

	vtp.start_time_p->head = FTT2HEAD(TR_FAC_TRACE, TR_START_TIME, 0);
	vtp.start_time_p->time.hi32 = (u_long) (start_hrtime >> 32);
	vtp.start_time_p->time.lo32 = (u_long) (start_hrtime & 0xffffffff);
	...
	vtp.raw_kthread_id_p->tid  = (u_long) curthread;
 ...
 */
u_char
trace_label(u_char fac, u_char tag, u_short len, char *name, int cpunum)
{
	u_char string_bits = 0;
	u_char mask = VT_STRING_1;
	char oldc, newc, *cp;

...
#pragma ident	"@(#)rwlock.c	1.11	98/02/01 SMI"
...
static sobj_ops_t rw_sobj_ops = {
	SOBJ_RWLOCK, rw_owner, turnstile_unsleep, turnstile_change_pri
};
...
rw_panic(char *msg, rwlock_impl_t *lp)
{
	...
	if (casptr((void **)&panic_rwlock_addr, NULL, lp) == NULL)
		panic_rwlock = *lp;
...
rw_enter_sleep(rwlock_impl_t *lp, krw_t rw)
{
	...
	for (;;) {
		...
		/*
		 * We really are going to block.  Note the reason (event) for
		 * lockstat, bump the stats, and drop kpri if we're a reader.
		 */
		...
		if (rw == RW_READER) {
			THREAD_KPRI_RELEASE();
			event = (old & RW_WRITE_LOCKED) ?
			    LS_RW_READER_BY_WRITER :
			    LS_RW_READER_BY_WRITE_WANTED;
			CPU_STAT_ADDQ(CPU, cpu_sysinfo.rw_rdfails, 1);
			turnstile_block(ts, TS_READER_Q, lp, &rw_sobj_ops);
		} else {
			event = (old & RW_WRITE_LOCKED) ?
			    LS_RW_WRITER_BY_WRITER :
			    LS_RW_WRITER_BY_READERS;
			CPU_STAT_ADDQ(CPU, cpu_sysinfo.rw_wrfails, 1);
			turnstile_block(ts, TS_WRITER_Q, lp, &rw_sobj_ops);
		}
		sleep_time += gethrtime();

		LOCKSTAT_RECORD(event, lp, sleep_time,
		    (old & RW_WRITE_LOCKED) ? 1 : old >> RW_HOLD_COUNT_SHIFT);
	...
	if (rw == RW_READER) {
		LOCKSTAT_ENTER(LS_RW_READER_HOLD, lp, curthread);
	} else {
		LOCKSTAT_ENTER(LS_RW_WRITER_HOLD, lp, curthread);
	}
}
...
rw_exit_wakeup(rwlock_impl_t *lp)
{
	...
	if (lock_value == RW_READ_LOCK) {
		THREAD_KPRI_RELEASE();
		LOCKSTAT_EXIT(LS_RW_READER_HOLD, lp, curthread,
		    old >> RW_HOLD_COUNT_SHIFT);
	} else {
		LOCKSTAT_EXIT(LS_RW_WRITER_HOLD, lp, curthread, 1);
	}
}
...
rw_tryupgrade(krwlock_t *rwlp)
{
	...
	membar_enter();
	THREAD_KPRI_RELEASE();
	LOCKSTAT_EXIT(LS_RW_READER_HOLD, lp, curthread, 1);
	LOCKSTAT_ENTER(LS_RW_WRITER_HOLD, lp, curthread);
...
{
	return ((((rwlock_impl_t *)rwlp)->rw_wwwh & RW_WRITE_LOCKED) ? 1 : 0);
}

int
rw_lock_held(krwlock_t *rwlp)
{
	return ((((rwlock_impl_t *)rwlp)->rw_wwwh & RW_LOCKED) ? 1 : 0);
}
...
{
	return ((((rwlock_impl_t *)rwlp)->rw_wwwh & RW_WRITE_CLAIMED) ? 1 : 0);
}

...
#pragma ident	"@(#)cmn_err.c	1.80	97/11/22 SMI"	/* from SVr4.0 1.17 */
...
static void	prf_internal(const char *fmt, va_list adx, vnode_t *vp,
	int prt_where, int prt_type, char *sprintf_buf, ssize_t sbuf_len);
static void	writekmsg(char *msgp, size_t count, vnode_t *vp, int prt_type,
	int prt_where);
...
static char	*printc(char c,
	...
	ssize_t sbuf_len);
static void	output_line(char *linep, size_t len, vnode_t *vp,
	int prt_type, int prt_where);
static void	extra_print(const char *fmt, va_list adx, int prt_where,
	int prt_type, char *extra_string);
static void	printf_internal(int prt_where, int prt_type,
	const char *fmt, ...);
...
static kmutex_t framebuffer_lock;
static lock_t prf_lock;

#define	PRF_SPL		ipltospl(SPL7)
...
/*PRINTFLIKE3*/
static void
printf_internal(int prt_where, int prt_type, const char *fmt, ...)
{
	va_list adx;
...
/*PRINTFLIKE3*/
char *
sprintf_len(ssize_t len, char *buf, const char *fmt, ...)
{
	char *rval;
	va_list ap;
...
char *
vsprintf_len(ssize_t len, char *buf, const char *fmt, va_list ap)
{
...
void
vcmn_err(int level, const char *fmt, va_list adx)
{
	short prt_where;
	static char whirl[] = "|/-\\";
	static int whirlpos;
 ...
 */
static void
extra_print(const char *fmt, va_list adx, int prt_where, int prt_type,
	char *extra_string)
{
	char	sbuf[256];
...
softwritekmsg(struct softkmsg *smp)
{
	...
	lock_set_spl(&prf_lock, PRF_SPL, &s);
...
static void
output_line(char *linep, size_t len, vnode_t *vp, int prt_type, int prt_where)
{
	struct softkmsg *smp;
	...
	if (CPU->cpu_on_intr && !panicstr) {
		...
		softcall((void (*)(void *))softwritekmsg, NULL);
	} else
...
printc(
	...
	ssize_t sbuf_len)	/* length of the sprintf buffer */
{
	size_t msgsize = lbp - linebuf + 1;
...
{
	/*
	 * Validate all the members, since it is possible that they
	 * may have been corrupted by a wild pointer on a previous
	 * instantiation of the system.
	 */
	if (msgbuf.msg_magic != MSG_MAGIC || msgbuf.msg_size != MSG_BSIZE ||
	    msgbuf.msg_bufx < 0 || msgbuf.msg_bufx >= msgbuf.msg_size ||
	    msgbuf.msg_bufr < 0 || msgbuf.msg_bufr >= msgbuf.msg_size) {
...
msgbuf_save(void)
{
	...
	if (!(new = kmem_zalloc(sizeof (msglist_t), KM_NOSLEEP)))
		return;
	if (!(new->buf = kmem_zalloc(MSG_BSIZE, KM_NOSLEEP))) {

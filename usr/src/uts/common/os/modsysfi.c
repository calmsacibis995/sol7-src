/*
 * Copyright (c) 1990-1994, 1997, 1998, by Sun Microsystems, Inc.
 * All rights reserved.
 */

#pragma ident	"@(#)modsysfile.c	1.69	98/01/09 SMI"
...
file_err(int type,  struct _buf *file, char *fmt, ...)
{
	...
	(void) sprintf(trailer, "on line %d of %s", kobj_linenum(file),
		kobj_filename(file));
...
do_sysfile_cmd(struct _buf *file, char *cmd)
{
	...
	case MOD_SWAPFS:
		...
		*(--cp) = '\0';
		...
		sysp->sys_ptr = kmem_zalloc(strlen(tok1) + 1, KM_SLEEP);
...
#define	MIN_DEFAULT	8u
#define	MAX_DEFAULT	1024u
...
set_int8_var(uintptr_t symaddr, struct sysparam *sysp)
{
	...
	if (moddebug & MODDEBUG_LOADMSG)
		modprintf("OP: %x: param '%s' was '0x%" PRIx8
	...
	if (moddebug & MODDEBUG_LOADMSG)
		modprintf("now it is set to '0x%" PRIx8 "'.\n",
		    *(uint8_t *)symaddr);
}
...
set_int16_var(uintptr_t symaddr, struct sysparam *sysp)
{
	...
	if (moddebug & MODDEBUG_LOADMSG)
		modprintf("OP: %x: param '%s' was '0x%" PRIx16
		    "' in module: '%s'.\n", sysp->sys_op, sysp->sys_ptr,
	...
	if (moddebug & MODDEBUG_LOADMSG)
		modprintf("now it is set to '0x%" PRIx16 "'.\n",
		    *(uint16_t *)symaddr);
}
...
set_int32_var(uintptr_t symaddr, struct sysparam *sysp)
{
	...
	if (moddebug & MODDEBUG_LOADMSG)
		modprintf("OP: %x: param '%s' was '0x%" PRIx32
		    "' in module: '%s'.\n", sysp->sys_op, sysp->sys_ptr,
	...
	if (moddebug & MODDEBUG_LOADMSG)
		modprintf("now it is set to '0x%" PRIx32 "'.\n",
		    *(uint32_t *)symaddr);
}
...
set_int64_var(uintptr_t symaddr, struct sysparam *sysp)
{
	...
	if (moddebug & MODDEBUG_LOADMSG)
		modprintf("OP: %x: param '%s' was '0x%" PRIx64
		    "' in module: '%s'.\n", sysp->sys_op, sysp->sys_ptr,
	...
	if (moddebug & MODDEBUG_LOADMSG)
		modprintf("now it is set to '0x%" PRIx64 "'.\n",
		    *(uint64_t *)symaddr);
}
...
setparams(int ask)
{
	...
	for (sysp = sysparam_hd; sysp != NULL; sysp = sysp->sys_next) {
		...
		if (ask && (sysp->sys_type != MOD_SET ||
		    ...
		    strcmp(sysp->sys_ptr, "pidmax") != 0)))
			continue;

		if (sysp->sys_type == MOD_SET) {
			if (strcmp(sysp->sys_ptr, "physmem") == 0) {
				set_int32_var((uintptr_t)&physmem, sysp);
				physmset++;
				continue;
			} else if (strcmp(sysp->sys_ptr, "maxusers") == 0) {
				set_int32_var((uintptr_t)&maxusers, sysp);
				maxuset++;
				continue;
			} else if (strcmp(sysp->sys_ptr, "pidmax") == 0) {
				set_int32_var((uintptr_t)&pidmax, sysp);
				continue;
			}
		}
...
add_class(char *exporter, char *class)
{
	...
	hcl = kmem_zalloc(sizeof (struct hwc_class), KM_NOSLEEP);
	...
	for (hcl = hcl_head; hcl != NULL; hcl = hcl->class_next) {
		if (strcmp(hwcp->hwc_class_name, hcl->class) == 0) {
			if (hwcp->hwc_parent_name != NULL) {
				...
				(void) strcpy(DEVI(hs1->hwc_proto)->devi_name,
				...
				(void) strcpy(hs1->hwc_parent_name,
				    hcl->class_exporter);
				copy_prop(
				...
				(void) strcpy(hwcp->hwc_parent_name,
				    hcl->class_exporter);
			}
		}
	}

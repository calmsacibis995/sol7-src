...
#pragma ident	"@(#)lwp.c	1.99	98/02/01 SMI"
	...
	proc_t *p, int state, int pri, k_sigset_t smask, int cid)
{
	...
	if (timerisset(&p->p_rprof_timer.it_value))
		t->t_rprof = kmem_zalloc(sizeof (struct rprof), KM_NOSLEEP);
...
lwp_exit(void)
{
	...
	/*
	 * If the /proc agent lwp is exiting, readjust p_lwptotal
	 * so that it appears that the agent never existed.
	 */
	if (t == p->p_agenttp) {

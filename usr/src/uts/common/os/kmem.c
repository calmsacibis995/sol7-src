...
#pragma	ident	"@(#)kmem.c	1.43	98/02/20 SMI"
...
struct kmem_cpu_kstat {
	...
	{ "alloc_from_cpu%d",		KSTAT_DATA_ULONG },
	{ "free_to_cpu%d",		KSTAT_DATA_ULONG },
	{ "buf_avail_cpu%d",		KSTAT_DATA_ULONG },
};
...
struct {
	...
	{ "arena_size",		KSTAT_DATA_ULONG },
	{ "huge_size",		KSTAT_DATA_ULONG },
	{ "huge_alloc",		KSTAT_DATA_ULONG },
	{ "huge_alloc_fail",	KSTAT_DATA_ULONG },
	{ "perm_size",		KSTAT_DATA_ULONG },
	{ "perm_alloc",		KSTAT_DATA_ULONG },
	{ "perm_alloc_fail",	KSTAT_DATA_ULONG },
};
...
u_int kmem_random;
static clock_t kmem_reap_lasttime;	/* time of last reap */
...
kmem_bufctl_display(kmem_bufctl_audit_t *bcp)
{
	...
	prom_printf("\nthread=%p  time=T-%d.%09d  slab=%p  cache: %s\n",
...
kmem_error(int error, kmem_cache_t *cp, void *buf, kmem_bufctl_t *bcp)
{
	...
	case KMERR_MODIFIED:
		...
		if (off == NULL)	/* shouldn't happen */
			off = buf;
		prom_printf("modification occurred at offset 0x%lx "
			"(0x%x replaced by 0x%x)\n",
			(uintptr_t)off - (uintptr_t)buf, KMEM_FREE_PATTERN,
			*off);
		break;
	...
	case KMERR_BADBUFCTL:
		...
		prom_printf("bcp->bc_addr = %p, should be %p\n",
			bcp->bc_addr, buf);
		bcp = kmem_locate(buf);
		break;
	...
	prom_printf("buffer=%p  bufctl=%p  cache: %s\n",
		buf, bcp, cp->cache_name);

	if (cp->cache_flags & KMF_AUDIT) {
		prom_printf("previous transaction on buffer %p:\n", buf);
		if (bcp != NULL)
			kmem_bufctl_display((kmem_bufctl_audit_t *)bcp);
	}
	if (kmem_panic)
 ...
 */
static void *
kmem_page_alloc(kmem_backend_t *bep, pgcnt_t npages, int flags)
{
	void *pages;
	...
	if (bep->be_memclass == KMEM_CLASS_WIRED)
		kmem_misc_kstat.arena_size.value.ul += (uint32_t)size;
 ...
 */
static void
kmem_page_free(kmem_backend_t *bep, void *pages, pgcnt_t npages)
{
	size_t size = npages << bep->be_pageshift;
	int bucket = highbit(size - 1);
...
kmem_perm_alloc(size_t size, int align, int flags)
{
	...
	char *buf;
	long best_avail = LONG_MAX;

	KMEM_RANDOM_ALLOCATION_FAILURE(flags,
		kmem_misc_kstat.perm_alloc_fail.value.ul);
	...
	kmem_misc_kstat.perm_alloc.value.ul++;
	best_prev_ppp = NULL;
	for (prev_ppp = &kmem_perm_freelist; (pp = *prev_ppp) != NULL;
	    prev_ppp = &pp->perm_next) {
		if (pp->perm_avail - (-(intptr_t)pp->perm_current &
		    (align - 1)) >= size && pp->perm_avail < best_avail) {
			best_prev_ppp = prev_ppp;
			best_avail = pp->perm_avail;
		}
	}
	if ((prev_ppp = best_prev_ppp) == NULL) {
		pgcnt_t npages = btopr(size + sizeof (kmem_perm_t));
		...
		mutex_enter(&kmem_perm_lock);
		kmem_misc_kstat.perm_size.value.ul += (uint32_t)ptob(npages);
		pp = (kmem_perm_t *)buf;
...
kmem_backend_kstat_snapshot(kstat_t *ksp, void *buf, int rw)
{
	...
	knp[3].value.ul = (uint32_t)(bep->be_pages_inuse << bep->be_pageshift);
	knp += 4;
	for (bucket = bep->be_pageshift; bucket < KMEM_PAGE_BUCKETS; bucket++) {
		(void) sprintf(knp->name, "alloc_%d%c",
			1 << (bucket % 10), mem_units[bucket / 10]);
		knp->data_type = KSTAT_DATA_ULONG;
		knp->value.ul = bep->be_alloc[bucket];
		knp++;
		(void) sprintf(knp->name, "inuse_%d%c",
			1 << (bucket % 10), mem_units[bucket / 10]);
		knp->data_type = KSTAT_DATA_ULONG;
		knp->value.ul = (uint32_t)bep->be_inuse[bucket];
		knp++;
		if (bep->be_alloc[bucket] != 0 || bep->be_inuse[bucket] != 0)
			ksp->ks_ndata = (int32_t)(knp - (kstat_named_t *)buf);
	}
	return (0);
}
...
kmem_backend_kstat_create(kmem_backend_t *bep, char *name)
{
	if ((bep->be_kstat = kstat_create("unix", 0, name, "kmem_backend",
	    KSTAT_TYPE_NAMED,
	    (int32_t)(KMEM_PAGE_BUCKETS - bep->be_pageshift) * 2 + 4,
...
kmem_backend_create(
	...
	void *(*page_alloc)(size_t, u_int),	/* page allocation routine */
	void (*page_free)(void *, size_t),	/* page free routine */
...
{
	pgcnt_t npages = (size + bep->be_pagesize - 1) >> bep->be_pageshift;
	void *pages;
	...
	kmem_misc_kstat.huge_alloc.value.ul++;

	if ((pcp = kmem_cache_alloc(kmem_pagectl_cache, flags)) == NULL) {
		kmem_misc_kstat.huge_alloc_fail.value.ul++;
		return (NULL);
	}
	...
	*hash_bucket = pcp;
	if (bep->be_memclass == KMEM_CLASS_WIRED)
		kmem_misc_kstat.huge_size.value.ul +=
			(uint32_t)(npages << bep->be_pageshift);
	mutex_exit(&kmem_backend_lock);
...
kmem_backend_free(kmem_backend_t *bep, void *pages, size_t size)
{
	...
	if (pcp == NULL)
		cmn_err(CE_PANIC, "kmem_backend_free('%s', %p, %lu): "
		    "no such allocation", bep->be_kstat->ks_name,
		    pages, size);

	if (pcp->pc_size != size)
		cmn_err(CE_PANIC, "kmem_backend_free('%s', %p, %lu): "
		    "wrong size (alloc was %lu)", bep->be_kstat->ks_name, pages,
		    size, pcp->pc_size);
...
kmem_slab_create(kmem_cache_t *cp, int flags)
{
	...
	ASSERT(((intptr_t)base & (bep->be_pagesize - 1)) == 0);
...
kmem_cache_free_debug(kmem_cache_t *cp, void *buf, int dodest, size_t exactsize)
{
	...
	if (cp->cache_flags & KMF_REDZONE) {
		if (exactsize != 0) {
			...
			rzsize = (exactsize + KMEM_ALIGN - 1) & -KMEM_ALIGN;
			if (rzsize != cp->cache_offset &&
			    btp->bt_redzone[-1] != (int32_t)exactsize) {
				btp->bt_redzone[-2] = (int32_t)exactsize;
...
#define	KMEM_PRECISE_REDZONE_SETUP(cp, buf, size)	\
	if ((cp->cache_flags & KMF_REDZONE) && \
		...
		KMEM_BUFTAG(cp, buf)->bt_redzone[-1] = (int32_t)size;
...
kmem_cache_alloc_global(kmem_cache_t *cp, int flags)
{
		...
		if (bcp == NULL) {
			...
			if ((sp = kmem_slab_create(cp, flags)) == NULL) {
				...
				TRACE_1(TR_FAC_KMEM, TR_KMEM_CACHE_ALLOC_END,
					"kmem_cache_alloc_end:buf %p", NULL);
				return (NULL);
			}
	...
	TRACE_1(TR_FAC_KMEM, TR_KMEM_CACHE_ALLOC_END,
		"kmem_cache_alloc_end:buf %p", buf);
...
kmem_cache_free_global(kmem_cache_t *cp, void *buf)
{
	...
	TRACE_2(TR_FAC_KMEM, TR_KMEM_CACHE_FREE_START,
		"kmem_cache_free_start:cache %S buf %p", cp->cache_name, buf);
...
{
	void *buf;
	long index = (long)(size - 1) >> KMEM_ALIGN_SHIFT;

	TRACE_2(TR_FAC_KMEM, TR_KMEM_ZALLOC_START,
		"kmem_zalloc_start:size %ld flags %x", size, flags);

	if ((u_long)index < KMEM_MAXBUF >> KMEM_ALIGN_SHIFT) {
	...
	TRACE_1(TR_FAC_KMEM, TR_KMEM_ZALLOC_END, "kmem_zalloc_end:buf %p", buf);
...
{
	void *buf;
	long index = (size - 1) >> KMEM_ALIGN_SHIFT;

	TRACE_2(TR_FAC_KMEM, TR_KMEM_ALLOC_START,
		"kmem_alloc_start:size %ld flags %x", size, flags);
	...
	TRACE_1(TR_FAC_KMEM, TR_KMEM_ALLOC_END, "kmem_alloc_end:buf %p", buf);
...
kmem_free(void *buf, size_t size)
{
	...
	TRACE_2(TR_FAC_KMEM, TR_KMEM_FREE_START,
		"kmem_free_start:buf %p size %ld", buf, size);
...
static void
kmem_async_dispatch(void (*func)(kmem_cache_t *), kmem_cache_t *cp, u_int flags)
{
	...
	TRACE_1(TR_FAC_KMEM, TR_KMEM_ASYNC_DISPATCH_END,
		"kmem_async_dispatch_end:async_entry %p", ap);
}
...
kmem_reap(void)
{
	...
	if ((clock_t)(lbolt - kmem_reap_lasttime) < kmem_reap_interval)
		return;
...
kmem_cache_magazine_enable(kmem_cache_t *cp, kmem_cache_t *mcp)
{
	...
	mutex_enter(&cp->cache_depot_lock);
	cp->cache_magazine_cache = mcp;
	cp->cache_magazine_size = mcp->cache_bufsize / (int)sizeof (void *) - 1;
...
kmem_cache_debug_enable(kmem_cache_t *cp)
{
	...
	if (cp->cache_debug == NULL) {
		char name[40];
		(void) sprintf(name, "%s.DEBUG", cp->cache_name);
...
kmem_cache_kstat_update(kstat_t *ksp, int rw)
{
	...
	for (cpu_seqid = 0; cpu_seqid < cp->cache_ncpus; cpu_seqid++) {
		...
		kmcpup->kmcpu_alloc_from.value.ul	= ccp->cc_alloc;
		kmcpup->kmcpu_free_to.value.ul		= ccp->cc_free;
		kmcpup->kmcpu_buf_avail.value.ul	= cpu_buf_avail;
	...
	kmcp->kmc_hash_size.value.ul	= (cp->cache_hash_mask + 1) & -2;
	kmcp->kmc_hash_lookup_depth.value.ul = cp->cache_lookup_depth;
	kmcp->kmc_hash_rescale.value.ul	= cp->cache_rescale;
	kmcp->kmc_memory_class.value.ul	= cp->cache_backend->be_memclass;
...
kmem_cache_create_finish(kmem_cache_t *cp)
{
	...
	if (cp->cache_ncpus > ncpus) {
		...
		if (size >= sizeof (kmem_perm_t) + KMEM_PERM_MINFREE) {
			...
			mutex_enter(&kmem_perm_lock);
			kmem_misc_kstat.perm_size.value.ul += (uint32_t)size;
			pp->perm_next = kmem_perm_freelist;
			pp->perm_current = buf + sizeof (kmem_perm_t);
	...
	if ((cp->cache_kstat = kstat_create("unix", 0, cp->cache_name,
	    ...
	    (u_int)(sizeof (struct kmem_cache_kstat) / sizeof (kstat_named_t) +
	...
	int cflags)		/* cache creation flags */
{
	...
	if ((cp = kmem_cache_freelist) == NULL) {
		 ...
		 * to prevent false sharing of cache lines.
		 */
		csize = (int)KMEM_CACHE_SIZE(max_ncpus);
	...
	chunksize = ((int)bufsize + (KMEM_ALIGN - 1)) & -KMEM_ALIGN;
	...
	if ((cflags & KMC_NOHASH) || (!(cflags & KMC_NOTOUCH) &&
		...
		cp->cache_maxcolor =
			(cp->cache_slabsize - (int)sizeof (kmem_slab_t))
			% chunksize;
		...
		cp->cache_hash_table = kmem_page_alloc(&kmem_default_backend,
			1L, KM_SLEEP);
		cp->cache_hash_mask = PAGESIZE / (int)sizeof (void *) - 1;
...
kmem_cache_destroy(kmem_cache_t *cp)
{
	...
	if (cp->cache_buftotal != 0)
		cmn_err(CE_PANIC, "kmem_cache_destroy: '%s' (%p) not empty",
...
kmem_async_thread(void)
{
	...
	do {
		...
		while (--nk >= 0)
			(void) sprintf((dst++)->name, (src++)->name,
			    cpup->cpu_id);
		cpup = cpup->cpu_next;
	} while (cpup != cpu_list);
	mutex_exit(&cpu_lock);
	...
	CALLB_CPR_INIT(&cprinfo, &kmem_async_lock, callb_generic_cpr, "kmem");
	mutex_enter(&kmem_async_lock);
	for (;;) {
		CALLB_CPR_SAFE_BEGIN(&cprinfo);
		while (kmem_async_queue.async_next == &kmem_async_queue)
			cv_wait(&kmem_async_cv, &kmem_async_lock);
		CALLB_CPR_SAFE_END(&cprinfo, &kmem_async_lock);
		...
		TRACE_3(TR_FAC_KMEM, TR_KMEM_ASYNC_SERVICE_START,
			"kmem_async_service_start:async_entry %p %K(%S)",
...
kmem_init(void)
{
	...
	mutex_init(&kmem_backend_lock, NULL, MUTEX_DEFAULT, NULL);
	mutex_init(&kmem_perm_lock, NULL, MUTEX_DEFAULT, NULL);
	cv_init(&kmem_async_cv, NULL, CV_DEFAULT, NULL);
	mutex_init(&kmem_async_lock, NULL, MUTEX_DEFAULT, NULL);
	mutex_init(&kmem_async_serialize, NULL, MUTEX_DEFAULT, NULL);
	...
	for (i = 0; i < sizeof (kmem_magazine_type) / sizeof (*mtp); i++) {
		...
		(void) sprintf(namebuf, "kmem_magazine_%d", mtp->mt_magsize);

...
#pragma ident	"@(#)clock.c	1.137	98/01/29 SMI"
...
clock(void)
{
	...
	if (panicstr) {
		lbolt++;
		if (sync_timeout && (--sync_timeout == 0) &&
...
tick_again:
	...
	if (one_sec) {
		...
		if (time_offset == 0)
			...
			if (!(time_status & STA_FLL))
				lltemp = lltemp / (SCALE_KG >> time_constant);
	...
	if (clock_pend > 0) {
		...
		lock_set_spl(&clock_lock, ipltospl(LOCK_LEVEL), &s);
		clock_reruns++;
		clock_pend--;
...
delay(clock_t ticks)
{
	...
	(void) cv_wait(&curthread->t_delay_cv, &delay_lock);
	mutex_exit(&delay_lock);
}
...
#ifdef DEBUG
u_int expx_cnt = 0;	/* number of calls to exp_x() */
u_int expx_mul = 0;	/* number of long multiplies in exp_x() */
#endif
...
exp_x(uint64_t x)
{
	...
	ui = ull;	/* OK.  Now we can use a u_int. */
	for (i = 0; ui != 0; i++)
		ui >>= 1;

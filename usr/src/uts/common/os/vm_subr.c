...
#ident	"@(#)vm_subr.c	1.89	97/10/22 SMI"
	...
	while (uio->uio_iovcnt > 0) {
		...
		while (iov->iov_len > 0) {
			if (uio->uio_resid == 0)
				break;
			/*
			 * Large Files: We now have file system specific
			 * maximum offsets. Physio is done on a special
			 * file and specfs' maximum offset is SPEC_MAXOFFSET_T.
			 */
			if (uio->uio_loffset < 0 ||
			    uio->uio_loffset > SPEC_MAXOFFSET_T) {
				error = EINVAL;
				break;
			}
			...
			as_pageunlock(asp, pplist, a, (uint)c,
	...
	*lenp = 0;
	if (cow) {
		...
		if (seg->s_ops != &segvn_ops) {
			AS_LOCK_EXIT(as, &as->a_lock);
			return (ENOTSUP);
		} else if (SEGOP_GETTYPE(seg, uaddr) != MAP_PRIVATE) {
			AS_LOCK_EXIT(as, &as->a_lock);
			return (ENOTSUP);
		}
	}

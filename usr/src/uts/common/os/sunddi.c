...
#pragma ident	"@(#)sunddi.c	1.145	98/01/22 SMI"
...
static void hp_op_nexus_config_children(hp_daemon_op_t *op);
static void hp_op_make_new_children(dev_info_t *pdip,
	struct par_list *pl);
    ...
    u_int (*hi_int_handler)(void))
{
	...
	/* request the parent node to add it */
	return (i_ddi_add_intrspec(dip, dip, ispec, iblock_cookiep,
	    idevice_cookiep, (u_int (*)(caddr_t))hi_int_handler, 0,
	    IDDI_INTR_TYPE_FAST));
}
...
static ddi_dma_lim_t standard_limits = {
	(u_int)0,	/* addr_t dlim_addr_lo */
	(u_int)-1,	/* addr_t dlim_addr_hi */
...
static ddi_dma_lim_t standard_limits = {
	(u_int)0,		/* addr_t dlim_addr_lo */
	(u_int)0xffffff,	/* addr_t dlim_addr_hi */
    ...
    ddi_dma_handle_t *handlep)
{
	...
	if ((bp->b_flags & (B_PAGEIO|B_REMAPPED)) == B_PAGEIO) {
		...
		dmareq.dmar_object.dmao_obj.pp_obj.pp_offset =
		    (u_int) (((uintptr_t)bp->b_un.b_addr) & MMU_PAGEOFFSET);
	} else {
		dmareq.dmar_object.dmao_type = DMA_OTYP_VADDR;
    ...
    ddi_dma_cookie_t *cookiep)
{
	...
	return (((ddi_dma_impl_t *)h)->dmai_mctl(HD, HD, h, DDI_DMA_SEGTOC,
		o, (size_t *)l, (caddr_t *)cookiep, (u_int)seg));
#else
...
ddi_map_fault(dev_info_t *dip, struct hat *hat, struct seg *seg,
    caddr_t addr, struct devpage *dp, pfn_t pfn, u_int prot, u_int lock)
{
...
/*ARGSUSED*/
static int
ddi_no_platform_power(power_req *req)
{
	return (DDI_FAILURE);
}
 ...
 */
int	(*pm_platform_power)(power_req *) = ddi_no_platform_power;

/*ARGSUSED*/
static int
ddi_no_dev_is_needed(dev_info_t *dip, int pm_cmpt, int pm_level)
{
	return (DDI_FAILURE);
}

/*
 * This function calls the entry point supplied by the pm driver
 * to bring the device component 'pm_cmpt' to power level 'pm_level'.
 * The use of global for getting the  function name from pm driver
 * is not ideal, but it is simple and efficient.
 */
int	(*pm_dev_is_needed)(dev_info_t *, int, int) = ddi_no_dev_is_needed;

int
ddi_dev_is_needed(dev_info_t *dip, int pm_cmpt, int pm_level)
{
	return ((*pm_dev_is_needed)(dip, pm_cmpt, pm_level));
}
...
{
	u_char *pf = (u_char *)src;
	u_char *pt = (u_char *)dst;
	u_char tmp;
	int nshorts;
...
ddi_create_minor_common(dev_info_t *dip, char *name, int spec_type,
    minor_t minor_num, char *node_type, int is_clone, ddi_minor_type mtype)
{
	 ...
	 */
	if (minor_num > (1 << L_BITSMINOR32)) {
#ifdef DEBUG
		cmn_err(CE_WARN,
		    "%s%d:%s minor %x too big for 32-bit applications!",
...
ddi_create_minor_node(dev_info_t *dip, char *name, int spec_type,
    minor_t minor_num, char *node_type, int is_clone)
{
...
e_ddi_deferred_attach(major_t major, dev_t dev)
{
	...
	/*
	 * first attach any nodes that happen to be on the newdevs list
	 */
...
ddi_dma_addr_bind_handle(ddi_dma_handle_t handle, struct as *as,
	caddr_t addr, size_t len, u_int flags, int (*waitfp)(caddr_t),
...
#define	swap32(value)   \
	((swap16((uint16_t)((value) & 0xffff)) << 16) | \
	swap16((uint16_t)((value) >> 16)))
...
pm_busy_component(dev_info_t *dip, int component_number)
{
	...
	if (DEVI(dip)->devi_components &&
		...
		cp = &DEVI(dip)->devi_components[component_number];
		cp->pmc_busycount++;
		cp->pmc_timestamp = 0;
		mutex_exit(&DEVI(dip)->devi_lock);
		return (DDI_SUCCESS);
...
static hp_daemon_op_t	*hp_daemon_ops;


static void i_ddi_insert_ordered_devinfo(struct devnames *dnp, dev_info_t *dip);
static char *i_encode_composite_string(char **strings, u_int nstrings,
    size_t *retsz);
static int impl_lookup_compatible(dev_info_t *dip);
    ...
    dev_info_t **ret_dip)
{
	...
	if ((devi = kmem_zalloc(sizeof (*devi),
	    KM_NOSLEEP)) == NULL) {
		cmn_err(CE_NOTE,
	 ...
	 * we re-bind in ndi_devi_attach_driver after the caller
	 * has had the opportinity to attach a "compatible" property
...
ndi_devi_attach_driver(dev_info_t *dip, uint_t flags)
{
	...
	static int i_sattach(struct devnames *, dev_info_t *);
	 ...
	 */
	if (impl_lookup_compatible(dip) != NDI_SUCCESS)
		return (NDI_FAILURE);
	...
	/*
	 * attach the node synchronously iff NDI_ONLINE_ATTACH
	 * is specified
	 *
	 */

	if (flags & NDI_ONLINE_ATTACH)
		return (i_sattach(dnp, dip));
...
static int
i_sattach(struct devnames *dnp, dev_info_t *dip)

{
	...
	/*
	 * simply attach the node if the driver has been already
	 * loaded and configured for this device.
	 */
	if (CB_DRV_INSTALLED(devopsp[major])) {
		...
		/*
		 * the node failed to attach.  cleanup the dev_info
		 * node to allow the caller to remove it using
		 * ndi_devi_free()
		 */
		ASSERT(!DDI_CF2(dip));
		...
		/*
		 * the driver is not currently loaded
		 * orphan the node and call ddi_hold_installed_driver()
		 * to load/configure/attach the driver for this node.
		 */
		ddi_orphan_devs(dip);
		e_ddi_exit_driver_list(dnp, listcnt);
		UNLOCK_DEV_OPS(&(dnp->dn_lock));
		...
		/* node remains on the orphanlist */
		ASSERT(!DDI_CF1(dip));
		return (NDI_FAILURE);
	}
}
...
ndi_devi_online(dev_info_t *dip, uint_t flags)
{
	...
	if ((DEVI(dip)->devi_binding_name == NULL) ||
	    ...
	    ddi_binding_name(dip))) == (major_t)-1)
		return (NDI_FAILURE);
...
ndi_devi_offline(dev_info_t *dip, uint_t flags)
{
	...
	if (major != (major_t)-1) {
		...
		if (DDI_CF2(dip)) {
			/*
			 * Verify the driver has the D_HOTPLUG flag set in
			 * cb_ops.cb_flag before detaching the node.
			 */
			if ((ops == NULL) || (ops->devo_cb_ops == NULL) ||
			    ((ops->devo_cb_ops->cb_flag & D_HOTPLUG) == 0)) {
				LOCK_DEV_OPS(&(dnp->dn_lock));
				e_ddi_exit_driver_list(dnp, listcnt);
				UNLOCK_DEV_OPS(&(dnp->dn_lock));
				return (NDI_FAILURE);
			 ...
			 * force the detach.
			 */
			if (devi_detach(dip, DDI_DETACH) != DDI_SUCCESS) {
				LOCK_DEV_OPS(&(dnp->dn_lock));
				e_ddi_exit_driver_list(dnp, listcnt);
				UNLOCK_DEV_OPS(&(dnp->dn_lock));
				return (NDI_FAILURE);
...
ndi_devi_find(dev_info_t *pdip, char *cname, char *caddr)
{
	...
	while ((child = next) != NULL) {
		...
		/*
		 * skip nodes whos node name does not match the name
...
static void i_migrate_newdevinfos(struct devnames *dnp);

static int hotplug_daemon_sleeping = 0;
static int hotplug_daemon_paused = 0;
static kcondvar_t paused_cv;

void
hotplug_daemon_pause(void)
{
	mutex_enter(&hotplug_lk);
	ASSERT(hotplug_daemon_paused > -1);
	hotplug_daemon_paused++;
	if (!hotplug_daemon_sleeping) {
		cv_wait(&paused_cv, &hotplug_lk);
	}
	mutex_exit(&hotplug_lk);
}

void
hotplug_daemon_resume(void)
{
	mutex_enter(&hotplug_lk);
	ASSERT(hotplug_daemon_paused > 0);
	hotplug_daemon_paused--;
	if (!hotplug_daemon_paused) {
		cv_signal(&hotplug_cv);
	}
	mutex_exit(&hotplug_lk);
}
...
void
hotplug_daemon_complete(void)
{
	hotplug_daemon_resume();
	hotplug_daemon_pause();
	hotplug_daemon_resume();
}
...
hotplug_daemon(void)
{
	...
	cv_init(&paused_cv, "paused_cv", CV_DRIVER, NULL);

	for (;;) {
		mutex_enter(&hotplug_lk);

		while ((op = hp_daemon_ops) != NULL) {
			hp_daemon_ops = op->hpd_op_next;
			mutex_exit(&hotplug_lk);

			switch (op->hpd_op_type) {
			case HPD_OP_CONFIG_CHILDREN:
				hp_op_nexus_config_children(op);
				break;
#ifdef	DEBUG
			default:
				cmn_err(CE_CONT,
				    "hotplug daemon: op type: 0x%x\n",
				    op->hpd_op_type);
				break;
#endif	/* DEBUG */
			}
			kmem_free(op, sizeof (hp_daemon_op_t));
			mutex_enter(&hotplug_lk);
		}
		...
		if ((last_attached == 0) && (hp_hasdevs == 0)) {
			...
			} while (hotplug_daemon_paused);
			hotplug_daemon_sleeping = 0;
			mutex_exit(&hotplug_lk);
			continue;
		}
			...
			if (dnp->dn_newdevs != NULL) {
				...
				if (!(dnp->dn_flags & DN_WALKED_TREE) &&
					...
					if (ddi_hold_installed_driver(major) !=
						 ...
						 * for some reason.  Move the
						 * nodes off the new list
						 * to prevent the daemon
						 * from forever looping.
						 */
						i_migrate_newdevinfos(dnp);
					}
					continue;
				} else {
					/*
 ...
 */
static void
i_migrate_newdevinfos(struct devnames *dnp)
{
	dev_info_t *dip;
	int listcnt;
	...
	while ((dip = dnp->dn_newdevs) != NULL) {
		 ...
		 * nodes are tossed.
		 */
		if (DEVI(dip)->devi_nodeid != DEVI_PSEUDO_NODEID)
			ddi_orphan_devs(dip);
		else
			(void) ddi_remove_child(dip, 0);
		LOCK_DEV_OPS(&(dnp->dn_lock));
 ...
 */
static int
impl_lookup_compatible(dev_info_t *dip)
{
	int rv;
	int prop_flags;
	u_int ncompatstrs;
	...
	if (DEVI_PROM_NODE(DEVI(dip)->devi_nodeid) == 0)
...
void
ndi_nexus_config_children(struct devnames *dnp, dev_info_t *dip)
{
	hp_daemon_op_t	*op;
	hp_daemon_op_t	*new_op;
	struct dev_ops	*ops;
	...
	new_op = kmem_zalloc(sizeof (hp_daemon_op_t), KM_SLEEP);

	new_op->hpd_op_type = HPD_OP_CONFIG_CHILDREN;
	new_op->u_op.cc.hpd_op_dnp = dnp;
	new_op->u_op.cc.hpd_op_dip = dip;
	new_op->hpd_op_next = NULL;

	mutex_enter(&hotplug_lk);
	if ((op = hp_daemon_ops) == NULL) {
		hp_daemon_ops = new_op;
	} else {
		while (op->hpd_op_next != NULL)
			op = op->hpd_op_next;
		op->hpd_op_next = new_op;
	}
	if (hotplug_thread != (kthread_id_t)0)
		cv_signal(&hotplug_cv);
	mutex_exit(&hotplug_lk);
...
static void
hp_op_nexus_config_children(hp_daemon_op_t *op)
{
	struct devnames	*dnp;
	dev_info_t	*dip;
	struct par_list	*plist;
	...
	ASSERT(op->hpd_op_type == HPD_OP_CONFIG_CHILDREN);

	dnp = op->u_op.cc.hpd_op_dnp;
	dip = op->u_op.cc.hpd_op_dip;
	...
	for (major = 0; major < devcnt; major++) {
		...
		plist = impl_read_driver_conf((major_t)major);
		if (plist != NULL) {
			for (pl = plist; pl != NULL; pl = pl->par_next) {
				if (ddi_name_to_major(dnp->dn_name) !=
				    ddi_name_to_major(
				    pl->par_specs->hwc_parent_name))
					continue;
				if (CB_DRV_INSTALLED(devopsp[major])) {
					hp_op_make_new_children(dip, pl);
				} else {
					if (ddi_hold_installed_driver(major) !=
					    NULL)
						ddi_rele_driver(major);
...
static void
hp_op_make_new_children(dev_info_t *pdip, struct par_list *pl)
{
	struct hwc_spec	*hp;
	dev_info_t	*dip;
	dev_info_t	*proto;

	ASSERT(ddi_get_driver(pdip));
	ASSERT(DEV_OPS_HELD(ddi_get_driver(pdip)));
	...
	for (hp = pl->par_specs; hp != NULL; hp = hp->hwc_next) {
		...
		if (ndi_devi_alloc(pdip, ddi_get_name(hp->hwc_proto),
		    (dnode_t)DEVI_PSEUDO_NODEID, &dip) == NDI_SUCCESS) {
			proto = hp->hwc_proto;
			...
			if (ndi_devi_attach_driver(dip, 0) != NDI_SUCCESS) {

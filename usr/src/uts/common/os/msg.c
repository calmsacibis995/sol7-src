...
#pragma	ident	"@(#)msg.c	1.43	97/10/28 SMI" /* from S5R4 1.15 */
...
u_short msginfo_msgseg = 1024;	/* # of msg segments (MUST be < 32768) */
...
_init(void)
{
	...
	if ((INTRND((uint64_t)msginfo.msgseg * (uint64_t)msginfo.msgssz)
		* sizeof (char) +
	    (uint64_t)msginfo.msgmap * sizeof (struct map) +
	    (uint64_t)msginfo.msgmni * sizeof (struct msqid_ds) +
	    INTRND((uint64_t)msginfo.msgmni * sizeof (struct msglock)) +
	...
	mapinit(msgmap, msginfo.msgseg, (ulong)1, "msgmap", msginfo.msgmap);
...
#define	btoq(X)	((ulong_t)(((X) + msginfo.msgssz - 1) / msginfo.msgssz))
...
msgctl(int msgid, int cmd, struct msqid_ds *buf)
{
	...
	case IPC_O_STAT:
		...
		ods32.msg_perm.seq = (ushort)qp->msg_perm.seq;
		ods32.msg_perm.key = qp->msg_perm.key;
		ods32.msg_first = NULL; 	/* kernel addr */
		ods32.msg_last = NULL;
		ods32.msg_cbytes = (ushort)qp->msg_cbytes;
		ods32.msg_qnum = (ushort)qp->msg_qnum;
		ods32.msg_qbytes = (ushort)qp->msg_qbytes;
 ...
 */
static ssize_t
msgrcv(int msqid, struct ipcmsgbuf *msgp, size_t msgsz,
...
findmsg:
	...
	if (smp) {
		...
		if (mdl == DATAMODEL_NATIVE) {
			...
			/*
			 * 32-bit callers need a imploded msg type.
			 */
		...
		if (xtsz && copyout((msg + msginfo.msgssz * smp->msg_spot),
 ...
 */
static int
msgsnd(int msqid, struct ipcmsgbuf *msgp, ssize_t msgsz, int msgflg)
{
	struct msqid_ds		*qp;	/* ptr to associated q */
	struct msg		*mp;	/* ptr to allocated msg hdr */
	ulong_t			spot;	/* msg pool allocation spot */
	struct msglock		*lockp;
	...
	if ((size_t)msgsz > msginfo.msgmax) {
		error = EINVAL;
		goto msgsnd_out;
	}
...
getres:
	 ...
	 */
	if (msgsz + qp->msg_cbytes > qp->msg_qbytes) {
		if (msgflg & IPC_NOWAIT) {
			error = EAGAIN;
			goto msgsnd_out;
		}
	...
	if (msgsz && (spot = rmalloc_locked(msgmap, btoq(msgsz))) == NULL) {
		if (msgflg & IPC_NOWAIT) {
			error = EAGAIN;
	 ...
	 */
	if (msgsz && copyin(STRUCT_FADDR(umsgp, mtext),
	    (msg + msginfo.msgssz * --spot), msgsz)) {
		error = EFAULT;
		rmfree(msgmap, btoq(msgsz), spot + 1);
		goto msgsnd_out1;
	}
	qp->msg_qnum++;
	qp->msg_cbytes += msgsz;
	...
	mp->msg_next = NULL;
	mp->msg_type = type;
	mp->msg_ts = (ushort)msgsz;
	mp->msg_spot = (short)(msgsz ? spot : -1);
	if (qp->msg_last == NULL)
	...
	uint32_t a4, uint32_t a5)
{
	...
	case MSGRCV:
		/*
		 * Need to check if msgsz > INT_MAX
		 * (SSIZE_MAX in 32-bit world).
		 */
		if ((size_t)a3 > INT32_MAX)
		...
		break;
	case MSGSND:
		if ((size_t)a3 > INT_MAX)
			return (set_errno(EINVAL));

		error = msgsnd((int)a1, (struct ipcmsgbuf *)a2,
		    (ssize_t)(int32_t)a3, (int)a4);
		break;
	default:
		error = set_errno(EINVAL);
		break;

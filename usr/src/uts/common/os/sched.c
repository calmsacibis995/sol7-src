...
#ident	"@(#)sched.c	1.61	97/06/26 SMI"	/* from SVr4.0 1.16 */
...
static int	swapout(proc_t *, u_int *, int);
static void	process_swap_queue();

/*
 * Counters to keep track of the number of swapins or swapouts.
 */
u_int tot_swapped_in, tot_swapped_out;
u_int softswap, hardswap, swapqswap;
...
top:
	...
	for (prp = practive; prp != NULL; prp = prp->p_next) {
		...
		if (prp->p_swapcnt) {
			...
			TRACE_5(TR_FAC_SCHED, TR_CHOOSE_SWAPIN,
			    "pickswapin:%s[%d] epri %d stime %d size: %d",
			    PTOU(prp)->u_comm, prp->p_pid, proc_pri,
			    (lbolt - proc_time) / hz, prp->p_swrss);
			...
			if (proc_pri != -1) {
				...
				TRACE_3(TR_FAC_SCHED, TR_SOFTSWAP,
				    "softswap:%s[%d] size: %lu",
				    PTOU(prp)->u_comm, prp->p_pid,
				    rm_asrss(prp->p_as));
	...
	if (avail > (spgcnt_t)needs) {
		...
		TRACE_5(TR_FAC_SCHED, TR_SWAPIN_VALUES,
		    "swapin_values:%s[%d] needs %d freemem %ld deficit %d",
		    PTOU(swapin_prp)->u_comm, swapin_prp->p_pid,
		    needs, freemem, deficit);
		 ...
		 */
		if (freemem > needs) {
			TRACE_5(TR_FAC_SCHED, TR_HIGH_DEFICIT,
			    "deficit:%s[%d] needs %d freemem %ld deficit %d",
			    PTOU(swapin_prp)->u_comm, swapin_prp->p_pid,
			    needs, freemem, deficit);
			goto block;
		}
	...
	TRACE_5(TR_FAC_SCHED, TR_UNLOAD,
	    "unload:%s[%d] needs %d freemem %ld deficit %d",
	    PTOU(swapin_prp)->u_comm, swapin_prp->p_pid,
	    needs, freemem, deficit);
...
unload:
	...
	for (prp = practive; prp != NULL; prp = prp->p_next) {
		...
		if (swapin_prp == NULL || swapin_pri > proc_pri) {
			TRACE_5(TR_FAC_SCHED, TR_CHOOSE_SWAPOUT,
			    "hardswap:%s[%d] pri %d psize %lu needs %d",
			    PTOU(prp)->u_comm, prp->p_pid, proc_pri,
			    rm_asrss(prp->p_as), needs);
	...
	do {
		...
		if (thread_pri != -1 && tp->t_state == TS_RUN &&
			...
			if (err) {
				...
				CPU_STAT_ADDQ(cpup, cpu_vminfo.swapin, 1);
				CPU_STAT_ADDQ(cpup, cpu_vminfo.pgswapin,
				    stack_pages);
				...
				TRACE_4(TR_FAC_SCHED, TR_SWAPIN,
				    "swapin:%s[%d] pages_in: %d freemem: %ld",
				    PTOU(pp)->u_comm, pp->p_pid, stack_pages,
				    freemem);
				goto top;
			}
		}
 ...
 */
static int
swapout(proc_t *pp, u_int *swrss, int swapflags)
{
	kthread_id_t tp;
	pgcnt_t ws_pages = 0;
	int err;
	...
	if (pp->p_lwpcnt == 0 || (pp->p_flag & EXITLWPS))
		return (0);
...
top:
		...
		if (thread_pri != -1) {
			if (tp->t_schedflag & TS_DONT_SWAP) {
				...
				if (lock_try(&tp->t_lock)) {
					...
					CPU_STAT_ADDQ(cpup,
					    cpu_vminfo.pgswapout, stack_pages);
	...
	if (pp->p_swapcnt == pp->p_lwpcnt) {
		...
		CPU_STAT_ADDQ(cpup, cpu_vminfo.pgswapout, btop(as_size));
		CPU_STAT_ADDQ(cpup, cpu_vminfo.swapout, 1);
		ws_pages += btop(as_size);

		TRACE_4(TR_FAC_SCHED, TR_SWAPOUT,
		    "swapout:%s[%d] pages_pushed: %d freemem %ld",
		    PTOU(pp)->u_comm, pp->p_pid, ws_pages, freemem);
		/* Kernel probe */
...
swapout_lwp(klwp_t *lwp)
{
	...
	if (tp->t_schedflag & TS_SWAPENQ) {
		...
		TRACE_4(TR_FAC_SCHED, TR_SWAPOUT_LWP,
		    "swapout_lwp:%s[%d] freemem %ld avefree %ld",
		    PTOU(pp)->u_comm, pp->p_pid, freemem, avefree);
...
process_swap_queue()
{
	...
	while ((tp = tswap_queue) != NULL) {
		...
		CPU_STAT_ADDQ(cpup, cpu_vminfo.pgswapout, stack_pages);
		...
		TRACE_5(TR_FAC_SCHED, TR_SWAPQ_LWP,
		    "swaplist:%s[%d] swapcnt: %d freemem %ld avefree %d",
		    PTOU(pp)->u_comm, pp->p_pid, pp->p_swapcnt,
		    freemem, avefree);
		...
		if (pp->p_swapcnt == pp->p_lwpcnt) {
			...
			CPU_STAT_ADDQ(cpup, cpu_vminfo.pgswapout,
			    btop(as_size));
			CPU_STAT_ADDQ(cpup, cpu_vminfo.swapout, 1);

			ws_pages += btop(as_size);

			TRACE_4(TR_FAC_SCHED, TR_SWAPQ_PROC,
			    "swaplist_proc:%s[%d] pages_pushed: %u freemem %ld",
			    PTOU(pp)->u_comm, pp->p_pid, ws_pages, freemem);
			/* Kernel probe */

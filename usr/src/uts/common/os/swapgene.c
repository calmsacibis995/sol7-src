...
#pragma ident	"@(#)swapgeneric.c	5.106	98/01/24 SMI"
 ...
 * External references
 */
extern int mod_sysctl_type(int, int (*)(struct sysparam *, void *), void *);
 ...
 */
void
dumpinit(struct vnode *vp, char *name)
{
	register int nblks;
	struct vattr vattr;
	...
	if (dumpfile.bo_vp == NULL) {
		/*
		 * Dump to the end of swap file passed in (by the swap layer).
		 * zzz We need to honor what's in dumpfile if it's not null.
		 * non-null means use what's there.  This way we can
		 * change rootfs with /etc/system AND with tunetool.
		 */
		dumpfile.bo_vp = vp;
		(void) strncpy(dumpfile.bo_name, name, BO_MAXOBJNAME);
	}
	dumpvp = dumpfile.bo_vp;
	vattr.va_mask = AT_SIZE|AT_TYPE|AT_RDEV;
	if (VOP_GETATTR(dumpvp, &vattr, 0, kcred) != 0)
		return;
	nblks = (int)lbtodb(vattr.va_size);
	if (dumpfile.bo_size == 0 || dumpfile.bo_size > nblks)
		dumpfile.bo_size = nblks;
	if ((vattr.va_type == VBLK) || (vattr.va_type == VCHR))
		(void) mod_hold_dev_by_major(getmajor(vattr.va_rdev));
	VN_HOLD(dumpvp);
	dumphdr_init();
	cmn_err(CE_CONT, "?dump on %s size %ldK\n", dumpfile.bo_name,
...
loadrootmodules(void)
{
	 ...
	 */
	for (this = get_impl_module(1);
	    this != (char *)0;
	    this = get_impl_module(0))
...
loaddrv_hierarchy(char *drvname, major_t prev_major)
{
	...
	while (pl) {
		 ...
		 */
		if (pl->par_major != major)
			(void) loaddrv_hierarchy(
			    ddi_major_to_name(pl->par_major), (major_t)0);
		pl = pl->par_next;
	}

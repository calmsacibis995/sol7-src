...
#pragma ident   "@(#)kstat_fr.c 1.40     98/02/01 SMI"
...
{
	struct page *pp, *page0;
	kobj_stat_t kobj_stat;
	int pagesfree = 0;
	int pageslocked = 0;
	int pagesio = 0;
	int userpageslocked = 0;
	...
	system_pages_kstat.kernelbase.value.ul	= (ulong_t)_start;
	...
	do {
		...
		if (pp->p_vnode != NULL && pp->p_vnode != &kvp) {
			userpageslocked += (pp->p_lckcnt != 0) + pp->p_cowcnt;
		}

	} while ((pp = page_next(pp)) != page0);

	system_pages_kstat.pagesfree.value.ul	= (ulong_t)pagesfree;
	system_pages_kstat.pageslocked.value.ul	= (ulong_t)pageslocked;
	system_pages_kstat.pagesio.value.ul	= (ulong_t)pagesio;
	...
	system_pages_kstat.pp_kernel.value.ul	= (ulong_t)(availrmem_initial -
		availrmem - k_anoninfo.ani_mem_resv - userpageslocked);
	return (0);
}

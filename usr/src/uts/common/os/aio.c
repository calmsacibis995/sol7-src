...
#pragma ident   "@(#)aio.c 1.53     98/02/23 SMI"
	...
	if (timout == NULL) {
		...
		if (model == DATAMODEL_NATIVE) {
			...
			if (copyin(timout, &wait_time_32,
			    sizeof (wait_time_32)))
				return (EFAULT);
			wait_time.tv_sec = (time_t)(wait_time_32.tv_sec);
			wait_time.tv_usec = (suseconds_t)(wait_time_32.tv_usec);
		}
#endif  /* _SYSCALL32_IMPL */
	...
	int	run_mode)
{
	...
	aiocb_t		*cbp, **ucbp;
#ifdef	_SYSCALL32_IMPL
	caddr32_t	scbp32[_AIO_LISTIO_MAX];
	aiocb32_t	*cbp32;
	caddr32_t	*ucbp32;
	...
	int		i;
	size_t		ssize;
	int		lio_dynamic_flg = 0;	/* kmem_alloc'ed list of cb's */
	model_t		model = get_udatamodel();
	...
	if (timout) {
		if (model == DATAMODEL_NATIVE) {
			...
			wait_time.tv_sec = (time_t)(wait_time_32.tv_sec);
			wait_time.tv_nsec = (int)(wait_time_32.tv_nsec);
		}
#endif  /* _SYSCALL32_IMPL */
	...
	if (model == DATAMODEL_NATIVE) {
		 ...
		 * Since we are copying only the array of pointers
		 * largefile data size is not considered seperately
		 * over here.
		 */
		if (copyin(aiocb, cbplist,
		    (sizeof (aiocb_t *) * nent))) {
			error = EFAULT;
			goto done;
		}
	...
	int	run_mode)
{
	...
	if (model == DATAMODEL_NATIVE) {
		...
		if (copyin(aiocb, cbplist, sizeof (aiocb_t *) * nent)) {
			error = EFAULT;
			goto done;
		}
	} else {
		...
		if (copyin(aiocb, cbplist, (sizeof (caddr32_t) * nent))) {
			error = EFAULT;
			goto done;
		}
	}
	...
	int		mode)
{
	...
	/*
	 * There is no way to know if this is a 32 bit largefile
	 * call , so things are bit tricky here.
	 */
	if ((data_model = get_udatamodel()) == DATAMODEL_LP64) {
		...
		/*
		 * If this is a 32 bit system call on 64 bit kernel OR
		 * if this is 32 bit system call on 32 bit kernel
		 * go ahead and use aiocb64 because it can handle
		 * large file offsets
		 */
		if (data_model != DATAMODEL_NATIVE) {
			/*
			 * This means either 32 bit or largefile call made on
			 * 64 bit kernel. In either case use native aiocb
			 * which is large enough to hold 64 bit offset.
			 */
			aiocb.aio_fildes = fdes;
			aiocb.aio_buf = bufp;
			aiocb.aio_nbytes = bufsize;
			...
			/*
			 * This must be a 32 bit or largefile 32 bit call
			 * on 32 bit kernel. Use aiocb64_32 .
			 */
 ...
 * Gets called only when largefile call is made on ILP32 and LP64 kernel
 */
	...
	void		*sigev)
{
	...
	aio_req_t	*reqp;
	aio_t		*aiop;
	caddr32_t	scbp[_AIO_LISTIO_MAX];
	caddr_t		cbplist;
	aiocb64_32_t	*cbp;
	caddr32_t	*ucbp;
	aiocb64_32_t	cb64;
	...
	if (copyin(aiocb_arg, cbplist, (sizeof (caddr32_t) * nent))) {
		error = EFAULT;
		goto done;
	}
	...
	if ((mode_arg == LIO_WAIT) || sigev) {
		...
		head->lio_nent = nent_arg;
		head->lio_refcnt = nent_arg;
		if (sigev && (sigevk.sigev_notify == SIGEV_SIGNAL) &&
		    (sigevk.sigev_signo > 0 && sigevk.sigev_signo < NSIG)) {
			sqp = kmem_zalloc(sizeof (sigqueue_t),
			    KM_NOSLEEP);
...
aiocb_LFton(aiocb64_32_t *src, aiocb_t *dest)
{
	...
	dest->aio_sigevent._sigev_pad1 = (int)src->aio_sigevent._sigev_pad1;
	dest->aio_sigevent._sigev_notify_attributes =
	    (caddr_t)src->aio_sigevent._sigev_notify_attributes;
	dest->aio_sigevent._sigev_pad2 = src->aio_sigevent._sigev_pad2;

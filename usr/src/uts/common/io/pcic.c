...
#pragma ident	"@(#)pcic.c	1.65	97/11/22 SMI"
...
pcic_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	if (pcic->pc_flags & PCF_PCIBUS) {
		...
		iline = 0;
#else
		iline = ddi_getb(pcic->cfg_handle,
...
pcic_calc_speed(pcicdev_t *pcic, ulong speed)
{
	ulong wspeed = 1;	/* assume 1 wait state when unknown */
	ulong bspeed = PCIC_ISA_DEF_SYSCLK;
 ...
 */
static void
pcic_set_cdtimers(pcicdev_t *pcic, int socket, ulong speed, int tset)
{
...
#if defined(PCIC_DEBUG)
	if (pcic_debug)
		cmn_err(CE_CONT, "pcic_card_state(%x) if status = %b for %d\n",
			(int)sockp,
			(long)value,
		...
		while (intr != NULL) {
			...
			if (--pcic_irq_map[i].count == 0) {
				...
				ddi_remove_intr(dip,
						0,
						intr->iblk_cookie);
				(void) pcm_return_intr(pcic->dip, i);
#if defined(PCIC_DEBUG)
				if (pcic_debug) {
					cmn_err(CE_CONT,
...
void
xxdmp_cl_regs(pcicdev_t *pcic, int socket, u_int len)
{
	int i, value, j;
	char buff[256];
	char *fmt;
...
pcic_init_assigned(dev_info_t *dip)
{
	...
	if (par != NULL) {
		...
		if (map != NULL) {
			/* we've done the work earlier */
			kmem_free((caddr_t)regs, rlen);
			return;
		}
		/*
		 * we now have the condition where we haven't processed this
		 * before and we have a resource map.  Extract out the
		 * resources and initialize a map.
		 */
		if (ra_init_null(par) == DDI_SUCCESS) {
		    int rcount;
		    rcount = rlen / sizeof (pci_regspec_t);
		    map = ra_find_mapset(par);
		    if (map != NULL) {
			for (i = 0; i < rcount; i++) {

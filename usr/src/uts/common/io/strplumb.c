...
#pragma ident	"@(#)strplumb.c	1.31	97/11/22 SMI"
...
static int resolve_netdrv(char *ifname, int *unitp, major_t *ndev);
...
strplumb(void)
{
	...
	LOG_MAJ	= ddi_name_to_major(LOG);

	/* this is independent of the protocols stream */
	if (strplumbdebug)
		printf("strplumb: configure dedicated log stream\n");
	if (error = kstr_open(LOG_MAJ, DEDICATED_Q, &logvp, &logfd)) {
		printf("strplumb: kstr_open LOG_MAJ/LOG_MIN failed error %d\n",
			error);
	}
	...
	if (error = kstr_open(CLONE_MAJ, IP_MAJ, &nvp, NULL)) {
...
static int
resolve_netdrv(char *ifname, int *unitp, major_t *ndev)
{
	minor_t			unit;
	dev_t			dev;
	char			*devname;
	...
	if ((strncmp(backfs.bo_fstype, "nfs", 3) == 0) ||
		...
		*ndev = getmajor(dev);
		...
		(void) sprintf(ifname + strlen(ifname), "%d", unit);
		*unitp = unit;
	} else {
		if (strplumbdebug) {
		...
		*ndev = ddi_name_to_major(ndev_name);
		...
		(void) sprintf(ifname, "%s%d", ndev_name, ndev_unit);
		*unitp = ndev_unit;
	}
	return (0);
}

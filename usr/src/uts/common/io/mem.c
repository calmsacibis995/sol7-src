...
#pragma ident	"@(#)mem.c	1.44	97/10/30 SMI"
...
mmrw(dev_t dev, struct uio *uio, enum uio_rw rw, cred_t *cred)
{
	...
	while (uio->uio_resid > 0 && error == 0) {
		...
		case M_MEM:
			...
			c = MIN((size_t)(NBPG - o), (size_t)iov->iov_len);
			/*
			 ...
			 */
			c = MIN(c, (size_t)(NBPG -
			    ((uintptr_t)iov->iov_base & MMU_PAGEOFFSET)));
			error = uiomove(&mm_map[o], c, rw, uio);
    ...
    u_int prot, u_int maxprot, u_int flags, struct cred *cred)
{
	...
	if ((flags & MAP_FIXED) == 0) {
		 ...
		 */
		map_addr(addrp, len, (offset_t)off, minor == M_KMEM, flags);
		if (*addrp == NULL) {
			as_rangeunlock(as);
			return (ENOMEM);
		}

...
#pragma ident	"@(#)pm.c	1.63	97/10/22 SMI"
...
typedef struct {
	...
	clock_t		pm_schedule;	/* time between scans in ticks */
...
extern int	nodev();
extern int	hz;
extern int	(*pm_dev_is_needed)(dev_info_t *, int, int);
static int	(*pm_dev_is_needed_save)(dev_info_t *, int, int);
...
/* Local functions */
static int		dev_is_needed(dev_info_t *, int, int);
...
_init(void)
{
	...
	(void) ddi_soft_state_init(&pm_state, sizeof (pm_unit), 1);
...
pm_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	case DDI_ATTACH:
		if (pm_instance != -1)		/* Only allow one instance */
			return (DDI_FAILURE);
		pm_dev_is_needed_save = pm_dev_is_needed;
		pm_dev_is_needed = dev_is_needed;
		...
		unitp->pm_dip = dip;
		unitp->pm_schedule = (clock_t)(5 * hz);
		unitp->pm_devices = 0;
		mutex_init(&unitp->pm_scan, NULL, MUTEX_DRIVER, NULL);
		rw_init(&unitp->pm_unload, NULL, RW_DRIVER, NULL);
		...
		ddi_report_dev(dip);
		unitp->pm_cpr_cb_id = callb_add(pm_cpr_callb, (void *)unitp,
		    CB_CL_CPR_DAEMON, "pm_cpr");
		unitp->pm_panic_cb_id = callb_add(pm_panic_callb, (void *)unitp,
		    CB_CL_PANIC, "pm_panic");
		unitp->pm_scan_id = timeout(start_scan, unitp,
		    unitp->pm_schedule);
		return (DDI_SUCCESS);
...
pm_detach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
	...
	case DDI_DETACH:
		...
		(void) callb_delete(unitp->pm_cpr_cb_id);
		(void) callb_delete(unitp->pm_panic_cb_id);
		pm_dev_is_needed = pm_dev_is_needed_save;
...
pm_ioctl(dev_t dev, int cmd, intptr_t arg, int mode, cred_t *cr, int *rval_p)
{
	...
	if ((mode & DATAMODEL_MASK) == DATAMODEL_ILP32) {
		...
		if (ddi_copyin((caddr_t)arg, &req32,
		    sizeof (pm_request32), mode) != 0)
			return (EFAULT);
		req.who = (void *)req32.who;
		req.select = req32.select;
		req.level = req32.level;
		req.dependent = (void *)req32.dependent;
		req.size = req32.size;
	} else
#endif
	{
		if (ddi_copyin((caddr_t)arg, &req,
		    sizeof (pm_request), mode) != 0)
			return (EFAULT);
	}
	...
	case PM_GET_DEP: {
		...
		if (strlen(buffer) >= req.size)
			ret = EINVAL;
		else if (ddi_copyout(buffer, req.dependent,
		    strlen(buffer)+1, mode) != 0)
			ret = EFAULT;
		*rval_p = 0;
		break;
	}
	...
	case PM_REM_DEVICE:
		...
		for (k = 0; k < info->dependents; k++) {
			...
			length = dep_info->dependees *
			    sizeof (dev_info_t *);
			new_dep = kmem_alloc(length -
			    sizeof (dev_info_t *), KM_SLEEP);
			for (i = 0, j = 0; j < dep_info->dependees-1; i++) {
				ASSERT(i < dep_info->dependees);
				if (dep_info->dependee[i] == dip)
					continue;
...
/*
 * Entry point for drivers. Exported by global variable pm_dev_is_needed.
...
dev_is_needed(dev_info_t *who, int cmpt, int level)
{
	...
	rw_enter(&unitp->pm_unload, RW_READER);
	/*
	 * Check if the power manager is active and if the the
...
start_scan(void *arg)
{
	...
	mutex_enter(&unitp->pm_scan);
	/*
	 * Protect from hotplugging tree changes
	 */
...
scan(dev_info_t *dip)
{
	...
	for (; dip != NULL; dip = ddi_get_next_sibling(dip)) {
		...
		ASSERT(info->components == size / (sizeof (time_t)));
...
power_up(dev_info_t *who, int component, int level, pm_info_t *info)
{
	...
	if (component != 0) {
		...
		if (info->cmpt_pwr[0] == 0) {
			...
			(void) power_dev(who, 0, power[0], info);
			kmem_free(power, size);
		}
	}
...
power_dev(dev_info_t *dip, int component, int level, pm_info_t *info)
{
	...
	if (resume_needed && devi_attach(dip, DDI_PM_RESUME) != DDI_SUCCESS) {
...
name_to_dip(char *pathname)
{
	...
	if (!pathname || copyinstr(pathname, dev_name, MAXNAMELEN,
	    &len) || dev_name[0] == '\0')
		return (NULL);
...
pm_panic_callb(void *arg, int code)
{
	...
	if (pm_get_norm_pwrs(dip, &power, &size) == DDI_SUCCESS) {
		for (i = 0; i < info->components; i++)
			if (info->cmpt_pwr[i] == 0)
				(void) power_up(dip, i, power[i], info);
		kmem_free(power, size);
	}
	mutex_exit(&info->node_lock);
	rw_exit(&unitp->pm_unload);
...
pm_check_permission(char *fname, cred_t *cr)
{
	...
	if (error = copyinstr(fname, dev_name, MAXNAMELEN, &len)) {
		cmn_err(CE_WARN,
		    "pm: Can't copy device name at user address %p "
		    "into the kernel", (void *)fname);
		return (error);
	}

	if (dev_name[0] == '\0' || (error =
	    lookupname(dev_name, UIO_SYSSPACE, FOLLOW, NULLVPP, &vp))) {

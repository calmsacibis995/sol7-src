...
#pragma ident	"@(#)vjcomp.c	1.15	97/09/25 SMI"
	...
	vjstruct_t *comp;
{
	...
	comp->last_recv = (u_char) 255;
	comp->last_xmit = (u_char) 255;
	comp->flags = SLF_TOSS;
...
void
vjsetinparms(comp, maxslot, cslot)
	vjstruct_t *comp;
	u_int	   maxslot;
	u_int	   cslot;
{
 ...
 */
void
vjsetoutparms(comp, maxslot, cslot)
	vjstruct_t *comp;
	u_int	   maxslot;
	u_int	   cslot;
{
 ...
 */
int
vjcompress_tcp(ptrmp, comp)
	mblk_t **ptrmp;
	vjstruct_t *comp;
{
	register mblk_t *mp, *newmp;
 ...
 */
	...
	th->th_sum  = htons((((u_short)*cp) << 8) + (u_short)cp[1]);
	cp += 2;
	...
	cp = newmp->b_wptr;
	cs->cs_ip.ip_len = htons(msgdsize(dmp) + cs->cs_hlen);

...
#pragma ident	"@(#)devinfo.c 1.17	98/01/14 SMI"
...
#ifdef DEBUG
static int di_debug;
#define	dcmn_err(args) if (di_debug > 0 && di_debug <= 1) cmn_err args
#define	dcmn_err2(args) if (di_debug > 0 && di_debug <= 2) cmn_err args
#define	dcmn_err3(args) if (di_debug > 0 && di_debug <= 3) cmn_err args
#else
...
static int di_max_opens = 32;	/* Up to this many simultaneous opens */

#define	DI_CHUNK_SIZE	(PAGESIZE << 3)	/* alloc 8 pages at a time */
#define	DI_LOW_WATER	(PAGESIZE >> 1)	/* mininum space left 1/2 page */
 ...
 * Each chunk is of size DI_CHUNK_SIZE
 */
struct di_cache {
...
static kcondvar_t di_open_cv;	/* wait for available open */
static int di_open_count;	/* number of current open instance */
...
#ifdef DI_DEVMAP_INTERFACE
static int di_devmap(dev_t, devmap_cookie_t, offset_t, size_t,
    size_t *, uint_t);
#endif

static int di_info(dev_info_t *dip, ddi_info_cmd_t infocmd, void *arg,
    void **result);
static int di_attach(dev_info_t *, ddi_attach_cmd_t cmd);
static int di_detach(dev_info_t *, ddi_detach_cmd_t cmd);
...
static di_off_t di_copytree(struct dev_info *, di_off_t *,
    struct di_state *);
static di_off_t di_copynode(struct di_stack *, struct di_state *);
static di_off_t di_getmdata(struct ddi_minor_data *, di_off_t *,
    struct di_state *);
static di_off_t di_getppdata(struct dev_info *, di_off_t *,
    struct di_state *);
static di_off_t di_getdpdata(struct dev_info *, di_off_t *,
    struct di_state *);
static di_off_t di_getprop(struct ddi_prop *, di_off_t *,
    struct di_state *, struct dev_info *, major_t, int);
static void di_initlist(struct di_state *st);
static void di_freelist(struct di_state *st);
static void di_insertlist(struct di_list **head, di_off_t *offp,
    struct dev_info *dip);
static void di_matchlist(struct di_state *st);
static void di_getmem(struct di_state *);
...
_init(void)
{
	...
	cv_init(&di_open_cv, NULL, CV_DRIVER, NULL);

	error = mod_install(&modlinkage);
	if (error != 0) {
		mutex_destroy(&di_open_lock);
		cv_destroy(&di_open_cv);
		return (error);
	}
...
di_open(dev_t *devp, int flag, int otyp, cred_t *credp)
{
	...
	if (di_open_count > di_max_opens)  {
		/*
		 * maximum open instance reached, wait for a close
		 */
		dcmn_err((CE_WARN,
		"devinfo: Maximum devinfo open reached, waiting for exit\n"));

		if (cv_wait_sig(&di_open_cv, &di_open_lock) == 0) {
			di_open_count--;
	...
	*devp = makedevice(getmajor(*devp), (minor_t)(m + 1));
	...
	dcmn_err((CE_NOTE, "di_open: threadd = %p, open count = %d (%d)",
		(void *)curthread, di_open_count, m + 1));
...
di_close(dev_t dev, int flag, int otype, cred_t *cred_p)
{
	...
	/*
	 * rectify count and wake up any waiter
	 */
	mutex_enter(&di_open_lock);
	di_open_count--;
	di_states[m] = NULL;
	dcmn_err((CE_NOTE, "di_close: thread = %p, open count = %d (%d)\n",
		(void *)curthread, di_open_count, m + 1));
	if (di_open_count >= di_max_opens) {
		cv_signal(&di_open_cv);
	}
	mutex_exit(&di_open_lock);
...
/*ARGSUSED*/
static int
di_ioctl(dev_t dev, int cmd, intptr_t arg, int mode,
		cred_t *credp, int *rvalp)
{
	...
	case DINFOLODRV:
		...
		if ((i != (major_t)-1) &&
		    (ddi_hold_installed_driver(i) != NULL)) {
			ddi_rele_driver(i);
			return (0);
		}
		return (ENXIO);
	...
	di_getmem(st);		/* allocate first memory chunk */
	...
	/*
	 * Now, pull ioctl arguments into kernel,
	 * store at the beginning of the snapshot.
	 */
	 ...
	 * XXX it's not supported in multidata model.
	 */
...
/*
 * Get a chunk of memory for the snapshot
 */
static void
di_getmem(struct di_state *st)
{
	struct di_cache *dcp;
	struct di_cache *cache = kmem_zalloc(sizeof (struct di_cache),
								KM_SLEEP);
	char *buf = ddi_umem_alloc((size_t)DI_CHUNK_SIZE,
					DDI_UMEM_SLEEP, &cache->cook);
	cache->buf = buf;
	...
	/*
	 * Allocate locked memory here because libdevinfo
	 * makes a copy of this memory and frees it immediately.
	 *
	 * XXX--If devmap interface is used, we should consider using
	 * pageable memory. In that case, we need to lock the
	 * memory explicitly during snapshot and unlock before
...
/*
 * Make sure there is at least size / DI_LOW_WATER memory left before
 * going on. Otherwise, start on a new chunk.
 */
static di_off_t
di_checkmem(struct di_state *st, di_off_t off, size_t size)
{
	dcmn_err2((CE_CONT, "di_checkmem: off=%x size=%x\n",
			off, (int)size));

	/*
	 * XXX better to make DI_CHUNK_SIZE based on the request size.
	 *	Assertion will do for now.
	 */
	ASSERT(size < DI_CHUNK_SIZE);
...
di_cache_addr(struct di_state *st, di_off_t off)
{
	...
	dcmn_err2((CE_CONT, "di_cache_addr: dcp=%p off=%x\n",
		(void *)dcp, off));

	while (off >= DI_CHUNK_SIZE) {
		off -= DI_CHUNK_SIZE;
		dcp = dcp->next;
	}
...
di_snapshot(struct di_state *st)
{
	...
	if (rootnode == NULL) {
		dcmn_err((CE_NOTE, "Devinfo node %s not found\n",
		    all->root_path));
		di_rele_drivers(st);
		return (0);
	}
...
/*
 * This function must be called with devinfo_tree_lock held
 */
static dev_info_t *
...
/*
 * Adapted from ddi_pathname_to_dev_t()
 * Return 0 on success, 1 on failure
 */
static dev_info_t *
di_hold_drivers_in_path(struct di_state *st, char *pathname)
{
	struct pathname pn;
	dev_info_t *child;
	...
	dcmn_err2((CE_CONT, "di_hold_drivers_in_path: pathname = %s\n",
		pathname));

	if (pn_get(pathname, UIO_SYSSPACE, &pn))
		return (NULL);

	child = ddi_root_node();	/* Begin at the top of the tree */
	pn_skipslash(&pn);
	...
	while (pn_pathleft(&pn)) {
		...
		if (child)
			drvname = ddi_binding_name(child);
		if (drvname == NULL)
			drvname = cname;
		rw_exit(&(devinfo_tree_lock));

		/*
		 * Now, hold the driver of the child node. If we cannot
		 * do this, then bail out. Make sure we don't hold a
		 * driver multiple times (e.g. USB hid nodes may appear
		 * multiple times along a single devinfo tree branch).
		 */
		...
		dcmn_err2((CE_CONT, "di_hold_drivers_in_path: "
		...
		/*
		 * We must look for child again because its address
		 * may have changed after we released devinfo_tree_lock
		 * and before we held the driver.
		 */
		rw_enter(&(devinfo_tree_lock), RW_READER);
		child = di_find_child(parent, cname, caddr);
		rw_exit(&(devinfo_tree_lock));
...
di_hold_drivers(struct di_state *st, char *path, int force_load)
{
	...
	if ((node = di_hold_drivers_in_path(st, path)) == NULL)
		return (NULL);
...
di_tryhold_driver(struct di_state *st, major_t major)
{
	...
	if (st->drivers_held[(int)major] == 0)
		dcmn_err((CE_NOTE, "Fail to tryhold driver major %d",
		    (int)major));
...
di_copytree(struct dev_info *root, di_off_t *off_p, struct di_state *st)
{
	...
	dcmn_err2((CE_CONT, "di_copytree: root = %p, *off_p = %x\n",
		(void *)root, *off_p));
	...
	struct di_state *st)
{
	 ...
	 * limited to DI_LOW_WATER. Can override by supplying
	 * a value.
	 */
	off = di_checkmem(st, *off_p, 0);

	if ((pdp->bytes <= 0) || pdp->bytes > DI_LOW_WATER) {
		goto failure;
	}
	...
	if (!on_fault(&ljb)) {
		...
		for (i = 0; i < MAX_PTR_IN_PRV; i++) {
			 ...
			 */
			if ((size <= 0) || (size > (DI_LOW_WATER - off0))) {

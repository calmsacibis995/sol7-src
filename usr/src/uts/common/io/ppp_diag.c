...
#pragma ident	"@(#)ppp_diag.c	1.16	97/10/22 SMI"
...
static int chrsrv(), chwsrv(), chrpu(), chwpu();
static int mclose(), mopen();
...
static int process_lcp_opts(u_char **, int *, ppp_diag_struct_t *);
static int process_ipcp_opts(u_char **, int *, ppp_diag_struct_t *);
static int process_ipcp(u_char **, int *, ppp_diag_struct_t *);
static int process_pap(u_char **, int *, ppp_diag_struct_t *);
static int process_chap(u_char **, int *, ppp_diag_struct_t *);
static int process_lcp(u_char **, int *, ppp_diag_struct_t *);
static int process_proto_rej(u_char **, int *, ppp_diag_struct_t *);
static int process_echo(u_char **, int *, ppp_diag_struct_t *);
static int process_discard(u_char **, int *, ppp_diag_struct_t *);
static int process_lqm(u_char **, int *, ppp_diag_struct_t *);
static void hexdump(int, unsigned char *, int, ppp_diag_struct_t *);
static void trunchexdump(int, u_char *, int, int, ppp_diag_struct_t *);
static int decompress_frame(mblk_t *, int *, int *, u_short *,
    u_short *, ppp_diag_struct_t *);
	...
	cred_t *credp;
{
	...
	dp->upmask = 0xffffffffUL;
	dp->downmask = 0xffffffffUL;
	...
	ppp_diag_struct_t *dp;
{
	...
	case M_DATA:
		...
		hexdump(PPP_DG_RAW, check, msgdsize(mp), dp);
		outputline(dp);

		if (dp -> diag_info.media_type == pppAsync) {
			bp = mp;
			while (check < (u_char *)bp-> b_wptr) {
				if (*check == 0x7e) {
					if (cp != NULL) {
...
static void
trunchexdump(debuglevel, buf, cnt, max_cnt, dp)
	int debuglevel;
	u_char *buf;
	int cnt, max_cnt;
	ppp_diag_struct_t *dp;
	...
	ppp_diag_struct_t *dp;
{
	...
	if (strlen(src) >= PPP_DG_MAX_OUTPUT - dp->outputbuflen)
		dp->outputbuflen = PPP_DG_MAX_OUTPUT;
	else
		dp->outputbuflen += strlen(src);
	dp->outputbuf[dp->outputbuflen] = '\0';
	}
}

static void
init_strlog(mid, sid, tracelevel, flags, dp)
int mid, sid, tracelevel, flags;
	ppp_diag_struct_t *dp;
{
	...
	ppp_diag_struct_t *dp;
{
	...
	if (dp->diag_info.media_type == pppAsync) {
		...
		if (filter_esc(direc, mp, dp) < 0) {
			...
			hexdump(PPP_DG_ERRREP, mp -> b_rptr, msgdsize(mp), dp);
			outputline(dp);
			return;
		}
		...
		trunchexdump(PPP_DG_RFR, bufp, msgdsize(mp), PROTOHEXLIM, dp);
		outbuf(PPP_DG_RFR, ") ", dp);
		bufp = mp-> b_rptr;
		cnt = msgdsize(mp);
...
static int
decompress_frame(mp, addrcomp, protocomp, proto, fcs, dp)
	mblk_t *mp;
	int	*addrcomp;
	int	*protocomp;
	u_short *proto;
	...
	ppp_diag_struct_t *dp;
{
	...
	int looking = 0;
	char valstr[20];
	u_char *sptr, *cptr, val;
	u_long unescaped = 0, tst;
	int i;

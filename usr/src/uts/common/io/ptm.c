...
#pragma ident	"@(#)ptm.c	1.39	97/10/22 SMI"
...
ptm_init(void)
{
	...
	mutex_init(&pt_lock, NULL, MUTEX_DEFAULT, NULL);
	for (dev = 0; dev < pt_cnt; dev++)
		cv_init(&ptms_tty[dev].pt_cv, NULL, CV_DEFAULT, NULL);
}
	...
	cred_t  *credp)		/* credentials */
{
	...
	if (ptmp->pts_rdq) {
		if (ptmp->pts_rdq->q_next) {
			...
			(void) putnextctl(ptmp->pts_rdq, M_HANGUP);
		}
	}
	PT_EXIT_READ(ptmp);

...
#pragma	ident	"@(#)iwscons.c	1.33	97/06/22 SMI"
...
static int	iwscnidentify(dev_info_t *);
...
static struct modldrv modldrv = {
	...
	"Workstation Redirection driver 'iwscn'",
...
static int
iwscnidentify(dev_info_t *devi)
{
	if (strcmp(ddi_get_name(devi), "iwscn") == 0)
...
srreset(wcd_data_t *wd, int flag, cred_t *cred)
{
	...
	if (wlp != NULL && wlp->wl_vp == wd->wd_vp) {
		 ...
		 */
		if (vp != wd->wd_vp)
			cmn_err(CE_PANIC, "srreset: Illegal clone");
		if (!err)
			wd->wd_wsconsopen++;
	}
	return (err);
...
/*
 * Return the vnode contained in the topmost entry of the redirection stack or
 * NULL if the stack is empty.
 *
 * NOTE:  If the topmost entry on the redirection stack is valid, the
 * "readers" version of iwscn_lock on the driver is not dropped.  The
 * caller must call srredirecteevp_rele() to release this lock.
 */
vnode_t *
srredirecteevp_lookup(void)
{
	wcrlist_t	*wlp;
	vnode_t		*vp;

	if (rw_tryenter(&iwscn_lock, RW_READER) == 0)
		return ((vnode_t *)0);
	if (wcddata == NULL || (wlp = wcddata->wd_list) == NULL) {
		rw_exit(&iwscn_lock);
		return (NULL);
	}
	ASSERT(wlp != NULL);
	...
	ASSERT(RW_READ_HELD(&iwscn_lock));
	return (vp);
}

void
srredirecteevp_rele(void)
{
	ASSERT(RW_READ_HELD(&iwscn_lock));

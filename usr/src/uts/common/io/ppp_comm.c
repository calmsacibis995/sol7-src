#pragma ident	"@(#)ppp_common.c	1.21	97/10/22 SMI"
...
int
is_open(machp)
pppMachine_t	*machp;		/* pointer to state machine */
{
...
{
	register pppTuple_t		tuple;
	register pppAction_t		func;
	register pppState_t		old_state, new_state;
	...
	if (tuple == FSM_ERR) {
#ifdef PPP_DEBUG
		PPP_FSMDBG4("ppp_fsm out of fsm: prot = %x machp = %x \
		    state %s, event = %s\n",
...
#ifdef PPP_DEBUG
	PPP_FSMDBG4("ppp_fsm: prot=%x machp=%x state %s, event=%s\n",
	    machp->protocol, machp,
	    state_txt[machp->state], event_txt[event]);
#endif
 ...
 */
void
do_incoming_cp(machp, mp)
pppMachine_t		*machp;
mblk_t			*mp;
{
	register pppEvent_t		event;
	register struct ppp_hdr		*hp;
	register pppOption_t		*imp_opt;
	pppOption_t			op;
	register mblk_t			*zmp, *nmp, *conf_nak, *conf_rej,
					*mag_nak;
	register int			rc;
	int				pass;
	...
	if (machp->pkt != NULL) { /* XXX check for corruption here */
		PPP_STRDBG("ppp: non-null pkt detected machp=%x\n", machp);
 ...
 */
			...
			freemsg(mp);
			machp->pkt = conf_rej;
			PPP_OPTDBG("ppp: machp=%x, conf.rej\n", machp);
			event = Rcrbad;
			break;
		}
 ...
 */
			...
			freemsg(mp);
			machp->pkt = conf_nak;
			PPP_OPTDBG("ppp: machp=%x, conf.nak\n", machp);
			event = Rcrbad;
			break;
		}
 ...
 */
	...
	case EchoReq:
		if (pktlength < 8)
			goto discard;
		magic_ptr = (u_int *)(mp->b_rptr + PPP_HDRSZ);
 ...
 */
static void
ppp_action(machp, action, new_state)
pppMachine_t	*machp;
pppAction_t	action;
pppState_t	new_state;
{
 ...
 */
	...
	hp->pkt.length = htons(ntohs(hp->pkt.length) + msgdsize(mp));
...
remove_opt(pppOption_t *optptr[], int type)
{
	...
	PPP_MEMDBG2("remove_opt: freeing %x, type=%x\n", op, type);
 ...
 */
int
add_opt(optptr, type, length, data, negotiate)
pppOption_t	*optptr[];
u_int		type;
u_int		length;
caddr_t		data;
 ...
 */
	...
	PPP_MEMDBG2("add_opt: allocated addr=%x,  type = %x\n", op, type);
...
int		optsz;
{
	...
	PPP_MEMDBG2("free_opt: optptr = %x,  optsz = %x\n", optptr, optsz);
	for (i = 0; i < optsz; i++) {

		for (op = optptr[i]; op != NULL; op = nop) {
			nop = op->next;
			PPP_MEMDBG2("free_opt: freeing %x,  type = %x\n",
			    op, i);
			kmem_free(op, sizeof (pppOption_t));
		}
...
{
	if (machp->restart) {
	    PPP_TIMDBG("start_restart_timer: on machp %x - already running\n",
							machp);
		return;
	}
	PPP_TIMDBG("start_restart_timer: on machp %x\n", machp);
...
pppMachine_t	*machp;
{
	...
	if (machp->restart == 0) {
		PPP_TIMDBG("cancel_restart_timer: not running on machp %x\n",
							machp);
		return;
	}

	PPP_TIMDBG("cancel_restart_timer: cancelling timer on machp %x\n",
							machp);
...
ppp_timeout(void *arg)
{
	...
	PPP_TIMDBG("ppp_timeout: on machp %x\n", machp);
...
pppMachine_t	*machp;
{
	PPP_MEMDBG("free_machine: freeing machine %x\n", machp);
...
/* ARGSUSED */
static int
is_contained_in(set, subset)
mblk_t	*set, *subset;
{
	u_char *optptr, *optptrsub;
 ...
 */
	...
	machp->stats.pppCPInOctets += msgdsize(mp) - PPP_FR_HDRSZ;
}
 ...
 */
	...
	machp->stats.pppCPOutOctets += msgdsize(mp) - PPP_FR_HDRSZ;
}


void
ip_pkt_in(machp, mp, proto)
pppMachine_t		*machp;
mblk_t			*mp;
u_int			proto;
{
 ...
 */
	...
	machp->ip.pppIPInOctets += msgdsize(mp) - PPP_FR_HDRSZ;
	switch (proto) {
		case pppIP_PROTO:
			machp->ip.pppIPInIP++;
			break;
 ...
 */
	...
	machp->ip.pppIPOutOctets += msgdsize(mp) - PPP_FR_HDRSZ;
	switch (proto) {
		case pppIP_PROTO:
			machp->ip.pppIPOutIP++;
			break;

...
#pragma ident	"@(#)tl.c	1.44	97/12/06 SMI"
...
/*
 * Table representing database of all options managed by O_T_OPTMGMT_REQ
...
tl_optmgmt(queue_t *wq, mblk_t *mp)
{
	...
	if (!(tep->te_mode & TL_SOCKET) && tep->te_state != TS_IDLE &&
		...
		(void) (STRLOG(TL_ID, tep->te_minor, 1,
			SL_TRACE|SL_ERROR,
			"tl_wput:O_T_OPTMGMT_REQ:out of state, state=%d",
			tep->te_state));
		/*
		...
		tl_error_ack(wq, ackmp, TOUTSTATE, 0, O_T_OPTMGMT_REQ);
		freemsg(mp);
		return;
	}

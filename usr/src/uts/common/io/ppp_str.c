...
#pragma ident	"@(#)ppp_str.c	1.39	97/10/22 SMI"
...
static void device_external_event(pppLink_t *, u_int);
static void external_event(pppLink_t *, u_int, pppProtocol_t);
 ...
 */
char ppp_module_name[] = "async/sync ppp v1.39";
...
static int
#ifndef _SunOS4
ppp_open(q, devp, flag, sflag, credp)
#else
ppp_open(q, devp, flag, sflag)
#endif
queue_t		*q;		/* -> readq		*/
dev_t		devp;		/* maj/min device num	*/
int		flag;		/* flags		*/
int		sflag;		/* streams open flags	*/
#ifndef _SunOS4
...
{
	register pppLink_t	*lp = (pppLink_t *)q->q_ptr;
...
{
	mblk_t			*mp, *zmp;
	register pppLink_t	*lp;
	register u_int		pck_type;
	register int		vj_type;
...
queue_t		*q;
{
	mblk_t				*mp;
	register pppLink_t		*lp;
	register struct ppp_frame_hdr	*hp;
	u_short				protocol;
 ...
 */
void
external_event(lp, event, protocol)
pppLink_t	*lp;
u_int		event;
 ...
 */
void
ppp_error_ind(lp, error, errdata, error_cnt)
pppLink_t		*lp;
enum ppp_errors		error;
u_char			*errdata;
u_int			error_cnt;
{
	register mblk_t			*mp;
	register pppError_t		*msg;
 ...
 */
void
ppp_cross_fsm(linkp, event, protocol, pri)
pppLink_t		*linkp;
u_int			event;
pppProtocol_t		protocol;
u_int			pri;
 ...
 */
void
ppp_notify_lm(lp, event, protocol, data, datalen)
pppLink_t		*lp;
u_int			event;
pppProtocol_t		protocol;
caddr_t			data;
 ...
 */
void
ppp_internal_event(linkp, message, protocol)
pppLink_t	*linkp;
u_int		message;
pppProtocol_t	protocol;
{
 ...
 */
void
ppp_notify_config_change(machp)
pppMachine_t	*machp;
{
	register mblk_t			*mp;
	register pppConfig_t	*msg;
...
check_param(mblk_t *mp, size_t size)
{
	...
	index = * (u_short *)mp->b_cont->b_rptr;
...
{
	register struct iocblk		*iocp;
	register pppLinkStatusEntry_t	*state;
	register pppCPEntry_t		*cpstats;
	register pppIPEntry_t		*ipstats;
	register pppLinkErrorsEntry_t	*errs;
	register int			rc;
	register pppLink_t		*lp;
	register u_int			mibent;
 ...
 */
	...
	lp->conf.pppLinkLocalACCMap		= (u_long) PPP_DEF_ASCM;
	lp->conf.pppLinkRemoteACCMap		= (u_long) PPP_DEF_ASCM;
	...
	for (i = 1; i <= PPP_MAX_MIB; i++)
		if (ppp_mib[i] == NULL) {
			ppp_mib[i] = lp;
			lp->conf.pppLinkControlIndex = (u_short) i;
			lp->state.pppLinkStatusIndex = (u_short) i;
			lp->errs.pppLinkErrorsIndex = (u_short) i;
			lp->lcp->stats.pppCPLinkNumber = (u_short) i;
			lp->ncp->stats.pppCPLinkNumber = (u_short) i;
			lp->ncp->ip.pppIPLinkNumber = (u_short) i;
			break;
		}
	rw_exit(&miblock);
	mutex_exit(&lp->lplock);
 ...
 */
mblk_t *
ppp_alloc_frame(protocol, code, id)
pppProtocol_t	protocol;
u_int		code;
u_int		id;
{
 ...
 */
	...
	*cp++ = PPP_FRAME_ADDR;
	*cp++ = PPP_FRAME_CTRL;
	*cp++ = (u_char) ((protocol >> 8) & 0xff);
	*cp++ = (u_char) (protocol & 0xff);
 ...
 */
	...
	if (protocol != pppLCP) {
		...
		if (!lp->conf.pppLinkSendAddrComp) {
			*cp++ = (u_char) PPP_FRAME_ADDR; /* 0xff */
			*cp++ = (u_char) PPP_FRAME_CTRL; /* 0x03 */
		}
		if (lp->conf.pppLinkSendProtoComp && protocol < 0x100) {
			*cp++ = (u_char) (protocol & 0xff);
		} else {
	...
	if (drv_getparm(LBOLT, &mylbolt) != -1) {
		if (ABS((long)mylbolt - lp->conf.pppLinkLastFlagTime) >
	...
	for (dp = mp->b_rptr; dp < mp->b_wptr; dp++) {
		...
		if (*dp == PPP_FLAG) {
			...
			if (lp->pib.fcs == PPP_GOODFCS) {
				rval = lp->pib.mblk;
				(void) adjmsg(rval, - sizeof (u_short));
				lp->pib.mblk = NULL;
/*
 ...
 */
	...
	printf("frame_dump %s: ");
	do {
		for (dp = (u_char *)fp->b_rptr; dp < (u_char *)fp->b_wptr;
								dp++) {
			printf("%x ", *dp);
			if (++i % 60 == 0) {
				printf("\n");

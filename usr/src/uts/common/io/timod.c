...
#pragma ident	"@(#)timod.c	1.63	97/12/10 SMI"	/* SVr4.0 1.11	*/
...
timodrproc(queue_t *q, mblk_t *mp)
{
	...
	case M_ERROR:
		 ...
		 * by timod.
		 */
		if ((tp->tim_provinfo == PI_DONTKNOW) &&
	...
	case M_PCPROTO:
		...
		if (((tp->tim_flags & WAITIOCACK) == 0) ||
		    ((tp->tim_saved_prim != O_T_OPTMGMT_REQ) &&
...
timodwproc(queue_t *q, mblk_t *mp)
{
	...
	case M_IOCTL:
	    ...
	    case TI_CAPABILITY: {
		    ...
		    ASSERT(tp->tim_provinfo->tpi_capability == PI_NO);
		/*
		 ...
		 */
		    if ((tcr->CAP_bits1 & TC1_INFO) != 0) {
			    tp->tim_flags |= (TI_CAP_RECVD | CAP_WANTS_INFO |
				WAIT_IOCINFOACK);
			    TILOG("timodwproc: sending down T_INFO_REQ, "
				"flags = %x\n", tp->tim_flags);

			/*
			 * Generate T_INFO_REQ message and send it down
			 */
		 ...
		 */
		    ...
		    tp->tim_flags &= ~(WAITIOCACK | WAIT_IOCINFOACK |
			TI_CAP_RECVD | CAP_WANTS_INFO);
		    ((struct T_capability_ack *)tcsr_mp->b_rptr)->CAP_bits1 = 0;

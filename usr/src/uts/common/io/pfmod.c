...
#pragma ident	"@(#)pfmod.c	1.12	97/10/22 SMI"
...
pfrput(queue_t *rq, mblk_t *mp)
{
	...
	case M_DATA:
		 ...
		 */
		if (((uintptr_t)mp->b_rptr & (sizeof (u_short) - 1)) ||
		    (mbp != NULL &&
		    ((uintptr_t)mbp->b_rptr & (sizeof (u_short) - 1)))) {
			freemsg(mpp);
			break;
		}
...
pfioctl(queue_t *wq, mblk_t *mp)
{
	...
	case PFIOCSETF:
		 ...
		 */
		bcopy((caddr_t)upfp, (caddr_t)pfp,

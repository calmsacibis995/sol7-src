...
#pragma ident	"@(#)ppp_pap.c	1.22	97/10/22 SMI"
...
static void make_response(u_int, u_int, papMachine_t *);
static papPasswdEntry_t *get_isdn_passwd(mblk_t *);
static int set_isdn_passwd(mblk_t *, papPasswdEntry_t *);
static int pap_extract_passwd(mblk_t *, papPasswdEntry_t *, u_int);
...
papEvent_t event;
{
	...
	register papAction_t		func;
	register papState_t		new_state;
 ...
 */
void
pap_action(machp, action, new_state)
papMachine_t *machp;
papAction_t action;
...
{
	register mblk_t			*mp;
	register pppReqValidation_t	*msg;
 ...
 */
void
make_response(value, repid, machp)
u_int value;
u_int repid;
papMachine_t	*machp;
{
...
mblk_t			*mp;
{
	...
	case Authenticate:
		...
		if (pap_verify(&passwd, machp)) {
			make_response((u_char) AuthenticateAck, repid, machp);
			event = papAuthGood;
		} else {
			make_response((u_char) AuthenticateNak, repid, machp);
			event = papAuthBad;
		}
		break;
 ...
 */
static int
pap_extract_passwd(mp, passwd, totlen)
mblk_t *mp;
papPasswdEntry_t *passwd;
u_int totlen;
{
 ...
 */
papMachine_t *
alloc_pap_machine(readq, linkp)
queue_t		*readq;
pppLink_t	*linkp;

{
	register papMachine_t *machp;

/*
 * Copyright (c) 1987-1990, 1996-1997, by Sun Microsystems, Inc.
 * All rights reserved.
 */

#pragma ident	"@(#)kbd.c	1.109	97/12/09 SMI"	/* SunOS-4.0 1.60	*/
...
struct	kbddata {
	...
	u_short	compose_key;		/* first compose key */
...
static void	kbdreset(struct kbddata *, u_int);
...
static struct	keymap *settable(struct kbddata *, u_int);
static void	kbdrpt(void *);
static void	kbdcancelrpt(struct kbddata *);
static void	kbdtranslate(struct kbddata *, unsigned, queue_t *);
static int	kbd_do_compose(u_short, u_short, u_short *);
static void	kbd_send_esc_event(char, struct kbddata *);
char		*strsetwithdecimal(char *, u_int, u_int);
static void	kbdkeypressed(struct kbddata *, u_char, Firm_event *, u_short);
static void	kbdqueuepress(struct kbddata *, u_char, Firm_event *);
static void	kbdkeyreleased(struct kbddata *, u_char);
static void	kbdreleaseall(struct kbddata *);
static void	kbdputcode(u_int, queue_t *);
...
kbdioctl(register queue_t *q, register mblk_t *mp)
{
	...
	case KIOCSETKEY:
		err = kbdsetkey(kbdd, (struct kiockey *)data, iocp->ioc_cr);
		/*
	...
	case KIOCSKEY:
		err = kbdskey(kbdd, (struct kiockeymap *)data, iocp->ioc_cr);
		/*
		 * Since this only affects any subsequent key presses,
		 * don't goto output_format_change.  One might want to
		 * toggle the keytable entries dynamically.
 ...
 */
static void
kbdreset(register struct kbddata *kbdd, u_int hard_reset)
{
 ...
 */
static u_short	special_old_to_new[] = {
	SHIFTKEYS,
	BUCKYBITS,
	FUNNY,
	STRING,
...
kbdsetkey(register struct kbddata *kbdd, struct kiockey *key, cred_t *cr)
{
	...
	if ((tablemask & ALTGRAPHMASK) ||
		...
		return (EINVAL);
	if (key->kio_entry >= (u_char)OLD_STRING &&
	    key->kio_entry <= (u_char)(OLD_STRING + 15)) {
 ...
 */
static u_char	special_new_to_old[] = {
	0,			/* normal */
...
kbdgetkey(register struct kbddata *kbdd, struct	kiockey *key)
{
	...
	if (entry & 0xFF00)
		key->kio_entry =
		    special_new_to_old[(u_short)(entry & 0xFF00) >> 8]
		    + (entry & 0x00FF);
	else {
		if (entry & 0x80)
			key->kio_entry = (u_short)OLD_ISOCHAR;	/* you lose */
		else
			key->kio_entry = (u_short)entry;
	}
	if (entry >= STRING && entry <= (u_char)(STRING + 15)) {
...
kbdskey(register struct kbddata *kbdd, struct kiockeymap *key, cred_t *cr)
{
	...
	if (key->kio_entry >= STRING &&
	    key->kio_entry <= (u_short)(STRING + 15)) {
...
settable(register struct kbddata *kbdd, register u_int mask)
{
...
kbdtranslate(struct kbddata *kbdd, unsigned keycode, queue_t *q)
{
	...
	entrytype = (u_short)(entry & 0xFF00) >> 8;
...
static int
kbd_do_compose(u_short first_entry, u_short second_entry,
	u_short *result_iso_ptr)
{
...
char *
strsetwithdecimal(char *buf, u_int val, u_int maxdigs)
{
	int	hradix = 5;
	char	*bp;
	int	lowbit;
...
static void
kbdkeypressed(struct kbddata *kbdd, u_char key_station, Firm_event *fe,
    u_short base)
{
	register struct keyboardstate *k;
...
static void
kbdqueuepress(struct kbddata *kbdd, u_char key_station, Firm_event *fe)
{
...
static void
kbdkeyreleased(register struct kbddata *kbdd, u_char key_station)
{
 ...
 */
static void
kbdputcode(u_int code, queue_t *q)
{
	register mblk_t *bp;

...
#pragma ident	"@(#)ipdcm.c	1.21	97/10/22 SMI"
...
static void		ipd_timer();
static void		ipd_flush_timers();
...
{
	register struct ipdcm_minor_info *ipdcm_data;
	dev_t  device;
...
cmioctl(queue_t *q, mblk_t *mp)
{
	...
	register int			iftype, ifunit, found, rc;
	register struct iocblk		*iocp = (struct iocblk *)mp->b_rptr;
	register union ipd_messages	*reqp;
	register struct ipd_softc	*ifp;
	register struct sockaddr_in	*sin;
	register struct ipd_addr_tbl	*ap;
	register struct ipdcm_minor_info *ipdcm_data;
...
{
	register struct ipd_softc	*ifp;
	register struct ipd_stat	*ipdsp;
 ...
 */
static void
ipd_tellcm(struct ipd_addr_tbl *ap, enum ipd_msgs type, u_long param)
{
	register mblk_t			*mp, *np;
	register union ipd_messages	*dp;
	register struct sockaddr_in	*sin;
	register struct ipdcm_minor_info *ipdcm_data;
	int				i;
	...
	if (mp == (mblk_t *)NULL) {
		...
		merror(ap->ifp->to_cnxmgr, NULL, ENOBUFS);
		return;
	}
...
print_ifs(struct ipd_softc *ifs)
{
	...
	while (ifs) {
		...
		for (ap = ifs->if_conn; ap; ap = ap->next) {
			printf("ap=%x, rq=%x, timeout=%d, addr_timeout=%d\n",
...
process_interface(struct ipd_softc *tbl)
{
	...
	for (ap = tbl->if_conn; ap; ap = nap) {
		...
		IPD_DEBUG1("ipd_timer: connection %x idle\n", ap->rq);
...
ipd_add_entry(caddr_t addr, int addr_len, struct ipd_addr_tbl **root)
{
	...
	*root = ap;
	...
	IPD_DEBUG1("ipd_add_entry: returning %x\n", ap);
	return (ap);
}
...
ipd_remove_entry(struct ipd_addr_tbl **root, struct ipd_addr_tbl *item)
{
	...
	IPD_DEBUG1("ipd_remove_entry: removing table entry %x\n", item);
...
{
	register struct ipd_str		*stp;
	register struct ipd_str		**prevstp;
	int				minordev;

	IPD_DEBUG1("ipd_open: opening stream %x\n", rq);
...
ipd_close(queue_t *rq)
{
	...
	IPD_DEBUG1("ipd_close: closing stream %x\n", rq);
...
ipd_lwsrv(queue_t *wq)
{
	...
	IPD_FLDEBUG1("ipd_lwsrv: rq=%x\n", RD(wq));

	mutex_enter(&ipd_mutex);

	for (stp = ipd_strup; stp; stp = stp->st_next) {
		IPD_FLDEBUG1("ipd_lwsrv: re-enabling rq %x\n", stp->st_rq);
		qenable(stp->st_rq);
		IPD_FLDEBUG1("ipd_lwsrv: re-enabling wq %x\n", WR(stp->st_rq));
...
ipd_lrput(queue_t *rq, mblk_t *mp)
{
	register struct ipd_addr_tbl	*ap;
	register ulong			*prim;
	register struct T_discon_ind	*tdisind;
	register dl_disconnect_ind_t	*dldisind;
...
ipd_proto(queue_t *wq, mblk_t *mp)
{
	...
	register struct ipdpriminfo	*pip;
	struct ipd_str			*stp;
	int				rc, len;
	u_long				prim;
...
ipd_ioctl(queue_t *wq, mblk_t *mp)
{
	...
	register struct ipd_addr_tbl	*ap = (struct ipd_addr_tbl *)NULL;
	register int			err = EINVAL;
	register mblk_t			*nmp;
	register struct ipd_softc	*ifp;
	...
	case	I_LINK:
		...
		IPD_DEBUG1("ipd_ioctl: I_LINK on q=%x\n", RD(wq));
	...
	case	I_UNLINK:
		...
		IPD_DEBUG1("ipd_ioctl: I_UNLINK on q=%x\n", RD(wq));
...
ipd_dlattachreq(struct ipd_str *stp, queue_t *wq, mblk_t *mp)
{
	...
	IPD_DLPI3("ipd_dlattachreq: stp=%x, ppa=%d state=%d\n", stp,
			ppa, stp->st_state);
...
{
	IPD_DLPI2("ipd_dldetachreq: stp=%x, state=%d\n", stp, stp->st_state);

	ipd_strdetach(stp);

	dlokack(wq, mp, DL_DETACH_REQ);

	IPD_DLPI2("ipd_dldetachreq: done. stp=%x, state=%d\n", stp,
								stp->st_state);
	return (0);
}
...
mblk_t	*mp;
{
	...
	IPD_DLPI3("ipd_dlbindreq: stp=%x, state=%d sap=%d\n",
	...
	IPD_DLPI2("ipd_dlbindreq: done stp=%x, state=%d\n", stp, stp->st_state);
 ...
 */
static int
ipd_dlunbindreq(stp, wq, mp)
struct ipd_str	*stp;
queue_t *wq;
mblk_t	*mp;
{
	IPD_DLPI2("ipd_dlunbindreq: stp=%x, state=%d\n", stp, stp->st_state);
	...
	IPD_DLPI2("ipd_dlunbindreq: done. stp=%x, state=%d\n", stp,
								stp->st_state);
...
mblk_t	*mp;
{
	...
	IPD_DLPI2("ipd_dlinforeq: stp=%x, state=%d\n", stp, stp->st_state);
...
{
	register struct ipd_addr_tbl	*ap, *p;
	register struct ipd_softc	*ifp = stp->st_ifp;
	register int			addr_len, off, len, i;
	mblk_t				*nmp;
	u_long				dst;
	register dl_unitdata_req_t	*dludp;


	IPD_DLPI2("ipd_dlunitdatareq: stp=%x, state=%d\n", stp, stp->st_state);
...
ipd_xmitpkt(queue_t *wq, mblk_t *mp, struct ipd_addr_tbl *ap)
{
	...
	if (canputnext(wq)) {
		 ...
		 */
		IPD_FLDEBUG1("ipd_xmitpkt: wq=%x is in flow control\n", wq);
...
{
	register int			type;
	register struct ipd_str		*stp, *nstp;
	register mblk_t			*nmp;
	...
	for (; nstp = (*acceptfunc)(stp->st_next, ifp, type); stp = nstp) {
		...
		if (canputnext(stp->st_rq)) {
			...
			IPD_DATADBG1("ipd_sendup: sent to q %x\n", stp->st_rq);
			}
		} else {
			IPD_FLDEBUG1("ipd_sendup: dropped data q %x\n",
								stp->st_rq);
			ifp->if_nocanput++;
		}
	}
	...
	if (canputnext(stp->st_rq)) {
		...
		IPD_DATADBG1("ipd_sendup: sent data to q %x\n", stp->st_rq);
	} else {
		IPD_FLDEBUG1("ipd_sendup: dropped data to q %x\n", stp->st_rq);
		ifp->if_nocanput++;
	}
}

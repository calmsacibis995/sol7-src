...
#pragma ident	"@(#)audio_4231.c	1.46	97/11/26 SMI"
...
static void audio_4231_timeout(void *);
...
audio_4231_probe(dev_info_t *dip)
{
	 ...
	 */
	REG_SELECT(MISC_IR);
	if ((ddi_get8(handle, &chip->pioregs.idr) & CODEC_ID_MASK) ==
	    CODEC_SIGNATURE) {
...
audio_4231_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	struct aud_4231_chip *chip;
	ddi_acc_handle_t handle;
	char *CD_line = (char *)NULL;
	char *spkr_mute = (char *)NULL;
#else
#error One of sparc or i386 must be defined.
	...
	if (ddi_prop_op(DDI_DEV_T_ANY, dip, PROP_LEN_AND_VAL_ALLOC,
		...
		if (strncmp(tmp, "SUNW,CS4231h", proplen) == 0) {
			...
			unitp->internal_loopback = B_FALSE;
			unitp->cd_input_line = NO_INTERNAL_CD;
			cmn_err(CE_WARN, "audiocs: unknown audio model: %s, "
				"some parts of audio may not work correctly",
				tmp != (char *)NULL ? tmp : "unknown");
		}
		...
		if (ddi_prop_op(DDI_DEV_T_ANY, dip, PROP_LEN_AND_VAL_ALLOC,
			...
			default:
				...
				unitp->internal_loopback = B_FALSE;
				unitp->cd_input_line = NO_INTERNAL_CD;
				cmn_err(CE_WARN, "audiocs: unknown audio "
			...
			if (ddi_getprop(DDI_DEV_T_ANY, dip, DDI_PROP_DONTPASS,
				...
				if (unitp->dma_engine == EB2_DMA)
					unitp->model = CS_DEV_VERSION_C;
				else
					unitp->model = CS_DEV_VERSION_B;
				unitp->internal_loopback = B_TRUE;
				unitp->cd_input_line = NO_INTERNAL_CD;
			} else {
				unitp->model = CS_DEV_VERSION;
				unitp->internal_loopback = B_FALSE;
				unitp->cd_input_line = INTERNAL_CD_ON_AUX1;
			}
		}
...
#elif defined(i386)
	/* spkr_mute = NULL from definition above, must be before this call */
	...
	if (spkr_mute)	/* we don't need this property string, so free it */
		kmem_free(spkr_mute, len);

	unitp->model = CS_DEV_VERSION_E;
	...
	/* CD_line = NULL from definition above */
	rc = ddi_getlongprop(DDI_DEV_T_NONE, dip, DDI_PROP_DONTPASS,
		"intl_CDROM", (caddr_t)&CD_line, &len);
	if (rc == DDI_PROP_SUCCESS && strncmp(CD_line, "AUX1", len) == 0)
		unitp->cd_input_line = INTERNAL_CD_ON_AUX1;
	else if (rc == DDI_PROP_SUCCESS && strncmp(CD_line, "AUX2", len) == 0)
		unitp->cd_input_line = INTERNAL_CD_ON_AUX2;
	else
		unitp->cd_input_line = NO_INTERNAL_CD;
	if (CD_line)	/* we don't need this property string, so free it */
		kmem_free(CD_line, len);
...
#error One of sparc or i386 must be defined.
	...
	mutex_init(&unitp->lock, NULL, MUTEX_DRIVER,
	    (void *)audio_4231_trap_cookie);
	...
	(void) sprintf(name, "audioc%d", instance);
	unitp->intrstats = kstat_create("audio", instance, name, "controller",
		KSTAT_TYPE_INTR, 1, KSTAT_FLAG_PERSISTENT);
	if (unitp->intrstats) {
		kstat_install(unitp->intrstats);
	}
...
audio_4231_detach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
	...
	if (unitp->intrstats) {
		kstat_delete(unitp->intrstats);
	}
	unitp->intrstats = NULL;
...
audio_4231_open(queue_t *q, dev_t *dp, int oflag, int sflag, cred_t *credp)
{
	...
	if (as == as->control_as) {
		as->type = AUDTYPE_CONTROL;
		/*
		 * If only the control device is opened then the last format
		 * information is used, instead of resetting to the defaults
		 * as described in audio(7I). So set them here.
		 */
		if (!as->input_as->info.open && !as->output_as->info.open) {
...
audio_4231_close(aud_stream_t *as)
{
	...
	if (as == as->output_as) {
		 ...
		 * to the default.
		 */
		as->output_as->info.buffer_size = audio_4231_play_bsize;
	...
	if (!as->output_as->info.open && !as->input_as->info.open) {
		/*
		 * Do this so ioctls on the control steam without an open
		 * data stream behave properly.
		 */
		as->control_as->info.sample_rate = AUD_CS4231_SAMPLERATE;
		as->control_as->info.channels = AUD_CS4231_CHANNELS;
		as->control_as->info.precision = AUD_CS4231_PRECISION;
		as->control_as->info.encoding = AUDIO_ENCODING_ULAW;

		if (!as->control_as->info.open) {
			unitp->hw_output_inited = B_FALSE;
			unitp->hw_input_inited = B_FALSE;
		}
	}
...
audio_4231_ioctl(aud_stream_t *as, queue_t *q, mblk_t *mp)
{
	...
	case AUDIO_GETDEV:	/* return device type */
		...
		(void) strcpy(devtp->name, CS_DEV_NAME);

		(void) strcpy(devtp->version, unitp->model);

		(void) strcpy(devtp->config, CS_DEV_CONFIG_ONBRD1);
...
audio_4231_start(aud_stream_t *as)
{
	...
	if (!unitp->hw_output_inited || !unitp->hw_input_inited) {
		...
		ddi_put8(handle, CS4231_IDR, (uchar_t)DEFAULT_DATA_FMAT);
...
/* a distinction for the initial calculation.  Truly generic? */
		...
		if ((ip->play.buffer_size <= 0) ||
			...
			*error = EINVAL;
			...
			if (as == as->output_as) {
				...
				(void) strqset(as->output_as->writeq, QMAXPSZ,
				    0, ip->play.buffer_size);
	...
	/*
	 * There are 9 possible states that ip->play, ip->record, and as->info
	 * can be in:
	 *
	 *	Case	Play	Record	Info
	 *	1	X	X	X	No change
	 *	2	X	X	Y	Change
	 *	3	-1	-1	DC	No Change (DC == Don't Care)
	 *	4	-1	X	X	No change
	 *	5	-1	X	Y	Change from record
	 *	6	X	-1	X	No Change
	 *	7	X	-1	Y	Change from play
	 *	8	X	Y	DC	Error if play & record channel
	 *	9	Y	X	DC	Error if play & record channel
	 *
	 * We figure out here what the new format should be and let later
	 * code figure out if a change is allowed or not. However, all
	 * errors send an EINVAL error back to the process.
	 */
	if (ip->play.sample_rate == ip->record.sample_rate) {	/* cases 1-3 */
		if (Modify(ip->play.sample_rate)) {	/* cases 1 & 2 */
			sample_rate = ip->play.sample_rate;
		} else {				/* case 3 */
			sample_rate = as->info.sample_rate;
		}
	} else if (!Modify(ip->play.sample_rate)) {		/* cases 4&5 */
		sample_rate = ip->record.sample_rate;
	} else if (!Modify(ip->record.sample_rate)) {		/* cases 6&7 */
		sample_rate = ip->play.sample_rate;
	} else if (ip->play.sample_rate != ip->record.sample_rate) {
		/* play != record,				cases 8 & 9 */
		if (as->input_as->info.open && !as->output_as->info.open) {

			/* okay, only record channel open */
			sample_rate = ip->record.sample_rate;
		} else if (!as->input_as->info.open &&
		    as->output_as->info.open) {

			/* okay, only play channel open */
			sample_rate = ip->play.sample_rate;
		} else {
			/* oops, both channels open, so error */
			*error = EINVAL;
			goto playdone;
		}
	}

	if (ip->play.channels == ip->record.channels) {		/* cases 1-3 */
		if (Modify(ip->play.channels)) {	/* cases 1 & 2 */
			channels = ip->play.channels;
		} else {				/* case 3 */
			channels = as->info.channels;
		}
	} else if (!Modify(ip->play.channels)) {		/* cases 4&5 */
		channels = ip->record.channels;
	} else if (!Modify(ip->record.channels)) {		/* cases 6&7 */
		channels = ip->play.channels;
	} else if (ip->play.channels != ip->record.channels) {
		/* play != record,				cases 8 & 9 */
		if (as->input_as->info.open && !as->output_as->info.open) {
	...
	if (ip->play.precision == ip->record.precision) {	/* cases 1-3 */
		if (Modify(ip->play.precision)) {	/* cases 1 & 2 */
			precision = ip->play.precision;
		} else {				/* case 3 */
			precision = as->info.precision;
		}
	} else if (!Modify(ip->play.precision)) {		/* cases 4&5 */
		precision = ip->record.precision;
	} else if (!Modify(ip->record.precision)) {		/* cases 6&7 */
		precision = ip->play.precision;
	} else if (ip->play.precision != ip->record.precision) {
		/* play != record,				cases 8 & 9 */
		if (as->input_as->info.open && !as->output_as->info.open) {
	...
	if (ip->play.encoding == ip->record.encoding) {		/* cases 1-3 */
		if (Modify(ip->play.encoding)) {	/* cases 1 & 2 */
			encoding = ip->play.encoding;
		} else {				/* case 3 */
			encoding = as->info.encoding;
		}
	} else if (!Modify(ip->play.encoding)) {		/* cases 4&5 */
		encoding = ip->record.encoding;
	} else if (!Modify(ip->record.encoding)) {		/* cases 6&7 */
		encoding = ip->play.encoding;
	} else if (ip->play.encoding != ip->record.encoding) {
		/* play != record,				cases 8 & 9 */
		if (as->input_as->info.open && !as->output_as->info.open) {
	...
	/*
	 * There are several conditions to check to see if there has been a
	 * format change. The first four checks are for the case where nothing
	 * has changed and the AUDIO_INITINFO macro wasn't used. The reset of
	 * the checks are for when the AUDIO_INITINFO macro was used.
	 *
	 * CAUTION: For audiocs, setting either play or record means setting
	 *	    both.
	 */
	format_changed = B_TRUE;	/* assume change until proven not */
	if (((sample_rate == as->info.sample_rate) &&
		(channels == as->info.channels) &&
		(precision == as->info.precision) &&
		(encoding == as->info.encoding)) ||
			(!Modify(ip->play.sample_rate) &&
		...
		/* we know there wasn't a change, so save that info for later */
		format_changed = B_FALSE;

		/* is this is a control stream or is the H/W initialized? */
	...
	/*
	 * We now know something has changed, in which case we don't care about
	 * the hardware becuase it'll be set anyway. Or that something didn't
	 * change and the hardware needs to be initialized. The latter is a
	 * boundary condition for when the driver is first loaded. The boolean
	 * format_changed will tell us which so we don't have to check that
	 * again.
	 *
	 * The only way this could be a control stream is if there was a format
	 * change. The checks above ensure this.
	 */
	if ((as != as->output_as) && (as != as->input_as)) {
		/*
		 * Not an output or input stream, so must be a control stream.
		 * Control steam isn't allowed to do this, regardless of what
		 * the hardware needs.
		 */
		*error = EINVAL;
		goto playdone;
	 ...
	 * is open first. If so then return an error. (We have to make
	 * this check because it may be that the format didn't change and
	 * the hardware needs to be initialized.)
	 */
	if (format_changed == B_TRUE &&
	    as->input_as->info.open && as->output_as->info.open &&
	    (as->input_as->readq != as->output_as->readq)) {

		/* format did change and there are two different channels */
		*error = EBUSY;
		goto playdone;
	}
	 ...
	 * record side for now. We also take care of initializing the
	 * hardware if needs it.
	 */
	switch (sample_rate) {
	case 8000:		/* ULAW and ALAW */
	case NEEDS_HW_INIT:	/* uninitialized, need to start somewhere! */
	...
	if ((encoding == NEEDS_HW_INIT) || (Modify(ip->play.encoding)) ||
		...
		case AUDIO_ENCODING_ULAW:
			if (Modify(channels) && (channels != 1))
				*error = EINVAL;
			REG_SELECT(IAR_MCE | PLAY_DATA_FR);
			tmp_bits = CHANGE_ENCODING(ddi_get8(handle, CS4231_IDR),
			    CS4231_DFR_ULAW);
	...
	ddi_put8(handle, &chip->pioregs.iar, (uchar_t)IAR_MCD);
	drv_usecwait(100);	/* chip bug workaround */
	audio_4231_pollready();
	drv_usecwait(1000);	/* chip bug */
...
/* XXXMERGE - can replace with codec_model? */
	...
	ddi_put8(handle, CS4231_IDR, AUX_INIT_VALUE);
	...
	REG_SELECT(IAR_MCE | PLAY_DATA_FR);
	ddi_put8(handle, CS4231_IDR, DEFAULT_DATA_FMAT);
	...
	ddi_put8(handle, CS4231_IDR, (uchar_t)(OLB_ENABLE | DACZ_ON));

	/* Turn on the hi pass filter */
	REG_SELECT(IAR_MCE | ALT_FEA_EN2R);
	if (CS4231_reva)
		ddi_put8(handle, CS4231_IDR, (HPF_ON | XTALE_ON));
	else
		ddi_put8(handle, CS4231_IDR, HPF_ON);

	/* Clear the playback and capture interrupt flags */
	REG_SELECT(IAR_MCE | ALT_FEAT_STATR);
	ddi_put8(handle, CS4231_IDR, RESET_STATUS);
	...
	ddi_put8(handle, &chip->pioregs.iar, IAR_MCD);
	audio_4231_pollready();
...
audio_4231_inport(cs_unit_t *unitp, uint_t val)
{
	...
	if ((val & AUDIO_INTERNAL_CD_IN) &&
		...
		case INTERNAL_CD_ON_AUX1:
			...
			tmpval = ddi_get8(handle, CS4231_IDR);
			ddi_put8(handle, CS4231_IDR, CDROM_ENABLE(tmpval));
		...
		case INTERNAL_CD_ON_AUX2:
			REG_SELECT(L_INPUT_CR);
			tmpval = ddi_get8(handle, CS4231_IDR);
			ddi_put8(handle, CS4231_IDR, OUTPUTLOOP_ENABLE(tmpval));
	...
	if (val & AUDIO_LINE_IN) {
		...
		REG_SELECT(L_INPUT_CR);
		tmpval = ddi_get8(handle, CS4231_IDR);
		ddi_put8(handle, CS4231_IDR, LINE_ENABLE(tmpval));
		...
		REG_SELECT(L_INPUT_CR);
		tmpval = ddi_get8(handle, CS4231_IDR);
		ddi_put8(handle, CS4231_IDR, MIC_ENABLE(tmpval));
...
audio_4231_monitor_gain(cs_unit_t *unitp, uint_t val)
{
	...
	if (aten >= AUD_CS4231_MON_MAX_ATEN) {
		ddi_put8(handle, CS4231_IDR, LOOPB_OFF);
	} else {
		 ...
		 */
		ddi_put8(handle, CS4231_IDR, ((aten << 2) | LOOPB_ON));
	}
...
audio_4231_play_gain(cs_unit_t *unitp, uint_t val, uchar_t balance)
{
	...
	REG_SELECT(L_OUTPUT_CR);
	old_gain = ddi_get8(handle, CS4231_IDR);
	ddi_put8(handle, CS4231_IDR, GAIN_SET(old_gain, la));
	REG_SELECT(R_OUTPUT_CR);
	old_gain = ddi_get8(handle, CS4231_IDR);
	ddi_put8(handle, CS4231_IDR, GAIN_SET(old_gain, ra));
...
audio_4231_record_gain(cs_unit_t *unitp, uint_t val, uchar_t balance)
{
	...
	REG_SELECT(L_INPUT_CR);
	tmpval = ddi_get8(handle, CS4231_IDR);
	ddi_put8(handle, CS4231_IDR, RECGAIN_SET(tmpval, lg));
	REG_SELECT(R_INPUT_CR);
	tmpval = ddi_get8(handle, CS4231_IDR);
	ddi_put8(handle, CS4231_IDR, RECGAIN_SET(tmpval, rg));
...
/* XXXMERGE */
			if (unitp->dma_engine == EB2_DMA &&
				...
				audio_4231_clear((aud_dma_list_t *)
					&dma_played_list, unitp);
			} else if (unitp->dma_engine != EB2_DMA) {
				audio_4231_clear((aud_dma_list_t *)
					&dma_played_list, unitp);
			}
			AUD_ERRPRINT(AUD_EP_L1, AUD_EM_PLAY, ("4231_playintr: "
...
EB2JUMP:
			 ...
			 * XXX this may also apply to APC dma.
...
EB2JUMP:
			...
			if (jumpflag && unitp->dma_engine == EB2_DMA) {
				return;
			}
...
/* XXXMERGE - can replace with codec_model check */
	...
	handle = CS4231_HANDLE;
	chip = unitp->chip;
	ddi_put8(handle, CS4231_IAR, (uchar_t)IAR_MCD);
	...
	for (i = 0; i < DMA_LIST_SIZE; i++) {
		if (dma_list[i].cmdp) {
			...
			if (dma_list[i].buf_dma_handle != NULL) {
				(void) ddi_dma_unbind_handle(dma_list[i].
				    buf_dma_handle);
...
#error One of sparc or i386 must be defined.
	...
	/*
	 * on x86, the DMA buffers are never freed.
	 */
#if defined(sparc) || defined(__sparc)
	DMA_RC_NCOOKIES = 0;
#endif
...
audio_4231_timeout(void *arg)
{
	...
	ddi_put8(handle, &unitp->chip->pioregs.iar, (u_char)IAR_MCD);

	/*
	 * Wait to see if chip is out of mode change enable
	 */
	while ((ddi_get8(handle, CS4231_IAR)) == IAR_NOTREADY &&
	    x <= CS_TIMEOUT) {
		x++;
	}
	 ...
	 */
	REG_SELECT(TEST_IR);
	if ((ddi_get8(handle, CS4231_IDR) == AUTOCAL_INPROGRESS)) {
		if (++timeout_count < CS_TIMEOUT)
			(void) timeout(audio_4231_timeout, arg, hz);
		else {
			timeout_count = 0;
			cmn_err(CE_WARN,
...
#define	NREGS 32
	...
	for (x = 0; x < NREGS; x++) {
		REG_SELECT(x);
		prom_printf("%d: 0x%x%c", x, ddi_get8(handle, CS4231_IDR),
		    ((x + 1) % 4) ? '\t' : '\n');
	}
...
audio_4231_int_status()
{
	...
	prom_printf("status(832): 0x%x estatus(24) 0x%x\n",
	    ddi_get8(handle, CS4231_STATUS), ddi_get8(handle, CS4231_IDR));

	REG_SELECT(PLAY_DATA_FR)
	prom_printf("PB format(8): 0x%x ", ddi_get8(handle, CS4231_IDR));

	REG_SELECT(PIN_CR);
	prom_printf("int on/off(10): 0x%x\n", ddi_get8(handle, CS4231_IDR));

	REG_SELECT(INTERFACE_CR);
	prom_printf("P/R enabled(9): 0x%x ", ddi_get8(handle, CS4231_IDR));
...
audio_4231_mute_channel(cs_unit_t *unitp, int channel)
{
	 ...
	 */
	REG_SELECT(channel);
	oldvol = ddi_get8(handle, CS4231_IDR);
	ddi_put8(handle, CS4231_IDR, MUTE_ON(oldvol));
}
...
audio_4231_unmute_channel(cs_unit_t *unitp, int channel)
{
	...
	REG_SELECT(channel);
	oldvol = ddi_get8(handle, CS4231_IDR);
	ddi_put8(handle, CS4231_IDR, MUTE_OFF(oldvol));
}
...
audio_4231_change_sample_rate(cs_unit_t *unitp, uchar_t dfr_mask)
{
	...
	REG_SELECT(IAR_MCE | PLAY_DATA_FR);
	tmp_bits = ddi_get8(handle, CS4231_IDR);
	ddi_put8(handle, CS4231_IDR,

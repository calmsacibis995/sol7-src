...
#pragma ident	"@(#)audio_4231_eb2dma.c	1.21	97/11/26 SMI"
...
ddi_dma_attr_t  aud_eb2dma_attr = {
	...
	0x00ffffffULL,			/* 24 bit counter */
	0x01ULL,			/* alignment */
	0x74ULL,			/* 4 and 16 byte transfers */
	0x01,				/* min xfer size */
	0x0000ffffULL,			/* maxxfersz 64K */
	0x0000ffffULL,			/* segment size */
...
audio_4231_eb2_mapregs(dev_info_t *dip, cs_unit_t *unitp)
{
	...
	attr.devacc_attr_endian_flags = DDI_STRUCTURE_LE_ACC;
	if (ddi_regs_map_setup(dip, 3, (caddr_t *)&unitp->audio_auxio, 0,
...
{
	AND_SET_LONG_R(unitp->cnf_handle_eb2play, (uint32_t *)EB2_PLAY_CSR,
		(~EB2_EN_DMA));
	AND_SET_LONG_R(unitp->cnf_handle_eb2play, (uint32_t *)EB2_PLAY_CSR,
		(~EB2_INT_EN));
	/*
	 ...
	 */
	drv_usecwait(1000);
	OR_SET_LONG_R(unitp->cnf_handle_eb2play, (uint32_t *)EB2_PLAY_CSR,
		EB2_RESET);
	audio_4231_eb2cycpend(unitp->cnf_handle_eb2play,
		(uint32_t *)EB2_PLAY_CSR);
	AND_SET_LONG_R(unitp->cnf_handle_eb2play, (uint32_t *)EB2_PLAY_CSR,
...
audio_4231_eb2_play_next(cs_unit_t *unitp, ddi_acc_handle_t handle)
{
	...
	ddi_put32(unitp->cnf_handle_eb2play,
		(uint32_t *)EB2_PLAY_BCR, (uint32_t)cookiep->dmac_size);
	unitp->typ_playlength = (uint32_t)cookiep->dmac_size;
...
audio_4231_eb2_rec_next(cs_unit_t *unitp)
{
	...
	ddi_put32(unitp->cnf_handle_eb2record,
		(uint32_t *)EB2_REC_BCR, (uint32_t)cookiep->dmac_size);
	unitp->typ_reclength = (uint_t)cookiep->dmac_size;
...
audio_4231_eb2recintr()
{
	...
	eb2intr = ddi_get32(unitp->cnf_handle_eb2record,
			(uint32_t *)EB2_REC_CSR);
	OR_SET_LONG_R(unitp->cnf_handle_eb2record, EB2_REC_CSR, EB2_TC);
	/*
	 * Check to see if it is an Err, if not check to see if
	...
	if (eb2intr & EB2_ERR_PEND) {
		...
		unitp->input.samples += audio_4231_sampleconv(&unitp->input,
							unitp->typ_reclength);
		unitp->rec_intr_flag = 0;
		audio_4231_recintr(unitp);
		rc = DDI_INTR_CLAIMED;
...
audio_4231_eb2playintr()
{
	...
	if (eb2intr & EB2_ERR_PEND) {
		...
		cmn_err(CE_WARN,
			...
			eb2intr, ddi_get32(unitp->cnf_handle_eb2play,
			(uint32_t *)EB2_PLAY_ACR), byte_count);

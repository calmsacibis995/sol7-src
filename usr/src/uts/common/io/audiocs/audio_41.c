...
#pragma ident	"@(#)audio_4231_8237dma.c	1.18	97/11/26 SMI"

/*
 * Platform-specific code for the 8237 (system) DMA controller used with the
 * x86 CS4231 audio device.
 */

#include <sys/errno.h>		/* ENXIO */
#include <sys/types.h>
#include <sys/systm.h>		/* XXX - temp hack for spl() */
...
#define	BUFLEN 32768	/* XXX static I/O buffer size for now */
...
/*ARGSUSED*/
ddi_acc_handle_t
audio_4231_8237_get_acchandle(cs_unit_t *unitp, uint32_t *eb2intr, int dirn)
{
	AUD_ERRPRINT(AUD_EP_L0, AUD_EM_GCNT, ("8237_get_count: unitp 0x%x\n",
	    unitp));
...
/* XXX test for now, in case FIFO asks for more data */

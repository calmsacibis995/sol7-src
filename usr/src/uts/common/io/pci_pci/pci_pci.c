...
#pragma ident	"@(#)pci_pci.c	1.35	98/01/21 SMI"
...
_init(void)
{
	 ...
	 */
	e = ddi_soft_state_init(&ppb_state,
				sizeof (ppb_devstate_t), 1);
	if (e != 0)
		return (e);
	...
	ddi_ctl_enum_t ctlop, void *arg, void *result)
{
	...
	case DDI_CTLOPS_REPORTDEV:
		...
		cmn_err(CE_CONT, "?PCI-device: %s@%s, %s #%d\n",
		    ...
		    ddi_major_to_name(ddi_name_to_major(ddi_get_name(rdip))),

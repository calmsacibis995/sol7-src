...
#pragma ident	"@(#)st.c	1.184	98/01/25 SMI"
#include <sys/note.h>
...
static struct modldrv modldrv = {
	...
	"SCSI tape Driver 1.184", /* Name of the module. */
	&st_ops			/* driver ops */
};
...
st_detach(dev_info_t *devi, ddi_detach_cmd_t cmd)
{
	...
	case DDI_DETACH:
		...
		if (un->un_arq_enabled) {
			(void) scsi_ifsetcap(ROUTE, "auto-rqsense", 1, 0);
		}
...
success:
		...
		un->un_laststate = un->un_state;
		un->un_state = ST_STATE_OPEN;
		(void) kmem_free(un->un_tmpbuf, (size_t)bp->b_bcount);
		un->un_tmpbuf = NULL;
		return;
	}

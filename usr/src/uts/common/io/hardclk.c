...
#pragma ident	"@(#)hardclk.c	1.28	97/11/22 SMI"
...
static void (*profile_handler)(void) = dummy_tick;
...
{
	/*
	 * All enable_profiling() does in the non-SPARC case is set
	 * kprof_tick to the profile_handler (kprof_tick is called
	 * from clock() every tick).
	 */
	kprof_tick = profile_handler;
	return (0);
}
...
{
	kprof_tick = profile_handler = dummy_tick;
}
...
/*ARGSUSED*/
int
enable_profiling_interrupt(dev_info_t *devi, u_int (*intr_handler)(void))
{
	profile_handler = (void (*)(void))intr_handler;
	return (DDI_SUCCESS);
}

#pragma ident	"@(#)ppp_chap.c	1.16	97/10/22 SMI"
 ...
 */
void
chap_external_event(pppLink_t *lp, u_int exevent)
{
	chapMachine_t	*machp;
	chapEvent_t	event;
	switch (exevent) {
 ...
 */
static void
chap_chall_timeout(chapMachine_t *machp)
{
	ASSERT(machp);
 ...
 */
static void
chap_resp_timeout(chapMachine_t *machp)
{
	ASSERT(machp);
...
static void md5get_digest(u_char *, u_char, u_char *, int, u_char *, int);
 ...
 */
static int
chap_md5verify(u_int repid, struct chall_resp *resp, chapMachine_t *machp)
{
 ...
 */
static void
md5get_digest(u_char *digest, u_char serid, u_char *secret, int secret_len,
    u_char *chall, int chall_len)
{
	MD5_CTX context;
...
static void
MD5Update(MD5_CTX *context, u_char *input, u_int inputLen)
{
 ...
 */
static void
MD5Transform(MDUINT4 state[4], u_char block[64])
{
 ...
 */
static void
Encode(u_char *output, MDUINT4 *input, u_int len)
{
	unsigned int i, j;
 ...
 */
static void
Decode(MDUINT4 *output, u_char *input, u_int len)
{
	unsigned int i, j;
...
static void
MD5_memcpy(MDPOINTER output, MDPOINTER input, u_int len)
{
	unsigned int i;
 ...
 */
static void
MD5_memset(MDPOINTER output, int value, u_int len)
{
	unsigned int i;

...
#pragma ident	"@(#)sedev.h	1.16	98/01/08 SMI"
...
/*
 * UCM_DEVICE defines the bit in the minor device number that specifies
 * the tty line is to be used for console/controlling a UCM device.
 */
#define	UCM_DEVICE	(1 << (NBITSMINOR32 - 4))
...
/*
 * This bit is also used to control the reset of the UCM device.
 */
#define	SAB_PVR_UCM   0x80
...
typedef
	...
	unsigned long	sc_dtrlow;	/* time dtr went low */
...
typedef
	...
	boolean_t	sec_is_ucm;	/* chip is controlling a UCM device */
	se_ctl_t	*sec_ucm_console;	/* UCM console port */
	se_ctl_t	*sec_ucm_control;	/* UCM control port */
} se_chip_t;
...
/*
 * Driver ioctl command for reseting the UCM device.  This command is only
 * valid for the minor node dedicated to the UCM control line.
 */
#define SE_IOC_UCM_RESET     (int)(_IO('u', 80))

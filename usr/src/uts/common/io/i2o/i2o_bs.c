...
#pragma ident "@(#)i2o_bs.c	1.14	98/01/23 SMI"
...
static	void	cap_translation(u_longlong_t, int *, int *, int *);
...
#ifdef lint
static	void	BsaStatusCheck_reply(void *, ddi_acc_handle_t);
static	int	BsaStatusCheck(bsa_data_t *);
static	void	BsaBlockReassign_reply(void *, ddi_acc_handle_t);
static	int	BsaBlockReassign(bsa_data_t *, uint32_t, u_longlong_t);
#endif
...
static	int	UtilAbort(bsa_data_t *, int, uint8_t, u_longlong_t);
#ifdef lint
static	void	UtilDeviceRelease_reply(void *, ddi_acc_handle_t);
static	int	UtilDeviceRelease(bsa_data_t *);
#endif
 ...
 */
#define	I2O_DEBUG_DIO	0x0001 	/* debug messages for disk specific info */
#define	I2O_DEBUG_DINT	0x0002  /* debug messages for initialization	*/
#define	I2O_DEBUG_DLBL	0x0004	/* debug messages for labeling info	*/
...
static ddi_dma_attr_t dma_attr = {
	...
	0,				/* dma_attr_addr_lo */
	(u_longlong_t)0xFFFFFFFe,	/* dma_attr_addr_hi 64 bit address */
	(u_longlong_t)0xFFFFFFFe,	/* dma_attr_count_max */
	1,				/* dma_attr_align. We do not care */
	1,				/* dma_attr_burstsizes.We do not care */
	1,				/* dma_attr_minxfer */
	(u_longlong_t)0xFFFFFFFF,	/* dma_attr_maxxfer 64 bit address */
	(u_longlong_t)0xFFFFFFFF,	/* dma_attr_seg 64 bit */
...
static ddi_dma_attr_t dma_attr_sglfrm = {
	DMA_ATTR_V0,			/* dma_attr version */
	0,				/* dma_attr_addr_lo */
	(u_longlong_t)0xFFFFFFFe,	/* dma_attr_addr_hi */
	(u_longlong_t)0xFFFFFFFe,	/* dma_attr_count_max. */
	1,				/* dma_attr_align */
	1,				/* dma_attr_burstsizes */
	1,				/* dma_attr_minxfer */
	(u_longlong_t)0xFFFFFFFF,	/* dma_attr_maxxfer */
	(u_longlong_t)0xFFFFFFFF,	/* dma_attr_seg */
	1,				/* dma_attr_sgllen */
	1,				/* dma_attr_granular */
	0,				/* dma_attr_flags */
};
...
static ddi_dma_attr_t dma_attr_lcttbl = {
	...
	0,				/* dma_attr_addr_lo */
	(u_longlong_t)0xFFFFFFFe,	/* dma_attr_addr_hi */
	(u_longlong_t)0xFFFFFFFe,	/* dma_attr_count_max. 24 bit  */
	1,				/* dma_attr_align */
	1,				/* dma_attr_burstsizes */
	1,				/* dma_attr_minxfer */
	(u_longlong_t)0xFFFFFFFF,	/* dma_attr_maxxfer */
...
bsa_strategy(struct buf *bp)
{
	...
	if (i2o_msg_alloc(bsadata->iop, I2O_MSG_SLEEP, NULL, (void **)&msgbuf,
		...
		cmn_err(CE_CONT, "?%d: Frame buffer"
	...
	if (!(bp->b_flags & B_READ) &&
	    ((bp->b_flags & B_ERROR) != B_ERROR) && blkno == 0) {
...
#else
	tcontextp = (bsa_context_t *)ddi_get32(acc_handle,
	    &(((i2o_single_reply_message_frame_t *)msg)->TransactionContext));
#endif
...
bsa_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	u_int	tid;			/* Target id of I2O device	 */
	...
	cv_init(&bsadata->reply_cv, (char *)NULL, CV_DRIVER, (void *)NULL);
	cv_init(&bsadata->state_cv, (char *)NULL, CV_DRIVER, (void *)NULL);
	mutex_init(&bsadata->bsa_mutex, (char *)NULL, MUTEX_DRIVER, NULL);
	mutex_init(&bsadata->lbl.mutex, "label mutex", MUTEX_DRIVER, NULL);
...
#ifdef lint
/*
 * This is a device release routine for BSA
 */

static int
UtilDeviceRelease(bsa_data_t *bsadata)
{
	bsa_context_t		*tcontextp;
	ddi_acc_handle_t acc_handle;
	i2o_msg_handle_t msg_handle;
	i2o_util_device_release_message_t *msgbuf;
	int ret = DDI_SUCCESS;
	...
	setup_msghdr(I2O_UTIL_DEVICE_RELEASE, UtilDeviceRelease_reply,
	    &msgbuf->StdMessageFrame, acc_handle, 0x01,
	    bsadata->tid, 0, sizeof (i2o_util_device_release_message_t));
	...
	if (i2o_msg_send(bsadata->iop, msgbuf, msg_handle) == DDI_FAILURE) {
		cmn_err(CE_CONT, "?UtilDeviceRelease: i2o_msg_send failed");
		return (DDI_FAILURE);
	}
...
/*
 * This is the UtilDeviceRealease reply routine
 */


static void
UtilDeviceRelease_reply(void *msg, ddi_acc_handle_t acc_handle)
{
...
UtilAbort(bsa_data_t *bsadata, int aborttype, uint8_t functoabort,
	u_longlong_t TransactionContextToAbort)
{
	bsa_context_t	*tcontextp;
	ddi_acc_handle_t acc_handle;
	i2o_msg_handle_t msg_handle;
...
UtilEventRegister_reply(void *msg, ddi_acc_handle_t acc_handle)
{
	...
	bsadata = (bsa_context_t *) ddi_get64(acc_handle,
			&(((i2o_single_reply_message_frame_t *)
						msg)->TransactiontContext));
#else
	bsadata = (bsa_data_t *) ddi_get32(acc_handle,
			&(((i2o_single_reply_message_frame_t *)
						msg)->TransactionContext));
#endif
	/*
...
#ifdef lint
static int
BsaBlockReassign(bsa_data_t *bsadata, uint32_t bytecount, u_longlong_t lba)
{
	bsa_context_t		*tcontextp;
	ddi_acc_handle_t acc_handle;
	i2o_msg_handle_t msg_handle;
	i2o_bsa_block_reassign_message_t *msgbuf = NULL;
	i2o_sge_simple_element_t  *segemp = NULL;
	void *buf = NULL; 	/* sgl buffer */
	u_int sgncookies = 0;
	ddi_dma_cookie_t dma_sgcookie;
	size_t	real_length;
	int	sglflags = 0;
	...
	setup_msghdr(I2O_BSA_BLOCK_REASSIGN, BsaBlockReassign_reply,
	    &msgbuf->StdMessageFrame, acc_handle, versionoffset,
	    bsadata->tid, 0, sizeof (i2o_bsa_block_reassign_message_t) +
	    sizeof (i2o_sge_simple_element_t) -
	    sizeof (i2o_sg_element_t));
	...
	if ((ddi_dma_alloc_handle(tcontextp->bsadata->dip, &dma_attr_sglfrm,
		...
		ret = ENOMEM;
		cmn_err(CE_CONT, "?bsa_Reassign: No resources "
		    "available\n");
		goto out;
	}
	...
	if ((ddi_dma_mem_alloc(tcontextp->dma_sghandle,
	    (sizeof (i2o_reassign_addr_message_t)),
	    &accattr, DDI_DMA_STREAMING, DDI_DMA_SLEEP, 0,
	    (caddr_t *)&buf, &real_length, &tcontextp->acc_sghandle))
	    != DDI_SUCCESS) {
	...
	sglflags |= DDI_DMA_WRITE | DDI_DMA_STREAMING;

	ret = ddi_dma_addr_bind_handle(tcontextp->dma_sghandle, NULL,
		(caddr_t) buf, real_length,
		sglflags, DDI_DMA_SLEEP, 0,
		&dma_sgcookie, &sgncookies);
	...
	ddi_put32(tcontextp->acc_sghandle,
	    &((i2o_reassign_addr_message_t *) buf)->ByteCount, bytecount);

	ddi_put64(tcontextp->acc_sghandle,
	    &((i2o_reassign_addr_message_t *) buf)->LogicalByteAddress, lba);

	if (i2o_msg_send(bsadata->iop, msgbuf, msg_handle) == DDI_FAILURE) {
		cmn_err(CE_CONT, "?BsaBlockReassign: i2o_msg_send failed");
		return (DDI_FAILURE);
	}
...
/*
 * Block storage Reassign routine
 */

static void
BsaBlockReassign_reply(void *msg, ddi_acc_handle_t acc_handle)
{
...
/*
 * Block storage progress report status routine
 */


static int
BsaStatusCheck(bsa_data_t *bsadata)
{
	bsa_context_t		*tcontextp;
	ddi_acc_handle_t acc_handle;
	i2o_msg_handle_t msg_handle;
	i2o_bsa_status_check_message_t *msgbuf = NULL;
	int ret = DDI_SUCCESS;
	...
	setup_msghdr(I2O_BSA_STATUS_CHECK, BsaStatusCheck_reply,
	    &msgbuf->StdMessageFrame, acc_handle, 0x01,
	    bsadata->tid, 0, sizeof (i2o_bsa_status_check_message_t));
	...
	if (i2o_msg_send(bsadata->iop, msgbuf, msg_handle) == DDI_FAILURE) {
		cmn_err(CE_CONT, "?BsaStatusCheck: i2o_msg_send failed");
		return (DDI_FAILURE);
	}
...
/*
 * Block Storage Status Check reply routine
 */

static void
BsaStatusCheck_reply(void *msg, ddi_acc_handle_t acc_handle)
{

/*
 * REPLY_STATUS_ABORT_NO_DATA_TRANSFER	StatusCheck request aborted
 * REPLY_STATUS_ERROR_NO_DATA_TRANSFER	Media not available
 * REPLY_STATUS_PROGRESS_REPORT		A request is currently active
 *					agianst the device that supports
 *					PROGRESS replies. (FORMAT or VERFY) *
 * RPELY_STATUS_SUCCESS			Medium is available
 */
...
BsaMediaLock(bsa_data_t *bsadata)
{
	...
	ddi_put64(acc_handle, &msgbuf->TransactionContext, (unit64_t)
								tcontextp);
#else
	ddi_put32(acc_handle, &msgbuf->TransactionContext, (uint32_t)
								tcontextp);
#endif
								...
								int *rval_p)
{
	...
	case DKIOCINFO:
		...
		if (COPYOUT(data, (int) arg, struct dk_cinfo, flag))
			return (EFAULT);
		break;
...
bsa_lbl_ioctl(dev_t dev, int cmd, int arg, int flag)
{
	...
	case DKIOCGAPART:
		...
		if (ddi_copyout((caddr_t)bsadata->lbl.un_map, (caddr_t)arg, i,
								flag)) {
			return (EFAULT);
		}
		break;
...
static void
cap_translation(u_longlong_t capacity, int *cyls, int *hds, int *secs)
{

	*secs = FIXED_SECNUM;
	*hds  = 16;

	if (1 < capacity <= (528 * MB))
		*hds  = 16;
	else if (528 < capacity <= (GB))
		*hds  *= 2;
	else if ((GB) < capacity <= (2.1 * GB))
		*hds  *= 4;
	else if ((2.1 * GB) < capacity <= (4.2 * GB))
		*hds *= 8;
	else if ((4.1 * GB) < capacity <= (8.4 * GB))
		*hds = FIXED_MAXHEAD;
	else { /* (capacity > (8.4 * GB)) */
		cmn_err(CE_CONT, "?cap_translation: The drive is greater"

...
#pragma ident   "@(#)i2o_scsi.c 1.7     98/01/23 SMI"
...
static struct modldrv modldrv = {
	...
	"I2O SCSI HBA OSM version 1.7",	/* module name */
...
retry:
	 ...
	 */
	/*
	 * not sure if we need to reset all targets here
	 * but may not be a bad idea. XXXLWXXX
...
*	}
	...
	DEBUGF(1, (CE_NOTE, "i2ohba_attach%d: Succeeded", instance));

	return (DDI_SUCCESS);

cleanup:
	DEBUGF(1, (CE_NOTE, "?i2ohba_attach%d: Unable to attach", instance));
	...
	if (tid2scsi) {
		...
		DEBUGF(1, (CE_NOTE, "?Utilunclaim %d", i2ohba->i2ohba_tid));
...
i2ohba_i_initcap(struct i2ohba *i2ohba)
{
	...
	for (i = 0; i < N_I2OHBA_TARGETS_WIDE; i++) {
		 ...
		 * Check for Wide data
		 */
		if (map[i]->scsi_info_scalar.Flags & I2O_SCSI_DATA_WIDTH_8) {
...
i2ohba_i_update_this_prop(struct i2ohba *i2ohba, char *property, int value)
{
	...
	if (ddi_prop_update_int(DDI_DEV_T_NONE, dip,
		...
		i2ohba_i_log(NULL, CE_NOTE, "?i2ohba_i_update_this_prop:"
	...
	int flags, int (*callback)(), caddr_t arg)
{
	...
	if (bp && bp->b_bcount != 0) {
		...
		ASSERT(sp->cmd_flags & CFLAG_DMAVALID);
		DEBUGF(1, (CE_NOTE, "init: bcount = %lx, resid = %lx",
	...
	struct buf *bp, int flags, int (*callback)())
{
	...
	DEBUGF(1, (CE_NOTE, "dma_alloc: cmd_ncookies is %d", sp->cmd_ncookies));
...
get_dma_cookies:
		...
		DEBUGF(1, (CE_NOTE, "bcount is %lx, dmacount is %lx,"
			" resid is %lx",
...
i2ohba_scsi_start(struct scsi_address *ap, struct scsi_pkt *pkt)
{
	...
	clock_t			cur_lbolt;
	uint16_t		tid;
	uint16_t		flags = 0, allocmsgsize, msgsize;
	int			i, val, bound = 0;
	size_t			sglen;
	uint_t			count;
	 ...
	 */
	allocmsgsize = ddi_get16(acc_handle, &req->StdMessageFrame.MessageSize);
	...
	msgsize = ((sizeof (i2o_scsi_scb_execute_message_t) -
	    sizeof (i2o_sg_element_t) + sizeof (i2o_sge_chain_element_t) +
	    sizeof (i2o_sge_ignore_element_t)) >> 2);

	ASSERT(msgsize <= allocmsgsize);
	...
	if (sp->cmd_flags & CFLAG_DMAVALID) {
		...
		DEBUGF(1, (CE_NOTE, "cookiecnt is %x, ncookie is %x,"
		    "cookie# is %d", sp->cmd_cookiecnt, sp->cmd_ncookies,
		    sp->cmd_cookie));
		DEBUGF(1, (CE_NOTE, "cmd_xfercount is %lx", sp->cmd_xfercount));
		...
		DEBUGF(1, (CE_NOTE, "start: cmd_dmacount=%lx,"
			    " cmd_xfercount=%lx",
	...
	sp->cmd_deadline = cur_lbolt + (clock_t)(pkt->pkt_time * 128);
...
i2ohba_callback(i2o_message_frame_t *msg, ddi_acc_handle_t acc_handle)
{
	...
	struct scsi_pkt *pkt;
	struct	i2ohba_cmd *sp;
	uint8_t	type, reqstatus;

	reply = (i2o_single_reply_message_frame_t *)msg;

	type = get_msg_Function(&reply->StdMessageFrame, acc_handle);

	ASSERT(type == I2O_SCSI_SCB_EXEC);
	...
	DEBUGF(1, (CE_NOTE, "reqstatus %x", reqstatus));
    ...
    acc_handle, struct i2ohba_cmd *sp)
{
	...
	DEBUGF(1, (CE_NOTE, "transcount is %d \n", transcount));
	...
	if (detailstat == 0)  {
		...
		DEBUGF(1, (CE_NOTE, "devstatus %x", devstatus));

		/* not sure if this is a hardware bug or not */
		if ((devstatus == I2O_SCSI_DSC_BUSY) && (transcount != 0) &&
			(sp->cmd_xfercount == 0xff)) {
			pkt->pkt_reason = CMD_CMPLT;
			pkt->pkt_state |= STATE_GOT_BUS | STATE_GOT_TARGET |
			    STATE_SENT_CMD | STATE_XFERRED_DATA;
			pkt->pkt_statistics = 0;
		...
		DEBUGF(1, (CE_NOTE, "adptrstatus %x", adptrstatus));
		 ...
		 */
done:		autocount = ddi_get32(acc_handle,
	...
	DEBUGF(1, (CE_NOTE, "transfer=%x, resid=%lx", transcount,
	    pkt->pkt_resid));
    ...
    struct i2ohba_cmd *sp, int aqcount)
{
	...
	if (sp->cmd_scblen >= sizeof (struct scsi_arq_status)) {
		...
		DEBUGF(1, (CE_NOTE, "tgt %d.%d: auto request sense",
		    TGT(sp), LUN(sp)));
		...
		DEBUGF(1, (CE_NOTE, "arq: %x %x %x %x %x %x %x",
		...
		/*
		 * bad sense data; can't copy over ARQ data
		 */
		pkt->pkt_reason =  CMD_TRAN_ERR;
	}
}
 ...
 *
 * Description	 : Starts up I/O in normal fashion by calling i2o_i_start_cmd().
	...
	while ((sp->cmd_flags & CFLAG_COMPLETED) == 0) {
		...
		if (--delay_loops <= 0) {
			...
			sp->cmd_flags |= (sp->cmd_flags &
			    ~CFLAG_IN_TRANSPORT) | CFLAG_COMPLETED |
			    CFLAG_FINISHED;
			break;
		}
	}
...
i2ohba_i_watch(void *arg)
{
	...
	if ((cur_lbolt = ddi_get_lbolt()) != 0) {
		for (sp = i2ohba->i2ohba_reqtail; sp != NULL;
			...
			if (deadline <= cur_lbolt) {
				if (sp->cmd_pkt->pkt_time) {
					i2ohba_i_log(NULL, CE_WARN,
					    "?i2ohba_i_watch: "
...
i2ohba_scsi_abort(struct scsi_address *ap, struct scsi_pkt *pkt)
{
	...
	i2ohba_i_log(NULL, CE_NOTE, "?i2ohba_scsi_abort: "
...
i2ohba_scsi_reset(struct scsi_address *ap, int level)
{
	...
	if (level == RESET_TARGET) {
		...
		i2ohba_i_log(NULL, CE_NOTE, "?i2ohba_scsi_reset: reset bus");
    ...
    struct i2ohba_cmd *cmd)
{
	...
	if (action == I2O_HBA_BUS_RESET) {
		rval = i2o_msg_send(i2ohba->i2ohba_iophdl, msgptr, msg_handle);
		goto cleanup;
	}
...
i2ohba_i_reset_interface(struct i2ohba *i2ohba, int action)
{
	 ...
	 */
	i2ohba_i_log(NULL, CE_NOTE, "?Resetting i2o SCSI BUS");
	 ...
	 */
	DEBUGF(1, (CE_NOTE, "Resetting i2o SCSI Host Adapter"));
...
/*
	DEBUGF(1, (CE_NOTE, "Initializing SCSI HBA ID"));
...
*/
	 ...
	 */
	DEBUGF(1, (CE_NOTE, "Initializing i2ohba capabilities"));
...
#ifdef	I2OHBA_DEBUG
	cmn_err(CE_NOTE, "\nmsgptr: %p\n", (void *)msgptr);
	cmn_err(CE_NOTE, "msgptr->VersionOffset: %x\n", msgptr->VersionOffset);
	cmn_err(CE_NOTE, "msgptr->MsgFlags: %x\n", msgptr->MsgFlags);
	cmn_err(CE_NOTE, "msgptr->MessageSize: %x\n", ddi_get16(acc_handle,
	    &msgptr->MessageSize));
	cmn_err(CE_NOTE, "msgptr->TargetAddress: %x\n",
	    get_msg_TargetAddress(msgptr, acc_handle));
	cmn_err(CE_NOTE, "msgptr->InitiatorAddress: %x\n",
	    get_msg_InitiatorAddress(msgptr, acc_handle));
	cmn_err(CE_NOTE, "msgptr->Function: %x\n",
	    get_msg_Function(msgptr, acc_handle));
	cmn_err(CE_NOTE, "msgptr->InitContext: %x\n",
	    ddi_get32(acc_handle,
	    &msgptr->InitiatorContext.initiator_context_32bits));
	cmn_err(CE_NOTE, "msgptr->TransContext: %x\n",
	    ddi_get32(acc_handle, &msgptr->TransactionContext));
	cmn_err(CE_NOTE, "msgptr->OperationFlags: Resevered\n");
	cmn_err(CE_NOTE, "msgptr->FlagsCount1.Flags: %x\n",
	    get_flags_count_Flags(&msgptr->FlagsCount1, acc_handle));
	cmn_err(CE_NOTE, "msgptr->FlagsCount1.Count: %x\n",
	    get_flags_count_Count(&msgptr->FlagsCount1, acc_handle));
	cmn_err(CE_NOTE, "msgptr->OperationCount: %x\n",
	    ddi_get16(acc_handle, &msgptr->OperationCount));
	cmn_err(CE_NOTE, "msgptr->Operation: %x\n",
	    ddi_get16(acc_handle, &msgptr->Operation));
	cmn_err(CE_NOTE, "msgptr->GroupNumber: %x\n",
	    ddi_get16(acc_handle, &msgptr->GroupNumber));
	cmn_err(CE_NOTE, "msgptr->FieldCount: %x\n",
	    ddi_get16(acc_handle, &msgptr->FieldCount));
	cmn_err(CE_NOTE, "msgptr->FlagsCount2.Flags: %x\n",
	    get_flags_count_Flags(&((i2o_getallparam_t *)msgptr)->FlagsCount2,
	    acc_handle));
	cmn_err(CE_NOTE, "msgptr->FlagsCount2.Count: %x\n",
	    get_flags_count_Count(&((i2o_getallparam_t *)msgptr)->FlagsCount2,
	    acc_handle));
	cmn_err(CE_NOTE, "msgptr->PhyAddr: %x\n", ddi_get32(acc_handle,
	    &((i2o_getallparam_t *)msgptr)->PhysicalAddress));
#endif
	/*
	 * initialized a mutex and condvariable to
	...
	struct scsi_reset_notify_entry **listp)
{
	...
	if (list == NULL) {
		i2ohba_i_log(NULL, CE_WARN, "?scsi_hba_reset_notify: "
		    "kmem_alloc failed");
		return;
	}

...
#pragma ident	"@(#)pci_to_i2o.c	1.6	98/01/23 SMI"
...
static int i2o_create_devinfo(iop_nexus_instance_t *iop);
#ifdef I2O_DEBUG
...
i2o_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	 ...
	 */
	if (i2o_create_devinfo(iop) != DDI_SUCCESS) {
		(void) i2o_msg_iop_uninit(&iop->i2o_iop_handle);
		ddi_remove_intr(dip, 0, iop->iblock_cookie);
		goto cleanup;
	}
...
i2o_create_devinfo(iop_nexus_instance_t *iop)
{
	...
	for (i = 0; i < nent; i++) {
		...
		if (ndi_devi_attach_driver(cdip, 0) != DDI_SUCCESS)
			goto fail;
	}
...
dump_exec_params_0001(iop_nexus_instance_t *iop)
{
	...
	mutex_init(&test_mutex, "test mutex lock", MUTEX_DRIVER, NULL);

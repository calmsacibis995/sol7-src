...
#pragma ident	"@(#)i2o_msg.c	1.6	98/01/23 SMI"
...
extern uint_t pcm_get_mem(dev_info_t *, int, uint_t *);
extern uint_t pcm_get_io(dev_info_t *, int, uint_t *);
extern void pcm_return_io(dev_info_t *, int, int);
...
i2o_msg_iop_init(dev_info_t *dip, i2o_msg_trans_t *trans)
{
	...
	mutex_init(&iop->iop_ib_mutex, "IOP ib_queue mutex lock",
			MUTEX_DRIVER, (void *)iop->nexus_trans->iblock_cookie);
	mutex_init(&iop->iop_ob_mutex, "IOP ob_queue mutex lock",
			MUTEX_DRIVER, (void *)iop->nexus_trans->iblock_cookie);
	mutex_init(&iop->osm_registry.osm_mutex, "IOP mutex lock",
			MUTEX_DRIVER, (void *)iop->nexus_trans->iblock_cookie);
	mutex_init(&iop->lct.lct_mutex, "LCT mutex lock",
			MUTEX_DRIVER, (void *)iop->nexus_trans->iblock_cookie);
...
try_again:
	...
	case I2O_IOP_STATE_OPERATIONAL:
		...
		if (i2o_send_exec_iop_reset(iop) != SUCCESS)
			goto cleanup;
	 ...
	 *
	 * Note: We are using the uncommited PCALLOC interfaces
	 * (misc/pcalloc) which may become obselete by the Hot-Plug-Pci
	 * project.
	 *
	...
	if (priv_mem_size > (uint32_t)0) {
	    ...
	    if (pcm_get_mem(dip, priv_mem_size, &iop->hw_config.mem_base) == 0)
			goto cleanup;
	...
	if (priv_io_size > (uint32_t)0) {
	    ...
	    if (pcm_get_io(dip, priv_io_size, &iop->hw_config.io_base) == 0)
			goto cleanup;
...
cleanup:
	...
	if (iop->hw_config.mem_base != NULL) {
		pcm_return_mem(dip, iop->hw_config.mem_base,
					iop->hw_config.mem_size);
	}

	if (iop->hw_config.io_base != NULL) {
		pcm_return_io(dip, iop->hw_config.io_base,
...
i2o_msg_iop_uninit(i2o_iop_handle_t *handlep)
{
	...
	if (niop > 1) { /* we have multiple IOPs */
	    ...
	    mutex_init(&tcxt.cv_mutex, "TCXT mutex", MUTEX_DRIVER, NULL);
	...
	if (iop->hw_config.mem_base != NULL) {
		pcm_return_mem(iop->dip, iop->hw_config.mem_base,
					iop->hw_config.mem_size);
	}

	if (iop->hw_config.io_base != NULL) {
		pcm_return_io(iop->dip, iop->hw_config.io_base,
					iop->hw_config.io_size);
	}
...
#ifdef IRTOS_FIXED
	/*
	 * NOTE: current version of RTOS is not setting the EntryLength
	 * correctly. Enable this code when we have a corrected version.
	 */
	hrt_size = (buf->EntryLength * 4) *
		ddi_get16(acc_hdl, &buf->NumberEntries);

	ASSERT((buf->EntryLength * 4) == sizeof (i2o_hrt_entry_t));
#else
	hrt_size = buf->EntryLength *
		ddi_get16(acc_hdl, &buf->NumberEntries);

	ASSERT(buf->EntryLength == sizeof (i2o_hrt_entry_t));
#endif
...
i2o_create_systab(iop_instance_t *iop)
{
	...
	for (p = iop, i = 0; i < niop; i++) {
		...
		if (p != iop) {
			 ...
			 */
			cv_init(&tcxt.cv, NULL, CV_DEFAULT, NULL);
			mutex_init(&tcxt.cv_mutex, "TCXT mutex",
						MUTEX_DRIVER, NULL);
			/* initialize the transcation context structure */
			tcxt.iop = p;
			tcxt.done_flag = 0;

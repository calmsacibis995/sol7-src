 ...
 * All rights reserved.
 */
#pragma ident	"@(#)llc1.c	1.21	97/10/22 SMI"
/*
...
static int llc1_identify(dev_info_t *dev);
...
static void llc1_force_close(llc1_t *lld);
...
DDI_DEFINE_STREAM_OPS(llc1_ops, llc1_identify, nulldev, \
			llc1_attach, llc1_detach, nodev, \
			llc1_getinfo, D_NEW | D_MP | D_MTPERMOD, &llc1_info);
...
#define	GETBUF(structure, size) \
	((structure *)kmem_zalloc((u_int) (size), KM_NOSLEEP))

static
struct llc1device llc1_device_list;
...
llc1_attach(dev_info_t *devinfo, ddi_attach_cmd_t cmd)
{
	...
	ddi_set_driver_private(devinfo, (caddr_t)&llc1_device_list);
...
/*
 * llc1_identify standard kernel interface routine
 */
static int
llc1_identify(dev_info_t *dev)
{
	if (strcmp(ddi_get_name(dev), "llc1") == 0)
...
llc1_close(queue_t *q, int flag, cred_t *cred)
{
	...
	if (llc1->llc_state == DL_IDLE || llc1->llc_state == DL_UNBOUND) {
		...
		if (llc1->llc_mcast != NULL) {
			...
			kmem_free((caddr_t)llc1->llc_mcast,
				sizeof (llc_mcast_t *)* llc1->llc_multicnt);
			llc1->llc_mcast = NULL;
		}
		llc1->llc_state = DL_UNATTACHED;
		if (llc1->llc_mac_info != NULL) {
			llc1->llc_mac_info->llcp_nstreams--;
			llc1->llc_mac_info = NULL;
		}
...
llc1_lrsrv(queue_t *q)
{
	...
	register llc_mac_info_t *macinfo = (llc_mac_info_t *)q->q_ptr;
	struct iocblk *iocp;
	...
	if (llc1_debug & LLCRECV) {
		...
		if (macinfo == NULL) {
			printf("NULL macinfo");
			cmn_err(CE_PANIC, "null macinfo in lrsrv");
		}
		printf("\n");
	}
#endif
			...
			case DL_INFO_ACK:
				if (macinfo->llcp_flags & LLC1_LINKED) {
					...
					bcopy((caddr_t)mp->b_rptr +
						...
						(caddr_t)macinfo->llcp_macaddr,
						macinfo->llcp_addrlen);
					bcopy((caddr_t)mp->b_rptr +
			prim->info_ack.dl_brdcst_addr_offset,
			(caddr_t)macinfo->llcp_broadcast,
...
{
	mblk_t *mp;
	register llc1_t *lld = (llc1_t *)q->q_ptr;
	union DL_primitives *prim;
	int	err;
	...
	if (lld->llc_mcast) {
		for (i = 0; i < lld->llc_multicnt; i++) {
			if (lld->llc_mcast[i] &&
				...
				if (bcmp((caddr_t)lld->llc_mcast[i]->llcm_addr,
						(caddr_t)addr->ether_addr_octet,
						ETHERADDRL) == 0)
					return (1);
			}
		}
...
llc1_ioctl(queue_t *q, mblk_t *mp)
{
	...
	case I_LINK:
		...
		bzero((caddr_t)tmp->b_rptr, sizeof (llc_mac_info_t));
	...
	case I_UNLINK:
		...
		for (macinfo = llc1_device_list.llc1_mac_next;
			...
			if (macinfo->llcp_lindex == link->l_index) {
			    ...
			    llc1_device_list.llc1_ndevice--;
			    macinfo->llcp_flags = 0;
			    macinfo->llcp_queue = NULL;
				...
				lld !=
				    (llc1_t *)&llc1_device_list.llc1_str_next;
				lld = lld->llc_next)
				if (lld->llc_mac_info == macinfo)
				llc1_force_close(lld);
	...
	case L_SETPPA:
		if (mp->b_cont != NULL) {
			if (llc1_setppa((struct ll_snioc *)mp->b_cont->b_rptr)
			    >= 0) {
				miocack(q, mp, 0, 0);
				return;
			}
	...
	case L_GETPPA:
		if (mp->b_cont == NULL) {
			mp->b_cont = allocb(sizeof (struct ll_snioc),
						BPRI_MED);
			if (mp->b_cont == NULL) {
...
llc1_bind(queue_t *q, mblk_t *mp)
{
...
#ifdef notdef
	state = lld->llc_state; /* save for possible undo */
#endif
	lld->llc_state = DL_BIND_PENDING;
#ifdef notdef
	/*
	 * the restriction on one SAP value at a time is being lifted
	 * for non-CODLS binds (we don't support those)
	 * this check can be added back if the restriction is desired
	 */
	/* search for SAP already in use */
	for (lldp = llc1_device_list.llc1_str_next;
	    lldp != (llc1_t *)&llc1_device_list.llc1_str_next;
	    lldp = lldp->llc_next) {
		if (lld == lldp)
			continue;
		if (lldp->llc_mac_info == lld->llc_mac_info &&
		    lldp->llc_state == DL_IDLE && lldp->llc_sap == sap) {
			/* SAP already in use */
			lld->llc_state = state;
			return (DL_BADSAP);	/* ??? */
		}
	...
	if (sap > 0xFF) {
		ushort snapsap = htons(sap);
		/* this is SNAP, so set things up */
		lld->llc_snap[3] = ((unchar *)&snapsap)[0];
		lld->llc_snap[4] = ((unchar *)&snapsap)[1];
...
llc1_inforeq(queue_t *q, mblk_t *mp)
{
	...
	if (nmp) {
		...
		if (lld->llc_mac_info) {
			...
			bcopy((caddr_t)lld->llc_mac_info->llcp_macaddr,
			...
			bcopy((caddr_t)lld->llc_mac_info->llcp_broadcast,
...
llc1_unitdata(queue_t *q, mblk_t *mp)
{
	...
	if (macinfo->llcp_flags & LLC1_USING_RAW) {
		...
		bcopy((caddr_t)llcp->llca_addr,
			(caddr_t)hdr->ether_dhost.ether_addr_octet,
			ETHERADDRL);
		bcopy((caddr_t)macinfo->llcp_macaddr,
			(caddr_t)hdr->ether_shost.ether_addr_octet,
			ETHERADDRL);
		...
		if (lld->llc_sap != LLC_NOVELL_SAP) {
			...
			if (lld->llc_flags & LLC_SNAP) {
				bcopy((caddr_t)lld->llc_snap,
					(caddr_t)nmp->b_wptr, 5);
	    ...
	    if (ismulticast(llcp->llca_addr)) {
		    if (bcmp((caddr_t)llcp->llca_addr,
				(caddr_t)macinfo->llcp_broadcast,
				ETHERADDRL) == 0)
			    xmt_type = 2;
		    else
...
{
	register struct ether_addr *addr;
	llc1_t *lld;
	mblk_t *nmp, *udmp;
...
{
	return (bcmp((caddr_t)addr->ether_addr_octet,
			(caddr_t)macinfo->llcp_macaddr,
...
llc1unattach(queue_t *q, mblk_t *mp)
{
	...
	if (llc->llc_mcast) {
		...
		kmem_free((caddr_t)llc->llc_mcast,
...
llc1_enable_multi(queue_t *q, mblk_t *mp)
{
	...
	if (MBLKL(mp) >= sizeof (dl_enabmulti_req_t) &&
		...
		if (llc->llc_mcast == NULL) {
			...
			for (i = 0; i < llc1_device_list.llc1_multisize; i++) {
				if (llc->llc_mcast[i] &&
				bcmp((caddr_t)llc->llc_mcast[i]->llcm_addr,
		...
		for (mcast = NULL, i = 0;
			...
			if (macinfo->llcp_mcast[i].llcm_refcnt &&
			    bcmp((caddr_t)macinfo->llcp_mcast[i].llcm_addr,
					(caddr_t)maddr->ether_addr_octet,
					ETHERADDRL) == 0) {
...
llc1_send_disable_multi(llc_mac_info_t *macinfo, llc_mcast_t *mcast)
{
	...
	if (mp) {
		...
		bcopy((caddr_t)mcast->llcm_addr,
			(caddr_t)(mp->b_rptr + sizeof (dl_disabmulti_req_t)),
			ETHERADDRL);
		DB_TYPE(mp) = M_PROTO;
		putnext(WR(macinfo->llcp_queue), mp);
	}
...
llc1_form_udata(llc1_t *lld, llc_mac_info_t *macinfo, mblk_t *mp)
{
	...
	if (macinfo->llcp_flags & LLC1_USING_RAW) {
	    ...
	    bcopy((caddr_t)hdr->ether_shost.ether_addr_octet,
			(caddr_t)LLCADDR(udata,
					udata->dl_src_addr_offset)->llca_addr,
			macinfo->llcp_addrlen);
	    if (lld->llc_flags & LLC_SNAP) {
...
llc1_xid_reply(llc_mac_info_t *macinfo, mblk_t *mp, int sap)
{
	...
	if (DB_TYPE(mp) == M_DATA) {
		...
		bcopy((caddr_t)hdr->ether_shost.ether_addr_octet,
			(caddr_t)msgether->ether_dhost.ether_addr_octet,
			macinfo->llcp_addrlen);
		bcopy((caddr_t)macinfo->llcp_macaddr,
			(caddr_t)msgether->ether_shost.ether_addr_octet,
			macinfo->llcp_addrlen);
		...
		DB_TYPE(rmp) = M_PROTO;
		bzero((caddr_t)rmp->b_rptr, sizeof (dl_unitdata_req_t));
		...
		bcopy((caddr_t)LLCADDR(mp->b_rptr, rud->dl_src_addr_offset),
			(caddr_t)LLCADDR(rmp->b_rptr, ud->dl_dest_addr_offset),
			macinfo->llcp_addrlen);
...
llc1_xid_ind_con(llc1_t *lld, llc_mac_info_t *macinfo, mblk_t *mp)
{
	...
	if (raw) {
		bcopy((caddr_t)hdr->ether_dhost.ether_addr_octet,
			(caddr_t)(nmp->b_rptr + xid->dl_dest_addr_offset),
			xid->dl_dest_addr_length);
	} else {
		dl_unitdata_ind_t *ind;
		ind = (dl_unitdata_ind_t *)mp->b_rptr;
		bcopy((caddr_t)LLCADDR(ind, ind->dl_dest_addr_offset),
			(caddr_t)(nmp->b_rptr + xid->dl_dest_addr_offset),
			xid->dl_dest_addr_length);
	}
	...
	if (raw) {
		bcopy((caddr_t)hdr->ether_shost.ether_addr_octet,
			(caddr_t)(nmp->b_rptr + xid->dl_src_addr_offset),
			xid->dl_src_addr_length);
	} else {
		dl_unitdata_ind_t *ind;
		ind = (dl_unitdata_ind_t *)mp->b_rptr;
		bcopy((caddr_t)LLCADDR(mp->b_rptr, ind->dl_src_addr_offset),
			(caddr_t)(nmp->b_rptr + xid->dl_src_addr_offset),
...
llc1_xid_req_res(queue_t *q, mblk_t *mp, int req_or_res)
{
	...
	if (macinfo->llcp_flags & LLC1_USING_RAW) {
		hdr = (struct ether_header *)nmp->b_rptr;
		bcopy((caddr_t)LLCADDR(xid,
					xid->dl_dest_addr_offset)->llca_addr,
		...
		bcopy((caddr_t)LLCADDR(xid,
					xid->dl_dest_addr_offset)->llca_addr,
			(caddr_t)LLCADDR(ud, ud->dl_dest_addr_offset),
			xid->dl_dest_addr_length);
...
llc1_test_ind_con(llc1_t *lld, llc_mac_info_t *macinfo, mblk_t *mp)
{
	...
	if (raw) {
		bcopy((caddr_t)hdr->ether_dhost.ether_addr_octet,
			(caddr_t)LLCADDR(nmp->b_rptr,
					test->dl_dest_addr_offset)->llca_addr,
			test->dl_dest_addr_length);
	} else {
		dl_unitdata_ind_t *ind;
		ind = (dl_unitdata_ind_t *)mp->b_rptr;
		bcopy((caddr_t)LLCADDR(ind, ind->dl_dest_addr_offset),
			(caddr_t)(nmp->b_rptr + test->dl_dest_addr_offset),
			test->dl_dest_addr_length);
	}
	...
	if (raw) {
		bcopy((caddr_t)hdr->ether_shost.ether_addr_octet,
			(caddr_t)LLCADDR(nmp->b_rptr,
					test->dl_src_addr_offset)->llca_addr,
			test->dl_src_addr_length);
	} else {
		dl_unitdata_ind_t *ind;
		ind = (dl_unitdata_ind_t *)mp->b_rptr;
		bcopy((caddr_t)LLCADDR(mp->b_rptr, ind->dl_src_addr_offset),
			(caddr_t)(nmp->b_rptr + test->dl_src_addr_offset),
			ind->dl_src_addr_length);
	}
...
/*
 * llc1_force_close(lld)
 *	if the lower stream has gone away, kill these off
 */

static void
llc1_force_close(llc1_t *lld)
{
	lld->llc_mac_info = NULL;	/* so we can't get back here */
}
...
{
	llc_mac_info_t *macinfo;
	register kstat_named_t *kstat;
	register struct llc_stats *stats;
...
llc1_init_kstat(llc_mac_info_t *macinfo)
{
	...
	macinfo->llcp_kstatp = kstat_create("llc", macinfo->llcp_ppa,
...
llc1_subs_bind(queue_t *q, mblk_t *mp)
{
	...
	if (lld == NULL || lld->llc_state != DL_IDLE) {
		...
		sapstr = (unchar *)(mp->b_rptr + subs->dl_subs_sap_offset);
		...
		case 2:		/* just the ethertype part */
			...
			((unchar *)&subssap)[0] = sapstr[0];
			((unchar *)&subssap)[1] = sapstr[1];
			subssap = htons(subssap);
			lld->llc_snap[3] = ((unchar *)&subssap)[0];
			lld->llc_snap[4] = ((unchar *)&subssap)[1];
			lld->llc_flags |= LLC_SNAP;
			break;
		...
		case 3:		/* just the OID part */
			...
			bcopy((caddr_t)sapstr, (caddr_t)lld->llc_snap, 3);
		...
		case 5:		/* full SNAP header */
			...
			bcopy((caddr_t)sapstr, (caddr_t)lld->llc_snap, 5);
...
{
	return (bcmp((caddr_t)snap->snap_oid,
			(caddr_t)lld->llc_snap, 5) == 0);
}

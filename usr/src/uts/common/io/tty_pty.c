...
#pragma ident	"@(#)tty_pty.c	1.47	98/01/26 SMI"
...
static int ptc_identify(dev_info_t *);
    ...
    int *rvalp)
{
	...
	case TIOCGPGRP:
		...
		if (pty->pt_vnode == NULL ||
		    ((d_arg = strgetpgrp(pty->pt_vnode)) == -1)) {
			mutex_exit(&pty->ptc_lock);
			return (EIO);
		}
		mutex_exit(&pty->ptc_lock);
	...
	case TIOCSPGRP:
		...
		mutex_enter(&pty->ptc_lock);
		if (pty->pt_vnode == NULL ||
		    (strsetpgrp(pty->pt_vnode, (pid_t)d_arg) == -1)) {
			mutex_exit(&pty->ptc_lock);
			return (EIO);
		}
		mutex_exit(&pty->ptc_lock);
...
makemsg(ssize_t count, struct uio *uiop, struct pty *pty, mblk_t **mpp)
{
	...
	*mpp = NULL;
	 ...
	 */
	if (count >= 0) {
		while ((bp = allocb(count, pri)) == NULL) {
			if (uiop->uio_fmode & (FNDELAY|FNONBLOCK))
				return (EAGAIN);
			if ((id = bufcall(count, pri,
			    (void (*)(void *))cv_broadcast,
			    &ttoproc(curthread)->p_flag_cv)) == 0)
				return (ENOSR);
			if (!cv_wait_sig(&(ttoproc(curthread)->p_flag_cv),
			    &pty->ptc_lock)) {
				mutex_exit(&pty->ptc_lock);
				unbufcall(id);
				mutex_enter(&pty->ptc_lock);

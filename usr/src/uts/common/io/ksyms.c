...
#pragma ident	"@(#)ksyms.c	1.20	97/09/22 SMI"
...
typedef struct ksyms_header {
	...
	Shdr	null_shdr;	/* first shdr is null */
	Shdr	sym_shdr;	/* symtab shdr */
	Shdr	str_shdr;	/* strtab shdr */
	Shdr	shstr_shdr;	/* shstrtab shdr */
...
ksyms_image_create(void)
{
	...
	Sym *locwlk, *gblwlk;	/* current ptr into locals, globals */
...
retry:
	locwlk = locbuf = segkp_get(segkp, roundup(3 * bufsz, PAGESIZE),
					KPD_ZERO | KPD_LOCKED | KPD_NO_ANON);
	if (locbuf == 0) {
		cmn_err(CE_NOTE, "ksyms: can't allocate space to build image");
		return (NULL);
	}
	...
	do {
		...
		for (sp = symtbl; sp < &symtbl[mod->nsyms]; sp++) {
			...
			if (ELF_ST_BIND(sp->st_info) == STB_LOCAL) {
				...
				if ((mod == modules.mod_mp &&
				    sp->st_value >= (uintptr_t)stubs_base &&
				    sp->st_value < (uintptr_t)stubs_end &&
				    ELF_ST_TYPE(sp->st_info) == STT_NOTYPE) ||
	...
	hdr->elf_hdr.e_phoff = (uintptr_t)&((ksyms_header_t *)0)->text_phdr;
	hdr->elf_hdr.e_shoff = (uintptr_t)&((ksyms_header_t *)0)->null_shdr;
	hdr->elf_hdr.e_phnum = 2;
	hdr->elf_hdr.e_shnum = 4;
	hdr->elf_hdr.e_shstrndx = 3;

	hdr->text_phdr.p_type = PT_LOAD;
	hdr->text_phdr.p_vaddr = (Addr)_start;	/* XXX - unix only */
	hdr->text_phdr.p_memsz = (Word)((uintptr_t)e_text - (uintptr_t)_start);
	hdr->text_phdr.p_flags = PF_R | PF_X;

	hdr->data_phdr.p_type = PT_LOAD;
	hdr->data_phdr.p_vaddr = (Addr)t0stack;	/* XXX - unix only */
	hdr->data_phdr.p_memsz = (Word)((uintptr_t)e_data - (uintptr_t)t0stack);
	...
	hdr->sym_shdr.sh_info = locsz / sizeof (Sym);
	hdr->sym_shdr.sh_addralign = 4;
	hdr->sym_shdr.sh_entsize = sizeof (Sym);
	...
	hdr->shstr_shdr.sh_offset = (uintptr_t)
	    &((ksyms_header_t *)0)->shstr_raw;
...
_init(void)
{
	...
	mutex_init(&ksyms_lock, NULL, MUTEX_DEFAULT, NULL);
	mutex_init(&ksyms_image_lock, NULL, MUTEX_DEFAULT, NULL);

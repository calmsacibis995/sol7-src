...
#pragma ident	"@(#)cis_handlers.c	1.41	97/03/31 SMI"
...
u_long config_regs_present_map[] = {
 ...
 */
extern uint_t cistpl_cftable_io_size_table[];
extern uint_t cistpl_cftable_shift_table[];
...
{
	cisdata_t pdesc;
	ulong_t exponent, mantisa, val, digits = 0;
					...
					uint32_t flags, void *arg)
{
	...
	if (flags & HANDTPL_PARSE_LTUPLE) {
		...
		cs->edc_type = ((u_int)GET_BYTE(tp) >> EDC_TYPE_SHIFT) &
								EDC_TYPE_MASK;
		...
		case TPLFMTTYPE_MEM:
			...
			cs->dev.mem.address = (caddr_t)GET_LONG(tp);
			cs->dev.disk.edcloc = GET_LONG(tp);
			break;
		default:
...
int cis_u_malloc_cnt = 0;
int cis_u_free_cnt = 0;
cis_u_malloc_tag_t cis_u_malloc_tag[1000];
...
cis_malloc(size_t len)
{
	...
	addr = (caddr_t)kmem_alloc(len+sizeof (int), KM_SLEEP);

	cis_u_malloc_tag[cis_u_malloc_cnt].addr = addr;
	cis_u_malloc_tag[cis_u_malloc_cnt].len = len + sizeof (int);

	*(int *)addr = len+sizeof (int);
	addr += sizeof (int);
...
cis_free(caddr_t addr)
{
	...
	for (i = 0; i < cis_u_malloc_cnt; i++)
	    if ((cis_u_malloc_tag[i].addr == addr) &&
					(cis_u_malloc_tag[i].len == len))
		break;

	if (i == cis_u_malloc_cnt) {
	    cmn_err(CE_CONT, "cis_free: ?a: 0x%x ?l: %d\n", addr, len);
	}
...
cis_malloc(size_t len)
{
	...
	addr = kmem_zalloc(len+sizeof (int), KM_SLEEP);
	*(int *)addr = len+sizeof (int);
	addr += sizeof (int);
	return (addr);
}

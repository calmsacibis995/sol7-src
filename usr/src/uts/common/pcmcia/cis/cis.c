...
#pragma ident	"@(#)cis.c	1.30	97/03/31 SMI"
...
static void cisp_init(void);
static void cis_deinit(void);
...
int cis_module_can_be_unloaded = 1;
...
CISParser(int function, ...)
{
	...
	switch (function) {
		...
		cs_socket_t *sp;
		    ...
		    retcode = (void *)cis_list_create(cistpl_callout, sp);
		}
		break;
		...
		cs_socket_t *sp;
		    ...
		    retcode = (void *)cis_list_destroy(sp);
		}
		break;
...
cistpl_t *
cis_get_ltuple(cistpl_t *tp, cisdata_t type, uint_t flags)
{
	cistpl_t *ltp = NULL;
 ...
 */
static uint_t cistpl_device_size[8] =
							...
							cisparse_t *cisparse)
{
	...
	do {
	    ...
	    if ((ret & HANDTPL_ERROR) || (ret == (uint_t)BAD_CIS_ADDR)) {
		if (tps != NULL)
		    cis_info->cis = tps;
		if (ret == (uint_t)BAD_CIS_ADDR)

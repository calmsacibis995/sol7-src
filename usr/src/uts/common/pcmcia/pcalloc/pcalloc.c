...
#pragma ident	"@(#)pcalloc.c	1.18	97/11/22 SMI"
...
struct ra_map_list *ra_map_list;
/*
struct ramap *ra_mem;
struct ramap *ra_io;
*/
...
/*
 * ra_find_mapset(dip)
 *	find the maps associated with this dip.
 *	basically, look up the tree until we find a set
 *	and use root as last resort (default for x86)
 */
struct ra_map_list *
ra_find_mapset(dev_info_t *dip)
{
	struct ra_map_list *map;

	while (dip != NULL) {
		for (map = ra_map_list; map != NULL; map = map->ra_next) {
			if (dip == map->ra_dip)
				return (map);
		}
...
pcm_get_mem(dev_info_t *dip, int len, uint_t *base)
{
	...
	req.ra_len = len;
	(void) ra_alloc(&map->ra_mem, &req, &ret);
	*base = ret.ra_addr_lo;
...
#if defined(PCALLOC_DEBUG)
	if (pcalloc_debug) {
		cmn_err(CE_CONT, "pcm_get_io(%x, %p)\n", len, (void *)base);
		if (base != 0)
			cmn_err(CE_CONT, "\t*base=%x\n", *base);
	}
#endif
	(void) ra_alloc(&map->ra_io, &req, &ret);
	*base = ret.ra_addr_lo;
#if defined(PCALLOC_DEBUG)
	if (pcalloc_debug)
...
pcm_return_io(dev_info_t *dip, int base, int len)
{
	...
	map = ra_find_mapset(dip);
	if (map != NULL) {
		ra_free(&map->ra_io, base, len);
	}
}

/*
 * pcm_get_intr()
 *	currently not for SPARC and probably needs to be rethought
 */
...
/* allocate an map for this parent nexus */

int
ra_init_null(dev_info_t *dip)
{
	struct ra_map_list *map;
	map = kmem_zalloc(sizeof (struct ra_map_list), KM_SLEEP);
	if (map == NULL)
		return (DDI_FAILURE);
	map->ra_dip = dip;
	mutex_enter(&ra_lock);
	map->ra_next = ra_map_list;
	ra_map_list = map;
	mutex_exit(&ra_lock);
	return (DDI_SUCCESS);
}
 ...
 *	initialize to a null set of resources. On
 *	SPARC, resources come from parent PCI node
 */
void
ra_resource_init()
{
...
#endif
	if (ra_init_null(ddi_get_parent(used)) == DDI_FAILURE) {
		return;
	}
	map = ra_find_mapset(used);
	if (map == NULL)
		return;
	map->ra_mem = ra_alloc_map();
	map->ra_io  = ra_alloc_map();
	ASSERT(map->ra_mem && map->ra_io);
	map->ra_mem->ra_base = 0;
	map->ra_mem->ra_len  = (uint_t)~0;
	map->ra_io->ra_base = 0;
	map->ra_io->ra_len = 0xFFFF;
...
#ifdef PCALLOC_DEBUG
	if (pcalloc_debug) {
		(void) ra_dump_all(map->ra_io, "I/O");
		(void) ra_dump_all(map->ra_mem, "memory");
	}
#endif
...
/*
 * ra_release()
 *	release all resources back to system.
 */
...
ra_release()
{
	...
	mutex_enter(&ra_lock);
	for (map = ra_map_list; ra_map_list != NULL && map != NULL;
		map = ra_map_list) {
		ra_map_list = ra_map_list->ra_next;
		while (map->ra_mem != NULL) {
			next = map->ra_mem->ra_next;
			kmem_free((caddr_t)map->ra_mem, sizeof (struct ramap));
			map->ra_mem = next;
		}
		while (map->ra_io != NULL) {
			next = map->ra_io->ra_next;
			kmem_free((caddr_t)map->ra_io, sizeof (struct ramap));
			map->ra_io = next;
		}
		kmem_free((caddr_t)map, sizeof (struct ra_map_list));
	}

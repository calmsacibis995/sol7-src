 ...
 * All rights reserved.
 */
#pragma ident	"@(#)pem.c	1.43	97/10/22 SMI"
...
u_long pem_minors;		/* minors are bit mask */
...
static	pem_devinfo(dev_info_t *, ddi_info_cmd_t, void *, void **);
static	pem_identify(dev_info_t *);
static pem_event_handler(int, int, int, void *);
static  u_int pem_soft_intr(caddr_t);
...
DDI_DEFINE_STREAM_OPS(pem_ops, pem_identify, nulldev, pem_attach, pem_detach,
...
pem_identify(dev_info_t *dip)
{
	if (strcmp(ddi_get_name(dip), "pem") == 0)
	...
	cred_t *cred;
{
	...
	if (ddi_soft_state_zalloc(pem_instances, minordev - 1) !=
	    DDI_SUCCESS) {
	...
	mblk_t *mp;		/* message pointer */
{
	...
	if (pem_debug & PEMTRACE)
		cmn_err(CE_CONT, "pem_wput(%x %x): type %x\n",
	...
	mblk_t *mp;		/* message pointer */
{
	...
	if (pem_debug & PEMTRACE)
		cmn_err(CE_CONT,
			"pem_cmds(%x, %x):pem=(N/A), prim->em_primitive=%d\n",
			(int)q, (int)mp, (int)prim->em_primitive);
#endif
...
#ifdef PEM_DEBUG
	if (pem_debug & PEMTRACE)
		cmn_err(CE_CONT, "pem_inforeq(%x %x)\n", (int)q, (int)mp);
#endif
	pem = (pem_t *)q->q_ptr;
	ASSERT(pem);
...
pem_init_req(queue_t *q, mblk_t *mp, em_init_req_t *initreq)
{
	...
	if (initreq->em_event_mask_offset != 0)
		event = ((u_char *)initreq) + initreq->em_event_mask_offset;
	else
		event = NULL;
	if (initreq->em_event_class_offset != 0)
		class = ((u_char *)initreq) + initreq->em_event_class_offset;
	else
		class = NULL;
...
static void
pem_event_ind(queue_t *q, long event, long socket, void *arg)
{
	mblk_t *mp;
	int len;
	em_event_ind_t *ind;
...
#if	defined(PEM_DEBUG)
	if (pem_debug)
		cmn_err(CE_CONT, "pem_event_dispatch(%x, %x, %x, %x)\n",
			id, event, socket, (int)arg);
#endif
...
#if	defined(PEM_DEBUG)
	if (pem_debug)
		cmn_err(CE_CONT, "pem_event_handler(%x, %x, %x, %x)\n",
			id, event, socket, (int)arg);
#endif
	for (i = 0, pe = pem_events; i < PEM_MAX_EVENTS; i++, pe++) {
		if (pem_claim(pe)) {
...
pem_ident_socket(queue_t *q, mblk_t *mp)
{
	...
	if (num_minors > 0)
		kmem_free((caddr_t)minors,
				num_minors * sizeof (struct pcm_make_dev));
	return (PEME_OK);
}

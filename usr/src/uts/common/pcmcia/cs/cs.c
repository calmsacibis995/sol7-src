...
#pragma ident	"@(#)cs.c	1.101	97/10/22 SMI"
 ...
 */
static uint_t cs_add_socket(uint_t);
static uint_t cs_drop_socket(uint_t);
static cs_socket_t *cs_get_sp(uint_t);
static cs_socket_t *cs_find_sp(uint_t);
static cs_window_t *cs_get_wp(uint_t);
static cs_window_t *cs_find_wp(uint_t);
static int cs_add_windows(int, uint_t);
static uint_t cs_ss_init();
...
static int cs_get_firstnext_tuple(client_handle_t, tuple_t *, uint_t);
...
static int cs_get_firstnext_client(get_firstnext_client_t *, uint_t);
...
static int cs_find_mem_window(uint_t, win_req_t *, uint_t *);
static int cs_memwin_space_and_map_ok(inquire_window_t *, win_req_t *);
static int cs_valid_window_speed(inquire_window_t *, uint_t);
static window_handle_t cs_create_window_handle(uint_t);
static cs_window_t *cs_find_window(window_handle_t);
static int cs_find_io_win(uint_t, iowin_char_t *, uint_t *, uint_t *);
...
static int cs_allocate_io_win(uint_t, uint_t, uint_t *);
static int cs_setup_io_win(uint_t, uint_t, baseaddru_t *,
...
static uint_t cs_get_socket(client_handle_t, uint_t *, uint_t *,
...
static int cs_convert_powerlevel(uint_t, uint_t, uint_t, unsigned *);
...
static int cs_init_cis_window(cs_socket_t *, uint_t *, acc_handle_t *, uint_t);
				...
				cisparse_t *cisparse, cisdata_t cisdata)
{
	...
	if ((sp->cis_flags & CW_VALID_CIS) &&
	    ...
	    ret = (int)CIS_PARSER(CISP_CIS_PARSE_TUPLE,
 ...
 */
int
cs_init_cis_window(cs_socket_t *sp, uint_t *offset,
					acc_handle_t *hp, uint_t flags)
{
	set_window_t sw;
	get_window_t gw;
	inquire_window_t iw;
						...
						get_ss_status_t *gs, int flags)
{
	...
	if ((sp->flags & SOCKET_CARD_INSERTED) ||
		...
		if (client->present & CONFIG_PINREPL_REG_PRESENT) {
		    acc_handle_t cis_handle;
		    uint_t newoffset = client->config_regs_offset;
...
cs_create_window_handle(uint_t aw)
{
...
/* BEGIN CSTYLED */
static int
cs_find_mem_window(uint_t sn, win_req_t *rw, uint_t *assigned_window)
{
	uint_t wn;
	int error = CS_OUT_OF_RESOURCE;
	uint_t window_num = PCMCIA_MAX_WINDOWS;
	/* LINTED C used to be so much fun before all the rules... */
	uint_t min_size = (uint_t)~0;
	inquire_window_t inquire_window, *iw;
	uint_t MinSize, MaxSize, ReqGran, MemWndCaps, WndCaps;
	uint_t tws;
 ...
 */
static int
cs_valid_window_speed(inquire_window_t *iw, uint_t AccessSpeed)
{
 ...
 */
static int
cs_find_io_win(uint_t sn, iowin_char_t *iwc, uint_t *assigned_window,
								uint_t *size)
{
	inquire_window_t inquire_window, *iw;
	unsigned wn;
 ...
 */
static int
cs_allocate_io_win(uint_t sn, uint_t Attributes, uint_t *assigned_window)
{
	iowin_char_t iowin_char;
	cs_window_t *cw;
 ...
 */
static int
cs_setup_io_win(uint_t sn, uint_t wn, baseaddru_t *Base, uint_t *NumPorts,
					uint_t IOAddrLines, uint_t Attributes)
{
	set_window_t set_window;
...
cs_request_configuration(client_handle_t client_handle, config_req_t *cr)
{
	...
	if (client->flags & REQ_IO_DONE) {
	    if (client->present & CONFIG_IOBASE0_REG_PRESENT) {
		uint_t base = client->io_alloc.BasePort1.base;
		uint_t present = (client->present &
					CONFIG_IOBASE_REG_MASK) >>
						CONFIG_IOBASE_REG_SHIFT;
		uint_t reg = crt->iobase0_p;
	    ...
	    if (client->present & CONFIG_IOLIMIT_REG_PRESENT) {
		uint_t np = client->io_alloc.NumPorts1 +
					client->io_alloc.NumPorts2;
		uint_t limit, do_bit = 0;
		int lm;
 ...
 */
static int
cs_convert_powerlevel(uint_t sn, uint_t volts, uint_t flags, unsigned *pl)
{
...
cs_add_socket(uint_t sn)
{
	...
	/*
	 * Create the per-socket event handler thread; if it gets created,
	 *	then set the flag that tells everyone else that the socket
	 *	is ready.
	 */
	if (!(sp->event_thread = CREATE_SOCKET_EVENT_THREAD(cs_event_thread,
		sn))) {
	    cmn_err(CE_CONT, "cs_add_socket: socket %d can't create event "
						"thread\n", sp->socket_num);
	    mutex_enter(&sp->lock);
	    sp->flags &= ~SOCKET_IS_VALID;
	    mutex_exit(&sp->lock);
	    return (CS_BAD_SOCKET);
	} /* CREATE_SOCKET_EVENT_THREAD */
	 ...
	 */
	if (!(sp->ss_thread = CREATE_SOCKET_EVENT_THREAD(cs_ss_thread, sn))) {
	    cmn_err(CE_CONT, "cs_add_socket: socket %d can't create Socket "
						"Services work thread\n",
							sp->socket_num);
	    mutex_enter(&sp->lock);
	    sp->flags &= ~SOCKET_IS_VALID;
	    mutex_exit(&sp->lock);
 ...
 *						socket number from
 *		uint_t *socket -  pointer to socket number to use if
 ...
 *					return CS_SUCCESS
 *		uint_t *function - pointer to return function number into
 *					if not NULL
 ...
 */
static uint_t
cs_get_socket(client_handle_t client_handle, uint_t *socket, uint_t *function,
 ...
 */
static int
cs_add_windows(int aw, uint_t bn)
{
...
cs_create_cis(cs_socket_t *sp)
{
	...
	ret = (uint_t)CIS_PARSER(CISP_CIS_LIST_CREATE,
 ...
 */
static int
cs_get_firstnext_client(get_firstnext_client_t *fnc, uint_t flags)
{
	cs_socket_t *sp;
	client_t *client;
	uint_t sn = 0;

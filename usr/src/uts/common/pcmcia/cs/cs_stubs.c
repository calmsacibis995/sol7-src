...
#pragma ident	"@(#)cs_stubs.c	1.19	97/03/31 SMI"
...
csx_register_cardservices(cs_register_cardservices_t *rcs)
{
	...
	if (cs_stubs_debug > 2)
	    cmn_err(CE_CONT, "csx_register_cardservices: "
					"magic: 0x%x function: 0x%x "
					"cardservices: 0x%x\n",
					rcs->magic, rcs->function,
					rcs->cardservices);
#endif
...
#ifdef	CS_STUBS_DEBUG
	if (cs_stubs_debug > 3)
	    cmn_err(CE_CONT, "csx_GetMappedAddr: handle: 0x%x\n", (int)handle);
#endif
...
#ifdef	CS_STUBS_DEBUG
	if (cs_stubs_debug > 3)
	    cmn_err(CE_CONT, "csx_GetPhysAddr: handle: 0x%x\n", (int)handle);
#endif
...
#ifdef	CS_STUBS_DEBUG
	if (cs_stubs_debug > 3)
	    cmn_err(CE_CONT, "csx_DupHandle: handle: 0x%x\n", (int)handle);
#endif
...
#ifdef	CS_STUBS_DEBUG
	if (cs_stubs_debug > 3)
	    cmn_err(CE_CONT, "csx_FreeHandle: handle: 0x%x\n", (int)handle);
#endif
	return (CS_BAD_HANDLE);

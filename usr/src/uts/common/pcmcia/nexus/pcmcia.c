...
#pragma ident	"@(#)pcmcia.c	1.89	98/01/24 SMI"
...
/*
 * all the modules we depend on
 */
char _depends_on[] = "misc/cs misc/cis misc/pcalloc";
...
int pcmcia_add_intrspec(dev_info_t *dip,
			    ...
			    u_int (*int_handler)(caddr_t intr_handler_arg),
...
int pcmcia_attaching = 1;
int pcmcia_timer_id;
/*
 * resource allocation prototypes
 */
...
u_int pcmcia_mfc_intr(caddr_t);
u_int nullintr(caddr_t);
	...
	ddi_ctl_enum_t ctlop, void *arg, void *result)
{
	...
	if (pcmcia_debug) {
		cmn_err(CE_CONT, "pcmciactlops(%x, %x, %x, %x, %x)\n",
	...
	case DDI_CTLOPS_INITCHILD:
...
#if defined(PCMCIA_DEBUG)
		if (pcmcia_debug) {
			cmn_err(CE_CONT, "pcmcia: init child: %s(%d) @%x\n",
		 ...
		 */
		if (ppd != NULL && ppd->ppd_function > 0) {
			int sock;
			int func;
			sock = ppd->ppd_socket;
	...
	case DDI_CTLOPS_REGSIZE:
		...
		if (ppd != NULL && ppd->ppd_nreg > 0)
			*((u_int *) result) =  sizeof (struct pcm_regs);
		else
			*((u_int *) result) = 0;
		return (DDI_SUCCESS);
...
struct pcmcia_props {
	...
	{ PCM_DEV_SOCKET, sizeof (int), PCMCIA_PROP_SOCKET },
	{ "compatible", sizeof (int), PCMCIA_PROP_COMPAT },
    ...
    char *name, caddr_t valuep, int *lengthp)
{
	...
	default:
		...
		case PCMCIA_PROP_SOCKET:
			sock = CS_MAKE_SOCKET_NUMBER(ppd->ppd_socket,
							ppd->ppd_function);
			propptr = (caddr_t)&sock;
			break;
		case PCMCIA_PROP_COMPAT:
			propptr = DEVI(dip)->devi_compat_names;
			proplen = DEVI(dip)->devi_compat_length;
			break;
			    ...
			    caddr_t intr_handler_arg, int kind)
{
	...
	handler.handler_id = (uint_t)rdip;
	...
	off_t offset, off_t len, caddr_t *vaddrp)
{
	...
	mr.map_obj.rp = pcmcia_cons_regspec(dip, check == PCA_RES_NEED_IO ?
							RA_MAP_IO : RA_MAP_MEM,
							regbuf, &ret);
	switch (mp->map_op) {
	case DDI_MO_UNMAP:
...
pcmcia_cons_regspec(dev_info_t *dip, int type, uchar_t *buff, ra_return_t *ret)
{
	...
	case PTYPE_PCI:
		...
		pcireg->pci_phys_hi = (type == RA_MAP_IO ? PCI_ADDR_IO :
	...
	default:
		...
		defreg->regspec_bustype = type == RA_MAP_IO ? 1 : 0;
		...
		for (sock = 0; sock < conf.NumSockets;
			...
			pcmcia_sockets[n] =
				kmem_zalloc(sizeof (pcmcia_logical_socket_t),
						KM_SLEEP);
			if (pcmcia_sockets[n] == NULL)
				cmn_err(CE_PANIC,
					"pcmcia_init_adapter out of memory");
...
pcmcia_number_socket(dev_info_t *dip, int localsocket)
{
	...
	child = ddi_add_child(dip, "pcs", DEVI_SID_NODEID, -1);
	if (child != NULL) {
		(void) i_ddi_bind_node_to_driver(child);
		ddi_unorphan_devs(ddi_name_to_major("pcs"));
		ppd = (struct pcmcia_parent_private *)
		...
		(void) modload("drv", "pcs");
		if (!DDI_CF2(child)) {
			int ret;
			ret = impl_proto_to_cf2(child);
			if (ret == DDI_SUCCESS)
				mod_rele_dev_by_devi(child);
		}
	}
	return (child);
}
...
#if defined(PCMCIA_DEBUG)
	if (pcmcia_debug) {
		cmn_err(CE_CONT, "pcm_adapter_callback: %x %x %x %x: ",
...
#if defined(PCMCIA_DEBUG)
	if (pcmcia_debug)
		cmn_err(CE_CONT, "find: initial dip = %x, socket=%d, name=%s "
			"(instance=%d, socket=%d, name=%s)\n",
			(int)dip, socket, info->pd_bind_name,
...
pcmcia_add_compatible(dev_info_t *dip, struct pcm_device_info *info)
{
	...
	if (DEVI(dip)->devi_compat_names) {
		...
		int l;
		/*
		 * an existing compat property that will
		 * "possibly" change so always change.
		 */
		l = DEVI(dip)->devi_compat_length;
		h = DEVI(dip)->devi_compat_names;
		DEVI(dip)->devi_compat_length = 0;
	...
	if (info->pd_flags & PCM_NAME_VERS1) {
		(void) sprintf(cp, "%s,%s",
			PCMDEV_NAMEPREF, info->pd_vers1_name);
		pcmcia_fix_string(cp); /* don't want spaces */
		length = strlen(cp) + 1;
	...
	if ((info->pd_flags & (PCM_NAME_1275 | PCM_MULTI_FUNCTION)) ==
	    (PCM_NAME_1275 | PCM_MULTI_FUNCTION) &&
	    (length + strlen(info->pd_bind_name) + 3) < MAXPATHLEN) {
		(void) sprintf(cp, "%s.%x",
			info->pd_bind_name, info->pd_function);
		length += i = strlen(cp) + 1;
		cp += i;
	}
	if (info->pd_flags & PCM_NAME_1275 &&
	    (length + strlen(info->pd_bind_name)) < MAXPATHLEN) {
		(void) strcpy(cp, info->pd_bind_name);
		length += i = strlen(info->pd_bind_name) + 1;
		cp += i;
	}
	if (info->pd_flags & PCM_NAME_GENERIC &&
	    (length + strlen(info->pd_generic_name) +
		strlen(PCMDEV_NAMEPREF)) < MAXPATHLEN) {
		if (strncmp(info->pd_generic_name, "class,", 6) == 0) {
			(void) sprintf(cp, "%s%s", PCMDEV_NAMEPREF,
				info->pd_generic_name);
			length += i = strlen(cp) + 1;
			cp += i;
			...
			if ((length + strlen(info->pd_generic_name)) <
			    MAXPATHLEN) {
				(void) strcpy(cp, info->pd_generic_name);
				length += i = strlen(cp) + 1;
				cp += i;
			}
		}
	...
	if (length > 0) {
		DEVI(dip)->devi_compat_names = kmem_alloc(length, KM_SLEEP);
		if (DEVI(dip)->devi_compat_names) {
#if defined(PCMCIA_DEBUG)
			int xxx;
#endif
			bcopy((caddr_t)buff,
				(caddr_t)DEVI(dip)->devi_compat_names,
				length);
			DEVI(dip)->devi_compat_length = length;
#if defined(PCMCIA_DEBUG)
			if (ddi_getproplen(DDI_DEV_T_NONE, dip, 0,
					    "compat", &xxx) != DDI_SUCCESS)
...
pcmcia_init_devinfo(dev_info_t *pdip, struct pcm_device_info *info)
{
	...
	if (dip != NULL &&
		...
		if (pcmcia_debug)
			cmn_err(CE_CONT, "pcmcia: create child [%s](%d): %s\n",
				info->pd_bind_name,
				info->pd_socket, info->pd_generic_name);
#endif
		...
		/*
		 * is there already an instance of this
		 * device that might be the same one?
		 */
		if ((major = ddi_name_to_major(name)) == -1) {
			major = ddi_name_to_major(info->pd_bind_name);
		}
		 ...
		 */
		/*
		 * special case of no instance in this socket
		 */

		dip = ddi_add_child(pdip, name, DEVI_SID_NODEID, -1);

		if (dip != NULL) {
			/*
			 * construct and add a "compatible" property
			 * so that driver binding works properly.
			 */
			pcmcia_add_compatible(dip, info);
			/*
			 * note that ddi_add_child assumes that the compatible
			 * property was present before the node was created.
			 * in our case, even though we know the value it can
			 * be accessed due to add_child using PROM property
			 * functions and we don't have a PROM to do it.
			 *
			 * Note: the i_ddi_bind_node_to_driver()
			 *	works around the fact that the "compatible"
			 *	property is processed in ddi_add_child()
			 *	and assumes a hardware property in PROM
			 *	The call to ddi_unorphan_devs() continues
			 *	the processing since the binding didn't
			 *	occur to the driver in ddi_add_child()
			 *	and got put on the orphan list.
			 */
			(void) i_ddi_bind_node_to_driver(dip);
			major = ddi_name_to_major(ddi_binding_name(dip));
			ddi_unorphan_devs(major);
		}

		/* need parent private data area */
		ppd = (struct pcmcia_parent_private *)
			kmem_zalloc(sizeof (struct pcmcia_parent_private),
					KM_SLEEP);
		...
		pcmcia_sockets[info->pd_socket]->ls_dip[info->pd_function] =
			dip;
		/* default to parental suspend/resume */
		(void) ddi_prop_create(DDI_DEV_T_NONE, dip, 0,
					"pm-hardware-state",
					pcmcia_default_pm_mode,
					strlen(pcmcia_default_pm_mode) + 1);
	}
	...
	if (pcmcia_debug)
		cmn_err(CE_CONT,
			"\tjust added \"active\" to %s in %d\n",
			ddi_get_name(dip),
			ddi_getprop(DDI_DEV_T_NONE, dip,
				    DDI_PROP_DONTPASS,
				    PCM_DEV_SOCKET, -1));
#endif
	pcmcia_sockets[info->pd_socket]->ls_dip[info->pd_function] = dip;

	/*
	 * inform the event manager that a child was added
	 * to the device tree.
	 */
	pcm_event_manager(PCE_DEV_IDENT,
				CS_MAKE_SOCKET_NUMBER(info->pd_socket,
							info->pd_function),
							ddi_get_name(dip));
	 ...
	 * The framework really ought to do this for us.
	 */

	if (pcmcia_check_for_cf2(dip) &&
	    impl_proto_to_cf2(dip) == DDI_SUCCESS) {
...
SSSetIRQHandler(set_irq_handler_t *handler)
{
	...
	if (pcmcia_debug) {
		...
		cmn_err(CE_CONT, "SSetIRQHandler: socket=%x, function=%x\n",
			sock, func);
		cmn_err(CE_CONT, "\thandler(%x): socket=%x, irq=%x, id=%x\n",
			(int)handler, handler->socket, handler->irq,
			handler->handler_id);
	}
#endif
	dip = sockp->ls_dip[func];
...
pcmcia_dump_minors(dev_info_t *dip)
{
	...
	unit = ddi_getprop(DDI_DEV_T_ANY, dip, DDI_PROP_DONTPASS,
				    PCM_DEV_SOCKET, -1);
	cmn_err(CE_CONT,
		"pcmcia_dump_minors: dip=%x, socket=%d\n", (int)dip, unit);
...
int
pcmcia_alloc_mem(dev_info_t *dip, ra_request_t *req, ra_return_t *ret)
{
	int r;
	struct ra_map_list *map;

	map = ra_find_mapset(dip);

	if (map == NULL)
		return (DDI_FAILURE);

	r = ra_alloc(&map->ra_mem, req, ret);
	return (r);
}

int
pcmcia_alloc_io(dev_info_t *dip, ra_request_t *req, ra_return_t *ret)
{
	int r;
	struct ra_map_list *map;
...
pcmcia_free_mem(dev_info_t *dip, ra_return_t *ret)
{
	...
	if (map == NULL)
		return (DDI_FAILURE);
	ra_free(&map->ra_mem, ret->ra_addr_lo, ret->ra_len);
	return (DDI_SUCCESS);
}
...
pcmcia_free_io(dev_info_t *dip, ra_return_t *ret)
{
	...
	if (map == NULL)
		return (DDI_FAILURE);
	ra_free(&map->ra_io, ret->ra_addr_lo, ret->ra_len);
	return (DDI_SUCCESS);
}

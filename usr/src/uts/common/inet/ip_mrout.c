...
#pragma ident	"@(#)ip_mroute.c	1.35	98/01/30 SMI"
...
int		ip_mrouter_set(int, queue_t *, int, u_char *, int);
int		ip_mrouter_get(int, queue_t *, u_char *);
static int	get_version(u_char *);
static int	set_assert(int *);
static int	get_assert(u_char *);
static int	ip_mrouter_init(queue_t *, u_char *, int);
...
#ifdef IP_DEBUG
void show_packet(char *, mblk_t *);
void dump_hdr_cksum(ipha_t *);
void dump_cksum(mblk_t *, int);
void dump_icmp_echo_pattern(mblk_t *, size_t);
#endif
 ...
 */
int
ip_mrouter_set(int cmd, queue_t *q, int checkonly, u_char *data,
    int datalen)
{
 ...
 */
int
ip_mrouter_get(int cmd, queue_t *q, u_char *data)
{
	if (q != ip_g_mrouter)
		return (EACCES);
 ...
 */
static int
ip_mrouter_init(queue_t *q, u_char *data, int datalen)
{
...
ip_mforward(ill_t *ill, ipha_t *ipha, mblk_t *mp)
{
	...
	if (rt != NULL) {
		...
		if (npkts == 0) {	/* first upcall */
			 ...
			 * Use src and dst which are ipaddr_t's.  Network and
...
#ifdef LOG
	if (ip_mrtdebug > 1)
		(void) mi_strlog(ip_g_mrouter, 1, SL_TRACE | SL_ERROR,
 ...
 */
static int
ip_mdq(mblk_t *mp, ipha_t *ipha, ill_t *ill,
	ipaddr_t tunnel_src, struct mfc *rt)
{
	vifi_t	vifi;
	struct vif	*vifp;
 ...
 */
int
ip_mroute_stats(struct opthdr *optp, mblk_t *mp)
{
 ...
 */
int
ip_mroute_vif(struct opthdr *optp, mblk_t *mp)
{
	struct vifctl 	vi;
	vifi_t		vifi;
 ...
 */
int
ip_mroute_mrt(struct opthdr *optp, mblk_t *mp)
{
	int			i, j;
	struct mfc		*rt;
	struct mfcctl	mfcc;
...
dump_cksum(mblk_t *mp, int off)
{
	...
	sum = 0; i = 0;
	for (; mp; mp = mp->b_cont) {
		uph = (u16 *)(mp->b_rptr + off);
		off = 0;
		if ((mp->b_wptr - (u8 *)uph) & 0x1)
			printf("dump_cksum: bad mblk length\n");
		printf(" mblk len %lu\n", (size_t)(mp->b_wptr - (u8 *)uph));
		while (uph < (u16 *)mp->b_wptr) {
			printf("	%d: %x + %x = %x\n",
...
void
dump_icmp_echo_pattern(mblk_t *mp, size_t off)
{
	u_char	* uph;
	u_int	i;
	...
	for (; mp; mp = mp->b_cont) {
		...
		off = 0;
		printf("dump_icmp_echo_pattern: mblk len %lu\n",
		    (size_t)(mp->b_wptr - uph));
		while (uph < mp->b_wptr) {
			if (*uph != (i & 0xff)) {
				if (!start) {
					printf(
					    "	%d (%lu): %x should be %x\n",
					    i, (size_t)(uph - mblkstart),
					    (int)uph[0], i & 0xff);
		...
		if (start) {
			printf(" %ld bytes wrong:", (ptrdiff_t)(uph - start));
			if (diff)
				printf(" by %d\n", wrong);
			else
				printf(" not a constant difference\n");
...
void
show_packet(char *msg, mblk_t *mp)
{
	ipha_t	*ipha;
	u32	optlen;
	...
	if (optlen*4 + sizeof (*ipha) > mp->b_wptr - mp->b_rptr) {
		printf("show_packet: runt packet %d - %ld\n",
		    optlen, (ptrdiff_t)(mp->b_wptr - mp->b_rptr));
		return;
	}

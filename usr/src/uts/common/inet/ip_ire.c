...
#pragma ident	"@(#)ip_ire.c	1.53	98/01/20 SMI"
...
ire_t	*ire_create(u_char *addr, u_char *mask,
    ...
    u_short type, clock_t rtt, clock_t rtt_sd, u_int ll_hdr_len, ipif_t *ipif,
    ire_t *sire, u32 flags);
 ...
 *
 * NOTE : This function is not called as writer (no exclusive access
 * to IP perimeter).
 */
/* ARGSUSED */
int
ip_ire_advise(queue_t *q, mblk_t *mp)
{
	 ...
	 * NOTE: The updates depend on the fact that 32 bit writes
	 * are atomic.
	 */
	if (ipic->ipic_rtt) {
		/*
		 ...
		 */
		if (ire->ire_rtt != 0) {
			ire->ire_rtt = (ire->ire_rtt + ipic->ipic_rtt) >> 1;
		} else {
			ire->ire_rtt = ipic->ipic_rtt + (ipic->ipic_rtt >> 1);
		}
		if (ire->ire_rtt_sd != 0) {
			ire->ire_rtt_sd = (ire->ire_rtt_sd +
			    ipic->ipic_rtt_sd) >> 1;
		} else {
			ire->ire_rtt_sd = ipic->ipic_rtt_sd +
			    (ipic->ipic_rtt_sd >> 1);
		}
	}
	...
	if (ipic->ipic_ssthresh != 0) {
		if (ire->ire_ssthresh != 0)
			ire->ire_ssthresh =
			    (ipic->ipic_ssthresh + ire->ire_ssthresh) / 2;
		else
			ire->ire_ssthresh = ipic->ipic_ssthresh;
	}
	return (0);
}
...
ip_ire_report(queue_t *q, mblk_t *mp, caddr_t arg)
{
	 ...
	 *   123.123.123.123 123.123.123.123 12345 12345 12345 12345678 123
...
ire_report_ftable(ire_t *ire, char *mp)
{
	...
	(void) mi_mpprintf((mblk_t *)mp,
	    "%p %p %p %s %s %s %s %05d %05D %06D %08D %03d %d/%d/%d %s",
	    ...
	    ire->ire_max_frag, ire->ire_rtt, ire->ire_rtt_sd,
	    ire->ire_ssthresh, ref,
...
ip_ire_append(mblk_t *mp, ipaddr_t addr)
{
	 ...
	 */
	ire = ire_route_lookup(addr, 0, 0, 0, NULL, NULL,
 ...
 */
void
ire_add_then_put(queue_t *q, mblk_t *mp)
{
	mblk_t	*pkt;
 ...
 */
ire_t *
ire_create(u_char *addr, u_char *mask, u_char *src_addr, u_char *gateway,
    u_int max_frag, mblk_t *ll_hdr_mp, queue_t *rfq, queue_t *stq,
    u_short type, clock_t rtt, clock_t rtt_sd, u_int ll_hdr_len, ipif_t *ipif,
    ire_t *sire, u32 flags)
{
	...
	*ire = ire_nil;
	...
	if (gateway) {
		bcopy(gateway, &ire->ire_gateway_addr,
			IP_ADDR_LEN);
	}
	...
	ire->ire_type = type;
	ire->ire_rtt = rtt;
	ire->ire_rtt_sd = rtt_sd;
	ire->ire_flags = RTF_UP | flags;
	ire->ire_ll_hdr_length = ll_hdr_len;
	ATOMIC_32_ASSIGN(&ire->ire_atomic_ident, (u32)TICK_TO_MSEC(lbolt));
	ire->ire_tire_mark = ire->ire_ob_pkt_count + ire->ire_ib_pkt_count;
	ire->ire_create_time = (u32)hrestime.tv_sec;
...
ire_create_bcast(ipif_t *ipif, ipaddr_t  addr, ire_t **irep, int match_flags)
{
	...
	if (ire_ctable_lookup(addr, 0, IRE_BROADCAST, ipif, NULL, match_flags))
		return (irep);
...
ire_expire(ire_t *ire, char *arg)
{
	...
	if (flush_flags & FLUSH_CACHE_TIME) {
		 ...
		 */
		if (((u32)hrestime.tv_sec - ire->ire_create_time) * MILLISEC >
...
ire_fastpath_update(ire_t *ire, char  *arg)
{
	...
	if (ll_hdr_mp) {
		...
		ire->ire_ll_hdr_length =
		    (u_int)(ll_hdr_mp->b_wptr - ll_hdr_mp->b_rptr);
	}
}
 ...
 */
static void
ire_walk1(ire_t *ire, pfv_t func, char *arg)
{
	ire_t	*ire1 = NULL;
 ...
 */
void
ire_walk_wq(queue_t *wq, pfv_t func, char *arg)
{
	ire_t	**hash_ptr;
	ire_t	*ire;
	int i, j;
 ...
 */
static void
ire_walk_wq1(queue_t *wq, ire_t *ire, pfv_t func, char *arg)
{
	ire_t	*ire1 = NULL;
...
ire_add(ire_t *ire)
{
	...
	if (ire->ire_mp) {
		ire1 = (ire_t *)mi_zalloc(sizeof (ire_t));
		if (!ire1) {
	...
	case IRE_IF_NORESOLVER:
		break;
	default:
		printf("ire_add: ire %p has unrecognized IRE type (%d)\n",
	 ...
	 * For broadcast IREs, within the set that has the same ire_addr,
	 * the ones that have the same ill are grouped
	 * together so that ip_wput_ire() can avoid sending multiple broadcast
	 * packets out the same physical interface.
	 */
	irep = hash_ptr;
	/*
	 * Find the first entry that matches ire_addr. *irep will be null if
	 * no match.
	 */
	...
	if ((ire->ire_type & IRE_BROADCAST) && *irep != NULL) {
		/*
		 * Find the last ire which matches ire_addr and
		 * ire_ipif->ipif_ill. If no match on ill then find
		 * the last ire which matches ire_addr.
		 * At the end of the two loops irep will be
		 * the insertion point to insert things after
		 * the found match.
		 *
		 * First loop until
		 *	- matching ill,
		 *	- end of ire_addr match (but no ill match), or
		 *	- end of list
		 */
		...
		/* If we found a match on ill loop until last match on ill */
		while (*irep != NULL) {
			ire1 = *irep;
 ...
 */
void
ire_delete_route_gw(ire_t *ire, char *cp)
{
	ipaddr_t	gw_addr;
 ...
 * just checks the cache handle (ire_chandle) and
 ...
 */
void
ire_flush_cache(ire_t *ire, int flag)
{
	int i;
	ire_t *cire;
	ire_t *cire_next;
...
ire_match_args(ire_t *ire, ipaddr_t addr, ipaddr_t mask, ipaddr_t gateway,
    int type, ipif_t *ipif, queue_t *wrq, int match_flags)
{
    ...
    int type, ipif_t *ipif, ire_t **pire, queue_t *wrq, int flags)
{
	 ...
	 * MATCH_IRE_SRC or MATCH_IRE_ILL is set.
	 */
	if ((flags & (MATCH_IRE_SRC | MATCH_IRE_RQ | MATCH_IRE_ILL)) &&
	    (ipif == NULL))
		return (NULL);

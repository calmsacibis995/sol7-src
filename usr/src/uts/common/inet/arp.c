...
#pragma ident	"@(#)arp.c	1.45	97/12/06 SMI"
...
static int	ar_ce_create(arl_t *arl, uint32_t proto, u_char *hw_addr,
    uint32_t hw_addr_len, u_char *proto_addr,
    uint32_t proto_addr_len, u_char *proto_mask,
    u_char *proto_extract_mask, uint32_t hw_extract_start,
    uint32_t flags);
static void	ar_ce_delete(ace_t *ace);
static	void	ar_ce_delete_per_arl(ace_t *ace, u_char *arl);
static ace_t	**ar_ce_hash(uint32_t proto, u_char *proto_addr,
    uint32_t proto_addr_length);
static ace_t	*ar_ce_lookup(arl_t *arl, uint32_t proto,
    u_char *proto_addr, uint32_t proto_addr_length);
...
static boolean_t	ar_mask_all_ones(u_char *mask, uint32_t mask_len);
...
static void	ar_query_reply(ace_t *ace, int ret_val,
    u_char *proto_addr, uint32_t proto_addr_len);
...
static void	ar_xmit(arl_t *arl, uint32_t operation, uint32_t proto,
    uint32_t plen, u_char *haddr1, u_char *paddr1,
 ...
 */
static int
ar_ce_create(arl_t *arl, u_int proto, u_char *hw_addr, u_int hw_addr_len,
    u_char *proto_addr, u_int proto_addr_len, u_char *proto_mask,
    u_char *proto_extract_mask, u_int hw_extract_start, u_int flags)
{
...
ar_ce_delete(ace_t *ace)
{
	...
	ar_query_reply(ace, ENXIO, nilp(u_char), (uint32_t)0);
 ...
 */
static void
ar_ce_delete_per_arl(ace_t *ace, u_char *arl)
{
...
static ace_t **
ar_ce_hash(uint32_t proto, u_char *proto_addr, uint32_t  proto_addr_length)
{
...
ace_t *
ar_ce_lookup(arl_t *arl, uint32_t proto, u_char *proto_addr,
    uint32_t proto_addr_length)
{
	ace_t	*ace;
 ...
 */
static ace_t *
ar_ce_lookup_entry(arl_t *arl, uint32_t proto, u_char *proto_addr,
    uint32_t proto_addr_length)
{
	ace_t	*ace;
 ...
 */
static ace_t *
ar_ce_lookup_mapping(arl_t *arl, uint32_t proto, u_char *proto_addr,
    uint32_t proto_addr_length)
{
	ace_t	*ace;
 ...
 */
static void
ar_ce_report1(ace_t *ace, u_char *mp_arg)
{
 ...
 */
static void
ar_ce_resolve(ace_t *ace, u_char *hw_addr, uint32_t hw_addr_length)
{
	if (hw_addr_length == ace->ace_hw_addr_length) {
		...
		if (ace->ace_query_mp)
			ar_query_reply(ace, 0, nilp(u_char), (uint32_t)0);
	}
}

/* Pass arg1 to the pfi supplied, along with each ace in existence. */
static void
ar_ce_walk(pfi_t pfi, u_char *arg1)
{
	ace_t	*ace;
	ace_t	*ace1;
	ace_t	**acep;
 ...
 */
static arl_t *
ar_ll_lookup_by_name(u_char *name, uint32_t name_length)
{
	arl_t	*arl;
...
ar_ll_subnet_defaults(queue_t *q, mblk_t *mp)
{
	 ...
	 */
	bcopy(&sap_addr, (char *)dlur + arl->arl_xmit_template_sap_offset,
	    sizeof (sap_addr));
...
static boolean_t
ar_mask_all_ones(u_char *mask, uint32_t mask_len)
{
	if (mask == nilp(u_char))
		return (true);
 ...
 */
static int
ar_query_delete(ace_t *ace, u_char *ar)
{
 ...
 */
static void
ar_query_reply(ace_t *ace, int ret_val, u_char *proto_addr,
    uint32_t proto_addr_len)
{
	mblk_t	*areq_mp;
...
static void
ar_set_address(ace_t *ace, u_char *addrpos, u_char *proto_addr,
		uint32_t proto_addr_len)
{
	u_char	*mask, * to;
	int	len;
...
static u_char *
ar_snmp_msg_element(mblk_t **mpp, u_char *oldptr, size_t len)
{
	mblk_t	*mp;
...
static void
ar_snmp_msg2(ace_t *ace, u_char *mp_arg)
{
...
ar_xmit(arl_t *arl, uint32_t operation, uint32_t proto, uint32_t plen,
    u_char *haddr1, u_char *paddr1, u_char *haddr2, u_char *paddr2)
{
	arh_t	*arh;
	char	*cp;

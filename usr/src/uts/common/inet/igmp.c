...
#pragma ident	"@(#)igmp.c	1.34	97/12/06 SMI"
...
static void	igmp_sendpkt(ilm_t *ilm, int type, ipaddr_t addr);
...
/* ARGSUSED */
int
igmp_input(queue_t *q, mblk_t *mp, ipif_t *ipif)
{
	igmpa_t 	*igmpa;
	...
	if ((ipif != NULL) && (ip_debug > 1))
		(void) mi_strlog(ipif->ipif_ill->ill_rq, 1, SL_TRACE,
	...
	case IGMP_MEMBERSHIP_QUERY:
		...
		if (ipif) {
			if (ip_debug > 1)
				(void) mi_strlog(ipif->ipif_ill->ill_rq,
				    1, SL_TRACE,
 ...
 */
static void
igmp_sendpkt(ilm_t *ilm, int type, ipaddr_t addr)
{
	mblk_t	*mp;
	igmpa_t 	*igmpa;
	ipha_t 	*ipha;

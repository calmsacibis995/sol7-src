...
#pragma ident	"@(#)mib2.h	1.16	97/12/04 SMI"
...
#define	MIB2_TCP_deleteTCB	12		/* only writeable value */
	...
	int		tcpConnLocalPort;	/* XXX host byte order */
	...
	int		tcpConnRemPort;		/* XXX host byte order */
...
typedef struct mib2_udpEntry {
	...
	int		udpLocalPort;		/* XXX host byte order */
	struct udpEntryInfo_s {
		int		ue_state;
		IpAddress	ue_RemoteAddress;
		int		ue_RemotePort;	/* XXX host byte order */
	}		udpEntryInfo;
} mib2_udpEntry_t;

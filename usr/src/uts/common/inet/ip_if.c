...
#pragma ident	"@(#)ip_if.c	1.94	98/01/20 SMI"
 ...
 */
static void	ill_downi(ire_t *, char *);
static boolean_t ip_local_addr_ok(ipaddr_t, ipaddr_t);
static ip_m_t	*ip_m_lookup(ill_t *, t_uscalar_t);
static int	ip_siocaddrt(struct rtentry *);
static int	ip_siocdelrt(struct rtentry *);
static int	ip_sioctl_newmask(ipif_t *, ipaddr_t, queue_t *, mblk_t *);
static ipaddr_t	ip_subnet_mask(ipaddr_t);
static void	ip_wput_ioctl(queue_t *, mblk_t *);

static ipif_t	*ipif_allocate(ill_t *, int, u_int);
static void	ipif_arp_down(ipif_t *);
static void	ipif_downi(ire_t *, char *);
static void	ipif_free(ipif_t *);
static ipif_t	*ipif_lookup_on_name(char *, int, int);
static void	ipif_mtu_change(ire_t *, char *);
static void	ipif_multicast_up(ipif_t *);
#if 0 /* notdef */
static void	ipif_multicast_down(ipif_t *);
#endif
static void	ipif_recover_ire(ipif_t *);
static void	ipif_set_default(ipif_t *);
static int	ipif_up(ipif_t *, queue_t *, mblk_t *);
static int	ipif_arp_off(ipif_t *, uint32_t);
static int	ipif_arp_on(ipif_t *, uint32_t);
static boolean_t ip_addr_availability_check(ipif_t *);
static void	ipif_check_bcast_ires(ipif_t *);
...
extern	u_int	loopback_packets;
static	u_int	ill_index = 1;
 ...
 */
mblk_t *
ill_arp_alloc(ill_t *ill, u_char *template, ipaddr_t addr)
{
 ...
 */
static void
ill_dlur_copy_address(u_char *phys_src, u_int phys_length, t_scalar_t sap_src,
			t_scalar_t sap_length, u_char *dst)
{
 ...
 */
mblk_t *
ill_dlur_gen(u_char *addr, u_int addr_length, t_uscalar_t sap,
 ...
 */
void
ill_frag_prune(ill_t *ill, u_int max_count)
{
	ipfb_t	*ipfb;
	ipf_t	*ipf;
	ipf_t	**ipfp;
...
ill_init(queue_t *q, ill_t *ill)
{
	...
	*ill = ill_nil;
	...
	ill->ill_frag_hash_tbl = (ipfb_t *)mp->b_rptr;
	...
	if (igmp_slowtimeout_id == 0) {
		igmp_slowtimeout_id = qtimeout(ill->ill_rq, igmp_slowtimo,
		    ill->ill_rq, MSEC_TO_TICK(IGMP_SLOWTIMO_INTERVAL));
		if (ip_mrtdebug > 0)
 ...
 */
ill_t *
ill_lookup_on_name(char *name, size_t namelen)
{
	ill_t	*ill;
	ipif_t	*ipif;
	kstat_t	*ksp;
	...
	*ill = ill_nil;
	...
	ipif->ipif_local_addr = (ipaddr_t)htonl(INADDR_LOOPBACK);
	ipif->ipif_net_mask = (ipaddr_t)htonl(IN_CLASSA_NET);
	...
	kstat_named_init(&kn[0], "ipackets", KSTAT_DATA_INT32);
	kstat_named_init(&kn[1], "opackets", KSTAT_DATA_INT32);
	kstat_install(ksp);
...
ip_ill_report(queue_t *q, mblk_t *mp, void *arg)
{
	...
	for (ill = ill_g_head; ill; ill = ill->ill_next) {
		(void) mi_mpprintf(mp, "%08p %08p %08p %05d %05d %03d %s",
...
ip_ll_subnet_defaults(ill_t *ill, mblk_t *mp)
{
	...
	if (brdcst_addr_length == 0) {
		...
		if (!ill->ill_bcast_mp)
			ill->ill_bcast_mp = ill_dlur_gen(brdcst_addr,
			    ...
			    ill->ill_sap,
			    sap_length);
		areq = (areq_t *)ill->ill_resolver_mp->b_rptr;
	 ...
	 */
	(void) ipif_lookup_on_name(ill->ill_name, ill->ill_name_length, 1);
 ...
 */
ipif_t *
ipif_lookup_addr(ipaddr_t addr)
{
	ipif_t	*ipif;
	ill_t	*ill;
 ...
 */
int
ip_rt_add(ipaddr_t dst_addr, ipaddr_t mask, ipaddr_t gw_addr, u_int type,
    int flags, boolean_t ioctl_msg)
{
	...
	if (!(flags & RTF_GATEWAY)) {
		 ...
		 */
		if (ioctl_msg)
			ire = ire_ftable_lookup(dst_addr, 0, 0, IRE_INTERFACE,
			    ipif, NULL, NULL,
		...
		if (!save_mp)
			return (0);
		bcopy(&dst_addr, save_mp->b_wptr, IP_ADDR_LEN);
		save_mp->b_wptr += IP_ADDR_LEN;
		bcopy(&mask, save_mp->b_wptr, IP_ADDR_LEN);
		save_mp->b_wptr += IP_ADDR_LEN;
...
ip_rt_delete(ipaddr_t dst_addr, ipaddr_t mask, ipaddr_t gw_addr,
	u_int rtm_addrs, int flags, boolean_t ioctl_msg)
{
	ire_t	*ire;
	ipif_t	*ipif;
	u_int	type;
	...
	if (ipif && (ipif->ipif_ire_type == IRE_LOOPBACK) &&
		...
		for (mpp = &ipif->ipif_saved_ire_mp; *mpp;
		    mpp = &(*mpp)->b_cont) {
			mp = *mpp;
			if (bcmp(mp->b_rptr, &dst_addr, IP_ADDR_LEN) == 0) {
				*mpp = mp->b_cont;
				freeb(mp);
				break;
			}
		 ...
		 * addresses.  We set the IRE type to lookup based on whether
		 * this is a host route, a default route or just a prefix.
		 *
		 * If this is the case of a host route, then we set the netmask
		 * to all ones.  Otherwise, we use the supplied netmask.
		 */
		if (flags & RTF_HOST) {
			type = IRE_HOST;
			mask = IP_HOST_MASK;
...
ip_siocaddrt(struct rtentry *rt)
{
	...
	dst_addr = *(ipaddr_t *)((ipa_t *)&rt->rt_dst)->ip_addr;
	gw_addr = *(ipaddr_t *)((ipa_t *)&rt->rt_gateway)->ip_addr;
...
ip_sioctl_copyin_done(queue_t *q, mblk_t *mp)
{
	...
	case SIOCGIFINDEX:
		...
		ipif = ipif_lookup_on_name(ifr->ifr_name,
			...
			(iocp->ioc_cr && drv_priv(iocp->ioc_cr) == 0));
		if (!ipif) {
			err = ENXIO;
			goto done;
		}
		...
		case SIOCSIFADDR:
			...
			if (ipif->ipif_flags & IFF_UP) {
				...
				if (err == EINPROGRESS) {
					/*
					 * If IFF_NOARP is already set,
					 * inform the resolver
					 */
					if (ipif->ipif_ill->ill_net_type ==
					    IRE_IF_RESOLVER &&
					    (ipif->ipif_flags & IFF_NOARP) &&
					    (ipif->ipif_local_addr != 0)) {
						int error;
						error = ipif_arp_off(ipif,
		...
		case SIOCGIFADDR:
			...
			*(ipaddr_t *)ipaddr->ip_addr = ipif->ipif_local_addr;
			*(uint16_t *)ipaddr->ip_port = 0;
			/* Break to copyout. */
			break;
		case SIOCSIFDSTADDR:
			/* Set the interface address. */
...
/* BEHXXX */
		...
		case SIOCSIFFLAGS:
			...
			if (turn_off & IFF_UP) {
				...
				/*
				 * If the interface has just come up, and
				 * it is a IRE_IF_RESOLVER type then check the
				 * ipif_flags to see if IFF_NOARP set/unset.
				 * Notify the resolver accordingly
				 */
				if ((err == EINPROGRESS) &&
					...
					if (ipif->ipif_flags & IFF_NOARP)
						err = ipif_arp_off(ipif,
							ipif->ipif_local_addr);
					else
						err = ipif_arp_on(ipif,
							ipif->ipif_local_addr);
					if (err == 0)
						err = EINPROGRESS;
				}
			...
			/*
			 * If the interface is/was up, set/unset IFF_NOARP
			 * accordingly and notify the resolver
			 * However if the interface is not up, just
			 * set/unset IFF_NOARP. In the latter case
			 * the resolver will be notified when the interface
			 * is actually brought up
			 */
			if ((err == EINPROGRESS) ||
			    (ipif->ipif_flags & IFF_UP)) {
				if (turn_on & IFF_NOARP)
					/* Notify resolver to turn OFF ARP */
					error = ipif_arp_off(ipif,
						ipif->ipif_local_addr);
				else if (turn_off & IFF_NOARP)
					/* Notify resolver to turn ON ARP */
					error = ipif_arp_on(ipif,
						ipif->ipif_local_addr);
				if (error != 0) {
					err = error;
					goto done;
...
#define	IP_MIN_MTU	(IP_MAX_HDR_LENGTH+8)
		...
		case SIOCSIFBRDADDR:
			 ...
			 */
			if (u1 != ipif->ipif_broadcast_addr)
				ipif_check_bcast_ires(ipif);
	 ...
	 */
		...
		for (ill = ill_g_head;	ill; ill = ill->ill_next) {
			for (ipif = ill->ill_ipif; ipif;
				...
				bcopy(&ipif->ipif_local_addr,
				    ipa->ip_addr, IP_ADDR_LEN);
				ifr++;
			}
		}
	...
	case IF_UNITSEL: {
		...
		(void) sprintf(ill->ill_name, "%s%d", cp, ppa);
		...
		if (ill_lookup_on_name(ill->ill_name, name_length)) {
			ip1dbg(("ip_sioctl_copyin_done: IF_UNITSEL %s busy\n",
			    ill->ill_name));
			(void) sprintf(ill->ill_name, "%s%d", cp,
			    ill->ill_ppa);
			ill->ill_name_length = mi_strlen(ill->ill_name) + 1;
			err = EBUSY;
			goto done;
...
ip_sioctl_copyin_setup(queue_t *q, mblk_t *mp)
{
	...
	default:
		 ...
		 *
		 * NOTE : As tcp sends down IP_IOC_IRE_ADVISE_NO_REPLY on
		 * every close, for performance reasons, we dont want to
		 * become a writer for this ioctl alone. ip_ire_advise
		 * sets the values atomically.
		 */
		if (iocp->ioc_cmd == IP_IOCTL) {
			ipllc = (ipllc_t *)mp->b_cont->b_rptr;
			if (ipllc->ipllc_cmd == IP_IOC_IRE_ADVISE_NO_REPLY) {
				ip_wput_ioctl(q, mp);
			} else {
				become_exclusive(q, mp, (pfi_t)ip_wput_ioctl);
			}
			return;
...
ip_sioctl_iocack(queue_t *q, mblk_t *mp)
{
	...
	if (iocp->ioc_error && iocp->ioc_cmd == AR_ENTRY_SQUERY) {
		...
		ipaddr = (ipa_t *)&ar->arp_pa;
		bcopy(ipaddr->ip_addr, &addr, sizeof (addr));
 ...
 */
static int
ip_sioctl_newmask(ipif_t *ipif, ipaddr_t mask, queue_t *q, mblk_t *mp)
{
	int err = 0;
 ...
 * IOCTLs it doesn't recognize. It is called as writer (exclusive access
 * to IP perimeter) except for IP_IOCTL - IP_IOC_IRE_ADVISE_NO_REPLY.
 */
static void
 ...
 */
static ipif_t *
ipif_allocate(ill_t *ill, int id, u_int ire_type)
{
 ...
 */
static void
ipif_downi(ire_t *ire, char *ipif_arg)
{
...
ipif_free(ipif_t *ipif)
{
	...
	ipif->ipif_arp_on_mp = nilp(mblk_t);
 ...
 */
static ipif_t *
ipif_lookup_on_name(char *name, int namelen, int priv)
{
	char	*cp;
	char	*endp;
	int	id;
	...
	/*
	 * Don't let non-priviledged users create ipif's
	 */
	if (!priv)
		return (nilp(ipif_t));
...
ipif_recover_ire(ipif_t *ipif)
{
	...
	for (mp = ipif->ipif_saved_ire_mp; mp; mp = mp->b_cont) {
		...
		bcopy(mp->b_rptr, &ipaddr, IP_ADDR_LEN);
		bcopy(mp->b_rptr + IP_ADDR_LEN, &net_mask, IP_ADDR_LEN);
...
{
	uint32_t	our_local_addr = new_ipif->ipif_local_addr;
	ill_t *ill;
	ipif_t *ipif;
...
ipif_up(ipif_t *ipif, queue_t *q, mblk_t *mp)
{
	...
	bind = ip_dlpi_alloc(sizeof (dl_bind_req_t), DL_BIND_REQ);
	if (!bind)
		goto bad;
	((dl_bind_req_t *)bind->b_rptr)->dl_sap = ill->ill_sap;
	((dl_bind_req_t *)bind->b_rptr)->dl_service_mode = DL_CLDLS;
	...
	if (ill->ill_needs_attach) {
		...
		if (!attach)
			goto bad;
		((dl_attach_req_t *)attach->b_rptr)->dl_ppa = ill->ill_ppa;
...
{
	mblk_t	*arp_off_mp = nilp(mblk_t);
	mblk_t	*arp_on_mp = nilp(mblk_t);
	ill_t	*ill = ipif->ipif_ill;
	 ...
	 */
	arp_off_mp = ill_arp_alloc(ill, (u_char *)&ip_aroff_template, 0);
	if (!arp_off_mp)
		return (ENOMEM);

	arp_on_mp = ill_arp_alloc(ill, (u_char *)&ip_aron_template, 0);
	if (!arp_on_mp)
		goto failed;
	...
	/* Send an AR_INTERFACE_ON request */
...
ifgrp_insert(ipif_t *ipif)
{
	...
	if ((ipif->ipif_type == IFT_OTHER) ||
		 ...
		 *
		 * On the other hand, IFF_POINTOPOINT interfaces usually
...
ipif_check_bcast_ires(ipif_t *test_ipif)
{
	...
	test_net_mask = ip_net_mask(test_ipif->ipif_local_addr);
	 ...
	 */
	test_net_addr = test_net_mask  & test_ipif->ipif_local_addr;
	 ...
	 */
	test_subnet_addr = test_subnet_mask  & test_ipif->ipif_local_addr;
	...
	for (ipif = test_ipif->ipif_ill->ill_ipif; ipif;
		...
		if (test_net_ire) {
			net_mask = ip_net_mask(ipif->ipif_local_addr);
			net_addr = net_mask & ipif->ipif_local_addr;
		...
		if (test_subnet_ire) {
			if (!subnet_bcast_ire_created) {
				subnet_mask = ipif->ipif_net_mask;
				subnet_addr =
				    subnet_mask & ipif->ipif_local_addr;
		...
		/*
		 * add in and IRE's that we might hav ecreated.
		 */
		while (irep-- > ire_array) {
			*irep = ire_add(*irep);
			if (*irep)

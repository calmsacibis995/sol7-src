...
#pragma ident	"@(#)rts.c	1.29	97/12/06 SMI"
...
static void 	rts_err_ack(queue_t *q, MBLKP mp, t_scalar_t t_error,
	int sys_error);
int		rts_opt_default(queue_t *q, t_uscalar_t level, t_uscalar_t name,
	u_char *ptr);
int		rts_opt_get(queue_t *q, t_uscalar_t level, t_uscalar_t name,
	u_char *ptr);
int		rts_opt_set(queue_t *q, t_scalar_t mgmt_flags,
	t_uscalar_t level, t_uscalar_t name, t_uscalar_t inlen, u_char *invalp,
...
static struct qinit winit = {
	...
	NULL, (pfi_t)rts_wrw, nil(pfi_t), STRUIOT_STANDARD
};
...
rts_ioctl_alloc(mblk_t *data)
{
	...
	mp->b_wptr = (u_char *)(mp->b_rptr + sizeof (ipllc_t));
	if (data != NULL)
		linkb(mp, data);
	...
	mp1->b_wptr = (u_char *)(mp1->b_rptr + sizeof (struct iocblk));
...
rts_open(queue_t *q, dev_t *devp, int flag, int sflag, cred_t *credp)
{
	...
	q->q_ptr = mi_zalloc(sizeof (rts_t));
	WR(q)->q_ptr = q->q_ptr;
	rts = (rts_t *)q->q_ptr;
	if (priv)
...
rts_bind(queue_t *q, mblk_t *mp)
{
	 ...
	 */
	mp1 = reallocb(mp, sizeof (struct T_bind_ack) + sizeof (ipa_t), 1);
	if (mp1 == NULL) {
...
/* ARGSUSED */
int
rts_opt_default(queue_t *q, t_uscalar_t level, t_uscalar_t name, u_char *ptr)
{
...
int
rts_opt_get(queue_t *q, t_uscalar_t level, t_uscalar_t name, u_char *ptr)
{
	int	*i1 = (int *)ptr;
	rts_t	*rts = (rts_t *)q->q_ptr;
...
int
rts_opt_set(queue_t *q, t_scalar_t mgmt_flags, t_uscalar_t level,
...
rts_param_set(queue_t *q, mblk_t *mp, char *value, caddr_t cp)
{
	...
	/* Convert the value from a string into a u_int. */
	new_value = (u_int)mi_strtol(value, &end, 10);
	/*
...
rts_wput_iocdata(queue_t *q, mblk_t *mp)
{
	...
	if (STRUCT_FGET(sb, maxlen) < (int)sizeof (struct sockaddr)) {
	...
	mp1 = mi_copyout_alloc(q, mp, STRUCT_FGETP(sb, buf),
			sizeof (struct sockaddr));
	if (!mp1)
		return;
	STRUCT_FSET(sb, len, (int)sizeof (struct sockaddr));

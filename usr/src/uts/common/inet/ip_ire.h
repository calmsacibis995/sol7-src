...
#pragma ident	"@(#)ip_ire.h	1.28	98/01/20 SMI"
...
#define	MATCH_IRE_ILL		0x0400	/* Match IRE on ill */

#ifndef _IP_IRE_C
#ifdef __STDC__
extern	int	ip_ire_advise(queue_t *q, mblk_t *mp);

extern	void	ip_ire_append(mblk_t *mp, ipaddr_t addr);
...
extern	ire_t	*ire_create(uchar_t *addr, uchar_t *mask, uchar_t *src_addr,
				uchar_t *gateway, uint_t max_frag,
				mblk_t *ll_hdr_mp, queue_t *rfq,
				queue_t *stq, ushort_t type,
				clock_t rtt, clock_t rtt_sd,
				uint_t ll_hdr_len, ipif_t *ipif, ire_t *sire,
				u32 flags);

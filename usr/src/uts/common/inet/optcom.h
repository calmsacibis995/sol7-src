...
#pragma ident	"@(#)optcom.h	1.13	98/01/06 SMI"
 ...
 */
#define	ALIGN_TPIOPT_size	(sizeof (t_uscalar_t))
#define	ROUNDUP_TPIOPT(p)	(((p) + (ALIGN_TPIOPT_size - 1))\
					& ~(ALIGN_TPIOPT_size - 1))
#define	ISALIGNED_TPIOPT(addr)	\
		(((uintptr_t)(addr) & (ALIGN_TPIOPT_size - 1)) == 0)
 ...
 * Function prototypes
 */
void	optcom_err_ack(queue_t *, mblk_t *, t_scalar_t, int);
...
int	tpi_optcom_buf(queue_t *, mblk_t *, t_scalar_t *, t_scalar_t, int,
				optdb_obj_t *);
t_uscalar_t optcom_max_optbuf_len(opdes_t *opt_arr, uint_t opt_arr_cnt);

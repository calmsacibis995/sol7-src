...
#pragma ident	"@(#)fifovnops.c	1.86	97/12/05 SMI"
...
static int fifo_pathconf(vnode_t *, int, u_long *, cred_t *);
	...
	if (flag & FREAD) {
		while ((fnp->fn_flag & FIFOWOCR) == 0 &&
			...
			if (!cv_wait_sig_swap(&fnp->fn_wait_cv,
				...
				mutex_exit(&fnp->fn_lock->flk_lock);
				(void) fifo_close(*vpp, ((uint)flag & FMASK),
				    1, 0, crp);
				error = EINTR;
				goto done;
			}
...
/* ARGSUSED */
int
fifo_pathconf(vnode_t *vp, int cmd, u_long *valp, cred_t *cr)
{
	u_long val;
	int error = 0;

...
#pragma ident	"@(#)fifosubr.c	1.69	97/09/23 SMI"	/* SVr4.0 1.38	*/
/*
...
#define	FIFOHASH(vp) (((uint)(vp) >> FIFOSHFT) & (FIFO_HASHSZ - 1))
#else
#define	FIFOHASH(vp) (((uint)(vp) >> FIFOSHFT) % FIFO_HASHSZ)
#endif
...
fifo_connld(struct vnode **vpp, int flag, cred_t *crp)
{
	 ...
	 * the file structure only, there is no u_flist entry allocated.
	 */
...
out:
	VTOF(vp2)->fn_flag &= ~FIFOOPEN;
	(void) fifo_close(vp2, flag, 1, (offset_t)0, filep->f_cred);
	VN_RELE(vp2);
	return (error);
}

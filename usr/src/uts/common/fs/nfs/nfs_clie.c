...
#pragma ident	"@(#)nfs_client.c	1.103	97/12/03 SMI"
...
doit:
	...
	if (!error) {
		...
		if (!error) {
			...
			fattr3_to_vattr(vp, &res.resok.obj_attributes, vap);
			nfs_cache_check(vp, vap->va_ctime, vap->va_mtime,
			    vap->va_size, &seq, cr);
			nfs_attrcache_va(vp, vap, seq);
		} else {
 ...
 */
void
nfs_lockcompletion(vnode_t *vp, int cmd, struct flock64 *bfp)
{

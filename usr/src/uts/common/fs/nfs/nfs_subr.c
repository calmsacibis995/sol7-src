 ...
 *  publication.
 *
 *  	Copyright (c) 1986-1989,1994-1997 by Sun Microsystems, Inc.
 *  	Copyright (c) 1983,1984,1985,1986,1987,1988,1989  AT&T.
 *	All rights reserved.
 */

#pragma ident	"@(#)nfs_subr.c	1.164	98/01/16 SMI"
...
#ifdef DEBUG
			printf("NFS%d %s failed for server %s: error %d (%s)\n",
			    mi->mi_vers, mi->mi_rfsnames[which],
			    svp->sv_hostname, status, clnt_sperrno(status));
			if (curproc->p_sessp->s_vp != NULL) {
				if (!(mi->mi_flags & MI_NOPRINT))
					uprintf(
			"NFS%d %s failed for server %s: error %d (%s)\n",
					    mi->mi_vers, mi->mi_rfsnames[which],
...
#ifdef DEBUG
			printf(
			"NFS_ACL%d %s failed for server %s: error %d (%s)\n",
...
start:
	...
	bzero(rp, sizeof (*rp));
	rw_init(&rp->r_rwlock, NULL, RW_DEFAULT, NULL);
	...
	if (FAILOVER_MOUNT(mi)) {
		...
		if (dnm != NULL && nm != NULL) {
			...
			len = (u_int)(strlen(dnm) + strlen(nm) + 2);
...
acachehash(rnode_t *rp, cred_t *cred)
{
	...
	cp = (char *)&cred->cr_uid;
	ecp = (char *)&cred->cr_groups[cred->cr_ngroups];
	sum = 0;
	while (cp < ecp)
		sum += *cp++;
	cp = (char *)&rp;
		...
		for (svp = mi->mi_servers; svp; svp = svp->sv_next) {
			...
			"NFS: failing over: selecting original server %s",
				...
				cmn_err(CE_NOTE,
				    ...
				    mi->mi_vers, mi->mi_curr_serv->sv_hostname,
				    svp->sv_hostname);
#else
				cmn_err(CE_NOTE,
...
failover_remap(failinfo_t *fi)
{
	...
	if (vp == mi->mi_rootvp) {
		...
		if (rp->r_flags & RHASHED)
			rp_rmhash(rp);
		rp->r_server = mi->mi_curr_serv;
		rp->r_fh = *((nfs_fhandle *)&(rp->r_server->sv_fhandle));
		rp_addhash(rp);
		mutex_exit(&nfs_rtable_lock);
	} else {
		/*
		...
		if (rp->r_size != VTOR(nvp)->r_size ||
			...
			cmn_err(CE_WARN,
			    ...
			    VTOR(nvp)->r_server->sv_hostname, rp->r_path);
			return (EINVAL);
		}
	...
	/*
	 * Free any cached pathconf information as it may apply only to
	 * the old server.
	 */

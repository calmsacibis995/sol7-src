 ...
 * publication.
 *
 *	Copyright (c) 1986-1992,1994,1996-1997 by Sun Microsystems, Inc.
 *	Copyright (c) 1983,1984,1985,1986,1987,1988,1989 AT&T.
 *	All rights reserved.
 */

#pragma ident	"@(#)nfs_vnops.c	1.174	97/12/03 SMI"
/* SVr4.0 1.40 */
...
static int	nfs_rdwrlbn(vnode_t *, page_t *, u_int, size_t, int, cred_t *);
static int	nfswrite(vnode_t *, caddr_t, u_int, int, cred_t *);
static int	nfsread(vnode_t *, caddr_t, u_int, int, size_t *, cred_t *);
...
static int	nfs_getapage(vnode_t *, u_offset_t, size_t, u_int *,
...
nfs_close(vnode_t *vp, int flag, int count, offset_t offset, cred_t *cr)
{
	...
	if ((flag & FWRITE) && vp->v_pages != NULL) {
		if (VTOMI(vp)->mi_flags & MI_NOCTO)
			error = nfs_putpage(vp, (u_offset_t)0, 0, B_ASYNC, cr);
		else
...
nfs_write(vnode_t *vp, struct uio *uiop, int ioflag, cred_t *cr)
{
	...
	} else if (uiop->uio_loffset > MAXOFF32_T)
		return (EFBIG);
 ...
 */
static int
nfs_rdwrlbn(vnode_t *vp, page_t *pp, u_int off, size_t len,
    int flags, cred_t *cr)
{
	struct buf *bp;
	int error;
 ...
 */
static int
nfswrite(vnode_t *vp, caddr_t base, u_int offset, int count, cred_t *cr)
{
	mntinfo_t *mi;
	...
	do {
		...
		if (mi->mi_io_kstats) {
			...
			KSTAT_IO_PTR(mi->mi_io_kstats)->nwritten += tsize;
			mutex_exit(&mi->mi_lock);
		}
	} while (!error && count);
 ...
 */
static int
nfsread(vnode_t *vp, caddr_t base, u_int offset, int count, size_t *residp,
	cred_t *cr)
{
	mntinfo_t *mi;
...
nfs_setattr(vnode_t *vp, struct vattr *vap, int flags, cred_t *cr)
{
	...
	if ((mask & AT_SIZE) && ((vap->va_type == VREG) ||
	    (vap->va_type == VDIR)) && (vap->va_size > MAXOFF32_T))
		return (EFBIG);
	...
	if (rp->r_unldvp != NULL) {
		...
		if (rp->r_unldvp != NULL) {
			...
			if (vp->v_pages != NULL &&
				...
				ASSERT(vp->v_type != VCHR);
				error = nfs_putpage(vp, (u_offset_t)0, 0,
				    0, cr);
				if (error) {
					mutex_enter(&rp->r_statelock);
					if (!rp->r_error)
...
#define	IFSOCK		0140000		/* socket */
	...
	/*
	 * Bug 1107325: dev_t is u_int in 5.x and short in 4.x. Both 4.x
	...
	vattr_to_sattr(&vattr, args.ca_sa);
		...
		if (rdc->nfs_cookie == uiop->uio_offset &&
			...
			if (rrdc != NULL) {
				...
				if (rrdc != NULL) {
					...
					rrdc->buflen = count;
					rrdc->flags = RDDIR;
					cv_init(&rrdc->cv, NULL, CV_DEFAULT,
					    NULL);
					rrdc->next = rp->r_dir;
					rp->r_dir = rrdc;
					mutex_exit(&rp->r_statelock);
...
nfs_seek(vnode_t *vp, offset_t ooff, offset_t *noffp)
{
	...
	if (vp->v_type == VDIR)
		return (0);
	return ((*noffp < 0 || *noffp > MAXOFF32_T) ? EINVAL : 0);
}
 ...
 */
static int
nfs_getpage(vnode_t *vp, offset_t off, size_t len, u_int *protp,
...
retry:
	 ...
	 */
	mutex_enter(&rp->r_statelock);
	if ((uint32_t)off + len > rp->r_size + PAGEOFFSET && seg != segkmap) {
		mutex_exit(&rp->r_statelock);
...
/* ARGSUSED */
static int
nfs_getapage(vnode_t *vp, u_offset_t off, size_t len, u_int *protp,
	page_t *pl[], size_t plsz, struct seg *seg, caddr_t addr,
	enum seg_rw rw, cred_t *cr)
{
	rnode_t *rp;
...
nfs_map(vnode_t *vp, offset_t off, struct as *as, caddr_t *addrp,
	size_t len, u_char prot, u_char maxprot, u_int flags, cred_t *cr)
{
...
nfs_delmap(vnode_t *vp, offset_t off, struct as *as, caddr_t addr,
	size_t len, u_int prot, u_int maxprot, u_int flags, cred_t *cr)
{
	int error;
	rnode_t *rp;

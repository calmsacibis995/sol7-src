...
#pragma ident	"@(#)nfs3_vnops.c	1.130	98/02/26 SMI"
...
static int	nfs3_getapage(vnode_t *, u_offset_t, size_t, u_int *,
...
static int	nfs3_commit_vp(vnode_t *, cred_t *);
...
static int	nfs3_getpage(vnode_t *, offset_t, size_t, u_int *,
			page_t *[], size_t, struct seg *, caddr_t,
			enum seg_rw, cred_t *);
	...
	cred_t *cr)
{
	...
	if (nfs3_do_mapped_writes && (mi->mi_flags & MI_LOOPBACK) == 0) {
		mp = desballoc((unsigned char *)args->data.data_val,
		    args->data.data_len, BPRI_LO, &frnop);
	} else
		mp = NULL;
	args->mblk = mp;
	do {
...
top:
	...
	return (nfs3mknod(dvp, nm, &vattr, exclusive, mode, vpp, cr));
}
    ...
    enum vcexcl exclusive, int mode, vnode_t **vpp, cred_t *cr)
{
	...
	if (exclusive == EXCL) {
		...
		if (verfp->tv_sec != 0)
			...
			bcopy(&hrestime, verfp, sizeof (hrestime));
		}
	} else {
		/*
 ...
 */
static int
nfs3_getpage(vnode_t *vp, offset_t off, size_t len, u_int *protp,
...
/* ARGSUSED */
static int
nfs3_getapage(vnode_t *vp, u_offset_t off, size_t len, u_int *protp,
	page_t *pl[], size_t plsz, struct seg *seg, caddr_t addr,
	enum seg_rw rw, cred_t *cr)
{
	rnode_t *rp;
...
nfs3_dispose(vnode_t *vp, page_t *pp, int fl, int dn, cred_t *cr)
{
	 ...
	 * and clear the commit required state.  They will get freed
	 * later.
	 */
	while (plist != pp) {
		pptr = plist;

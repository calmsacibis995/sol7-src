...
#pragma ident	"@(#)nfs3_srv.c	1.42	97/06/25 SMI"
/* SVr4.0 1.21 */
	...
	struct svc_req *req, cred_t *cr)
{
	...
	if (!EQFSID(&args->from.dir.fh3_fsid, &args->to.dir.fh3_fsid) ||
	    !EQFID((fid_t *)&args->from.dir.fh3_xlen,
	...
	struct svc_req *req, cred_t *cr)
{
	...
	if (!EQFSID(&args->file.fh3_fsid, &args->link.dir.fh3_fsid) ||
	    !EQFID((fid_t *)&args->file.fh3_xlen,
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	*/
	...
	if ((args->maxcount < NFS3_READDIRPLUS_MIN_MAXCOUNT) ||
		 ...
		 * to bytes.
		 */
		if (args->dircount != uio.uio_resid) {
			namlen = strlen(((struct dirent64 *)data)->d_name);

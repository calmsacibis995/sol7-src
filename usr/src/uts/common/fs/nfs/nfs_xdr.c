...
#pragma ident	"@(#)nfs_xdr.c	1.63	97/08/12 SMI"
/* SVr4.0 1.6	*/
...
static bool_t xdr_fastshorten(XDR *, u_int);
...
xdr_fattr(XDR *xdrs, struct nfsfattr *na)
{
	...
	if (ptr != NULL) {
		if (xdrs->x_op == XDR_DECODE) {
			...
			na->na_atime.tv_sec = IXDR_GET_INT32(ptr);
			na->na_atime.tv_usec = IXDR_GET_INT32(ptr);
			na->na_mtime.tv_sec = IXDR_GET_INT32(ptr);
			na->na_mtime.tv_usec = IXDR_GET_INT32(ptr);
			na->na_ctime.tv_sec = IXDR_GET_INT32(ptr);
			na->na_ctime.tv_usec = IXDR_GET_INT32(ptr);
		} else {
			...
			IXDR_PUT_INT32(ptr, na->na_atime.tv_sec);
			IXDR_PUT_INT32(ptr, na->na_atime.tv_usec);
			IXDR_PUT_INT32(ptr, na->na_mtime.tv_sec);
			IXDR_PUT_INT32(ptr, na->na_mtime.tv_usec);
			IXDR_PUT_INT32(ptr, na->na_ctime.tv_sec);
			IXDR_PUT_INT32(ptr, na->na_ctime.tv_usec);
		}
		return (TRUE);
	}
	...
	if (xdr_enum(xdrs, (enum_t *)&na->na_type) &&
	    ...
	    xdr_timeval32(xdrs, &na->na_atime) &&
	    xdr_timeval32(xdrs, &na->na_mtime) &&
	    xdr_timeval32(xdrs, &na->na_ctime)) {
		return (TRUE);
	}
	return (FALSE);
}
...
xdr_sattr(XDR *xdrs, struct nfssattr *sa)
{
	if (xdr_u_int(xdrs, &sa->sa_mode) &&
	    ...
	    xdr_timeval32(xdrs, &sa->sa_atime) &&
	    xdr_timeval32(xdrs, &sa->sa_mtime)) {
		return (TRUE);
	}
	return (FALSE);
}
 ...
 */
bool_t
xdr_timeval32(XDR *xdrs, struct timeval32 *tv)
{
	...
	if (ptr != NULL) {
		if (xdrs->x_op == XDR_DECODE) {
			...
			*arg = *ptr++;
			...
			sa->sa_atime.tv_sec = IXDR_GET_INT32(ptr);
			sa->sa_atime.tv_usec = IXDR_GET_INT32(ptr);
			sa->sa_mtime.tv_sec = IXDR_GET_INT32(ptr);
			sa->sa_mtime.tv_usec = IXDR_GET_INT32(ptr);
		} else {
			arg = (int32_t *)&argp->saa_fh;
			*ptr++ = *arg++;
			*ptr++ = *arg++;
			...
			*ptr++ = *arg;
			...
			IXDR_PUT_INT32(ptr, sa->sa_atime.tv_sec);
			IXDR_PUT_INT32(ptr, sa->sa_atime.tv_usec);
			IXDR_PUT_INT32(ptr, sa->sa_mtime.tv_sec);
			IXDR_PUT_INT32(ptr, sa->sa_mtime.tv_usec);
		}
		return (TRUE);
	}
...
static bool_t
xdr_fastshorten(XDR *xdrs, u_int ressize)
{
	u_int curpos;

...
#pragma ident	"@(#)nfs_server.c	1.119	97/06/29 SMI"
/* SVr4.0 1.21 */
...
nfs_svc(struct nfs_svc_args *arg, model_t model)
{
	...
	addrmask.buf = mem_alloc(addrmask.maxlen);
	...
	if ((error = svc_tli_kcreate(fp, readsize, buf, &addrmask,
	    STRUCT_FGET(uap, maxthreads), &xprt)) != 0) {
		RELEASEF(STRUCT_FGET(uap, fd));
		kmem_free(addrmask.buf, addrmask.maxlen);
		return (error);
	}
...
static struct rpcdisp rfsdisptab_v3[] = {
	...
	/* RFS3_GETATTR = 1 */
	{rfs3_getattr,
	    xdr_GETATTR3args, NULL_xdrproc_t, sizeof (GETATTR3args),
	...
	/* RFS3_LOOKUP = 3 */
	{rfs3_lookup,
	    xdr_LOOKUP3args, NULL_xdrproc_t, sizeof (LOOKUP3args),
	...
	/* RFS3_READLINK = 5 */
	{rfs3_readlink,
	    xdr_READLINK3args, NULL_xdrproc_t, sizeof (READLINK3args),
	...
	/* RFS3_REMOVE = 12 */
	{rfs3_remove,
	    xdr_REMOVE3args, NULL_xdrproc_t, sizeof (REMOVE3args),
	...
	/* RFS3_RMDIR = 13 */
	{rfs3_rmdir,
	    xdr_RMDIR3args, NULL_xdrproc_t, sizeof (RMDIR3args),
	...
	/* RFS3_FSSTAT = 18 */
	{rfs3_fsstat,
	    xdr_FSSTAT3args, NULL_xdrproc_t, sizeof (FSSTAT3args),
	...
	/* RFS3_FSINFO = 19 */
	{rfs3_fsinfo,
	    xdr_FSINFO3args, NULL_xdrproc_t, sizeof (FSINFO3args),
	...
	/* RFS3_PATHCONF = 20 */
	{rfs3_pathconf,
	    xdr_PATHCONF3args, NULL_xdrproc_t, sizeof (PATHCONF3args),
...
rfs_dispatch(struct svc_req *req, SVCXPRT *xprt)
{
	...
	if (vers < NFS_VERSMIN || vers > NFS_VERSMAX) {
		...
		error++;
		cmn_err(CE_NOTE, "nfs_server: bad version number %u", vers);
		goto done;
	}
	vers -= NFS_VERSMIN;
	...
	if ((auth_flavor == RPCSEC_GSS) ||
		...
		if (!SVC_GETARGS(xprt, disp->dis_xdrargs, args)) {
			...
			svcerr_decode(xprt);
			error++;
			cmn_err(CE_NOTE, "nfs_server: bad getargs for %u/%d",
...
acl_dispatch(struct svc_req *req, SVCXPRT *xprt)
{
	...
	if (vers < NFS_ACL_VERSMIN || vers > NFS_ACL_VERSMAX) {
		...
		error++;
		cmn_err(CE_NOTE, "acl_server: bad version number %u", vers);
		goto done;
	}
	...
	if ((auth_flavor == RPCSEC_GSS) ||
		...
		if (!SVC_GETARGS(xprt, disp->dis_xdrargs, args)) {
			svcerr_decode(xprt);
			error++;
			cmn_err(CE_NOTE, "acl_server: bad getargs for %u/%d",
...
/*
 *  checkwin() validate the window value for AUTH_DES and AUTH_KERB
...
again:
	...
	/*
	 * *vpp may be an AutoFS node, so we perform a VOP_ACCESS() to
	 * trigger the mount of the intended filesystem, so we can perform
	 * the lookup in the intended filesystem instead of the AUTOFS
	 * filesystem.
	 */
	(void) VOP_ACCESS(*vpp, 0, 0, cr);

	if (mc_dvp->v_vfsmountedhere != NULL ||
	    (*vpp)->v_vfsmountedhere != NULL) {
		/*
		 * Either the vnode that the lookup resolved to, or its
		 * parent vnode has been covered by a new filesystem. Redo
		 * the lookup so that we may get the component and parent
		 * directory vnodes in the mounted filesystem.
		 * This only occurs when VOP_ACCESS() triggers the mount
		 * via AutoFS. Note that this condition will not be reached
		 * if AutoFS had previously triggered the mount since
		 * rfs_pathname() has already performed the filesystem
		 * traverse.
		 */
		VN_RELE(mc_dvp);
		VN_RELE(*vpp);
	...
	if ((*vpp)->v_type == VDIR) {
		...
		VN_RELE(mc_dvp);
		mc_dvp = NULL;
	} else if (VOP_REALVP(mc_dvp, &realvp) == 0 && realvp != mc_dvp) {
		/*

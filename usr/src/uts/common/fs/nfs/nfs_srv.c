...
#pragma ident	"@(#)nfs_srv.c	1.40	97/12/02 SMI"
/* SVr4.0 1.21 */
...
#define	BIG(va)		 ((va.va_size > (u_longlong_t)MAXOFF32_T) &&  \
			    (va.va_type == VREG || va.va_type == VDIR) || \
			    ((va.va_nodeid > MAXOFF32_T) && nfs2_limit_inum))
/*
...
static void	sattr_to_vattr(struct nfssattr *, struct vattr *);
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	do {
		...
		do {
			...
			rp->thread->t_flag |= t_flag;
			if (!error)
				vattr_to_nattr(&va, &rp->ns->ns_attr);
			rp = rp->list;
		} while (rp != lrp);
	} while (rp != NULL);
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	if ((va.va_mode & IFMT) == IFCHR) {
		if (args->ca_sa->sa_size == (u_int)NFS_FIFO_DEV)
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	if (!EQFSID(&args->rna_from.da_fhandle->fh_fsid,
	    &args->rna_to.da_fhandle->fh_fsid) ||
	    !EQFID((fid_t *)&args->rna_from.da_fhandle->fh_xlen,
	    (fid_t *)&args->rna_to.da_fhandle->fh_xlen)) {
		*status = NFSERR_XDEV;
		TRACE_1(TR_FAC_NFS, TR_RFS_RENAME_END,
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	if (!EQFSID(&args->la_from->fh_fsid,
	    &args->la_to.da_fhandle->fh_fsid) ||
	    !EQFID((fid_t *)&args->la_from->fh_xlen,
	    (fid_t *)&args->la_to.da_fhandle->fh_xlen)) {
		*status = NFSERR_XDEV;
		TRACE_1(TR_FAC_NFS, TR_RFS_LINK_END,
...
sattr_to_vattr(struct nfssattr *sa, struct vattr *vap)
{
	 ...
	 * of the mode.
	 */
	if (sa->sa_mode != (uint32_t) ((ushort)-1) &&
...
vattr_to_nattr(struct vattr *vap, struct nfsfattr *na)
{
	 ...
	 * uses on the wire then peg the time at the oldest or newest time
	 * NFS2 can represent.
	 */
	if (vap->va_atime.tv_sec > TIME32_MAX)
		na->na_atime.tv_sec = TIME32_MAX;
	else if (vap->va_atime.tv_sec < TIME32_MIN)
		na->na_atime.tv_sec = TIME32_MIN;
	else
		na->na_atime.tv_sec = vap->va_atime.tv_sec;
	na->na_atime.tv_usec = vap->va_atime.tv_nsec / 1000;

	if (vap->va_mtime.tv_sec > TIME32_MAX)
		na->na_mtime.tv_sec = TIME32_MAX;
	else if (vap->va_mtime.tv_sec < TIME32_MIN)
		na->na_mtime.tv_sec = TIME32_MIN;
	else
		na->na_mtime.tv_sec = vap->va_mtime.tv_sec;
	na->na_mtime.tv_usec = vap->va_mtime.tv_nsec / 1000;

	if (vap->va_ctime.tv_sec > TIME32_MAX)
		na->na_ctime.tv_sec = TIME32_MAX;
	else if (vap->va_ctime.tv_sec < TIME32_MIN)
		na->na_ctime.tv_sec = TIME32_MIN;
	else

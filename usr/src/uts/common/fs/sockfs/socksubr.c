...
#pragma ident	"@(#)socksubr.c	1.36	98/02/06 SMI"
...
#define	TOPT_NEXT(tohp)	\
	(struct T_opthdr *)((uintptr_t)(tohp) + ROUNDUP_tpi_opt((tohp)->len))
#define	TOPT_CONTENT(tohp)	(&((tohp)[1]))
 ...
 */
int
so_addr_verify(struct sonode *so, struct sockaddr *name, socklen_t namelen)
{
	int		family;
...
cleanup:
	 ...
	 */
	{
		...
		for (j = 0; j < i; j++) {
			/*
			 * See comment above for using bcopy instead of
			 * assignment (fp = fdbuf->fd_fds[j]).
			 */
			bcopy((char *)&fdbuf->fd_fds[j],
				(char *)&fp, sizeof (fp));
			dprint(0, ("fdbuf_extract: cleanup[%d] = %d, %p\n",
				j, *rp, fp));
			/* Decrement f_count */
			(void) closef(fp);
			setf(*rp++, NULLFP);
...
so_optlen(void *control, t_uscalar_t controllen, int oldflg)
{
	...
	for (cmsg = (struct cmsghdr *)control;
		...
		optlen += (t_uscalar_t)(ROUNDUP_tpi_opt(len) +
...
so_cmsglen(mblk_t *mp, void *opt, t_uscalar_t optlen, int oldflg)
{
	...
	for (tohp = (struct T_opthdr *)opt;
	    TOPT_VALID(tohp, opt, (uintptr_t)opt + optlen);
	    tohp = TOPT_NEXT(tohp)) {
		...
		if (tohp->level == SOL_SOCKET && tohp->name == SO_FILEP) {
			...
			fdbuflen = (int)TOPT_CONTENTLEN(tohp);
			...
			if (oldflg)
				continue;
			len = (t_uscalar_t)TOPT_CONTENTLEN(tohp);
		}
		cmsglen += (t_uscalar_t)ROUNDUP_cmsglen(len +
    ...
    void *control, t_uscalar_t controllen)
{
	...
	for (tohp = (struct T_opthdr *)opt;
		...
		if (tohp->level == SOL_SOCKET && tohp->name == SO_FILEP) {
			...
			cmsg->cmsg_len = (socklen_t)(TOPT_CONTENTLEN(tohp) +
			    sizeof (struct cmsghdr));
	...
	*srclenp = 0;
	...
	for (tohp = (struct T_opthdr *)opt;
		...
		if (tohp->level == SOL_SOCKET &&
			...
			*srcp = TOPT_CONTENT(tohp);
			*srclenp = (t_uscalar_t)TOPT_CONTENTLEN(tohp);
		}
	}
}

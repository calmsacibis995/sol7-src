...
#pragma ident	"@(#)sockvnops.c	1.29	98/01/26 SMI"
...
static int so_copyin(void *, void *, size_t, int);
static int so_copyout(void *, void *, size_t, int);
...
static int
so_copyin(void *from, void *to, size_t size, int fromkernel)
{
	if (fromkernel) {
		bcopy(from, to, size);
		return (0);
	}
	return (copyin(from, to, size));
}

static int
so_copyout(void *from, void *to, size_t size, int tokernel)
{
	if (tokernel) {
		bcopy(from, to, size);
		return (0);
	}
	return (copyout(from, to, size));
}
    ...
    int32_t *rvalp)
{
	...
	if (so->so_version == SOV_STREAM) {
		...
		if (cmd == I_PUSH) {
			...
			error = so_copyin((void *)arg, mname, FMNAMESZ,
			    (mode & (int)FKIOCTL));
	...
	case I_LIST: {
		...
		kstrlist.sl_modlist = freebuf =
			kmem_alloc(freebufsize, KM_SLEEP);
		...
		for (i = 0; i < so->so_pushcnt; i++) {
			...
			error = so_copyout(kstrlist.sl_modlist,
			    mlist_ptr, FMNAMESZ, (mode & (int)FKIOCTL));
			if (error)
				goto done;
			num_modules++;
			kstrlist.sl_modlist++;
		...
		/* Copyout "sockmod" */
		mlist_ptr = STRUCT_FGETP(u_strlist, sl_modlist);
		error = so_copyout("sockmod", mlist_ptr, FMNAMESZ,
		    (mode & (int)FKIOCTL));
		if (error)
			goto done;
		num_modules++;
	...
	case I_LOOK:
		...
		if (so->so_pushcnt == 0) {
			/* return sockmod */
			error = so_copyout("sockmod", (void *)arg, FMNAMESZ,
			    (mode & (int)FKIOCTL));
		} else {
			error = strioctl(vp, cmd, arg, mode, U_TO_K, cr,
			    rvalp);
	...
	case I_FIND:
		...
		if (*rvalp == 0 || error == EINVAL) {
			...
			error = so_copyin((void *)arg, mname, FMNAMESZ,
			    (mode & (int)FKIOCTL));
			if (error == 0 && strcmp(mname, "sockmod") == 0) {
				*rvalp = 1;
				error = 0;
			}
		}

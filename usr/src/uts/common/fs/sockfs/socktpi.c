...
#pragma ident	"@(#)socktpi.c	1.54	98/01/30 SMI"
...
solisten(struct sonode *so, int backlog, int flags)
{
	...
	dprintso(so, 1, ("solisten(%p, %d, 0x%x) %s\n",
	...
	if (!(so->so_state & SS_ISBOUND)) {
		 ...
		 * Assumes that all AF_INET transports can handle a O_T_BIND_REQ
...
/*
 * Copy the known SOL_SOCKET options using a single O_T_OPTMGMT_REQ
 * message.
 * Only used for transport that are known to handle such O_T_OPTMGMT_REQ
 * messages.
 * Note that such transports must handle all the SOL_SOCKET options.
 * Otherwise the O_T_OPTMGMT_REQ might fail to set any of the options.
 */
static void
soinheritoptions_multi(struct sonode *so, struct sonode *nso)
{
	 ...
	 */
	optmgmt_req.PRIM_type = O_T_OPTMGMT_REQ;
	...
	error = sowaitprim(nso, O_T_OPTMGMT_REQ, T_OPTMGMT_ACK,
...
soaccept(struct sonode *so, int fflag, struct sonode **nsop)
{
	...
	if (nso->so_family == AF_INET && nso->so_type == SOCK_STREAM &&
		 ...
		 * that can handle a {O_}T_CONN_RES without being bound.
		 */
		mutex_enter(&nso->so_lock);
	...
	int flags)
{
	...
	if (name == NULL || namelen == 0 ||
		namelen > (t_uscalar_t)so->so_faddr_maxlen) {
		error = EINVAL;
		goto done;
	}
	/*
	...
	if (srclen == 0) {
		...
		/* NOTE: holding so_lock while sleeping */
		mp = soallocproto2(&conn_req, sizeof (conn_req),
		...
		toh.status = 0;
		conn_req.OPT_length =
			(t_scalar_t)(sizeof (toh) + ROUNDUP_tpi_opt(srclen));
...
sorecvmsg(struct sonode *so, struct nmsghdr *msg, struct uio *uiop)
{
	 ...
	 */
	mutex_enter(&so->so_lock);
	error = so_lock_single(so, SOREADLOCKED, uiop->uio_fmode);
	mutex_exit(&so->so_lock);
	if (error)
		return (error);
		...
		int flags)
{
	...
	ASSERT(mp->b_wptr - mp->b_rptr > (ssize_t)(size - ALIGN_TPI_size));
...
sogetsockname(struct sonode *so)
{
	...
	if (!(so->so_state & SS_ISBOUND) && so->so_family != AF_UNIX) {
		...
		so->so_laddr_len = (socklen_t)sizeof (struct sockaddr);
		ASSERT(so->so_laddr_len <= so->so_laddr_maxlen);
		bzero(so->so_laddr_sa, so->so_laddr_len);
		/*
		 * Can not assume there is a sa_family for all
 ...
 * fallback should the O_T_OPTMGMT_REQ fail.
 *
		...
		void *optval, socklen_t *optlenp, int flags)
{
	...
	if (level == SOL_SOCKET) {
		...
		case SO_TYPE:
			value = so->so_type;
			option = &value;
			goto copyout;	/* No need to issue O_T_OPTMGMT_REQ */
		...
		case SO_SNDBUF: {
			 ...
			 * returned if the transport fails the O_T_OPTMGMT_REQ.
			 */
			lvalue = so->so_sndbuf;
			if (lvalue == 0) {
				mutex_exit(&so->so_lock);
	...
	error = sowaitprim(so, O_T_OPTMGMT_REQ, T_OPTMGMT_ACK,
...
/*
 * Set socket options. All options are passed down in a O_T_OPTMGMT_REQ.
 * SOL_SOCKET options are also recorded in the sonode. A setsockopt for
 * SOL_SOCKET options will not fail just because the O_T_OPTMGMT_REQ fails -

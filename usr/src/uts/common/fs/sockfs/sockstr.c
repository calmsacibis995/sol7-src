...
#pragma ident	"@(#)sockstr.c	1.28	98/01/15 SMI"
...
so_strinit(struct sonode *so, struct sonode *tso)
{
	...
	if (so->so_addr_size == 0) {
		...
		so->so_addr_size = (t_scalar_t)sizeof (struct sockaddr);
	...
	if (so->so_family == AF_UNIX) {
		...
		error = strioctl(vp, I_STR, (intptr_t)&strioc, 0, K_TO_K,
				CRED(), &retval);
		if (error)
		...
		strsigset_t *allmsgsigs, strpollset_t *pollwakeups)
{
	...
	*firstmsgsigs = *wakeups = *allmsgsigs = *pollwakeups = 0;
	...
	if (!ISALIGNED_tpi_prim_t(mp->b_rptr)) {
	...
	case T_UNITDATA_IND: {
		...
		/*
		 * A connect datagram socket. For AF_INET we verify that the
		 * source address matches the "connected to" address.
		 ...
		 * XXX AF_INET/SOCK_RAW shouldn't inspect the port numbers.
		 */
	...
	case T_UDERROR_IND: {
		...
		if (tudi->ERROR_type != 0)
			...
			error = ECONNRESET;
		so->so_delayed_error = (u_short)error;
		mutex_exit(&so->so_lock);
		return (NULL);
	}
...
{
	ASSERT(terr != TSYSERR);
	if (terr > (int)sizeof (tli_errs) / (int)sizeof (ushort))
		return (EPROTO);
	else

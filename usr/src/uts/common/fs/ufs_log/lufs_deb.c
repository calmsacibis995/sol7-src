#ident "@(#)lufs_debug.c   1.17     98/02/20 SMI"
...
int
top_begin_debug(ml_unit_t *ul, top_t topid, u_long size)
{
...
int
top_end_debug_1(ml_unit_t *ul, mt_map_t *mtm, top_t topid, u_long size)
{
	struct threadtrans	*tp;
...
top_check_debug(char *sva, offset_t smof, off_t snb, ml_unit_t *ul)
{
	...
	nbad = 0;
	sa = (u_char *)sva;
	da = (u_char *)(dva + (smof & (DEV_BSIZE - 1)));
...
static void
top_end_noasync(struct ufstrans *ufstrans, top_t topid, u_long size)
{
	int		error;
...
static void
top_begin_noasync(struct ufstrans *ufstrans, top_t topid, u_long size)
{

...
#pragma ident "@(#)lufs.c   1.48     98/02/20 SMI"
...
offset_t		lufs_bp_sv_off;

/* Adjustable constants */
uint32_t	ldl_maxlogsize	= LDL_MAXLOGSIZE;
uint32_t	ldl_minlogsize	= LDL_MINLOGSIZE;
...
/*ARGSUSED*/
static int
lufs_sv_constructor(void *voidp, void *cdrarg, int kmflags)
{
	lufs_save_t *sv = (lufs_save_t *)voidp;

	mutex_init(&sv->sv_mx, NULL, MUTEX_DEFAULT, NULL);
	sv->sv_frags = 0;
	sv->sv_error = 0;
	sv->sv_bp = NULL;
...
/*ARGSUSED*/
void
lufs_sv_destructor(void *voidp, void *cdrarg)
{
	lufs_save_t *sv = (lufs_save_t *)voidp;

	mutex_destroy(&sv->sv_mx);
}

/*ARGSUSED*/
static int
lufs_bp_constructor(void *voidp, void *cdrarg, int kmflags)
{
	lufs_buf_t *lbuf = (lufs_buf_t *)voidp;
...
/*ARGSUSED*/
void
lufs_bp_destructor(void *voidp, void *cdrarg)
{
	lufs_buf_t *lbuf = (lufs_buf_t *)voidp;

	bioreset(&lbuf->lb_buf);
}
...
{
	/*
	 * In case of panic, busy wait at low ipl and run md daemon queues
	 */
	if (panicstr)
		trans_wait_panic(cb);
	return (biowait(cb));
...
lufs_write_strategy(ml_unit_t *ul, buf_t *bp)
{
	...
	/*
	 * no metadata deltas, shouldn't be in matamap
	 */
...
_init(void)
{
	...
	lufs_sv = kmem_cache_create("lufs save", sizeof (lufs_save_t), 0,
	    lufs_sv_constructor, lufs_sv_destructor, NULL, NULL, NULL, 0);
	lufs_bp = kmem_cache_create("lufs bufs", sizeof (lufs_buf_t), 0,
	    lufs_bp_constructor, lufs_bp_destructor, NULL, NULL, NULL, 0);
	lufs_bp_sv_off = sizeof (lufs_buf_t) - sizeof (buf_t);

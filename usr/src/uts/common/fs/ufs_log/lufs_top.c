...
#pragma ident "@(#)lufs_top.c   1.18     98/02/06 SMI"
 ...
 */
static void
top_issue_sync(struct ufstrans *ufstrans)
{
	int	error;

	(*ufstrans->ut_ops->trans_begin_sync)
		(ufstrans, TOP_COMMIT_ASYNC, 0);
	(*ufstrans->ut_ops->trans_end_sync)
		(ufstrans, &error, TOP_COMMIT_ASYNC, 0);
}
...
/*ARGSUSED*/
void
top_begin_sync(struct ufstrans *ufstrans, top_t topid, u_long size)
{
	ml_unit_t	*ul	= (ml_unit_t *)ufstrans->ut_data;
	mt_map_t	*mtm	= ul->un_logmap;
...
/*ARGSUSED*/
static void
top_begin_async(struct ufstrans *ufstrans, top_t topid, u_long size)
{
	ml_unit_t	*ul	= (ml_unit_t *)ufstrans->ut_data;
	mt_map_t	*mtm	= ul->un_logmap;
...
/*ARGSUSED*/
void
top_end_sync(struct ufstrans *ufstrans, int *ep, top_t topid, u_long size)
{
	ml_unit_t	*ul	= (ml_unit_t *)ufstrans->ut_data;
	mt_map_t	*mtm	= ul->un_logmap;
	uint32_t	tid;
...
/*ARGSUSED*/
static void
top_end_async(struct ufstrans *ufstrans, top_t topid, u_long size)
{
	ml_unit_t	*ul	= (ml_unit_t *)ufstrans->ut_data;
	mt_map_t	*mtm	= ul->un_logmap;
...
top_snarf(ml_unit_t *ul)
{
	...
	ul->un_ut = ufs_trans_replace(ul->un_dev, tops, (void *)ul);
}

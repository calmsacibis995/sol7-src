...
#pragma ident "@(#)lufs_log.c	1.29	98/02/06 SMI"
...
extern int		sema_held();
extern int		maxphys;

extern int		nlogs;		/* number of logs */

extern struct kmem_cache	*lufs_sv;
extern struct kmem_cache	*lufs_bp;
extern offset_t			lufs_bp_sv_off;
...
ldl_strategy_done(buf_t *cb)
{
	 ...
	 */
	lbp = (lufs_buf_t *)((caddr_t)cb - lufs_bp_sv_off);
...
map_frag(
	...
	size_t		*pbcount)
{
	extent_t	*ext = ul->un_ebp->extents;
	uint32_t	nexts = ul->un_ebp->nextents;
	uint32_t	e = nexts;
	uint32_t	s = 0;
	uint32_t	i = e >> 1;
	uint32_t	lasti = i;
...
again:
	if (ext[i].lbno <= lblkno) {
		if ((ext[i].lbno + ext[i].nbno) > lblkno) {
			/* FOUND IT */
			bno_off = lblkno - (uint32_t)ext[i].lbno;
			*pbcount = MIN(bcount, dbtob(ext[i].nbno - bno_off));
			*pblkno = ext[i].pbno + bno_off;
			return (0);
		} else
			s = i;
	} else
...
/*
 * The log is a ``file'' and hence the logical offsets into the log
 ...
 * in several IO requests if this request spans disjoint device
 * locations.
 */
void
ldl_strategy(ml_unit_t *ul, buf_t *pb)
{
	...
	do {
		...
		if (cb->b_flags & B_READ)
			ub.ub_ldlreads.value.ul++;
		else
			ub.ub_ldlwrites.value.ul++;
...
push_dirty_bp(ml_unit_t *ul, buf_t *bp)
{
	 ...
	 *	allocate one w/o sleeping
	 */
	if ((newbp = cb->cb_free) != NULL) {
		cb->cb_free = newbp->b_forw;
	} else {
		newbp = trans_zalloc(sizeof (buf_t));
...
static int
within_range(off_t lof, daddr_t blkno, u_long bcount)
{
...
static u_long
fetchzeroes(caddr_t dst_va, offset_t dst_mof, u_long dst_nb, mapentry_t *me)
{
	...
	off_t *dst_lofp)
{
	...
	u_long	sav_dst_nb	= dst_nb;
	u_long	src_nb		= bp->b_bcount;
	...
	/*
	 * copy complete
	 *	return current lof for no good reason
	 */
	sema_v(&bp->b_sem);
	*dst_lofp = dst_lof;
	return (sav_dst_nb);
...
ldl_round_commit(ml_unit_t *ul)
{
	 ...
	 */
	bcount = dbtob(btod(bp->b_bcount));
	if (bcount == bp->b_bcount) {
		sema_v(&bp->b_sem);
		return;
	}
	bp->b_bcount = bcount;
	ul->un_tail_lof = dbtob(bp->b_blkno) + bp->b_bcount;
	wrapped = 0;
	if (ul->un_tail_lof == ul->un_eol_lof) {
		ul->un_tail_lof = ul->un_bol_lof;
		++wrapped;
	...
	/* LINTED */
	st = (sect_trailer_t *)
		((bp->b_un.b_addr + bp->b_bcount) - sizeof (*st));
	st->st_tid = ul->un_logmap->mtm_tid;
	st->st_ident = ul->un_tail_ident++;
	 ...
	 */
	if (wrapped || bp->b_bcount == bp->b_bufsize)
		push_dirty_bp(ul, bp);
	else
		sema_v(&bp->b_sem);
}
...
/*ARGSUSED*/
static int
logseek(ml_unit_t *ul, off_t lof, size_t nb, off_t *lofp, mapentry_t *me)
{
	buf_t	*bp;
	u_long	actual;
	...
	while (nb) {
		...
		ASSERT((me == NULL) || ((me->me_flags & ME_ROLL) == 0) ||
	...
	mapentry_t *me)		/* Map entry list */
{
	...
	while (me) {
		...
		/*
		 * this is a delta full of zeroes; no data exists in the log
		 */
		if (me->me_dt == DT_ABZERO) {
			actual = fetchzeroes(va, mof, nb, me);
			ASSERT(actual);
			me = me->me_agenext;
			continue;
		}
		...
		if (mof > me->me_mof) {
			...
			error = logseek(ul, me->me_lof, rnb, &lof, me);
			if (error)
				return (EIO);
			rva = va;
			rnb = me->me_nb - rnb;
...
ldl_sethead(ml_unit_t *ul, off_t data_lof, uint32_t tid)
{
	...
	if (data_lof == -1)
		...
		/* woops, header spans sectors; subtract out sector trailer */
		if (btodb(new_lof) != btodb(data_lof))
			new_lof -= sizeof (sect_trailer_t);

		/* woops, header wrapped the log; go to last sector */
...
ldl_settail(ml_unit_t *ul, off_t lof, size_t nb)
{
	 ...
	 */
	(void) logseek(ul, lof, nb, &new_lof, NULL);
...
ldl_logscan_end(ml_unit_t *ul)
{
	 ...
	 */
	bufsize = ldl_bufsize(ul);
	alloc_rdbuf(&ul->un_rdbuf, bufsize, MAPBLOCKSIZE);
	alloc_wrbuf(&ul->un_wrbuf, bufsize);
...
int
ldl_empty(ml_unit_t *ul)
{
	return (ul->un_head_lof == ul->un_tail_lof);
}
...
ldl_seterror(ml_unit_t *ul, char *why)
{
	...
	cmn_err(CE_WARN, "ufs log for %s changed state to Error",
	    ((ufsvfs_t *)ul->un_ufsvfs)->vfs_fs->fs_fsmnt);

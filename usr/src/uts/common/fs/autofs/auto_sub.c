...
#pragma ident	"@(#)auto_subr.c	1.59	98/01/23 SMI"
...
auto_mount_thread(struct autofs_callargs *argsp)
{
	...
	mutex_init(&auto_mount_thread_cpr_lock, "auto_mount_thread cpr lock",
		MUTEX_DEFAULT, NULL);
	...
	cred_t *cred)
{
	...
	if (!cmpldev(&req.devid, devid)) {
		error = EOVERFLOW;
		goto out;
	}

	error = auto_calldaemon(fnip, AUTOFS_POSTMOUNT,
	    xdr_postmountreq, &req,
	    xdr_postmountres, &result,
	    cred, hard);
	if (!error)
		error = result.status;
	...
	bool_t hard)
{
	...
	AUTOFS_DPRINT((4, "auto_postunmount_request: u=%p\n", (void *)ul));

	error = auto_calldaemon(fnip, AUTOFS_POSTUNMOUNT,
	    xdr_postumntreq, ul,
	    xdr_postumntres, &result,
	    cred, hard);
	if (!error)
		error = result.status;
...
mount:
		m->flags |= MS_SYSSPACE;
		/*
		 * XXX:	In the non-error case, the code below relies on the
		 *	hold that domount establishes.  This is dubious
		 *	practice at best.  The VN_RELE really ought to be
		 *	deferred until after the last use of mvp within the
		 *	routine.  For that matter, shouldn't the code above
		 *	that obtains mvp by calling fntovn hold mvp?
		 */
		error = domount(NULL, m, mvp, kcred, &vfsp);
		VN_RELE(mvp);
		if (error != 0) {
			auto_log(CE_WARN,
...
auto_enter(fnnode_t *dfnp, char *name, fnnode_t **fnpp, cred_t *cred)
{
	...
	for (; cfnp; cfnp = cfnp->fn_next) {
		...
		if (cfnp->fn_next != NULL) {
			...
			ASSERT(diff != 0);
			if (diff > 1 && offset == 0) {
				offset = (u_short)cfnp->fn_offset + 1;
				spp = &cfnp->fn_next;
			}
		} else if (offset == 0) {
			offset = (u_short)cfnp->fn_offset + 1;
...
unmount_triggers(fnnode_t *fnp, action_list **alp)
{
	...
	*alp = fnp->fn_alp;
	...
	while ((tp = next) != NULL) {
		...
		if (!cmpldev(&p->devid, vfsp->vfs_dev)) {
	...
	if (error = auto_postunmount_request(vfsip, ul, kcred, TRUE))
		auto_log(CE_WARN,
		    "unmount_triggers: postunmount failed fnp=%p error=%d",
		    (void *)fnp, error);
	/*
...
unmount_node(vfs_t *vfsp)
{
	...
	AUTOFS_DPRINT((4, "\tunmount_node vfsp=%p\n", (void *)vfsp));
	...
	if ((error = VFS_ROOT(vfsp, &root_vp)) == 0) {
		...
		if (error = VOP_GETATTR(root_vp, &vattr, ATTR_HINT, kcred)) {
			auto_log(CE_WARN,
	...
	if (cfnp->fn_flags & MF_IK_MOUNT) {
		...
		if ((error = vn_vfslock(cvp)) == 0) {
			...
			pul.next = NULL;
			if (!cmpldev(&pul.devid, vattr.va_fsid) ||
			    !cmpldev(&pul.rdevid, vattr.va_rdev)) {
				vn_vfsunlock(cvp);
				error = EOVERFLOW;
				goto done;
			}
		...
		if (!cmpldev(&ul.devid, vattr.va_fsid) ||
		    !cmpldev(&ul.rdevid, vattr.va_rdev)) {
			vn_vfsunlock(cvp);
			error = EOVERFLOW;
			goto done;
		}
	...
	AUTOFS_DPRINT((5, "\tunmount_node vfsp=%p error=%d\n", (void *)vfsp,
	    error));
	return (error);
}
...
unmount_tree(void)
{
	...
	mutex_init(&unmount_tree_cpr_lock, "unmount_tree cpr lock",
		MUTEX_DEFAULT, NULL);

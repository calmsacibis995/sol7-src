...
#pragma ident	"@(#)hsfs_vnops.c	1.61	97/11/22 SMI"
	...
	struct cred *cred)
{
	...
	if ((intlf = hp->hs_dirent.intlf_sz + hp->hs_dirent.intlf_sk) != 0) {
		...
		/* no interleaving, set default klustsize */
		klustsz = KLUSTSIZE;
	}
...
again:
	...
	if ((pagefound = page_exists(vp, off)) == 0) {
		...
		if ((blkoff + klustsz) <= (u_long) filsiz)
			blksz = klustsz;
		else
#define	BUG_1152652	/* real fix may be to the cdrom driver */
#if defined(BUG_1152652) && defined(i386)
		 ...
		 */
		if (intlfsz)
			blksz = MIN(blksz, (intlfsz - (blkoff % intlfsz)));

		pp = pvn_read_kluster(vp, off, seg, addr, &io_off_tmp,
		    &io_len_tmp, (u_offset_t)blkoff, blksz, 0);
		ASSERT(io_off_tmp <= MAXOFF_T);
		io_off = (u_int)io_off_tmp;
		io_len1 = (u_int)io_len_tmp;
		...
		blkno = (daddr_t)btodb(bof+(xarsiz+io_off) /
			intlfsz * intlf + ((xarsiz+io_off) % intlfsz));

		xlen = (u_int) (io_off + io_len) & PAGEOFFSET;
		if (xlen != 0) {
			pagezero(pp->p_prev, xlen, PAGESIZE - xlen);
		}

		bp2 = hsfs_startio(devvp, blkno, pp, (u_int) io_len, 0,
		    pl == NULL ? (B_ASYNC | B_READ) : B_READ, (seg == segkmap));
		...
		bp3 = hsfs_startio(devvp, blkno, pp, (u_int) io_len, 0,
		    pl == NULL ? (B_ASYNC | B_READ) : B_READ, (seg == segkmap));

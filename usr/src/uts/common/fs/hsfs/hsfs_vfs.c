/*
 * Copyright (c) 1990, 1994, 1996-1997, by Sun Microsystems, Inc.
 * All rights reserved.
 */

#pragma ident   "@(#)hsfs_vfsops.c 1.55     97/10/22 SMI"
...
static int hsfs_unmount(struct vfs *vfsp, struct cred *cr);
...
static int hs_parsehsvol(struct hsfs *fsp, u_char *volp,
	struct hs_volume *hvp);
static int hs_findisovol(struct hsfs *fsp, struct vnode *vp,
	struct hs_volume *hvp);
static int hs_parseisovol(struct hsfs *fsp, u_char *volp,
	struct hs_volume *hvp);
    ...
    struct mounta *uap, struct cred *cr)
{
	...
	if (data && datalen) {
		if (datalen != sizeof (flags))
			return (EINVAL);
		if (copyin(data, (caddr_t)&flags, sizeof (flags)))
			return (EFAULT);
		if (flags & ~ (HSFSMNT_NORRIP | HSFSMNT_NOTRAILDOT |
		    HSFSMNT_NOMAPLCASE)) {
			return (EINVAL);
		}
	} else
	...
	/*
	 * XXX: the following should go away in the future. It is only
	 *	provided here for backward compatibility with programs
	 *	compiled under SunOS 5.x (0 <= x <= 3)
	 */
	if (uap->flags & MS_NO_RRIP)
...
#ifdef LOCKNEST
	(void) spec_close(fsp->hsfs_devvp, FREAD, 1, (offset_t)0, cr);
#else
...
hsfs_vget(struct vfs *vfsp, struct vnode **vpp, struct fid *fidp)
{
	...
	if ((*vpp = hs_findhash(nodeid, vfsp)) == NULL) {
		...
		error = hs_remakenode(fid->hf_dir_lbn, (u_int) fid->hf_dir_off,
		    vfsp, vpp);
		return (error);
	}
	rw_exit(&htp->hshash_lock);
	...
	int		isroot)
{
	 ...
	 */
#ifdef LOCKNEST
	if (error = spec_open(&devvp, FREAD, cr))
#else
	...
	vap.va_mask = AT_SIZE;
#ifdef LOCKNEST
	if ((error = spec_getattr(devvp, &vap, ATTR_COMM, cr)) != 0)
#else
...
cleanup:
#ifdef LOCKNEST
	(void) spec_close(devvp, FREAD, 1, (offset_t)0, cr);
#else
 ...
 */
static int
hs_parsehsvol(struct hsfs *fsp, u_char *volp, struct hs_volume *hvp)
{
 ...
 */
static int
hs_parseisovol(struct hsfs *fsp, u_char *volp, struct hs_volume *hvp)
{
...
hs_getmdev(char *fspec, int flags, dev_t *pdev, mode_t *mode)
{
	 ...
	 */
	error = lookupname(fspec, UIO_USERSPACE, FOLLOW,
	    NULLVPP, &vp);
	if (error) {
		if (error == ENOENT) {
...
#ifdef LOCKNEST
	(void) spec_getattr(vp, &vap, 0, CRED());
#else

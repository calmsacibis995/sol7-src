...
#pragma ident	"@(#)hsfs_susp_subr.c	1.14	97/09/22 SMI"
		...
		for (extnp = extension_name_table, impl_bit_num = 0;
			...
			for (ext_sigp = extnp->signature_table;
				...
				if (strncmp((char *)sig_string,
					 ...
					 * do not agree use what the kernel
					 * expects or risk a panic.
					 */
					if (SUA_len < SUF_LEN(sig_string)) {
						cmn_err(CE_NOTE,
					"parse_signatures: SUA length too big: "
					"expected=%d, found=%d: adjusting",
						    SUA_len,
						    SUF_LEN(sig_string));
						SUF_LEN(sig_string) = SUA_len;
					}

...
#pragma ident	"@(#)prvnops.c	1.98	98/01/06 SMI"
...
static int praccess(vnode_t *, int, int, struct cred *);
...
prclose(vnode_t *vp, int flag, int count, offset_t offset, struct cred *cr)
{
	...
	/*
	 * Decrement the count of opens for writing.
	 * Cancel exclusive opens when only self-opens remain.
	 */
	if ((flag & FWRITE) && --pcp->prc_writers == pcp->prc_selfopens)
		pcp->prc_flags &= ~PRC_EXCL;
	ASSERT(pcp->prc_writers >= pcp->prc_selfopens);
	mutex_exit(&pr_pidlock);
...
pr_read_map_common(prnode_t *pnp, struct uio *uiop, int reserved)
{
	...
	if (uiop->uio_offset < nmaps * sizeof (prmap_t)) {
...
static int
pr_read_xmap(prnode_t *pnp, struct uio *uiop)
{
	proc_t *p;
	struct as *as;
	int nmems;
	...
	if ((size_t)uiop->uio_offset < nmems * sizeof (prxmap_t)) {
		start = (char *)prxmapp + uiop->uio_offset;
		count = nmems * sizeof (prxmap_t) - uiop->uio_offset;
		error = uiomove(start, count, UIO_READ, uiop);
	}
...
pr_read_cred(prnode_t *pnp, struct uio *uiop)
{
	...
	pcrp->pr_ngroups = ngroups = MIN(cp->cr_ngroups, (uint_t)ngroups_max);
...
pr_read_usage(prnode_t *pnp, struct uio *uiop)
{
	...
	start = (char *)upup + uiop->uio_offset;
	count = sizeof (prusage_t) - uiop->uio_offset;
	error = uiomove(start, count, UIO_READ, uiop);
...
static int pr_read_status_32(),
	...
	pr_read_lwpusage_32(), pr_read_xregs_32(),
#if defined(sparc) || defined(__sparc)
	pr_read_gwindows_32(),
#endif
...
static int (*pr_read_function_32[PR_NFILES])() = {
	...
	pr_read_xregs_32,	/* /proc/<pid>/lwp/<lwpid>/xregs	*/
#if defined(sparc) || defined(__sparc)
...
/* ARGSUSED */
static int
pr_read_status_32(prnode_t *pnp, struct uio *uiop)
{
	pstatus32_t *sp;
	int error;
	long count;
...
/* ARGSUSED */
static int
pr_read_lstatus_32(prnode_t *pnp, struct uio *uiop)
{
	proc_t *p;
	kthread_t *t;
	size_t size;
...
/* ARGSUSED */
static int
pr_read_psinfo_32(prnode_t *pnp, struct uio *uiop)
{
	psinfo32_t psinfo;
	proc_t *p;
	int error = 0;
...
/* ARGSUSED */
static int
pr_read_lpsinfo_32(prnode_t *pnp, struct uio *uiop)
{
	proc_t *p;
	kthread_t *t;
	size_t size;
...
static int
pr_read_map_common_32(prnode_t *pnp, struct uio *uiop, int reserved)
{
	proc_t *p;
	struct as *as;
	int nmaps;
	...
	if (uiop->uio_offset < nmaps * sizeof (prmap32_t)) {
		start = (char *)prmapp + uiop->uio_offset;
		count = nmaps * sizeof (prmap32_t) - uiop->uio_offset;
		error = uiomove(start, count, UIO_READ, uiop);
	}
...
static int
pr_read_map_32(prnode_t *pnp, struct uio *uiop)
{
	ASSERT(pnp->pr_type == PR_MAP);
...
static int
pr_read_rmap_32(prnode_t *pnp, struct uio *uiop)
{
	ASSERT(pnp->pr_type == PR_RMAP);
...
static int
pr_read_xmap_32(prnode_t *pnp, struct uio *uiop)
{
	proc_t *p;
	struct as *as;
	int nmems;
	...
	if ((size_t)uiop->uio_offset < nmems * sizeof (prxmap32_t)) {
		start = (char *)prxmapp + uiop->uio_offset;
		count = nmems * sizeof (prxmap32_t) - uiop->uio_offset;
		error = uiomove(start, count, UIO_READ, uiop);
	}
...
/* ARGSUSED */
static int
pr_read_sigact_32(prnode_t *pnp, struct uio *uiop)
{
	proc_t *p;
	...
	start = (char *)sap + uiop->uio_offset;
	count = (NSIG-1) * sizeof (struct sigaction32) - uiop->uio_offset;
	error = uiomove(start, count, UIO_READ, uiop);
out:
...
/* ARGSUSED */
static int
pr_read_auxv_32(prnode_t *pnp, struct uio *uiop)
{
...
static int
pr_read_usage_32(prnode_t *pnp, struct uio *uiop)
{
	prhusage_t *pup;
	prusage32_t *upup;
	proc_t *p;
	...
	start = (char *)upup + uiop->uio_offset;
	count = sizeof (prusage32_t) - uiop->uio_offset;
	error = uiomove(start, count, UIO_READ, uiop);
...
static int
pr_read_lusage_32(prnode_t *pnp, struct uio *uiop)
{
	int nlwp;
	prhusage_t *pup;
	prheader32_t *php;
...
/* ARGSUSED */
static int
pr_read_pagedata_32(prnode_t *pnp, struct uio *uiop)
{
	proc_t *p;
	int error;
...
/* ARGSUSED */
static int
pr_read_opagedata_32(prnode_t *pnp, struct uio *uiop)
{
	proc_t *p;
	struct as *as;
	int error;
...
/* ARGSUSED */
static int
pr_read_watch_32(prnode_t *pnp, struct uio *uiop)
{
	proc_t *p;
	int error;
	char *start;
...
/* ARGSUSED */
static int
pr_read_lwpstatus_32(prnode_t *pnp, struct uio *uiop)
{
	lwpstatus32_t *sp;
	int error;
	char *start;
...
/* ARGSUSED */
static int
pr_read_lwpsinfo_32(prnode_t *pnp, struct uio *uiop)
{
...
static int
pr_read_lwpusage_32(prnode_t *pnp, struct uio *uiop)
{
	prhusage_t *pup;
	prusage32_t *upup;
	proc_t *p;
...
static int
pr_read_xregs_32(prnode_t *pnp, struct uio *uiop)
{
	proc_t *p;
	kthread_t *t;
	int error;
...
/* ARGSUSED */
static int
pr_read_gwindows_32(prnode_t *pnp, struct uio *uiop)
{
	proc_t *p;
	kthread_t *t;
	gwindows32_t *gwp;
...
prgetattr(vnode_t *vp, struct vattr *vap, int flags, struct cred *cr)
{
	...
	case PR_FDDIR:
		vap->va_nlink = 2;
		vap->va_size = (up->u_nofiles + 2) * PRSDSIZE;
		break;
	case PR_LWPDIR:
	...
	case PR_LWPUSAGE:
		...
		break;
	case PR_XREGS:
		/* XX64 -- should extra register file disappear in 64-bit? */
		if (prhasx())
...
pr_lookup_procdir(vnode_t *dp, char *comp)
{
	...
	/*
	 * If a process vnode already exists and it is not invalid and
	 * it belongs to the same /proc mount point as our parent vnode,
	 * then just use it and discard the newly-allocated prnode.
	 */
...
pr_lookup_lwpdir(vnode_t *dp, char *comp)
{
	...
	/*
	 * If an lwp vnode already exists and it is not invalid and
	 * it belongs to the same /proc mount point as our parent vnode,
	 * then just use it and discard the newly-allocated prnode.
	 */
...
pr_lookup_fddir(vnode_t *dp, char *comp)
{
	...
	pufe = &up->u_flist[fd];
	if (fd < up->u_nofiles &&
	    (pufe->uf_pofile & (FRESERVED|FCLOSING)) == 0 &&
 ...
 */
vnode_t *
prlwpnode(prnode_t *pnp, u_int tid)
{
	char comp[12];
	vnode_t *dp;
	vnode_t *vp;
...
rebuild_objdir(struct as *as)
{
	...
	if (nold + nnew > as->a_sizedir) {
		 ...
		 */
		u_long newsize = (nold + nnew + 0xf) & ~0xf;
...
pr_readdir_lwpiddir(prnode_t *pnp, struct uio *uiop, int *eofp)
{
	...
	bcopy((caddr_t)lwpiddir, mylwpiddir, sizeof (mylwpiddir));
...
/* ARGSUSED */
static int
pr_readdir_fddir(prnode_t *pnp, struct uio *uiop, int *eofp)
{
	/* bp holds one dirent64 structure */
	longlong_t bp[DIRENT64_RECLEN(PLNSIZ) / sizeof (longlong_t)];
	struct dirent64 *dirent = (struct dirent64 *)bp;
	...
	for (; uiop->uio_resid > 0; uiop->uio_offset = off + PRSDSIZE) {
		if ((off = uiop->uio_offset) == 0) {	/* "." */
			...
			if ((fd = (int)((off-2*PRSDSIZE)/PRSDSIZE)) >=
				...
				break;
			pufe = &up->u_flist[fd];
			if ((pufe->uf_pofile & (FRESERVED|FCLOSING)) ||
...
/* ARGSUSED */
static int
prreadlink(vnode_t *vp, struct uio *uiop, struct cred *cr)
{
	prnode_t *pnp = VTOP(vp);

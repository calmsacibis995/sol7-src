...
#pragma ident	"@(#)prcontrol.c	1.18	98/02/06 SMI"
...
pr_control32(int32_t cmd, arg32_t *argp, prnode_t *pnp, cred_t *cr)
{
	...
	case PCRUN:	/* make lwp or process runnable */
		error = pr_setrun(pnp, (u_long)argp->flags);
		break;
...
int
pr_setrun(prnode_t *pnp, u_long flags)
{
	prcommon_t *pcp = pnp->pr_common;
	proc_t *p = pcp->prc_proc;
	kthread_t *t;
...
retry:
	...
	CL_FORK(t, ct, bufp);
	ct->t_cid = t->t_cid;
	ct->t_proc_flag |= (TP_PRSTOP|TP_HOLDLWP);
	/*
	...
	ct->t_sig_check = 1;
	p->p_agenttp = ct;
	lwp_continue(ct);	/* set running and turn off TP_HOLDLWP */
	/*
...
pr_rdwr(proc_t *p, enum uio_rw rw, priovec_t *pio)
{
	...
	if ((p->p_flag & SSYS) || p->p_as == &kas)
		...
		mutex_exit(&p->p_lock);
		error = prusrio(p->p_as, rw, &auio, 0);
		mutex_enter(&p->p_lock);
...
pr_scred(proc_t *p, prcred_t *prcred, cred_t *cr)
{
	...
	if ((u_int)prcred->pr_euid > MAXUID ||
	    (u_int)prcred->pr_ruid > MAXUID ||
	    (u_int)prcred->pr_suid > MAXUID ||
	    (u_int)prcred->pr_egid > MAXUID ||
	    (u_int)prcred->pr_rgid > MAXUID ||
	    (u_int)prcred->pr_sgid > MAXUID)
		return (EINVAL);

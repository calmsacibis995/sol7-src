...
#pragma ident	"@(#)prsubr.c	1.116	98/02/22 SMI"	/* from SVr4.0 1.44 */
...
prchoose(proc_t *p)
{
	...
	do {		/* for eacn lwp in the process */
		...
		default:
			cmn_err(CE_PANIC,
			    "prchoose: bad thread state %d, thread %p",
			    t->t_state, (void *)t);
...
prexecend()
{
	...
	if ((vp = curthread->t_trace) != NULL) {
		pcp = VTOP(vp)->pr_pcommon;
		VTOP(vp)->pr_common->prc_datamodel =
			pcp->prc_datamodel = p->p_model;
		mutex_enter(&pcp->prc_mutex);
		cv_broadcast(&pcp->prc_wait);
		mutex_exit(&pcp->prc_mutex);
...
prinvalidate(struct user *up)
{
	...
	if ((vp = p->p_trace) != NULL) {
		 ...
		 */
		writers += VTOP(vp)->pr_pcommon->prc_writers;
	}
	vp = t->t_trace;
	while (vp != NULL) {
		/*
...
#ifdef _SYSCALL32_IMPL
void
prgetaction32(proc_t *p, user_t *up, u_int sig, struct sigaction32 *sp)
{
	sp->sa_handler = (caddr32_t)SIG_DFL;
	premptyset(&sp->sa_mask);
	sp->sa_flags = 0;
 ...
 */
int
prpdread(proc_t *p, u_int hatid, struct uio *uiop)
{
	struct as *as = p->p_as;
	caddr_t buf;
	size_t size;
...
#ifdef _SYSCALL32_IMPL
int
prpdread32(proc_t *p, u_int hatid, struct uio *uiop)
{
	struct as *as = p->p_as;
	caddr_t buf;
	size_t size;
...
#ifdef _SYSCALL32_IMPL
void
prgetpsinfo32(proc_t *p, struct psinfo32 *psp)
{
	kthread_t *t;
	struct cred *cred;
	...
	psp->pr_pgid = p->p_pgrp;
	psp->pr_sid = p->p_sessp->s_sid;
	psp->pr_addr = (caddr32_t)prgetpsaddr(p);
	switch (p->p_model) {
	case DATAMODEL_ILP32:
		psp->pr_dmodel = PR_MODEL_ILP32;
		break;
...
#ifdef _SYSCALL32_IMPL
void
prgetlwpsinfo32(kthread_t *t, struct lwpsinfo32 *psp)
{
	klwp_t *lwp = ttolwp(t);
	sobj_ops_t *sobj;
	char c, state;
	...
	psp->pr_flag = t->t_flag;
	psp->pr_lwpid = t->t_tid;
	psp->pr_addr = (caddr32_t)t;
	psp->pr_wchan = (caddr32_t)t->t_wchan;
 ...
 * and copy the count fields uint64_t => u_long.
 */
void
prcvtusage(prhusage_t *pup, prusage_t *upup)
{
	...
	bzero(pup->filltime, sizeof (pup->filltime));
...
again:
	...
	if (!(p->p_flag & SVFWAIT)) {
		...
		if (pwp->wp_oprot == 0 &&
			...
			pwp->wp_oprot = (u_char)prot;
			pwp->wp_prot = (u_char)prot;
		}
		if (pwp->wp_oprot != 0) {
			prot = pwp->wp_oprot;
			if (pwp->wp_read)
 ...
 */
static void
clear_watched_page(proc_t *p, caddr_t vaddr, caddr_t eaddr, u_long flags)
{
	struct as *as = p->p_as;
	struct watched_page *pwp;
	int npage;
...
pr_pagev_create(struct seg *seg, int check_noreserve)
{
	...
	/*
	 * Limit the size of our vectors to 256K pages at a time.  We need
	 * 4 or 5 bytes of storage per page, so this means we limit ourself
	 * to about a megabyte of kernel heap.
	 */
	pagev->pg_npages = MIN(total_pages, 256 * 1024);
	pagev->pg_pnbase = 0;

	pagev->pg_protv =
	    kmem_alloc(pagev->pg_npages * sizeof (u_int), KM_SLEEP);
...
pr_pagev_destroy(prpagev_t *pagev)
{
	...
	kmem_free(pagev->pg_protv, pagev->pg_npages * sizeof (u_int));
	kmem_free(pagev, sizeof (prpagev_t));
}

static caddr_t
pr_pagev_fill(prpagev_t *pagev, struct seg *seg, caddr_t addr)
{
	caddr_t eaddr = seg->s_base + seg->s_size;
	u_long lastpg = seg_page(seg, eaddr - 1);
...
static caddr_t
pr_pagev_nextprot(prpagev_t *pagev, struct seg *seg, caddr_t addr,
    caddr_t *saddr, u_int *protp)
{
	caddr_t eaddr = seg->s_base + seg->s_size;
	u_long pn = seg_page(seg, addr);
	...
	if (pagev->pg_incore != NULL) {
		while (pagev->pg_incore[pn - pagev->pg_pnbase] == 0) {
			...
			if (++pn == pagev->pg_pnbase + pagev->pg_npages) {
				addr = pr_pagev_fill(pagev, seg, addr);
				if (addr == eaddr) {
					*saddr = addr;
					prot = 0;
					goto out;
	...
	*saddr = addr;
	...
	while ((addr += PAGESIZE) < eaddr) {
		 ...
		 * have information on, refill the page vector.
		 */
		if (++pn == pagev->pg_pnbase + pagev->pg_npages) {
			addr = pr_pagev_fill(pagev, seg, addr);
			if (addr == eaddr)
	...
	caddr_t *saddr, caddr_t *naddr)
{
	...
	if (!check_noreserve &&
		...
		if (addr == seg->s_base) {
			...
			*saddr = pr_pagev_fill(pagev, seg, addr);
			ASSERT(*tmp == NULL);
			*tmp = pagev;
		...
		addr = pr_pagev_nextprot(pagev, seg, addr, saddr, &prot);
	}
...
{
	return (vp->v_type == VPROC &&
	    VTOP(vp)->pr_type == PR_OBJECT);
}

/*
 * Return true iff the vnode is a /proc file referring to the current process.
 */
int
pr_isself(vnode_t *vp)
{
	return (vp->v_type == VPROC &&
	    VTOP(vp)->pr_pcommon->prc_proc == curproc);
}

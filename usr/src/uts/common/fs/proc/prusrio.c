...
#pragma ident	"@(#)prusrio.c	1.34	97/09/05 SMI"	/* from SVr4.0 1.12 */
...
int
prusrio(struct as *as, enum uio_rw rw, struct uio *uiop, int old)
{
	...
	switch (rw) {
	case UIO_READ:
		error = pr_uread(as, uiop, bp, old);
		break;
	case UIO_WRITE:
		error = pr_uwrite(as, uiop, bp);
		break;
	default:
		cmn_err(CE_PANIC,

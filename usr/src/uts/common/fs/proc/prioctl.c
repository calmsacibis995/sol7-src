...
#pragma ident	"@(#)prioctl.c	1.103	98/02/06 SMI"	/* SVr4.0 1.29	*/
...
static	u_long	prmaprunflags(long);
...
startover:
	...
	{
		u_int sig;
		struct sigaction *sap = (struct sigaction *)thing;
	...
	case PIOCGROUPS:	/* get supplementary groups */
	{
		...
		prunlock(pnp);
		if (copyout(&cp->cr_groups[0], cmaddr,
	...
	case PIOCLDT:		/* get LDT entries */
	{
		...
		bzero(&ssd[n], sizeof (struct ssd));
		if (copyout(ssd, cmaddr, (n+1) * sizeof (struct ssd)))
			error = EFAULT;
		kmem_free(ssd, (n+1) * sizeof (struct ssd));
		break;
	}
#endif	/* i386 */
#if sparc
	case PIOCGWIN:		/* get gwindows_t (see sys/reg.h) */
	{
		...
		mutex_exit(&p->p_lock);
		bzero(gwp, sizeof (gwindows_t));
		prgetwindows(lwp, gwp);
		mutex_enter(&p->p_lock);
		prunlock(pnp);
		if (copyout(gwp, cmaddr, sizeof (gwindows_t)))
			error = EFAULT;
...
void
oprgetstatus32(kthread_t *t, prstatus32_t *sp)
{
	proc_t *p = ttoproc(t);
	klwp_t *lwp = ttolwp(t);
	int32_t flags;
	...
	sp->pr_brkbase = (caddr32_t)p->p_brkbase;
	sp->pr_brksize = (size32_t)p->p_brksize;
	sp->pr_stkbase = (caddr32_t)prgetstackbase(p);
	...
	mutex_enter(&p->p_lock);
	/* special case for the parent of a vfork()d child */
	if (p->p_flag & SVFWAIT) {
...
oprgetpsinfo32(proc_t *p, prpsinfo32_t *psp, kthread_t *tp)
{
	...
	bzero(psp, sizeof (struct prpsinfo));
	...
	if (state == SZOMB || t == NULL) {
		...
		if ((t = tp) == NULL)
			t = p->p_tlist;
		do {
			clock_t ticks = lbolt - t->t_lbolt - 1;
...
startover:
	...
	case PIOCOPENM:		/* open mapped object for reading */
		if (p->p_model != DATAMODEL_ILP32 && cmaddr != NULL) {
			prunlock(pnp);
			error = EOVERFLOW;
			break;
		}
		error = propenm(pnp, cmaddr, (caddr_t)un32.va, rvalp, cr);
		/* propenm() called prunlock(pnp) */
		break;
	...
	case PIOCRUN:		/* make lwp or process runnable */
	{
		...
		if ((flags & PRSVADDR) && p->p_model != DATAMODEL_ILP32) {
			prunlock(pnp);
			error = EOVERFLOW;
			break;
		}
		...
		if (flags & (PRSHOLD|PRSTRACE|PRSFAULT|PRSVADDR)) {
			...
			un.prrun.pr_vaddr = (caddr_t)un32.prrun.pr_vaddr;
			prsetrun(t, &un.prrun);
		}
	...
	case PIOCGREG:		/* get general registers */
		...
		prunlock(pnp);
		if (copyout(un32.regs, cmaddr, sizeof (un32.regs)))
			error = EFAULT;
		break;
	...
	case PIOCGFPREG:	/* get floating-point registers */
		...
		prunlock(pnp);
		if (copyout(&un32.fpregs, cmaddr, sizeof (un32.fpregs)))
			error = EFAULT;
		break;
	...
	{
		u_int sig;
		struct sigaction32 *sap = (struct sigaction32 *)thing;
		...
		prunmap(p);
		prunlock(pnp);
		if (copyout(sap, cmaddr, (NSIG-1)*sizeof (struct sigaction32)))
			error = EFAULT;
	...
	case PIOCCRED:		/* get process credentials */
	{
		...
		mutex_enter(&p->p_crlock);
		cp = p->p_cred;
		un32.prcred.pr_euid = cp->cr_uid;
		un32.prcred.pr_ruid = cp->cr_ruid;
		un32.prcred.pr_suid = cp->cr_suid;
		un32.prcred.pr_egid = cp->cr_gid;
		un32.prcred.pr_rgid = cp->cr_rgid;
		un32.prcred.pr_sgid = cp->cr_sgid;
		un32.prcred.pr_ngroups = cp->cr_ngroups;
		mutex_exit(&p->p_crlock);
	...
	case PIOCGWIN:		/* get gwindows_t (see sys/reg.h) */
	{
		gwindows32_t *gwp = (gwindows32_t *)thing;
 ...
 */
void
oprgetstatus(kthread_t *t, prstatus_t *sp)
{
	proc_t *p = ttoproc(t);
	klwp_t *lwp = ttolwp(t);
	int flags;
 ...
 */
int
oprpdread(struct as *as, u_int hatid, struct uio *uiop)
{
	caddr_t buf;
	size_t size;
	prpageheader_t *php;
...
#ifdef _SYSCALL32_IMPL
int
oprpdread32(struct as *as, u_int hatid, struct uio *uiop)
{
	caddr_t buf;
	size_t size;

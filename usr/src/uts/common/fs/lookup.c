...
#ident	"@(#)lookup.c	1.36	98/01/23 SMI"	/* SVr4 1.18	*/
...
next:
	...
	if (rpnp && (vnode_match(vp, rootvp) || vnode_match(vp, rootdir)))
		(void) pn_set(rpnp, "/");
...
checkforroot:
		if (vnode_match(vp, rootvp) || vnode_match(vp, rootdir)) {
			component[1] = '\0';
	...
	if (error) {
		...
		if (audit_active) {	/* directory access */
			if (error = audit_savepath(pnp, vp, /* error */ 0, cr))
				goto bad_noaudit;
		}
#endif
		pn_setlast(pnp);
	...
	if (pn_pathleft(pnp) == 0) {
		...
		pn_setlast(pnp);
		if (rpnp) {
			if (vnode_match(cvp, rootvp) ||
			    vnode_match(cvp, rootdir))
				(void) pn_set(rpnp, "/");

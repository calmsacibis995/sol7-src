...
#pragma ident	"@(#)pc_vnops.c	1.60	97/11/07 SMI"
...
static int pcfs_getpage(struct vnode *, offset_t, size_t, u_int *, page_t *[],
	size_t, struct seg *, caddr_t, enum seg_rw, struct cred *);
static int pcfs_getapage(struct vnode *, u_offset_t, size_t, u_int *,
...
static int pcfs_addmap(struct vnode *, offset_t, struct as *, caddr_t,
	size_t, u_char, u_char, uint, struct cred *);
	...
	int ioflag)
{
	...
	do {
		 ...
		 */
		if (newpage)
			segmap_pageunlock(segkmap, base, (u_int)n,
...
reread:
	if ((pagefound = page_exists(vp, off)) == 0) {
		...
		xferoffset = (u_int)off & ~(fsp->pcfs_secsize -1);

		pp = pvn_read_kluster(vp, off, seg, addr, &io_off, &io_len,
		    off, MIN(pc_blksize(fsp, pcp, (u_int)off), PAGESIZE), 0);
		if (pp == NULL)
			/*
...
set_long_fn_chunk(struct pcdir_lfn *ep, char *buf, int len)
{
	...
	for (i = 0; i < PCLF_FIRSTNAMESIZE; i += 2) {
		if (len > 0) {
			...
			ep->pcdl_firstfilename[i] = (u_char)0xff;
			ep->pcdl_firstfilename[i+1] = (u_char)0xff;
		}
	}
	...
	for (i = 0; i < PCLF_SECONDNAMESIZE; i += 2) {
		if (len > 0) {
			...
			ep->pcdl_secondfilename[i] = (u_char)0xff;
			ep->pcdl_secondfilename[i+1] = (u_char)0xff;
		}
	}
	for (i = 0; i < PCLF_THIRDNAMESIZE; i += 2) {
		if (len > 0) {
			...
			ep->pcdl_thirdfilename[i] = (u_char)0xff;
			ep->pcdl_thirdfilename[i+1] = (u_char)0xff;
		}
	}
}
...
pcfs_fid(struct vnode *vp, struct fid *fidp)
{
	...
	if (fidp->fid_len < (sizeof (struct pc_fid) - sizeof (ushort))) {
		fidp->fid_len = sizeof (struct pc_fid) - sizeof (ushort);
		pc_unlockfs(fsp);
		return (ENOSPC);
	}
	...
	pcfid->pcfid_len = sizeof (struct pc_fid) - sizeof (ushort);
	if (vp->v_flag & VROOT) {
		pcfid->pcfid_block = 0;

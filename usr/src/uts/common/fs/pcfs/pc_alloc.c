/*
 * Copyright (c) 1989, 1992-1994, 1997 by Sun Microsystems, Inc.
 * All rights reserved.
 */

#pragma ident	"@(#)pc_alloc.c	1.19	97/10/21 SMI"
...
pc_bfree(struct pcnode *pcp, pc_cluster32_t skipcl)
{
	...
	if (skipcl == 0) {
		if (IS_FAT32(fsp))
			pcp->pc_scluster = PCF_LASTCLUSTER32;
		else
	...
	int zwrite)			/* boolean for writing zeroes */
{
	...
	for (cn = fsp->pcfs_nxfrecls;
		...
		if (pc_getcluster(fsp, cn) == PCF_FREECLUSTER) {
			...
			if (IS_FAT32(fsp)) {
				pc_setcluster(fsp, cn, PCF_LASTCLUSTER32);
				if (fsp->fsinfo_native.fs_free_clusters !=
				    FSINFO_UNKNOWN)
					fsp->fsinfo_native.fs_free_clusters--;
			} else

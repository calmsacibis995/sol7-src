/*
 * Copyright (c) 1989, 1992, 1996-1997 by Sun Microsystems, Inc.
 * All rights reserved.
 */

#pragma ident	"@(#)pc_node.c	1.31	97/11/07 SMI"
 ...
 */
void
pc_gldiskchanged(struct pcfs *fsp)
{
		...
		for (pcp = hp->pch_forw;
			...
			if (VFSTOPCFS(PCTOV(pcp)->v_vfsp) == fsp &&
				...
				if (!(pcp->pc_flags & PC_EXTERNAL)) {
					mutex_destroy(&(PCTOV(pcp)->v_lock));
					kmem_free(PCTOV(pcp),
					    sizeof (struct vnode));
				}
				kmem_free(pcp, sizeof (struct pcnode));

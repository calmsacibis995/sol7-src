/*
 * Copyright (c) 1989, 1992, 1997 by Sun Microsystems, Inc.
 * All rights reserved.
 */

#pragma ident	"@(#)pc_dir.c	1.25	97/10/13 SMI"
...
static int pc_makedirentry(struct pcnode *dp, struct pcdir *direntries,
    int	ndirentries, struct vattr *vap, int offset);
static int pc_dirempty(struct pcnode *);
static int pc_findentry(struct pcnode *, char *, struct slot *, int *);
...
static int pc_find_free_space(struct pcnode *pcp, int ndirentries);
	...
	char *tnm)			/* target file name */
{
	 ...
	 */
	if (strcasecmp(snm, tnm) == 0) {
		goto out;
	}
	/*
	 * see if the target exists
	 */
	error = pc_findentry(dp, tnm, &slot, NULL);
	if (error == 0) {
		/*
		 * Target exists.
		 */
		 ...
		 */
		ndirentries = direntries_needed(dp, tnm);
		if (ndirentries == -1) {
			VN_RELE(PCTOV(pcp));
			return (EINVAL);
		}
		 ...
		 * Make an entry from the supplied attributes.
		 */
		direntries = pc_name_to_pcdir(dp, tnm, ndirentries, &error);
		if (direntries == NULL) {
			VN_RELE(PCTOV(pcp));
			return (error);
		}
		error = pc_makedirentry(dp, direntries, ndirentries, NULL,
		    offset);
		kmem_free(direntries, ndirentries * sizeof (struct pcdir));
		if (error) {
			VN_RELE(PCTOV(pcp));
 ...
 */
static int
pc_remove_long_fn(struct pcnode *pcp, int lfn_offset)
{
	struct vnode *vp = PCTOV(pcp);
	struct pcfs *fsp = VFSTOPCFS(vp->v_vfsp);
	int	boff;
...
{
	int	offset = 0;
	int	spaceneeded = ndirentries * sizeof (struct pcdir);
	int	spaceoffset;
	int	spaceavail = 0;
	int	boff;
	struct buf *bp = NULL;
...
pc_name_to_pcdir(struct pcnode *dp, char *namep, int ndirentries, int *errret)
{
	...
	for (i = 0; i < (ndirentries - 1); i++) {
		...
		lep->pcdl_ordinal = (u_char)(ndirentries - i - 1);
		nchars = PCLFNCHUNKSIZE;
	}

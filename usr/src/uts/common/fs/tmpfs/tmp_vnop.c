...
#pragma ident	"@(#)tmp_vnops.c	1.125	98/02/04 SMI"
	...
	struct cred *cr)
{
	...
	if (err) {
		pvn_write_done(pplist, B_ERROR | B_WRITE | (flags & ~B_FORCE));
		/*
		 * If this is routine is called as a result of segvn_sync
		 * operation and we have no physical swap then we can get an
		 * error here. In such case we would return SE_NOSWAP as error.
		 */
		if (!((flags & B_FORCE) && (flags & B_INVAL) == 0 &&
			err == SE_NOSWAP))
			err = ENOMEM;
		goto out;
	}
	...
	if (io_off + io_len < pp_off + pp_len) {
		...
		pvn_write_done(npplist, B_ERROR | B_WRITE | (flags & ~B_FORCE));
	}
	...
	if ((flags & B_ASYNC) == 0) {
		if (err)
			pvn_write_done(pplist, B_ERROR | B_WRITE |
			    (flags & ~B_FORCE));
		else
			pvn_write_done(pplist, B_WRITE | flags);
	}
out:
	if (!err) {
		if (offp)

...
#pragma ident	"@(#)tmp_vfsops.c	1.50	97/09/23 SMI"
...
tmpfsinit(struct vfssw *vswp, int fstype)
{
	...
	if (tmpfs_maxkmem == 0)
		tmpfs_maxkmem = MAX(PAGESIZE,
		    (long)((kmem_maxavail() * TMPMAXPROCKMEM) / 100));
	...
	if (datalen != 0) {
		if (get_udatamodel() == DATAMODEL_NATIVE) {
			if (datalen != sizeof (targs))
				...
				if (copyin(data, &targs, sizeof (targs)))
					return (EFAULT);
			}
		} else {
			struct tmpfs_args32 targs32;

			if (datalen != sizeof (targs32))
				return (EINVAL);
			else {
				if (copyin(data, &targs32, sizeof (targs32)))
					return (EFAULT);
				targs.anonmax = targs32.anonmax;
				targs.flags = targs32.flags;
			}
		}
	} else {
		targs.anonmax = 0;

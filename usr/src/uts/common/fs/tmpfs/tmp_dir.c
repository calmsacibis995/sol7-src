...
#pragma ident  "@(#)tmp_dir.c 1.41    97/08/12 SMI"
...
static struct tmpnode *tdirmaketnode(struct tmpnode *, struct tmount *,
	...
	struct cred	*cred)
{
	...
	if (tdp) {
		...
		if (op == DE_CREATE || op == DE_MKDIR) {
			 ...
			 */
			tp = tdirmaketnode(dir, tm, va, op, cred);
		}
	...
	struct cred *cred)
{
	...
	if (nm[0] == '\0')
		panic("tdirdelete: NULL name for %p", tp);
	...
	struct tmpnode *dir)		/* the new directory */
{
	...
	dir->tn_dirents = 2;
	dir->tn_nlink = 2;	/* one for daddy, and one just for being me */

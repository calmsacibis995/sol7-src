 ...
 * publication.
 *
 * 	(c) 1986,1987,1988,1989,1996-1998  Sun Microsystems, Inc
 * 	(c) 1983,1984,1985,1986,1987,1988,1989  AT&T.
 *		All rights reserved.
 *
 */

#ident	"@(#)quotacalls.c	1.61	98/02/06 SMI"
...
opendq_scan_inode(struct inode *ip, void *arg)
{
	 ...
	 * and is not a shadow inode, but does not have a quota so get
	...
	struct cred *cr)
{
	...
	error = lookupname(addr, UIO_USERSPACE, FOLLOW, (struct vnode **)NULL,
			&vp);
	if (error)
		return (error);
	/*
	 * Either the wrong file system or not a regular file.
	 */
...
"Previous quota file still in use. Disable quotas on %s before enabling.\n",
...
#ifdef DEBUG
	/*
	 * If we have vfs_dqrwlock as writer and quotas are disabled,
	 * then vfs_qinod should be NULL or we have a race somewhere.
	 */
	else {
		ASSERT(ufsvfsp->vfs_qinod == 0);
	}
#endif /* DEBUG */
	...
	/*
	 * Quotas are enabled so update all valid inodes in the
	 * cache with quota information.
	 */
	if (ufsvfsp->vfs_qflags & MQ_ENABLED) {
		(void) ufs_scan_inodes(0, opendq_scan_inode, ufsvfsp);
	}
...
setquota_scan_inode(struct inode *ip, void *arg)
{
	...
	/*
	 * This inode is in the cache (by definition), is still valid,
	 * is not a shadow inode and has the right uid.
	 */
	if (ip->i_mode && (ip->i_mode & IFMT) != IFSHAD &&
	    ip->i_uid == sqdp->sqd_uid) {
		/*
    ...
    caddr_t addr, struct cred *cr)
{
	...
	if (scan_type) {
		...
		sqd.sqd_uid = uid;
		(void) ufs_scan_inodes(0, setquota_scan_inode, &sqd);
	}
	rw_exit(&ufsvfsp->vfs_dqrwlock);
	return (0);
}
...
quotasync(struct ufsvfs *ufsvfsp, int do_lock)
{
	...
	if (ufsvfsp) {
		/*
		 * Need to make the quota subsystem non-quiescent.
		 */
		if (do_lock) {
			rw_enter(&ufsvfsp->vfs_dqrwlock, RW_READER);
		}

...
#pragma ident	"@(#)ufs_panic.c	1.25	98/01/23 SMI"
...
#if !defined(RPCBIND_SERVNAME) || !defined(RPCBIND_SERVNAMELEN)
#define	RPCBIND_SERVNAME	"rpc"	/* see /etc/net/.../services */
#define	RPCBIND_SERVNAMELEN	3	/* convenient */
#endif
...
const time_t	UF_TOO_SOON		= 32;	/* min. time between failures */
...
const u_long	GB			= 1024 * 1024 * 1024;
const u_long	SecondsPerGig		= 1024;	/* ~17 minutes (overestimate) */

/*
 * per filesystem flags
 */
const int	UFSFX_REPAIR		= (UFSMNT_ONERROR_REPAIR >> 4);
...
const int	UFSFX_READONLY		= (UFSMNT_ONERROR_RDONLY >> 4);
...
struct ufs_failure_tunable {
	time_t	 uft_too_soon;		/* limit panic interval */
...
#define	UAFLG_SKIP		0x00000001	/* for debug and test (only) */
#define	UAFLG_CONNECTED		0x00000002
#define	UAFLG_ERROR		0x00000004
#define	UAFLG_UFSD_NOEXIT	0x10000000
#define	TIMEOUT			{1, 0}

/*
 * most of this data is static.  The mutex protects the changable items:
 *	ua_kn.knc_rdev, ua_flags, ua_addr, ua_cl, ua_symaddr, ua_counter
 */
static struct ufsd_addr {
	struct knetconfig	 ua_kn;
	char			*ua_devname;
	char			*ua_netid;
	struct netbuf		 ua_addr;
	u_int			 ua_flags;
	CLIENT			*ua_cl;
				/* +1+1 = dot (".") and NULL terminator */
	char			 ua_symaddr[SYS_NMLN+RPCBIND_SERVNAMELEN+1+1];
	uint32_t		 ua_addr_prefix;
	int			 ua_counter;
	kmutex_t		 ua_mutex;
} ufsd_addr_tab[] =
/*
...
{	{ NC_TPI_COTS_ORD, NC_LOOPBACK,	NC_NOPROTO,	NODEV },
	/* devname	  netid		netbuf 		flags */
	"/dev/ticotsord", "ticotsord",	{0, 0, NULL},	UAFLG_NONE,
	/* CLIENT *	address name,	address prefix,	counter */
	NULL,		NULSTRING,	0 /* unused */,	0
},
...
{	{ NC_TPI_COTS,	NC_LOOPBACK,	NC_NOPROTO,	NODEV },
	/* devname	netid		netbuf		flags */
	"/dev/ticots",	"ticots",	{0, 0, NULL},	UAFLG_NONE,
	/* CLIENT *,	address name,	address prefix,	counter */
	NULL,		NULSTRING,	0 /* unused */,	0
},
...
{	{ NC_TPI_CLTS,	NC_LOOPBACK,	NC_NOPROTO,	NODEV },
	/* devname	netid		netbuf		flags */
	"/dev/ticlts",	"ticlts",	{0, 0, NULL},	UAFLG_NONE,
	/* CLIENT *,	address name,	address prefix,	counter */
	NULL,		NULSTRING,	0 /* unused */,	0
},
...
{	{ NC_TPI_COTS_ORD, NC_INET,	NC_TCP,		NODEV },
	/* devname	netid		netbuf		flags */
	"/dev/tcp",	"tcp-loopback",	{0, 0, NULL},	UAFLG_NONE,
	/* CLIENT *,	address name,	address prefix,		counter */
	NULL,		NULSTRING,	INADDR_LOOPBACK,	0
},
...
{	{ NC_TPI_COTS_ORD, NC_INET,	NC_TCP,		NODEV },
	/* devname	netid	netbuf		flags */
	"/dev/tcp",	"tcp",	{0, 0, NULL},	UAFLG_NONE,
	/* CLIENT *,	address name,	net address prefix,	counter */
	NULL,		NULSTRING,	INADDR_ANY,		0
},
...
{	{ NC_TPI_CLTS,	NC_INET,	NC_UDP,		NODEV },
	/* devname	netid			netbuf		flags */
	"/dev/udp",	"udp-loopback", 	{0, 0, NULL},	UAFLG_NONE,
	/* CLIENT *	address name,	net address prefix,	counter */
	NULL,		NULSTRING,	INADDR_LOOPBACK,	0
},
...
/* UDP */
	/* semantics	protofmly	proto,		dev_t */
{	{ NC_TPI_CLTS,	NC_INET,	NC_UDP,		NODEV },
	/* devname	netid	netbuf		flags		CLIENT * */
	"/dev/udp",	"udp", 	{0, 0, NULL},	UAFLG_NONE,	NULL,
	/* address name,	net address prefix,	counter */
	NULSTRING,		INADDR_ANY,		0
},

/* table terminator (NULL ua_devname is the terminator key.) */
{ { 0, NULL, NULL, NODEV },
	NULL, NULL, {0, 0, NULL}, UAFLG_NONE, NULL, NULSTRING, 0, 0 }
};
...
/*PRINTFLIKE2*/
static void		 real_panic(ufs_failure_t *, char *, ...);
static void		 real_panic_v(ufs_failure_t *, char *, va_list);
...
static sfrc_t	sf_set_locked(ufs_failure_t *, ufs_failure_states_t);
...
static void	trigger_fix(ufs_failure_t *);
...
static int	ufsd_connect(struct ufsd_addr *);
static int 	create_ufsd_client(struct ufsd_addr *);
static void	destroy_ufsd_clients(void);

/*
 * change UFSD_ORIGVERS in the following line to the latest
 * verified version of the UFSD protocol
 */
#if (UFSD_THISVERS != UFSD_ORIGVERS)
#error	The ufsd RPC protocol has been changed. \
	The kernel xdr routines may need to be recoded.
#else
static bool_t	xdr_repairfs_args(XDR *, ufsd_repairfs_args_t *);
static bool_t	xdr_ufsdrc(XDR *, ufsdrc_t *);
#endif /* UFSD_VERS != UFSD_ORIGVERS */
 ...
 * State Transitions:
 *
 * normally:					(UFSMNT_ONERROR_REPAIR) or
...
struct ufsfx_info {
	...
	struct ufsd_addr		*ufi_ufsd_addr;
	ufsd_t				*ufi_statetab;
} uffsinfo;
...
ufs_fault_v(vnode_t *vp, char *fmt, va_list adx)
{
	...
	case TRIAGE_ATTEND_TO:
		...
		if ((new = init_failure(vp, fmt, adx)) != NULL) {
			queue_failure(new);
			break;
		}
		real_panic_v(new, fmt, adx);
...
triage(vnode_t *vp)
{
	...
	if (fs_flags & UFSFX_REPAIR) {
		/*
		 * Verify that we've got at least one shot at connecting
		 * to the user-level daemon, but prime the connection for
		 * all of them.
		 */
		connected = 0;
		for (uap = &ufsd_addr_tab[0]; uap->ua_devname; uap++) {

			if (uap->ua_flags & UAFLG_SKIP) {
				MINUTE((": %s:%s - marked \"skip\"\n",
						uap->ua_netid,
						NULLSTR(uap->ua_symaddr)));
				continue;
			}

			ASSERT(MUTEX_NOT_HELD(&uap->ua_mutex));
			mutex_enter(&uap->ua_mutex);
			err = ufsd_connect(uap);
			if (err != 0 || !uap->ua_cl) {
		...
		if (!connected) {
			MINUTE((": can't connect to %s => TRIAGE_DEAD]\n",
								UFSD_SERVNAME));
			return (TRIAGE_DEAD);
		}
	}
...
init_failure(vnode_t *vp, char *fmt, va_list adx)
{
	...
	rp = vsprintf_len(LOCKFS_MAXCOMMENTLEN-1, new->uf_panic_str, fmt, adx);
	if (rp != &new->uf_panic_str[0])
		goto errout;
...
/*PRINTFLIKE2*/
void
real_panic(ufs_failure_t *f, char *fmt, ...)
{
	va_list	adx;
...
void
real_panic_v(ufs_failure_t *f, char *fmt, va_list adx)
{
...
ufsfx_init(void)
{
	...
	ufsfx_tune.uft_too_soon		= UF_TOO_SOON;
	...
	uffsinfo.ufi_ufsd_addr	= &ufsd_addr_tab[0];
	uffsinfo.ufi_statetab	= &state_desc[0];

	for (uap = &ufsd_addr_tab[0]; uap->ua_devname; uap++) {
		uap->ua_addr.buf	= NULL;
		uap->ua_addr.len	= 0;
		uap->ua_addr.maxlen	= 0;
 ...
 */
int
ufsfx_mount(struct ufsvfs *ufsvfsp, int flags, long toosoon)
{
	MINUTE(("[ufsfx_mount (%d, %ld)", flags, toosoon));
	/* don't check/need vfs_lock because it's still being initialized */

	ufsvfsp->vfs_fsfx.fx_count = 0;
	ufsvfsp->vfs_fsfx.fx_prev_start_tm = 0;

	ufsvfsp->vfs_fsfx.fx_too_soon = toosoon? toosoon:
						ufsfx_tune.uft_too_soon;
	...
	/*
	 *	onerror={panic ^ lock only ^ unmount ^ read only ^ repair}
	 */
	...
	} else if (ufsvfsp->vfs_fsfx.fx_flags & UFSFX_REPAIR) {
		MINUTE((" REPAIR"));

	} else if (ufsvfsp->vfs_fsfx.fx_flags & UFSFX_READONLY) {
		MINUTE((" READONLY (EINVAL)]\n"));
		return (EINVAL);	/* XXX */
	} else {
...
sf_found_queue(ufs_failure_t *f)
{
	...
	/* have we exceeded time limit for this fs? */
	if (f->uf_vfs_ufsfxp->fx_prev_start_tm != 0 &&
	    (time <= f->uf_vfs_ufsfxp->fx_prev_start_tm +
					f->uf_vfs_ufsfxp->fx_too_soon)) {

		cmn_err(CE_WARN, "too soon ufs_failure again on %s",
								fs_name(f));
		if (!set_state(f, UF_PANIC)) {
			real_panic(f, NULL, NULL);
			/* NOTREACHED */
	...
	/*
	 * set system and this failure start time here so that
	 * they are identical, although we could set the system
	 * start time earlier (at set_state(UF_QUEUE))
	 */
	mutex_enter(&uf_stats.ufst_mutex);
	if (!replica && uf_stats.ufst_last_start_tm != 0 &&
	    (time <= uf_stats.ufst_last_start_tm +
					ufsfx_tune.uft_too_soon)) {

		cmn_err(CE_WARN, "too soon ufs_failure on %s (fs: %s)",
						utsname.nodename, fs_name(f));
	...
	uf_stats.ufst_last_start_tm = time;
	f->uf_vfs_ufsfxp->fx_prev_start_tm = uf_stats.ufst_last_start_tm;
	mutex_exit(&uf_stats.ufst_mutex);
...
static sfrc_t
sf_set_locked(ufs_failure_t *f, ufs_failure_states_t s)
{
	TRIVIA(("[sf_set_locked"));

	if (f->uf_flags & UFSFX_REPAIR)
		f->uf_retry = ufsfx_tune.uft_fixstart_period;
	else
		f->uf_retry = -ufsfx_tune.uft_fixstart_period;
	...
	/*
	 * don't trigger repair if just resetting to current state
	 */
	if (!(s & f->uf_s) && (f->uf_flags & UFSFX_REPAIR))
		trigger_fix(f);
#if defined(DEBUG)
	else {
		TRIVIA(("locked->locked || UFSFX_REPAIR set "));
	}
#endif /* DEBUG */
	...
	if (s & UF_LOCKED) {
		...
		if (!toolong) {
			...
			if (!(f->uf_flags & UFSFX_REPAIR_START)) {
				...
				if (f->uf_flags & UFSFX_REPAIR) {
					cmn_err(CE_WARN,
				    "Assuming deadlock and forcing shutdown.");
					...
					f->uf_retry =
						ufsfx_tune.uft_long_err_period;
				}
			} else {
...
static void
trigger_fix(ufs_failure_t *f)
{
	enum clnt_stat			 stat;
	ufsd_repairfs_args_t		 repairfs_args;
	int				 err;
	int				 connected = 0;
	struct ufsd_addr		*uap;
	const struct timeval		 to = TIMEOUT;
	struct rpc_err			 re;
	...
	/*
	 * possible mechanisms:
	 *   1) RPC call (to inetd to start) ufsd/fsck (PSARC preference)
	 *	(most complicated protocol, but provides general features
	 *	and an extensible mechanism for adding UFS-aware services
	 *	outside of the kernel)
	 *   2) raise a condition for which ufs_poll() would check
	 *	(standard-compliant protocol, but requires a pre-existing
	 *	user-level daemon)
	 *   3) do nothing, and have user-level timeout protocol
	 *	(simplest protocol, but would need lots of rules
	 *	to handle boundary conditions for extreme-sized file
	 *	systems, large and small)
	 */
	f->uf_counter++;
	for (uap = &ufsd_addr_tab[0]; uap->ua_devname && !connected; uap++) {
		if (uap->ua_flags & UAFLG_SKIP) {
			MINUTE((": %s:%s - marked \"skip\"\n",
				uap->ua_netid,
				NULLSTR(uap->ua_symaddr)));
		...
		if (!(uap->ua_flags & UAFLG_ERROR)) {
			...
			repairfs_args.ua_fsid.fs_dev	= f->uf_dev;
			repairfs_args.ua_fsid.fs_name	= f->uf_fsname;
			repairfs_args.ua_attempts	= f->uf_counter;

			TRIVIA((": trying UFSD_REPAIRFS(%s:%s)\n",
					uap->ua_netid, uap->ua_symaddr));

			err = UFSDRC_OK;

			ra.prog		= UFSD_PROG;
			ra.vers		= UFSD_VERS;
			ra.proc		= UFSD_REPAIRFS;
			ra.arglen	= sizeof (ufsd_repairfs_args_t);
			ra.args_ptr	= (caddr_t)&repairfs_args;
			ra.xdr_args	= xdr_repairfs_args;

			res.addr_ptr	= addr;	/* XXX any use for this? */
			res.resultslen	= sizeof (err);
			res.results_ptr	= (caddr_t)&err;
			res.xdr_results	= xdr_ufsdrc;

			stat = CLNT_CALL(uap->ua_cl, RPCBPROC_INDIRECT,
					xdr_rpcb_rmtcallargs, (caddr_t)&ra,
					xdr_rpcb_rmtcallres, (caddr_t)&res,
					to);

			CLNT_GETERR(uap->ua_cl, &re);

			if ((stat == RPC_SUCCESS &&
			    (err == UFSDRC_OK || err == UFSDRC_BUSY)) ||
			    (stat == RPC_TIMEDOUT && err == UFSDRC_OK)) {
				/* BUSY means that a fsck is already active */
				connected = 1;
				MINUTE((
				    ": worked (%s:%s), stat: %d, err:%d %s\n",
				    uap->ua_netid, uap->ua_symaddr,
				    stat, err, err_name(err)));
			} else {
				MINUTE((
": UFSD_REPAIRFS(%s:%s):%d, err:%d (%s), re_err:%d (%s), stat:%d count:%d]\n",
				    uap->ua_netid, uap->ua_symaddr,
				    stat, err, err_name(err),
				    re.re_errno, err_name(re.re_errno),
				    re.re_status, f->uf_counter));

				/* two tries, timeouts don't count */
				if ((uap->ua_counter % 2) == 0 &&
				    (re.re_status != RPC_TIMEDOUT ||
				    re.re_errno != ETIMEDOUT)) {
					uap->ua_flags |= UAFLG_ERROR;
				}
			}
		}
		mutex_exit(&uap->ua_mutex);
	...
	/*
	 * reduce retry interval gradually
	 * don't check err or stat because trigger_fix()
	 * shouldn't be called once repair is started
	 */
	if (!connected) {
		if ((f->uf_counter % 8) == 0) {
			if (f->uf_retry != 1)
		...
		cmn_err(CE_WARN, "%s not responding", UFSD_SERVNAME);
	}
	MINOR(("] "));
}

static bool_t
xdr_repairfs_args(XDR *xdrs, ufsd_repairfs_args_t *objp)
{
	return (xdr_u_int(xdrs, &objp->ua_fsid.fs_dev)			&&
		xdr_string(xdrs, &objp->ua_fsid.fs_name, MAXMNTLEN)	&&
		xdr_u_int(xdrs, &objp->ua_attempts));
}

static bool_t
xdr_ufsdrc(XDR *xdrs, ufsdrc_t *objp)
{
...
dump_uf(ufs_failure_t *f, int i)
{
	...
	printf("\tFlags: (%d) %s%s%s%s%s", f->uf_flags,
		f->uf_flags & UFSFX_REPAIR?	 "\"autorepair\" "	: "",
...
ufsfx_kill_fix_failure_thread(void *arg)
{
	...
	if (qlen < 0) {
		...
		for (uap = &ufsd_addr_tab[0];
		    uap->ua_devname && !exitted;
		    uap++) {
			if (uap->ua_flags & UAFLG_SKIP) {
				MINUTE((
				    ": %s:%s - marked \"skip\"\n",
			...
			ASSERT(MUTEX_NOT_HELD(&uap->ua_mutex));
			mutex_enter(&uap->ua_mutex);
			if (!(uap->ua_flags | UAFLG_UFSD_NOEXIT)) {
				int			err;
				enum clnt_stat		stat;
				...
				/*
				 * UFSDRC_OPNOTSUP implies standalone
				 * so server refuses to exit
				 */
				if (stat == RPC_SUCCESS) {
					switch (err) {
					case UFSDRC_OPNOTSUP:
						uap->ua_flags |=
							UAFLG_UFSD_NOEXIT;
						/* FALLTHROUGH */
...
static int
ufsd_connect(struct ufsd_addr *uap)
{
	enum clnt_stat		 stat;
	int			 err;
	vnode_t			*kvp;
	...
	ASSERT(MUTEX_HELD(&uap->ua_mutex));

	if (!(uap->ua_flags & UAFLG_CONNECTED)) {
		if (uap->ua_kn.knc_rdev == NODEV) {
			if ((err = lookupname(uap->ua_devname,
					UIO_SYSSPACE, FOLLOW, NULLVPP,
					&kvp)) != 0) {
				MINUTE((": lookupname(%s) failed, err: %d\n",
					uap->ua_devname, err));
				return (err);
			}
			uap->ua_kn.knc_rdev = kvp->v_rdev;
			VN_RELE(kvp);
		}

		/* build daemon address */
		if (!uap->ua_kn.knc_protofmly) {
			MINUTE((": bad protofmly]"));
			return (EINVAL);

		} else if (strcmp(uap->ua_kn.knc_protofmly, NC_LOOPBACK) == 0) {

			(void) strcpy(uap->ua_symaddr, utsname.nodename);
			(void) strcat(uap->ua_symaddr, ".");
			(void) strcat(uap->ua_symaddr, RPCBIND_SERVNAME);

			uap->ua_addr.buf = uap->ua_symaddr;
			uap->ua_addr.maxlen = (unsigned)strlen(uap->ua_symaddr);
		}
			...
			s = kmem_alloc(sizeof (struct sockaddr_in), KM_NOSLEEP);
			if (!s) {
				MINUTE((": no memory to allocate socket\n"));
				return (ENOMEM);
			}
			port = htons(PMAPPORT);
			prefix = htonl(uap->ua_addr_prefix);

			s->sin_port		= port;
			s->sin_addr.s_addr	= prefix;
			s->sin_family		= AF_INET;
			uap->ua_addr.buf	= (caddr_t)s;
			uap->ua_addr.maxlen	= sizeof (struct sockaddr_in);
			(void) sprintf(uap->ua_symaddr, "%d.%d.%d.%d.%d.%d",
					(int)((prefix >> 24) & 0xff),
					(int)((prefix >> 16) & 0xff),
					(int)((prefix >> 8) & 0xff),
					(int)(prefix & 0xff),
		...
		uap->ua_addr.len = uap->ua_addr.maxlen;
		bzero(&uap->ua_kn.knc_unused, sizeof (uap->ua_kn.knc_unused));

		if ((err = create_ufsd_client(uap)) != 0) {
			MINUTE((": %s client_create(%s:%s) failed %s (%d)]\n",
				uap->ua_netid, uap->ua_symaddr,
				UFSD_SERVNAME, err_name(err), err));
			return (err);
		}
	}
	...
	if (stat != RPC_SUCCESS) {
		...
		err = (int)stat;
		MINUTE((
		": UFSD_NULL(%s:%s) failed:%s (%d), re_err:%d %s, re_stat:%d\n",
			uap->ua_netid, uap->ua_symaddr,
			err_name((int)stat), (int)stat,
			re.re_errno, err_name(re.re_errno), re.re_status));
	} else if (err != UFSDRC_OK) {
		MINUTE((": %s returned err: %d %s\n", UFSD_SERVNAME,
						err, err_name(err)));
		err = (int)RPC_SYSTEMERROR;
	} else {
		uap->ua_flags |= UAFLG_CONNECTED;
		MINUTE((": %s:%s worked\n", uap->ua_netid, uap->ua_symaddr));
	}

	if (stat != RPC_SUCCESS || err != UFSDRC_OK)
		MINOR(("%s connection error (%s:%s)",
			UFSD_SERVNAME, uap->ua_netid, uap->ua_symaddr));
	uap->ua_counter++;
	MINUTE(("]\n"));
	return (err);
}
...
static int
create_ufsd_client(struct ufsd_addr *uap)
{
	int		rc = 0;
	struct rpc_err	re;
	struct timeval	to = TIMEOUT;

	TRIVIA(("[create_ufsd_client"));
	...
	if (!uap->ua_cl) {
		/*
		 * RPCBVERS (= 3) doesn't provide RPCBPROC_INDIRECT()
		 */
		rc = clnt_tli_kcreate(&uap->ua_kn, &uap->ua_addr,
					    RPCBPROG, RPCBVERS4,
					    sizeof (ufs_failure_t), 1, kcred,
					    &uap->ua_cl);
		if (rc) {
			TRIVIA((": clnt_tli_kcreate(%s:%s) failed(%s) rc:%d",
				uap->ua_netid, uap->ua_symaddr,
				err_name(rc), rc));
			goto out;
		}
	} else {
		rc = clnt_tli_kinit(uap->ua_cl, &uap->ua_kn,
			&uap->ua_addr, sizeof (ufs_failure_t), 1, kcred);
		if (rc != 0) {
			CLNT_GETERR(uap->ua_cl, &re);
			TRIVIA((
": clnt_tli_kinit(%s:%s) failed(%s) rc:%d, re_errno:%d re_status:%d",
					uap->ua_netid, uap->ua_symaddr,
					err_name(rc), rc,
					re.re_errno, re.re_status));
			goto out;
		}
	}
	rc = CLNT_CONTROL(uap->ua_cl, CLSET_TIMEOUT, (caddr_t)&to);
	if (rc != 0) {
		CLNT_GETERR(uap->ua_cl, &re);
		TRIVIA((
": CLNT_CONTROL(CLSET_TIMEOUT, %s:%s) failed(%s) rc:%d, errno:%d, status:%d",
					    uap->ua_netid, uap->ua_symaddr,
					    err_name(rc), rc,
					    re.re_errno, re.re_status));
	}
...
destroy_ufsd_clients()
{
	...
	MINOR(("[destroy_ufsd_clients"));

	for (uap = &ufsd_addr_tab[0]; uap->ua_kn.knc_protofmly; uap++) {
		ASSERT(MUTEX_NOT_HELD(&uap->ua_mutex));
		mutex_enter(&uap->ua_mutex);
		if (uap->ua_cl) {
			if (uap->ua_cl->cl_auth != NULL) {
				AUTH_DESTROY(uap->ua_cl->cl_auth);
				uap->ua_cl->cl_auth = NULL;
			}
			CLNT_DESTROY(uap->ua_cl);
			uap->ua_cl = NULL;
			uap->ua_flags = UAFLG_NONE;
		}
		mutex_exit(&uap->ua_mutex);
	}
	MINOR(("]\n"));

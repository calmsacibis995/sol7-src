/*
 * Copyright 1992,1996,1997 by Sun Microsystems, Inc.
 *	All rights reserved.
 */
#pragma	ident	"@(#)ufs_acl.c	1.57	97/11/03 SMI"
...
/*
 * XXX Move protos to header file(s)
 */
int ufs_si_load(struct inode *, cred_t *);
int ufs_si_store(struct inode *, si_t *, int, cred_t *);
void ufs_si_free_mem(si_t *);
int ufs_si_free(si_t *, struct vfs *, cred_t *);
int ufs_si_inherit(struct inode *, struct inode *, o_mode_t, cred_t *);
void ufs_si_cache_flush(dev_t);
void ufs_si_del(struct inode *);
...
extern void ksort(caddr_t, int, int, int (*)());
...
loop:
	...
	sip->i_vnode.v_type = VREG;
	sip->i_nlink = 1;
	sip->i_uid = cr->cr_uid;
	sip->i_suid = (u_long)sip->i_uid > (u_long)USHRT_MAX ?
		UID_LONG : sip->i_uid;
	sip->i_gid = cr->cr_gid;
	sip->i_sgid = (u_long)sip->i_gid > (u_long)USHRT_MAX ?
		GID_LONG : sip->i_gid;
	sip->i_shadow = 0;
	TRANS_INODE(ufsvfsp, sip);
	sip->i_ufs_acl = NULL;
	...
	if ((err = ufs_rdwri(UIO_WRITE, FWRITE|FSYNC, sip, acldata,
	    acldatalen, (offset_t)0, UIO_SYSSPACE, (int *)0, cr)) != 0) {
...
switchshadows:
	...
	if (oldsp) {
		...
		ASSERT(RW_WRITE_HELD(&ip->i_contents));
		vfsp = ITOV(ip)->v_vfsp;
		if ((err = ufs_iget(vfsp, oldshadow, &sip, cr)) != 0) {
			return (EIO);
		}
 ...
 */
{
	...
	type = ip->i_mode & IFMT;
	if ((type != IFDIR) && (type != IFREG) && (type != IFIFO))
		return (EIO);
	ASSERT(ip != NULL);
	ASSERT(RW_WRITE_HELD(&ip->i_contents));
	ASSERT(ip->i_shadow && ip->i_ufs_acl == NULL);
	ASSERT(((ip->i_mode & IFMT) == IFIFO) ||
		((ip->i_mode & IFMT) == IFDIR) ||
	...
	for (fsdp = (ufs_fsd_t *)acldata;
		fsdp < (ufs_fsd_t *)(acldata + acldatalen);
			fsdp = (ufs_fsd_t *)((caddr_t)fsdp + fsdp->fsd_size)) {
		if (fsdp->fsd_size <= 0)
			break;
 ...
 */
{
	 ...
	 */
	if (cr->cr_uid == sp->aowner->acl_ic_who) {
		return (MODE_CHECK(mode, (sp->aowner->acl_ic_perm << 6)));
	}
	...
	if (sp->ausers)
		for (acl = sp->ausers; acl != NULL; acl = acl->acl_ic_next) {
			if (acl->acl_ic_who == cr->cr_uid) {
				return (MODE_CHECK(mode,
				    (mask & acl->acl_ic_perm) << 6));
			}
		}
	...
	if (groupmember((uid_t)sp->agroup->acl_ic_who, cr)) {
		...
		if (!ismask)
			return (MODE_CHECK(mode, (gperm << 6)));
	}
	...
	if (ngroup != 0)
		return (MODE_CHECK(mode, ((gperm & mask) << 6)));

	/*
	 * (5) Finally, use the "other" mode bits
	 */
	return (MODE_CHECK(mode, (sp->aother->acl_ic_perm << 6)));
}
...
ufs_acl_get(struct inode *ip, vsecattr_t *vsap, int flag, cred_t *cr)
{
	...
	/* If no acl list present - return mode bits as acl's */
	if (!ip->i_ufs_acl) {
	...
	error = aclentry2vsecattr(ip->i_ufs_acl, vsap);
	return (error);
}
/*ARGSUSED2*/
int
ufs_acl_set(struct inode *ip, vsecattr_t *vsap, int flag, cred_t *cr)
{
	...
	if ((type != IFREG) && (type != IFDIR) && (type != IFIFO))
		return (EINVAL);
	/*
	 * only the owner of the file can change the ACLs
	 */
...
aclentry2vsecattr(si_t *sp, vsecattr_t *vsap)
{
	...
	vsap->vsa_aclentp = kmem_zalloc(numacls * sizeof (aclent_t), KM_SLEEP);
	aclentp = vsap->vsa_aclentp;
 ...
 */
{
	...
	int error;
	o_mode_t old_modes, old_uid, old_gid, type;
	int mask;
	 ...
	 * or moved to a directory with a defalult acl do not allow inheritance
	 * just return.
	 */
	type = ip->i_mode & IFMT;
	if ((type != IFDIR) && (type != IFREG) && (type != IFIFO))
	...
	/*
	 * If there are no default attributes other than owner/group/other
	 * and if the object to be created is not a directory, then
	 * just return since no shadow inode is required.  If the object to
	 * be created is a directory, then we must still copy the default
	 * ACLs.
	 */
	...
	/* set the owner, group, and other values from the mode */

	MODE2ACL(tsp->aowner, (mode >> 6), cr->cr_uid);
	MODE2ACL(tsp->aother, (mode), 0);
	MODE2ACL(tsp->agroup, (mode >> 3), cr->cr_gid);
...
ufs_acl_setattr(struct inode *ip, struct vattr *vap, cred_t *cr)
{
	...
	if (!(mask & AT_MODE) && !(mask & AT_UID) && !(mask & AT_GID))
		return (0);

	/*
	 * if no regular acl's, nothing to do, so let's get out
	 */
	if (!(ip->i_ufs_acl->aowner))
		return (0);

	rw_enter(&ip->i_ufs_acl->s_lock, RW_WRITER);
	...
	if (mask & AT_UID) {
	/* XXX should not allow chown if not super user */
		sp->aowner->acl_ic_who = ip->i_uid;
	}
...
/*
 * The sp passed in is a candidate for deletion from the cache.  We
 * acquire the cache lock first, so no cache searches can be done, then
 * we acquire a lock on the sp itself.  If its reference count is still
 * zero (no one attached to it while we were acquiring the locks), we
 * remove it from the cache and deallocate it.  If the reference count
 * is non-zero, someone attached to this sp while we were acquiring
 * the locks, so we simply unlock and return.
 *
 * Parameters:
...
si_cache_del(si_t *sp)
{
	...
	rw_enter(&si_cache_lock, RW_WRITER);
	rw_enter(&sp->s_lock, RW_WRITER);
	/* Somebody attached to it while we were getting the locks */
	if (sp->s_ref) {
		rw_exit(&sp->s_lock);
		rw_exit(&si_cache_lock);
		return;
	...
	/*
	 * Unlink & free the sp from the other queues,
	 * then destory it.
	 */
	ASSERT(sp->s_fore == NULL);
	ASSERT(sp->s_flags & SI_CACHED);
	/* The 'by acl' chains */
	hash = SI_HASH(sp->s_signature);

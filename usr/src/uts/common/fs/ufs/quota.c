...
#ident	"@(#)quota.c	1.54	98/02/20 SMI"
...
loop:
	...
	if (dqp->dq_cnt != 0 || dqp->dq_flags != 0) {
		cmn_err(CE_PANIC, "getdiskquota: dqp->dq_cnt: "
	 ...
	 * It thinks that qip and ip (the inode that caused the quota
	 * routines to get called) are the same inode.
	 */
#ifndef LOCKNEST
	/*
	...
	register struct ufsvfs *ufsvfsp;
{
	...
	for (dqp = dquot; dqp < dquotNDQUOT; dqp++) {
		 ...
		 * associated with the target file system, must have a
		 ...
		 * cache and reused (via getdiskquota()).
		 */

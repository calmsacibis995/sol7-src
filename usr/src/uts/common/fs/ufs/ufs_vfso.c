 ...
 */
#pragma ident	"@(#)ufs_vfsops.c	2.173	98/01/23 SMI"
	...
	struct cred *cr;
{
	...
	if (data && datalen) {
		...
		flagmask = UFSMNT_NOINTR | UFSMNT_SYNCDIR | UFSMNT_NOSETSEC |
		    UFSMNT_LARGEFILES | UFSMNT_FORCEDIRECTIO |
		    UFSMNT_NOFORCEDIRECTIO | UFSMNT_LOGGING;
		switch (datalen) {
...
int args_len;
{
	 ...
	 * is no need to make the quota subsystem non-quiescent. If
	 * a quota applies to the root inode, then it will be updated
...
ufs_unmount(struct vfs *vfsp, struct cred *cr)
{
	...
	(void) VOP_PUTPAGE(common_specvp(bvp), (offset_t)0, (u_int)0,
	    B_INVAL, cr);
	...
	/*
	 * It is safe to NULL out the ufsvfs pointer.  BUT, when the
	 * heck do we discard the root inode!!! XXX
	 */
	rip->i_ufsvfs = NULL;

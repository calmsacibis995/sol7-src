...
#pragma	ident	"@(#)ufs_inode.c	2.173	98/02/01 SMI"
...
u_long	ufs_inode_max;	/* hard limit of allowable idle inodes */
...
ufsinit(struct vfssw *vswp, int fstype)
{
	 ...
	 */
	if (ncsize > ufs_ninode)
		ufs_ninode = ncsize;
	if (ufs_inode_max == 0)
		ufs_inode_max = (u_long)((kmem_maxavail() >> 2) /
					sizeof (struct inode));
	if (ufs_ninode > ufs_inode_max) {
...
again:
	...
	if (ftype == IFBLK || ftype == IFCHR) {
		dev_t dv;
		u_int top16 = ip->i_ordev & 0xffff0000u;
...
indirtrunc(struct inode *ip, daddr_t bn, daddr_t lastbn, int level, int flags)
{
	...
	if ((flags & I_CHEAP) == 0) {
		...
		zb = (u_int)((NINDIR(fs) - (last + 1)) * sizeof (daddr32_t));
...
ufs_itrunc(struct inode *oip, u_offset_t length, int flags, cred_t *cr)
{
	...
	if (boff == 0) {
		...
		bsize = (int)lblkno(fs, length - 1) >= NDADDR ?
		    fs->fs_bsize : fragroundup(fs, boff);
		pvn_vpzero(ITOV(oip), length, (u_int)(bsize - boff));

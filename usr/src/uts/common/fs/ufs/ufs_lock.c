...
#ident	"@(#)ufs_lockfs.c	1.63	98/01/23 SMI"	/* SunOS-4.1.1 */
...
ufs_unfreeze(struct ulockfs *ulp, struct lockfs *lockfsp)
{
	...
	if (LOCKFS_IS_HLOCK(&ulp->ul_lockfs)) {
		 ...
		 */
		comlen  = (u_int)lockfsp->lf_comlen;
		 ...
		 */
		comlen  = (u_int)ulp->ul_lockfs.lf_comlen;
 ...
 */
int
ufs_check_lockfs(struct ufsvfs *ufsvfsp, struct ulockfs *ulp, u_long mask)
{
	k_sigset_t	smask;
	int		sig, slock;
 ...
 */
int
ufs_lockfs_begin(struct ufsvfs *ufsvfsp, struct ulockfs **ulpp, u_long mask)
{
	int 		error;
	struct ulockfs *ulp;
	...
	*ulpp = ulp = &ufsvfsp->vfs_ulockfs;
	...
	if (ULOCKFS_IS_JUSTULOCK(ulp))
		...
		if (svd->type != MAP_SHARED) {
			mask = (u_long)ULOCKFS_GETREAD_MASK;
			...
			*protp &= ~PROT_WRITE;
			...
			mask = (u_long)ULOCKFS_GETWRITE_MASK;

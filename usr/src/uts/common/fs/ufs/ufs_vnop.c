 ...
 * publication.
 *
 * 	Copyright (c) 1986-1990, 1996-1997 by Sun Microsystems, Inc.
...
#pragma ident	"@(#)ufs_vnops.c	2.285	98/02/20 SMI"
...
static  int ufs_space(struct vnode *, int, struct flock64 *, int, offset_t,
		struct cred *);
static	int ufs_getpage(struct vnode *, offset_t, size_t, u_int *,
...
static	int ufs_map(struct vnode *, offset_t, struct as *, caddr_t *, size_t,
			u_char, u_char, uint, struct cred *);
...
wrip(struct inode *ip, struct uio *uio, int ioflag, struct cred *cr)
{
	...
	do {
		 ...
		 *
		 * We used to drop the i_contents lock and allowed it
		 * to be reacquired in ufs_getpage().  However, because
		 * of the i_size update above an mmap()'ed process can
		 * see stale data.  The ufs_getpage() dolock logic
		 * already allows for a thread holding i_contents for
		 * write to work properly so we keep the lock.
		 */
...
rdip(struct inode *ip, struct uio *uio, int ioflag, cred_t *cr)
{
	...
	ASSERT((ip->i_mode & IFMT) == IFREG || (ip->i_mode & IFMT) == IFDIR ||
		(ip->i_mode & IFMT) == IFLNK || (ip->i_mode & IFMT) == IFSHAD);
	...
	do {
		...
		if (!error) {
			 ...
			 */
			if (n + on == MAXBSIZE && freebehind && dofree &&
	...
	int		*rvalp)
{
	...
	switch (cmd) {
		case _FIOLFS:
			...
			if ((flag & DATAMODEL_MASK) == DATAMODEL_NATIVE) {
				...
				if (copyin((caddr_t)arg, &lockfs32,
					...
					return (EFAULT);
				lockfs.lf_lock = (u_long)lockfs32.lf_lock;
				lockfs.lf_flags = (u_long)lockfs32.lf_flags;
				lockfs.lf_key = (u_long)lockfs32.lf_key;
				lockfs.lf_comlen = (u_long)lockfs32.lf_comlen;
				lockfs.lf_comment =
		...
		case _FIOSDIO:
			 ...
			 * set delayed-io
			 */
			return (ufs_fiosdio(vp, (u_int *)arg, flag, cr));

		case _FIOGDIO:
			/*
			 * get delayed-io
			 */
			return (ufs_fiogdio(vp, (u_int *)arg, flag, cr));
	...
	struct cred *cr;
{
...
#ifdef QUOTA
	/*
	 * Make the quota subsystem non-quiescent if we are changing the
	 * file's owner.
	 */
	if (mask & AT_UID) {
		rw_enter(&ufsvfsp->vfs_dqrwlock, RW_READER);
	...
	if (mask & (AT_ATIME|AT_MTIME)) {
		...
		if (((mask & AT_ATIME) && vap->va_atime.tv_sec >= TIME32_MAX) ||
		    ((mask & AT_MTIME) && vap->va_mtime.tv_sec >= TIME32_MAX)) {
			error = EOVERFLOW;
			goto update_inode;
		}
...
#ifdef QUOTA
		/*
		 * Make the quota subsystem non-quiescent.  Even though
		 * this is an error case, we need to grab the lock since
		 * the error handling code below is common.
		 */
		rw_enter(&ufsvfsp->vfs_dqrwlock, RW_READER);
#endif /* QUOTA */
		rw_enter(&ip->i_contents, RW_WRITER);
	...
	int *eofp;
{
	...
	offset = (u_int)uiop->uio_offset & ~(DIRBLKSIZ - 1);

	/* Quit if at end of file or link count of zero (posix) */
	if (offset >= (u_int)ip->i_size || ip->i_nlink <= 0) {
		if (eofp)
			*eofp = 1;
		error = 0;
		goto unlock;
...
nextblk:
	...
	while (incount < bytes_wanted) {
		...
		if (idp->d_ino != 0 && offset >= (u_int)uiop->uio_offset) {
			u_short this_reclen =
			...
			odp->d_reclen = (u_short)this_reclen;
	...
	struct cred *cr;		/* user credentials */
{
	...
	if (error == 0) {
		 ...
		 * or EDQUOT.
		 */
		error = ufs_rdwri(UIO_WRITE, FWRITE, ip, target, strlen(target),
	...
	struct cred *cr;
{
	...
	auio.uio_resid = len;
	auio.uio_llimit = U_CURLIMIT(PTOU(ttoproc(curthread)), RLIMIT_FSIZE);
	...
	struct fid *fidp;
{
	...
	if (fidp->fid_len < (sizeof (struct ufid) - sizeof (ushort))) {
		fidp->fid_len = sizeof (struct ufid) - sizeof (ushort);
		return (ENOSPC);
	}
...
#ifdef QUOTA
		/*
		 * Make the quota subsystem non-quiescent if we need to
		 * call bmap_write() below (with i_contents as writer).
		 */
...
#ifdef QUOTA
			/*
			 * Make the quota subsystem non-quiescent before
	...
	int 		seq;
{
	...
	if (crpage) {
		if ((pp = page_create_va(vp, off, PAGESIZE, PG_WAIT, seg->s_as,
		    addr)) == NULL) {
			return (ufs_fault(vp,
 ...
 * free = 0: Preparation before dump, allocate dump private data
 * structure to hold all the direct and indirect blocks info for
 * dump.
 * free = 1: Clean up after dump, deallocate the dump private
 * data structure.
 */
static int
ufs_dumpctl(vnode_t *vp, int free)
{
	struct inode	*ip = VTOI(vp);
	ufsvfs_t	*ufsvfsp = ip->i_ufsvfs;
	...
	if (!free) {
		...
		mutex_exit(&ip->i_tlock);
	} else {			/* deallocate the dump structure */
		if (dump_info == NULL)
			return (EINVAL);

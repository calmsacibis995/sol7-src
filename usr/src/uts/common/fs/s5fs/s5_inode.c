 ...
 *
 */
#pragma ident   "@(#)s5_inode.c	1.13    97/06/22 SMI"
...
s5_iupdat(struct inode *ip, int waitfor)
{
	 ...
	 */
	ASSERT((u_long)ip->i_uid <= (u_long)USHRT_MAX);
	ASSERT((u_long)ip->i_gid <= (u_long)USHRT_MAX);
	...
	int level;
{
	...
	bcopy((caddr_t)bap, (caddr_t)copy->b_un.b_daddr, (u_int)bsize);
	bzero((caddr_t)&bap[last + 1],
	    (u_int)(NINDIR(ip->i_s5vfs) - (last + 1)) * sizeof (daddr_t));
	bwrite(bp);
	...
	struct cred *cr;
{
	...
	if (boff == 0) {
		 ...
		 */
		rw_exit(&oip->i_contents);
		pvn_vpzero(ITOV(oip), (offset_t)length, (u_int)(bsize - boff));
		rw_enter(&oip->i_contents, RW_WRITER);

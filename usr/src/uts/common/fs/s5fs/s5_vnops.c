...
#pragma ident	"@(#)s5_vnops.c	1.29	98/01/15 SMI"
...
static int s5_page_fill(struct inode *ip, page_t *pp, u_int off,
    u_int bflgs, u_int *pg_off, struct seg *seg);
static int s5_iodone(struct buf *bp);

static int s5_getpage_ra(struct vnode *vp, u_int off, struct seg *seg,
    caddr_t addr);
static int s5_getpage_miss(struct vnode *vp, u_int off, size_t len,
...
static int s5_getpage(struct vnode *vp, offset_t off, size_t len, u_int *protp,
...
static struct buf *s5_startio(struct inode *ip, daddr_t bn,
    page_t *pp, size_t len, u_int pgoff, int flags, int io);
static int s5_map(struct vnode *vp, offset_t off, struct as *as,
    caddr_t *addrp, size_t len, u_char prot, u_char maxprot, u_int flags,
    struct cred *cr);
static int s5_addmap(struct vnode *vp, offset_t off, struct as *as,
    caddr_t addr, size_t len, u_char prot, u_char maxprot, u_int flags,
    struct cred *cr);
static int s5_delmap(struct vnode *vp, offset_t off, struct as *as,
    caddr_t addr, size_t len, u_int prot, u_int maxprot, u_int flags,
    struct cred *cr);

static int s5_l_pathconf(struct vnode *vp, int cmd, u_long *valp,
    struct cred *cr);
    ...
    int flags, struct cred *cr)
{
	...
	if (mask & (AT_UID|AT_GID)) {
		...
		if (mask & AT_UID) {
			 ...
			 */
			if ((u_long)vap->va_uid > (u_long)USHRT_MAX) {
				error = EOVERFLOW;
				goto update_inode;
			}
			ip->i_uid = vap->va_uid;
		...
		if (mask & AT_GID) {
			 ...
			 */
			if ((u_long)vap->va_gid > (u_long)USHRT_MAX) {
				error = EOVERFLOW;
				goto update_inode;
			}
			ip->i_gid = vap->va_gid;
	...
	if (mask & AT_SIZE) {
		...
		if (error = s5_itrunc(ip, (u_long)vap->va_size, cr))
			goto update_inode;
	}
	/*
    ...
    int *eofp)
{
	...
	int incount = 0;
	int outcount = 0;
	u_int bytes_wanted, total_bytes_wanted;
	caddr_t outbuf;
	size_t bufsize;
	int error;
	struct fbuf *fbp;
 ...
 */
static int
s5_getpage(struct vnode *vp, offset_t off, size_t len, u_int *protp,
    page_t *plarr[], size_t plsz, struct seg *seg, caddr_t addr,
    enum seg_rw rw, struct cred *cr)
{
	struct inode 	*ip = VTOI(vp);
	u_int		uoff = (u_int)off; /* type conversion */
	int 		err;
	page_t		**pl;
	int		beyond_eof;
	int		seqmode;
 ...
 */
static int
s5_getpage_miss(struct vnode *vp, u_int off, size_t len, struct seg *seg,
...
/*ARGSUSED*/
static int
s5_getpage_ra(struct vnode *vp, u_int off, struct seg *seg,
    caddr_t addr)
{
	...
	u_long		io_len;
	caddr_t		addr2 = addr + ((u_int)io_off - off);
	page_t		*pp;
	u_int		lfoff = ip->i_nextrio;
	u_int		pgoff;
 ...
 */
static int
s5_page_fill(struct inode *ip, page_t *pp, u_int off,
    u_int bflgs, u_int *pg_off, struct seg *seg)
{
	int		err = 0;
	daddr_t		bn;
...
s5_startio(struct inode *ip, daddr_t bn, page_t *pp,
    size_t len, u_int pgoff, int flags, int io)
{
	struct buf *bp;
...
s5_map(struct vnode *vp, offset_t off, struct as *as, caddr_t *addrp,
    size_t len, u_char prot, u_char maxprot, u_int flags,
    struct cred *cr)
{
...
static int
s5_l_pathconf(struct vnode *vp, int cmd, u_long *valp,
    struct cred *cr)
{
	struct ulockfs	*ulp;
	int		error;

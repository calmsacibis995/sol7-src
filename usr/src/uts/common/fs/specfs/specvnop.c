...
#pragma ident	"@(#)specvnops.c	1.116	98/01/26 SMI"
...
static int spec_getpage(struct vnode *, offset_t, size_t, u_int *, page_t **,
    size_t, struct seg *, caddr_t, enum seg_rw, struct cred *);
static int spec_putapage(struct vnode *, page_t *, u_offset_t *, size_t *, int,
	struct cred *);
static struct buf *spec_startio(struct vnode *, page_t *, u_long, size_t, int);
static int spec_getapage(struct vnode *, u_offset_t, size_t, u_int *,
...
spec_open(struct vnode **vpp, int flag, struct cred *cr)
{
	...
	case VCHR:
		...
		if (STREAMSTAB(maj)) {
			...
			error = stropen(cvp, &newdev, flag, 0, cr);
...
#else
				nsp->s_size = csp->s_size = cdev_size(newdev);
#endif /* _LP64 */
			}
			if ((error == 0) && (dev == newdev)) {
				/*
...
#else
				sp->s_size = csp->s_size = cdev_size(dev);
#endif /* _LP64 */
			}
		}
		break;
	...
	case VBLK:
		 ...
		 */
		if (error == 0) {
			size_t sn_size = csp->s_size;
			if (sn_size == 0 || sn_size == UNKNOWN_SIZE) {
#ifdef _LP64
				/*
				 * for now we only check the "Nblocks"
				 * property on 64-bit systems
				 */
...
#endif /* _LP64 */
				if (sn_size == (size_t)-1 ||
				    sn_size > btodb((u_long)MAXOFF_T + 1))
					csp->s_size = UNKNOWN_SIZE;
				else
					csp->s_size = dbtob(sn_size);
			}
		}
		break;
	...
	struct cred		*cr)
{
	...
	do {
		...
		off = uiop->uio_offset & (off_t)MAXBMASK;
	...
	struct cred	*cr)
{
	...
	struct buf *bp;
	page_t *pp, *pp2;
	u_long io_off1, io_off2;
	size_t io_len1;
	size_t io_len2;
	size_t blksz;
	u_long blkoff;
	int dora, err, pagefound;
	u_int xlen;
...
reread:
	...
	if (spec_ra && sp->s_nextr == (u_long)off)
		dora = 1;
	else
		dora = 0;
...
again:
	if ((pagefound = page_exists(vp, off)) == 0) {
		if (rw == S_CREATE) {
			...
			io_len1 = PAGESIZE;
			sp->s_nextr = (u_long)off + PAGESIZE;
		} else {
			/*
			...
			pp = pvn_read_kluster(vp, off, seg, addr, &tmpoff,
			    &io_len1, (u_offset_t)blkoff, blksz, 0);
			io_off1 = (u_long)tmpoff;
			/*
	...
	if (dora && rw != S_CREATE) {
		...
		if (off2 >= size)
			...
			pp2 = pvn_read_kluster(vp, off2, seg, addr2, &tmpoff,
			    &io_len2, (u_offset_t)off2, blksz, 1);
			io_off2 = (u_long)tmpoff;
		}
	...
	if (pp != NULL)
		pvn_plist_init(pp, pl, plsz, (u_offset_t)off, io_len1, rw);
	return (0);
}
	...
	struct cred	*cr)
{
	...
	if (len == 0) {
		 ...
		 */
		eoff = off + len;
		for (io_off = (u_long)off; io_off < eoff && io_off < size;
		    io_off += io_len) {
			/*
	...
	struct cred	*cr)
{
	 ...
	 */
	pp = pvn_write_kluster(vp, pp, &tmpoff, &io_len, (u_offset_t)blkoff,
		blksz, flags);
	io_off = (u_long)tmpoff;
...
{
	int error = 0;
	major_t maj = getmajor(vp->v_rdev);
	int map_flag;
	...
	if (vp->v_type == VCHR) {
		...
		int (*segmap)(dev_t, off_t, struct as *,
		    caddr_t *, off_t, u_int, u_int, u_int, cred_t *);
		...
		if (segmap == NULL || segmap == nulldev || segmap == nodev) {
			...
			if ((map_flag & D_DEVMAP) || mmap == NULL ||
				...
				error = devmap_setup(dev, off, as, addrp, len,
		...
		if ((ssize_t)off < 0 || (ssize_t)(off + len) < 0)
			return (EINVAL);
	...
	cred_t	*cr)
{
	...
	bp = spec_startio(vp, pp, (u_long)io_off, io_len, flags);

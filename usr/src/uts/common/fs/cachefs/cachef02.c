 ...
 * All rights reserved.
 */
#pragma ident	"@(#)cachefs_dir.c	1.68	97/10/30 SMI"
    ...
    u_offset_t *d_offsetp, cfs_cid_t *cidp)
{
	...
	while (blockoff < va.va_size) {
		...
		while (offset < MAXBSIZE && (blockoff + offset) < va.va_size) {
			...
			dep = (struct c_dirent *)((uintptr_t)addr + offset);
...
cachefs_dir_new(cnode_t *dcp, cnode_t *cp)
{
	...
	dep = (struct c_dirent *)((uintptr_t)addr + len);
    ...
    int issync)
{
	...
	dep->d_id = *cidp;
	dep->d_namelen = (u_short)strlen(nm);
    ...
    cfs_cid_t *cidp)
{
	...
	u_int off = (u_int)(offset & (offset_t)MAXBOFFSET);
	caddr_t addr;
	vnode_t *dvp;
	...
	addr = segmap_getmap(segkmap, dvp, blockoff);
	dep = (struct c_dirent *)((uintptr_t)addr + off);
	if (cookiep) {
		dep->d_flag |= CDE_COMPLETE;
		CACHEFS_FID_COPY(cookiep, &dep->d_cookie);
	}
...
#define	DIR_ENDOFF	0x7fffffffLL
	...
	*cntp = bufsize;
	...
	for (;;) {
		...
		ASSERT((uintptr_t)dep < ((uintptr_t)addr + MAXBSIZE));
		if (dep->d_flag & CDE_VALID) {
    ...
    vnode_t *frontvp, u_offset_t *offsetp, u_offset_t *fsizep)
{
	...
	/*LINTED alignment okay*/
	u_int off = (u_int)(*offsetp & (offset_t)MAXBOFFSET);
	...
	cdep = (struct c_dirent *)((uintptr_t)addr + off);
	inblk = MAXBSIZE-off;
	if (*offsetp != 0) {
	...
	while (count > 0) {
		...
		cdep->d_namelen = (u_short)strlen(dep->d_name);

...
#pragma ident   "@(#)cachefs_vnops.c 1.219     98/02/04 SMI"
...
static int	cachefs_pathconf(vnode_t *, int, u_long *, cred_t *);
...
cachefs_open(vnode_t **vpp, int flag, cred_t *cr)
{
	...
	for (;;) {
		...
		if (fscp->fs_cdconnected != CFS_CD_CONNECTED) {
			 ...
			 */
			if (cachefs_cred_cmp(cp->c_cred, CRED()) == 0) {
				mutex_exit(&cp->c_statelock);
				connected = 1;
				continue;
			}
    ...
    u_offset_t iooff, cred_t *cr)
{
	...
	error = bp->b_error = vn_rdwr(UIO_WRITE, backvp, bp->b_un.b_addr,
	    iolen, iooff, UIO_SYSSPACE, FSYNC,
...
cachefs_setattr_common(vnode_t *vp, vattr_t *vap, int flags, cred_t *cr)
{
	 ...
	 */
	if (mask & AT_SIZE) {
		bcnt = (u_int)(cp->c_size & PAGEOFFSET);
		if (bcnt)
...
cachefs_getpage(struct vnode *vp, offset_t off, size_t len,
	u_int *protp, struct page *pl[], size_t plsz, struct seg *seg,
...
/*ARGSUSED*/
static int
cachefs_getapage(struct vnode *vp, u_offset_t off, size_t len, u_int *protp,
    ...
    int flags, cred_t *cr)
{
	...
	if (flags & B_ASYNC) {
		...
		if ((cp->c_ioflags & CIO_PUTPAGES) == 0 || tflags != 0) {
			...
			rp->cfs_req_u.cu_putpage.cp_len = (u_int)len;
...
static int
cachefs_pathconf(vnode_t *vp, int cmd, u_long *valp, cred_t *cr)
{
	int error = 0;

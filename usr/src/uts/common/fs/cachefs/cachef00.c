 ...
 * All rights reserved.
 */
#pragma ident	"@(#)cachefs_cnode.c	1.137	97/10/30 SMI"
...
cachefs_cnode_idle(struct vnode *vp, cred_t *cr)
{
	...
	/* put cnode on idle list and increment number of idle cnodes */
	mutex_enter(&fscp->fs_idlelock);
	cachefs_cnode_idleadd(cp);
	if ((fscp->fs_idlecnt > CFS_FS_MAXIDLE) &&
	    (fscp->fs_idleclean == 0) &&
...
cachefs_cnode_idleclean(fscache_t *fscp, int unmount)
{
	...
	if (unmount) {
		...
		remcnt = (fscp->fs_idlecnt - CFS_FS_MAXIDLE) +
		    (CFS_FS_MAXIDLE >> 2);
	}
	...
	for (; remcnt > 0; remcnt--) {
		...
		if (cp->c_flags & CN_IDLE) {
			...
			/* destroy the cnode */
			VN_RELE(CTOV(cp));
			cachefs_cnode_inactive(CTOV(cp), kcred);
		} else {
...
cachefs_cnode_inactive(register struct vnode *vp, cred_t *cr)
{
	...
	/* remove the cnode from the filegrp list */
...
/*
 * Search for the cnode hanging off of this file group.
 * On success, returns 0 and *cpp is set to the cnode.
 * If a cnode matches file but the cookie does not match then
    ...
    struct cnode **cpp)
{
	...
	CFS_DEBUG(CFSDEBUG_CNODE)
		printf("cachefs_cnode_find: ENTER fileno %llu fgp %p\n",
	...
	*cpp = NULL;
	...
	for (cp = fgp->fg_cnodelist; cp != NULL; cp = cp->c_next) {
		if (cidp->cid_fileno != cp->c_id.cid_fileno)
			continue;
		/*
		 * XXX tony: maybe we should also skip CN_DESTROY,
		 * if they exist???
		 */
		...
		if ((cookiep->fid_len != cp->c_cookie.fid_len) ||
			...
			CFS_DEBUG(CFSDEBUG_GENERAL) {
				printf("cachefs: dup fileno %llu, cp %p\n",
...
#ifdef CFSDEBUG
	CFS_DEBUG(CFSDEBUG_CNODE)
		printf("cachefs_cnode_find: EXIT cp %p\n", (void *)*cpp);
#endif
	return (badcookie);
}
    ...
    int flag, cred_t *cr)
{
	...
	if (cp->c_flags & CN_NOCACHE) {
		...
		if (cookiep) {
			...
			VN_INIT(vp, fscp->fs_cfsvfsp, cp->c_attr.va_type,
				cp->c_attr.va_rdev);
	...
	/* try to read the cached metadata for the file */
	error = filegrp_read_metadata(cp->c_filegrp, cidp, &cp->c_metadata);

	/* if the metadata is not cached */
	if (error == ENOENT) {
		if (cookiep == NULL)
			goto out;
		error = CFSOP_INIT_COBJECT(fscp, cp, vap, cr);
	...
	/* else if the metadata is cached */
	else if (error == 0) {
		...
		if ((cachep->c_flags & CACHE_CHECK_RLTYPE) &&
			...
			rlp = cachefs_rl_entry_get(cachep,
	...
	vattr_t *vap, vnode_t *backvp, cred_t *cr, int flag, cnode_t **cpp)
{
	...
	error = cachefs_cnode_find(fgp, cidp, cookiep, &cp);

	/*
	 * If there already is a cnode with this cid
	...
	if (error) {
		...
		/* verify the backvp */
		error = cachefs_getcookie(backvp, &cookie, NULL, cr);
		if (error ||
...
int
cachefs_cid_inuse(fscache_t *fscp, cfs_cid_t *cidp)
{
	struct cachefs_metadata metad;
	struct filegrp *fgp;
	cnode_t *cp;
	int error;
	...
	error = 0;
	cp = NULL;
	error = cachefs_cnode_find(fgp, cidp, NULL, &cp);
	if (cp != NULL) {
		mutex_exit(&fgp->fg_cnodelock);
		filegrp_rele(fgp);
		return (1);
	...
	error = filegrp_read_metadata(fgp, cidp, &metad);
	/* metad.md_rltype may be incorrect, but who cares. */
	mutex_exit(&fgp->fg_cnodelock);
	filegrp_rele(fgp);
...
cachefs_cnode_create(fscache_t *fscp, vattr_t *vap, int flag, cnode_t **cpp)
{
	...
	for (;;) {
		...
		if (cachefs_cid_inuse(fscp, &cid2)) {
			mutex_exit(&fscp->fs_fslock);
			continue;
		}
		...
		/* see if a cnode already has this fileno */
		error = 0;
		cp = NULL;
		error = cachefs_cnode_find(fgp, &cid, NULL, &cp);
		ASSERT(error == 0);
		if (cp) {
			mutex_exit(&fgp->fg_cnodelock);
			filegrp_rele(fgp);
		...
		/* see if metadata already exists for this fileno */
		error = filegrp_read_metadata(fgp, &cid, &metad);
		/* metad.md_rltype may be incorrect, but who cares */
		if (error != ENOENT) {
			mutex_exit(&fgp->fg_cnodelock);
			filegrp_rele(fgp);
...
#define		MAXTRIES 1000
		...
		for (index = 0; index < MAXTRIES; index++) {
			(void) sprintf(namebuf, "%s.%" PRIx64, strp,
			    hrestime.tv_nsec * cp->c_id.cid_fileno * index);
...
cnode_enable_caching(struct cnode *cp)
{
	...
	if (error == 0) {
		if ((cachep->c_flags & CACHE_CHECK_RLTYPE) &&
			...
			mutex_enter(&cachep->c_contentslock);
			rlp = cachefs_rl_entry_get(cachep, md.md_rlno);
			rl = *rlp;
			mutex_exit(&cachep->c_contentslock);
...
cachefs_cnode_stale(cnode_t *cp)
{
	...
	/*
	 * This routine assumes that the cnode is locked with -
	 * mutex_enter(&cp->c_statelock);
	 */
	/*

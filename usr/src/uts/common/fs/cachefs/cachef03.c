...
#pragma ident   "@(#)cachefs_dlog.c 1.35     97/11/03 SMI"
...
#endif /* CFSDEBUG */
				/*LINTED alignment okay*/
				fscp->fs_dlogseq = (u_int)vattr.va_size;
			}
		} else {
#ifdef CFSDEBUG
			CFS_DEBUG(CFSDEBUG_DLOG)
 ...
 */
static off_t
cachefs_dlog_output(fscache_t *fscp, cfs_dlog_entry_t *entp, u_int *seqp)
{
	int error;
	off_t offset;
	int xx;
    ...
    cnode_t *cp, cred_t *cr)
{
	...
	CACHEFS_VATTR_TO_CFS_VATTR_COPY(vap, &up->dl_attrs, error);
	up->dl_flags = flags;
	up->dl_cid = cp->c_id;
	CACHEFS_TS_TO_CFS_TS_COPY(&cp->c_metadata.md_vattr.va_mtime,
		&up->dl_times.tm_mtime, error);
	CACHEFS_TS_TO_CFS_TS_COPY(&cp->c_metadata.md_vattr.va_ctime,
		&up->dl_times.tm_ctime, error);

	if (error)
		cmn_err(CE_WARN, "cachefs: dlog record time overflow");

	/* store the cred info */
	len = sizeof (cred_t) + (cr->cr_ngroups - 1) * sizeof (gid_t);
	bcopy(cr, &up->dl_cred, len);
    ...
    char *nnm, cred_t *cr, cnode_t *cp, cnode_t *delcp)
{
	...
	if (delcp) {
		...
		CACHEFS_TS_TO_CFS_TS_COPY(&delcp->c_metadata.md_vattr.va_mtime,
			&up->dl_del_times.tm_mtime, error);
		CACHEFS_TS_TO_CFS_TS_COPY(&delcp->c_metadata.md_vattr.va_ctime,
			&up->dl_del_times.tm_ctime, error);
	} else {
...
off_t
cachefs_dlog_modify(fscache_t *fscp, cnode_t *cp, cred_t *cr, u_int *seqp)
{
	struct cfs_dlog_entry *entp;

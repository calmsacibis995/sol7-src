...
#pragma ident	"@(#)lofs_vnops.c	1.42	98/01/23 SMI"
	...
	*vpp = NULL;	/* default(error) case */
	...
	if (doingdotdot) {
		...
		if ((vtol(dvp))->lo_looping &&
		    realdvp->v_vfsmountedhere) {
			/*
			 * if looping get the actual found vnode
			 * instead of the vnode covered
			 */
			error = vn_vfsrlock(realdvp);
			if (error)
				return (error);

			error = VFS_ROOT(realdvp->v_vfsmountedhere, &tvp);
...
#endif
				/*
				 * looping because parent vnode was looping.
				 *
				 * in general when a vnode is marked looping
				 * it should yield a vnode representing a
				 * directory prior to the lofs mount; which
				 * will stop the looping.
				 *
				 * But when an autofs is mixed with lofs
				 * /net/hostname, we have to allow one more
				 * looping level:
				 *
				 * "/net/localhost/net/localhost"  is looping
				 * but access to the vnode covered by the lofs
				 * node (in this case autonode) will just
				 * go back to the lonode mounted on it, and
				 * hence loop.  But we stop further looping if
				 * "net" is accessed again by returning the
				 * vnode covered by the lofs node.
				 */
				VN_RELE(vp);
...
static int
lo_pathconf(vnode_t *vp, int cmd, u_long *valp, struct cred *cr)
{
	vp = realvp(vp);

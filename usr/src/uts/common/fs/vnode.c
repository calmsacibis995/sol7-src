...
#pragma ident	"@(#)vnode.c	1.46	98/02/22 SMI"
...
top:
	...
	*vpp = NULL;
	 ...
	 * in the directory.
	 * Disallow writing a core file through a symlink as super-user.
	 * If this is a non-exclusive create we also find the node itself.
	 */
	if (excl == EXCL)
		error = lookuppn(&pn, NULL, NO_FOLLOW, &dvp, NULLVPP);
	else if (why == CRCORE && suser(CRED()))
		error = lookuppn(&pn, NULL, NO_FOLLOW, &dvp, vpp);
	else
	...
	if (umask) {
		...
		/*
		 * We have to free the memory allocated by VOP_GETSECATTR.
		 */
...
/*
 * vn_vfsrlock is used to implement a lock which is logically a readers lock
 * protecting the v_vfsmountedhere field.  This implementation, however,
 * is lacking in that it really only implements a mutex.  However, we
 * use it because we are not yet prepared to change the contents of the
 * vnode structure but look forward to the day when we can, at which point
 * we will include a real readers/writer lock.
 *
 * The existing lock is a bit strange; when you hold it as a (logical)
 * read lock, you actually hold the mutex, because the expectation is that
 * the read version will only be held for a short time in traverse() (for
 * the duration of calling VFS_ROOT).  When you hold the logical write lock,
 * you only hold the mutex long enough to set VVFSLOCK.  Thus, the mutex
 * plays two roles: it is the read lock, and it provides exclusion for
 * manipulating the write lock.  Strange, no?  The read version of the
 * used to be inline in traverse, so its function was obscure.
 *
 * traverse() and routines re-implementing part of traverse (e.g. autofs)
 * need to hold the readers lock.  mount(), vn_rename(), vn_remove() and so
 * on need the writers version of the lock.
 */
int
...
int
vn_vfsrlock_held(vnode_t *vp)
{
	ASSERT(vp != NULL);

	return (MUTEX_HELD(&vp->v_lock));
}

...
#pragma ident	"@(#)lm_client.c	1.63	97/09/22 SMI"
/* NCR OS2.00.00 1.1 */
	...
	netobj *fh)
{
	...
	lm_set_oh(oh_bytes, sizeof (oh_bytes), curproc->p_pid);
	...
	ls = lm_get_sysid(VTOMI(vp)->mi_curr_serv->sv_knconf,
		&VTOMI(vp)->mi_curr_serv->sv_addr,
		VTOMI(vp)->mi_curr_serv->sv_hostname, TRUE, NULL);
...
#if 0
		LM_DEBUG((2, "shrlck", "lm_callrpc() returned %d", error));
#else

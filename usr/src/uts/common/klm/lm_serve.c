...
#pragma ident "@(#)lm_server.c 1.66     97/11/13 SMI" /* NCR OS2.00.00 1.2 */
...
#ifndef lint
_NOTE(MUTEX_PROTECTS_DATA(lm_lck, lm_server_status))
_NOTE(DATA_READABLE_WITHOUT_LOCK(lm_server_status))
_NOTE(MUTEX_PROTECTS_DATA(lm_vnodes_lock, lm_vnodes))
_NOTE(MUTEX_PROTECTS_DATA(lm_lck, lm_vnode_len))
_NOTE(MUTEX_PROTECTS_DATA(lm_lck, lm_status_cv))
#endif
...
#ifndef lint
_NOTE(MUTEX_PROTECTS_DATA(lm_lck, lockd_pid))
_NOTE(DATA_READABLE_WITHOUT_LOCK(lockd_pid))
#endif

/*
 * lm_xprts is a null-terminated (next == NULL) list.
 */
static struct lm_xprt *lm_xprts = NULL;
static struct kmem_cache *lm_xprt_cache = NULL;

#ifndef lint
_NOTE(MUTEX_PROTECTS_DATA(lm_lck, lm_xprts))
#endif
...
lm_svc(struct lm_svc_args *uap)
{
	...
	if (lm_server_status == LM_DOWN || lockd_pid == 0) {
		 ...
		 * not interpreted.
		 */
		(void) strncpy((char *)&lm_owner_handle_sys,
	...
	if (lm_server_status == LM_DOWN) {
		...
		if (error != 0 || me == (struct lm_sysid *)NULL) {
		...
		(void) delay_sig(LM_RLOCK_SLP*hz);
	...
	LM_DEBUG((1, "svc", "Running, servers= %d, start_time= %lx\n",
...
lm_relock_server(char *server)
{
	...
	LM_DEBUG((1, "rlck_serv", "entering lm_lck"));
	for (ls = lm_sysids; ls; ls = ls->next) {
 ...
 */
int
lm_shr_sysid_has_locks(int32_t sysid)
{
	int result = 0;
	struct lm_vnode *lv;
	struct shrlock shr;

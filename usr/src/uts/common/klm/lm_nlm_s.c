...
#pragma ident   "@(#)lm_nlm_server.c 1.53     97/11/13 SMI"
...
lm_nlm_reclaim(struct vnode *vp, struct flock64 *flkp)
{
	...
	lm_set_oh(oh_bytes, sizeof (oh_bytes), flkp->l_pid);
	nla.alock.oh.n_len = sizeof (oh_bytes);
	nla.alock.oh.n_bytes = &oh_bytes[0];
	nla.alock.svid = flkp->l_pid;
	nla.alock.l_offset = (u_int)flkp->l_start;
	nla.alock.l_len = (u_int)flkp->l_len;
	nla.reclaim = TRUE;
	nla.state = 1;
	 ...
	 */
	mi = VTOMI(vp);
	ls = lm_get_sysid(mi->mi_curr_serv->sv_knconf,
	    &mi->mi_curr_serv->sv_addr,
	    mi->mi_curr_serv->sv_hostname, FALSE, NULL);

	error = 0;
	if (ls != NULL) {
		error = lm_callrpc(ls, NLM_PROG, NLM_VERS, NLM_LOCK,
		    xdr_nlm_lockargs, (caddr_t)&nla,
		    xdr_nlm_res, (caddr_t)&nr,
		    lm_sa.retransmittimeout, LM_RECLAIM_RETRY);

...
#pragma	ident	"@(#)coff.c	1.26 97/10/21 SMI"
	...
	struct cred *cred)
{
	...
	*execsz = btopr(SINCR) + btopr(SSIZE) + btopr(NCARGS-1);
	 ...
	 */
	if (*execsz > btopr(U_CURLIMIT(up, RLIMIT_VMEM)))
		return (ENOMEM);
...
coffcore(vnode_t *vp, proc_t *pp, struct cred *credp, rlim64_t rlimit, int sig)
{
	...
	up->u_dsize = btopr(up->u_exdata.ux_dsize + up->u_exdata.ux_bsize +
	    pp->p_brksize);
	...
	*/
	...
	if ((sizeof (struct user) + ptob(up->u_dsize)+pp->p_stksize) > rlimit) {
		up->u_dsize = 0;
	...
	if (error == 0 && up->u_dsize) {
		...
		count = ptob(up->u_dsize) - PAGOFF(base);
...
coffaddaux(struct uarg *args, caddr_t entloc, int fd)
{
	...
	klwp_id_t const lwp = ttolwp(curthread);
	...
	up->u_auxv[naux-1].a_un.a_val = (u_int)aux[0].a_un.a_val;
	up->u_auxv[naux].a_type = aux[1].a_type;
	up->u_auxv[naux].a_un.a_val = (u_int)aux[1].a_un.a_val;

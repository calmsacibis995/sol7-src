 ...
 * All rights reserved.
 */
#pragma ident	"@(#)elf.c	1.81	97/11/22 SMI" /* from S5R4 1.31 */
	...
	struct cred *cred)
{
	...
	*execsz = btopr(SINCR) + btopr(SSIZE) + btopr(NCARGS-1);
	 ...
	 */
	limit = btop(U_CURLIMIT(&u, RLIMIT_VMEM));
	roundlimit = btopr(U_CURLIMIT(&u, RLIMIT_VMEM));
...
#elif defined(i386)
#error "no definitions for 64-bit intel"
#endif

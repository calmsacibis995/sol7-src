...
#pragma ident	"@(#)audit.c	1.99	98/02/06 SMI"
...
{	/* AUDIT_SAVEPATH */
	...
	sp = kmem_alloc((u_int)len, KM_SLEEP);
...
{
	if (((t_audit_data_t *)T2A(curthread))->tad_flag)
		au_uwrite(au_to_arg32(3, "stropen: flag", (uint32_t) flag));
}
...
{
	if (((t_audit_data_t *)T2A(curthread))->tad_flag)
		au_uwrite(au_to_arg32(2, "strclose: flag", (uint32_t) flag));
}
    ...
    int copyflag, cred_t *crp, int *rvalp)
{
	if (((t_audit_data_t *)T2A(curthread))->tad_flag)
#ifdef _LP64
			au_uwrite(au_to_arg64(2, "strioctl:vnode",
						(uint64_t) vp));
#else
			au_uwrite(au_to_arg32(2, "strioctl:vnode",
						(uint32_t) vp));
#endif
...
{	/* AUDIT_CLOSEF */
	...
	if ((flag = (success || failure)) != 0) {
		...
		/* add a process token */
		au_write((caddr_t *)&(ad), au_to_subject(curproc));
...
void
audit_exec(
	uintptr_t Usrstack,	/* USRSTACK */
	uintptr_t argvp_end,	/* top end of user stack */
	size_t size,	/* size of argc, argv, aux vector and arg strings */
	ssize_t na,	/* total # arguments; ie, # argv + # env variable */
	ssize_t ne)	/* total # environment arguments */
{
	char **argvp;		/* pointers to user address arguments */
	...
	argvp = (char **)(argvp_end - size);	/* points to argc */
	argvp++;
	argc = (int)(na - ne);

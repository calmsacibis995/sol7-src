...
#pragma ident	"@(#)audit_io.c	1.42	97/11/20 SMI"
...
ADDTRACE("[%x] xfer strt - AR cnt: %lx cAR: %p cMB: %p used: %lx bp: %p off: %lx",
...
debug_enter("HELP ---- au_queue.head is null and we are still here");
			...
			len = (ssize_t)MIN(sz,
			    	au_queue.buflen - off); /* len to move */
...
ADDTRACE("[%x] new offsets - off: %lx cAR: %p sp: %p", off, cAR, sp, 0, 0, 0);
			...
			if (used >= (ssize_t)cMB->len) { /* advance to next au_membuf */
				used = 0;
				cMB  = cMB->next_buf;
			}
			if (cMB == (au_buff_t *)0) { /* advance to next AR */
				sp   = cAR;
				cAR  = cAR->next_rec;
				cMB  = cAR;
			}
...
ADDTRACE("[%x] flush buffer - AQ.head: %p AQ.len: %lx sp: %p",
	au_queue.head, au_queue.cnt, sp, 0, 0, 0);
				...
				if (sp) {	/* if we've written an AR */
					...
					au_queue.head && au_queue.head != cAR);
				}
...
ADDTRACE("[%x] bufs freed - AQ.head: %p aq.cnt: %lx",
	au_queue.head, au_queue.cnt, 0, 0, 0, 0);
				...
				if (sb.f_blocks && (fsblkcnt64_t)limit >

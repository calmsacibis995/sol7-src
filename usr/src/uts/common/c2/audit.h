...
#pragma ident	"@(#)audit.h	1.64	98/02/06 SMI"
...
/*
 * The structure of the terminal ID
 */
struct au_tid {
	dev_t port;
	uint_t machine;
...
/*
 * Portaudit data structure for getportaudit(2) system call.
 */
#define	MAXPA 4
...
struct portaudit {
	int	pa_vers;
	int	pa_cnt;
	struct auditinfo pa_entry[MAXPA];
};

typedef struct portaudit portaudit_t;
...
#define	AUDIT_SET  1		/* set current process audit characteristics */
#define	AUDIT_PROC 2		/* return peer process information */
...
void	audit_exec(uintptr_t, uintptr_t, size_t, ssize_t, ssize_t);

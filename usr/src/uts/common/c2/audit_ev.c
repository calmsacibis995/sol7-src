...
#pragma ident	"@(#)audit_event.c	1.81	98/02/06 SMI"
...
static void	aus_bind(struct t_audit_data *);
static void	aus_connect(struct t_audit_data *);
static void	aus_shutdown(struct t_audit_data *);
static void	aus_setsockopt(struct t_audit_data *);
static void	aus_recvmsg(struct t_audit_data *);
static void	auf_recvmsg(struct t_audit_data *, int, rval_t *);
static void	aus_sendmsg(struct t_audit_data *);
static void	aus_recvfrom(struct t_audit_data *);
static void	auf_recvfrom(struct t_audit_data *, int, rval_t *);
static void	aus_sendto(struct t_audit_data *);
...
aui_null,	AUE_NULL,	aus_null,	/* 231 so_socketpair */
		auf_null,	0,
aui_null,	AUE_BIND,	aus_bind,	/* 232 bind */
		auf_null,	0,
aui_null,	AUE_NULL,	aus_null,	/* 233 listen */
		auf_null,	0,
aui_null,	AUE_ACCEPT,	aus_null,	/* 234 accept */
		auf_accept,	0,
aui_null,	AUE_CONNECT,	aus_connect,	/* 235 connect */
		auf_null,	0,
aui_null,	AUE_SHUTDOWN,	aus_shutdown,	/* 236 shutdown */
		auf_null,	0,
aui_null,	AUE_NULL,	aus_null,	/* 237 recv */
		auf_null,	0,
aui_null,	AUE_RECVFROM,	aus_recvfrom,	/* 238 recvfrom */
		auf_recvfrom,	0,
aui_null,	AUE_RECVMSG,	aus_recvmsg,	/* 239 recvmsg */
		auf_recvmsg,	0,
aui_null,	AUE_NULL,	aus_null,	/* 240 send */
		auf_null,	0,
aui_null,	AUE_SENDMSG,	aus_sendmsg,	/* 241 sendmsg */
		auf_null,	0,
aui_null,	AUE_SENDTO,	aus_sendto,	/* 242 sendto */
		auf_null,	0,
...
aui_null,	AUE_NULL,	aus_null,	/* 245 getsockopt */
		auf_null,	0,
aui_null,	AUE_SETSOCKOPT,	aus_setsockopt,	/* 246 setsockopt */
		auf_null,	0,
aui_null,	AUE_NULL,	aus_null,	/* 247 sockconfig */
		auf_null,	0,
...
/*ARGSUSED*/
static void
aus_bsmsys(struct t_audit_data *tad)
{
	klwp_id_t clwp = ttolwp(curthread);
	struct file *fp;
	struct f_audit_data *fad;
	...
	case AUE_SETUSERAUDIT:
		au_uwrite(au_to_arg32(1, "auditID", a1));
		break;
	case AUE_AUDITSVC:
			/*
			 * convert file pointer to file descriptor
		...
		if (fad->fad_lpbuf) {
			...
			au_uwrite(au_to_arg32(1, "no path: fd", a1));
		}
		...
		au_uwrite(au_to_arg32(2, "limit", (uint)a2));
		break;
	case AUE_AUDITON:
		au_uwrite(au_to_arg32(1, "new audit state", a1));
		break;
	case AUE_AUDIT_SPOLICY:	/* use of privilege */
		if (copyin((caddr_t)a2,
				(caddr_t)&policy,
				sizeof (policy)))
			return;
		au_uwrite(au_to_arg32(1, "policy", policy));
		break;
...
auf_mknod(struct t_audit_data *tad, int error, rval_t *rval)
{
	...
	AS_INC(as_memused, (uint)(size + len));
	...
	*ptr++ = '/';
	...
	AS_DEC(as_memused, (uint)(size + len));
...
{	/* AUS_START */
	...
	if (strncmp(fstype, "nfs", 4) == 0) {
		...
		au_uwrite(au_to_arg32(3, "internal flags",
			(uint)STRUCT_FGET(nfsargs, flags)));
...
auf_symlink(struct t_audit_data *tad, int error, rval_t *rval)
{
	...
	link_len = (u_int)tad->tad_pathlen;
	...
	if ((!error) && (link_len)) {
		...
		path.pn_pathlen = (u_int)link_len - 1;	/* don't include \0 */
		path.pn_bufsize = (u_int)link_len;
...
#endif
	au_uwrite(au_to_arg32(3, "cmd", (uint)uap->cmd));
#ifdef _LP64
	au_uwrite(au_to_arg64(4, "arg", (uint64_t)uap->arg));
#else
	au_uwrite(au_to_arg32(4, "arg", (uint32_t)uap->arg));
#endif
	au_uwrite(au_to_arg32(5, "attr", (uint)uap->attr));
	au_uwrite(au_to_arg32(6, "mask", (uint)uap->mask));
}
...
aus_setgroups(struct t_audit_data *tad)
{
	...
	gidsetsize = (uint)uap->gidsetsize;
...
aus_sysinfo(struct t_audit_data *tad)
{
	...
	case SI_SET_KERB_REALM:
	{
		...
		maxlen = REALM_SZ;
		name = (char *)kmem_zalloc(maxlen, KM_SLEEP);
		if (copyinstr((caddr_t)buf, name, REALM_SZ, &len))
			break;
...
aus_modctl(struct t_audit_data *tad)
{
	...
	case AUE_MODLOAD: {
		...
		use_path = (uint)((modloada_t *)a)->use_path;
	...
	case AUE_MODCONFIG: {
		...
		if (copyin((caddr_t)data, (caddr_t)STRUCT_BUF(mc),
				STRUCT_SIZE(mc)) != 0)
			return;
		rootdir = STRUCT_FGET(mc, rootdir);
		if (rootdir[0] != NULL) {
				/* safety */
			rootdir[255] = '0';
			au_uwrite(au_to_text(rootdir));
		} else
	...
	rval_t	*rval)
{
	...
	if ((so = au_getsonode(fd, &err, NULL)) == NULL) {
		au_uwrite(au_to_arg32(1, "bad fd", (uint32_t)fd));
		return;
	}

	mutex_enter(&so->so_lock);
	(void) au_so_lock_single(so, SOLOCKED, 0);
	mutex_exit(&so->so_lock);

	so_state = so->so_state;
	so_laddr = so->so_laddr;

	mutex_enter(&so->so_lock);
	au_so_unlock_single(so, SOLOCKED);
	mutex_exit(&so->so_lock);

	if (so_state & SS_ISBOUND) {
	    if (so_laddr.soa_sa != NULL && so_laddr.soa_len != 0 &&
		so_laddr.soa_sa->sa_family == AF_INET)
		au_uwrite(au_to_sock_inet(
				(struct sockaddr_in *)(so_laddr.soa_sa)));
	    else
		au_uwrite(au_to_arg32(1, "fd", (uint32_t)fd));
	} else
		au_uwrite(au_to_arg32(1, "fd", (uint32_t)fd));
	...
	if (copyin((caddr_t)(uap->len), (caddr_t)&len, sizeof (len)) != 0)
		return;

	if (copyin((caddr_t)(uap->addr), (caddr_t)&addr, len) != 0)
		return;

	if (addr.sin_family == AF_INET)
		au_uwrite(au_to_sock_inet(&addr));
}


/*ARGSUSED*/
static void
aus_bind(struct t_audit_data *tad)
{
	struct a {
		long	fd;
		long	addr;
	...
	if (au_getsonode(fd, &err, NULL) == NULL) {
		au_uwrite(au_to_arg32(1, "bad fd", (uint32_t)fd));
		goto bind_addr;
	}
...
bind_addr:
	if (copyin((caddr_t)(uap->addr), (caddr_t)&addr, uap->len) != 0)
		return;
...
/*ARGSUSED*/
static void
aus_connect(struct t_audit_data *tad)
{
	struct a {
		long	fd;
		long	addr;
...
connect_addr:
	if (uap->addr == NULL) {
		au_uwrite(au_to_arg32(2, "addr", (uint32_t)0));
		return;
	}
...
aus_shutdown(struct t_audit_data *tad)
{
	...
	struct sonode *so;
	struct soaddr so_laddr, so_faddr;
	u_int so_state;
	int err, fd;
	...
	if (so_state & SS_ISCONNECTED) {
	    if (so_faddr.soa_sa != NULL && so_faddr.soa_len != 0 &&
		so_faddr.soa_sa->sa_family == AF_INET)
		au_uwrite(au_to_sock_inet(
				(struct sockaddr_in *)&(so_faddr.soa_sa)));
	}
	RELEASEF(fd);
...
setsockopt_args:
	...
	if (copyin((caddr_t)(uap->optval), (caddr_t)&val, uap->optlen) == 0)
		au_uwrite(au_to_arg32(4, "optval", (uint32_t)val));
...
/*ARGSUSED*/
static void
aus_recvmsg(struct t_audit_data *tad)
{
	struct a {
		long	fd;
	...
	rval_t *rvp)
{
	...
	if (msg_name == NULL) {
		au_uwrite(au_to_arg32(2, "msg->msg_name", (uint32_t)0));
		return;
	} else {
		msg_namelen = (socklen_t)STRUCT_FGET(msg, msg_namelen);
		name = (struct sockaddr *)kmem_alloc(msg_namelen, KM_SLEEP);
		if (copyin(msg_name, (caddr_t)name, msg_namelen) != 0) {
			kmem_free(name, msg_namelen);
			return;
		}
	}

	if (name->sa_family == AF_INET)
		au_uwrite(au_to_sock_inet((struct sockaddr_in *)name));
...
/*ARGSUSED*/
static void
aus_recvfrom(struct t_audit_data *tad)
{
	struct a {
		long	fd;
		long	msg;	/* char */
	...
	au_uwrite(au_to_arg32(3, "len", (uint32_t)(uap->len)));
	au_uwrite(au_to_arg32(4, "flags", (uint32_t)(uap->flags)));
}
	...
	rval_t *rvp)
{
	...
	if (uap->from == NULL) {
		au_uwrite(au_to_arg32(5, "from", (uint32_t)0));
		return;
	} else {
		if (fromlen == 0)
			return;
		name = (struct sockaddr *)kmem_alloc(fromlen,
							KM_SLEEP);
		if (copyin((caddr_t)(uap->from), (caddr_t)name,
				fromlen) != 0) {
			kmem_free(name, fromlen);
			return;
		}
	}
	...
	kmem_free(name, fromlen);
	au_uwrite(au_to_arg32(6, "fromlen", (uint32_t)fromlen));
}

/*ARGSUSED*/
static void
aus_sendmsg(struct t_audit_data *tad)
{
	struct a {
		long	fd;
		long	msg;	/* struct msghdr */
...
/*ARGSUSED*/
static void
aus_sendto(struct t_audit_data *tad)
{
	struct a {
		long	fd;
		long	msg;	/* char */
	...
	au_uwrite(au_to_arg32(3, "len", (uint32_t)(uap->len)));
	au_uwrite(au_to_arg32(4, "flags", (uint32_t)(uap->flags)));
	au_uwrite(au_to_arg32(6, "tolen", (uint32_t)(uap->tolen)));
	...
	if (uap->tolen == 0) {
		...
		name = (struct sockaddr *)kmem_alloc(uap->tolen, KM_SLEEP);
		if (copyin((caddr_t)(uap->to), (caddr_t)name,
				uap->tolen) != 0) {
			kmem_free(name, uap->tolen);
			return;
		}
	}

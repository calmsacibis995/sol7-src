...
#pragma ident   "@(#)audit_mem.c 1.20     97/10/29 SMI"
 ...
 */
au_append_buf(char *data, int len, au_buff_t *buf)
{
	au_buff_t *new_buf;
	int	new_len;
	...
	new_len = (u_int) (buf->len + len) > AU_BUFSIZE ?
		AU_BUFSIZE - buf->len : len;
	bcopy(data, (buf->buf + buf->len), (u_int) new_len);
	buf->len += (u_char) new_len;
	len -= new_len;
	...
	while (len > NULL) {
		...
		bcopy(data, buf->buf, (u_int) new_len);
		buf->len = (u_char) new_len;
		len -= new_len;
	}

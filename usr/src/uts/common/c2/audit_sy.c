...
#pragma	ident	"@(#)audit_syscalls.c	1.51	97/11/12 SMI"
...
getaudit(caddr_t info_p)
{
	...
	STRUCT_FSET(info, ai_termid.port, DEVCMPL(pad->pad_termid.port));
	STRUCT_FSET(info, ai_termid.machine, pad->pad_termid.machine);
...
setaudit(caddr_t info_p)
{
	...
	pad->pad_termid.port = DEVEXPL(STRUCT_FGET(info, ai_termid.port));
	pad->pad_termid.machine = STRUCT_FGET(info, ai_termid.machine);
...
getpinfo(caddr_t data)
{
	...
	STRUCT_FSET(apinfo, ap_termid.port, DEVCMPL(pad->pad_termid.port));
	STRUCT_FSET(apinfo, ap_termid.machine, pad->pad_termid.machine);
...

{
	...
	int		nfound;	/* count of file entries found */
	struct file	*fp;	/* pointer to file table entry */
	struct inpcb	*pcb;	/* pointer to pcb found */
	struct inpcb	*head;	/* head of pcb chang to look up */
	struct socket	*sp;	/* pointer to socket to match */

...
#pragma ident	"@(#)cpr_mod.c	1.65	98/01/07 SMI"
...
cpr(int fcn)
{
	...
	case AD_CPR_REUSEINIT:
		...
		if (!cpr_statefile_is_spec()) {
			cmn_err(CE_CONT, "cpr: statefile must be on block "
			    "special device for reusable statefile.\nSee "
			    "power.conf(4) and pmconfig(1M).\n");
			return (EINVAL);
		}
		if ((rc = cpr_check_spec_statefile()) != 0)
			return (rc);
		if (swapinfo) {
			cmn_err(CE_CONT, "cpr: reusable statefile requires "
	...
	case AD_CPR_FORCE:
		if (cpr_reusable_mode) {
			cmn_err(CE_CONT, "cpr: cannot do normal "
			    "checkpoint/resume when in reusable statefile "
			    "mode\nuadmin A_FREEZE AD_REUSEFINI (uadmin %d %d)"
			    " exits reusable statefile mode.\n", A_FREEZE,
			    AD_REUSEFINI);
			return (ENOTSUP);
		}
		cpr_test_mode = 0;
	...
	case AD_CPR_CHECK:
		if (!cpr_is_supported() || cpr_reusable_mode)
			return (ENOTSUP);
		return (0);

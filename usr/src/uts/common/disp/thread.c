...
#pragma ident	"@(#)thread.c	1.95	98/02/01 SMI"
	...
	int pri)
{
	 ...
	 */
	ts = kmem_zalloc(sizeof (turnstile_t), (state == TS_ONPROC) ?
	    KM_NOSLEEP : KM_SLEEP);
	...
	if (stk == NULL) {
		...
		if (stk == NULL) {
			kmem_free(ts, sizeof (turnstile_t));
			return (NULL);
		}
		/*
...
thread_free(kthread_id_t t)
{
	...
	kmem_free(t->t_ts, sizeof (turnstile_t));
...
void
forkctx(kthread_id_t t, kthread_id_t ct)
{
	struct ctxop *ctx;

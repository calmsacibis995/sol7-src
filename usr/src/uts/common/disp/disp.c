...
#pragma ident	"@(#)disp.c	1.136	98/02/01 SMI"	/* from SVr4.0 1.30 */
...
dispinit(void)
{
	...
	for (cid = 0; cid < nclass; cid++) {
		...
		if (SCHED_INSTALLED(sc)) {
			(*sc->cl_init)(cid, PC_CLPARMSZ, &sc->cl_funcs,
			    &cl_maxglobpri, &sc->cl_size);
...
disp_getwork(cpu_t *cp)
{
	...
	for (ocp = cp->cpu_next_part; ocp != cp; ocp = ocp->cpu_next_part) {
		...
		/*
		 * Don't take thread if it is the only runnable thread and
		 * the CPU has been switching threads reasonably often
		 * (arbitrarily set to "in the last second").  If the CPU
		 * is just handling an interrupt, taking the thread might
		 * cause it to experience unnecessary cache misses.
		 */
		nrun = ocp->cpu_disp.disp_nrunnable;
		if (nrun == 0 ||
		    (nrun == 1 && curtime - ocp->cpu_last_swtch < hz))
			continue;

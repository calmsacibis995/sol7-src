...
#pragma ident	"@(#)cpr_readfile.c	1.16	98/01/07 SMI"
...
extern int cpr_ufs_read(int, caddr_t, int);
...
static func_t
readelf(int fd, int verbosemode, Elf64_Ehdr *elfhdrp)
{
	...
	if (prom_seek(fd, elfhdrp->e_phoff) == -1)
		goto elferror;
	if (cpr_ufs_read(fd, allphdrs, phdrsize) != phdrsize)
		goto elferror;
	...
	for (i = 0; i < nphdrs; i++) {
		...
		nhdr = (Elf64_Nhdr *)prom_alloc((caddr_t)nhdr,
		    phdr->p_filesz, 1);
		if (nhdr == NULL)
			goto elferror;
		if (verbosemode)
		...
		if (cpr_ufs_read(fd, (caddr_t)nhdr,
	...
	for (i = 0; i < nphdrs; i++) {
		...
		if (phdr->p_type == PT_LOAD) {
			...
			while (left_to_do) {
				...
				if ((rc = cpr_ufs_read(fd,
				    (char *)base, rsize)) != rsize) {
...
readfile(int fd, int verbosemode)
{
	...
	nread = cpr_ufs_read(fd, (char *)&elfhdr, sizeof (elfhdr));
	if (nread != sizeof (elfhdr)) {
		errp("Error reading ELF header.\n");
		return (FAIL);
	}

	elfmagic = *(int *)elfhdr.e_ident;
	if (elfmagic != *(int *)ELFMAG) {
		DEBUG4(errp("File not executable, elfmagic = 0x%x\n",
		    elfmagic));
		return (FAIL);
	}

	if (elfhdr.e_ident[EI_CLASS] != ELFCLASS64) {
		errp("bad EI_CLASS, expect %d, found %d\n",
		    ELFCLASS64, elfhdr.e_ident[EI_CLASS]);
		return (FAIL);
	}

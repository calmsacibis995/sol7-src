...
#ident "@(#)dosemul.c	1.44	98/02/11 SMI"
...
struct rpcopy {
	...
	unsigned short flags;	/* reserved for future use */
};
...
dospcopy(struct real_regs *rp)
{
	 ...
	 */
	pagestop();
	bcopy((char *)p->src, (char *)p->dest, p->nbytes);
	pagestart();
...
/*
 * dosdebug - absolutely minimal debugger for boot panic time.
 * This is required to debug the many field failures at beta. It
 * should be replaced by a usable subset of kadb commands asap.
 */
void
dosdebug(struct real_regs *rp)
{
	char			*p, *ep;
	char			cmd[80];
	static unsigned long	*base = 0;
	static unsigned long 	cnt = 32;
	static unsigned long 	tcnt;
	extern void		dregister(long, long);
	extern	unsigned long 	pp_table;
	unsigned long		*rbase = (unsigned long *)&pp_table;
	int 		i;
	char *rname[] = {
		"gs",
		"fs",
	...
	while (cmd[0] != 'q' && strcmp(cmd, ":c")) {
		...
		case 'h': 	/* help */
		case '?':
			printf("p - dump boot panic registers\n");
			printf("s - dump current stack pointer\n");
			printf("d <addr> <val> - dump longs\n");
			printf("W <addr> <val> - write long\n");
			printf("w <addr> <val> - write short\n");
			printf("b <dr0> <dr7> - set one-shot debug register\n");
			break;
		case 'W': 	/* W addr val - write long */
		case 'w': 	/* w addr val - write short */
		case 'd': 	/* d addr <len> - dump long */
		case 'b': 	/* b addr dr7 bits - breakpoint addr dr7 */
			p++;
			while (*p && (*p == ' ' || *p == '\t'))
				p++;
			ep = p+1;
			...
			if (*p) {
				base = (unsigned long *)strtoul(p, 0L, 16);
				base = (unsigned long *)
			...
			if (*cmd == 'W') {
				...
				*(unsigned short *)base = (unsigned short)tcnt;
				break;
			} else if (*cmd == 'b') {
				dregister((long)base, tcnt);
				break;
			} else if (tcnt != 0 && tcnt < 0x10000)
				cnt = tcnt;
			...
			break;
		case 'p': 	/* print panic registers */
			for (i = 0; rname[i]; i++) {
				if (i%4 == 0)
					printf("\n%x: ", &rbase[i]);
				printf("%s %8x ", rname[i], rbase[i]);
			}
			break;
		case 's': 	/* print current stack ptr */
				printf("\n%x: ", &rp);
			break;
		}
		printf("\n");
...
handle21(struct real_regs *rp)
{
	...
	case 0xfe:	/* rw protected memory range */

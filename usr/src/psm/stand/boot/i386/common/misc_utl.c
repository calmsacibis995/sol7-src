...
#pragma ident "@(#)misc_utls.c 1.37       98/02/12 SMI"
...
#define	NORM_ATTRIB ((BLACK << 4)|WHITE)
...
#undef SET_BACKGROUND
	...
	if (c != '\r' && c != '\n' && c != '\007' && c != '\b') {
		ic.ax = 0x0900 | c;
		ic.bx = (ushort)console_color_attr;
		ic.cx = 0x0001;
		ic.intval = 0x10;
		(void) doint_r(&ic);
	}
...
serial_direct_init(int Port, int port_vals)
{
	 ...
	 * OUTB's can go to the writter of that driver.
	 */
...
serial_init(char *name, int port, int port_vals, int port_flags)
{
	...
	/*
	 * if a port is marked as direct, or baud rate is
	 * above 9600, set direct console.
	 */
	if ((port_vals & SDIRECT) || (port_vals & 0x3e0) > S9600) {
		asy_direct = port == 0 ? 0x3f8 : 0x2f8;
	}
	...
	if (asy_direct) {
		s->cookie = (char *)asy_direct;
		serial_direct_init(asy_direct, port_vals);
	} else {

		ic.ax = (ushort)port_vals & 0xff;
		ic.dx = (ushort)port;
		ic.intval = 0x14;
...
serial_putc(_char_io_p p, char c)
{
	if (asy_direct) {
		...
		while (((INB(asy_direct, LSR) & XHRE) == 0) && checks--)
			;
		OUTB(asy_direct, DAT, c);
	} else {
		struct int_pb	ic;
		...
		if ((lsr = INB(asy_direct, LSR)) & RCA) {
			if (lsr & (SERIAL_BREAK | SERIAL_FRAME |
				...
				if (lsr & SERIAL_OVERRUN) {
					...
					return (INB(asy_direct, DAT));
				} else {
					/* Toss the garbage */
					(void) INB(asy_direct, DAT);
					return (0);
				}
			} else {
				p->in++;

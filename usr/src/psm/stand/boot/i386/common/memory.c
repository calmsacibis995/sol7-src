...
#ident "@(#)memory.c	1.20	98/02/11 SMI"
...
struct	bootmem64 mem64avail[B_MAXMEM64];
int	mem64availcnt = 0;
...
init_paging(void)
{
	 ...
	 */
	pgs = (u_int)magic_phys/MMU_PAGESIZE;
	...
	pdp->page[0] = ((uint)ptp) | (MMU_PFEC_P| MMU_PFEC_WRITE);
...
peeks(unsigned short *addr)
{
	 ...
	 * temporarily map in the zeroth page of memory so we can
	 * see the page without generating a page fault.
	 */
	extern uint bpt_loc;
	ushort  rv;
	ptbl_t	*ptp;
...
pokes(unsigned short *addr, unsigned short val)
{
	 ...
	 * So we temporarily map in the zeroth page of memory so we can
	 * see the page without generating a page fault.
	 */
	extern uint bpt_loc;
	ptbl_t	*ptp;
...
rm_check()
{
	...
	top_realmem = (caddr_t)(peeks((ushort *)MEMBASE_ADDR) * BOOTMEMBLKSIZE);
...
caddr_t
rm_malloc(size_t size, u_int align, caddr_t virt)
{
	/* BEGIN CSTYLED */
	/*
	...
	if ((vad >= bot_realmem) && ((vad+size) <= top_realmem)) {
		...
		if ((vad+size) == top_realmem) {
			...
			pokes((ushort *)MEMBASE_ADDR,
				(ushort)(v/BOOTMEMBLKSIZE));
...
rm_resize(caddr_t vad, size_t oldsz, size_t newsz)
{
	...
	if ((dif = newsz - oldsz) <= 0) {
		...
		top_realmem = (caddr_t)(peeks((ushort *)MEMBASE_ADDR) *
		    BOOTMEMBLKSIZE);
...
setup_memlist(int type, struct bootmem *bmem, int cnt)
{
	...
	/* append >4G memory ranges to the end of the regular memory ranges */
	if (mem64availcnt > 0 && (type == MEM_AVAIL || type == MEM_INSTALLED)) {
		for (bmem64 = &mem64avail[0], i = 0; i < mem64availcnt;
			i++, bmem64++) {
			this = (struct memlist *)
				rm_malloc(sizeof (struct memlist), 0, 0);
			if (this == (struct memlist *)0) {
				return (-1);
			}
			if (!head) {
			...
			this->address = bmem64->base;
			this->size = bmem64->extent;
			this->prev = prev;
			if (prev)
				prev->next = this;
			this->next = (struct memlist *)0;
...
setup_memlists(void)
{
	...
	bvirtlist.size = (u_int)magic_phys;
	bvirtlist.next = 0;
	...
	for (mlp = pinstalledp; mlp && mlp->address < (u_int)magic_phys;
							    mlp = mlp->next) {
		/*
		...
		if (this =
			 ...
			 */
			if ((this->size = (u_int)magic_phys - mlp->address) >
			    mlp->size) {
				/*
...
{
	return ((caddr_t)map_mem((u_int)vaddr, size, align));
}

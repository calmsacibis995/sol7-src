...
#ident "@(#)devtree.c	1.19	97/05/06 SMI"
...
bmknod(struct bootops *bop, phandle_t node)
{
	...
	if (dnp != (struct dnode *)0) {
		...
		/*
		 * don't touch the odering of the nodes. insert the new
		 ...
		 */
		dnp->dn_peer = (struct dnode *)0;
		if (((struct dnode *)node)->dn_child == (struct dnode *)0)
			((struct dnode *)node)->dn_child = dnp;
		else {
			struct dnode *lp = ((struct dnode *)node)->dn_child;
			while (lp->dn_peer)
				lp = lp->dn_peer;
			lp->dn_peer = dnp;
		}

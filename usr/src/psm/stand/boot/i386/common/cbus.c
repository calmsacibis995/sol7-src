...
#pragma ident	"@(#)cbus.c	1.10	97/06/09 SMI"
...
)
{
	...
	do {
		...
		finishup:
			...
			psm_unmap_phys((PVOID)Bios, 0x8000);
			CbusMemToSolaris(mrp, minaddr, cnt, maxcnt, flags);
				return (TRUE);
		default:
			/*
...
/*
 * Parse our memory table and generate booter memory structure.
 *
 */

void
CbusMemToSolaris(
struct bootmem *mrp,	/* place in array to start */
unsigned long minaddr,	/* minimum address to describe */
int *cnt,		/* total number of regions */
int maxcnt,		/* maximum allowed regions */
ushort flags		/* type of region */
)
{
	int		i, j;
	ULONG		Address, Size, a1, a2;
	PEXTRA_MEMORY	Descriptor;
	EXTRA_MEMORY	tmp;

	/*
	 * Sort the Cbus memory descriptors by address.
	 */
	for (i = 0; i < CbusExtraMemoryIndex - 1; i++) {
		for (j = i + 1; j < CbusExtraMemoryIndex; j++) {
			a1 = CbusExtraMemory[i].BaseAddress;
			a2 = CbusExtraMemory[j].BaseAddress;
			if (a2 < a1) {
				tmp = CbusExtraMemory[i];
				CbusExtraMemory[i] = CbusExtraMemory[j];
				CbusExtraMemory[j] = tmp;
			}
		}
	}

	/*
	 * Generate bootmem structures. Addresses below minaddr
	 * are already setup.
	 */
	for (i = 0; i < CbusExtraMemoryIndex; i++) {
		...
		/*
		 * Ignore any regions contained below minaddr
		 */
		if (Address < minaddr && (Address + Size) <= minaddr) {
			continue;
		}

		/*
		 * Trim back a region if it straddles minaddr
		 */
		if (Address < minaddr) {
			Size -= (minaddr - Address);
			Address = minaddr;
		}
		...
		if (*cnt < maxcnt) {
			...
			*cnt += 1;
			...
			printf("CbusMemtoSolaris: "
			"Too many segments - some memory lost\n");
			return;
		}
	}
}

...
#ident "@(#)load.c	1.26	97/11/25 SMI"
...
/*
 *  getnextpath
 *	Given pointer to a pointer into colon separated, null terminated
 *	path string; return pointer to first path, and modify original
 *	pointer to point where to start the next search.
 */
...
getnextpath(char **pp)
{
	...
	if (!nxtpthbuf && !(nxtpthbuf = (char *)bkmem_alloc(MAXPATHLEN))) {
		/* Can't do much without a path buffer! */
	...
	if (((s = *pp) != 0) && *s) {
		...
		*pp = s;
		...
		/*
		 * The path '.' or '' implies no extra path information.
		 * Return an empty path in these cases.
		 *
		 * If we don't have a trailing / on the path; then
		 * we need to add one if possible.
		 */
		if (l == 0 || (strcmp(nxtpthbuf, ".") == 0))
			nxtpthbuf[0] = '\0';
		else if ((nxtpthbuf[l-1] != '/') && (l < MAXPATHLEN-2))
			(void) strcat(nxtpthbuf, "/");
	}
	if (loaddebug)
		printf("Nextpath is %s\n", (rp == NULL) ? "(NULL)" : rp);
	return (rp);
}
...
findnrun(int (*loader)(), char *name, int argc, char *argv[], int prt)
{
	...
	if ((*name == '/') || (volume_specified(name)) ||
	    ((npathp = (char *)var_ops(PATHVAR, NULL, FIND_VAR)) == NULL) ||
	    !(*npathp)) {
		/* Absolute pathname, no path set, null path set */
		(void) strcpy(runpath, name);
		success = (*loader)(runpath, argc, argv, prt);
	} else {
		/* Loop through paths until one works */
		while ((pathp = getnextpath(&npathp)) != NULL) {
			int totlen;
			totlen = strlen(pathp) + strlen(name) + 1;
			if (totlen > MAXPATHLEN) {
				if (prt) {
					printf("Run Error: ");
					printf("Maximum run command ");
					printf("path length exceeded.\n");
				}
				continue;
			}
			(void) strcpy(runpath, pathp);
			(void) strcat(runpath, name);
			if (prt && loaddebug)
				printf("Attempt: %s + %s ?= %s\n",
				    pathp, name, runpath);
			success = (*loader)(runpath, argc, argv, prt);
			if (success > RUN_NOTFOUND)
				break;
		}
	}

...
#ident "@(#)ix_alts.c	1.24	97/03/07 SMI"
...
extern int read_sectors(struct ihandle *ihp, daddr_t off, int cnt);
...
get_vtoc(struct ihandle *ihp, int prt)
{
	...
	/* NOTE: read_sectors returns -1 when it SUCCEEDS! */
	if (read_sectors(ihp, (daddr_t)0, 1) != -1)
		goto ex;
	 ...
	 * Solaris partition.
	 */
	if (((struct mboot *)ihp->cachep)->signature == MBB_MAGIC) {
		int k = ihp->dev.disk.num;
		int j = ((k >= 0) ? 0 : FD_NUMPART);
		struct ipart fpt[FD_NUMPART];

		/*
		 * There's a DOS fat signature on this disk, which
		...
		(void) memcpy(&fpt, &((struct mboot *)ihp->cachep)->parts,
	    ...
	    (read_sectors(ihp, cd2hd(ISO_VOLDESC_SEC), cd2hd(1)) == -1)) {
		/*
		 * We were able to read an ISO_9660 id sector from
		 * the disk.  Make sure it's really a CDROM!
		 */
		if ((ISO_DESC_TYPE(ihp->cachep) == ISO_VD_PVD) &&
		    strncmp((char *)ISO_std_id(ihp->cachep), ISO_ID_STRING,
			ISO_ID_STRLEN) == 0 &&
		    ISO_STD_VER(ihp->cachep) == ISO_ID_VER) {
			/*
 ...
 *
 * This routine behaves much like the "read_sectors" routine
...
bd_readalts(struct ihandle *ihp, daddr_t off, int cnt)
{
	...
	if (ihp->dev.disk.alt && (dbp = build_map(ihp, off, cnt))) {
		 ...
		 * calls to "read_sectors" based on the number of "dbp"
		 ...
		 */
		(void) memcpy(ihp->cachep, track, cnt * ihp->dev.disk.bps);
		bkmem_free(buf, cnt * sizeof (struct d_blk));
		bkmem_free(track, cnt * ihp->dev.disk.bps);
		return (rc);
	}

	return (read_sectors(ihp, off + ihp->dev.disk.bas, cnt));
}

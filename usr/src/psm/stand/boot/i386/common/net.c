...
net_buffer(struct ihandle *ihp, caddr_t buf, int len)
{
	...
	if ((buf+len) > (caddr_t)USER_START) {
		 ...
		 *  directly.  Let's see if we can obtain an intermediate
		 *  buffer to use in its place.
		 */
		if (len > ihp->cfirst) {
			/*
			 *  If caller needs a buffer that's bigger than the
			 *  one we've got reserved, see if we can resize ...
			 */
			if (ihp->cachep != (caddr_t)0) {
				/*
				 *  Since we don't have a "rm_realloc" we have
				 *  to do this the stupid way.  Free up any
				 *  buffer that we might have already had
				 *  lying around before allocating a new one.
				 */
				rm_free(ihp->cachep, ihp->cfirst);
				ihp->cachep = buf = 0;
				ihp->cfirst = 0;
			}

			if (ihp->cachep = rm_malloc(len, 0, 0)) {
				/*
				 *  Buffer resize completed successfully.  Save
				 *  the new buffer length so we'll know how big
				 *  it is the next time we're asked to use it.
				 */
				ihp->cfirst = len;
			}
		}
...
close_net(struct ihandle *ihp)
{
	 ...
	 *
	 *  If we allocated a low-memory transfer buffer, free it up.
	 */

	if (ihp->cachep) {
		/*
		 *  Free the low-memory transfer buffer that we seem to have
		 *  acquired along the way.
		 */

		rm_free(ihp->cachep, ihp->cfirst);
		ihp->cachep = (caddr_t)0;
		ihp->cfirst = 0;
	}
}

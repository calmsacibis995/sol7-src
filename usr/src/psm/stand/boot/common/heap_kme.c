/*
 * Copyright (c) 1988-1994,1996, by Sun Microsystems, Inc.
 */

#pragma	ident	"@(#)heap_kmem.c	1.31	97/06/30 SMI" /* From SunOS 4.1 1.10 */
...
caddr_t		kmem_alloc(size_t);
void		kmem_free(caddr_t ptr, size_t nbytes);
Freehdr		getfreehdr(void);
bool		morecore(size_t);
...
#ifdef DEBUG
	printf("kmem_alloc  bzero complete - returning %x\n", retblock);
#endif
	return (retblock);
 ...
 */
void
kmem_free(caddr_t ptr, size_t nbytes)
{
	...
	while (neighbor != NIL) {
		...
		if (ptr < neigh_block) {
			if (ptr + nbytes == neigh_block) {
				 ...
				 * is bad news.
				 */
				printf("kmem_free: free block overlap %x+%d"
...
void
bkmem_free(caddr_t p, size_t bytes)
{
	kmem_free(p, bytes);
}
...
morecore(size_t nbytes)
{
	...
	p = (Dblk) resalloc(RES_BOOTSCRATCH, nbytes, (caddr_t)0, 0);
	if (p == 0) {
		return (false);
	}
	kmem_free((caddr_t)p, nbytes);
#ifdef DEBUG
	printf("morecore() returing, p = %x\n", p);
#endif
	return (true);
...
getfreehdr(void)
{
	...
	if (kmem_info.free_hdr_list != NIL) {
		...
		r = (Freehdr)resalloc(RES_BOOTSCRATCH, CLSIZE, (caddr_t)0, 0);
		if (r == 0) {

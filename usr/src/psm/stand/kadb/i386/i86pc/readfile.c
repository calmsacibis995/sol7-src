...
#pragma ident	"@(#)readfile_i386.c	1.25	98/02/12 SMI"
...
static	caddr_t sav_kadb_heaptr;	/* what a name! */
extern char *prompt;
extern char *strrchr();
...
char bootname[80];	/* space for "/unix" and room to patch to whatever */
	...
	register char **arg;
{
	...
	*arg = "";
	...
	if (BOP_GETPROP(bopp, "boot-args", bargs) != -1)
	...
	if (BOP_GETPROP(bopp, "use-align", bargs) != -1)
		use_align = 1;
	 ...
	 */
	*aline = '\0';
	if (BOP_GETPROP(bopp, "default-name", aline) == -1) {
		/* no property set so we will use our own default */
		*aline = '\0';
		strcpy(aline, "kernel/unix");
	}
	if (interactive) {
		printf("%s: ", prompt);
		gets_p(aline);
	...
	*arg = target_bootname;
	(void) BOP_SETPROP(bopp, "whoami", target_bootname);
	(void) BOP_SETPROP(bopp, "boot-args", target_bootargs);

	io = open(target_bootname, 0);

	if (io >= 0) {
		go2 = readfile(io, 1, target_bootname);
	...
	char *name;
{
	...
	for (i = 0; i < nphdrs; i++) {
		...
		if (phdr->p_type == PT_LOAD) {
			...
			if (phdr->p_flags & PF_X) {
				...
				sav_kadb_heaptr = (caddr_t)vlimit(0);
				err = prom_alloc(loadaddr, size, align);
				if (err != (caddr_t)base)
					prom_panic("Unable to get memory "
					    "for text_seg.\n");
				vlimit(sav_kadb_heaptr);
	...
	if (interp) {
		...
		if (size > sizeof (auxv)) {
			printf("readfile: overrun of available aux vectors\n");
			goto elferr;
		}
		bv = elfbootvec;
		if ((bv->eb_un.eb_ptr =
			...
			return ((int (*)()) -1);
		bcopy(auxv, bv->eb_un.eb_ptr, size);
	...
	register char *cp;
{
	...
	if (cp && *cp == '-') {
		do {
			while (cp && *cp && *cp == ' ') /* pass over space */
				cp++;
			while (cp && *cp && *cp == '-') /* pass over - */
				cp++;
...
parseparam(char *line, int defaults, char *bootname, char *bootargs)
{
	...
	*bootargs++ = ' ';	/* kernel needs leading space */

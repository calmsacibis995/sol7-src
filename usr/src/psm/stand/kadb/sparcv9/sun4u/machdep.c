...
#pragma ident "@(#)machdep.c	1.6	97/12/10 SMI"
...
cmd()
{
	...
	if (addr > (int)etext && addr < (int)estack) {
		printf("cmd: fault inside debugger! pc=%lX, tt=%X\n",
		    (long) regsave.r_pc, regsave.r_tt);
		_exit();
	}
	...
	caddr_t sp;
{
	...
	flush_windows();
	tospage = (u_int)btopr((u_long)sp);
	fp = (struct frame *)sp;
	printf("Begin traceback... sp = %llx\n", sp);
	while (btopr((u_long)fp) == tospage) {
		if (fp == fp->fr_savfp) {
			printf("FP loop at %llx", fp);
			break;
		}
		printf("Called from %llx, fp=%llx, args=%llx %llx %llx %llx %llx %llx\n",
...
idle_other_cpus(void)
{
	...
	for (i = 0; i < NCPU; i++) {
		if (i != cpuid && cpu_nodeid[i] != -1) {
			...
			/* 
 			 * XXX LP64 kadb send_mondo may need to be changed 
			 * if cpusave and mpfpregs resides at 64 bit address 
			 */
...
u_longlong_t saved_t2;
u_longlong_t saved_wstate;
u_longlong_t saved_otherwin;
u_longlong_t saved_o0;
u_longlong_t saved_o6, saved_i6;
u_longlong_t saved_o7, saved_i7;
...
char debug_hook[] = " debugger-hook (see) ";
...
format_prom_callback(void)
{
	...
	(void) sprintf(prom_str, kadb_defer_word, &saved_pc, &saved_npc, &saved_g1,
	    &saved_g2, &saved_g3, &saved_g4, &saved_g5, &saved_g6,
	    &saved_g7, &saved_o0, &saved_i6, &saved_i7, &saved_tstate, &saved_t2,&saved_tt, &saved_tt2,&saved_wstate, &saved_otherwin,&saved_pil, trap);
...
/*
 * Initialize cpu_nodeid array since it's possible that some CPUs may have
 * been added or removed during DR operations.
 */

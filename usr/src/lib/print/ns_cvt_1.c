...
#pragma ident "@(#)ns_cvt_pconf.c	1.7	98/02/08 SMI"
...
_cvt_pconf_entry_to_printer(char *entry, char *source)
{
	...
	if ((cp = strchr(list[0], '|')) != NULL)
		*cp = '\0';

	if ((name = strdup(list[0])) == NULL)
		return (NULL);

	if (cp != NULL) {
		*cp = '|';
		namelist = strsplit(cp, "|");
	}

	if (list[1] == NULL) {  /* no attributes */
		free(list);
		free(name);
		if (namelist)
			free(namelist);
	...
	if ((printer = (ns_printer_t *)ns_printer_create(name, namelist,
				source, NULL)) != NULL)
		list_iterate((void **)&list[1], (VFUNC_T)_add_attribute, "=",
				printer);

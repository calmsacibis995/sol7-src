...
#pragma ident "@(#)job.c	1.12	97/12/05 SMI"
...
/*
 * cae_access(char *path, int mode) takes in a file and mode to determine
 * if the requested operation can be performed on the file by the real user.
 * access(2) get the vnode for the file and then checks the privilege.  This
 * means that for setuid processes, it really only checked the last component
 * of the path.  This routine walks back down the path checking directory
 * privilege at each level, which is my interpretation of what the CAE spec
 * says should happen.
 */
static int
cae_access(char *path, int mode)
{
	int result = 0;
	...
	do {
		result = access(tmp, mode);
		if ((cp = strrchr(tmp, '/')) != NULL)
			*cp = NULL;
		mode = (R_OK|X_OK);
	} while ((result == 0) && (cp != NULL) && (*tmp != NULL));
...
_job_alloc_id(char *printer, char *spool)
{
	...
	sprintf(buf, "%s/%s", spool, _sequence_file);
	...
	char *host)
{
	...
	sprintf(buf, "%s/%s%c%.3d%s", spool, prefix, *key, id, host);
	path = strrchr(buf, '/') + 1;
...
job_primative(job_t *job, char option, char *value)
{
	...
	if (strlen(value) > BUFSIZ - 2) {
		syslog(LOG_AUTH,
			"attempt to overrun buffer by %d, ignoring (%c%s)",
			getuid(), option, value);
		return (-1);
	}
	...
	if (cf->jf_data == NULL) {
		...
		memset(cf->jf_data, NULL, cf->jf_size + 1);
	} else
		cf->jf_data = realloc(cf->jf_data, cf->jf_size + 1);

	sprintf(buf, "%c%s\n", option, value);
	strcat(cf->jf_data, buf);
...
job_svr4_primative(job_t *job, char option, char *value)
{
	...
	if (strlen(value) > BUFSIZ - 3) {
		syslog(LOG_AUTH,
			"attempt to overrun buffer by %d, ignoring (5%c%s)",
			getuid(), option, value);
		return (-1);
	}

	sprintf(buf, "%c%s", option, value);
		    ...
		    int linked, int delete)
{
	...
	if (cae_access(path, R_OK) < 0)	{ /* can we read this file */
		int result = -1;
		gid_t gid = getgid(),
			egid = getegid();
	...
	memset(full_path, NULL, sizeof (full_path));
	if (path[0] != '/') {
...
job_destroy(job_t *job)
{
	...
	seteuid(get_user_id(job->job_user));	/* lose privilege temporarily */
...
job_create_binding_file(job_t *job, char **xfile)
{
	...
	src = malloc(strlen(job->job_spool_dir) + strlen(tmp) + 3);
	sprintf(src, "%s/%s", job->job_spool_dir, tmp);
	dst = malloc(strlen(job->job_spool_dir) + strlen(*xfile) + 3);
	sprintf(dst, "%s/%s", job->job_spool_dir, *xfile);
...
job_store(job_t *job)
{
	...
	sprintf(buf, "%s:%s\n", job->job_server, job->job_printer);
	if (write(lock, buf, strlen(buf)) < 0)
		return (-1);

	/* create the control_file */
	sprintf(buf, "%s/%s", job->job_spool_dir, cf->jf_spl_path);
...
job_retrieve(char *xFile, char *spool)
{
	...
	if ((tmp = (job_t *)malloc(sizeof (*tmp))) != NULL) {
		...
		strcpy(buf, xFile + strlen(_xfer_file_prefix) + 1);
		buf[3] = NULL;
		tmp->job_id = atoi(buf);
		...
		strcpy(buf, _control_file_prefix);
		strcat(buf, xFile + strlen(_xfer_file_prefix));
 ...
 */
job_t **
job_list_append(job_t **list, char *printer, char *spool)
{
	struct dirent *d;
	DIR	*dirp;
	job_t	*job;
	...
	syslog(LOG_DEBUG, "job_list_append(0x%x, %s)", list,
	...
	while ((d = readdir(dirp)) != NULL) {
		...
		if ((job = job_retrieve(d->d_name, spool)) == NULL)
			continue;
		syslog(LOG_DEBUG, "job_printer is (%s)", job->job_printer);
		if ((printer == NULL) ||
	...
	if (((strcmp(user, "root") == 0) ||
		...
		if (list_iterate((void **)list,
			...
			syslog(LOG_DEBUG, "\t%s-%d: canceled\n", printer,
				job->job_id);
			printf((char *)gettext("\t%s-%d: canceled\n"), printer,
				(int)job->job_id);
			close(lock);
		}
	}

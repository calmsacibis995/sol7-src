...
#pragma ident	"@(#)deflt.c	1.10	97/07/16 SMI"	/* SVr4.0 1.1	*/
/* LINTLIBRARY */
...
static void strlower(char *, char *);
static void strnlower(char *, char *, size_t);
...
static char *
_get_buf(thread_key_t *key)
{
	char *buf = NULL;

	if (_thr_getspecific(*key, (void **)&buf) != 0) {
...
defread(char *cp)
{
	...
	char *buf = _get_buf(&key);
#else /* _REENTRANT */
	static char buf[CHARS];	/* M002 */
#endif _REENTRANT
	size_t len;
	size_t patlen;
...
static void
strnlower(char *from, char *to, size_t cnt)
{
	int  ch;

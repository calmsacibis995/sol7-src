#ident	"@(#)simple_client.c	1.8	97/11/25 SMI"
/*
...
char **argv;
{
	...
	if (bind(sock, (struct sockaddr *)&c_sock,
			(socklen_t)sizeof(c_sock)) < 0) {
	...
	i = (int)sendto(sock, ktxt->dat, (size_t)ktxt->length, flags,
		   (struct sockaddr *)&s_sock, (socklen_t)sizeof(s_sock));
	if (i < 0)
	...
	/* Get my address */
	bzero((char *) &c_sock, sizeof(c_sock));
	slen = (socklen_t)sizeof(c_sock);
	if (getsockname(sock, (struct sockaddr *)&c_sock, &slen) < 0) {
		perror("getsockname");
		exit(1);
	}
	...
	len = krb_mk_safe((u_char *)MSG, ktxt->dat, strlen(MSG)+1,
	...
	/* Send it */
	i = (int)sendto(sock, ktxt->dat, (size_t)len, flags,
		   (struct sockaddr *)&s_sock, (socklen_t)sizeof(s_sock));
	if (i < 0)

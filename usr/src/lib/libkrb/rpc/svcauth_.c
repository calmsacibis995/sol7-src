#ident	"@(#)svcauth_kerb.c	1.7	97/11/25 SMI"
 ...
 */
struct {
	u_int ncachehits;	/* times cache hit, and is not replay */
	u_int ncachereplays;	/* times cache hit, and is replay */
	u_int ncachemisses;	/* times cache missed */
} svcauthkerb_stats;
	...
	register struct rpc_msg *msg;
{
	...
	struct mycred *mcp;
	extern key_sched(), cbc_encrypt(), des_ecb_encrypt();
	 ...
	 * Get the credential
	 */
	ixdr = (int *)msg->rm_call.cb_cred.oa_base;
	...
	case AKN_FULLNAME:
		...
		window = (u_int)*ixdr++;	/* get the window */
	 ...
	 * Get the verifier
	 */
	ixdr = (int *)msg->rm_call.cb_verf.oa_base;
	verf.akv_xtimestamp.key.high = (u_int)*ixdr++;
	verf.akv_xtimestamp.key.low =  (u_int)*ixdr++;
	verf.akv_winverf = (u_int)*ixdr++;
...
_svcauth_kerb: window %d wverf %d timestamp %d %d\n",
		...
		if ((u_int)timestamp.tv_usec >= USEC_PER_SEC) {
			debug("invalid usecs");
...
/* XXX */		_kmsgout("*** kerberos ticket expired - rejecting");
	 ...
	 */
	ixdr = (int *)cryptbuf;
	IXDR_PUT_INT32(ixdr, (int)timestamp.tv_sec - 1);
	IXDR_PUT_INT32(ixdr, (int)timestamp.tv_usec);
	 ...
	 */
	ixdr = (int *)msg->rm_call.cb_verf.oa_base;
	*ixdr++ = (int)verf.akv_xtimestamp.key.high;
	*ixdr++ = (int)verf.akv_xtimestamp.key.low;
	*ixdr++ = (int)verf.akv_nickname;
...
register gid_t *groups;		/* gid_t groups[NGROUPS_UMAX]; */
{
	register int i;
	register struct cache_entry *entry;
	struct bsdcred *cred;
	struct passwd *pw;
	u_int sid;				/* nickname into cache */
...
gid_t *groups;		/* array to initialize */

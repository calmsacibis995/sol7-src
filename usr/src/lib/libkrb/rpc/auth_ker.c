#ident	"@(#)auth_kerb.c	1.11	97/12/17 SMI"
/*
...
static int Kerr;
static bool_t authkerb_refresh();
	...
	XDR *xdrs;
{
	...
	extern bool_t xdr_authkerb_cred(), xdr_authkerb_verf();
	...
	struct opaque_auth *rverf;
{
	...
	ixdr = (u_int *)rverf->oa_base;
	buf.key.high = (u_int)*ixdr++;
	buf.key.low = (u_int)*ixdr++;
	...
	(void) key_sched((des_cblock *)&auth->ah_key, key_s);
	(void) des_ecb_encrypt(&buf, &buf, key_s, 0);
	/* clean up */
	memset((char *) key_s, 0, sizeof (key_s));
#endif SUNDES_CRYPT
#endif /* NO ENCRYPTION */
	...
	void *dummy;
{
	...
	_kmsgout("authkerb_refresh: ticket length (%d) > MAX_AUTH_BYTES (%d)",

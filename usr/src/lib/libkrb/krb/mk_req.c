#ident	"@(#)mk_req.c	1.3	97/11/25 SMI"
/*
...
"$Header: mk_req.c,v 4.17 89/07/07 15:20:35 jtkohl Exp $";
#endif /* lint */
...
krb_mk_req(authent,service,instance,realm,checksum)
    ...
    int    checksum;           /* Checksum of data (optional) */
{
    ...
    int tmp;
    extern krb_get_tf_realm(), get_ad_tkt(), key_sched(), pcbc_encrypt();
    ...
    *(req_id->dat+(req_id->length)++) =
    ...
    pcbc_encrypt((C_Block *)req_id->dat,(C_Block *)req_id->dat,
                 (int) req_id->length,key_s,cr.session,1);
    /* clean up */

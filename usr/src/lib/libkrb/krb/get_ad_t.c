#ident	"@(#)get_ad_tkt.c	1.4	97/11/25 SMI"
/*
...
"$Header: get_ad_tkt.c,v 4.15 89/07/07 15:18:51 jtkohl Exp $";
#endif /* lint */
    ...
    int     lifetime;
{
    ...
    unsigned int kdc_time;   /* KDC time */
    extern krb_get_tf_realm(), send_to_kdc(), pkt_clen(),
		des_key_sched(), des_pcbc_encrypt(), save_credentials();
    ...
    kerror = krb_mk_req(pkt,"krbtgt",realm,lrealm,0);
    ...
    *(pkt->dat+(pkt->length)++) = (char) lifetime;
    ...
    pcbc_encrypt((C_Block *)cip->dat,(C_Block *)cip->dat,
                 (int) cip->length,key_s,cr.session,0);
#endif /* !NOENCRYPTION */
    ...
    lifetime = (unsigned int) ptr[0];
    kvno = (unsigned int) ptr[1];

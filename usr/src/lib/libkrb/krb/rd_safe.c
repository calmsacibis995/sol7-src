#ident	"@(#)rd_safe.c	1.3	97/11/25 SMI"
/*
...
#include <kerberos/lsb_addr_comp.h>
...
int krb_rd_safe(in,in_length,key,sender,receiver,m_data)
    u_char *in;                 /* pointer to the msg received */
    u_int in_length;           /* length of "in" msg */
...
{
    register u_char     *p,*q;
    static      u_int  src_addr; /* Can't send structs since no
                                   * guarantees on size */
    extern unsigned int quad_cksum();

    /* Be very conservative */
    if (sizeof(u_int) != sizeof(struct in_addr)) {
        fprintf(stderr,"\n\
krb_rd_safe protocol err sizeof(u_int) != sizeof(struct in_addr)");
        exit(-1);
    }
    ...
    if (src_addr != (u_int) sender->sin_addr.s_addr)
        return RD_AP_MODIFIED;
    ...
    delta_t = abs((int)(local_time.tv_sec - m_data->time_sec));
    ...
    if (krb_debug)
        printf("\ncalc_cksum = %u, received cksum = %u",
               (int) calc_cksum[0], (int) big_cksum[0]);

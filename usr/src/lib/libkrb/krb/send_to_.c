...
#ident	"@(#)send_to_kdc.c	1.5	97/11/25 SMI"

#ifndef lint
static char rcsid_send_to_kdc_c[] =
"$Id: send_to_kdc.c,v 4.20 90/01/02 13:40:37 jtkohl Exp $";
#endif /* lint */
...
/* poll - either recv is ready, or timeout */
/*
 * CLIENT_KRB_TIMEOUT indicates the time to wait before
 * retrying a server.  It's defined in "krb.h".
 */

	if ((poll(&readfd, 1, CLIENT_KRB_TIMEOUT * MILLISEC) < 1) ||
		((readfd.revents & POLLRDNORM) == 0)) {
		if (krb_debug) {
			fprintf(stderr, "poll failed: readfd=%x", f);
			perror("");
		}
		return (0);
	}
	...
	if (recvfrom(f, (char *)(rpkt->dat), sizeof (rpkt->dat), 0,
		(struct sockaddr *)&from, (socklen_t *)&sin_size) < 0) {
		if (krb_debug)
			perror("recvfrom");
		return (0);
	}

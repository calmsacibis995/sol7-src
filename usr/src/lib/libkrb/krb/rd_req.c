#ident	"@(#)rd_req.c	1.3	97/11/25 SMI"
/*
...
extern des_key_sched(), read_service_key(), decomp_ticket();
...
krb_rd_req(authent,service,instance,from_addr,ad,fn)
    ...
    int from_addr;		/* Net address of originating host */
    ...
    char *fn;			/* Filename to get keys from */
{
    ...
    unsigned int r_time_sec;   /* Coarse time from authenticator */
    register char *ptr;		/* For stepping through */
    unsigned int delta_t;      /* Time in authenticator - local time */
    int tkt_age;		/* Age of ticket */
    ...
    pcbc_encrypt((C_Block *)req_id->dat,(C_Block *)req_id->dat,
                 (int) req_id->length, seskey_sched,ad->session,
		KRBDES_DECRYPT);

#ident	"@(#)tf_util.c	1.4	97/11/25 SMI"
/*
...
"$Id: tf_util.c,v 4.9 90/03/10 19:19:45 jon Exp $";
#endif /* lint */
    ...
    char   *tf_name;
{
    ...
    if (stat64(shmidname,&stat_buf) < 0)
	return(TKT_FIL_ACC);
    if ((stat_buf.st_uid != me && me != 0) ||
	((stat_buf.st_mode & S_IFMT) != S_IFREG))
	return TKT_FIL_ACC;
    ...
    */
    ...
    /*
     *  XXX
     *  This really need to be opened for readonly and have a shared
     *  lock applied.  But I don't know how to do that yet.
     */
    fd = open(tf_name, O_RDWR, 0600);
#else
    ...
    int    issue_date;		/* The issue time */
{
	...
	goto bad;
    /* Issue date */
    if (write(fd, (char *) &issue_date, sizeof(int))
	!= sizeof(int))
	goto bad;

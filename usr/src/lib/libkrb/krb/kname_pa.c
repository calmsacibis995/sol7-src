#ident	"@(#)kname_parse.c	1.3	97/11/25 SMI"
/*
...
"$Header: kname_parse.c,v 4.4 88/12/01 14:07:29 jtkohl Exp $";
#endif /* lint */
    ...
    char *np, *ip, *rp, *fullname;
{
    ...
    if ((int)strlen(fullname) > FULL_SZ)
        return KNAME_FMT;
    ...
    *wnext = '\0';
    if (((int)strlen(np) > ANAME_SZ - 1) ||
        ((int)strlen(ip) > INST_SZ  - 1) ||
        ((int)strlen(rp) > REALM_SZ - 1))
        return KNAME_FMT;
    return KSUCCESS;
}

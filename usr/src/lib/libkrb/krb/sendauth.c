#ident	"@(#)sendauth.c	1.3	97/11/25 SMI"
/*
...
/*
 * $Source: /afs/athena.mit.edu/astaff/project/kerberos/src/lib/krb/RCS/sendauth.c,v $
 * $Author: jon $
 *
...
"$Id: sendauth.c,v 4.6 90/03/10 23:18:28 jon Exp $";
#endif	lint
...
int options;			 /* bit-pattern of options */
...
u_int checksum;		 /* checksum to include in request */
...
char *version;			 /* version string */
{
    ...
    tkt_len = (int) htonl((uint32_t) ticket->length);
...
#endif /* !NOENCRYPTION */
	if (cc = krb_rd_priv(priv_buf,(unsigned int) tkt_len, schedule,
	...
	(void) bcopy((char *)msg_data->app_data, (char *)&cksum,
		     sizeof(cksum));
	cksum = (u_int) ntohl((uint32_t) cksum);
...
char *service;
{
    ...
    serv_length = htonl((uint32_t) strlen(service));

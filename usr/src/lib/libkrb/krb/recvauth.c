#ident	"@(#)recvauth.c	1.3	97/11/25 SMI"
/*
...
/*
 * $Source: /afs/athena.mit.edu/astaff/project/kerberos/src/lib/krb/RCS/recvauth.c,v $
 * $Author: jon $
 *
...
"$Header: /afs/athena.mit.edu/astaff/project/kerberos/src/lib/krb/RCS/recvauth.c,v 4.4 90/03/10 19:03:08 jon Exp $";
#endif	lint
...
char *version;			 /* version string (filled in) */
{
	...
	if (i < KRB_SENDAUTH_VLEN) {
	    ...
	    if (krb_net_read(fd, (char *)(tmp_buf+i), tkt_len) !=
		 tkt_len)
		return(errno);
	}
	...
	/* sanity check */
	ticket->length = (int) ntohl((uint32_t)tkt_len);
...
mutual_fail:
	    priv_len = -1;
	    tkt_len = (int) htonl((uint32_t)priv_len);

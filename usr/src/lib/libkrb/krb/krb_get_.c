#ident	"@(#)krb_get_in_tkt.c	1.3	97/11/25 SMI"
/*
  ...
  KTEXT *cipp;
{
    ...
    Key_schedule key_s;
    extern int key_sched(), pcbc_encrypt();
    ...
    char *arg;
{
    ...
    extern send_to_kdc(), in_tkt(), save_credentials(), pkt_clen();
    ...
    *(pkt->dat+(pkt->length)++) = (char) life;
    ...
    if (((int)strlen(ptr) + (ptr - (char *) cip->dat)) > cip->length)
	return(INTK_BADPW);

#ident	"@(#)create_ciph.c	1.3	97/11/25 SMI"
/*
...
"$Header: create_ciph.c,v 4.8 89/05/18 21:24:26 jis Exp $";
#endif /* lint */
...
create_ciph(c, session, service, instance, realm, life, kvno, tkt,
    ...
    unsigned int   life;	/* Lifetime of the ticket */
    ...
    C_Block         key;	/* Key to encrypt ciphertext with */
{
    char            *ptr;
    Key_schedule    key_s;
    extern int key_sched(), des_pcbc_encrypt();
    ...
    *(ptr++) = (unsigned char) tkt->length;
    ...
    pcbc_encrypt((C_Block *)c->dat,(C_Block *)c->dat,
		 (int) c->length,key_s,key,ENCRYPT);
#endif /* NOENCRYPTION */

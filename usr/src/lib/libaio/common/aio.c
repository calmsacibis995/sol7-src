#pragma ident	"@(#)aio.c	1.38	97/08/29 SMI"
/*
...
void _aio_req_add(struct aio_req *);
...
#define	AIOHASH(resultp)	(((uintptr_t)(resultp) >> 3) & (HASHSZ-1))
...
/*
 * Separate READ and WRITE workers
 */
struct aio_worker *_workers_rd;		/* circular list of AIO workers */
struct aio_worker *_nextworker_rd;	/* next worker in list of workers */
struct aio_worker *_lastworker_rd;	/* last worker to have a full q */
struct aio_worker *_workers_wr;		/* circular list of AIO workers */
struct aio_worker *_nextworker_wr;	/* next worker in list of workers */
struct aio_worker *_lastworker_wr;	/* last worker to have a full q */

/*
 * separate fsync worker
 */
struct aio_worker *_workers_fs;		/* circular list of AIO workers */
struct aio_worker *_nextworker_fs;	/* next worker in list of workers */
struct aio_worker *_lastworker_fs;	/* last worker to have a full q */
...
__uaio_init()
{
	...
	if (!__uaio_ok) {
		...
		while (i++ < (_min_workers/2))
			_aio_create_worker(NULL, NULL, AIOREAD);
		i = 0;
		while (i++ < (_min_workers/2))
			_aio_create_worker(NULL, NULL, AIOWRITE);

		/*
		 * Create 1 fsync worker
		 */
		_aio_create_worker(NULL, NULL, AIOFSYNC);
...
_kaio_init(void)
{
	caddr_t stk;
	int stksize;
	long error = 0;	/* kaio() can contain a pointer or value */
	sigset_t set;
	ucontext_t uc;
	...
	int mode;
{
	...
	struct stat stat;
	int err = 0;
	long kerr;	/* kaio() can contain a pointer or vaulue */
	...
	struct timeval *uwait;
{
	...
	while (1) {
		...
		_aiowait_flag++;
		_aio_unlock();
		kresultp = (aio_result_t *)_kaio(AIOWAIT, wait,
...
/*
 * Check for outstanding requests for an fd
 */
int
_aio_outstanding(fd)
	int fd;
{
	...
	next = first;
	do {
		aiorp = next->work_tail1;
		while (aiorp != NULL) {
			if (fd < 0 || aiorp->req_args.fd == fd) {
				if (aiorp->req_state == AIO_REQ_INPROGRESS)
					return (1);
			}
		...
		switch (aiorp->req_op) {
			...
			case AIOFSYNC:
				if (arg->offset == O_SYNC)
					...
					retval = __fdsync(arg->fd, O_DSYNC);
				break;
		}
		if (retval == -1)
			err = errno;
...
_aio_req_add(aiorp)
	aio_req_t *aiorp;
{
	struct aio_worker *aiowp, **_nextworker;
	...
	switch (aiorp->req_op) {
		case AIOWRITE:
			_nextworker = &_nextworker_wr;
			break;
		case AIOREAD:
			_nextworker = &_nextworker_rd;
			break;
		case AIOFSYNC:
			_nextworker = &_nextworker_fs;
			break;
	}
	...
	} else if (aiorp->req_op == AIOREAD) { /* AIOREAD */
		while (1) {
	...
	if (clogged) {
		...
		_clogged++;
#endif
		if (_aio_create_worker(aiorp, 0, aiorp->req_op))
...
_fill_aiocache(n)
{
	...
	if ((ptr = (uintptr_t)malloc(sizeof (struct aio_req) * n)) == -1) {
	...
	aio_lio_t 	*lio_head;
{
	...
	if (_kaio_flg &  AIO_KAIO) {
		...
		if (_kaio_ok > 0) {
			...
			if (mode == AIOREAD)
				kerr = _kaio(AIOAREAD, cb);
			else
				kerr = _kaio(AIOAWRITE, cb);
			if (kerr == 0) {
				cb->aio_state = CHECK;
				return (0);
			} else if (errno != ENOTSUP) {
	...
	aio_lio_t 	*lio_head;
{
	...
	if (_kaio_flg &  AIO_KAIO) {
		...
		if (_kaio_ok > 0) {
			cb->aio_resultp.aio_errno = EINPROGRESS;
			if (mode == AIOREAD)
				kerr = _kaio(AIOAREAD64, cb);
			else
				kerr = _kaio(AIOAWRITE64, cb);
			if (kerr == 0) {
				cb->aio_state = CHECK;
				return (0);
			} else if (errno != ENOTSUP) {

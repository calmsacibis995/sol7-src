...
#pragma ident	"@(#)sig.c	1.21	97/11/12 SMI"
...
int
_aio_create_worker(struct aio_req *aiop, int flag, int mode)
{
	struct aio_worker *aiowp, **_workers, **_nextworker;
	caddr_t stk = 0;
	ucontext_t uc;
	int stksize;
	...
	switch (mode) {
	case AIOWRITE:
		_workers = &_workers_wr;
		_nextworker = &_nextworker_wr;
		break;
	case AIOREAD:
		_workers = &_workers_rd;
		_nextworker = &_nextworker_rd;
		break;
	case AIOFSYNC:
		_workers = &_workers_fs;
	...
	_lwp_makecontext(&uc, ((void (*) ())_aio_do_request),
	    aiowp, aiowp, stk, stksize);

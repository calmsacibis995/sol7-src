...
#pragma ident	"@(#)FNSP_PrinternameContext.cc	1.12	97/08/15 SMI"
...
FNSP_PrinternameContext::resolve(const FN_string &aname,
    FN_status_csvc& cstat)
{
	FN_ref *ref_files, *ref_ns;
	unsigned status;

	// Get the references form both /etc/printers.conf
	// and from the name service.
	ref_files = FNSP_PrinterContext::resolve(aname, cstat);
	if (ns_impl)
		ref_ns =  ns_impl->lookup_binding(aname, status);
	else
		ref_ns = 0;

	// If only one is present, return the correct one
	if ((cstat.is_success()) && (ref_ns == 0))
		return (ref_files);
	else if (!cstat.is_success() && ref_ns) {
		cstat.set_success();
		return (ref_ns);
	} else if (!cstat.is_success() && (ref_ns == 0)) {
		if (ns_impl)
			cstat.set_error(status, *my_reference, empty_string);
		return (0);
	}

	// Since reference is present in both, merge the results
	// Get the "onc_fn_printer_" and "sspc___printer_"
	// address type from ref_ns
	...
	for (address = ref_ns->first(ip); address;
		...
		if ((strncmp((char *) type->str(), "onc_fn_printer_",
		    ...
		    (strncmp((char *) type->str(), "sspc___printer_",
		    strlen("sspc___printer_")) == 0))
			ref_files->prepend_addr(*address);
	}

...
#pragma ident	"@(#)request.c	1.3	97/11/07 SMI"
...
#include <stdio.h>
#include <string.h>
#include <stdlib.h> /* calloc(), free(), atoi(), malloc(), realloc() */
...
#ifdef NEEDPROTOS
static int initiate_request(LDAP *ld, BerElement *ber, int msgid,
	...
	BerElement *ber)
{
	...
	return (initiate_request(ld, ber, ld->ld_msgid, NULL, servers,
	    NULL, 0));
#endif /* !LDAP_REFERRALS && !LDAP_DNS */
}
...
#if defined(LDAP_REFERRALS) || defined(LDAP_DNS)
static int
initiate_request(LDAP *ld, BerElement *ber, int msgid, LDAPRequest *parentreq,
	LDAPServer *srvlist, LDAPConn *lc, int referred)
{
	LDAPRequest	*lr;

	Debug(LDAP_DEBUG_TRACE, "initiate_request\n", 0, 0, 0);

	if (lc == NULL) {
		if (srvlist == NULL) {
			lc = ld->ld_defconn;
		} else if ((lc = find_connection(ld, srvlist, 1)) != NULL) {
			if (lc->lconn_status != LDAP_CONNST_CONNECTED) {
				ld->ld_errno = LDAP_SERVER_DOWN;
			}
			free_servers(srvlist);
		} else {
			lc = new_connection(ld, &srvlist, 0, 1, 0, referred);
			free_servers(srvlist);
		}
	}
	...
	int connect, int set_instance, int bind)
{
	 ...
	 * XXX synchronously send bind for now
	 */
	...
	if (connect) {
		...
		for (srv = *srvlistp; srv != NULL; srv = srv->lsrv_next) {
			if (open_ldap_connection(ld, lc->lconn_sb,
			    srv->lsrv_host, srv->lsrv_port, set_instance, 0)
			    != -1) {
				break;
			}
			prevsrv = srv;
	...
	/*
	 * XXX if a bind is needed, we always do a synchronous NULL bind.
	 * This will have to change in the long run...
	 */
	if (bind) {
		BerElement	*ber;
		int		err;
		...
		if (err == 0 && ber_printf(ber, "{it{ists}}", ++ld->ld_msgid,
		    LDAP_REQ_BIND, ld->ld_version, "", LDAP_AUTH_SIMPLE, "")
		    == -1) {
		...
		if (err == 0 && (err = initiate_request(ld, ber,
		    ld->ld_msgid, NULL, NULL, lc, 0)) >= 0) {
			++lc->lconn_refcnt;	/* avoid freeing early */
			if ((err = ldap_result(ld, err, 1,
			    (struct timeval *)0, &result)) > 0) {
				if (ldap_result2error(ld, result, 1) !=
				    LDAP_SUCCESS) {
					err = -1;
				} else {
					err = 0;
	...
	*hadrefp = 0;
	...
	for (ref = p; rc == 0 && ref != NULL; ref = p) {
		...
		if (len > LDAP_LDAP_REF_STR_LEN && strncasecmp(ref,
			...
			rc = append_referral(&unfollowed, ref);
			*hadrefp = 1;
			continue;
		}
		...
		*hadrefp = 1;
		...
		if (srv != NULL && initiate_request(ld, ber, ld->ld_msgid,
		    lr, srv, NULL, 1) >= 0) {
			++count;
		} else {
			Debug(LDAP_DEBUG_ANY,
...
int
append_referral(char **referralsp, char *s)
{
	int	first;
	...
	if (first) {
		(void) strcpy(*referralsp, LDAP_REF_STR);
	} else {
		(void) strcat(*referralsp, "\n");
	}
	(void) strcat(*referralsp, s);
 ...
 */
	...
	if (tag == LDAP_REQ_BIND && ber_get_int(&tmpber, (int *)&ver) ==
	    LBER_DEFAULT) {
		ld->ld_errno = LDAP_DECODING_ERROR;
		ber_free(ber, 1);
		return (NULL);
	...
	if (ber_write(ber, tmpber.ber_ptr, (int)(tmpber.ber_end -

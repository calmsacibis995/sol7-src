...
#pragma ident	"@(#)open.c	1.3	97/11/07 SMI"
...
ldap_open(char *host, int port)
{
	...
	if ((ld->ld_defconn = new_connection(ld, &srv, 1, 1, 1, 0)) == NULL) {
	...
	if (host != NULL) {
		for (p = host; p != NULL && *p != '\0'; p = q) {
			...
			if ((r = strchr(curhost, ':')) != NULL) {
			    if (curhost != hostname) {
				(void) strcpy(hostname, curhost); /* now copy */
...
#ifdef KERBEROS
	if (set_instance && (ld->ld_host = host_connected_to(sb)) != NULL &&
	    (p = strchr(ld->ld_host, '.')) != NULL) {
		*p = '\0';
	}
#endif /* KERBEROS */

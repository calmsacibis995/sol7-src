...
#pragma ident	"@(#)cldap.c	1.3	97/11/07 SMI"
...
#ifndef lint
static char copyright[] = "@(#) Copyright (c) 1990, 1994 Regents of the "
...
#include <string.h>
#include <errno.h>
#include <stdlib.h> /* calloc(), free() for Solaris */
#include <netinet/in.h>
#include <arpa/inet.h> /* inet_addr() for Solaris */
#include <unistd.h> /* close() for Solaris */
#ifdef MACOS
#include <stdlib.h>
#include "macos.h"
#else /* MACOS */
...
cldap_open(char *host, int port)
{
	...
	for (; host != NULL; host = p) {
		...
		for (i = 0; hp->h_addr_list[ i ] != 0; ++i) {
		    (void) SAFEMEMCPY((char *)&sock.sin_addr.s_addr,
	...
	if (ld->ld_sb.sb_addrs == NULL ||
	    ...
	    (ld->ld_defconn = new_connection(ld, NULL, 1, 0, 1, 0)) == NULL
...
add_addr(LDAP *ld, struct sockaddr *sap)
{
	...
	(void) SAFEMEMCPY((char *)newsap, (char *)sap,
	...
	struct cldap_retinfo *crip, char *base)
{
	...
	crip->cri_useaddr = 0;
	Debug(LDAP_DEBUG_TRACE, "cldap_result tries %d timeout %ld\n",

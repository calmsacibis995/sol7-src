...
#pragma ident	"@(#)cache.c	1.4	97/11/12 SMI"
...
#ifndef lint
static char copyright[] = "@(#) Copyright (c) 1993 Regents of the University "
			    "of Michigan.\nAll rights reserved.\n";
#endif

#include <stdio.h>
#include <string.h>
#include <stdlib.h> /* malloc(), free(), calloc() for Solaris */
#ifdef MACOS
#include <stdlib.h>
#include <time.h>
#include "macos.h"
...
ldap_flush_cache(LDAP *ld)
{
	...
	if (ld->ld_cache != NULLLDCACHE) {
		...
		for (i = 0; i < LDAP_CACHE_BUCKETS; ++i) {
			for (m = ld->ld_cache->lc_buckets[ i ].lm_next;
			    m != NULLMSG; m = next) {
				next = m->lm_next;
				ldap_msgfree(m);
			}
			ld->ld_cache->lc_buckets[ i ].lm_next = NULLMSG;
		}
		ld->ld_cache->lc_memused = sizeof (LDAPCache);
	}
}
...
{
	Debug(LDAP_DEBUG_TRACE, "ldap_uncache_request %d ld_cache %p\n",
	    msgid, ld->ld_cache, 0);
...
{
	Debug(LDAP_DEBUG_TRACE, "ldap_uncache_entry %s ld_cache %p\n",
	    dn, ld->ld_cache, 0);
	...
	int msgid)		/* request to uncache (if dn == NULL) */
{
	...
	Debug(LDAP_DEBUG_TRACE,
	    "ldap_uncache_entry_or_req  dn %s  msgid %d  ld_cache %p\n",
	...
	for (i = 0; i < LDAP_CACHE_BUCKETS; ++i) {
		prev = NULLMSG;
		for (m = ld->ld_cache->lc_buckets[ i ].lm_next; m != NULLMSG;
		    m = next) {
			next = m->lm_next;
			if ((dn != NULL && chain_contains_dn(m, dn)) ||
				(dn == NULL && m->lm_msgid == msgid)) {
				if (prev == NULLMSG) {
					ld->ld_cache->lc_buckets[ i ].lm_next
					    = next;
				} else {
					prev->lm_next = next;
				}
...
add_request_to_cache(LDAP *ld, unsigned int msgtype, BerElement *request)
{
	...
	if ((new = (LDAPMessage *) calloc(1, sizeof (LDAPMessage)))
		...
		(void) SAFEMEMCPY(new->lm_ber->ber_buf, request->ber_buf, len);
...
{
	LDAPMessage	*m, *req, *new, *prev;
	int		err, keep;

	Debug(LDAP_DEBUG_TRACE, "add_result_to_cache\n", 0, 0, 0);

	if (ld->ld_cache == NULLLDCACHE ||
	    (ld->ld_cache->lc_enabled == 0)) {
		Debug(LDAP_DEBUG_TRACE, "add_result_to_cache DISABLED\n", 0, 0,
		    0);
		return;
	}
	...
	if (result->lm_msgtype != LDAP_RES_SEARCH_ENTRY &&
		 ...
		 */
		Debug(LDAP_DEBUG_TRACE,
		    "add_result_to_cache NOT SEARCH OR COMPARE\n", 0, 0, 0);
		return;
	}
	...
	if (m != NULLMSG) {	/* found request; add to end of chain */
		...
		if ((new = msg_dup(result)) != NULLMSG) {
			...
			m->lm_chain = new;
			Debug(LDAP_DEBUG_TRACE,
			    "add_result_to_cache MSGID %d  TYPE %d\n",
		...
		if (result->lm_msgtype == LDAP_RES_SEARCH_RESULT ||
			...
			if (!keep) {
				Debug(LDAP_DEBUG_TRACE,
				    "add_result_to_cache not caching request\n",
					0, 0, 0);
				ldap_msgfree(req);
			} else {
				m = &ld->ld_cache->lc_buckets[
				    cache_hash(req->lm_ber) ];
				req->lm_next = m->lm_next;
				m->lm_next = req;
			...
			/* EMPTY */
			Debug(LDAP_DEBUG_TRACE,
			    "add_result_to_cache didn't find msgid %d\n",
			    result->lm_msgid, 0, 0);
		}
	}
}
...
check_cache(LDAP *ld, unsigned int msgtype, BerElement *request)
{
	...
	for (m = ld->ld_cache->lc_buckets[ hash ].lm_next; m != NULLMSG;
	    m = next) {
		Debug(LDAP_DEBUG_TRACE, "check_cache examining id %d,type %d\n",
		    m->lm_msgid, m->lm_msgtype, 0);
		if (m->lm_time < validtime) {
			/* delete expired message */
			next = m->lm_next;
			if (prev == NULL) {
				ld->ld_cache->lc_buckets[ hash ].lm_next = next;
			} else {
				prev->lm_next = next;
			}
			Debug(LDAP_DEBUG_TRACE, "check_cache expired id %d\n",
			    m->lm_msgid, 0, 0);
			ld->ld_cache->lc_memused -= msg_size(m);
			ldap_msgfree(m);
		} else {
	...
	for (m = m->lm_chain; m != NULLMSG; m = m->lm_chain) {
		...
		prev = new;
		Debug(LDAP_DEBUG_TRACE, "check_cache added type %d\n",
		    new->lm_msgtype, 0, 0);
	}

	Debug(LDAP_DEBUG_TRACE, "check_cache OK\n", 0, 0, 0);
	return (0);
}
...
cache_hash(BerElement *ber)
{
	...
	Debug(LDAP_DEBUG_TRACE, "cache_hash: len is %d, returning %d\n",
...
msg_dup(LDAPMessage *msg)
{
	...
	if ((new = (LDAPMessage *)malloc(sizeof (LDAPMessage))) != NULL) {
		*new = *msg;	/* struct copy */
		...
		(void) SAFEMEMCPY(new->lm_ber->ber_buf, msg->lm_ber->ber_buf,
		   len);
		new->lm_ber->ber_ptr = new->lm_ber->ber_buf +
 ...
 */
	...
	Debug(LDAP_DEBUG_TRACE, "check_cache_memused %ld/%ld\n", lc->lc_memused,
	    lc->lc_maxmem, 0);
	...
	while (lc->lc_memused > lc->lc_maxmem * SIZE_FACTOR) {
		...
		for (i = 0; i < LDAP_CACHE_BUCKETS; ++i) {
			prev = NULLMSG;
			for (m = lc->lc_buckets[ i ].lm_next; m != NULLMSG;
			    m = next) {
				next = m->lm_next;
				if (m->lm_time < validtime) {
					if (prev == NULLMSG) {
						lc->lc_buckets[ i ].lm_next
						    = next;
					} else {
						prev->lm_next = next;
					}
	...
	Debug(LDAP_DEBUG_TRACE, "check_cache_memused reduced to %d/%d\n",

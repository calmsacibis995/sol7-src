...
#pragma ident	"@(#)getfilter.c	1.3	97/11/07 SMI"
...
#include <string.h>
#include <ctype.h>
#include <re_comp.h> /* re_exec() for Solaris */
#if defined(NeXT)
#include <regex.h>
#endif
#ifdef MACOS
...
ldap_init_getfilter_buf(char *buf, ssize_t buflen)
{
	...
	case 5:		/* start of filter info. list */
		...
		ldap_getfilter_free(lfdp);
		(void) fprintf(stderr, "bad regular expresssion %s\n",
			nextflp->lfl_pattern);
#if !defined(MACOS) && !defined(DOS)
		errno = EINVAL;
#endif
		...
		if (*p == 'v') {
			...
			for (i = wordnum; i <= endwordnum; ++i) {
				...
				*f++ = ' ';
			    ...
			    (void) SAFEMEMCPY(f, valwords[ i ], slen);
			    f += slen;
			}
		    } else if (*(p+1) == '$') {
			++p;
			...
			(void) SAFEMEMCPY(f, valwords[ wordnum ], slen);
			f += slen;
		    } else {
			slen = strlen(value);
			(void) SAFEMEMCPY(f, value, slen);
			f += slen;
		    }
		} else if (*p == 'a' && attr != NULL) {
		    slen = strlen(attr);
		    (void) SAFEMEMCPY(f, attr, slen);
		    f += slen;
		} else {
		    *f++ = *p;
		}

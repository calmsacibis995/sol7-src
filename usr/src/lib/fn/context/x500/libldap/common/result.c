...
#pragma ident	"@(#)result.c	1.3	97/11/07 SMI"
...
static void merge_error_info(LDAPRequest *parentr, LDAPRequest *lr);
#else /* LDAP_REFERRALS */
	...
	*result = NULLMSG;
	...
	for (lm = ld->ld_responses; lm != NULLMSG; lm = nextlm) {
		...
		if (ldap_abandoned(ld, lm->lm_msgid)) {
			(void) ldap_mark_abandoned(ld, lm->lm_msgid);
	...
	while (rc == -2) {
		...
		if (lc == NULL) {
			...
			for (lc = ld->ld_conns; rc == -2 && lc != NULL;
				...
				if (lc->lconn_status ==
				    LDAP_CONNST_CONNECTED && is_read_ready(ld,
				    lc->lconn_sb)) {
	...
	LDAPMessage **result)
{
	...
	Debug(LDAP_DEBUG_TRACE, "got %s msgid %d, original id %d\n",
	...
	if (tag != LDAP_RES_SEARCH_ENTRY) {
		if (ld->ld_version >= LDAP_VERSION2 &&
			...
			if (ber_scanf(&tmpber, "{iaa}", &lderr, &matchedstr,
			    &errstr) != LBER_ERROR) {
				...
				if (!hadref || (lr->lr_res_error = errstr)
				    == NULL) {
	...
	lr->lr_res_matched ? lr->lr_res_matched : "");
		...
		if (refer_cnt != 0) {	/* chasing referrals */
			...
			while (lr->lr_parent != NULL) {
				merge_error_info(lr->lr_parent, lr);
			...
			if (lr->lr_outrefcnt <= 0 && lr->lr_parent == NULL) {
				...
				Debug(LDAP_DEBUG_ANY, "request %d done\n",
				    id, 0, 0);
Debug(LDAP_DEBUG_TRACE,
"res_errno: %d, res_error: <%s>, res_matched: <%s>\n",
lr->lr_res_errno, lr->lr_res_error ? lr->lr_res_error : "",
lr->lr_res_matched ? lr->lr_res_matched : "");
				if (!simple_request && build_result_ber(ld,
				    &ber, lr) == LBER_ERROR) {
					free(ber.ber_buf); /* gack! */
					ber.ber_buf = NULL;
...
build_result_ber(LDAP *ld, BerElement *ber, LDAPRequest *lr)
{
	...
	if (ber_printf(ber, "{it{ess}}", lr->lr_msgid,
	    (int)lr->lr_res_msgtype, lr->lr_res_errno,
	    lr->lr_res_matched ? lr->lr_res_matched : "",
	    lr->lr_res_error ? lr->lr_res_error : "") == LBER_ERROR) {
		return ((int)LBER_ERROR);
	}
...
static void
merge_error_info(LDAPRequest *parentr, LDAPRequest *lr)
{
	if (lr->lr_res_errno == LDAP_PARTIAL_RESULTS) {
		lr->lr_parent->lr_res_errno = lr->lr_res_errno;
		if (lr->lr_res_error != NULL) {
			(void) append_referral(&parentr->lr_res_error,
			    lr->lr_res_error);
		}
	...
	Debug(LDAP_DEBUG_TRACE, "errno %d, error <%s>, matched <%s>\n",
	    lr->lr_res_errno, lr->lr_res_error ? lr->lr_res_error : "",
	    lr->lr_res_matched ? lr->lr_res_matched : "");
}
#endif /* LDAP_REFERRALS */

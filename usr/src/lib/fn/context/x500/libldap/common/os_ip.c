...
#pragma ident	"@(#)os-ip.c	1.3	97/11/07 SMI"
...
/* ARGSUSED4 */
int
connect_to_host(Sockbuf *sb, char *host, unsigned int address,
	int port, int async)
/*
 ...
 */
{
	...
	for (i = 0; !use_hp || (hp->h_addr_list[ i ] != 0); i++) {
		...
		sin.sin_port = port;
		(void) SAFEMEMCPY((char *) &sin.sin_addr.s_addr,
...
host_connected_to(Sockbuf *sb)
{
	...
	(void) memset((char *)&sin, 0, sizeof (struct sockaddr_in));
	len = sizeof (sin);
	if (getpeername(sb->sb_sd, &sin, &len) == -1) {
		return (NULL);
	}
...
int
do_select(LDAP *ld, struct timeval *timeout)
{
	struct selectinfo	*sip;
	static int		tblsize;

 ...
 * All rights reserved.
 */
#pragma ident	"@(#)td_po.c	1.67	97/11/12 SMI"
...
__td_ta_tsd_iter(const td_thragent_t *ta_p, td_key_iter_f *cb, void *cbdata_p)
{
	...
	if (model == PR_MODEL_NATIVE) {
		ps_ret = ps_pdread(ta_p->ph_p,
		    ...
		    (char *) &tsd_common, sizeof (tsd_common));
	}
#ifdef  _SYSCALL32_IMPL
	else {
		ps_ret = ps_pdread(ta_p->ph_p,
		    (psaddr_t) ta_p->tdb_invar32.tsd_common_addr,
		    (char *) &tsd_common32, sizeof (tsd_common32));
	}
#endif /* _SYSCALL32_IMPL */
	if (return_val == TD_OK && ps_ret != PS_OK)
		return_val = TD_DBERR;
	...
	if (return_val == TD_OK) {
		...
		if (model == PR_MODEL_NATIVE) {
			for (key = 1; key <= tsd_common.nkeys; key++) {
				(*cb)(key, tsd_common.destructors[key],
				    cbdata_p);
			}
		}
#ifdef  _SYSCALL32_IMPL
		else {
			for (key = 1; key <= tsd_common32.nkeys; key++) {
				(*cb)(key,
				    (void (*)())tsd_common32.destructors[key],
				    cbdata_p);
			}
		}
#endif /* _SYSCALL32_IMPL */
...
#ifdef  _SYSCALL32_IMPL
	tdb_sync_desc32_t *next_desc32;
	tdb_sync_desc32_t sync_desc32;
	tdb_sync_desc32_t *sync_desc_hash32[TDB_SYNC_DESC_HASHSIZE];
#endif /* _SYSCALL32_IMPL */
	ps_err_e	ps_ret = PS_OK;
	int		model = PR_MODEL_NATIVE;
...
*/
td_err_e
__td_ta_map_addr2thr(const td_thragent_t *ta_p, psaddr_t thread,
	td_thrhandle_t *th_p)
{

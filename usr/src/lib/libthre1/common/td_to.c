 ...
 * All rights reserved.
 */
#pragma ident	"@(#)td_to.c	1.87	97/11/12 SMI"
	...
	struct tsd_common * tsd_common_p, void **tsd_array_ptr)
{
	...
	if (model == PR_MODEL_NATIVE) {
		return_val = __td_read_thr_struct(ta_p, th_p->th_unique,
		    &thr_struct);
		t_tls = thr_struct.t_tls;
	}
#ifdef  _SYSCALL32_IMPL
	else {
		return_val = __td_read_thr_struct32(ta_p, th_p->th_unique,
		    &thr_struct32);
		t_tls = (caddr_t)thr_struct32.t_tls;
	}
#endif /* _SYSCALL32_IMPL */
	...
	if (return_val == TD_OK) {
		...
		*thr_tsd_p = NULL_TSD_T;
		 ...
		 * for this thread.
		 */
		if (thr_struct.t_tls == NULL)
			return_val = TD_NOTSD;
		else if (ps_pdread(ph_p, (psaddr_t) t_tls,
		    (char *) thr_tsd_p, sizeof (*thr_tsd_p)) != PS_OK)
			return_val = TD_DBERR;
		else if ((array_addr = (psaddr_t) thr_tsd_p->array) != NULL) {
			/*
			 * Allocate storage to hold TSD array
			 * for this this thread only and read
			 * TSD array.
			 */
			tsd_size = sizeof (void *) * thr_tsd_p->count;
			*tsd_array_ptr = (void *) malloc(tsd_size);
			if (*tsd_array_ptr == NULL)
			 ...
			 * this thread.
			 */
			else if (ps_pdread(ph_p,  array_addr,
			    *tsd_array_ptr, tsd_size) != PS_OK)
				return_val = TD_DBERR;

			/* Get tsd_common to check number of keys used. */
			else if (ps_pdread(ph_p,
			    (psaddr_t) ta_p->tdb_invar.tsd_common_addr,
			    (char *) tsd_common_p,
			    sizeof (*tsd_common_p)) != PS_OK)
				return_val = TD_DBERR;
		} else {
			return_val = TD_NOTSD;
		}

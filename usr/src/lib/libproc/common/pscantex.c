...
#pragma ident	"@(#)Pscantext.c	1.1	97/12/23 SMI"
...
Pscantext(struct ps_prochandle *P)
{
	...
	if ((mapfd = open(mapfile, O_RDONLY)) < 0) {
		perror("Pscantext(): cannot open map file");
		return (-1);
	}
	...
	for (;;) {
		...
		if (prbuf == NULL) {
			(void) close(mapfd);
			perror(
			    "Pscantext(): cannot allocate buffer for map file");
			return (-1);
		}
		...
		if (nmappings < 0) {
			free(prbuf);
			(void) close(mapfd);
			perror("Pscantext(): cannot read map file");
			return (-1);
		}

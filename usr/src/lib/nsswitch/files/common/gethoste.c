...
#pragma ident	"@(#)gethostent.c	1.18	97/10/29 SMI"
...
static nss_status_t _nss_files_XY_hostbyname();
static int __nss2herrno();
	...
	void			*a;
{
	...
	res = _nss_files_XY_hostbyname(be, argp, argp->key.name);
	if (res != NSS_SUCCESS)
 ...
 */
static nss_status_t
_nss_files_XY_hostbyname(be, args, filter)
	files_backend_ptr_t be;
	nss_XbyY_args_t *args;
	const char *filter;		/* hint for name string */
{
	...
	for (;;) {
		...
		if (nhosts == 0 && strcasestr(instr, filter) == 0) {
			continue;
		} else if (nhosts && strcasestr(instr, hp->h_name) == 0) {
			break;
		}
		...
		*last-- = '\0';
		...
		parsestat = (*args->str2ent)(instr, linelen, thp,
		...
		if (check_name(thp, args)) {
			taddr[nhosts++] = (*(in_addr_t *)thp->h_addr_list[0]);
			if (nhosts == 1) {
				res = NSS_SUCCESS;
	...
	char *as2;
{
	...
	while (*s1) {
		if (tolower(c1 = *s1++) == tolower(c2 = *s2)) {
			tptr = s1;
			while ((tolower(c2 = *++s2) ==
			    tolower(c1 = *s1++)) && c2 != 0)
				;

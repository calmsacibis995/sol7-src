...
#pragma	ident	"@(#)pam_framework_utils.c	1.4	97/01/27 SMI"
...
{
	char *data_buffer = 0;
	static xmutex_rec thread_lock;
	static xthread_key_t thread_key;
	...
	char *s)
{
	...
	if (pamh != NULL) {
	    ...
	    int status = pam_get_data(pamh, filename, (const void**) &msg_data);

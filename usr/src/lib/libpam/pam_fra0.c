...
#ifdef sun
#pragma	ident	"@(#)pam_framework.c	1.9	97/02/24 SMI"
#include <dlfcn.h>
#endif
...
get_pam_conf_entry(struct pam_fh *pam_fh, pamtab **pam)
{
	...
	*pam = 0;
	...
	/* free old information if it exists */
	if (current_line) {
		free(current_line);	current_line = 0;
	}
	if (*pam) {
		free_pamconf(*pam);	*pam = 0;
	}
	...
	if (arg[0] != '/') {
		...
		if (((*pam)->module_path = strdup(arg)) == 0) {
			syslog(LOG_ERR, "strdup: out of memory");
			goto out;
		}
	}

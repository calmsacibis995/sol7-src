...
#pragma ident	"@(#)regex.c	1.21	96/12/06 SMI"
...
{
	vars_storage *varsp = (vars_storage *)0;

	if (_thr_getspecific(*keyp, (void **)&varsp) != 0) {
	    if (_thr_keycreate(keyp, free) != 0) {
		return ((vars_storage *)0);
	    }
	}
	if (varsp == (vars_storage*)0) {
	    varsp = (vars_storage *)calloc(1, sizeof (vars_storage));
	    if (varsp == (vars_storage *)0) {
		return ((vars_storage *)0);
	} else if (_thr_setspecific(*keyp, (void *)varsp) != 0) {
		free((void *)varsp);
		return ((vars_storage *)0);
	    }
	}
	...
	const char *stringp, ...)
{
	...
	if (____loc1() == (char **)0) {
	    ...
	    (void) _mutex_lock(&regex_lock);
	    __loc1 = (char *)0;
	}

	if ((stringp == (char *)0) || (regexp == (char *)0)) {
	    (void) _mutex_unlock(&regex_lock);
	return ((char *)0);
	}

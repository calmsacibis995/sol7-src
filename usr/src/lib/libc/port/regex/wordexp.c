...
#pragma ident	"@(#)wordexp.c 1.8	98/01/29 SMI"
...
static int append ANSI((wordexp_t *wp, char *str));
...
wordexp(const char *word, wordexp_t *wp, int flags)
{
	...
	*optendp = '\0';
	...
	if (pipe(pv) < 0 || (fp = fdopen(pv[0], "rb")) == NULL)
		return (WRDE_ERRNO);
	fd1 = dup(1);
	(void) dup2(pv[1], 1);			/* set stdout to pipe write */
	(void) close(pv[1]);
	fd2 = dup(2);
	if (!(flags&WRDE_SHOWERR)) {
		(void) close(2);
		(void) open(M_NULLNAME, O_WRONLY);
	}
	...
	if ((pid = _fexecve(path, args, environ)) == -1) {
		(void) dup2(fd1, 1);
		(void) dup2(fd2, 2);
		(void) close(pv[0]);
		return (WRDE_ERRNO);
	...
	(void) dup2(fd2, 2);
	(void) close(fd2);
	(void) close(pv[0]);		/* kill shell if still writing */

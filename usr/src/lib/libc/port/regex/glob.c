...
#pragma ident	"@(#)glob.c 1.7	97/08/08 SMI"
...
static int globit ANSI((size_t dend, const char *sp, glob_t *gp, int flag,
			int (*errfn) ANSI((const char *path, int err))));
static int pstrcmp ANSI((const void *, const void *));
static int append ANSI((glob_t *gp, const char *str));
...
glob(const char *pattern, int flags,
	int (*errfn) ANSI((const char *path, int err)), glob_t *gp)
{
	int rv;
	ssize_t i;
	ssize_t ipathc;

...
#pragma ident	"@(#)doscan.c	1.54	97/12/06 SMI"
...
#else  /* _WIDE */
int
__doscan_u(FILE *iop, const char *fmt, va_list va_Alist)
#endif /* _WIDE */
{
#ifdef	_WIDE
	wchar_t	ch;
...
#else  /* _WIDE */
	const char	*sformat = fmt;	/* save the beginning of the format */
#endif /* _WIDE */
	...
	for (; ; ) {
		...
		if (iswspace(ch)) {
			if (!flag_eof) {
				while (iswspace(inchar = _wd_getwc(&chcount, iop)))
					;
...
charswitch:	/* target of a goto 8-( */
...
#ifdef	_WIDE
		for (len = 0; ((ch >= 0) && (ch < 256) && isdigit(ch)); ch = *fmt++)
			len = len * 10 + ch - '0';
#else  /* _WIDE */
		...
		if (ch == '[') {
			...
			p = (wchar_t *)fmt - 1;	/* p points to the address of '[' */
			len = 0;
			if (*fmt == '^') {
				len++;
				fmt++;
			...
			if (bracket_str == NULL)
				return (EOF);
			clen = wcstombs((char *)bracket_str, wbracket_str, wlen + 1);
			free(wbracket_str);
			if (clen == (size_t)-1) {
				free(bracket_str);
				return (EOF);
...
_wd_getwc(int *chcount, FILE *iop)
{
	...
	if (!(iop->_flag & _IOWRT)) {
		...
		if (*iop->_ptr == '\0')
			return (WEOF);
		len = mbtowc((wchar_t *)&wc, (const char *)iop->_ptr, MB_CUR_MAX);
		if (len == -1)
			return (WEOF);
		iop->_ptr += len;
		(*chcount)++;

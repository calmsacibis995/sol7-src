...
#pragma ident	"@(#)doprnt.c	1.69	98/02/18 SMI"
...
#define	PUT(p, n) \
	{ \
		if (sflag) { \
			wchar_t	*tmpwp; \
			tmpwp = ((wchar_t *)bufptr) + n; \
			if (tmpwp > (wchar_t *)bufferend) { \
				(void) wmemcpy((wchar_t *)bufptr, \
					(const wchar_t *)p, \
					(size_t)(bufferend - bufptr)); \
				bufptr = (unsigned char *)bufferend; \
				return ((ssize_t)maxcount); \
			} else { \
				(void) wmemcpy((wchar_t *)bufptr, \
					(const wchar_t *)p, (size_t)n); \
				bufptr = (unsigned char *)tmpwp; \
			} \
		} else { \

...
#pragma	ident  "@(#)gettext.c 1.30     97/10/24 SMI"
...
static struct domain_binding *firstbind = NULL;
static struct nls_cache	*nls_cache = NULL;
	...
	const char	*binding)
{
	...
	(void) mutex_lock(&gt_lock);
	res = _bindtextdomain_u(domain, binding);
	(void) mutex_unlock(&gt_lock);
	return (res);
}
	...
	const char	*binding)
{
...
#ifdef DEBUG
	(void) printf("_bindtestdomain_u(%s, %s)\n", (domain ? domain : ""),
		(binding ? binding : ""));
#endif
	...
	if (binding) {
		...
		if ((bind = malloc(sizeof (struct domain_binding))) == NULL) {
			return (NULL);
		}
	...
	const int	category)
{
	...
	} else if (strlen(domain) > MAX_DOMAIN_LENGTH) {
	...
	/* content of cur_locale will be overridden by */
	...
	while (*cur_binding) {
		...
		if ((p_msg = malloc(sizeof (struct msg_head))) == NULL) {
			free(cur_locale);
			/* errno isn't restored */
			return ((char *)msg_id);
		}
		...
		p_msg->fd = fd;
		if ((p_msg->path = strdup(msgfile)) == NULL) {
			free(p_msg);
			free(cur_locale);
			/* errno isn't restored */
			return ((char *)msg_id);
		...
		struct_size = (int)(sizeof (struct msg_struct)) * msg_count;
...
printlist(void)
{
	...
	(void) fprintf(stderr, "===Printing default list and regural list\n");
	(void) fprintf(stderr, "   Default domain=<%s>, binding=<%s>\n",
	    DEFAULT_DOMAIN, defaultbind);

	ppp = firstbind;
	while (ppp) {
		(void) fprintf(stderr, "   domain=<%s>, binding=<%s>\n",
	...
	int	*pnp)
{
	...
	char	mydomain[MAX_DOMAIN_LENGTH + 1]; /* 256 + 1 + 1 */
	int	nlscount = 0;
	size_t	nlspath_len, domain_len, locale_len;
	size_t	ppaths_len = 0;
	struct nls_cache	*p, *q;

#ifdef DEBUG
	(void) printf("process_nlspath(%s, %s, %s, %d)\n", current_domain,
	...
	if (nls_cache) {		/* cache is not empty */
		...
		if ((p = malloc(sizeof (struct nls_cache))) == NULL) {
			*pnp = -1;		/* error */
			return (NULL);
		}
	} else {
	...
	while (*s) {				/* march through NLSPATH */
		(void) memset(pathname, 0, sizeof (pathname));
		if (*s == ':') {
			/*
			 ...
			 */
			if ((pnlstmp = malloc(sizeof (struct nlstmp)))
			    == NULL) {
	...
	if ((ssize_t)ppaths_len > 0) {
		if ((ppaths = malloc(ppaths_len + 1)) == NULL) {
			allfree(3, lang, p, nlstmp);
			*pnp = -1;	/* error */
			return (NULL);
		}
	...
	if ((p->domain = malloc(domain_len)) == NULL) {
		allfree(4, lang, p, NULL);
		*pnp = -1;	/* error */
		return (NULL);
	} else {
		(void) strcpy(p->domain, current_domain);
	}
	if ((p->locale = malloc(locale_len)) == NULL) {
		allfree(2, lang, p, NULL);
		*pnp = -1;	/* error */
		return (NULL);
	} else {
		(void) strcpy(p->locale, current_locale);
	}
	if ((p->nlspath = malloc(nlspath_len)) == NULL) {
		allfree(1, lang, p, NULL);
		*pnp = -1;	/* error */
		return (NULL);
	} else {

...
#pragma	ident	"@(#)__regexec_std.c 1.15	97/01/28  SMI"
...
/* ******************************************************************** */
/*	__reg_bits[] is also defined in __regcomp_C.c, __regcomp_stdc, */
	...
	size_t nmatch, regmatch_t pmatch[], int eflags)
{
	...
	static const EXEC zero = { 0 };
				/* faster than bzero() call		*/
 ...
 */
	...
	pstr = (uchar_t *) string;
	mb_cur_max = hdl->cmapp->cm_mb_cur_max;

	if (*(uchar_t *) preg->re_comp != CC_BOL) {
		if (mb_cur_max > 1) {
			while (*pstr && pmap[*pstr] == 0) {
				i = mblen((const char *)pstr, mb_cur_max);
...
match_re(uchar_t *ppat, uchar_t *pstr, regex_t *preg, EXEC *pe)
{
	...
	sav_str    = pstr;
	pe->wander = 0;
	mb_cur_max = __lc_charmap->cm_mb_cur_max;
	...
	if (0) {
		...
		if (wander) {
			...
			if (mb_cur_max == 1) {
				...
				int i = mblen((const char *)sav_str,
					mb_cur_max);

				pstr = sav_str + ((i < 1) ? 1 : i);

				while (*pstr && pmap[*pstr] == 0) {
					int i = mblen((const char *)pstr,
						mb_cur_max);
 ...
 */
	...
	case CC_SUBEXP:
		...
		if (((cp2 = (*(uchar_t *)preg->re_sc->re_esub[idx] & CR_MASK))
			== CR_QUESTION || cp2 == CR_STAR) ||
 ...
 */
	...
	case CC_WDOT:
		if (*pstr == 0 || (*pstr == '\n' &&
			...
			goto no_match;
		wclen = mblen((const char *)pstr, mb_cur_max);
		if (wclen < 0)
			wclen = 1;
		pstr += wclen;
		continue;
 ...
 */
	...
	case CC_WBITMAP:
	{
		...
		ucoll = _mbucoll(pe->pcoltbl, (char *)pstr,
			(char **)&ptemp);
 ...
 */
	...
	case CC_I_WBACKREF:
		...
		pea = pe->tmatch[idx].ea;
		while (pback < pea) {
			wclen = MBTOWC_NATIVE(&wc_p,
				(const char *)pback, mb_cur_max);
			if (wclen < 1)
				wc_p = *pback;
			wclen = MBTOWC_NATIVE(&wc_s,
				(const char *)pstr, mb_cur_max);
			if (wclen < 1)
				wc_s = *pstr;
			if ((wc_p != wc_s) &&
				(TOWLOWER_NATIVE(wc_p) !=
 ...
 */
	...
	cc_subexpe:
		...
		if (pe->submatch[idx] < min + max) {
			r = *pe;
			stat = match_re((uchar_t *)
				preg->re_sc->re_lsub[idx], pstr, preg, &r);
 ...
 */
	...
	if (*pstr != '\0' && (*pstr != '\n' ||
		...
		pb->bit_count++;
		i = mblen((const char *)pstr, __lc_charmap->cm_mb_cur_max);
		if (i < 0)
			i = 1;
 ...
 */
static wchar_t
prev_wchar(uchar_t *beg, uchar_t *cur, size_t n)
{
	wchar_t wc;
	int len;
	uchar_t *prev, *s;

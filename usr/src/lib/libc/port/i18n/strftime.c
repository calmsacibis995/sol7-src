 ...
 * All rights reserved.
 */
#pragma	ident	"@(#)strftime.c 1.25	97/11/13  SMI"
...
#define	BADFORMAT  	bufp = (format = fbad, "%")

#define	BUFSIZE		1000
#define	PUT(c)		((size + 1) < maxsize ? (*strp++  = (c), size++) : \
				toolong++)

#define	STRTONUM(str, num)	{			\
				num = 0; 		 \
				while (isdigit (*str)) { \
					num *= 10; 	 \
					num += *str++ - '0'; \
	...
	while ((ch = *format++) != '\0') {
		if (ch != '%')
			...
			case 'r': 	/* X/Open - time in AM/PM notation */
				(void) DOFORMAT(hdl, locbuffer, BUFSIZE,
			...
			case 'X': 	/* X/Open - the locale time notation */
				...
				(void) DOFORMAT(hdl, locbuffer, BUFSIZE, p,
					timeptr, 0);

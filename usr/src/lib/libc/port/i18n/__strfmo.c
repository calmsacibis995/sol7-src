...
#pragma	ident	"@(#)__strfmon_std.c 1.16	97/04/19  SMI"
...
static ssize_t	do_format(_LC_monetary_t *, char *, size_t,
    ...
    double dval, struct global_flags *flags_g)
{
	...
	char *lead_zero = NULL;	/* pointer to buffer for leading zeros */
	...
	*/
	...
	if (2*maxsize < SIZE)
		temp = temp_buff;
	else if ((temp_m = malloc(2 * maxsize)) != NULL)
		temp = temp_m;
	else
		return (-1);
	/*
	...
	*/
	if ((i = j = (flags_g->n_width - dig_to_left)) > 0) {
		...
		if ((lead_zero = malloc(strlen(number) + i + 2)) == NULL) {
			free(buf);
			if (temp_m)
				free(temp_m);
			return (-1);
 ...
 *
 * PARAMETERS:
 *           _LC_monetary_t hdl - the handle of the pointer to the
    ...
    int sign, struct global_flags *flags_g)
{
	...
	if (flags_g->nflag) {
		...
		*padded = '\0';
		padded = p;
		(void) strcat(padded, pn_sign);
	} else
		padded = pn_sign;

	do_out_cur_sign(pn_cs_precedes, pn_sign_posn,
		pn_sep_by_space, padded, begin, end, flags_g);
...
digits_to_left(double dvalue)
{
	struct log {	/* scale factors */
		...
		int	pow;
			...
			1e2,    2,
			1e1,    1 };
	struct log	*scale = log;
	int		digits = 1;	/* default (no scale) */

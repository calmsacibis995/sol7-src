...
#pragma	ident	"@(#)strptime.c 1.51	97/11/25  SMI"
...
static int compare_str(const unsigned char *, const unsigned char *, size_t *);
...
recurse:
		...
		case 'C':		/* Base year */
			{
			size_t len;
			if (compare_str(buf, (const unsigned char *)era->name,
			    &len) != 0)
				continue;
			buf += len;
			goto matched;
		...
		case 'y':		/* offset from %EC(year only) */
			if ((off = GET_NUMBER(&buf, 8, NULL, NULL)) ==
					-1) {
				continue;
			} else {
				/*
		...
		strptime_data_t *strptime_data, int flag)
{
	...
	size_t	width;		/* the field width of an locale entry */
	size_t 	lwidth;		/* the field width of an locale entry */
	int	i;
	...
	if (flag == 0)
		getnow(tm, ct);
	SKIP_TO_NWHITE(fmt);
	while (((fmtchr = *fmt++) != NULL) && (bufchr = *buf)) {
		...
		if (fmtchr != '%') {
			...
			case 'p':		/* locale's AM or PM */
				SKIP_TO_NWHITE(buf);
				if (compare_str(buf, (const unsigned char *)
				    hdl->am_pm[0], &width) == 0) {
					i = AM;
					buf += width;
				} else if (compare_str(buf,
				    (const unsigned char *)
				    hdl->am_pm[1], &lwidth) == 0) {
					i = PM;
					buf += lwidth;
				}
				else
 ...
 */
static struct tm *
calc_date(struct tm *t, struct tm *ct)
{
	time_t	tv;
	struct  tm nct;
...
static void
getnow(struct tm *t, struct tm *ct)	/*  get current date */
{
	time_t now;
...
verify_getdate(struct tm *t, struct tm *ct, strptime_data_t *strptime_data)
{
	 ...
	 * start at 1902, and stop at 2038.  If year is set, bounds
	 * check between 1901-2037.
	 */
	if ((set_yr != 0) && ((t->tm_year < 2) ||
	    (t->tm_year > 137))) {
		wrong_input++;
		return (0);
	}
...
#define	STRTOK_R(a, b, c)	strtok_r((a), (b), (c))
	...
	for (i = 0, num = -1; cand; i++) {
		if (compare_str((const unsigned char *)*buf,
		    (const unsigned char *)cand, &length) == 0) {
...
static int
compare_str(const unsigned char *s1, const unsigned char *s2, size_t *n)
{
	ssize_t count;

	SKIP_TO_NWHITE(s2);	/* assume s1 already points to non-white */
	*n = strlen((char *)s2);	/* return number of bytes */
	count = (ssize_t) *n;
	...
	while (--count >= 0 && (tolower(*s1) == tolower(*s2++)))
		if (*s1++ == '\0')
			return (0);
	return (count < 0 ? 0 : tolower(*s1) - tolower(*(s2 - 1)));
}

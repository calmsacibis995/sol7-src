...
#pragma ident	"@(#)forw_pos.c 1.9	97/04/09 SMI"

/*LINTLIBRARY*/

/*
#if !defined(lint) && !defined(_NOIDENT)
static char sccsid[] = "@(#)13	1.1.1.1  "
	"src/bos/usr/ccs/lib/libc/forw_pos.c, bos, bos410 5/25/92 14:08:09";
#endif
 */
/*
 *   COMPONENT_NAME: LIBCSTR
 *
 *   FUNCTIONS: forw_pos_collate_std,forw_pos_collate_sb
 *
 *   ORIGINS: 27
 *
 *
...
int
forw_pos_collate_std(_LC_collate_t *hdl, const char *str1, const char *str2,
			int order)
{
	wchar_t wc;
	int rc;
	...
	while ((*str1 != '\0') && (*str2 != '\0')) {
		...
		/*
		 * If the collating values are not zero, check the position in
		 * the string.
		 */
	...
	/*
	 * To get here, str1 and/or str2 are NULL and they have been equal
	 * so far.  If one of them is non-null, check if the remaining
	 * characters are non-collating.  If they are, then the strings are
	 * equal for this order.
	 */
	if (*str1 != '\0') {
		/*
		 * If str1 is non-null and there are collating characters left,
		 * then str1 is greater than str2.
		 */
		do {
			if ((rc = (METHOD_NATIVE(__lc_collate->cmapp, mbtowc)(
			    __lc_collate->cmapp, &wc, str1,
		...
		/*
		 * If str2 is non-null and there are collating characters left,
		 * then str1 is less than str2.
		 */
		do {
			if ((rc = (METHOD_NATIVE(__lc_collate->cmapp, mbtowc)(
			    __lc_collate->cmapp, &wc, str2,
...
int
forw_pos_collate_sb(_LC_collate_t *hdl, const char *str1, const char *str2,
			int order)
{
	wchar_t wc;
	wchar_t str1_colval;

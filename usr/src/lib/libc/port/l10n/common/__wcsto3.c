...
#pragma	ident	"@(#)__wcstombs_dense_pck.c 1.8	97/09/23  SMI"

/*
 * static char sccsid[] = "@(#)47	1.4.1.2  src/bos/usr/ccs/lib/libc/"
 * "__wcstombs_dense_pck.c, bos, bos410 1/12/93 11:11:36";
*/
/*
 * COMPONENT_NAME: (LIBCCPPC) LIBC Code-Point/Process-Code Conversion Functions
 *
 * FUNCTIONS: __wcstombs_dense_pck
 *
 * ORIGINS: 27
 *
...
extern int	__wctomb_dense_pck(_LC_charmap_t *, char *, wchar_t);

size_t
__wcstombs_dense_pck(_LC_charmap_t *hdl, char *s, const wchar_t *pwcs,
						size_t n)
{
	size_t cnt = 0;
	int len = 0;
	...
	if (s == (char *)NULL) {
		...
		if ((len = __wctomb_dense_pck(hdl, tmps, *pwcs)) == -1)
			return ((size_t)-1);
	    cnt += len;
	    pwcs++;
	}

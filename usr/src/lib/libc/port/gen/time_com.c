...
#pragma ident	"@(#)time_comm.c	1.56	98/01/08 SMI"
...
mktime(struct tm *timeptr)
{
	...
	/*
	 * calculate a time_t value based on the the struct tm passed in.
...
#ifdef _LP64
	/*
	 * With a 64-bit time_t value, time can range from forever
	 * to forever.  But let's stick to 4-digit years.
	 */
	if ((timeptr->tm_year < 1) || (timeptr->tm_year >= 8100))
		bad_year = 1;
#else
	/*
...
_ltzset_u(time_t tim)
{
	...
	if (_tzload(name) != 0) {
		...
		/*
		 * if the time has not been set by the caller, use the
		 * current time.
		 */
		if (tim == 0)
			tim = time(NULL);

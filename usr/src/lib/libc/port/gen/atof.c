...
#pragma	ident	"@(#)atof.c	1.13	96/11/14 SMI"	/* SVr4.0 2.25	*/
...
#if STRTOD
#define	STORE_PTR	(*ptr = (char *)p)
#define	DEC_PTR		(*ptr = (char *)(p - 1))
#define	GOT_DIGIT	(got_digit++)
#define	RET_ZERO(val)	if (!got_digit) return (0.0)

double
strtod(const char *p, char **ptr)
#else
#define	STORE_PTR
#define	DEC_PTR
#define	GOT_DIGIT
...
#endif
{
	...
	{	/* accumulate value */
		long high = 0, low = 0, scale = 1;
		int decpt = 0, nzeroes = 0;
	...
	if (exp != 0) { /* apply exponent */
		double *powptr = pow1_25, fl_exp = fl_val;

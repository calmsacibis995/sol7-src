...
#pragma ident	"@(#)nss_common.c	1.18	98/02/03 SMI"
...
#define	NSS_ROOTLOCK(r, sp)	((void) mutex_lock(&(r)->lock), *(sp) = (r)->s)

#define	NSS_UNLOCK(r)		((void) mutex_unlock(&(r)->lock))

#define	NSS_CHECKROOT(rp, s)	((s) != (*(rp))->s &&			\
				((void) mutex_unlock(&(*(rp))->lock),	\
				(void) mutex_lock(&(s)->orphan_root.lock),\
				*(rp) = &(s)->orphan_root))

#define	NSS_RELOCK(rp, s)	((void) mutex_lock(&(*(rp))->lock),	\
				NSS_CHECKROOT(rp, s))
...
_nss_db_state_destr(struct nss_db_state *s)
{
	...
	if (s->config != 0) {
		(void) __nsw_freeconfig(s->config);
	}
	if (s->src != 0) {
		int	n_src;
...
_nss_status_vec_p(void)
{
	...
	if (!key_created) {
		...
		key_created =
		    key_created || _thr_keycreate(&status_vec_key, free) == 0;
		(void) mutex_unlock(&keylock);
	}
	if (!key_created) {
		return (NULL);
	}
	(void) _thr_getspecific(status_vec_key, &status_vec_p);
...
nss_setent(nss_db_root_t *rootp, nss_db_initf_t initf, nss_getent_t *contextpp)
{
	...
	(void) mutex_lock(&contextpp->lock);
	nss_setent_u(rootp, initf, contextpp);
	(void) mutex_unlock(&contextpp->lock);
}

...
#pragma	ident	"@(#)getgrnam_r.c	1.26	97/07/31 SMI"
...
/* align buffer on a pointer boundry 4bytes in 32bits and 8 bytes in 64bits */
	...
	if (sptr->nsc_ret.nsc_bufferbytesused -
		(sizeof (group32_t) + sizeof (char **) +8) > buflen) {
#else
 ...
 */
	...
	memcpy(buffer, (sptr->nsc_ret.nsc_u.buff + sizeof (group32_t)),
	...
	group64.gr_mem = (char **) (buffer + sptr->nsc_ret.nsc_bufferbytesused);
	group64.gr_mem = (char **)(((uintptr_t)group64.gr_mem + 7) & ~7);

	gr_mem32 = (char **)sptr->nsc_ret.nsc_u.grp.gr_mem;
	buffer32 = buffer;

	for (i = 0; 1; i++) {
		index = *((uptr32_t *) ((unsigned long) gr_mem32 + buffer32));
		if (index == 0)
			break;
		group64.gr_mem[i] = (char *) (index + buffer);
		if (&group64.gr_mem[i] > (char **) (buffer + buflen)) {
			errno = ERANGE;
			return (NULL);
		}
	...
	*result = group64;
	...
	sptr->nsc_ret.nsc_u.grp.gr_mem =
	    (char **)((char *) sptr->nsc_ret.nsc_u.grp.gr_mem +
		(uintptr_t) buffer);

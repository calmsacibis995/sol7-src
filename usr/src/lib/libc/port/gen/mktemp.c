...
#pragma ident	"@(#)mktemp.c	1.13	97/02/05 SMI"	/* SVr4.0 1.16	*/
...
mktemp(char *as)
{
	static int		seeded = 0;
	char			*num, *first_x, *s;
	unsigned int		xcnt;	/* keeps track of number of X's seen */
	struct stat64		buf;
	...
	first_try = current_try = (int) lrand48();
	...
	do {
		...
		s = first_x;
		if (xcnt != 0) {
			(void) sprintf(s, "%*.*s", xcnt, xcnt, num);
		}

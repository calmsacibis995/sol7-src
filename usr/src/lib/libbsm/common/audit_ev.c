#ifndef lint
static char	sccsid[] = "@(#)audit_event.c 1.20 97/10/29 SMI";
#endif
	...
	au_event_ent_t *au_event_entry;
{
	...
	while (fgets(input, 256, au_event_file)) {
		if (input[0] != '#') {
			found = 1;
			s = input;
			...
			i = strcspn(s, ":");
			s[i] = '\0';
			(void) strncpy(au_event_entry->ae_name, s,
				AU_EVENT_NAME_MAX);
			s = &s[i+1];
			...
			i = strcspn(s, "\n\0");
			s[i] = '\0';
			au_event_entry->ae_class = flagstohex(s);

...
#ident	"@(#)devfswalk.c	1.22	97/07/16 SMI"
...
static kvm_t *(*kopen)(char *, char *, char *, int, char *);
static int (*kclose)(kvm_t *);
static int (*kread)(kvm_t *, unsigned long, char *, unsigned);
...
local_addr(caddr_t addr)
{
	...
	for (i = 0; i < CACHESIZE; i++)
	    if (cache[i].va != NULL &&
		(caddr_t)((uintptr_t)addr & pagemask) == cache[i].va) {
			hitcnt++;
			return (const caddr_t)(cache[i].pa +
				((uintptr_t)addr & pageoffset));
	}

	if ((pa = mmap(0, 2 * pagesize, PROT_READ, MAP_SHARED, km_fd,
	    (uintptr_t)addr & pagemask)) == (caddr_t)-1) {
	...
	cache[cp].va = (caddr_t)((uintptr_t)addr & pagemask);
	cache[cp].pa = pa;
	cp = (cp + 1) % CACHESIZE;
	misscnt++;

	return (const caddr_t)(pa + ((uintptr_t)addr & pageoffset));
}
 ...
 */
static boolean_t
node_type_match(const char *desired, const char *node)
{
...
/*
 * Called by devinfo_prop_get_* routines.  Expects that devfs_kvmsetup
 * has already been called so that global kd is valid.  This function
 * does the bulk of the work by examining all 3 property lists for
 * the property name and type.  Returns data in npropp if match found.
 */
int
devinfo_prop_get_common(dev_t dev, const dev_info_t *dip, char *name,
		    ddi_prop_t *npropp, int type)
{
	ddi_prop_t *propa[3], *kpropp;
	int proplist = 0;
	char c, *kaddr, *namep;

	/* search driver list, system list, and hardware list, in this order */

	propa[0] = DEVI(dip)->devi_drv_prop_ptr;
	propa[1] = DEVI(dip)->devi_sys_prop_ptr;
	propa[2] = DEVI(dip)->devi_hw_prop_ptr;

	for (; proplist < 3; proplist++) {

		for (kpropp = propa[proplist]; kpropp != 0;
		    kpropp = npropp->prop_next)  {

			if ((*kread)(kd, (u_long)kpropp, (char *)npropp,
				    sizeof (ddi_prop_t)) !=
			    sizeof (ddi_prop_t)) {
				return (DEVFS_ERR);
			}
			 ...
			 * the prop has been undefined, or
			 * the data type of the property does not match, or
			 * the properties device number does not match.
			 */

			if ((npropp->prop_flags & DDI_PROP_UNDEF_IT) ||
			    !(npropp->prop_flags & type) ||
			    ((dev != DDI_DEV_T_ANY) &&
			    (dev != npropp->prop_dev)))
				continue;

			/*
			 * now check to see if name of this prop matches 'name'
			 */

			for (namep = name, kaddr = npropp->prop_name; /*EMPTY*/;
				namep++) {
				if ((*kread)(kd, (u_long)kaddr++, (char *)&c, 1)
				    != 1) {
					return (DEVFS_ERR);
				}
				if (c == (char)0 || *namep == 0 || c != *namep)
					break;
			}

			/*
			 * return this property if the names matched
			 */
	...
	return (DEVFS_NOTFOUND);

}

/*
 * Once we kvm_open, remain open for life of executable, using the same
 * rationale localaddr uses when leaving pages mmaped in:  whoever calls us
...
devinfo_kvmsetup()
{
	...
	if ((kd = (*kopen)(NULL, NULL, NULL, O_RDONLY, "devinfo_dlopen")) ==
	    (kvm_t *)0) {
		perror(kopen_name);
		exit(1);
	}
...
int
prop_int_from_prom(u_char *intp, int n)
{
	int	i = 0;
...
/*
 * This function examines the property lists of the dev_info node looking
 * for property "prop_name" of type int.
 */
int
devinfo_prop_get_int(dev_t dev, const dev_info_t *dip, char *prop_name,
		    int *prop_data)
{
	ddi_prop_t prop;
	int error;
	...
	if (!(error = devinfo_prop_get_common(dev, dip, prop_name, &prop,
				    DDI_PROP_TYPE_INT))) {

		if (prop.prop_len == sizeof (int))  {
			if ((*kread)(kd, (u_long)prop.prop_val,
				    (char *)prop_data,
				    (unsigned)prop.prop_len) !=
			    (int)prop.prop_len) {
				return (DEVFS_ERR);
			}
			*prop_data = prop_int_from_prom((u_char *)prop_data,
							sizeof (int));
			return (0);
		}

		/*
		 * the int prop name is found, but its size
		 * is > int (it is an array) so return not found.
		 */

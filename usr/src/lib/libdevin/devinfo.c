...
#pragma ident   "@(#)devinfo.c 1.5     98/01/08 SMI"
...
di_init_driver(const char *drv_name, uint_t flag)
{
	...
	strcpy(driver, drv_name);

	/*
	 * open the devinfo driver
	 */
	if ((fd = open("/devices/pseudo/devinfo@0:devinfo", O_RDONLY)) == -1) {
	...
	struct di_priv_data *priv)
{
	...
	u_int pageoffset = sysconf(_SC_PAGESIZE) - 1;
	u_int pagemask = ~pageoffset;
	...
	tmp = phys_path;
	if (strlen(tmp) == 0)
		sprintf(dinfo_io.root_path, "/");
	else if (*tmp != '/')
		sprintf(dinfo_io.root_path, "/%s", tmp);
	else
		sprintf(dinfo_io.root_path, "%s", tmp);
	...
	/*
	 * Verify that there is not major conflict, i.e., we are indeed
	 * opening the devinfo driver.
	 */
...
{
	if (DINO(node)->nodeid == DEVI_PSEUDO_NODEID)
		return (DI_PSEUDO_NODEID);

	if (DINO(node)->nodeid == DEVI_SID_NODEID)
...
di_minor_next(di_node_t node, di_minor_t minor)
{
	...
	if (minor != DI_MINOR_NIL) {
		if (DIMI(minor)->next != 0)
			return ((caddr_t)minor - DIMI(minor)->self
			    + DIMI(minor)->next);
		else {
			errno = ENXIO;
			return (DI_MINOR_NIL);
	...
	if (DINO(node)->minor_data != 0)
		return ((caddr_t)node - DINO(node)->self +
...
di_prop_drv_next(di_node_t node, di_prop_t prop)
{
	...
	if (prop != DI_PROP_NIL) {
		if (DIPROP(prop)->next)
			return ((caddr_t)prop - DIPROP(prop)->self
			    + DIPROP(prop)->next);
		else
			return (DI_PROP_NIL);
	}
...
di_prom_init()
{
	...
	mutex_init(&p->lock, USYNC_THREAD, NULL);
...
/*
 * Internal library interfaces walking the OBP tree
 *	Returns nodeid or 0 if none exist.
 *
 *	XXX: ph->lock must be held for the duration of call.
 */
static int
prom_next(di_prom_handle_t ph, int id)
{
	struct di_prom_handle *p = (struct di_prom_handle *)ph;
	struct openpromio *opp = &(p->oppbuf.opp);
	int *ip = (int *)(opp->oprom_array);

	dprintf((stderr, "Get peer of 0x%x\n", id));
	bzero(opp->oprom_array, OBP_MAXBUF);
	opp->oprom_size = OBP_MAXPROPLEN;
	*ip = id;
	if (ioctl(p->fd, OPROMNEXT, opp) < 0)
		return (0);
...
static int
prom_child(di_prom_handle_t ph, int id)
{
	struct di_prom_handle *p = (struct di_prom_handle *)ph;
	struct openpromio *opp = &(p->oppbuf.opp);
	int *ip = (int *)(opp->oprom_array);

	dprintf((stderr, "get child of 0x%x\n", id));
	bzero(opp->oprom_array, OBP_MAXBUF);
	opp->oprom_size = OBP_MAXPROPLEN;
	*ip = id;
	if (ioctl(p->fd, OPROMCHILD, opp) < 0)
		return (0);
...
/*
 * Internal library interface for locating a given nodeid
 *	Returns 1 if located, 0 if nodeid doesn't exist.
 *
 *	XXX: ph->lock must be held for the duration of call.
 */
static int
di_prom_setnodeid(di_prom_handle_t ph, int nodeid)
{
	int depth = 1;
	int id[MAX_TREE_DEPTH];

	id[0] = prom_next(ph, 0);

	while (depth) {
		dprintf((stderr, "id[%d] = 0x%x\n", depth - 1, id[depth - 1]));
		if (id[depth - 1] == nodeid)
			return (1);

		if (id[depth] = prom_child(ph, id[depth - 1])) {
			depth++;
			continue;
		}

		while (depth &&
		    ((id[depth - 1] = prom_next(ph, id[depth - 1])) == 0))
			depth--;
	}
	return (0);
}
	...
	di_prom_prop_t prom_prop)
{
	...
	/*
	 * If openprom supported OPROMSETNODEID, one ioctl would
	 * do the job. As it is, we need to walk to the OBP tree
	 * and find a match.
	 *
	 * XXX Should performance becomes a problem, openprom driver
	 *	can be modified to implement OPROMSETNODEID.
	 */
	if (di_prom_setnodeid(ph, nodeid) == 0) {
	...
	if ((ioctl(p->fd, OPROMGETPROP, opp) < 0) || (opp->oprom_size == -1)) {
		free(prop->name);
		free(prop);
		return (DI_PROM_PROP_NIL);
	}
...
di_prom_prop_lookup_common(di_prom_handle_t ph, di_node_t node,
	const char *prom_prop_name, void *data)
{
	struct openpromio *opp;
	...
	if (di_prom_setnodeid(ph, DINO(node)->nodeid) == 0) {
		errno = ENXIO;
		dprintf((stderr, "*** Nodeid not found 0x%x\n",
		    DINO(node)->nodeid));
		mutex_unlock(&p->lock);
	...
	bzero(opp, OBP_MAXBUF);
	opp->oprom_size = OBP_MAXPROPLEN;
	strcpy(opp->oprom_array, prom_prop_name);
	...
	const char *prom_prop_name, int **prom_prop_data)
{
	...
	len = di_prom_prop_lookup_common(ph, node, prom_prop_name,
		(void *)prom_prop_data);
	...
	return (di_prop_decode_common((void *)prom_prop_data, len,
		DI_PROP_TYPE_INT, 1));
}
	...
	const char *prom_prop_name, char **prom_prop_data)
{
	 ...
	 */
	if ((*prom_prop_data)[len - 1] != '\0') {
		char *str;
		len++;
		if ((str = malloc(len)) == NULL)
			return (-1);
		bcopy(*prom_prop_data, str, len - 1);
		str[len - 1] = '\0';
		free(*prom_prop_data);
		*prom_prop_data = str;
		dprintf((stderr, "OBP string not NULL terminated: "
		    "node=%s, prop=%s, val=%s\n",
		    di_node_name(node), prom_prop_name, *prom_prop_data));
	}

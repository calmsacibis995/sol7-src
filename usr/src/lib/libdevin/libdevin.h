...
#pragma ident	"@(#)libdevinfo.h	1.4	97/12/03 SMI"
...
typedef void * di_node_t;	/* handle to a node in snapshot */
typedef void * di_minor_t;	/* handle to a minor data in snapshot */
typedef void * di_prop_t;	/* handle to a property in snapshot */
typedef void * di_prom_prop_t;	/* handle to a prom property */
...
/*
 * Private debug interface
 */
extern void di_enable_debug();

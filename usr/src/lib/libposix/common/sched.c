...
#pragma ident	"@(#)sched.c	1.10	97/07/30 SMI"
...
sched_getparam(pid_t pid, struct sched_param *param)
{
	...
	if (pcparm.pc_cid == rt_class.pcc_info.pc_cid) {
		...
		param->sched_priority = ts_class.pcc_primin;
		param->sched_nice =
			((tsparms_t *)pcparm.pc_clparms)->ts_upri;
		param->sched_nicelim =
			((tsparms_t *)pcparm.pc_clparms)->ts_uprilim;
		...
		*((pc_clparms_t *)(&(param->sched_priority)+1)) =
		    *((pc_clparms_t *)pcparm.pc_clparms);
		param->sched_priority =
		    sched_get_priority_min(pcparm.pc_cid + _SCHED_NEXT);
	}
...
sched_getscheduler(pid_t pid)
{
	...
	if (pcparm.pc_cid == rt_class.pcc_info.pc_cid)
		 ...
		 */
		policy = _SCHED_NEXT + pcparm.pc_cid;
	}
...
sched_yield(void)
{
	if (thr_main() == -1)
		...
		thr_yield();	/* multithreaded, libthread has been linked */
	return (0);
}

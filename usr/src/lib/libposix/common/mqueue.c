...
#pragma ident   "@(#)mqueue.c 1.17     97/11/04     SMI"
...
static void
mq_init(mqhdr_t *mqhp, int msgsize, int maxmsg)
{
	int	i;
	size_t	temp;
	msghdr_t *currentp;
...
mq_unlink(const char *name)
{
	...
	(void) __pos4obj_unlink(name, MQ_LOCK_TYPE);
	__pos4obj_unlock(lfd);
	return (err);
...
mq_send(mqd_t mqdes, const char *msg_ptr, size_t msg_len, unsigned int msg_prio)
{
	 ...
	 * this mechanism does _not_ guarantee that kicked process will
	 * be able to receive a message without blocking;  another
	 * receiver could intervene in the mean time.  Thus,
	 ...
	 * there are clearly receivers blocked.
	 */
	 ...
	 * mq_notempty.  This prevents the notification check, above,
	 * from missing a notification.
	 */
	(void) sem_post(&mqhp->mq_notempty);
	(void) sem_post(&mqhp->mq_exclusive);
	MQ_ASSERT_SEMVAL_LEQ(&mqhp->mq_notempty, mqhp->mq_maxmsg);
...
mq_receive(mqd_t mqdes, char *msg_ptr, size_t msg_len, unsigned int *msg_prio)
{
	...
	(void) sem_post(&mqhp->mq_exclusive);
	(void) sem_post(&mqhp->mq_notfull);
	MQ_ASSERT_SEMVAL_LEQ(&mqhp->mq_notfull, mqhp->mq_maxmsg);

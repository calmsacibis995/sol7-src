 ...
 * All rights reserved.
 *
 * ident  "@(#)dmech.c 1.23     98/01/30 SMI"
 *
	...
	gss_cred_id_t *delegated_cred_handle;
{
	dummy_gss_ctx_id_t ctx;
	char token_string[64];
	gss_buffer_t name = (gss_buffer_t)malloc(sizeof (gss_buffer_desc));
	OM_uint32 status;
	gss_name_t temp;
	unsigned char *ptr;
	int bodysize;
...
dummy_gss_compare_name(ctx, minor_status, name1, name2, name_equal)
	...
	int *name_equal;
{
	dummy_name_t name_1 = name1;
	dummy_name_t name_2 = name2;
...
*/
		...
		memcpy(output_name_buffer->value, name->buffer->value,
			name->buffer->length);
		if (output_name_type)
			status = generic_gss_copy_oid(minor_status, name->type,
				output_name_type);
	...
	gss_OID_set *mechanisms;
{
	...
	*name = (gss_name_t)make_dummy_token("dummy gss credential");

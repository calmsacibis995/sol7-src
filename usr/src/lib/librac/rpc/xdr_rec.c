...
#pragma ident	"@(#)xdr_rec.c	1.6	97/12/17 SMI"

#if !defined(lint) && defined(SCCSIDS)
static char sccsid[] = "@(#)xdr_rec.c	1.6	97/12/17 SMI";
#endif
	...
	register XDR *xdrs;
{
	...
	pos = lseek((int)rstrm->tcp_handle, (int) 0, 1);
	if (pos != -1)
		switch (xdrs->x_op) {
	...
	bool_t sendnow;
{
	...
	if (sendnow || rstrm->frag_sent ||
		((uint32_t)rstrm->out_finger + sizeof (u_int) >=
	...
	bool_t eor;
{
	...
	register u_int len = (uint32_t)(rstrm->out_finger) -
		(uint32_t)(rstrm->frag_header) - sizeof (u_int);

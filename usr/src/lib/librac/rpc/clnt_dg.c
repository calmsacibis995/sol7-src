 ...
 * All rights reserved.
 */
#ident	"@(#)clnt_dg.c	1.12	97/09/24 SMI"

#if !defined(lint) && defined(SCCSIDS)
static char sccsid[] = "@(#)clnt_dg.c 1.32 91/06/25 Copyr 1988 Sun Micro";
#endif
...
int __rpc_select_to_poll();
int __rpc_timeval_to_msec();
int xdr_opaque_auth();
static int netbuf_copy();
...
err2:
	if (cl) {
		...
		if (cu) {
			mem_free(cu->cu_addr.buf, cu->cu_addr.len);
			mem_free((caddr_t)cu, sizeof (*cu));
		}
	}
	return ((CLIENT *)NULL);
	...
	register int flag;
{
	...
	for (;;) {
		...
		if (_svc_getreqset_proc)
			/* ``+ 1'' because of pfdp[0] */
			nfds = __rpc_select_to_poll(dtbsize,
						    &svc_fdset, &pfdp[1]) + 1;
		else
	...
	char *info;
{
	...
	case CLSET_SVC_ADDR:		/* set to new address */
		...
		if (cu->cu_addr.maxlen < addr->len) {
			mem_free(cu->cu_addr.buf, cu->cu_addr.maxlen);
			if ((cu->cu_addr.buf = mem_alloc(addr->len)) == NULL)
				return (FALSE);
	...
	CLIENT *cl;
{
	...
	if (cu->cu_addr.buf)
		(void) mem_free(cu->cu_addr.buf, cu->cu_addr.len);
	...
	struct callinfo	*h;
{
	 ...
	 */
	pktxid = *(uint32_t *) (tmp_trdata->udata.buf);
	if (pktxid == h->ci_xid) {

...
#pragma	ident	"@(#)reg_step.c	1.19	97/08/12 SMI" /* SVr4.0 1.1.7.5 */
...
_get_vars_storage(thread_key_t *key)
{
	...
	if (_thr_getspecific(*key, (void **)&vars) != 0) {
		if (_thr_keycreate(key, free) != 0) {
			return (NULL);
		}
	}
	if (!vars) {
		if (_thr_setspecific(*key, (void *)(vars =
...
static cclass(char *ep, char **rp, int neg)
{
	char	*lp;
	wchar_t	c, d, f;
	int	n;

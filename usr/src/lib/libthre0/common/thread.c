...
#pragma ident	"@(#)thread.c	1.181	98/01/14 SMI"
			...
			void *arg, long flags, thread_t *new_thread)
{
	...
	return (_thrp_create(stk, stksize, func, arg,
	    flags, new_thread, 0, _lpagesize));
}

int
_thrp_create(void *stk, size_t stksize, void *(*func)(), void *arg,
		long flags, thread_t *new_thread, int prio, size_t guardsize)
{
	int tid;
	thrtab_t *tabp;
	uthread_t *first;
...
_thrp_continue(thread_t tid, int whystopped)
{
	...
	if ((t = THREAD(tid)) == (uthread_t *)-1) {
		_unlock_bucket(ix);
		return (ESRCH);
	} else if (whystopped != TSTP_INTERNAL) {
		if (t->t_flag & T_INTERNAL) {
			_unlock_bucket(ix);
			return (ESRCH);
		}
...
_thr_exit_common(void *status, int ex)
{
	...
	*((int *)&(t->t_can_pending)) = 0;
	...
	if (_cleanuphndlr != NULL &&
	    ...
	    (uintptr_t) status == PTHREAD_CANCELED) {
		(*_cleanuphndlr)();
	}
...
_thr_stksegment(stack_t *stk)
{
	...
	if (_t0 == NULL)
		...
		if (_thr_main() == 1) {
			if (_getcontext(&uc) == 0) {
				curthread->t_stk = (caddr_t)uc.uc_stack.ss_sp +
					uc.uc_stack.ss_size;
				_getrlimit(RLIMIT_STACK, &rl);
				curthread->t_stksize = rl.rlim_cur;
			} else {
				return (errno);
			}

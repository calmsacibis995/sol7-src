...
#pragma ident	"@(#)libthread.h	1.126	98/02/02 SMI"
...
typedef struct thread32 {
	...
	int		t_rtldbind;	/* dynamic linking flags */
	/* libthread_db support */
	caddr32_t	t_td_evbuf;	/* libthread_db event buffer */
	/* PROBE_SUPPORT begin */
...
extern int _dqactmap[];		/* bit map of priority queues */
...
#define	NSLEEPQ		64
#define	slpqhash(X)	(&_slpq[((uintptr_t)X >> 4) & (NSLEEPQ - 1)])
...
extern mutex_t _devzero_dontfork_lock;
extern u_int   _devzero_dontfork;
extern cond_t  _devzero_dontfork_cv;
...
extern void (*_tsiguhandler[])(int, siginfo_t *, ucontext_t *, sigset_t *);
...
uthread_t *_idle_thread_create(int size);
...
int	_thr_alloc_ev_struct(uthread_t *);

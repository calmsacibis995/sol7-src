...
#pragma ident	"@(#)ma.c	1.39	98/02/02 SMI"
...
mutex_t _tsslock;
mutex_t _devzero_dontfork_lock = DEFAULTMUTEX;
u_int   _devzero_dontfork = 0;
cond_t  _devzero_dontfork_cv = DEFAULTCV;
...
_alloc_chunk(caddr_t at, size_t size, caddr_t *cp)
{
	...
	_lmutex_lock(&_devzero_dontfork_lock);
	_devzero_dontfork++;
	_lmutex_unlock(&_devzero_dontfork_lock);
	if ((devzero = _open("/dev/zero", O_RDWR)) < 0) {
		rc = 0;
	} else {
		*cp = (caddr_t) _mmap(at, size, sysconf(_SC_STACK_PROT),
	...
	_lmutex_lock(&_devzero_dontfork_lock);
	if (--_devzero_dontfork == 0)
		_cond_signal(&_devzero_dontfork_cv);
	_lmutex_unlock(&_devzero_dontfork_lock);
	return (rc);
}

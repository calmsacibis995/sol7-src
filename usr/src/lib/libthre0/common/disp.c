...
#pragma ident	"@(#)disp.c	1.105	97/08/27 SMI"
...
int 	_dqactmap[MAXRUNWORD];	/* bit map of priority queues */
...
again:
	...
	/*
	 * if thread is being suspended, manifest its state, and
	 * broadcast to anyone waiting for this thread to be suspended.
	 */
	if (t->t_stop) {
		t->t_sp = _flush_and_tell();
...
_setrq(uthread_t *t)
{
	...
	if (t->t_stop) {
		 ...
		 * Its state should be manifested by _swtch().
		 */
		return;
	}
	if (ISPARKED(t)) {

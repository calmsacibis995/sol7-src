...
#pragma ident	"@(#)sparcsubr.s	1.10	97/09/25 SMI"
 ...
 * _flush_and_tell()
 *
 * flush all register windows to the stack, and return the fp
 */
	ENTRY(_flush_and_tell)
	flushw				! save all regs on stack
	retl
	mov	%sp, %o0
	SET_SIZE(_flush_and_tell)
...
UTR_thr_lwp_map:
	...
	nop
	call	_sc_setup		! else if bound call _sc_setup(-1)
	or	%g0, -1, %o0

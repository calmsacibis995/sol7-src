...
#pragma ident	"@(#)sigaction.c	1.104	98/01/15 SMI"
 ...
 * Global variables
 */
void  (*_tsiguhandler[NSIG])(int, siginfo_t *, ucontext_t *, sigset_t *);
...
sigacthandler(int sig, siginfo_t *sip, ucontext_t *uap)
{
	...
	char bufa [64];
	char *buf = bufa;
	void (*handler)(int, siginfo_t *, ucontext_t *, sigset_t *);
	int impossible2 = 0;
	sigset_t omask;
	sigset_t _lsigumask;
	...
	if (t->t_nosig > 0) {
		...
		ASSERT((!_sigismember(&t->t_hold, sig)) ||
...
typedef void (*handler_t)(int, siginfo_t *, ucontext_t *, sigset_t *);

...
#pragma	ident	"@(#)npd_clnt.c	1.4	97/10/09 SMI"	/* PAM 2.6 */
...
char		**key_type;		/* key length/algtype str buf; out */
{
	 ...
	 */
	tmpclnt = nis_make_rpchandle_gss_svc(master_srv, 0, NISPASSWD_PROG,
...
int		*err;		/* error code, returned */
{
	...
	case NPD_TRYAGAIN:
		...
		if (! __npd_ecb_crypt(ident, randval, &cryptbuf,
			sizeof (des_block), DES_DECRYPT, deskeys[0].c)) {
			*err = NPD_DECRYPTFAIL;
			return (NPD_FAILED);
		}
		return (res.status);

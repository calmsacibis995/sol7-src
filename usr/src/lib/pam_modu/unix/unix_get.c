...
#pragma	ident	"@(#)1.7	unix_get_authtokattr.c	97/05/18 SMI"	/* PAM 2.6 */
	...
	const char		**argv)
{
	...
	if ((get_attributes = (char **)calloc(PAM_MAX_NUM_ATTR,
	    sizeof (char *))) == NULL)
		return (PAM_BUF_ERR);
	...
	if (usrname == NULL || *usrname == NULL) {
		...
		while ((psp = _np_getspent()) != NULL) {
			...
			if (psp->sp_pwdp == NULL || strlen(psp->sp_pwdp) == 0) {
				snprintf(messages[0],
					sizeof (messages[0]),
					"%s NP  ", value);
	...
	if (shpwd->sp_pwdp == NULL || strlen(shpwd->sp_pwdp) == 0)
	...
	if (shpwd->sp_max != -1) {
		...
		if (shpwd->sp_lstchg) {
			...
			snprintf(value, sizeof (value), "%d", lstchg);
		} else {
			snprintf(value, sizeof (value), "%d", shpwd->sp_lstchg);
		}
		...
		if ((shpwd->sp_min >= 0) && (shpwd->sp_warn > 0)) {
			snprintf(value, sizeof (value), "%d", shpwd->sp_min);
			setup_attr(get_attributes, k++,
				"AUTHTOK_MINAGE=", value);
			snprintf(value, sizeof (value), "%d", shpwd->sp_max);
			setup_attr(get_attributes, k++,
				"AUTHTOK_MAXAGE=", value);
			snprintf(value, sizeof (value), "%d", shpwd->sp_warn);
			setup_attr(get_attributes, k++,

...
#pragma ident	"@(#)_utility.c	1.40	97/12/10 SMI"	/* SVr4.0 1.11 */
...
void
_t_blockallsignals(sigset_t *maskp)
{
	sigset_t	new;
...
void
_t_restoresigmask(sigset_t *maskp)
{
...
_t_create(int fd,  struct t_info *info, int api_semantics, int *t_capreq_failed)
{
	...
	if (api_semantics == TX_TLI_API)
		tsap->tsa_qlen = 0; /* not needed for TLI */
			 ...
			 */
			info->flags = 0;
			if (tsap->PROVIDER_flag & (SENDZERO|OLD_SENDZERO))

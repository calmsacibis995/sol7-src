...
#pragma ident	"@(#)tx.h	1.9	97/12/10 SMI"
...
int _t_snd_conn_req(struct _ti_user *tiptr, const struct t_call *call,
	struct strbuf *ctlbufp);
int _t_aligned_copy(struct strbuf *strbufp, int len, int init_offset,
	char *datap, t_scalar_t *rtn_offset);
...
int _t_is_ok(int fd, struct _ti_user *tiptr, t_scalar_t type);
int _t_look_locked(int fd, struct _ti_user *tiptr,
	int do_expinline_peek, int api_semantics);
 ...
 */
extern int _tx_accept(int fildes, int resfd, const struct t_call *call,
	int api_semantics);
extern char *_tx_alloc(int fildes, int struct_type, int fields,
	int api_semantics);
extern int _tx_bind(int fildes, const struct t_bind *req, struct t_bind *ret,
	int api_semantics);
extern int _tx_close(int fildes, int api_semantics);
extern int _tx_connect(int fildes, const struct t_call *sndcall,
	struct t_call *rcvcall, int api_semantics);
extern int _tx_error(const char *errmsg, int api_semantics);
...
extern int _tx_getname(int fildes, struct netbuf *name, int type,
	int api_semantics);
...
extern int _tx_open(const char *path, int oflag, struct t_info *info,
		int api_semantics);
extern int _tx_optmgmt(int fildes, const struct t_optmgmt *req,
...
extern int _tx_snd(int fildes, char *buf, unsigned nbytes, int flags,
		int api_semantics);
extern int _tx_snddis(int fildes, const struct t_call *call, int api_semantics);
extern int _tx_sndrel(int fildes, int api_semantics);
extern int _tx_sndudata(int fildes, const struct t_unitdata *unitdata,
			int api_semantics);
extern const char *_tx_strerror(int errnum, int api_semantics);
...
extern int _tx_unbind_locked(int fd, struct _ti_user *tiptr,
	struct strbuf *ctlbufp);
extern int _t_expinline_queued(int fd, int *expedited_queuedp);

...
#pragma ident	"@(#)t_optmgmt.c	1.22	97/08/12 SMI"
		/* SVr4.0 1.3.4.1	*/
...
)
{
	...
	if ((api_semantics == TX_XTI_API) && (optlen > 0)) {
		/*
	...
	if (api_semantics == TX_TLI_API || ret->opt.maxlen > 0) {
	 ...
	 * to O_T_OPTMGMT_REQ, the Internet protocols in Solaris 2.X return
	 ...
	 *
	 * Note: Even for T_OPTMGMT_ACK is response to O_T_OPTMGMT_REQ,

...
#pragma ident	"@(#)t_listen.c	1.21	97/08/12 SMI"	/* SVr4.0 1.5 */
...
_tx_listen(int fd, struct t_call *call, int api_semantics)
{
	...
	case T_CONN_IND:
		...
		if (api_semantics == TX_TLI_API || call->addr.maxlen > 0) {
		...
		if (api_semantics == TX_TLI_API || call->opt.maxlen > 0) {
		...
		if (api_semantics == TX_TLI_API || call->udata.maxlen > 0) {

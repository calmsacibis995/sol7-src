...
#ident	"@(#)mapped_cache.cc	1.23	97/12/12 SMI"
...
NisMappedCache::createCache()
{
	...
	if (status == 0) {
		if (fd != -1)
			(void) close(fd);
		/* don't leave broken cache files around */
		(void) unlink(CACHE_FILE);
		(void) unlink(PRIVATE_CACHE_FILE);
	}
...
NisMappedCache::updatePublicCache()
{
	...
	if (fd == -1) {
		...
		pub = (CacheHeader *)mmap(0, pubsize,
				PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0);
		if (pub == (CacheHeader *)-1) {
			syslog(LOG_ERR, "updatePublicCache: mmap(\"%s\"): %m",
					CACHE_FILE);
		}
		(void) close(fd);
	}
	...
	if (pub != (CacheHeader *)-1) {
		pub->valid = 0;
		if (msync((caddr_t)pub, pubsize, MS_SYNC) == -1) {
			syslog(LOG_ERR, "updatePublicCache: msync(\"%s\"): %m",
					CACHE_FILE);
		}
		if (munmap((caddr_t)pub, pubsize) < 0) {
			syslog(LOG_ERR, "updatePublicCache: munmap<old>: %m");
		}
	}

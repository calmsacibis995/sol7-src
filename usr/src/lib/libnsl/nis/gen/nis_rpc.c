...
#pragma ident	"@(#)nis_rpc.c	1.46	97/10/30 SMI"
...
static
CLIENT *
nis_handle(nis_bound_directory *binding, int n, u_int flags, int create)
{
	int fd;
	...
	for (cur = srv_listhead; cur; cur = next_srv) {
		...
		if (strcmp(ep->uaddr, cur->rpcb_uaddr) == 0) {
			 ...
			 *  handle.
			 */
			if (strcmp(bep->uaddr, cur->serv_uaddr) != 0) {
				if (__nis_debug_bind) {
	...
	if (cur) {
		/*
		 * We have a cached handle.  Prepare to return
		 * it and move the binding to the head of the list.
		 */
		if (srv_listhead != cur) {
			remove_server(cur);
	...
	/* Free any servers we found to be invalid */
	prev_srv = 0;
	next_srv = 0;
	for (scan = srv_tobefree; scan; scan = next_srv) {
		next_srv = scan->next;
		if (scan->ref_cnt == 0) {
			if (prev_srv == 0)
			...
			scan->flags = SRV_IS_FREE;
			scan->next = srv_isfree;
			srv_isfree = scan;
		} else {
			prev_srv = scan;
		}
...
new_address:
	...
	cur = srv_isfree;
	ASSERT(cur->flags == SRV_IS_FREE);
	ASSERT(cur->mach_name == NULL);
	ASSERT(cur->rpcb_uaddr == NULL);
	ASSERT(cur->clnt == NULL);
...
CLIENT *
nis_client_handle(nis_bound_directory *binding, int n, u_int flags)
{
	return (nis_handle(binding, n, flags, 1));
}

CLIENT *
nis_cached_handle(nis_bound_directory *binding, int n, u_int flags)
{

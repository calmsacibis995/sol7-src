...
#pragma ident	"@(#)	nis_names.c	1.18	97/10/30	SMI"
#include <stdlib.h>
#include <string.h>
	...
	int		rmax)		/* max array value	*/
{
	 ...
	 *		--------------	--------------------	------
	 *	1)	org_dir.mpk17	mpk17.eng.sun.com.	(continue)
	 *		mpk17			"		(done)
	 *		    (name is a substring of domainname, so
	 *		    guess is org_dir.mpk17.eng.sun.com.)
	 *
	 *	2)	x.mpk17.eng	mpk17.eng.sun.com.	(continue)
	 *		mpk17.eng		"		(done)
	 *		    (guess is x.mpk17.eng.sun.com.)
	 *
	 *	3)	hosts		mpk17.eng.sun.com.	(continue)
	 *					"		(done)
	...
	if (!(mkr = strstr(dom, name))) {
		...
		while (tmpnam < (name + strlen(name))) {
			 ...
			 *
			 *	buf    =   org_dir
			 *	tmpnam =   mpk17 ('org_dir' stripped off to buf)
			 *	tmpdom =   mpk17.eng.sun.com.
			 *			^
			 *			mkr
			 *
			 *	We check to make sure that 'mkr' points to a
			 *	'.', otherwise, the match isn't correct, i.e.
			 *	"mpk17" is not a label of "mpk17500" though
		 ...
		 *	---------------	---------------------	------
		 *	mpk17.eng.sun	mpk17.eng.sun.com.	(continue)
		 *			com.			(done)
		 *	    (guess is mpk17.eng.sun.com.)
		 */
    ...
    nis_name *result, int rmax)
{
	...
	for (i = 0, cur = 0; (i < comps) && (cur < rmax); i++) {
		...
		/* otherwise it's something like org_dir.$ or groups_dir.$ */
			...
			if (*(list[i] + (len - 1)) == '$') {
				*(list[i] + (len - 1)) = '\0';
				(void) sprintf(buf, "%s.%s%s", name, list[i],
				    local);
			} else
				(void) sprintf(buf, "%s.%s", name, list[i]);
...
int	append = 0;
	...
	/* services, sendmailvars */
	case 's':
		p1 = p1 + 2;
		switch (*p1) {
		case 'r':
			p1++;
			if (strcmp(p1, "vices") == 0)
				append = 1;
			break;

		case 'n':
			p1++;
			if (strcmp(p1, "dmailvars") == 0)
				append = 1;
			break;
		}
...
nis_getnames(nis_name name)
{
	...
	if (name[strlen(name)-1] != '.') {
		...
		i = parse_path(buf, path, local, result, 128);

		/* check case were name is "near" the root. */
		if (i == 0) {
			(void) sprintf(buf, "%s.%s", buf, local);

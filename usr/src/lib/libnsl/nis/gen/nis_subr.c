...
#pragma ident	"@(#)nis_subr.c	1.61	98/01/16 SMI"
...
CLIENT * nis_make_rpchandle_uaddr(nis_server *,
		int, rpcprog_t, rpcvers_t, u_int, int, int, char *);
		...
		for (s = zattrs[i].zattr_ndx; *s != '\0'; s++) {
			if (*s == '=') {
				*s = '\0';
				s++;
				zattrs[i].ZVAL = s;
				zattrs[i].ZLEN = (u_int)strlen(s) + 1;
				break;
			} else if (*s == '"') {
				if (*(s+1) == '"') { /* escaped quote */
					s += 2;
...
have_addr:
	...
	clnt = clnt_tli_create(RPC_ANYFD, nc, addr, prog, ver, olen, ilen);
	if (clnt) {
...
nis_get_static_storage(
	...
	u_int			el,	/* Sizeof elements	 */
	u_int			nel)	/* Number of elements	 */
{
	u_int	sz;
	sigset_t	oset;

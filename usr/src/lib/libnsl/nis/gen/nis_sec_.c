...
#pragma ident	"@(#)nis_sec_mechs.c 1.2     97/12/18 SMI"
...
static void **
list_append_ent(void *ent, void **list, u_int cnt, void (*free_ent)())
{
	void **new_l;
...
__nis_get_mechanisms(bool_t qop_secserv)
{
	...
	if (sbuf.st_mtime > last) {
		...
		if (! (fptr = fopen(NIS_SEC_CF_PATHNAME, "r"))) {
...
mf_get_mechs()
{
	...
	u_int		ent_cnt = 0;		/* valid cf file entry count */
	static u_int	last = 0;		/* file last modified date */
	struct stat	sbuf;
	FILE 		*fptr;
	...
	if (sbuf.st_mtime > last) {
		...
		while (mp = get_mechfile_ent(fptr)) {
			ent_cnt++;
			if (! (tmechs = (mfent_t **)list_append_ent((void *)mp,
						(void **)tmechs,
						ent_cnt,
						(void (*)()) mf_free_ent)))
				return (NULL);

...
#pragma ident	"@(#)nislib.c	1.42	97/10/30 SMI"
...
u_int  __nis_force_hard_lookups = HARD_LOOKUP;
...
__nis_nss_lookup_policy(unsigned int in)
{
	...
	thr_sigblock(&oset);
	mutex_lock(&__nis_force_lookup_policy_lock);
	...
	mutex_unlock(&__nis_force_lookup_policy_lock);
...
static nis_result *nis_list_partial(nis_result *, ib_request *, u_int,
				int (*)(), void *);
...
nis_list_partial(
	nis_result	*res,
	ib_request	*req,
	u_int		flags,		/* Flags for the search */
	int		(*cback)(),	/* Callback function. */
	void		*cbdata)	/* Callback private data */
{
	nis_object	*obj;
...
nis_nameops(nis_name name, nis_object *obj, rpcproc_t func)
{
	...
	if (obj) {
		 ...
		 */
		oname = obj->zo_name;
		(void) strcpy(nname, nis_leaf_of(name));
		obj->zo_name = nname;
		odomain = obj->zo_domain;
		(void) strcpy(ndomain, nis_domain_of(name));
...
nis_ibops(ib_request *req, rpcproc_t func)
{
	...
	if (req->ibr_obj.ibr_obj_len) {
		...
		oname = obj->zo_name;
		(void) strcpy(nname, nis_leaf_of(req->ibr_name));
		obj->zo_name = nname;
		odomain = obj->zo_domain;
		(void) strcpy(ndomain, nis_domain_of(req->ibr_name));
		obj->zo_domain = ndomain;
		if (ndomain[strlen(ndomain)-1] != '.')
			(void) strcat(ndomain, ".");

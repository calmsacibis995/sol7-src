...
#pragma ident	"@(#)nis_misc_proc.c	1.15	98/01/12 SMI"
...
void
__nis_auth2princ(char *name, int flavor, caddr_t auth, int verbose)
{
...
{
#define	INC_SIZE 512
	int			addr_size = INC_SIZE;
	endpoint		*addr, *tmpaddr;
	nis_server		*hostinfo;
	int			num_ep = 0, i;
	struct netconfig	*nc;
	void			*nch;
	...
	char			hostname[MAXNETNAMELEN+1];
	char			pkey[HEXKEYBYTES+1];
	mechanism_t		**mechlist;

	addr = (endpoint *) calloc(addr_size, sizeof (endpoint));
	if (addr == NULL) {
		if (errcode)
	...
	if (host) {
		...
		if (gethostname(hostname, MAXNETNAMELEN) != 0) {
			if (errcode)
	...
	if ((hostinfo = (nis_server *) malloc(sizeof (nis_server))) == NULL) {
		if (errcode)
			*errcode = NIS_NOMEMORY;
		return (NULL);
	}
	...
	if (addpubkey) {
		...
		if (mechlist = __nis_get_mechanisms(0)) {
			...
			hostinfo->pkey.n_len = (u_short) keylistsize;
...
__nis_dhext_extract_pkey(netobj *no, keylen_t keylen, algtype_t algtype)
{
	...
	while (keyent < (extdhkey_t *)(no->n_bytes + no->n_len)) {
		...
		keyoffset = (char *)keyent + (sizeof (u_short) * 2) +
			binpadlen;

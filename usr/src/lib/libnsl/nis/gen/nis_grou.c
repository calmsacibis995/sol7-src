...
#pragma ident	"@(#)nis_groups.c	1.22	97/10/30 SMI"
...
typedef nis_result *(*nis_lookup_func)(nis_name, u_int);
static nis_object *get_group(nis_name, nis_name, nis_lookup_func, nis_error *);
static enum ismem do_ismember_2(nis_name, nis_name, nis_name, nis_lookup_func);
	...
	nis_lookup_func	lookup)
{
	...
	for (nl = varp->recursive;  nl != 0;  nl = nl->prochain) {
		switch (do_ismember_2(princp, nl->nom, refname, lookup)) {
		    case ISMEM_YES:
			ASSERT(RW_READ_HELD(&g_cache_lock));
			return (ISMEM_YES);
		    case ISMEM_NO:
...
static g_entry *
cached_group_entry(nis_name group, nis_name refname, nis_lookup_func lookup)
{
	...
	if (ge != 0) {
		...
		gc->nmisses++;
		obj = get_group(group, refname, lookup, &dummy);
		if (obj == 0) {
			gc->ncalls++;
			rw_unlock(&g_cache_lock);
			rw_rdlock(&g_cache_lock);
...
static thread_key_t visit_log_key;
struct visit_log {
	g_entry		 *ge_id;
...
{
	struct visit_log *visit_list;
	struct visit_log *v;

	v = (struct visit_log *)calloc(1, sizeof (struct visit_log));
	v->ge_id = ge;
	if (_thr_main()) {
		v->next = visit_list_main;
		visit_list_main = v;
	} else {
		visit_list = (struct visit_log *)
			thr_get_storage(&visit_log_key, 0, free);
		v->next = visit_list;
		(void) thr_setspecific(visit_log_key, (void *) v);
	}
}
...
unmark_visit(g_entry *ge)
{
	...
	if (_thr_main()) {
		v = visit_list_main;
		if ((v == NULL) || (v->ge_id != ge)) {
			/* umark must be in LIFO order */
		...
		visit_list = v->next;
		(void) thr_setspecific(visit_log_key, (void *) visit_list);
	}
	free(v);
}
	...
	nis_lookup_func	lookup)
{
	...
	/* always enter with a READ LOCK and exit with one too */
	ASSERT(RW_READ_HELD(&g_cache_lock));
	ge = cached_group_entry(group, refname, lookup);
	if (ge == 0) {
	...
	switch (lookup_recursive(&ge->exclude, princp, group, lookup)) {
	    case ISMEM_YES:
		answer = ISMEM_NO;
		break;
	    case ISMEM_NO:
	    ...
	    default:
		if (!easy_include &&
		    (lookup_recursive(&ge->include, princp, group, lookup) ==
			ISMEM_NO)) {
			answer = ISMEM_NO;
		} else {
			answer = ISMEM_DUNNO;
	...
	nis_name	group)	/* NIS group name 		*/
{
	...
	ret = (do_ismember_2(princp, group, 0, nis_lookup) == ISMEM_YES);
	rw_unlock(&g_cache_lock);
	return (ret);
}

/*
 * __do_ismember(princp, group, lookup)
 *
	...
	nis_lookup_func	lookup)
{
	...
	rw_rdlock(&g_cache_lock);
	/* Err on the side of security:  in case of doubt, return FALSE */
	ret = (do_ismember_2(princp, group, 0, lookup) == ISMEM_YES);
	rw_unlock(&g_cache_lock);
	return (ret);
}
	...
	nis_name	group) /* Name of the group to print */
{
	...
	rw_rdlock(&g_cache_lock);
	ge = cached_group_entry(group, (nis_name)0, nis_lookup);
	if (ge == 0) {
...
"nislib:get_group() group object \"%s\" does not exist.",
		...
		*stat = res->status;
		...
		if (refname)
			syslog(LOG_ERR,
"nislib:get_group() object \"%s\", referenced by \"%s\", is not reachable.",
			    name, refname);
		else
			syslog(LOG_ERR,
"nislib:get_group() object \"%s\" is not reachable.",
			    name);

		nis_lerror(res->status, "nislib:get_group:");
		*stat = res->status;
		nis_freeresult(res);
		return (0);
	}

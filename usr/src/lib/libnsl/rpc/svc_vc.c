#ident	"@(#)svc_vc.c	1.49	98/02/09 SMI"
...
extern void 	*malloc(), free(), *calloc();
		...
		if (option && option_ret) {
			...
			*p_optval = SO_KEEPALIVE;
			optreq.opt.maxlen = optreq.opt.len =
				sizeof (struct opthdr) + sizeof (long);
...
__svc_nisplus_enable_timestamps()
{
	...
	mutex_lock(&timestamp_lock);
	if (!timestamps) {
		timestamps = (long *)calloc(dtbsize, sizeof (long));
	}
	mutex_unlock(&timestamp_lock);
}

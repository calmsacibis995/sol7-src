...
int __rpc_max_nofile;	/* to allocate the global data structures */
...
add_pollfd(int fd)
{
	...
	if (fd < RPC_MAX_NOFILE) {
		if (fd >= svc_max_pollfd)
			svc_max_pollfd = fd + 1;
		if (svc_max_pollfd > svc_pollfd_allocd) {
			register int i = svc_pollfd_allocd;
			register pollfd_t *tmp;
			do {
...
clear_pollfd(int fd)
{
	...
	if (fd < RPC_MAX_NOFILE && POLLFD_ISSET(fd, svc_pollfd)) {
	...
	const SVCXPRT *xprt;
{
	...
	register int dtbsize = RPC_MAX_NOFILE;
...
/* VARIABLES PROTECTED BY svc_fd_lock: svc_xports, svc_fdset */
	...
	if (svc_xports == NULL) {
		svc_xports = (SVCXPRT **)
			mem_alloc((dtbsize + 1) * sizeof (SVCXPRT *));
			memset(svc_xports, 0,
					(dtbsize + 1) * sizeof (SVCXPRT *));
#ifdef CALLBACK
		/*
	...
	if (svc_xdrs == NULL) {
		svc_xdrs = (XDR **) mem_alloc((dtbsize + 1) *
							sizeof (XDR *));
		memset(svc_xdrs, 0, (dtbsize + 1) * sizeof (XDR *));
	}
	...
	const SVCXPRT *xprt;
{
	...
	rw_wrlock(&svc_fd_lock);
	if ((fd < RPC_MAX_NOFILE) && (svc_xports[fd] == xprt)) {
 ...
 *		u_int *resp_bufsz,truct dupcache *dc, uint32_t drxid,
 * 		uint32_t drhash)
 ...
 * 	u_int resp_bufsz, int status, struct dupcache *dc, uint32_t drxid,
 * 	uint32_t drhash)

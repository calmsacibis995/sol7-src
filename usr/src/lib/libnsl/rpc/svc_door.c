...
#ident	"@(#)svc_door.c	1.8	97/04/29 SMI"
...
make_tmp_dir()
{
	...
	if (stat(RPC_DOOR_DIR, &statbuf) < 0) {
		(void) mkdir(RPC_DOOR_DIR, (mode_t)01777);
...
/* ARGSUSED */
static void
door_server(cookie, argp, arg_size, dp, n_did)
	void			*cookie;
	void			*argp;
	size_t			arg_size;
	door_desc_t		*dp;		/* not used */
	size_t			n_did;		/* not used */
{

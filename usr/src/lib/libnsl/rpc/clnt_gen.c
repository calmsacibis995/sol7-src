...
#ident	"@(#)clnt_generic.c	1.35	97/08/25 SMI"
...
extern __td_setnodelay();
extern bool_t __rpc_is_local_host();
extern bool_t __rpc_try_doors();
 ...
 */
CLIENT *
clnt_create_vers_timed(hostname, prog, vers_out, vers_low, vers_high, nettype,
				tp)
	const char *hostname;
	rpcprog_t prog;
	rpcvers_t *vers_out;
 ...
 */
CLIENT *
clnt_create_timed(hostname, prog, vers, netclass, tp)
	const char *hostname;			/* server name */
	rpcprog_t prog;				/* program number */
	rpcvers_t vers;				/* version number */
	const char *netclass;			/* net type */
	const struct timeval *tp;		/* timeout pointer */
{
...
clnt_tp_create(hostname, prog, vers, nconf)
	...
	rpcprog_t prog;				/* program number */
	rpcvers_t vers;				/* version number */
	register const struct netconfig *nconf;	/* net config struct */
{
 ...
 */
CLIENT *
clnt_tp_create_timed(hostname, prog, vers, nconf, tp)
	const char *hostname;			/* server name */
	rpcprog_t prog;				/* program number */
	rpcvers_t vers;				/* version number */
	register const struct netconfig *nconf;	/* net config struct */
	const struct timeval *tp;		/* timeout pointer */
{
	...
	extern struct netbuf *__rpcb_findaddr_timed();
	 ...
	 */
	if ((svcaddr = __rpcb_findaddr_timed(prog, vers, nconf, hostname,
		&cl, tp)) == NULL) {

...
#ident	"@(#)netdir.c	1.25	97/08/01 SMI"	/* SVr4.0 1.	*/
...
extern int *_t_tsdalloc(thread_key_t *, size_t);
...
buf()
{
	...
	if (nderrbuf_key == 0) {
		mutex_lock(&tsd_lock);
		if (nderrbuf_key == 0)
			(void) thr_keycreate(&nderrbuf_key, free);
	...
	(void) thr_getspecific(nderrbuf_key, (void *)&__nderrbuf);
	if (!__nderrbuf) {

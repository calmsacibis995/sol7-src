...
#ident	"@(#)netdir_inet.c	1.20	98/02/11	SMI"
	...
	switch (args->op_t) {
		...
		if (args->arg.nd_hs->h_host == 0) {
			...
			haddrlist = dotnamelist;
				...
				HOST_BROADCAST) == 0)) {
			/*
			 * Now that inaddrs and baddrlist are dynamically
			 * allocated, care must be taken in freeing up the
			 * memory at each 'return()' point.
			 *
			 * Early return protection (using FREE_return()) is
			 * needed only in NETDIR_BY cases because dynamic
			 * allocation is used when args->op_t == NETDIR_BY.
			 *
...
#define	FREE_return(ret) \
			...
			if (bnets == 0)
				return (ND_NOHOST);
			baddrlist = (char **) malloc((bnets+1)*sizeof (char *));
			if (baddrlist == NULL)
		...
		if (haddrlist && servp) {
			 ...
			 */
			ret = hent2ndaddr(haddrlist, servp, res->nd_alist);
			FREE_return(ret)
		}
		break;
	}
	...
	switch (args->op_t) {
		...
		if (haddrlist == 0) {
			...
			if (!he) {
				NSS_XbyY_FREE(&ndbuf4switch);
				return (__herrno2netdir(h_errnop));
			}
			/*
			 * Convert h_addr_list into ordered nd_addrlist.
			 * malloc's will be done, freed using netdir_free.
	...
	union	nss_netdirbyaddr_out *res;
{
	...
	switch (args->op_t) {
		 ...
		 */
		if (*(uint32_t *)(args->arg.nss.host.addr) == INADDR_LOOPBACK) {
			int	ret;
	...
	struct	sockaddr_in	**res_salist;
{
	...
	if (inplace) {
		...
		(void) memcpy(haddrlist, addrs, num * sizeof (addrs[0]));
		free(addrs);
	} else {
		struct	sockaddr_in *sa;
		...
		*res_salist = sa;
		if (num == 1) {
			(void) memcpy((char *)&(sa->sin_addr), *haddrlist,
			    sizeof (sa->sin_addr));
			return (ND_OK);
		}
		localinfo = get_local_info();
		...
		for (localif = 1; localif >= 0; localif--) {
			...
			for (t = haddrlist; *t; t++) {
				...
				if ((localif && !islocal(localinfo, ina)) ||
					...
					continue;
				(void) memcpy((char *)&(sa->sin_addr), &ina,
				    sizeof (sa->sin_addr));
				sa++;
			}
		}

...
#ident	"@(#)gethostent.c	1.16	97/08/01	SMI"
 ...
 */
	...
	(void) memcpy(addrbuf, addrstart, (size_t)addrlen);
	addrbuf[addrlen] = '\0';
	if ((first_addr.s_addr = inet_addr(addrbuf)) == (uint32_t)-1) {
	...
	host->h_aliases = _nss_netdb_aliases(p, (int)(lenstr - (p - instr)),
	    buffer, (int)(((char *)addrvec) - buffer));
	if (host->h_aliases == 0) {

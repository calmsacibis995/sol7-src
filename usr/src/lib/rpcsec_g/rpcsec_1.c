...
#ident	"@(#)rpcsec_gss_misc.c	1.16	98/02/17 SMI"
	...
	caddr_t			xdr_ptr;
{
	...
	if ((buf = (char *)malloc(bufsiz)) == NULL) {
		fprintf(stderr, gettext("malloc failed in"
			"__rpc_gss_wrap_data\n"));
		return (FALSE);
	}
	...
	int		max_tp_unit_len;
{
	...
	if (maj_stat == GSS_S_COMPLETE)
		return ((int)(max_input_size));
	else {
		fprintf(stderr, gettext("gss_wrap_size_limit failed in"

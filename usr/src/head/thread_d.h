...
#pragma ident	"@(#)thread_db.h	1.24	98/01/06 SMI"
...
/*
 * The structure containing per-thread event data; allocated within
 * libthread on demand.
 */
typedef struct {
...
/*
 * De-allocate a libthread_db process handle, releasing all
 * related resources.
 */
td_err_e
...
/*
 * Map a uthread_t structure address to a thread handle.
 */
td_err_e
td_ta_map_addr2thr(const td_thragent_t *, psaddr_t, td_thrhandle_t *);

/*
 * Map the address of a synchronization object to a sync. object
 * handle.
 */
td_err_e
...
/*
 * Iterate over a process's synchronization objects.
 */
td_err_e
...
/*
 * Enable event reporting for a thread.
 */
td_err_e
td_thr_event_enable(const td_thrhandle_t *, int);

/*
 * Enable a specific event type for a thread.
 */
td_err_e
td_thr_set_event(const td_thrhandle_t *, td_thr_events_t *);

/*
 * Disable a specific event type for a thread.
 */
td_err_e
